context('List View', () => {
	before(() => {
		...
		return cy.window().its('vmraid').then(vmraid => {
			return vmraid.xcall("vmraid.tests.ui_test_helpers.setup_workflow");
		});
	});
	...
	it('Keep checkbox checked after Refresh', () => {
		...
		cy.intercept('/api/method/vmraid.desk.reportview.get').as('list-refresh');
	...
	it('enables "Actions" button', () => {
		...
		cy.get('.dropdown-menu li:visible .dropdown-item').should('have.length', 9).each((el, index) => {
			...
			cy.intercept({
				method: 'POST',
				url: 'api/method/vmraid.model.workflow.bulk_workflow_approval'
			}).as('bulk-approval');
			cy.intercept({
				method: 'POST',
				url: 'api/method/vmraid.desk.reportview.get'
			}).as('real-time-update');
