context('Form Tour', () => {
	before(() => {
		...
		return cy.window().its('vmraid').then(vmraid => {
			return vmraid.call("vmraid.tests.ui_test_helpers.create_form_tour");
		});
	});
	...
	it('navigates a form tour', () => {
		...
		cy.get('.vmraid-driver').should('be.visible');
		cy.get('.vmraid-control[data-fieldname="first_name"]').as('first_name');
		cy.get('@first_name').should('have.class', 'driver-highlighted-element');
		cy.get('.vmraid-driver').findByRole('button', {name: 'Next'}).as('next_btn');
		...
		cy.get('.vmraid-control[data-fieldname="last_name"]').as('last_name');
		...
		// assert field is highlighted
		cy.get('.vmraid-control[data-fieldname="phone_nos"]').as('phone_nos');
		...
		cy.get('.grid-row-open .vmraid-control[data-fieldname="phone"]').as('phone');
		...
		cy.wait(500);
		cy.get('.vmraid-driver').findByRole('button', {name: 'Save'}).should('be.visible');
