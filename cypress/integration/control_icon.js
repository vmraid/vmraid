context('Control Icon', () => {
	...
	it('should set icon', () => {
		...
		cy.get('.vmraid-control[data-fieldname=icon]').findByRole('textbox').click();

		cy.get('.icon-picker .icon-wrapper[id=heart-active]').first().click();
		cy.get('.vmraid-control[data-fieldname=icon]').findByRole('textbox').should('have.value', 'heart-active');
		...
		cy.get('.vmraid-control[data-fieldname=icon]').findByRole('textbox').should('have.value', 'heart');
		cy.get('@dialog').then(dialog => {
			let value = dialog.get_value('icon');
