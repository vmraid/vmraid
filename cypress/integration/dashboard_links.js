...
context('Dashboard links', () => {
	before(() => {
		...
		return cy.window().its('vmraid').then(vmraid => {
			return vmraid.xcall("vmraid.tests.ui_test_helpers.update_child_table", {
	...
	it('check if child table is populated with linked field on creation from dashboard link', () => {
		...
		cy.get('.vmraid-control[data-fieldname="child_table"] .rows .data-row .col[data-fieldname="doctype_to_link"]')
