...
context('Grid Search', () => {
	before(() => {
		...
		return cy.window().its('vmraid').then(vmraid => {
			return vmraid.xcall("vmraid.tests.ui_test_helpers.insert_doctype_with_child_table_record", {
	...
	it('Test search row visibility', () => {
		cy.window().its('vmraid').then(vmraid => {
			vmraid.model.user_settings.save('Doctype With Child Table', 'GridView', {
		...
		cy.get('.vmraid-control[data-fieldname="child_table_1"]').as('table');
