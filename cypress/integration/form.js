context('Form', () => {
	before(() => {
		...
		return cy.window().its('vmraid').then(vmraid => {
			return vmraid.call("vmraid.tests.ui_test_helpers.create_contact_records");
		});
	});
	it('create a new form', () => {
		...
		cy.intercept({
			method: 'POST',
			url: 'api/method/vmraid.desk.form.save.savedocs'
		}).as('form_save');
	...
	it('validates behaviour of Data options validations in child table', () => {
		...
		cy.get('.vmraid-control[data-fieldname="email_ids"]').as('table');
