...
Cypress.Commands.add('create_records', doc => {
	return cy
		.call('vmraid.tests.ui_test_helpers.create_if_not_exists', {doc: JSON.stringify(doc)})
		.then(r => r.message);
});

Cypress.Commands.add('set_value', (doctype, name, obj) => {
	return cy.call('vmraid.client.set_value', {
		doctype,
		name,
		fieldname: obj
	});
...
Cypress.Commands.add('get_table_field', (tablefieldname, row_idx, fieldname, fieldtype = 'Data') => {
	let selector = `.vmraid-control[data-fieldname="${tablefieldname}"]`;
...
Cypress.Commands.add('dialog', opts => {
	return cy.window().then(win => {
		var d = new win.vmraid.ui.Dialog(opts);
		d.show();
		return d;
	});
});
...
Cypress.Commands.add('clear_filters', () => {
	...
	cy.intercept({
		method: 'POST',
		url: 'api/method/vmraid.model.utils.user_settings.save'
	}).as('filter-saved');
...
Cypress.Commands.add('select_listview_row_checkbox', (row_no) => {
	cy.get('.vmraid-list .select-like > .list-row-checkbox').eq(row_no).click();
});
