...
from vmraid.vmraidclient import AuthError, VMRaidClient, VMRaidException
from vmraid.utils.data import get_url


class TestVMRaidClient(unittest.TestCase):
	PASSWORD = vmraid.conf.admin_password or "admin"

	@classmethod
	def setUpClass(cls) -> None:
		site_url = get_url()
		try:
			VMRaidClient(site_url, "Administrator", cls.PASSWORD, verify=False)
		except AuthError:
	...
	def test_insert_many(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": ("in", ("Sing", "a", "song", "of", "sixpence"))})
		vmraid.db.commit()
	...
	def test_create_doc(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": "test_create"})
		vmraid.db.commit()
	...
	def test_get_doc(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": "get_this"})
		vmraid.db.commit()
	...
	def test_get_value(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": "get_value"})
		vmraid.db.commit()
	...
	def test_update_doc(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": ("in", ("Sing", "sing"))})
		vmraid.db.commit()
	...
	def test_update_child_doc(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Contact", {"first_name": "George", "last_name": "Steevens"})
		vmraid.db.delete("Contact", {"first_name": "William", "last_name": "Shakespeare"})
		vmraid.db.delete("Communication", {"reference_doctype": "Event"})
		vmraid.db.delete("Communication Link", {"link_doctype": "Contact"})
		vmraid.db.delete("Event", {"subject": "Sing a song of sixpence"})
		vmraid.db.delete("Event Participants", {"reference_doctype": "Contact"})
		vmraid.db.commit()
	...
	def test_delete_doc(self):
		server = VMRaidClient(get_url(), "Administrator", self.PASSWORD, verify=False)
		vmraid.db.delete("Note", {"title": "delete"})
		vmraid.db.commit()
	...
	def test_auth_via_api_key_secret(self):
		...
		header = {
			"Authorization": "Basic {}".format(
				base64.b64encode(vmraid.safe_encode("{}:{}".format(api_key, generated_secret))).decode()
			)
		}
		res = requests.post(get_url() + "/api/method/vmraid.auth.get_logged_user", headers=header)
		self.assertEqual(res.status_code, 200)
