...
class TestResourceAPI(unittest.TestCase):
	SITE_URL = get_site_url(vmraid.local.site)
	...
	def setUpClass(self):
		for _ in range(10):
			doc = vmraid.get_doc(
				{"doctype": "ToDo", "description": vmraid.mock("paragraph")}
			).insert()
	...
	def tearDownClass(self):
		for name in self.GENERATED_DOCUMENTS:
			vmraid.delete_doc_if_exists(self.DOCTYPE, name)
	...
	def sid(self):
		if not getattr(self, "_sid", None):
			self._sid = requests.post(
				...
				data={
					"usr": "Administrator",
					"pwd": vmraid.conf.admin_password or "admin",
				},
			).cookies.get("sid")
	...
	def post(self, path, data):
		return requests.post(
			f"{self.RESOURCE_URL}/{path}?sid={self.sid}", data=vmraid.as_json(data)
		)
	...
	def test_create_document(self):
		...
		data = {"description": vmraid.mock("paragraph")}
...
class TestMethodAPI(unittest.TestCase):
	METHOD_URL = f"{get_site_url(vmraid.local.site)}/api/method"
