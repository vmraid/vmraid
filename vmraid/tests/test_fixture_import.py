...
from vmraid.desk.form.save import savedocs
...
class TestFixtureImport(unittest.TestCase):
	def create_new_doctype(self, DocType: str) -> None:
		file = vmraid.get_app_path("vmraid", "custom", "fixtures", f"{DocType}.json")
	...
	def insert_dummy_data_and_export(self, DocType: str, dummy_name_list: List[str]) -> str:
		for name in dummy_name_list:
			doc = vmraid.get_doc({"doctype": DocType, "member_name": name})
			doc.insert()
	...
	def test_fixtures_import(self):
		self.assertFalse(vmraid.db.exists("DocType", "temp_doctype"))
		...
		vmraid.db.truncate("temp_doctype")
		...
		self.assertEqual(vmraid.db.count("temp_doctype"), len(dummy_name_list))

		data = vmraid.get_all("temp_doctype", "member_name")
		vmraid.db.truncate("temp_doctype")
	...
	def test_singles_fixtures_import(self):
		self.assertFalse(vmraid.db.exists("DocType", "temp_singles"))
		...
		singles_doctype = vmraid.qb.DocType("Singles")
		truncate_query = (
			vmraid.qb.from_(singles_doctype).delete().where(singles_doctype.doctype == "temp_singles")
		)
		truncate_query.run()
		...
		data = vmraid.db.get_single_value("temp_singles", "member_name")
		truncate_query.run()
