...
import unittest, vmraid, pyotp
from vmraid.auth import HTTPRequest
from vmraid.utils import cint
from vmraid.utils import set_request
from vmraid.auth import validate_ip_address, get_login_attempt_tracker
from vmraid.twofactor import (should_run_2fa, authenticate_for_2factor, get_cached_user_pass,
...
class TestTwoFactor(unittest.TestCase):
	...
	def setUp(self):
		...
		self.login_manager = vmraid.local.login_manager
	...
	def tearDown(self):
		vmraid.local.response['verification'] = None
		vmraid.local.response['tmp_id'] = None
		disable_2fa()
		vmraid.clear_cache(user=self.user)
		update_system_settings({
	...
	def test_authenticate_for_2factor(self):
		'''Verification obj and tmp_id should be set in vmraid.local.'''
		authenticate_for_2factor(self.user)
		verification_obj = vmraid.local.response['verification']
		tmp_id = vmraid.local.response['tmp_id']
		self.assertTrue(verification_obj)
		self.assertTrue(tmp_id)
		for k in ['_usr','_pwd','_otp_secret']:
			self.assertTrue(vmraid.cache().get('{0}{1}'.format(tmp_id,k)),
	...
	def test_two_factor_is_enabled(self):
		...
		user.restrict_ip = vmraid.local.request_ip
		user.save()
		self.assertTrue(should_run_2fa(self.user))
	...
	def test_get_otpsecret_for_user(self):
		...
		self.assertTrue(vmraid.db.get_default(self.user + '_otpsecret'))

	def test_confirm_otp_token(self):
		'''Ensure otp is confirmed'''
		vmraid.flags.otp_expiry = 2
		authenticate_for_2factor(self.user)
		tmp_id = vmraid.local.response['tmp_id']
		otp = 'wrongotp'
		with self.assertRaises(vmraid.AuthenticationError):
		...
		vmraid.flags.otp_expiry = None
		if vmraid.flags.tests_verbose:
	...
	def test_render_string_template(self):
		...
		args = {'issuer_name':'VMRaid Technologies'}
		_str = 'Verification Code from {{issuer_name}}'
		_str = vmraid.render_template(_str,args)
		self.assertEqual(_str,'Verification Code from VMRaid Technologies')
	...
	def test_otp_attempt_tracker(self):
		...
		with self.assertRaises(vmraid.SecurityException):
			confirm_otp_token(self.login_manager,otp=otp,tmp_id=tmp_id)
...
def create_http_request():
	...
	enable_2fa()
	vmraid.form_dict['usr'] = 'test@example.com'
	vmraid.form_dict['pwd'] = 'Eastern_43A1W'
	vmraid.local.form_dict['cmd'] = 'login'
...
def toggle_2fa_all_role(state=None):
	...
	all_role = vmraid.get_doc('Role','All')
	if state == None:
