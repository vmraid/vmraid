...
import vmraid
from vmraid.auth import HTTPRequest, get_login_attempt_tracker, validate_ip_address
...
from vmraid.utils import cint, set_request
...
class TestTwoFactor(unittest.TestCase):
	...
	def test_authenticate_for_2factor(self):
		...
		for k in ["_usr", "_pwd", "_otp_secret"]:
			self.assertTrue(vmraid.cache().get("{0}{1}".format(tmp_id, k)), "{} not available".format(k))
