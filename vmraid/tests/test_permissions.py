...
from vmraid.permissions import (add_user_permission, remove_user_permission,
	clear_user_permissions_for_doctype, get_doc_permissions, add_permission, update_permission_property)
from vmraid.core.page.permission_manager.permission_manager import update, reset
...
class TestPermissions(unittest.TestCase):
	...
	def test_not_allowed_to_set_user_permissions(self):
		...
		self.assertRaises(vmraid.PermissionError, add_user_permission,
	...
	def test_not_allowed_to_remove_user_permissions(self):
		...
		self.assertRaises(vmraid.PermissionError, remove_user_permission,
			"Blog Post", "-test-blog-post", "test2@example.com")
	...
	def test_clear_user_permissions(self):
		...
		blog_post_user_permission_count = vmraid.db.count('User Permission', filters={
		...
		blog_category_user_permission_count = vmraid.db.count('User Permission', filters={
			'user': 'test2@example.com',
			'allow': 'Blog Category'
		})
