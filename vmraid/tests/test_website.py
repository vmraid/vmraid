...
from vmraid.website.page_renderers.static_page import StaticPage
from vmraid.website.serve import get_response, get_response_content
from vmraid.website.utils import build_response, clear_website_cache, get_home_page
...
class TestWebsite(unittest.TestCase):
	...
	def test_home_page(self):
		...
		vmraid.db.set_value("Role", "home-page-test", "home_page", "home-page-test")
		vmraid.set_user("test-user-for-home-page@example.com")
		self.assertEqual(get_home_page(), "home-page-test")

		vmraid.set_user("Administrator")
		vmraid.db.set_value("Role", "home-page-test", "home_page", "")
		...
		vmraid.db.set_value("Portal Settings", None, "default_portal_home", "")
		clear_website_cache()

		# home page via website settings
		vmraid.db.set_value("Website Settings", None, "home_page", "contact")
		self.assertEqual(get_home_page(), "contact")

		vmraid.db.set_value("Website Settings", None, "home_page", None)
		clear_website_cache()
		...
		clear_website_cache()
		set_home_page_hook(
			"get_website_user_home_page", "vmraid.www._test._test_home_page.get_website_user_home_page"
		)
	...
	def test_custom_page_renderer(self):
		...
		vmraid.hooks.page_renderer = ["vmraid.tests.test_website.CustomPageRenderer"]
		vmraid.cache().delete_key("app_hooks")
		...
		delattr(vmraid.hooks, "page_renderer")
		vmraid.cache().delete_key("app_hooks")

	def test_printview_page(self):
		vmraid.db.value_cache[("DocType", "Language", "name")] = (("Language",),)
	...
	def test_json_sidebar_data(self):
		vmraid.flags.look_for_sidebar = False
		content = get_response_content("/_test/_test_folder/_test_page")
		self.assertNotIn("Test Sidebar", content)
		clear_website_cache()
		vmraid.flags.look_for_sidebar = True
		content = get_response_content("/_test/_test_folder/_test_page")
	...
	def test_caching(self):
		# to enable caching
		vmraid.flags.force_website_cache = True
	...
	def test_safe_render(self):
		...
		self.assertNotIn("vmraid.exceptions.ValidationError: Illegal template", content)
