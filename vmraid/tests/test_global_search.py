...
from vmraid.utils import global_search, now_datetime
...
class TestGlobalSearch(unittest.TestCase):
	def setUp(self):
		...
		self.assertTrue("__global_search" in vmraid.db.get_tables())
		doctype = "Event"
		global_search.reset()
	...
	def tearDown(self):
		vmraid.db.delete("Property Setter", {"doc_type": "Event"})
	...
	def test_update_doc(self):
		...
		event = vmraid.get_doc("Event", vmraid.get_all("Event")[0].name)
	...
	def test_delete_doc(self):
		self.insert_test_events()
		event_name = vmraid.get_all("Event")[0].name
		event = vmraid.get_doc("Event", event_name)
		...
		vmraid.delete_doc("Event", event_name)
		global_search.sync_global_search()
		vmraid.db.commit()
	...
	def test_insert_child_table(self):
		...
		for text in phrases:
			doc = vmraid.get_doc({"doctype": "Event", "subject": text, "starts_on": now_datetime()})
			doc.insert()
