...
class TestDB(unittest.TestCase):
	def test_get_value(self):
		...
		self.assertEqual(vmraid.db.sql("""SELECT name FROM `tabUser` WHERE name > 's' ORDER BY MODIFIED DESC""")[0][0],
			vmraid.db.get_value("User", {"name": [">", "s"]}))

		self.assertEqual(vmraid.db.sql("""SELECT name FROM `tabUser` WHERE name >= 't' ORDER BY MODIFIED DESC""")[0][0],
			vmraid.db.get_value("User", {"name": [">=", "t"]}))

	def test_set_value(self):
		todo1 = vmraid.get_doc(dict(doctype='ToDo', description = 'test_set_value 1')).insert()
		todo2 = vmraid.get_doc(dict(doctype='ToDo', description = 'test_set_value 2')).insert()

		vmraid.db.set_value('ToDo', todo1.name, 'description', 'test_set_value change 1')
		self.assertEqual(vmraid.db.get_value('ToDo', todo1.name, 'description'), 'test_set_value change 1')

		# multiple set-value
		vmraid.db.set_value('ToDo', dict(description=('like', '%test_set_value%')),
			'description', 'change 2')

		self.assertEqual(vmraid.db.get_value('ToDo', todo1.name, 'description'), 'change 2')
		self.assertEqual(vmraid.db.get_value('ToDo', todo2.name, 'description'), 'change 2')
	...
	def test_get_single_value(self):
		...
		limit = vmraid.db.get_single_value('System Settings', 'backup_limit')
	...
	def test_db_keywords_as_fields(self):
		...
		# Testing read
		self.assertEqual(list(vmraid.get_all("ToDo", fields=[random_field], limit=1)[0])[0], random_field)
		self.assertEqual(list(vmraid.get_all("ToDo", fields=["`{0}` as total".format(random_field)], limit=1)[0])[0], "total")
		...
		)[0], "count" if vmraid.conf.db_type == "postgres" else "count(`{0}`)".format(random_field))
