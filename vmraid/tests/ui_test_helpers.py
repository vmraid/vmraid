...
def create_if_not_exists(doc):
	...
	if not vmraid.local.dev_server:
		vmraid.throw(_("This method can only be accessed in development"), vmraid.PermissionError)
	...
	doc = vmraid.get_doc(
		{
			...
			"communication_date": vmraid.utils.now_datetime(),
		}
	)
	doc.insert()
	return doc
...
def create_multiple_todo_records():
	values = []
	if vmraid.db.get_all("ToDo", {"description": "Multiple ToDo 1"}):
		return

	for index in range(1, 1002):
		values.append(("100{}".format(index), "Multiple ToDo {}".format(index)))

	vmraid.db.bulk_insert("ToDo", fields=["name", "description"], values=set(values))


def insert_contact(first_name, phone_number):
	doc = vmraid.get_doc({"doctype": "Contact", "first_name": first_name})
...
def create_form_tour():
	if vmraid.db.exists("Form Tour", {"name": "Test Form Tour"}):
		return
...
def create_web_page(title, route, single_thread):
	web_page = vmraid.db.exists("Web Page", {"route": route})
	if web_page:
		return web_page
	web_page = vmraid.get_doc(
	...
	web_page.append(
		"page_blocks",
		{
			...
			"web_template_values": vmraid.as_json(
				{
					"title": "Discussions",
...
def update_webform_to_multistep():
	if not vmraid.db.exists("Web Form", "update-profile-duplicate"):
		doc = vmraid.get_doc("Web Form", "edit-profile")
		_doc = vmraid.copy_doc(doc)
...
def insert_doctype_with_child_table_record(name):
	if vmraid.db.get_all(name, {"title": "Test Grid Search"}):
		return
