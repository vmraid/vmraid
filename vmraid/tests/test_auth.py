...
from vmraid.vmraidclient import VMRaidClient, AuthError
...
class TestAuth(unittest.TestCase):
	...
	def tearDown(self):
		vmraid.delete_doc('User', self.test_user_email, force=True)
	...
	def test_allow_login_using_mobile(self):
		...
		VMRaidClient(vmraid.get_site_config().host_name, self.test_user_mobile, self.test_user_password)
		VMRaidClient(vmraid.get_site_config().host_name, self.test_user_email, self.test_user_password)

		# login by username should fail
		with self.assertRaises(AuthError):
			VMRaidClient(vmraid.get_site_config().host_name, self.test_user_name, self.test_user_password)
