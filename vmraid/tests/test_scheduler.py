...
class TestScheduler(TestCase):
	...
	def test_enqueue_jobs(self):
		...
		self.assertTrue('vmraid.email.doctype.auto_email_report.auto_email_report.send_monthly', vmraid.flags.enqueued_jobs)
	...
	def test_queue_peeking(self):
		...
		vmraid.db.sql('DELETE FROM `tabScheduled Job Log` WHERE `scheduled_job_type`=%s', job.name)
	...
	def test_once_a_day_for_dormant(self):
		...
		self.assertTrue(schedule_jobs_based_on_activity(check_time = add_days(vmraid.db.get_last_created('Activity Log'), 5)))
		...
		self.assertFalse(schedule_jobs_based_on_activity(check_time = add_days(vmraid.db.get_last_created('Activity Log'), 5)))

		# one more day has passed
		self.assertTrue(schedule_jobs_based_on_activity(check_time = add_days(vmraid.db.get_last_created('Activity Log'), 6)))
