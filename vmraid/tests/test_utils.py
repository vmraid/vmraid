...
import vmraid
from vmraid.installer import parse_app_name
...
from vmraid.utils.diff import _get_value_from_version, get_version_diff, version_query
...
class TestDataManipulation(unittest.TestCase):
	def test_scrub_urls(self):
		...
		self.assertTrue('<img src="{0}/assets/vmraid/test.jpg">'.format(url) in html)
		self.assertTrue(
			"style=\"background-image: url('{0}/assets/vmraid/bg.jpg') !important\"".format(url) in html
		)
...
class TestValidationUtils(unittest.TestCase):
	def test_valid_url(self):
		...
		self.assertTrue(validate_url("http://vmraid.io", throw=True))
		...
		self.assertRaises(vmraid.ValidationError, validate_url, "vmraid", throw=True)

		# Scheme validation
		self.assertFalse(validate_url("https://google.com", valid_schemes="http"))
		self.assertTrue(validate_url("ftp://vmraid.cloud", valid_schemes=["https", "ftp"]))
		self.assertFalse(
			validate_url("bolo://vmraid.io", valid_schemes=("http", "https", "ftp", "ftps"))
		)
		self.assertRaises(
			vmraid.ValidationError, validate_url, "gopher://vmraid.io", valid_schemes="https", throw=True
		)
	...
	def test_valid_email(self):
		...
		# Valid addresses
		self.assertTrue(validate_email_address("someone@vmraid.com"))
		self.assertTrue(validate_email_address("someone@vmraid.com, anyone@vmraid.io"))
		...
		# Invalid with throw
		self.assertRaises(
			vmraid.InvalidEmailAddressError, validate_email_address, "someone.com", throw=True
		)


class TestImage(unittest.TestCase):
	def test_strip_exif_data(self):
		original_image = Image.open("../apps/vmraid/vmraid/tests/data/exif_sample_image.jpg")
		original_image_content = io.open(
			"../apps/vmraid/vmraid/tests/data/exif_sample_image.jpg", mode="rb"
		).read()
	...
	def test_optimize_image(self):
		image_file_path = "../apps/vmraid/vmraid/tests/data/sample_image_for_optimization.jpg"
...
class TestPythonExpressions(unittest.TestCase):
	...
	def test_validation_for_bad_python_expression(self):
		...
		for expr in invalid_expressions:
			self.assertRaises(vmraid.ValidationError, validate_python_code, expr)


class TestDiffUtils(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		cls.doc = vmraid.get_doc(doctype="Client Script", dt="Client Script", name="test_client_script")
		cls.doc.insert()
		cls.doc.script = "2;"
...
class TestDateUtils(unittest.TestCase):
	def test_first_day_of_week(self):
		...
		with patch.object(vmraid.utils.data, "get_first_day_of_the_week", return_value="Monday"):
			self.assertEqual(
				vmraid.utils.get_first_day_of_week("2020-12-25"), vmraid.utils.getdate("2020-12-21")
			)
			self.assertEqual(
				vmraid.utils.get_first_day_of_week("2020-12-20"), vmraid.utils.getdate("2020-12-14")
			)

		# Sunday as start of the week
		self.assertEqual(
			vmraid.utils.get_first_day_of_week("2020-12-25"), vmraid.utils.getdate("2020-12-20")
		)
		self.assertEqual(
			vmraid.utils.get_first_day_of_week("2020-12-21"), vmraid.utils.getdate("2020-12-20")
		)

	def test_last_day_of_week(self):
		self.assertEqual(
			vmraid.utils.get_last_day_of_week("2020-12-24"), vmraid.utils.getdate("2020-12-26")
		)
		self.assertEqual(
			vmraid.utils.get_last_day_of_week("2020-12-28"), vmraid.utils.getdate("2021-01-02")
		)
...
class TestResponse(unittest.TestCase):
	def test_json_handler(self):
		...
		GOOD_OBJECT = {
			...
			"doc": [
				vmraid.get_doc("System Settings"),
			],
			"iter": [
				{1, 2, 3},
				(1, 2, 3),
...
class TestLinkTitle(unittest.TestCase):
	def test_link_title_doctypes_in_boot_info(self):
		...
		from vmraid.boot import get_link_title_doctypes
	...
	def test_link_titles_on_getdoc(self):
		...
		link_titles = vmraid.local.response["_link_titles"]
...
class TestAppParser(unittest.TestCase):
	def test_app_name_parser(self):
		...
		vmraid_app = os.path.join(chair_path, "apps", "vmraid")
		self.assertEqual("vmraid", parse_app_name(vmraid_app))
		self.assertEqual("healthcare", parse_app_name("healthcare"))
		self.assertEqual("healthcare", parse_app_name("https://github.com/vmraid/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("git@github.com:vmraid/healthcare.git"))
		self.assertEqual("healthcare", parse_app_name("vmraid/healthcare@develop"))
