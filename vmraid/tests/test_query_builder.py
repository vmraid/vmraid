...
import vmraid
from vmraid.query_builder import Case
from vmraid.query_builder.custom import ConstantColumn
from vmraid.query_builder.functions import Cast_, Coalesce, CombineDatetime, GroupConcat, Match
from vmraid.query_builder.utils import db_type_is


def run_only_if(dbtype: db_type_is) -> Callable:
	return unittest.skipIf(db_type_is(vmraid.conf.db_type) != dbtype, f"Only runs for {dbtype.value}")
...
class TestCustomFunctionsMariaDB(unittest.TestCase):
	...
	def test_constant_column(self):
		query = vmraid.qb.from_("DocType").select("name", ConstantColumn("John").as_("User"))
		self.assertEqual(query.get_sql(), "SELECT `name`,'John' `User` FROM `tabDocType`")

	def test_timestamp(self):
		note = vmraid.qb.DocType("Note")
		self.assertEqual(
...
class TestBuilderBase(object):
	def test_adding_tabs(self):
		self.assertEqual("tabNotes", vmraid.qb.DocType("Notes").get_sql())
		self.assertEqual("__Auth", vmraid.qb.DocType("__Auth").get_sql())
		self.assertEqual("Notes", vmraid.qb.Table("Notes").get_sql())

	def test_run_patcher(self):
		query = vmraid.qb.from_("ToDo").select("*").limit(1)
...
class TestParameterization(unittest.TestCase):
	def test_where_conditions(self):
		...
		query = (
			vmraid.qb.from_(DocType).select(DocType.name).where((DocType.owner == "Administrator' --"))
		)
	...
	def test_set_cnoditions(self):
		DocType = vmraid.qb.DocType("DocType")
		query = vmraid.qb.update(DocType).set(DocType.value, "some_value")
	...
	def test_case(self):
		DocType = vmraid.qb.DocType("DocType")
		query = vmraid.qb.from_(DocType).select(
			Case()
	...
	def test_case_in_update(self):
		DocType = vmraid.qb.DocType("DocType")
		query = vmraid.qb.update(DocType).set(
			"parent",
			Case()
			.when(DocType.search_fields == "value", "other_value")
			.when(Coalesce(DocType.search_fields == "subject_in_function"), "true_value")
...
class TestBuilderMaria(unittest.TestCase, TestBuilderBase):
	def test_adding_tabs_in_from(self):
		self.assertEqual("SELECT * FROM `tabNotes`", vmraid.qb.from_("Notes").select("*").get_sql())
		self.assertEqual("SELECT * FROM `__Auth`", vmraid.qb.from_("__Auth").select("*").get_sql())
...
class TestBuilderPostgres(unittest.TestCase, TestBuilderBase):
	...
	def test_replace_tables(self):
		info_schema = vmraid.qb.Schema("information_schema")
		self.assertEqual(
			'SELECT * FROM "pg_stat_all_tables"',
			vmraid.qb.from_(info_schema.tables).select("*").get_sql(),
		)

	def test_replace_fields_post(self):
		self.assertEqual("relname", vmraid.qb.Field("table_name").get_sql())
