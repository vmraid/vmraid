...
from vmraid.permissions import add_user_permission, clear_user_permissions_for_doctype
from vmraid.query_builder import Column
...
class TestReportview(unittest.TestCase):
	...
	def test_build_match_conditions(self):
		...
		test2user = vmraid.get_doc("User", "test2@example.com")
	...
	def test_none_filter(self):
		query = vmraid.db.query.get_sql("DocType", fields="name", filters={"restrict_to_domain": None})
	...
	def test_ignore_permissions_for_get_filters_cond(self):
		vmraid.set_user("test2@example.com")
		self.assertRaises(vmraid.PermissionError, get_filters_cond, "DocType", dict(istable=1), [])
	...
	def test_nested_permission(self):
		...
		from vmraid.core.page.permission_manager.permission_manager import update
	...
	def test_of_not_of_descendant_ancestors(self):
		...
		data = vmraid.get_all("Nested DocType", {"name": ("descendants of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)

		data = vmraid.get_all("Nested DocType", {"name": ("descendants of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("ancestors of", "Level 2 A")})
		...
		data = vmraid.get_all("Nested DocType", {"name": ("ancestors of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertFalse({"name": "Level 2 B"} in data)
		self.assertFalse({"name": "Level 1 B"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("not descendants of", "Level 2 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("not descendants of", "Level 1 A")})
		self.assertFalse({"name": "Level 3 A"} in data)
		self.assertFalse({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("not ancestors of", "Level 2 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("not ancestors of", "Level 1 A")})
		self.assertTrue({"name": "Level 3 A"} in data)
		self.assertTrue({"name": "Level 2 A"} in data)
		self.assertTrue({"name": "Level 2 B"} in data)
		self.assertTrue({"name": "Level 1 B"} in data)
		...
		data = vmraid.get_all("Nested DocType", {"name": ("ancestors of", "Root")})
		self.assertTrue(len(data) == 0)
		self.assertTrue(
			len(vmraid.get_all("Nested DocType", {"name": ("not ancestors of", "Root")}))
			== len(vmraid.get_all("Nested DocType"))
		)
	...
	def test_prepare_select_args(self):
		# vmraid.get_all inserts modified field into order_by clause
	...
	def test_column_comparison(self):
		...
		users_unedited = vmraid.get_all(
			"User",
			filters={"creation": Column("modified")},
			fields=["name", "creation", "modified"],
			limit=1,
		)
		users_edited = vmraid.get_all(
			"User",
			filters={"creation": ("!=", Column("modified"))},
			fields=["name", "creation", "modified"],
			limit=1,
	...
	def test_reportview_get(self):
		...
		response = execute_cmd("vmraid.desk.reportview.get")
	...
	def test_fieldname_starting_with_int(self):
		...
		vmraid.get_doc({"doctype": "dt_with_int_named_fieldname", "1field": 10}).insert(
...
def create_nested_doctype():
	if vmraid.db.exists("DocType", "Nested DocType"):
		return
...
def create_nested_doctype_records():
	...
	for r in records:
		d = vmraid.new_doc("Nested DocType")
		d.update(r)
