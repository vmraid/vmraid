...
import vmraid
from vmraid.geo.utils import get_coords


class TestGeoUtils(unittest.TestCase):
	def setUp(self):
		self.todo = vmraid.get_doc(
		...
		self.test_location = vmraid.get_doc({'name': 'Test Location', 'doctype': 'Location',
	...
	def test_get_coords_from_not_existable_location(self):
		self.assertRaises(vmraid.ValidationError, get_coords, 'ToDo', self.test_filter_todo, 'location_field')

	def test_get_coords_from_not_existable_coords(self):
		self.assertRaises(vmraid.ValidationError, get_coords, 'ToDo', self.test_filter_todo, 'coordinates')
