...
import vmraid
from vmraid.utils.password import check_password, decrypt, encrypt, passlibctx, update_password


class TestPassword(unittest.TestCase):
	def setUp(self):
		vmraid.delete_doc("Email Account", "Test Email Account Password")
		vmraid.delete_doc("Email Account", "Test Email Account Password-new")
	...
	def make_email_account(self, name="Test Email Account Password"):
		if not vmraid.db.exists("Email Account", name):
			...
			return vmraid.get_doc("Email Account", name)
	...
	def test_hashed_password(self, user="test@example.com"):
		...
		self.assertRaises(vmraid.AuthenticationError, check_password, user, new_password)
	...
	def test_password_on_rename_user(self):
		...
		vmraid.rename_doc(doc.doctype, old_name, new_name)

		new_doc = vmraid.get_doc(doc.doctype, new_name)
		self.assertEqual(new_doc.get_password(), password)
		self.assertTrue(not get_password_list(doc))

		vmraid.rename_doc(doc.doctype, new_name, old_name)
