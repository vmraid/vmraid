...
from vmraid.data_migration.doctype.data_migration_mapping.data_migration_mapping import (
	get_source_value,
)
...
class DataMigrationRun(Document):
	...
	def enqueue_next_page(self):
		...
		if percent_complete < 100:
			vmraid.publish_realtime(
				self.trigger_name, {"progress_percent": percent_complete}, user=vmraid.session.user
			)
	...
	def get_last_modified_condition(self):
		last_run_timestamp = vmraid.db.get_value(
			"Data Migration Run",
			dict(
	...
	def begin(self):
		...
		self.mappings = [
			vmraid.get_doc("Data Migration Mapping", m.mapping) for m in plan_active_mappings
		]
	...
	def complete(self):
		...
		vmraid.publish_realtime(self.trigger_name, {"progress_percent": 100}, user=vmraid.session.user)
	...
	def get_deleted_local_data(self):
		...
		data = vmraid.get_all("Deleted Document", fields=["name", "data"], filters=filters)
	...
	def get_connection(self):
		if not hasattr(self, "connection"):
			self.connection = vmraid.get_doc(
...
def local_doc_exists(mapping, migration_id_value):
	return vmraid.db.exists(mapping.local_doctype, {mapping.migration_id_field: migration_id_value})
