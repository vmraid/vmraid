...
import vmraid, json, math
from vmraid.model.document import Document
from vmraid import _
from vmraid.utils import cstr
from vmraid.data_migration.doctype.data_migration_mapping.data_migration_mapping import get_source_value
...
class DataMigrationRun(Document):
	...
	def enqueue_next_mapping(self):
		...
		if next_mapping_name:
			...
			vmraid.enqueue_doc(self.doctype, self.name, 'run_current_mapping', now=vmraid.flags.in_test)
		else:
			self.complete()
	...
	def enqueue_next_page(self):
		...
		if(percent_complete < 100):
			vmraid.publish_realtime(self.trigger_name,
				{"progress_percent": percent_complete}, user=vmraid.session.user)
	...
	def get_last_modified_condition(self):
		last_run_timestamp = vmraid.db.get_value('Data Migration Run', dict(
	...
	def complete(self):
		...
		vmraid.publish_realtime(self.trigger_name,
			{"progress_percent": 100}, user=vmraid.session.user)
	...
	def execute_postprocess(self, status):
		...
		if postprocess_method_path:
			vmraid.get_attr(postprocess_method_path)({
				"status": status,
				"stats": {
	...
	def get_plan(self):
		if not hasattr(self, 'plan'):
			self.plan = vmraid.get_doc('Data Migration Plan', self.data_migration_plan)
		return self.plan
		...
		if hasattr(self, 'mappings'):
			for m in self.mappings:
				if m.name == mapping_name:
					return m
		return vmraid.get_doc('Data Migration Mapping', mapping_name)
	...
	def get_next_mapping_name(self):
		...
		raise vmraid.ValidationError('Mapping Broken')
	...
	def get_data(self, filters):
		...
		data = []
		doclist = vmraid.get_all(mapping.local_doctype,
			filters=filters, or_filters=or_filters,
			start=start, page_length=mapping.page_length)

		for d in doclist:
			doc = vmraid.get_doc(mapping.local_doctype, d['name'])
			data.append(doc)
		return data

	def get_new_local_data(self):
		'''Fetch newly inserted local data using `vmraid.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
	...
	def get_updated_local_data(self):
		'''Fetch local updated data using `vmraid.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
		filters = mapping.get_filters() or {}
	...
	def get_deleted_local_data(self):
		'''Fetch local deleted data using `vmraid.get_all`. Used during Push'''
		mapping = self.get_mapping(self.current_mapping)
		...
		data = vmraid.get_all('Deleted Document', fields=['name', 'data'],
			filters=filters)
	...
	def get_count(self, mapping):
		...
		to_insert = vmraid.get_all(mapping.local_doctype, ['count(name) as total'],
			filters=filters, or_filters=or_filters)[0].total

		to_delete = vmraid.get_all('Deleted Document', ['count(name) as total'],
	...
	def get_connection(self):
		if not hasattr(self, 'connection'):
			self.connection = vmraid.get_doc('Data Migration Connector',
	...
	def _push_insert(self):
		...
		for d in data:
			...
			try:
				...
				vmraid.db.set_value(mapping.local_doctype, d.name,
			...
			if migration_id_value:
				try:
					if not local_doc_exists(mapping, migration_id_value):
						...
						# set migration id
						vmraid.db.set_value(mapping.local_doctype, local_doc.name,
...
def update_local_doc(mapping, remote_doc, migration_id_value):
	try:
		...
		docname = vmraid.db.get_value(mapping.local_doctype,
			filters={ mapping.migration_id_field: migration_id_value })

		doc = vmraid.get_doc(mapping.local_doctype, docname)
...
def local_doc_exists(mapping, migration_id_value):
	return vmraid.db.exists(mapping.local_doctype, {
		mapping.migration_id_field: migration_id_value
	})
