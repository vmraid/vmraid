...
class TestDataMigrationRun(unittest.TestCase):
	def test_run(self):
		...
		new_todo = vmraid.get_doc({"doctype": "ToDo", "description": description}).insert()
	...
	vmraid.get_doc(
		{
			...
			"fields": [
				{"remote_fieldname": "subject", "local_fieldname": "description"},
				{
					...
					"local_fieldname": "eval:vmraid.utils.get_datetime_str(vmraid.utils.get_datetime())",
				},
			],
