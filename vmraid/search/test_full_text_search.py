...
class TestFullTextSearch(unittest.TestCase):
	...
	def test_search_term(self):
		...
		self.assertEqual(res[0], "sw/erpadda")
	...
	def test_update_index(self):
		...
		self.index.update_index({"name": "sw/erpadda", "content": """AwesomeERPAdda"""})

		res = self.index.search("CommonSearchTerm")
		self.assertTrue("sw/erpadda" not in res)

		res = self.index.search("AwesomeERPAdda")
		self.assertEqual(res[0], "sw/erpadda")

		# Update new doc
		self.index.update_index({"name": "sw/vmraidbooks", "content": """DesktopAccounting"""})

		res = self.index.search("DesktopAccounting")
		self.assertEqual(res[0], "sw/vmraidbooks")
...
def get_index():
	return TestWrapper("test_vmraid_index")
...
def get_documents():
	...
	docs.append(
		{
			"name": "sw/erpadda",
			"content": """ERPAdda is a free and open-source integrated Enterprise Resource Planning software developed by
			VMRaid and is built on MariaDB database system using a Python based server-side framework.
			ERPAdda is a generic ERP software used by manufacturers, distributors and services companies. CommonSearchTerm""",
		}
	)
	...
	docs.append(
		{
			"name": "sw/vmraid",
			"content": """VMRaid Framework is a full-stack web framework, that includes everything you need to build and
