...
class Workflow(Document):
	...
	def on_update(self):
		...
		vmraid.clear_cache(doctype=self.document_type)
		vmraid.cache().delete_key('workflow_' + self.name) # clear cache created in model/workflow.py

	def create_custom_field_for_workflow_state(self):
		vmraid.clear_cache(doctype=self.document_type)
		meta = vmraid.get_meta(self.document_type)
		if not meta.get_field(self.workflow_state_field):
			...
			vmraid.msgprint(_("Created Custom Field {0} in {1}").format(self.workflow_state_field,
				self.document_type))
			...
			for key in new_states:
				if key in before_save_states:
					if not new_states[key].doc_status == before_save_states[key].doc_status:
						vmraid.db.set_value(self.document_type, {
	...
	def validate_docstatus(self):
		def get_state(state):
			...
			vmraid.throw(vmraid._("{0} not a valid State").format(state))
		...
		for t in self.transitions:
			...
			if state.doc_status=="2":
				vmraid.throw(vmraid._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx))

			if state.doc_status=="1" and next_state.doc_status=="0":
				vmraid.throw(vmraid._("Submitted Document cannot be converted back to draft. Transition row {0}").format(t.idx))

			if state.doc_status=="0" and next_state.doc_status=="2":
				vmraid.throw(vmraid._("Cannot cancel before submitting. See Transition {0}").format(t.idx))

	def set_active(self):
		if int(self.is_active or 0):
			# clear all other
			vmraid.db.sql("""UPDATE `tabWorkflow` SET `is_active`=0
				WHERE `document_type`=%s""",
				self.document_type)

@vmraid.whitelist()
def get_fieldnames_for(doctype):
	return [f.fieldname for f in vmraid.get_meta(doctype).fields \
		if f.fieldname not in no_value_fields]

@vmraid.whitelist()
def get_workflow_state_count(doctype, workflow_state_field, states):
	states = vmraid.parse_json(states)
