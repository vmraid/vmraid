	...
	def validate_docstatus(self):
		...
		for t in self.transitions:
			...
			if state.doc_status == "2":
				vmraid.throw(
					vmraid._("Cannot change state of Cancelled Document. Transition row {0}").format(t.idx)
				)

			if state.doc_status == "1" and next_state.doc_status == "0":
				vmraid.throw(
					vmraid._("Submitted Document cannot be converted back to draft. Transition row {0}").format(
						t.idx
					)
				)
...
def get_fieldnames_for(doctype):
	return [
		f.fieldname for f in vmraid.get_meta(doctype).fields if f.fieldname not in no_value_fields
	]
