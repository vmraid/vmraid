...
import vmraid
from vmraid.model.workflow import (
...
class TestWorkflow(unittest.TestCase):
	...
	def setUp(self):
		...
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
			if not vmraid.db.has_column("Workflow Action", "user"):
				...
				vmraid.cache().delete_value("table_columns")

	def tearDown(self):
		vmraid.delete_doc("Workflow", "Test ToDo")
		if self._testMethodName == "test_if_workflow_actions_were_processed_using_user":
			if vmraid.db.has_column("Workflow Action", "user"):
				# mariadb would raise this statement would create an implicit commit
				# if we do not commit before alter statement
				# nosemgrep
				vmraid.db.commit()
	...
	def test_if_workflow_actions_were_processed_using_role(self):
		vmraid.db.delete("Workflow Action")
		...
		workflow_actions = vmraid.get_all("Workflow Action", fields=["*"])
		...
		workflow_actions = vmraid.get_all("Workflow Action", fields=["status"])
		self.assertEqual(len(workflow_actions), 1)
	...
	def test_if_workflow_actions_were_processed_using_user(self):
		...
		vmraid.qb.update(WorkflowAction).set(WorkflowAction.user, "test2@example.com").run()
		vmraid.qb.update(WorkflowActionPermittedRole).set(WorkflowActionPermittedRole.role, "").run()
...
def create_todo_workflow():
	if vmraid.db.exists("Workflow", "Test ToDo"):
		vmraid.delete_doc("Workflow", "Test ToDo")

	if not vmraid.db.exists("Role", "Test Approver"):
		vmraid.get_doc(dict(doctype="Role", role_name="Test Approver")).insert(ignore_if_duplicate=True)
	workflow = vmraid.new_doc("Workflow")
...
def create_new_todo():
	return vmraid.get_doc(dict(doctype="ToDo", description="workflow " + random_string(10))).insert()
