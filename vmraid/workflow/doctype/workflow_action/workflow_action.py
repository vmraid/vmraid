...
from vmraid.utils import get_datetime, get_url
from vmraid.utils.background_jobs import enqueue
from vmraid.utils.data import get_link_to_form
...
def on_doctype_update():
	...
	vmraid.db.add_index("Workflow Action", ["reference_name", "reference_doctype", "status"])
...
def get_permission_query_conditions(user):
	...
	permitted_workflow_actions = (
		vmraid.qb.from_(WorkflowAction)
...
def has_permission(doc, user):
	...
	user_roles = set(vmraid.get_roles(user))
...
def return_success_page(doc):
	vmraid.respond_as_web_page(
		...
		_("{0}: {1} is set to state {2}").format(
			doc.get("doctype"), vmraid.bold(doc.get("name")), vmraid.bold(get_doc_workflow_state(doc))
		),
...
def update_completed_workflow_actions_using_role(
	...
	(
		vmraid.qb.update(WorkflowAction)
...
def create_workflow_actions_for_roles(roles, doc):
	workflow_action = vmraid.get_doc(
		{
...
def get_workflow_action_url(action, doc, user):
	apply_action_method = (
		"/api/method/vmraid.workflow.doctype.workflow_action.workflow_action.apply_action"
	)
...
def get_confirm_workflow_action_url(doc, action, user):
	confirm_action_method = (
		"/api/method/vmraid.workflow.doctype.workflow_action.workflow_action.confirm_action"
	)
...
def get_common_email_args(doc):
	...
	common_args = {
		...
		"attachments": [vmraid.attach_print(doctype, docname, file_name=docname, doc=doc)],
		"subject": subject,
		"message": response,
	}
	return common_args
...
def get_email_template(doc):
	...
	template_name = vmraid.db.get_value(
