...
def build_message_files(context):
	...
	for site in context.sites:
		try:
			...
			vmraid.connect()
			vmraid.translate.rebuild_all_translation_files()
		finally:
			vmraid.destroy()
...
@click.argument('app') #, help="App name eg. vmraid")
def new_language(context, lang_code, app):
	...
	# init site
	vmraid.connect(site=context['sites'][0])
	vmraid.translate.write_translations_file(app, lang_code)

	print("File created at ./apps/{app}/{app}/translations/{lang_code}.csv".format(app=app, lang_code=lang_code))
	print("You will need to add the language in vmraid/geo/languages.json, if you haven't done it already.")
...
def get_untranslated(context, lang, untranslated_file, all=None):
	...
	try:
		vmraid.init(site=site)
		vmraid.connect()
		vmraid.translate.get_untranslated(lang, untranslated_file, get_all=all)
	finally:
		vmraid.destroy()
...
def update_translations(context, lang, untranslated_file, translated_file):
	...
	try:
		vmraid.init(site=site)
		vmraid.connect()
		vmraid.translate.update_translations(lang, untranslated_file, translated_file)
	finally:
		vmraid.destroy()
...
def import_translations(context, lang, path):
	...
	try:
		vmraid.init(site=site)
		vmraid.connect()
		vmraid.translate.import_translations(lang, path)
	finally:
		vmraid.destroy()
