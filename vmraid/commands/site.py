...
def new_site(
	...
	"Create a new site"
	from vmraid.installer import _new_site

	vmraid.init(site=site, new_site=True)
...
def restore(
	...
	from vmraid.utils.backups import Backup
	...
	try:
		...
		if is_partial(decompressed_file_name):
			click.secho(
				"Partial Backup file detected. You cannot use a partial file to restore a VMRaid site.",
				fg="red",
			)
			click.secho(
				"Use `chair partial-restore` to restore a partial backup to an existing site.", fg="yellow"
			)
		...
		if encryption_key:
			...
			encryption_key = vmraid.get_site_config().encryption_key
			_backup.backup_decryption(encryption_key)
	...
	# dont allow downgrading to older versions of vmraid without force
...
def partial_restore(context, sql_file_path, verbose, encryption_key=None):
	from vmraid.installer import extract_sql_from_archive, partial_restore
	from vmraid.utils.backups import Backup
	...
	try:
		...
		with open(decompressed_file_name) as f:
			...
			if "Partial Backup" not in header:
				click.secho(
					"Full backup file detected.Use `chair restore` to restore a VMRaid Site.", fg="red"
				)
				_backup.decryption_rollback()
				sys.exit(1)
		...
		if encryption_key:
			...
			click.secho("Encrypted backup file detected. Decrypting using site config.", fg="yellow")
			key = vmraid.get_site_config().encryption_key
...
def install_app(context, apps):
	...
	from vmraid.installer import install_app as _install_app
...
def list_apps(context, format):
	...
	for site in context.sites:
		...
		apps = vmraid.get_single("Installed Applications").installed_applications
		...
		if apps:
			...
			installed_applications = vmraid.get_installed_apps()
			applications_summary = "\n".join(installed_applications)
			summary = f"{site_title}\n{applications_summary}\n"
	...
	if format == "json":
		click.echo(vmraid.as_json(summary_dict))
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		vmraid.connect(site=site)
		try:
			vmraid.utils.user.add_system_manager(email, first_name, last_name, send_welcome_email, password)
			vmraid.db.commit()
		finally:
			vmraid.destroy()
	if not context.sites:
...
def disable_user(context, email):
	...
	with vmraid.init_site(site):
		vmraid.connect()
		user = vmraid.get_doc("User", email)
		user.enabled = 0
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	from vmraid.migrate import SiteMigration
...
@click.argument("vmraid_provider")
@pass_context
def migrate_to(context, vmraid_provider):
	"Migrates site to the specified provider"
	from vmraid.integrations.vmraid_providers import migrate_to

	for site in context.sites:
		vmraid.init(site=site)
		vmraid.connect()
		migrate_to(site, vmraid_provider)
		vmraid.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def run_patch(context, module, force):
	...
	for site in context.sites:
		...
		try:
			vmraid.connect()
			vmraid.modules.patch_handler.run_single(module, force=force or context.force)
		finally:
			vmraid.destroy()
	if not context.sites:
		raise SiteNotSpecifiedError
...
def reload_doc(context, module, doctype, docname):
	...
	for site in context.sites:
		try:
			vmraid.init(site=site)
			vmraid.connect()
			vmraid.reload_doc(module, doctype, docname, force=context.force)
			vmraid.db.commit()
		finally:
			vmraid.destroy()
	if not context.sites:
...
def reload_doctype(context, doctype):
	...
	for site in context.sites:
		try:
			vmraid.init(site=site)
			vmraid.connect()
			vmraid.reload_doctype(doctype, force=context.force)
			vmraid.db.commit()
		finally:
			vmraid.destroy()
	if not context.sites:
...
def add_to_hosts(context):
	"Add site to hosts"
	for site in context.sites:
		vmraid.commands.popen("echo 127.0.0.1\t{0} | sudo tee -a /etc/hosts".format(site))
	if not context.sites:
		raise SiteNotSpecifiedError
...
def backup(
	...
	for site in context.sites:
		try:
			...
			exit_code = 1
			continue
		if vmraid.get_system_settings("encrypt_backup") and vmraid.get_site_config().encryption_key:
			click.secho(
...
def uninstall(context, app, dry_run, yes, no_backup, force):
	...
	from vmraid.installer import remove_app
...
def _drop_site(
	...
	from vmraid.database import drop_user_and_database
	from vmraid.utils.backups import scheduled_backup
	...
	try:
		...
		if force:
			...
			messages = [
				...
				"Hint: Use 'chair drop-site {0} --force' to force the removal of {0}".format(site),
			]
			click.echo("\n".join(messages))
			sys.exit(1)

	click.secho("Dropping site database and user", fg="green")
	drop_user_and_database(vmraid.conf.db_name, db_root_username, db_root_password)

	archived_sites_path = archived_sites_path or os.path.join(
		vmraid.get_app_path("vmraid"), "..", "..", "..", "archived", "sites"
	)
...
def move(dest_dir, site):
	...
	vmraid.init(site)
	old_path = vmraid.utils.get_site_path()
...
def set_last_active_for_user(context, user=None):
	...
	with vmraid.init_site(site):
		...
		vmraid.db.set_value("User", user, "last_active", now_datetime())
		vmraid.db.commit()
...
def publish_realtime(context, event, message, room, user, doctype, docname, after_commit):
	"Publish realtime event from chair"
	from vmraid import publish_realtime
...
def browse(context, site, user=None):
	...
	if site not in vmraid.utils.get_sites():
	...
	sid = ""
	if user:
		if vmraid.conf.developer_mode or user == "Administrator":
			...
			sid = f"/app?sid={vmraid.session.sid}"
		else:
			click.echo("Please enable developer mode to login as a user")

	url = f"{vmraid.utils.get_site_url(site)}{sid}"
...
def trim_database(context, dry_run, format, no_backup):
	...
	for site in context.sites:
		...
		queried_result = (
			...
			.select(table_name)
			.where(information_schema.tables.table_schema == vmraid.conf.db_name)
			.run()
		)

		database_tables = [x[0] for x in queried_result]
		doctype_tables = vmraid.get_all("DocType", pluck="name")
		...
		if not TABLES_TO_DROP:
			if format == "text":
				click.secho(f"No ghost tables found in {vmraid.local.site}...Great!", fg="green")
		else:
			...
			for table in TABLES_TO_DROP:
				...
				if not dry_run:
					vmraid.db.sql_ddl(f"drop table `{table}`")

			ALL_DATA[vmraid.local.site] = TABLES_TO_DROP
		vmraid.destroy()
...
def get_standard_tables():
	...
	sql_file = os.path.join(
		...
		"database",
		vmraid.conf.db_type,
		f"framework_{vmraid.conf.db_type}.sql",
	)
...
def trim_tables(context, dry_run, format, no_backup):
	...
	for site in context.sites:
		...
		if not (no_backup or dry_run):
			click.secho(f"Taking backup for {vmraid.local.site}", fg="green")
		...
		try:
			...
			if format == "table" and not dry_run:
				click.secho(f"The following data have been removed from {vmraid.local.site}", fg="green")
...
def handle_data(data: dict, format="json"):
	if format == "json":
		...
		print(json.dumps({vmraid.local.site: data}, indent=1, sort_keys=True))
	else:
