...
def new_site(site, mariadb_root_username=None, mariadb_root_password=None, admin_password=None,
	...
	if len(vmraid.utils.get_sites()) == 1:
		use(site)
...
def restore(context, sql_file_path, mariadb_root_username=None, mariadb_root_password=None, db_name=None, verbose=None, install_app=None, admin_password=None, force=None, with_public_files=None, with_private_files=None):
	...
	if is_partial(decompressed_file_name):
		...
		click.secho(
			"Use `chair partial-restore` to restore a partial backup to an existing site.",
			fg="yellow"
		)
		sys.exit(1)
	...
	_new_site(vmraid.conf.db_name, site, mariadb_root_username=mariadb_root_username,
		mariadb_root_password=mariadb_root_password, admin_password=admin_password,
		verbose=context.verbose, install_apps=install_app, source_sql=decompressed_file_name,
		force=True, db_type=vmraid.conf.db_type)
...
def partial_restore(context, sql_file_path, verbose):
	from vmraid.installer import partial_restore
...
def _reinstall(site, admin_password=None, mariadb_root_username=None, mariadb_root_password=None, yes=False, verbose=False):
	...
	vmraid.init(site=site)
	_new_site(vmraid.conf.db_name, site, verbose=verbose, force=True, reinstall=True, install_apps=installed,
	...
	for site in context.sites:
		...
		for app in apps:
			try:
				...
				err_msg = ":\n{}".format(err if str(err) else vmraid.get_traceback())
...
def add_system_manager(context, email, first_name, last_name, send_welcome_email, password):
	...
	for site in context.sites:
		vmraid.connect(site=site)
		try:
			vmraid.utils.user.add_system_manager(email, first_name, last_name,
...
def migrate(context, skip_failing=False, skip_search_index=False):
	...
	import re
	from vmraid.migrate import migrate
...
def _drop_site(site, root_login='root', root_password=None, archived_sites_path=None, force=False, no_backup=False):
	...
	drop_user_and_database(vmraid.conf.db_name, root_login, root_password)

	if not archived_sites_path:
		archived_sites_path = os.path.join(vmraid.get_app_path('vmraid'), '..', '..', '..', 'archived_sites')
...
def set_last_active_for_user(context, user=None):
	...
	from vmraid.utils.user import set_last_active_to_now
...
def browse(context, site):
	...
	if not site:
		click.echo('''Please provide site name\n\nUsage:\n\tchair browse [site-name]\nor\n\tchair --site [site-name] browse''')
		return

	site = site.lower()

	if site in vmraid.utils.get_sites():
		webbrowser.open(vmraid.utils.get_site_url(site), new=2)
	else:
