...
from vmraid.desk.doctype.desktop_icon.desktop_icon import set_hidden, clear_desktop_icons_cache
from vmraid.cache_manager import build_domain_restriced_doctype_cache, build_domain_restriced_page_cache, build_table_count_cache
...
def hide_module(module):
	set_hidden(module, vmraid.session.user, 1)
...
def filter_by_restrict_to_domain(data):
	...
	doctypes = vmraid.cache().get_value("domain_restricted_doctypes") or build_domain_restriced_doctype_cache()
	pages = vmraid.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
...
def build_standard_config(module, doctype_info):
	...
	if not vmraid.db.get_value("Module Def", module):
		vmraid.throw(_("Module Not Found"))
...
def get_doctype_info(module):
	...
	doctype_info = vmraid.get_all("DocType", filters={
		"module": module,
		"istable": 0
	}, or_filters={
...
def get_disabled_reports():
	if not hasattr(vmraid.local, "disabled_reports"):
		vmraid.local.disabled_reports = set(r.name for r in vmraid.get_all("Report", {"disabled": 1}))
	return vmraid.local.disabled_reports

def get_config(app, module):
	"""Load module info from `[app].config.[module]`."""
	config = vmraid.get_module("{app}.config.{module}".format(app=app, module=module))
...
def config_exists(app, module):
	try:
		vmraid.get_module("{app}.config.{module}".format(app=app, module=module))
		return True
	except ImportError:
		return False
...
def get_links(app, module):
	try:
		sections = get_config(app, vmraid.scrub(module))
	except ImportError:
		return []
...
def get_desktop_settings():
	...
	def apply_user_saved_links(module):
		...
		if module.module_name in user_saved_links_by_module:
			user_links = vmraid.parse_json(user_saved_links_by_module[module.module_name])
...
def update_hidden_modules(category_map):
	category_map = vmraid.parse_json(category_map)
	home_settings = get_home_settings()

	saved_hidden_modules = home_settings.hidden_modules or []

	for category in category_map:
		config = vmraid._dict(category_map[category])
...
def update_global_hidden_modules(modules):
	modules = vmraid.parse_json(modules)
	vmraid.only_for('System Manager')

	doc = vmraid.get_doc('User', 'Administrator')
...
def get_options_for_global_modules():
	...
	blocked_modules = vmraid.get_doc('User', 'Administrator').get_blocked_modules()
...
def set_home_settings(home_settings):
	vmraid.cache().hset('home_settings', vmraid.session.user, home_settings)
	vmraid.db.set_value('User', vmraid.session.user, 'home_settings', json.dumps(home_settings))

@vmraid.whitelist()
def get_home_settings():
	def get_from_db():
		settings = vmraid.db.get_value("User", vmraid.session.user, 'home_settings')
		return vmraid.parse_json(settings or '{}')

	home_settings = vmraid.cache().hget('home_settings', vmraid.session.user, get_from_db)
...
def get_last_modified(doctype):
	def _get():
		try:
			last_modified = vmraid.get_all(doctype, fields=["max(modified)"], as_list=True, limit_page_length=1)[0][0]
	...
	last_modified = vmraid.cache().hget("last_modified", doctype, _get)
...
def get_report_list(module, is_standard="No"):
	...
	reports =  vmraid.get_list("Report", fields=["name", "ref_doctype", "report_type"], filters=
