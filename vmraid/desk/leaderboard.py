...
def get_leaderboards():
	leaderboards = {
		'User': {
			'fields': ['points'],
			'method': 'vmraid.desk.leaderboard.get_energy_point_leaderboard',
			'company_disabled': 1,
			'icon': 'users'
		}
	}
...
def get_energy_point_leaderboard(date_range, company = None, field = None, limit = None):
	all_users = vmraid.db.get_all('User',
		filters = {
	...
	if date_range:
		date_range = vmraid.parse_json(date_range)
		filters.append(['creation', 'between', [date_range[0], date_range[1]]])
	energy_point_users = vmraid.db.get_all('Energy Point Log',
