...
import vmraid, json
from vmraid.utils import cstr, unique, cint
...
def sanitize_searchfield(searchfield):
	...
	def _raise_exception(searchfield):
		vmraid.throw(_('Invalid Search Field {0}').format(searchfield), vmraid.DataError)
...
def search_link(doctype, txt, query=None, filters=None, page_length=20, searchfield=None, reference_doctype=None, ignore_user_permissions=False):
	...
	vmraid.response['results'] = build_for_autosuggest(vmraid.response["values"])
	del vmraid.response["values"]
...
def search_widget(doctype, txt, query=None, searchfield=None, start=0,
	...
	standard_queries = vmraid.get_hooks().standard_queries or {}

	if query and query.split()[0].lower()!="select":
		# by method
		try:
			is_whitelisted(vmraid.get_attr(query))
			vmraid.response["values"] = vmraid.call(query, doctype, txt,
				searchfield, start, page_length, filters, as_dict=as_dict)
		except vmraid.exceptions.PermissionError as e:
			if vmraid.local.conf.developer_mode:
				raise e
			else:
				vmraid.respond_as_web_page(title='Invalid Method', html='Method not found',
		...
		if query:
			vmraid.throw(_("This query style is discontinued"))
			# custom query
			# vmraid.response["values"] = vmraid.db.sql(scrub_custom_query(query, searchfield, txt))
		else:
			...
			formatted_fields.append("""locate({_txt}, `tab{doctype}`.`name`) as `_relevance`""".format(
				_txt=vmraid.db.escape((txt or "").replace("%", "").replace("@", "")), doctype=doctype))


			# In order_by, `idx` gets second priority, because it stores link count
			from vmraid.model.db_query import get_order_by
			...
			values = vmraid.get_list(doctype,
				filters=filters,
			...
			if as_dict:
				for r in values:
					r.pop("_relevance")
				vmraid.response["values"] = values
			else:
				vmraid.response["values"] = [r[:-1] for r in values]
...
def validate_and_sanitize_search_inputs(fn, instance, args, kwargs):
	...
	if kwargs['doctype'] and not vmraid.db.exists('DocType', kwargs['doctype']):
		return []
...
def get_names_for_mentions(search_term):
	users_for_mentions = vmraid.cache().get_value('users_for_mentions', get_users_for_mentions)
	user_groups = vmraid.cache().get_value('user_groups', get_user_groups)
	...
	for mention_data in users_for_mentions + user_groups:
		...
		mention_data['link'] = vmraid.utils.get_url_to_form(
...
def get_user_groups():
	return vmraid.get_all('User Group', fields=['name as id', 'name as value'], update={
		'is_group': True
	})
