...
from vmraid.utils import cint, cstr, unique
...
def search_link(
	...
	vmraid.response["results"] = build_for_autosuggest(vmraid.response["values"], doctype=doctype)
...
def search_widget(
	...
	if query and query.split()[0].lower() != "select":
		...
		try:
			...
			vmraid.response["values"] = vmraid.call(
		...
		if query:
			...
			translated_search_doctypes = vmraid.get_hooks("translated_search_doctypes")
			# build from doctype
			if txt:
				search_fields = ["name"]
				if meta.title_field:
...
def get_link_title(doctype, docname):
	...
	if meta.title_field and meta.show_title_field_in_link:
		return vmraid.db.get_value(doctype, docname, meta.title_field)
