vmraid.pages['translation-tool'].on_page_load = function(wrapper) {
	...
	vmraid.translation_tool = new TranslationTool(page);
};
...
class TranslationTool {
	constructor(page) {
		...
		this.wrapper.append(vmraid.render_template('translation_tool'));
	...
	setup_language_filter() {
		let languages = Object.keys(vmraid.boot.lang_dict).map(language_label => {
			let value = vmraid.boot.lang_dict[language_label];
			return {
		...
		if (translation_language || vmraid.boot.lang !== 'en') {
			language_selector.set_value(translation_language || vmraid.boot.lang);
		} else {
			vmraid.prompt(
				{
	...
	fetch_messages() {
		vmraid.dom.freeze(__('Fetching...'));
		return vmraid
			.xcall('vmraid.translate.get_messages', {
			...
			<div
				...
				<div class="bold ellipsis">
					<span class="indicator ${this.get_indicator_color(message)}">
						<span>${vmraid.utils.escape_html(message.source_text)}</span>
					</span>
				</div>
			</div>
		`;
	...
	get_additional_info(source_id) {
		vmraid.dom.freeze('Fetching...');
		return vmraid.xcall('vmraid.translate.get_source_additional_info', {
			source: source_id,
			language: this.page.fields_dict['language'].get_value()
		}).finally(vmraid.dom.unfreeze);
	}

	make_edit_form(translation, { contributions, positions }) {
		if (!this.form) {
			this.form = new vmraid.ui.FieldGroup({
				fields: [
					{
						fieldtype: 'HTML',
						fieldname: 'header',
	...
	show_confirmation_dialog() {
		this.confirmation_dialog = new vmraid.ui.Dialog({
			fields: [
				{
					label: __('Language'),
					fieldname: 'language',
	...
	create_translations() {
		vmraid.dom.freeze(__('Submitting...'));
		return vmraid
			.xcall(
				'vmraid.core.doctype.translation.translation.create_translations',
				{
			...
			.then(() => {
				vmraid.dom.unfreeze();
				vmraid.show_alert({ message: __('Successfully Submitted!'), indicator: 'success'});
				this.edited_translations = {};
				this.update_header();
				this.fetch_messages_then_render();
			})
	...
	setup_local_contributions() {
		// TODO: Refactor
		vmraid
			.xcall('vmraid.translate.get_contributions', {
				language: this.language
			})
			.then(messages => {
				let template = message => `
	...
	show_translation_status_modal(e, $el) {
		...
		vmraid.xcall('vmraid.translate.get_contribution_status', { message_id })
			.then(doc => {
	...
	get_code_url(path, line_no, app) {
		...
		return `https://github.com/vmraid/${app}/blob/develop/${code_path}#L${line_no}`;
	}
}
