...
from vmraid.utils import strip, cint
from vmraid.translate import (set_default_language, get_dict, send_translations)
...
def setup_complete(args):
	...
	try:
		...
		for idx, stage in enumerate(stages):
			vmraid.publish_realtime('setup_task', {"progress": [idx, len(stages)],
				"stage_status": stage.get('status')}, user=vmraid.session.user)
...
def update_user_name(args):
	...
	if args.get("email"):
		...
		vmraid.db.sql("""update `tabUser` SET first_name=%(first_name)s,
...
@vmraid.whitelist()
def load_languages():
	language_codes = vmraid.db.sql('select language_code, language_name from tabLanguage order by name', as_dict=True)
	codes_to_names = {}
	for d in language_codes:
		codes_to_names[d.language_code] = d.language_name
	return {
		"default_language": vmraid.db.get_value('Language', vmraid.local.lang, 'language_name') or vmraid.local.lang,
...
def email_setup_wizard_exception(traceback, args):
	if not vmraid.local.conf.setup_wizard_exception_email:
		return

	pretty_args = prettify_args(args)

	if vmraid.local.request:
		user_agent = UserAgent(vmraid.local.request.headers.get('User-Agent', ''))

	else:
		user_agent = vmraid._dict()
...
- **Browser Languages**: `{accept_languages}`""".format(
		...
		headers=vmraid.local.request.headers,
		accept_languages=", ".join(vmraid.local.request.accept_languages.values()))

	vmraid.sendmail(recipients=vmraid.local.conf.setup_wizard_exception_email,
