...
from vmraid.utils import get_site_path, cint, get_url
from vmraid.utils.data import convert_utc_to_user_timezone
import datetime
...
def get_scheduled_backup_limit():
	backup_limit = vmraid.db.get_singles_value('System Settings', 'backup_limit')
...
def schedule_files_backup(user_email):
	...
	queued_jobs = get_jobs(site=vmraid.local.site, queue="long")
	method = 'vmraid.desk.page.backups.backups.backup_files_and_notify_user'

	if method not in queued_jobs[vmraid.local.site]:
		enqueue("vmraid.desk.page.backups.backups.backup_files_and_notify_user", queue='long', user_email=user_email)
		vmraid.msgprint(_("Queued for backup. You will receive an email with the download link"))
	else:
		vmraid.msgprint(_("Backup job is already queued. You will receive an email with the download link"))
