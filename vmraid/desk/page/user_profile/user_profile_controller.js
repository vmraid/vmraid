import BaseTimeline from "../../../public/js/vmraid/form/footer/base_timeline";
...
class UserProfile {
	...
	show() {
		...
		this.user_id = route[1] || vmraid.session.user;

		//validate if user
		if (route.length > 1) {
			vmraid.dom.freeze(__('Loading user profile') + '...');
			vmraid.db.exists('User', this.user_id).then(exists => {
				vmraid.dom.unfreeze();
				if (exists) {
					this.make_user_profile();
				} else {
					vmraid.msgprint(__('User does not exist'));
				}
			});
		} else {
			vmraid.set_route('user-profile', vmraid.session.user);
		}
	}

	make_user_profile() {
		this.user = vmraid.user_info(this.user_id);
		this.page.set_title(this.user.fullname);
		this.setup_user_search();
		this.main_section.empty().append(vmraid.render_template('user_profile'));
	...
	render_heatmap() {
		this.heatmap = new vmraid.Chart('.performance-heatmap', {
			type: 'heatmap',
	...
	update_heatmap_data(date_from) {
		vmraid.xcall('vmraid.desk.page.user_profile.user_profile.get_energy_points_heatmap_data', {
			user: this.user_id,
			date: date_from || vmraid.datetime.year_start(),
		}).then((r) => {
	...
	render_line_chart() {
		...
		this.line_chart = new vmraid.Chart('.performance-line-chart', {
			type: 'line',
			height: 200,
			data: {
				labels: [],
	...
	update_line_chart_data() {
		...
		vmraid.xcall('vmraid.desk.doctype.dashboard_chart.dashboard_chart.get', {
	...
	render_percentage_chart(field, title) {
		vmraid.xcall('vmraid.desk.page.user_profile.user_profile.get_energy_points_percentage_chart_data', {
			user: this.user_id,
			field: field
		}).then(chart => {
			if (chart.labels.length) {
				this.percentage_chart = new vmraid.Chart('.performance-percentage-chart', {
					type: 'percentage',
					data: {
						labels: chart.labels,
	...
	create_line_chart_filters() {
		...
		vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.line-chart-options', 1);
	}
	...
	create_percentage_chart_filters() {
		...
		vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.percentage-chart-options');
	}

	create_heatmap_chart_filters() {
		let filters = [
			{
				label: vmraid.dashboard_utils.get_year(vmraid.datetime.now_date()),
				options: vmraid.dashboard_utils.get_years_since_creation(vmraid.boot.user.creation),
				action: (selected_item) => {
					this.update_heatmap_data(vmraid.datetime.obj_to_str(selected_item));
				}
			},
		];
		vmraid.dashboard_utils.render_chart_filters(filters, 'chart-filter', '.heatmap-options');
	}


	edit_profile() {
		let edit_profile_dialog = new vmraid.ui.Dialog({
			...
			primary_action: values => {
				...
				vmraid.xcall('vmraid.desk.page.user_profile.user_profile.update_profile_info', {
					profile_info: values
				}).then(user => {
	...
	render_user_details() {
		this.sidebar.empty().append(vmraid.render_template('user_profile_sidebar', {
			user_image: this.user.image,
	...
	setup_user_profile_links() {
		if (this.user_id !== vmraid.session.user) {
	...
	get_user_rank() {
		return vmraid.xcall('vmraid.desk.page.user_profile.user_profile.get_user_rank', {
			user: this.user_id,
		}).then(r => {
	...
	get_user_points() {
		return vmraid.xcall(
			'vmraid.social.doctype.energy_point_log.energy_point_log.get_user_energy_and_review_points',
			{
				user: this.user_id,
			}
		).then(r => {
	...
	render_points_and_rank() {
		...
		const _get_stat_dom = (value, label, icon) => {
			return `<div class="user-stats-item mt-4">
				${vmraid.utils.icon(icon, "lg", "no-stroke")}
				<div>
	...
	go_to_user_settings() {
		vmraid.set_route('Form', 'User', this.user_id);
	}
...
class UserProfileTimeline extends BaseTimeline {
	...
	get_user_activity_data() {
		return vmraid.xcall('vmraid.desk.page.user_profile.user_profile.get_energy_points_list', {
	...
	get_activity_timeline_item(data) {
		...
		return {
			...
			is_card: true,
			content: vmraid.energy_points.format_history_log(data),
		};
	}
...
vmraid.ui.UserProfile = UserProfile;
