...
# Author - Shivam Mishra <shivam@vmraid.io>

from __future__ import unicode_literals
import vmraid
from json import loads, dumps
from vmraid import _, DoesNotExistError, ValidationError, _dict
from vmraid.boot import get_allowed_pages, get_allowed_reports
from six import string_types
from functools import wraps
from vmraid.cache_manager import (
...
class Workspace:
	def __init__(self, page_name, minimal=False):
		...
		self.restricted_doctypes = vmraid.cache().get_value("domain_restricted_doctypes") or build_domain_restriced_doctype_cache()
		self.restricted_pages = vmraid.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
	...
	def is_page_allowed(self):
		...
		def _in_active_domains(item):
			if not item.restrict_to_domain:
				...
				return item.restrict_to_domain in vmraid.get_active_domains()
	...
	def get_cached(self, cache_key, fallback_fn):
		...
		value = _cache.get_value(cache_key, user=vmraid.session.user)
		if value:
			return value
		...
		_cache.set_value(cache_key, value, vmraid.session.user, 21600)
		return value
	...
	def get_page_for_user(self):
		...
		user_pages = vmraid.get_all("Workspace", filters=filters, limit=1)
		if user_pages:
			return vmraid.get_cached_doc("Workspace", user_pages[0])
		...
		default_page = vmraid.get_all("Workspace", filters=filters, limit=1)
		if default_page:
			return vmraid.get_cached_doc("Workspace", default_page[0])

		self.get_pages_to_extend()
		return vmraid.get_cached_doc("Workspace", self.page_name)

	def get_onboarding_doc(self):
		# Check if onboarding is enabled
		if not vmraid.get_system_settings("enable_onboarding"):
			return None

		if not self.doc.onboarding:
			return None

		if vmraid.db.get_value("Module Onboarding", self.doc.onboarding, "is_complete"):
			return None

		doc = vmraid.get_doc("Module Onboarding", self.doc.onboarding)

		# Check if user is allowed
		allowed_roles = set(doc.get_allowed_roles())
		user_roles = set(vmraid.get_roles())
	...
	def get_pages_to_extend(self):
		pages = vmraid.get_all("Workspace", filters={
			"extends": self.page_name,
			'restrict_to_domain': ['in', vmraid.get_active_domains()],
			'for_user': '',
			'module': ['in', self.allowed_modules]
		})

		pages = [vmraid.get_cached_doc("Workspace", page['name']) for page in pages]
	...
	def _doctype_contains_a_record(self, name):
		...
		if not exists and vmraid.db.exists(name):
			if not vmraid.db.get_value('DocType', name, 'issingle'):
				exists = bool(vmraid.db.get_all(name, limit=1))
			else:
				exists = True
	...
	def get_links(self):
		...
		default_country = vmraid.db.get_default("country")
	...
	def get_charts(self):
		all_charts = []
		if vmraid.has_permission("Dashboard Chart", throw=False):
			charts = self.doc.charts
			if len(self.extended_charts):
				charts = charts + self.extended_charts

			for chart in charts:
				if vmraid.has_permission('Dashboard Chart', doc=chart.chart_name):
					# Translate label
	...
	def get_onboarding_steps(self):
		...
		for doc in self.onboarding_doc.get_steps():
			...
			if step.action == "Create Entry":
				step.is_submittable = vmraid.db.get_value("DocType", step.reference_document, 'is_submittable', cache=True)
			steps.append(step)
...
def get_desk_sidebar_items():
	...
	blocked_modules = vmraid.get_doc('User', vmraid.session.user).get_blocked_modules()
	...
	if not vmraid.local.conf.developer_mode:
		filters['developer_mode_only'] = '0'

	# pages sorted based on pinned to top and then by name
	order_by = "pin_to_top desc, pin_to_bottom asc, name asc"
	all_pages = vmraid.get_all("Workspace", fields=["name", "category", "icon",  "module"],
...
def get_table_with_counts():
	counts = vmraid.cache().get_value("information_schema:counts")
	if not counts:
...
def get_custom_doctype_list(module):
	doctypes =  vmraid.get_all("DocType", fields=["name"], filters={"custom": 1, "istable": 0, "module": module}, order_by="name")
...
def get_custom_report_list(module):
	...
	reports =  vmraid.get_all("Report", fields=["name", "ref_doctype", "report_type"], filters=
...
def get_custom_workspace_for_user(page):
	...
	if pages:
		return vmraid.get_doc("Workspace", pages[0])
	doc = vmraid.new_doc("Workspace")
	doc.extends = page
	doc.for_user = vmraid.session.user
	return doc
...
def save_customization(page, config):
	...
	"""
	original_page = vmraid.get_doc("Workspace", page)
	...
	# Set label
	page_doc.label = page + '-' + vmraid.session.user
	...
	try:
		...
		vmraid.log_error(log, _("Could not save customization"))
		return False
...
def update_onboarding_step(name, field, value):
	...
	"""
	vmraid.db.set_value("Onboarding Step", name, field, value)
