...
import vmraid
from vmraid import DoesNotExistError, ValidationError, _, _dict
...
class Workspace:
	def __init__(self, page, minimal=False):
		...
		self.workspace_manager = "Workspace Manager" in vmraid.get_roles()

		self.user = vmraid.get_user()
		self.allowed_modules = self.get_cached("user_allowed_modules", self.get_allowed_modules)

		self.doc = vmraid.get_cached_doc("Workspace", self.page_name)
		...
		self.restricted_pages = (
			vmraid.cache().get_value("domain_restricted_pages") or build_domain_restriced_page_cache()
		)
	...
	def is_permitted(self):
		...
		allowed = [
			d.role for d in vmraid.get_all("Has Role", fields=["role"], filters={"parent": self.doc.name})
		]
	...
	def get_onboarding_doc(self, onboarding):
		...
		if vmraid.db.get_value("Module Onboarding", onboarding, "is_complete"):
			return None

		doc = vmraid.get_doc("Module Onboarding", onboarding)
...
def get_workspace_sidebar_items():
	...
	has_access = "Workspace Manager" in vmraid.get_roles()
	...
	for page in all_pages:
		try:
			...
			if has_access or workspace.is_permitted():
				if page.public:
					pages.append(page)
				elif page.for_user == vmraid.session.user:
