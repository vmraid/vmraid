...
def get_data(doctypes, last_modified):
	data_map = {}
	for dump_report_map in vmraid.get_hooks().dump_report_map:
		data_map.update(vmraid.get_attr(dump_report_map))
	...
	for d in doctypes:
		...
		if d in last_modified:
			...
			out[dt]["modified_names"] = vmraid.db.sql_list("""select %sname from %s
		...
		out[dt]["data"] = [list(t) for t in vmraid.db.sql("""select %s from %s %s %s""" \
		...
		tmp = vmraid.db.sql("""select `modified`
