...
def update_follow(doctype, doc_name, following):
	if following:
		return follow_document(doctype, doc_name, vmraid.session.user)
	else:
		return unfollow_document(doctype, doc_name, vmraid.session.user)
...
def follow_document(doctype, doc_name, user):
	...
	if ((not vmraid.get_meta(doctype).track_changes)
		or user == "Administrator"):
		return

	if not vmraid.db.get_value("User", user, "document_follow_notify", ignore=True, cache=True):
		return

	if not is_document_followed(doctype, doc_name, user):
		doc = vmraid.new_doc("Document Follow")
		doc.update({
			"ref_doctype": doctype,
...
def unfollow_document(doctype, doc_name, user):
	...
	if doc:
		vmraid.delete_doc("Document Follow", doc[0].name)
		return 1
	return 0
...
def send_document_follow_mails(frequency):
	...
	users = vmraid.get_list("Document Follow",
		fields=["*"])
	...
	for user in grouped_by_user:
		user_frequency = vmraid.db.get_value("User", user, "document_follow_frequency")
		message = []
		...
		if user_frequency == frequency:
			...
			if message and vmraid.db.get_value("User", user, "document_follow_notify", ignore=True):
...
def get_version(doctype, doc_name, frequency, user):
	...
	version = vmraid.get_all("Version",
		filters=filters,
		fields=["ref_doctype", "data", "modified", "modified", "modified_by"]
	)
	if version:
		for v in version:
			change = vmraid.parse_json(v.data)
			time = vmraid.utils.format_datetime(v.modified, "hh:mm a")
			timeline_items = []
			if change.changed:
...
def get_comments(doctype, doc_name, frequency, user):
	...
	for comment in comments:
		...
		time = vmraid.utils.format_datetime(comment.modified, "hh:mm a")
		timeline.append({
...
def get_filters(search_by, name, frequency, user):
	...
	if frequency == "Weekly":
		filters = [
			[search_by, "=", name],
			["modified", ">", vmraid.utils.add_days(vmraid.utils.nowdate(),-7)],
			["modified", "<", vmraid.utils.nowdate()],
			["modified_by", "!=", user]
		]
	elif frequency == "Daily":
		filters = [
			[search_by, "=", name],
			["modified", ">", vmraid.utils.add_days(vmraid.utils.nowdate(),-1)],
			["modified", "<", vmraid.utils.nowdate()],
			["modified_by", "!=", user]
		]
		...
		filters = [
			[search_by, "=", name],
			["modified", ">", vmraid.utils.add_to_date(vmraid.utils.now_datetime(), hours=-1)],
			["modified", "<", vmraid.utils.now_datetime()],
			["modified_by", "!=", user]
		]
