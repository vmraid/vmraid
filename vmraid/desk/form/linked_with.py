...
class SubmittableDocumentTree:
	def __init__(self, doctype: str, name: str):
		...
		* Node is referenced(linked) by many other documents and those are the child nodes.
		...
		NOTE: child document is a property of child node (not same as VMRaid child docs of a table field).
		"""
	...
	def get_submittable_doctypes(self) -> List[str]:
		...
		if not self._submittable_doctypes:
			self._submittable_doctypes = vmraid.db.get_all("DocType", {"is_submittable": 1}, pluck="name")
...
def get_references_across_doctypes_by_dynamic_link_field(
	...
	for doctype, fieldname, doctype_fieldname in links:
		try:
			...
			for linked_to in vmraid.db.get_all(
		...
		except vmraid.db.ProgrammingError:
			# TODO: FIXME
			continue
	return links_by_doctype
...
def get_referencing_documents(
	...
	if not link_info.get("is_child"):
		...
		return {from_table: vmraid.db.get_all(from_table, filters, pluck="name")}

	filters.extend(child_filters or [])
	res = vmraid.db.get_all(from_table, filters=filters, fields=["name", "parenttype", "parent"])
	...
	for parent, rows in itertools.groupby(res, key=lambda row: row["parenttype"]):
		...
		documents[parent].extend(vmraid.db.get_all(parent, filters=filters, pluck="name") or [])
	return documents
...
def get_linked_docs(doctype: str, name: str, linkinfo: Optional[Dict] = None) -> Dict[str, List]:
	...
	for dt, link in linkinfo.items():
		...
		try:
			...
			if isinstance(e, vmraid.DoesNotExistError):
		...
		if not linkmeta.get("issingle"):
			...
			try:
				if link.get("filters"):
					ret = vmraid.get_all(doctype=dt, fields=fields, filters=link.get("filters"))
					...
					if me and me.parenttype == dt:
						ret = vmraid.get_all(doctype=dt, fields=fields, filters=[[dt, "name", "=", me.parent]])
					...
					if link_fieldnames:
						...
						ret = vmraid.get_all(doctype=dt, fields=fields, filters=filters, or_filters=or_filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	for parent, options in vmraid.get_all(
...
def get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	for df in links:
		...
		is_child = vmraid.get_meta(df.doctype).istable
		possible_link = vmraid.get_all(
			df.doctype,
