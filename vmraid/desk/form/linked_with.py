...
def cancel_all_linked_docs(docs, ignore_doctypes_on_cancel_all=[]):
	...
	for i, doc in enumerate(docs, 1):
		if validate_linked_doc(doc, ignore_doctypes_on_cancel_all):
			linked_doc = vmraid.get_doc(doc.get("doctype"), doc.get("name"))
			linked_doc.cancel()
		vmraid.publish_progress(percent=i/len(docs) * 100, title=_("Cancelling documents"))
...
def validate_linked_doc(docinfo, ignore_doctypes_on_cancel_all=[]):
	...
	if not vmraid.get_meta(docinfo.get('doctype')).is_submittable:
		return False
...
def get_exempted_doctypes():
	...
	for doctypes in vmraid.get_hooks('auto_cancel_exempted_doctypes'):
...
def get_linked_docs(doctype, name, linkinfo=None, for_doctype=None):
	...
	if for_doctype:
		links = vmraid.get_doc(doctype, name).get_link_filters(for_doctype)
	...
	me = vmraid.db.get_value(doctype, name, ["parenttype", "parent"], as_dict=True)

	for dt, link in linkinfo.items():
		filters = []
		link["doctype"] = dt
		link_meta_bundle = vmraid.desk.form.load.get_meta_bundle(dt)
		linkmeta = link_meta_bundle[0]
		if not linkmeta.get("issingle"):
			fields = [d.fieldname for d in linkmeta.get("fields", {
				"in_list_view": 1,
				"fieldtype": ["not in", ("Image", "HTML", "Button") + vmraid.model.table_fields]
			...
			try:
				if link.get("filters"):
					ret = vmraid.get_list(doctype=dt, fields=fields, filters=link.get("filters"))

				elif link.get("get_parent"):
					if me and me.parent and me.parenttype == dt:
						ret = vmraid.get_list(doctype=dt, fields=fields,
					...
					ret = vmraid.get_list(doctype=dt, fields=fields, filters=filters, or_filters=or_filters, distinct=True)
					...
					if link_fieldnames:
						...
						ret = vmraid.get_list(doctype=dt, fields=fields, filters=filters, or_filters=or_filters)
...
def get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	if(without_ignore_user_permissions_enabled):
		return vmraid.cache().hget("linked_doctypes_without_ignore_user_permissions_enabled",
			doctype, lambda: _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled))
	else:
		return vmraid.cache().hget("linked_doctypes", doctype, lambda: _get_linked_doctypes(doctype))
...
def _get_linked_doctypes(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = vmraid.get_all("DocField", fields=["parent as dt"], filters=filters)
	links+= vmraid.get_all("Custom Field", fields=["dt"], filters=filters)
...
def get_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	# find links of parents
	links = vmraid.get_all("DocField", fields=["parent", "fieldname"], filters=filters, as_list=1)
	links+= vmraid.get_all("Custom Field", fields=["dt as parent", "fieldname"], filters=filters, as_list=1)
	...
	table_doctypes = vmraid.get_all("DocType", filters=[["istable", "=", "1"], ["name", "in", tuple(links_dict)]])
	child_filters = [['fieldtype','in', vmraid.model.table_fields], ['options', 'in', tuple(doctype.name for doctype in table_doctypes)]]
	...
	for parent, options in vmraid.get_all("DocField", fields=["parent", "options"], filters=child_filters, as_list=1):
...
def get_dynamic_linked_fields(doctype, without_ignore_user_permissions_enabled=False):
	...
	links = vmraid.get_all("DocField", fields=["parent as doctype", "fieldname", "options as doctype_fieldname"], filters=filters)
	links+= vmraid.get_all("Custom Field", fields=["dt as doctype", "fieldname", "options as doctype_fieldname"], filters=filters)

	for df in links:
		if is_single(df.doctype): continue

		# optimized to get both link exists and parenttype
		possible_link = vmraid.get_all(df.doctype, filters={df.doctype_fieldname: doctype},
