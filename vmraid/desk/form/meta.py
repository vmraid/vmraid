...
from vmraid.modules import scrub, get_module_path, load_doctype_module
from vmraid.utils import get_html_format
from vmraid.translate import make_dict_from_messages, extract_messages_from_code
from vmraid.model.utils import render_include
from vmraid.build import scrub_html_template
...
def get_meta(doctype, cached=True):
	...
	if cached and not vmraid.conf.developer_mode:
		meta = vmraid.cache().hget("form_meta", doctype)
		if meta:
			meta = FormMeta(meta)
		else:
			meta = FormMeta(doctype)
			vmraid.cache().hset("form_meta", doctype, meta.as_dict())
	else:
		meta = FormMeta(doctype)

	if vmraid.local.lang != 'en':
		meta.set_translations(vmraid.local.lang)
...
class FormMeta(Meta):
	...
	def add_code(self):
		...
		system_country = vmraid.get_system_settings("country")
	...
	def add_custom_script(self):
		...
		# custom script
		client_scripts = vmraid.db.get_all("Client Script",
	...
	def add_search_fields(self):
		...
		for df in self.get("fields", {"fieldtype": "Link", "options":["!=", "[Select]"]}):
			if df.options:
				search_fields = vmraid.get_meta(df.options).search_fields
				if search_fields:
	...
	def add_linked_document_type(self):
		for df in self.get("fields", {"fieldtype": "Link"}):
			if df.options:
				try:
					df.linked_document_type = vmraid.get_meta(df.options).document_type
				except vmraid.DoesNotExistError:
					# edge case where options="[Select]"
					pass

	def load_print_formats(self):
		print_formats = vmraid.db.sql("""select * FROM `tabPrint Format`
	...
	def load_workflows(self):
		...
		if workflow_name and vmraid.db.exists("Workflow", workflow_name):
			workflow = vmraid.get_doc("Workflow", workflow_name)
			workflow_docs.append(workflow)

			for d in workflow.get("states"):
				workflow_docs.append(vmraid.get_doc("Workflow State", d.state))
		...
		if not self.custom:
			...
			if hasattr(module, "form_grid_templates"):
				for key, path in iteritems(module.form_grid_templates):
					templates[key] = get_html_format(vmraid.get_app_path(app, path))

				self.set("__form_grid_templates", templates)

	def set_translations(self, lang):
		self.set("__messages", vmraid.get_lang_dict("doctype", self.name))
...
def get_code_files_via_hooks(hook, name):
	...
	for app_name in vmraid.get_installed_apps():
		code_hook = vmraid.get_hooks(hook, default={}, app_name=app_name)
		if not code_hook:
			continue
		...
		for file in files:
			path = vmraid.get_app_path(app_name, *file.strip("/").split("/"))
			code_files.append(path)

	return code_files

def get_js(path):
	js = vmraid.read_file(path)
	if js:
