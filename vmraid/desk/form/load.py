...
def getdoc(doctype, name, user=None):
	...
	try:
		...
		if not doc.has_permission("read"):
			vmraid.flags.error_message = _('Insufficient Permission for {0}').format(vmraid.bold(doctype + ' ' + name))
...
def get_milestones(doctype, name):
	return vmraid.db.get_all('Milestone', fields = ['creation', 'owner', 'track_field', 'value'],
		filters=dict(reference_type=doctype, reference_name=name))

def get_attachments(dt, dn):
	return vmraid.get_all("File", fields=["name", "file_name", "file_url", "is_private"],
		filters = {"attached_to_name": dn, "attached_to_doctype": dt})

def get_versions(doc):
	return vmraid.get_all('Version', filters=dict(ref_doctype=doc.doctype, docname=doc.name),
...
def get_comments(doctype, name, comment_type='Comment'):
	...
	comments = vmraid.get_all('Comment', fields = ['name', 'creation', 'content', 'owner', 'comment_type'], filters=dict(
...
def get_point_logs(doctype, docname):
	return vmraid.db.get_all('Energy Point Log', filters={
		'reference_doctype': doctype,
...
def _get_communications(doctype, name, start=0, limit=20):
	...
	for c in communications:
		if c.communication_type=="Communication":
			c.attachments = json.dumps(vmraid.get_all("File",
...
def get_view_logs(doctype, docname):
	...
	if hasattr(vmraid.get_meta(doctype), 'track_views') and vmraid.get_meta(doctype).track_views:
		view_logs = vmraid.get_all("View Log", filters={
			"reference_doctype": doctype,
			"reference_name": docname,
...
def get_tags(doctype, name):
	tags = [tag.tag for tag in vmraid.get_all("Tag Link", filters={
...
def get_automatic_email_link():
	return vmraid.db.get_value("Email Account", {"enable_incoming": 1, "enable_automatic_linking": 1}, "email_id")
