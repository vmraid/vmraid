...
def get_notifications():
	...
	if vmraid.flags.in_install or not vmraid.db.get_single_value("System Settings", "setup_complete"):
		return out
...
def get_open_count(doctype, name, items=None):
	...
	for d in items:
		...
		if filters:
			...
			total = len(
				vmraid.get_all(d, fields="name", filters=filters, limit=100, distinct=True, ignore_ifnull=True)
			)
