...
vmraid.ui.form.on('Dashboard Chart', {
	setup: function(frm) {
	...
	before_save: function(frm) {
		...
		static_filters =
			vmraid.dashboard_utils.remove_common_static_filter_values(static_filters, dynamic_filters);
	...
	refresh: function(frm) {
		...
		frm.is_disabled = !vmraid.boot.developer_mode && frm.doc.is_standard;
		...
		frm.add_custom_button('Add Chart to Dashboard', () => {
			const dialog = vmraid.dashboard_utils.get_add_to_dashboard_dialog(
				frm.doc.name,
				'Dashboard Chart',
				'vmraid.desk.doctype.dashboard_chart.dashboard_chart.add_chart_to_dashboard'
			);

			if (!frm.doc.chart_name) {
				vmraid.msgprint(__('Please create chart first'));
			} else {
				dialog.show();
			}
		});
	...
	is_standard: function(frm) {
		if (vmraid.boot.developer_mode && frm.doc.is_standard) {
	...
	set_heatmap_year_options: function(frm) {
		if (frm.doc.type == 'Heatmap') {
			vmraid.db.get_doc('System Settings').then(doc => {
				const creation_date = doc.creation;
				frm.set_df_property('heatmap_year', 'options', vmraid.dashboard_utils.get_years_since_creation(creation_date));
			});
		}
	},
	...
	set_chart_report_filters: function(frm) {
		...
		if (report_name) {
			if (frm.doc.filters_json.length > 2) {
				...
				vmraid.report_utils.get_report_filters(report_name).then(filters => {
					if (filters) {
						frm.chart_filters = filters;
						let filter_values = vmraid.report_utils.get_filter_values(filters);
	...
	set_chart_field_options: function(frm) {
		...
		if (frm.doc.dynamic_filters_json && frm.doc.dynamic_filters_json.length > 2) {
			filters = vmraid.dashboard_utils.get_all_filters(frm.doc);
		}
		vmraid.xcall(
			'vmraid.desk.query_report.run',
			{
			...
			if (!frm.doc.use_report_chart) {
				if (data.result.length) {
					frm.field_options = vmraid.report_utils.get_field_options_from_report(data.columns, data);
					frm.set_df_property('x_field', 'options', frm.field_options.non_numeric_fields);
					if (!frm.field_options.numeric_fields.length) {
						vmraid.msgprint(__("Report has no numeric fields, please change the Report Name"));
					} else {
						let y_field_df = vmraid.meta.get_docfield('Dashboard Chart Field', 'y_field', frm.doc.name);
						y_field_df.options = frm.field_options.numeric_fields;
					}
				} else {
					vmraid.msgprint(__('Report has no data, please modify the filters or change the Report Name'));
				}
			} else {
	...
	update_options: function(frm) {
		...
		if (doctype) {
			vmraid.model.with_doctype(doctype, () => {
				...
				vmraid.get_meta(doctype).fields.map(df => {
	...
	show_filters: function(frm) {
		frm.chart_filters = [];
		vmraid.dashboard_utils.get_filters_for_chart_type(frm.doc).then(filters => {
			if (filters) {
				frm.chart_filters = filters;
			}
	...
	render_filters_table: function(frm) {
		...
		table.on('click', () => {
			frm.is_disabled && vmraid.throw(__('Cannot edit filters for standard charts'));
			...
			vmraid.dashboards.filters_dialog = dialog;

			if (is_document_type) {
				frm.filter_group = new vmraid.ui.FilterGroup({
			...
			if (frm.doc.chart_type == 'Report') {
				...
				vmraid.query_report = new vmraid.views.QueryReport({'filters': dialog.fields_list});
				vmraid.query_reports[frm.doc.report_name]
					&& vmraid.query_reports[frm.doc.report_name].onload
						&& vmraid.query_reports[frm.doc.report_name].onload(vmraid.query_report);
			}
	...
	render_dynamic_filters_table(frm) {
		...
		let fields = vmraid.dashboard_utils.get_fields_for_dynamic_filter_dialog(
	...
	set_parent_document_type: async function(frm) {
		...
		let doc_is_table = document_type &&
			(await vmraid.db.get_value('DocType', document_type, 'istable')).message.istable;

		frm.set_df_property('parent_document_type', 'hidden', !doc_is_table);

		if (document_type && doc_is_table) {
			let parent = await vmraid.db.get_list('DocField', {
				filters: {
					'fieldtype': 'Table',
