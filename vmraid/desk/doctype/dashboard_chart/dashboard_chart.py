...
from vmraid.utils import cint, get_datetime, getdate, now_datetime, nowdate
from vmraid.utils.dashboard import cache_source
from vmraid.utils.dateutils import (
...
def get_permission_query_conditions(user):
	...
	allowed_doctypes = [
		vmraid.db.escape(doctype) for doctype in vmraid.permissions.get_doctypes_with_read()
	]
	allowed_reports = [
		vmraid.db.escape(key) if type(key) == str else key.encode("UTF8")
		for key in get_allowed_reports()
	]
	allowed_modules = [
		vmraid.db.escape(module.get("module_name")) for module in get_modules_from_all_apps_for_user()
	]
...
def has_permission(doc, ptype, user):
	...
	if doc.chart_type == "Report":
		...
		allowed_doctypes = vmraid.permissions.get_doctypes_with_read()
...
def get(
	...
	filters = vmraid.parse_json(filters) or vmraid.parse_json(chart.filters_json)
	if not filters:
		filters = []
...
def create_dashboard_chart(args):
	...
	doc = vmraid.new_doc("Dashboard Chart")
	...
	if vmraid.db.exists("Dashboard Chart", args.chart_name):
...
def add_chart_to_dashboard(args):
	...
	dashboard_link = vmraid.new_doc("Dashboard Chart Link")
	dashboard_link.chart = args.chart_name or args.name

	if args.set_standard and dashboard.is_standard:
		chart = vmraid.get_doc("Dashboard Chart", dashboard_link.chart)
		chart.is_standard = 1
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	or_filters = {"owner": vmraid.session.user, "is_public": 1}
...
class DashboardChart(Document):
	def on_update(self):
		vmraid.cache().delete_key("chart-data:{}".format(self.name))
	...
	def validate(self):
		if not vmraid.conf.developer_mode and self.is_standard:
			vmraid.throw(_("Cannot edit Standard charts"))
	...
	def check_required_field(self):
		if not self.document_type:
			vmraid.throw(_("Document type is required to create a dashboard chart"))

		if (
			self.document_type
			and vmraid.get_meta(self.document_type).istable
			and not self.parent_document_type
		):
			vmraid.throw(_("Parent document type is required to create a dashboard chart"))

		if self.chart_type == "Group By":
			if not self.group_by_based_on:
				vmraid.throw(_("Group By field is required to create a dashboard chart"))
			if self.group_by_type in ["Sum", "Average"] and not self.aggregate_function_based_on:
				vmraid.throw(_("Aggregate Function field is required to create a dashboard chart"))
		else:
			if not self.based_on:
				vmraid.throw(_("Time series based on is required to create a dashboard chart"))

	def check_document_type(self):
		if vmraid.get_meta(self.document_type).issingle:
			vmraid.throw(_("You cannot create a dashboard chart from single DocTypes"))
	...
	def validate_custom_options(self):
		if self.custom_options:
			try:
				...
				vmraid.throw(_("Invalid json added in the custom options: {0}").format(error))
