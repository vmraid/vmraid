...
from vmraid.utils import nowdate, getdate, get_datetime, cint, now_datetime
...
def get_permission_query_conditions(user):
	...
	allowed_reports = [vmraid.db.escape(key) if type(key) == str else key.encode('UTF8') for key in get_allowed_reports()]
...
def get_charts_for_user(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.db.get_list('Dashboard Chart',
		fields=['name'],
		filters=filters,
...
class DashboardChart(Document):
	...
	def validate(self):
		if not vmraid.conf.developer_mode and self.is_standard:
			vmraid.throw('Cannot edit Standard charts')
