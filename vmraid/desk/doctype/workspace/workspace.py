...
class Workspace(Document):
	def validate(self):
		if (self.is_standard and not vmraid.conf.developer_mode and not disable_saving_as_standard()):
			vmraid.throw(_("You need to be in developer mode to edit this document"))
		validate_route_conflict(self.doctype, self.name)

		duplicate_exists = vmraid.db.exists("Workspace", {
			"name": ["!=", self.name], 'is_default': 1, 'extends': self.extends
		})

		if self.is_default and self.name and duplicate_exists:
			vmraid.throw(_("You can only have one default page that extends a particular standard page."))
	...
	def get_module_page_map():
		...
		pages = vmraid.get_all("Workspace", fields=["name", "module"], filters=filters, as_list=1)
...
def disable_saving_as_standard():
	return vmraid.flags.in_install or \
			vmraid.flags.in_patch or \
			vmraid.flags.in_test or \
			vmraid.flags.in_fixtures or \
