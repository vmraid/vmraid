...
class KanbanBoard(Document):
	...
	def on_change(self):
		...
		vmraid.cache().delete_keys("_user_settings")
	...
	def validate_column_name(self):
		for column in self.columns:
			if not column.column_name:
				vmraid.msgprint(_("Column Name cannot be empty"), raise_exception=True)
...
def update_order(board_name, order):
	...
	for col_name, cards in order_dict.items():
		for card in cards:
			column = vmraid.get_value(doctype, {"name": card}, fieldname)
			if column != col_name:
...
def save_settings(board_name: str, settings: str) -> Document:
	...
	resp = doc.as_dict()
	resp["fields"] = vmraid.parse_json(resp["fields"])
