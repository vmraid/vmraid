...
from vmraid.desk.doctype.notification_settings.notification_settings import (is_notifications_enabled, is_email_notifications_enabled_for_type, set_seen_value)
...
def get_title(doctype, docname, title_field=None):
	...
	title = docname if title_field == "name" else \
		vmraid.db.get_value(doctype, docname, title_field)
	return title
...
def mark_all_as_read():
	unread_docs_list = vmraid.db.get_all('Notification Log', filters = {'read': 0, 'for_user': vmraid.session.user})
