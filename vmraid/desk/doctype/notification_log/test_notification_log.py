...
class TestNotificationLog(unittest.TestCase):
	...
	def test_share(self):
		...
		content = "Subject: {} shared a document ToDo".format(
			vmraid.utils.get_fullname(vmraid.session.user)
		)
		self.assertTrue(content in email.message)


def get_last_email_queue():
	res = vmraid.db.get_all("Email Queue", fields=["message"], order_by="creation desc", limit=1)
	return res[0]
