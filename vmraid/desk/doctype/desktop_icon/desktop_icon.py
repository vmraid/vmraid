...
def after_doctype_insert():
	vmraid.db.add_unique('Desktop Icon', ('module_name', 'owner', 'standard'))
...
def get_desktop_icons(user=None):
	...
	user_icons = vmraid.cache().hget('desktop_icons', user)
	...
	if not user_icons:
		...
		user_icons = vmraid.db.get_all('Desktop Icon', fields=fields,
		...
		user_blocked_modules = vmraid.get_doc('User', user).get_blocked_modules()
		for icon in user_icons:
		...
		vmraid.cache().hset('desktop_icons', user, user_icons)
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type='link', standard=0):
	...
	icon_name = vmraid.db.exists('Desktop Icon', {'standard': standard, 'link': link,
		'owner': vmraid.session.user})

	if icon_name:
		if vmraid.db.get_value('Desktop Icon', icon_name, 'hidden'):
			# if it is hidden, unhide it
			vmraid.db.set_value('Desktop Icon', icon_name, 'hidden', 0)
			clear_desktop_icons_cache()

	else:
		idx = vmraid.db.sql('select max(idx) from `tabDesktop Icon` where owner=%s',
			vmraid.session.user)[0][0] or \
			vmraid.db.sql('select count(*) from `tabDesktop Icon` where standard=1')[0][0]

		if not vmraid.db.get_value("Report", _report):
			_report = None
			userdefined_icon = vmraid.db.get_value('DocType', _doctype, ['icon','color','module'], as_dict=True)
		else:
			userdefined_icon = vmraid.db.get_value('Report', _report, ['icon','color','module'], as_dict=True)

		module_icon = vmraid.get_value('Desktop Icon', {'standard':1, 'module_name':userdefined_icon.module},
			['name', 'icon', 'color', 'reverse'], as_dict=True)

		if not module_icon:
			module_icon = vmraid._dict()
			opts = random.choice(palette)
			module_icon.color = opts[0]
			module_icon.reverse = 0 if (len(opts) > 1) else 1

		try:
			new_icon = vmraid.get_doc({
		...
		except vmraid.UniqueValidationError as e:
			vmraid.throw(_('Desktop Icon already exists'))
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ('Explore',):
			if user:
				...
				name = vmraid.db.get_value('Desktop Icon',
					{'standard': 1, 'module_name': module_name})
				if name:
					icon = vmraid.get_doc('Desktop Icon', name)
				else:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(vmraid.defaults.get_defaults().setup_complete or 0):
		vmraid.db.sql('delete from `tabDesktop Icon` where standard=0')

	# set standard as blocked and hidden if setting first active domain
	if not vmraid.flags.keep_desktop_icons:
		vmraid.db.sql('update `tabDesktop Icon` set blocked=0, hidden=1 where standard=1')

	# set as visible if present, or add icon
	for module_name in visible_list:
		name = vmraid.db.get_value('Desktop Icon', {'module_name': module_name})
		if name:
			vmraid.db.set_value('Desktop Icon', name, 'hidden', 0)
		else:
			if vmraid.db.exists('DocType', module_name):
				try:
...
def set_hidden(module_name, user=None, hidden=1):
	...
	if user:
		...
		if hidden and icon.custom:
			vmraid.delete_doc(icon.doctype, icon.name, ignore_permissions=True)
			return

		# hidden by user
		icon.db_set('hidden', hidden)
	else:
		icon = vmraid.get_doc('Desktop Icon', {'standard': 1, 'module_name': module_name})

		# blocked is globally hidden
		icon.db_set('blocked', hidden)

def get_all_icons():
	return [d.module_name for d in vmraid.get_all('Desktop Icon',
		filters={'standard': 1}, fields=['module_name'])]

def clear_desktop_icons_cache(user=None):
	vmraid.cache().hdel('desktop_icons', user or vmraid.session.user)
	vmraid.cache().hdel('bootinfo', user or vmraid.session.user)
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = vmraid.db.get_value('Desktop Icon',
		{'module_name': module_name, 'owner': user, 'standard': 0})

	if desktop_icon_name:
		return vmraid.get_doc('Desktop Icon', desktop_icon_name)
	else:
...
def make_user_copy(module_name, user):
	...
	standard_name = vmraid.db.get_value('Desktop Icon', {'module_name': module_name, 'standard': 1})

	if not standard_name:
		vmraid.throw(_('{0} not found').format(module_name), vmraid.DoesNotExistError)

	original = vmraid.get_doc('Desktop Icon', standard_name)

	desktop_icon = vmraid.get_doc({
		'doctype': 'Desktop Icon',
		'standard': 0,
		'owner': user,
...
def sync_from_app(app):
	...
	try:
		modules = vmraid.get_attr(app + '.config.desktop.get_data')() or {}
	except ImportError:
		return []
	...
	for i, m in enumerate(modules_list):
		...
		if desktop_icon_name:
			desktop_icon = vmraid.get_doc('Desktop Icon', desktop_icon_name)
		else:
			# new icon
			desktop_icon = vmraid.get_doc({
				'doctype': 'Desktop Icon',
		...
		try:
			desktop_icon.save()
		except vmraid.exceptions.UniqueValidationError:
			pass
...
def update_icons(hidden_list, user=None):
	...
	vmraid.msgprint(vmraid._('Updated'), indicator='green', title=_('Success'), alert=True)

def get_context(context):
	context.icons = get_user_icons(vmraid.session.user)
	context.user = vmraid.session.user

	if 'System Manager' in vmraid.get_roles():
		context.users = vmraid.db.get_all('User', filters={'user_type': 'System User', 'enabled': 1},
...
def get_module_icons(user=None):
	...
	if not user:
		icons = vmraid.db.get_all('Desktop Icon',
			fields='*', filters={'standard': 1}, order_by='idx')
	else:
		vmraid.cache().hdel('desktop_icons', user)
		icons = get_user_icons(user)

	for icon in icons:
		icon.value = vmraid.db.escape(_(icon.label or icon.module_name))
...
def get_user_icons(user):
	...
	from vmraid.boot import get_allowed_pages
