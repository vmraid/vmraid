...
def get_desktop_icons(user=None):
	...
	if not user_icons:
		...
		blocked_doctypes = vmraid.get_all(
			"DocType",
		...
		standard_icons = vmraid.db.get_all("Desktop Icon", fields=fields, filters={"standard": 1})
...
def add_user_icon(_doctype, _report=None, label=None, link=None, type="link", standard=0):
	...
	icon_name = vmraid.db.exists(
		"Desktop Icon", {"standard": standard, "link": link, "owner": vmraid.session.user}
	)
	...
	if icon_name:
		...
		idx = (
			vmraid.db.sql("select max(idx) from `tabDesktop Icon` where owner=%s", vmraid.session.user)[0][
				0
			]
			or vmraid.db.sql("select count(*) from `tabDesktop Icon` where standard=1")[0][0]
		)

		if not vmraid.db.get_value("Report", _report):
			_report = None
			userdefined_icon = vmraid.db.get_value(
	...
	for i, module_name in enumerate(new_order):
		if module_name not in ("Explore",):
			if user:
				...
				name = vmraid.db.get_value("Desktop Icon", {"standard": 1, "module_name": module_name})
				if name:
...
def set_desktop_icons(visible_list, ignore_duplicate=True):
	...
	if not int(vmraid.defaults.get_defaults().setup_complete or 0):
		vmraid.db.delete("Desktop Icon", {"standard": 0})
...
def get_all_icons():
	return [
		d.module_name
		for d in vmraid.get_all("Desktop Icon", filters={"standard": 1}, fields=["module_name"])
	]
...
def get_user_copy(module_name, user=None):
	...
	desktop_icon_name = vmraid.db.get_value(
...
def get_context(context):
	...
	if "System Manager" in vmraid.get_roles():
		context.users = vmraid.db.get_all(
			"User",
...
def get_module_icons(user=None):
	...
	if not user:
		icons = vmraid.db.get_all("Desktop Icon", fields="*", filters={"standard": 1}, order_by="idx")
	else:
