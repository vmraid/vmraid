...
class GlobalSearchSettings(Document):
	...
	def validate(self):
		...
		for dt in self.allowed_in_global_search:
			...
			if vmraid.get_meta(dt.document_type).module == "Core":
				core_dts.append(dt.document_type)

			dts.append(dt.document_type)

		if core_dts:
			core_dts = (", ".join([vmraid.bold(dt) for dt in core_dts]))
			vmraid.throw(_("Core Modules {0} cannot be searched in Global Search.").format(core_dts))

		if repeated_dts:
			repeated_dts = (", ".join([vmraid.bold(dt) for dt in repeated_dts]))
			vmraid.throw(_("Document Type {0} has been repeated.").format(repeated_dts))

		# reset cache
		vmraid.cache().hdel('global_search', 'search_priorities')

def get_doctypes_for_global_search():
	def get_from_db():
		doctypes = vmraid.get_list("Global Search DocType", fields=["document_type"], order_by="idx ASC")
		return [d.document_type for d in doctypes] or []

	return vmraid.cache().hget("global_search", "search_priorities", get_from_db)
...
def update_global_search_doctypes():
	...
	installed_apps = [app for app in vmraid.get_installed_apps() if app]
	active_domains = [domain for domain in vmraid.get_active_domains() if domain]
	active_domains.append("Default")

	for app in installed_apps:
		search_doctypes = vmraid.get_hooks(hook="global_search_doctypes", app_name=app)
	...
	doctype_list = set([dt.name for dt in vmraid.get_all("DocType")])
	...
	global_search_settings = vmraid.get_single("Global Search Settings")
...
def show_message(progress, msg):
	vmraid.publish_realtime('global_search_settings', {"progress":progress, "total":3, "msg": msg}, user=vmraid.session.user)
