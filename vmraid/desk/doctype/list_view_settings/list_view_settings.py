...
def save_listview_settings(doctype, listview_settings, removed_listview_fields):
	...
	return {"meta": vmraid.get_meta(doctype, False), "listview_settings": doc}
...
def set_listview_fields(doctype, listview_fields, removed_listview_fields):
	...
	listview_fields = [
		f.get("fieldname") for f in vmraid.parse_json(listview_fields) if f.get("fieldname")
	]
...
def get_default_listview_fields(doctype):
	meta = vmraid.get_meta(doctype)
	path = vmraid.get_module_path(
		vmraid.scrub(meta.module), "doctype", vmraid.scrub(meta.name), vmraid.scrub(meta.name) + ".json"
	)
