...
from vmraid.utils import (getdate, cint, add_months, date_diff, add_days,
...
class Event(Document):
	def validate(self):
		...
		if self.repeat_on == "Daily" and self.ends_on and getdate(self.starts_on) != getdate(self.ends_on):
			vmraid.throw(_("Daily Events should finish on the Same Day."))

		if self.sync_with_google_calendar and not self.google_calendar:
			vmraid.throw(_("Select Google Calendar to which event should be synced."))

	def on_update(self):
		self.sync_communication()

	def on_trash(self):
		communications = vmraid.get_all("Communication", dict(reference_doctype=self.doctype, reference_name=self.name))
		if communications:
			for communication in communications:
				vmraid.delete_doc_if_exists("Communication", communication.name)
	...
	def sync_communication(self):
		if self.event_participants:
			for participant in self.event_participants:
				...
				comms = vmraid.get_all("Communication", filters=filters, fields=["name"])

				if comms:
					for comm in comms:
						communication = vmraid.get_doc("Communication", comm.name)
						self.update_communication(participant, communication)
				else:
					meta = vmraid.get_meta(participant.reference_doctype)
	...
	def update_communication(self, participant, communication):
		...
		communication.sender_full_name = vmraid.utils.get_fullname(self.owner)
		communication.reference_doctype = self.doctype
		communication.reference_name = self.name
...
def delete_communication(event, reference_doctype, reference_docname):
	deleted_participant = vmraid.get_doc(reference_doctype, reference_docname)
	...
	comms = vmraid.get_list("Communication", filters=filters, fields=["name"])

	if comms:
		deletion = []
		for comm in comms:
			delete = vmraid.get_doc("Communication", comm.name).delete()
...
def send_event_digest():
	...
	for user in get_enabled_system_users():
		...
		if events:
			vmraid.set_user_lang(user.name, user.language)
			...
			vmraid.sendmail(
				...
				subject=vmraid._("Upcoming Events for Today"),
				template="upcoming_events",
				args={
					'events': events,
				},
				header=[vmraid._("Events in Today's Calendar"), 'blue']
			)
...
def delete_events(ref_type, ref_name, delete_event=False):
	participations = vmraid.get_all("Event Participants", filters={"reference_doctype": ref_type, "reference_docname": ref_name,
		"parenttype": "Event"}, fields=["parent", "name"])

	if participations:
		for participation in participations:
			if delete_event:
				vmraid.delete_doc("Event", participation.parent, for_reload=True)
			else:
				total_participants = vmraid.get_all("Event Participants", filters={"parenttype": "Event", "parent": participation.parent})

				if len(total_participants) <= 1:
					vmraid.db.sql("DELETE FROM `tabEvent` WHERE `name` = %(name)s", {'name': participation.parent})

				vmraid.db.sql("DELETE FROM `tabEvent Participants ` WHERE `name` = %(name)s", {'name': participation.name})

# Close events if ends_on or repeat_till is less than now_datetime
def set_status_of_events():
	events = vmraid.get_list("Event", filters={"status": "Open"}, fields=["name", "ends_on", "repeat_till"])
	for event in events:
		if (event.ends_on and getdate(event.ends_on) < getdate(nowdate())) \
			...
			vmraid.db.set_value("Event", event.name, "status", "Closed")
