...
def get_report_doc(report_name):
	...
	if doc.report_type == "Custom Report":
		...
		doc = vmraid.get_doc("Report", reference_report)
	...
	if not vmraid.has_permission(doc.ref_doctype, "report"):
		vmraid.throw(
	...
	if doc.disabled:
		vmraid.throw(_("Report {0} is disabled").format(report_name))
...
def generate_report_result(
	...
	return {
		...
		"status": None,
		"execution_time": vmraid.cache().hget("report_execution_time", report.name) or 0,
	}
...
def get_script(report_name):
	report = get_report_doc(report_name)
	module = report.module or vmraid.db.get_value("DocType", report.ref_doctype, "module")

	is_custom_module = vmraid.get_cached_value("Module Def", module, "custom")
	...
	if not script:
		script = "vmraid.query_reports['%s']={}" % report_name

	# load translations
	if vmraid.lang != "en":
		send_translations(vmraid.get_lang_dict("report", report_name))

	return {
		"script": render_include(script),
		"html_format": html_format,
		"execution_time": vmraid.cache().hget("report_execution_time", report_name) or 0,
	}
...
def run(
	...
	if not user:
		user = vmraid.session.user
	if not vmraid.has_permission(report.ref_doctype, "report"):
		vmraid.msgprint(
...
def get_prepared_report_result(report, filters, dn="", user=None):
	...
	if dn:
		# Get specified dn
		doc = vmraid.get_doc("Prepared Report", dn)
	else:
		...
		if doc_list:
			# Get latest
			doc = vmraid.get_doc("Prepared Report", doc_list[0])

	if doc:
		try:
			# Prepared Report data is stored in a GZip compressed JSON file
			attached_file_name = vmraid.db.get_value(
				"File",
				{"attached_to_doctype": doc.doctype, "attached_to_name": doc.name},
				"name",
			)
			attached_file = vmraid.get_doc("File", attached_file_name)
			...
			vmraid.delete_doc("Prepared Report", doc.name)
			vmraid.db.commit()
			doc = None
...
def export_query():
	...
	if data.get("report_name"):
		...
		vmraid.permissions.can_export(
			vmraid.get_cached_value("Report", report_name, "ref_doctype"),
			raise_exception=True,
		)

	file_format_type = data.get("file_format_type")
	custom_columns = vmraid.parse_json(data.get("custom_columns", "[]"))
	...
	if file_format_type == "Excel":
		...
		vmraid.response["filename"] = report_name + ".xlsx"
		vmraid.response["filecontent"] = xlsx_file.getvalue()
		vmraid.response["type"] = "binary"


def format_duration_fields(data: vmraid._dict) -> None:
...
def get_data_for_custom_field(doctype, field):
	...
	if not vmraid.has_permission(doctype, "read"):
		vmraid.throw(_("Not Permitted"), vmraid.PermissionError)

	value_map = vmraid._dict(vmraid.get_all(doctype, fields=["name", field], as_list=1))
...
def save_report(reference_report, report_name, columns):
	...
	if docname:
		report = vmraid.get_doc("Report", docname)
		...
		report.save()
		vmraid.msgprint(_("Report updated successfully"))

		return docname
	else:
		new_report = vmraid.get_doc(
			{
				"doctype": "Report",
				"report_name": report_name,
		...
		vmraid.msgprint(_("{0} saved successfully").format(new_report.name))
...
def get_filtered_data(ref_doctype, columns, data, user):
	...
	shared = vmraid.share.get_shared(ref_doctype, user)
	columns_dict = get_columns_dict(columns)

	role_permissions = get_role_permissions(vmraid.get_meta(ref_doctype), user)
			...
			for match_filters in filter_list:
				...
				for dt, idx in linked_doctypes.items():
					...
					if (
						...
						and vmraid.db.exists(dt, cell_value)
					):
						match = False
						break
...
def get_column_as_dict(col):
	...
	if isinstance(col, str):
		...
		col_dict["fieldname"] = vmraid.scrub(col[0])

	# dict
	else:
		col_dict.update(col)
		if "fieldname" not in col_dict:
			col_dict["fieldname"] = vmraid.scrub(col_dict["label"])
...
def get_user_match_filters(doctypes, user):
	...
	for dt in doctypes:
		filter_list = vmraid.desk.reportview.build_match_conditions(dt, user, False)
		if filter_list:
