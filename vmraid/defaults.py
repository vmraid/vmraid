...
def get_user_permissions(user=None):
	from vmraid.core.doctype.user_permission.user_permission \
...
def clear_default(key=None, value=None, parent=None, name=None, parenttype=None):
	...
	vmraid.db.sql("""delete from tabDefaultValue where {0}""".format(" and ".join(conditions)),
		tuple(values))
...
def get_defaults_for(parent="__default"):
	...
	if defaults==None:
		...
		res = vmraid.db.sql("""select defkey, defvalue from `tabDefaultValue`
