...
vmraid.ui.form.on('DocType Layout', {
		...
		frm.set_fields_as_options('fields', frm.doc.document_type, null, [], 'fieldname').then(() => {
			...
			if (frm.doc.document_type) {
				if (!(frm.doc.fields || []).length) {
					for (let f of vmraid.get_doc('DocType', frm.doc.document_type).fields) {
	...
	set_button(frm) {
		if (!frm.is_new()) {
			frm.add_custom_button(__('Go to {0} List', [frm.doc.name]), () => {
				window.open(`/app/list/${vmraid.router.slug(frm.doc.name)}/list`);
			});
		}
	}
});
