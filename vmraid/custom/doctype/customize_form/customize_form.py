...
from vmraid.model import no_value_fields, core_doctypes_list
from vmraid.core.doctype.doctype.doctype import validate_fields_for_doctype, check_email_append_to
from vmraid.custom.doctype.custom_field.custom_field import create_custom_field
from vmraid.custom.doctype.property_setter.property_setter import delete_property_setter
from vmraid.model.docfield import supports_translation
from vmraid.core.doctype.doctype.doctype import validate_series

class CustomizeForm(Document):
	def on_update(self):
		vmraid.db.sql("delete from tabSingles where doctype='Customize Form'")
		vmraid.db.sql("delete from `tabCustomize Form Field`")
	...
	def validate_doctype(self, meta):
		...
		if self.doc_type in core_doctypes_list:
			vmraid.throw(_("Core DocTypes cannot be customized."))

		if meta.issingle:
			vmraid.throw(_("Single DocTypes cannot be customized."))

		if meta.custom:
			vmraid.throw(_("Only standard DocTypes are allowed to be customized from Customize Form."))
	...
	def get_name_translation(self):
		...
		return vmraid.get_value('Translation', {
				'source_text': self.doc_type,
				'language': vmraid.local.lang or 'en'
	...
	def set_name_translation(self):
		...
		if current:
			if self.label and current.translated_text != self.label:
				vmraid.db.set_value('Translation', current.name, 'translated_text', self.label)
				vmraid.translate.clear_cache()
			else:
				# clear translation
				vmraid.delete_doc('Translation', current.name)

		else:
			if self.label:
				vmraid.get_doc(dict(doctype='Translation',
					source_text=self.doc_type,
					translated_text=self.label,
					language_code=vmraid.local.lang or 'en')).insert()
	...
	def save_customization(self):
		...
		if self.flags.update_db:
			vmraid.db.updatedb(self.doc_type)

		if not hasattr(self, 'hide_success') or not self.hide_success:
			vmraid.msgprint(_("{0} updated").format(_(self.doc_type)), alert=True)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			if not vmraid.db.get_value("DocField",
				{"parent": self.doc_type, "fieldname": df.fieldname}, "allow_on_submit"):
				vmraid.msgprint(_("Row {0}: Not allowed to enable Allow on Submit for standard fields")\
					.format(df.idx))
				return False

		elif prop == "reqd" and \
			((vmraid.db.get_value("DocField",
				{"parent":self.doc_type,"fieldname":df.fieldname}, "reqd") == 1) \
				and (df.get(prop) == 0)):
			vmraid.msgprint(_("Row {0}: Not allowed to disable Mandatory for standard fields")\
					.format(df.idx))
			return False

		elif prop == "in_list_view" and df.get(prop) \
			and df.fieldtype!="Attach Image" and df.fieldtype in no_value_fields:
					vmraid.msgprint(_("'In List View' not allowed for type {0} in row {1}")
			...
			self.flags.update_db = True
				...
				and vmraid.db.get_value("DocField", {"parent": self.doc_type,
			...
			vmraid.msgprint(_("You cannot unset 'Read Only' for field {0}").format(df.label))
			return False

		elif prop == "options" and df.get("fieldtype") not in ALLOWED_OPTIONS_CHANGE:
			vmraid.msgprint(_("You can't set 'Options' for field {0}").format(df.label))
			return False

		elif prop == 'translatable' and not supports_translation(df.get('fieldtype')):
			vmraid.msgprint(_("You can't set 'Translatable' for field {0}").format(df.label))
			return False
	...
	def set_property_setters_for_actions_and_links(self, meta):
		...
		for doctype, fieldname, field_map in (
			...
			for i, d in enumerate(self.get(fieldname) or []):
				d.idx = i
				if vmraid.db.exists(doctype, d.name) and not d.custom:
					# check property and apply property setter
					original = vmraid.get_doc(doctype, d.name)
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			vmraid.db.delete('Property Setter', dict(property=property_name,
				doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		'''
		if items:
			vmraid.db.delete(doctype, dict(parent=self.doc_type, custom=1,
				name=('not in', items)))
		else:
			vmraid.db.delete(doctype, dict(parent=self.doc_type, custom=1))
	...
	def update_custom_fields(self):
		for i, df in enumerate(self.get("fields")):
			if df.get("is_custom_field"):
				if not vmraid.db.exists('Custom Field', {'dt': self.doc_type, 'fieldname': df.fieldname}):
	...
	def update_in_custom_field(self, df, i):
		...
		custom_field = vmraid.get_doc("Custom Field", meta_df[0].name)
		changed = False
		for prop in docfield_properties:
	...
	def delete_custom_fields(self):
		...
		for fieldname in fields_to_remove:
			...
			if df.get("is_custom_field"):
				vmraid.delete_doc("Custom Field", df.name)
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			property_value = vmraid.db.get_value("DocField", {"parent": self.doc_type,
				"fieldname": fieldname}, property_name)
		else:
			if vmraid.db.has_column("DocType", property_name):
				property_value = vmraid.db.get_value("DocType", self.doc_type, property_name)
			else:
				property_value = None
	...
	def validate_fieldtype_change(self, df, old_value, new_value):
		...
		if allowed:
			old_value_length = cint(vmraid.db.type_map.get(old_value)[1])
			new_value_length = cint(vmraid.db.type_map.get(new_value)[1])
			...
			if new_value_length and (old_value_length > new_value_length):
				...
				self.flags.update_db = True
		if not allowed:
			vmraid.throw(_("Fieldtype cannot be changed from {0} to {1} in row {2}").format(old_value, new_value, df.idx))

	def validate_fieldtype_length(self):
		for field in self.check_length_for_fieldtypes:
			df = field.get('df')
			max_length = cint(vmraid.db.type_map.get(df.fieldtype)[1])
			fieldname = df.fieldname
			docs = vmraid.db.sql('''
			...
			label = df.label
			for doc in docs:
				links.append(vmraid.utils.get_link_to_form(self.doc_type, doc.name))
			links_str = ', '.join(links)

			if docs:
				vmraid.throw(_('Value for field {0} is too long in {1}. Length should be lesser than {2} characters')
					.format(
						vmraid.bold(label),
						links_str,
						vmraid.bold(max_length)
...
def reset_customization(doctype):
	setters = vmraid.get_all("Property Setter", filters={
		'doc_type': doctype,
	...
	for setter in setters:
		vmraid.delete_doc("Property Setter", setter)
