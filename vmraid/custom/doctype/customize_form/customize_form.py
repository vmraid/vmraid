...
from vmraid.core.doctype.doctype.doctype import (
...
from vmraid.model import core_doctypes_list, no_value_fields
...
class CustomizeForm(Document):
	def on_update(self):
		vmraid.db.delete("Singles", {"doctype": "Customize Form"})
		vmraid.db.delete("Customize Form Field")
	...
	def get_name_translation(self):
		...
		return vmraid.get_value(
			"Translation",
			{"source_text": self.doc_type, "language": vmraid.local.lang or "en"},
			...
			vmraid.get_doc(
				{
					...
					"language_code": vmraid.local.lang or "en",
				}
			).insert()
			return
	...
	def save_customization(self):
		...
		if self.flags.rebuild_doctype_for_global_search:
			vmraid.enqueue(
				"vmraid.utils.global_search.rebuild_for_doctype", now=True, doctype=self.doc_type
			)
	...
	def allow_property_change(self, prop, meta_df, df):
		if prop == "fieldtype":
			...
			(
				vmraid.db.get_value("DocField", {"parent": self.doc_type, "fieldname": df.fieldname}, "reqd")
				== 1
			)
			and (df.get(prop) == 0)
		):
	...
	def update_order_property_setter(self, has_custom, fieldname):
		...
		if has_custom:
			...
			vmraid.db.delete("Property Setter", dict(property=property_name, doc_type=self.doc_type))
	...
	def clear_removed_items(self, doctype, items):
		...
		"""
		if items:
			vmraid.db.delete(doctype, dict(parent=self.doc_type, custom=1, name=("not in", items)))
		else:
	...
	def get_existing_property_value(self, property_name, fieldname=None):
		...
		if fieldname:
			property_value = vmraid.db.get_value(
		...
		for field in self.check_length_for_fieldtypes:
			...
			if docs:
				vmraid.throw(
					...
					).format(vmraid.bold(label), links_str, vmraid.bold(max_length)),
...
def reset_customization(doctype):
	...
	for field in custom_fields:
		vmraid.delete_doc("Custom Field", field)
