...
def _new_site(
	...
	scheduler_status = (
		"disabled" if vmraid.utils.scheduler.is_scheduler_disabled() else "enabled"
	)
...
def remove_from_installed_apps(app_name):
	...
	if app_name in installed_apps:
		...
		vmraid.db.set_value("DefaultValue", {"defkey": "installed_apps"}, "defvalue", json.dumps(installed_apps))
...
def remove_app(app_name, dry_run=False, yes=False, no_backup=False, force=False):
	...
	for module_name in modules:
		...
		doctypes_with_linked_modules = [
			x for x in all_doctypes_with_linked_modules if vmraid.db.exists("DocType", x)
		]
		for doctype in doctypes_with_linked_modules:
			for record in vmraid.get_all(doctype, filters={"module": module_name}, pluck="name"):
	...
	for doctype in set(drop_doctypes):
		...
		if not dry_run:
			vmraid.db.sql_ddl(f"drop table `tab{doctype}`")
...
def set_all_patches_as_completed(app):
	patch_path = os.path.join(vmraid.get_pymodule_path(app), "patches.txt")
	if os.path.exists(patch_path):
		for patch in vmraid.get_file_items(patch_path):
			vmraid.get_doc({
				"doctype": "Patch Log",
				"patch": patch
				...
				for app in all_apps:
					...
					if app_name == "vmraid":
						...
						if verbose and downgrade:
							print("Your site will be downgraded from VMRaid {0} to {1}".format(current_version, backup_version))
