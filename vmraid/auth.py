...
from vmraid.translate import get_language
from vmraid.twofactor import (
	authenticate_for_2factor,
	confirm_otp_token,
	get_cached_user_pass,
	should_run_2fa,
)
from vmraid.utils import cint, date_diff, datetime, get_datetime, today
...
class HTTPRequest:
	...
	def domain(self):
		if not getattr(self, "_domain", None):
			self._domain = vmraid.request.host
	...
	def validate_csrf_token(self):
		if vmraid.local.request and vmraid.local.request.method in ("POST", "PUT", "DELETE"):
			if not vmraid.local.session:
				return
			if (
				not vmraid.local.session.data.csrf_token
	...
	def set_lang(self):
		vmraid.local.lang = get_language()
	...
	def connect(self):
		...
		vmraid.local.db = vmraid.database.get_db(
...
class LoginManager:
	def __init__(self):
		...
		if (
			vmraid.local.form_dict.get("cmd") == "login" or vmraid.local.request.path == "/api/method/login"
		):
	...
	def login(self):
		...
		if self.force_user_to_reset_password():
			...
			vmraid.local.response["redirect_to"] = doc.reset_password(
	...
	def clear_preferred_language(self):
		vmraid.local.cookie_manager.delete_cookie("preferred_language")

	def make_session(self, resume=False):
		# start session
		vmraid.local.session_obj = Session(
	...
	def clear_active_sessions(self):
		...
		if not (
			cint(vmraid.conf.get("deny_multiple_sessions"))
			or cint(vmraid.db.get_system_setting("deny_multiple_sessions"))
		):
			return
	...
	def force_user_to_reset_password(self):
		...
		if self.user in vmraid.STANDARD_USERS:
			return False

		reset_pwd_after_days = cint(
			vmraid.db.get_single_value("System Settings", "force_user_to_reset_password")
		)

		if reset_pwd_after_days:
			last_password_reset_date = (
				vmraid.db.get_value("User", self.user, "last_password_reset_date") or today()
			)
...
class CookieManager:
	...
	def init_cookies(self):
		if not vmraid.local.session.get("sid"):
			return
...
def clear_cookies():
	if hasattr(vmraid.local, "session"):
		vmraid.session.sid = ""
	vmraid.local.cookie_manager.delete_cookie(
...
def validate_ip_address(user):
	...
	user = (
		vmraid.get_cached_doc("User", user) if not vmraid.flags.in_test else vmraid.get_doc("User", user)
	)
	...
	system_settings = (
		vmraid.get_cached_doc("System Settings")
		if not vmraid.flags.in_test
		else vmraid.get_single("System Settings")
	)
