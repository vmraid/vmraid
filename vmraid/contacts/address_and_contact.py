...
def load_address_and_contact(doc, key=None):
	...
	address_list = vmraid.get_list("Address", filters=filters, fields=["*"])
	...
	contact_list = vmraid.get_list("Contact", filters=filters, fields=["*"])

	for contact in contact_list:
		contact["email_ids"] = vmraid.get_all(
			"Contact Email",
			filters={"parenttype": "Contact", "parent": contact.name, "is_primary": 0},
			fields=["email_id"],
		)

		contact["phone_nos"] = vmraid.get_all(
			"Contact Phone",
			filters={
				"parenttype": "Contact",
				"parent": contact.name,
...
def filter_dynamic_link_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	_doctypes = vmraid.db.get_all("Custom Field", filters=filters, fields=["dt"], as_list=True)
