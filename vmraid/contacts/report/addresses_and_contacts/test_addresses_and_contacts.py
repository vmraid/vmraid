...
from vmraid.contacts.report.addresses_and_contacts.addresses_and_contacts import get_data

def get_custom_linked_doctype():
	if bool(vmraid.get_all("DocType", filters={'name':'Test Custom Doctype'})):
		return
...
def create_linked_address(link_list):
	if vmraid.flags.test_address_created:
		return
	...
	address.insert()
	vmraid.flags.test_address_created = True

	return address.name

def create_linked_contact(link_list, address):
	if vmraid.flags.test_contact_created:
		return
	...
	vmraid.flags.test_contact_created = True
