...
def get_address_templates(address):
	result = vmraid.db.get_value(
		"Address Template", {"country": address.get("country")}, ["name", "template"]
	)

	if not result:
		result = vmraid.db.get_value("Address Template", {"is_default": 1}, ["name", "template"])
...
def address_query(doctype, txt, searchfield, start, page_len, filters):
	...
	for fieldname, value in filters.items():
		if meta.get_field(fieldname) or fieldname in vmraid.db.DEFAULT_COLUMNS:
			condition += " and {field}={value}".format(field=fieldname, value=vmraid.db.escape(value))

	searchfields = meta.get_search_fields()

	if searchfield and (meta.get_field(searchfield) or searchfield in vmraid.db.DEFAULT_COLUMNS):
