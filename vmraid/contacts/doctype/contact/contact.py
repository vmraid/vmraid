...
from vmraid.utils import cstr, has_gravatar
...
class Contact(Document):
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			vmraid.throw(
				_("Only one {0} can be set as primary.").format(vmraid.bold(vmraid.unscrub(fieldname)))
			)
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if (
		not vmraid.get_meta("Contact").get_field(searchfield)
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = vmraid.get_meta(doctype).get("fields", {"fieldtype": "Link", "options": "Contact"})
	if not link_fields:
		return []
	...
	return vmraid.get_list("Contact", fields=fields, filters={"name": ("in", contact_names)})
