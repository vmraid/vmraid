...
import vmraid
from vmraid.utils import cstr, has_gravatar, cint
...
class Contact(Document):
	def autoname(self):
		...
		if vmraid.db.exists("Contact", self.name):
	...
	def validate(self):
		...
		if self.get("sync_with_google_contacts") and not self.get("google_contacts"):
			vmraid.throw(_("Select Google Contacts to which contact should be synced."))

		deduplicate_dynamic_links(self)

	def set_user(self):
		if not self.user and self.email_id:
			self.user = vmraid.db.get_value("User", {"email": self.email_id})
	...
	def add_email(self, email_id, is_primary=0, autosave=False):
		if not vmraid.db.exists("Contact Email", {"email_id": email_id, "parent": self.name}):
	...
	def add_phone(self, phone, is_primary_phone=0, is_primary_mobile_no=0, autosave=False):
		if not vmraid.db.exists("Contact Phone", {"phone": phone, "parent": self.name}):
	...
	def set_primary_email(self):
		...
		if len([email.email_id for email in self.email_ids if email.is_primary]) > 1:
			vmraid.throw(_("Only one {0} can be set as primary.").format(vmraid.bold("Email ID")))
	...
	def set_primary(self, fieldname):
		...
		if len(is_primary) > 1:
			vmraid.throw(_("Only one {0} can be set as primary.").format(vmraid.bold(vmraid.unscrub(fieldname))))
...
def get_default_contact(doctype, name):
	...
	out = vmraid.db.sql('''select parent,
...
def invite_user(contact):
	contact = vmraid.get_doc("Contact", contact)
...
def update_contact(doc, method):
	...
	contact_name = vmraid.db.get_value("Contact", {"email_id": doc.name})
	if contact_name:
...
def contact_query(doctype, txt, searchfield, start, page_len, filters):
	...
	if not vmraid.get_meta("Contact").get_field(searchfield)\
		and searchfield not in vmraid.db.DEFAULT_COLUMNS:
		return []
...
def address_query(links):
	...
	for link in links:
		if not vmraid.has_permission(doctype=link.get("link_doctype"), ptype="read", doc=link.get("link_name")):
			continue
...
def get_contact_with_phone_number(number):
	...
	contacts = vmraid.get_all('Contact Phone', filters=[
...
def get_contact_name(email_id):
	contact = vmraid.get_list("Contact Email", filters={"email_id": email_id}, fields=["parent"], limit=1)
...
def get_contacts_linking_to(doctype, docname, fields=None):
	...
	return vmraid.get_list('Contact', fields=fields, filters=[
...
def get_contacts_linked_from(doctype, docname, fields=None):
	...
	link_fields = vmraid.get_meta(doctype).get('fields', {
		'fieldtype': 'Link',
		'options': 'Contact'
	})
	if not link_fields:
	...
	contact_names = vmraid.get_value(doctype, docname, fieldname=[f.fieldname for f in link_fields])
