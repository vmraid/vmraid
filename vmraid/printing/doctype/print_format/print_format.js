...
vmraid.ui.form.on("Print Format", "onload", function (frm) {
	frm.add_fetch("doc_type", "module", "module");
});

vmraid.ui.form.on("Print Format", {
	refresh: function (frm) {
		frm.set_intro("");
		frm.toggle_enable(["html", "doc_type", "module"], false);
		if (vmraid.session.user === "Administrator" || frm.doc.standard === "No") {
			frm.toggle_enable(["html", "doc_type", "module"], true);
			frm.enable_save();
		}

		if (frm.doc.standard === "Yes" && vmraid.session.user !== "Administrator") {
			frm.set_intro(__("Please duplicate this to make changes"));
		}
		frm.trigger('render_buttons');
		frm.toggle_display('standard', vmraid.boot.developer_mode);
		...
		if (!frm.is_new()) {
			if (!frm.doc.custom_format) {
				frm.add_custom_button(__("Edit Format"), function () {
					if (!frm.doc.doc_type) {
						vmraid.msgprint(__("Please select DocType first"));
						return;
					}
					if (frm.doc.print_format_builder_beta) {
						vmraid.set_route("print-format-builder-beta", frm.doc.name);
					} else {
						vmraid.set_route("print-format-builder", frm.doc.name);
					}
				});
			}
			...
			if (vmraid.model.can_read(frm.doc.doc_type)) {
				vmraid.db.get_value('DocType', frm.doc.doc_type, 'default_print_format', (r) => {
					if (r.default_print_format != frm.doc.name) {
						frm.add_custom_button(__("Set as Default"), function () {
							vmraid.call({
								method: "vmraid.printing.doctype.print_format.print_format.make_default",
								args: {
									name: frm.doc.name
								},
								callback: function() {
