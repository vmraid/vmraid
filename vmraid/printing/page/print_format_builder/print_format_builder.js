vmraid.pages['print-format-builder'].on_page_load = function(wrapper) {
	vmraid.print_format_builder = new vmraid.PrintFormatBuilder(wrapper);
	vmraid.breadcrumbs.add("Setup", "Print Format");
}

vmraid.pages['print-format-builder'].on_page_show = function(wrapper) {
	var route = vmraid.get_route();
	if(route.length>1) {
		vmraid.model.with_doc('Print Format', route[1], function() {
			vmraid.print_format_builder.print_format = vmraid.get_doc('Print Format', route[1]);
			vmraid.print_format_builder.refresh();
		});
	} else if(vmraid.route_options) {
		if(vmraid.route_options.make_new) {
			let { doctype, name, based_on, beta } = vmraid.route_options;
			vmraid.route_options = null;
			vmraid.print_format_builder.setup_new_print_format(doctype, name, based_on, beta);
		} else {
			vmraid.print_format_builder.print_format = vmraid.route_options.doc;
			vmraid.route_options = null;
			vmraid.print_format_builder.refresh();
		}
	}
...
vmraid.PrintFormatBuilder = class PrintFormatBuilder {
	constructor(parent) {
		this.parent = parent;
		this.make();
		this.refresh();
	...
	make() {
		this.page = vmraid.ui.make_app_page({
			parent: this.parent,
		...
		this.page.main = $('<div class="col-md-12 border print-format-builder-main vmraid-card"></div>').appendTo(this.page.main);
	...
	show_start() {
		this.page.main.html(vmraid.render_template("print_format_builder_start", {}));
	...
	start_edit_print_format() {
		...
		var me = this;
		this.print_format_input = vmraid.ui.form.make_control({
		...
		this.page.main.find(".btn-edit-print-format").on("click", function() {
			...
			if(!name) return;
			vmraid.model.with_doc("Print Format", name, function(doc) {
				vmraid.set_route('print-format-builder', name);
			});
		});
	}
	start_new_print_format() {
		var me = this;
		this.doctype_input = vmraid.ui.form.make_control({
		...
		this.name_input = vmraid.ui.form.make_control({
		...
		this.page.main.find(".btn-new-print-format").on("click", function() {
			...
			if(!(doctype && name)) {
				vmraid.msgprint(__("Both DocType and Name required"));
				return;
			}
	...
	setup_new_print_format(doctype, name, based_on, beta) {
		vmraid.call({
			method: 'vmraid.printing.page.print_format_builder.print_format_builder.create_custom_format',
			args: {
				doctype: doctype,
				name: name,
				based_on: based_on,
			...
			callback: (r) => {
				if (r.message) {
					...
					if (print_format.print_format_builder_beta) {
						vmraid.set_route('print-format-builder-beta', print_format.name);
					} else {
	...
	setup_print_format() {
		var me = this;
		vmraid.model.with_doctype(this.print_format.doc_type, function(doctype) {
			me.meta = vmraid.get_meta(me.print_format.doc_type);
			me.setup_sidebar();
			me.render_layout();
			...
			me.page.add_inner_button(__("Edit Properties"), function() {
				vmraid.set_route("Form", "Print Format", me.print_format.name);
			});
		});
	}
	setup_sidebar() {
		...
		this.page.sidebar.html(
			$(vmraid.render_template("print_format_builder_sidebar", {fields: fields}))
		);
	...
	render_layout() {
		...
		this.prepare_data();
		$(vmraid.render_template("print_format_builder_layout", {
		...
		$.each(this.data, function(i, f) {
			...
			if(!f.name && f.fieldname) {
				...
				if(f.fieldname==="_custom_html") {
					...
					f = $.extend(vmraid.meta.get_docfield(me.print_format.doc_type,
						f.fieldname) || {}, f);
				}
			}
	...
	init_visible_columns(f) {
		f.visible_columns = []
		$.each(vmraid.get_meta(f.options).fields, function(i, _f) {
			...
			onAdd: function(evt) {
				...
				if(!$item.hasClass("print-format-builder-field")) {
					...
					if(fieldname==="_custom_html") {
						...
						var field = vmraid.meta.get_docfield(me.print_format.doc_type,
							fieldname);
					}

					var html = vmraid.render_template("print_format_builder_field",
						{field: field, me:me});
	...
	setup_add_section() {
		...
		this.page.main.find(".print-format-builder-add-section").on("click", function() {
			...
			var $section = $(vmraid.render_template("print_format_builder_section",
	...
	setup_column_selector() {
		...
		this.page.main.on("click", ".select-columns", function() {
			...
			var doc_fields = vmraid.get_meta(doctype).fields;
			...
			// render checkboxes
			$(vmraid.render_template("print_format_builder_column_selector", {
				fields: fields,
	...
	save_print_format() {
		...
		vmraid.call({
			...
			callback: function(r) {
				...
				vmraid.show_alert({message: __("Saved"), indicator: 'green'});
			}
		});
	}
};
