...
vmraid.ui.form.PrintView = class {
	...
	setup_menu() {
		...
		if (
			vmraid.model.get_doc(':Print Settings', 'Print Settings')
	...
	setup_print_format_dom(out, $print_format) {
		...
		let print_css = vmraid.assets.bundled_asset('print.bundle.css');
		this.$print_format_body.find('head').html(
			`<style type="text/css">${out.style}</style>
			<link href="${base_url}${print_css}" rel="stylesheet">`
		);

		if (vmraid.utils.is_rtl(this.lang_code)) {
			let rtl_css = vmraid.assets.bundled_asset('vmraid-rtl.bundle.css');
	...
	printit() {
		...
		vmraid.call({
			method:
				'vmraid.printing.doctype.print_settings.print_settings.is_print_server_enabled',
