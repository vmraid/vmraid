vmraid.pages['print'].on_page_load = function(wrapper) {
	vmraid.ui.make_app_page({
		parent: wrapper,
	});

	let print_view = new vmraid.ui.form.PrintView(wrapper);

	$(wrapper).bind('show', () => {
		const route = vmraid.get_route();
		const doctype = route[1];
		const docname = route[2];
		if (!vmraid.route_options || !vmraid.route_options.frm) {
			vmraid.model.with_doc(doctype, docname, () => {
				let frm = { doctype: doctype, docname: docname };
				frm.doc = vmraid.get_doc(doctype, docname);
				vmraid.model.with_doctype(doctype, () => {
					frm.meta = vmraid.get_meta(route[1]);
					print_view.show(frm);
				});
			});
		} else {
			print_view.frm = vmraid.route_options.frm;
			vmraid.route_options.frm = null;
			print_view.show(print_view.frm);
		}
	});
};

vmraid.ui.form.PrintView = class {
	constructor(wrapper) {
	...
	make() {
		this.print_wrapper = this.page.main.empty().html(
			`<div class="print-preview-wrapper"><div class="print-preview">
				${vmraid.render_template('print_skeleton_loading')}
		...
		this.print_settings = vmraid.model.get_doc(
			':Print Settings',
			'Print Settings'
		);
		this.setup_menu();
	...
	add_sidebar_item(df, is_dynamic) {
		...
		let field = vmraid.ui.form.make_control({
			df: df,
	...
	setup_menu() {
		...
		if (vmraid.model.can_create('Print Format')) {
	...
	set_breadcrumbs() {
		vmraid.breadcrumbs.add(this.frm.meta.module, this.frm.doctype);
	}
	...
	setup_additional_settings() {
		...
		vmraid
			.xcall('vmraid.printing.page.print.print.get_print_settings_to_show', {
	...
	edit_print_format() {
		...
		if (is_standard_but_editable) {
			vmraid.set_route('Form', 'Print Format', print_format.name);
			return;
		}
		if (is_custom_format) {
			if (print_format.print_format_builder_beta) {
				...
				vmraid.set_route('print-format-builder', print_format.name);
			}
			return;
		}
		...
		vmraid.prompt(
			...
			(data) => {
				...
				vmraid.set_route('print-format-builder');
	...
	setup_keyboard_shortcuts() {
		this.wrapper.find('.print-toolbar a.btn-default').each((i, el) => {
			vmraid.ui.keys.get_shortcut_group(this.frm.page).add($(el));
		});
	}
	...
	preview() {
		...
		this.get_print_html((out) => {
			...
			const print_height_inches = vmraid.dom.pixel_to_inches(print_height);
	...
	setup_print_format_dom(out, $print_format) {
		...
		let base_url = vmraid.urllib.get_base_url();
		let print_css = vmraid.assets.bundled_asset('print.bundle.css', vmraid.utils.is_rtl(this.lang_code));
		this.$print_format_body.find('html').attr('dir', vmraid.utils.is_rtl(this.lang_code) ? 'rtl': 'ltr');
			...
			if (me.is_raw_printing()) {
				me.get_raw_commands(function(out) {
					vmraid.ui.form
						...
						.then(vmraid.ui.form.qz_success)
						.catch((err) => {
							vmraid.ui.form.qz_fail(err);
						});
				});
			} else {
				vmraid.show_alert(
	...
	print_by_server() {
		...
		if (localStorage.getItem('network_printer')) {
			vmraid.call({
				method: 'vmraid.utils.print_format.print_by_server',
				args: {
	...
	network_printer_setting_dialog(callback) {
		vmraid.call({
			method: 'vmraid.printing.doctype.network_printer_settings.network_printer_settings.get_network_printer_settings',
			callback: function(r) {
				if (r.message) {
	...
	render_pdf() {
		...
		if (print_format.print_format_builder_beta) {
			...
			let w = window.open(`/api/method/vmraid.utils.weasyprint.download_pdf?${params}`);
			if (!w) {
				vmraid.msgprint(__('Please enable pop-ups'));
				return;
			}
		} else {
			this.render_page('/api/method/vmraid.utils.print_format.download_pdf?');
		}
	}
	...
	get_print_html(callback) {
		...
		this._req = vmraid.call({
			method: 'vmraid.www.printview.get_html_and_style',
			args: {
				doc: this.frm.doc,
	...
	get_raw_commands(callback) {
		...
		vmraid.call({
			method: 'vmraid.www.printview.get_rendered_raw_commands',
			args: {
				doc: this.frm.doc,
				print_format: this.selected_format(),
				_lang: this.lang_code,
	...
	refresh_print_options() {
		this.print_formats = vmraid.meta.get_print_formats(this.frm.doctype);
	...
	set_style(style) {
		vmraid.dom.set_style(style || vmraid.boot.print_css, 'print-style');
	}
	...
	printer_setting_dialog() {
		...
		this.printer_list = [];
		vmraid.ui.form.qz_get_printer_list().then((data) => {
			...
			if (!(this.printer_list && this.printer_list.length)) {
				vmraid.throw(__('No Printer is Available.'));
			}
		});
	}
};
