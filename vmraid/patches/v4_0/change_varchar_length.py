...
def execute():
	vmraid.db.sql('update tabDocField set search_index=0 where fieldtype="Small Text"')
	vmraid.db.sql('update tabDocField set in_list_view=0 where fieldtype="Image"')

	for dt in vmraid.db.sql_list("""select name from `tabDocType` where issingle=0"""):
		desc = dict((d["Field"], d) for d in vmraid.db.sql("desc `tab{}`".format(dt), as_dict=True))
		alter_table = []
		...
		if alter_table:
			...
			vmraid.db.sql_ddl(alter_table_query)
