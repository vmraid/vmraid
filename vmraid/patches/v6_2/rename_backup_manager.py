...
def execute():
	unset = False
	vmraid.reload_doc("integrations", "doctype", "dropbox_backup")

	dropbox_backup = vmraid.get_doc("Dropbox Backup", "Dropbox Backup")
	for df in dropbox_backup.meta.fields:
		value = vmraid.db.get_single_value("Backup Manager", df.fieldname)
		if value:
