...
def execute():
	...
	vmraid.db.sql("""delete from `tabProperty Setter` where property='previous_field'""")

	all_custom_fields = vmraid._dict()
	for d in vmraid.db.sql("""select name, dt, fieldname, insert_after from `tabCustom Field`
		where insert_after is not null and insert_after != ''""", as_dict=1):
			all_custom_fields.setdefault(d.dt, vmraid._dict()).setdefault(d.fieldname, d.insert_after)

	for dt, custom_fields in all_custom_fields.items():
		_idx = []
		existing_ps = vmraid.db.get_value("Property Setter",
		...
		if not existing_ps:
			...
			if idx_needs_to_be_fixed:
				vmraid.delete_doc("Property Setter", existing_ps.name)
				_idx = get_sorted_fields(dt, custom_fields)
...
def get_sorted_fields(doctype, custom_fields):
	...
	fields_dict = vmraid.get_meta(doctype).get("fields")

	standard_fields_count = vmraid.db.sql("""select count(name) from `tabDocField`
