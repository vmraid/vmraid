...
def delete_doc(doctype, doctype_name):
	vmraid.delete_doc(doctype, doctype_name)
...
def setup_services():
	for service in [{"old_name": "Razorpay", "new_name": "Razorpay"},
		...
		try:
			service_doc = vmraid.get_doc("Integration Service", service["old_name"])
			settings = json.loads(service_doc.custom_settings_json)

			service_settings = vmraid.new_doc("{0} Settings".format(service["new_name"]))
			...
			if service["old_name"] in ["Dropbox Integration", "LDAP Auth"]:
				...
				new_service_doc = vmraid.get_doc({
