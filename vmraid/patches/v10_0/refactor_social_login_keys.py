...
def execute():
	...
	if social_login_keys.get("vmraid_server_url"):
		...
		if not (
			vmraid_login_key.client_secret and vmraid_login_key.client_id and vmraid_login_key.base_url
		):
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	query = """INSERT INTO `tabUser Social Login` (`{source_cols}`)
		VALUES ({values})
	""".format(
		source_cols="`, `".join(source_cols), values=", ".join([vmraid.db.escape(d) for d in values])
	)
