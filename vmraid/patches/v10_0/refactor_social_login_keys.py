...
def execute():
	...
	vmraid.reload_doc("integrations", "doctype", "social_login_key", force=True)

	if not vmraid.db.exists('DocType', 'Social Login Keys'):
		return

	social_login_keys = vmraid.get_doc("Social Login Keys", "Social Login Keys")
	if social_login_keys.get("facebook_client_id") or social_login_keys.get("facebook_client_secret"):
		facebook_login_key = vmraid.new_doc("Social Login Key")
	...
	if social_login_keys.get("vmraid_server_url"):
		vmraid_login_key = vmraid.new_doc("Social Login Key")
		vmraid_login_key.get_social_login_provider("VMRaid", initialize=True)
		vmraid_login_key.social_login_provider = "VMRaid"
		vmraid_login_key.base_url = social_login_keys.get("vmraid_server_url")
		vmraid_login_key.client_id = social_login_keys.get("vmraid_client_id")
		vmraid_login_key.client_secret = social_login_keys.get("vmraid_client_secret")
		if not (vmraid_login_key.client_secret and vmraid_login_key.client_id and vmraid_login_key.base_url):
			vmraid_login_key.enable_social_login = 0
		vmraid_login_key.save()

	if social_login_keys.get("github_client_id") or social_login_keys.get("github_client_secret"):
		github_login_key = vmraid.new_doc("Social Login Key")
	...
	if social_login_keys.get("google_client_id") or social_login_keys.get("google_client_secret"):
		google_login_key = vmraid.new_doc("Social Login Key")
	...
	vmraid.delete_doc("DocType", "Social Login Keys")

def run_patch():
	vmraid.reload_doc("core", "doctype", "user", force=True)
	vmraid.reload_doc("core", "doctype", "user_social_login", force=True)

	users = vmraid.get_all("User", fields=["*"], filters={"name":("not in", ["Administrator", "Guest"])})

	for user in users:
		idx = 0
		if user.vmraid_userid:
			insert_user_social_login(user.name, user.modified_by, 'vmraid', idx, userid=user.vmraid_userid)
			idx += 1
...
def insert_user_social_login(user, modified_by, provider, idx, userid=None, username=None):
	...
	creation_time = vmraid.utils.get_datetime_str(vmraid.utils.get_datetime())
	values = [
		vmraid.generate_hash(length=10),
		creation_time,
		creation_time,
		user,
		modified_by,
	...
	""".format(
		...
		values= ", ".join([vmraid.db.escape(d) for d in values])
	)
