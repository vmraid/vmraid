...
def execute():
	if not vmraid.db.exists('DocType', 'Custom Role'):
		vmraid.reload_doc("core", 'doctype', "custom_role")
...
def update_ref_doctype_in_custom_role():
	vmraid.reload_doc("core", 'doctype', "custom_role")
	vmraid.db.sql("""update `tabCustom Role` 
				set 
...
def make_custom_roles_for_page_and_report():
	for doctype in ['Page', 'Report']:
		for data in get_data(doctype):
			doc = vmraid.get_doc(doctype, data.name)
...
def get_data(doctype):
	...
	return vmraid.get_all(doctype, fields = fields)

def get_roles(doctype, data, doc):
	roles = []
	if doctype == 'Page':
		for d in doc.roles:
			if vmraid.db.exists('Role', d.role):
				roles.append({'role': d.role})
	else:
		out = vmraid.get_all('Custom DocPerm', fields='distinct role', filters=dict(parent = data.ref_doctype))
		for d in out:
			roles.append({'role': d.role})
	return roles
...
def make_custom_roles(doctype, name, roles):
	...
	if roles:
		custom_permission = vmraid.get_doc({
			'doctype': 'Custom Role',
			field : name,
			'roles' : roles
		}).insert()
