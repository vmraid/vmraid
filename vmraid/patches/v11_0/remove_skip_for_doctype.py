...
from vmraid.patches.v11_0.replicate_old_user_permissions import get_doctypes_to_skip
...
def execute():
	vmraid.reload_doctype('User Permission')

	# to check if we need to migrate from skip_for_doctype
	has_skip_for_doctype = vmraid.db.has_column("User Permission", "skip_for_doctype")
	...
	for user_permission in vmraid.get_all('User Permission', fields=['*']):
		skip_for_doctype = []
		...
		if skip_for_doctype:
			...
			vmraid.db.set_value('User Permission', user_permission.name, 'apply_to_all_doctypes', 1)
	...
	if user_permissions_to_delete:
		vmraid.db.sql('DELETE FROM `tabUser Permission` WHERE `name` in ({})' # nosec
