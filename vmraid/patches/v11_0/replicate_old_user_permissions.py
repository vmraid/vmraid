...
from vmraid.permissions import get_valid_perms

def execute():
	vmraid.reload_doctype("User Permission")
	user_permissions = vmraid.get_all('User Permission', fields=['allow', 'name', 'user'])
	...
	for key, doctype_to_skip in doctype_to_skip_map.items():
		...
		if not vmraid.db.has_column("User Permission", "applicable_for") \
			and vmraid.db.has_column("User Permission", "skip_for_doctype"):
...
def get_linked_doctypes(doctype):
	from vmraid.permissions import get_linked_doctypes
	linked_doctypes = get_linked_doctypes(doctype)
	child_doctypes = [d.options for d in vmraid.get_meta(doctype).get_table_fields()]
