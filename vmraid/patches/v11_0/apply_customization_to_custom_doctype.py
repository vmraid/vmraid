...
def execute():
	custom_doctypes = vmraid.get_all("DocType", filters={"custom": 1})

	for doctype in custom_doctypes:
		property_setters = vmraid.get_all(
			"Property Setter",
			filters={"doc_type": doctype.name, "doctype_or_field": "DocField"},
			fields=["name", "property", "value", "property_type", "field_name"],
		)

		custom_fields = vmraid.get_all("Custom Field", filters={"dt": doctype.name}, fields=["*"])

		property_setter_map = {}

		for prop in property_setters:
			property_setter_map[prop.field_name] = prop
			vmraid.db.delete("Property Setter", {"name": prop.name})

		meta = vmraid.get_meta(doctype.name)
		...
		for cf in custom_fields:
			...
			if field:
				...
				df = vmraid.new_doc("DocField", meta, "fields")
				df.update(cf)
				meta.fields.append(df)
			vmraid.db.delete("Custom Field", {"name": cf.name})
