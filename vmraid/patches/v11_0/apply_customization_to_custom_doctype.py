...
def execute():
	custom_doctypes = vmraid.get_all('DocType', filters={
		'custom': 1
	})

	for doctype in custom_doctypes:
		property_setters = vmraid.get_all('Property Setter', filters={
		...
		for prop in property_setters:
			...
			vmraid.db.sql('DELETE FROM `tabProperty Setter` WHERE `name`=%s', prop.name)
		...
		for cf in custom_fields:
			...
			vmraid.db.sql('DELETE FROM `tabCustom Field` WHERE name=%s', cf.name)
