...
def execute():
    ...
    reports = vmraid.get_all('Report', { 'report_type': 'Report Builder' })

    for report_name in reports:
        settings = vmraid.db.get_value('Report', report_name, 'json')
        if not settings:
            continue

        settings = vmraid._dict(json.loads(settings))
        ...
        vmraid.db.set_value('Report', report_name, 'json', json.dumps(settings))
