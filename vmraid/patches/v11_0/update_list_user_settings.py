...
import vmraid, json
from vmraid.model.utils.user_settings import update_user_settings, sync_user_settings

def execute():
	""" Update list_view's order by property from __UserSettings """

	users = vmraid.db.sql("select distinct(user) from `__UserSettings`", as_dict=True)

	for user in users:
		# get user_settings for each user
		settings = vmraid.db.sql("select * from `__UserSettings` \
			where user={0}".format(vmraid.db.escape(user.user)), as_dict=True)
