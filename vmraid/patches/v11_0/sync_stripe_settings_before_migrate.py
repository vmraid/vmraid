...
def execute():
	publishable_key = vmraid.db.sql("select value from tabSingles where doctype='Stripe Settings' and field='publishable_key'")
	if publishable_key:
		...
		if secret_key:
			...
			settings.gateway_name = vmraid.db.get_value("Global Defaults", None, "default_company") or "Stripe Settings"
			settings.publishable_key = publishable_key
			settings.secret_key = secret_key
			settings.save(ignore_permissions=True)

	vmraid.db.sql("""DELETE FROM tabSingles WHERE doctype='Stripe Settings'""")