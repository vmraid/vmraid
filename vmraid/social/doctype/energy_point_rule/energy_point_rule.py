...
from vmraid.social.doctype.energy_point_log.energy_point_log import \
...
class EnergyPointRule(Document):
	...
	def eval_condition(self, doc):
		return self.condition and vmraid.safe_eval(self.condition, None, {
			'doc': doc.as_dict()
		})
...
def process_energy_points(doc, state):
	...
	for d in vmraid.cache_manager.get_doctype_map('Energy Point Rule', doc.doctype,
...
def get_energy_point_doctypes():
	return [
		d.reference_doctype for d in vmraid.get_all('Energy Point Rule',
