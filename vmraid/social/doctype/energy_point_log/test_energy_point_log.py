...
from vmraid.desk.form.assign_to import add as assign_to

class TestEnergyPointLog(unittest.TestCase):
	def setUp(self):
		vmraid.cache().delete_value('energy_point_rule_map')

	def tearDown(self):
		vmraid.set_user('Administrator')
		vmraid.db.sql('DELETE FROM `tabEnergy Point Log`')
		vmraid.db.sql('DELETE FROM `tabEnergy Point Rule`')
		vmraid.cache().delete_value('energy_point_rule_map')
	...
	def test_disabled_energy_points(self):
		settings = vmraid.get_single('Energy Point Settings')
		settings.enabled = 0
		settings.save()
	...
	def test_revert_points_on_cancelled_doc(self):
		...
		energy_point_logs = vmraid.get_all('Energy Point Log')
		...
		energy_point_logs = vmraid.get_all('Energy Point Log', fields=['reference_name', 'type', 'reverted'])
	...
	def test_allow_creation_of_new_log_if_the_previous_log_was_reverted(self):
		...
		log_name = vmraid.db.exists('Energy Point Log', {'reference_name': created_todo.name})
		vmraid.get_doc('Energy Point Log', log_name).revert('Just for test')
...
def create_energy_point_rule_for_todo(multiplier_field=None, for_doc_event='Custom', max_points=None,
	...
	name = 'ToDo Closed'
	point_rule_exists = vmraid.db.exists('Energy Point Rule', name)

	if point_rule_exists: return vmraid.get_doc('Energy Point Rule', name)
