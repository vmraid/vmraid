...
from vmraid.utils import cint, get_fullname, get_link_to_form, getdate
...
class EnergyPointLog(Document):
	...
	def map_milestone_reference(self):
		...
		if self.reference_doctype == "Milestone":
			self.reference_doctype, self.reference_name = vmraid.db.get_value(
	...
	def after_insert(self):
		...
		if self.type != "Review" and vmraid.get_cached_value(
...
def create_energy_points_log(ref_doctype, ref_name, doc, apply_only_once=False):
	...
	if log_exists:
		return vmraid.get_doc("Energy Point Log", log_exists)
