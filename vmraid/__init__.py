...
"""
VMRaid - Low Code Open Source Framework in Python and JS

VMRaid, pronounced fra-pay, is a full stack, batteries-included, web
framework written in Python and Javascript with MariaDB as the database.
It is the framework which powers ERPAdda. It is pretty generic and can
be used to build database driven apps.

Read the documentation: https://vmraidframework.com/docs
"""
import os, warnings
...
def _(msg, lang=None, context=None):
	...
	"""
	from vmraid.translate import get_full_dict
...
def get_lang_dict(fortype, name=None):
	...
	from vmraid.translate import get_dict
	return get_dict(fortype, name)

def set_user_lang(user, user_language=None):
	"""Guess and set user language for the session. `vmraid.local.lang`"""
	from vmraid.translate import get_user_lang
...
if typing.TYPE_CHECKING:
	from vmraid.database.mariadb.database import MariaDBDatabase
	from vmraid.database.postgres.database import PostgresDatabase
	db: typing.Union[MariaDBDatabase, PostgresDatabase]
# end: static analysis hack

def init(site, sites_path=None, new_site=False):
	"""Initialize vmraid for the current site. Reset thread locals `vmraid.local`"""
...
def connect(site=None, db_name=None, set_admin_as_user=True):
	...
	:param site: If site is given, calls `vmraid.init`.
	:param db_name: Optional. Will use from `site_config.json`.
	:param set_admin_as_user: Set Administrator as current user.
	"""
	from vmraid.database import get_db
	if site:
		init(site)
...
def cache():
	...
	global redis_server
	if not redis_server:
		from vmraid.utils.redis_wrapper import RedisWrapper
...
def get_traceback():
	...
	from vmraid.utils import get_traceback
...
def throw(msg, exc=ValidationError, title=None, is_minimizable=None, wide=None, as_list=False):
	...
	:param msg: Message.
	:param exc: Exception class. Default `vmraid.ValidationError`"""
...
def create_folder(path, with_init=False):
	...
	from vmraid.utils import touch_file
...
def get_roles(username=None):
	...
	return vmraid.permissions.get_roles(username or local.session.user)
...
def sendmail(recipients=[], sender="", subject="No Subject", message="No Message",
	...
	if as_markdown:
		from vmraid.utils import md_to_html
		message = md_to_html(message)

	if not delayed:
		now = True

	from vmraid.email import queue
...
def only_for(roles, message=False):
	"""Raise `vmraid.PermissionError` if the user does not have any of the given **Roles**.
...
def clear_cache(user=None, doctype=None):
	...
	if doctype:
		vmraid.cache_manager.clear_doctype_cache(doctype)
		reset_metadata_version()
	elif user:
		vmraid.cache_manager.clear_user_cache(user)
	else: # everything
		from vmraid import translate
		vmraid.cache_manager.clear_user_cache()
		vmraid.cache_manager.clear_domain_cache()
...
def only_has_select_perm(doctype, user=None, ignore_permissions=False):
	...
	import vmraid.permissions
	permissions = vmraid.permissions.get_role_permissions(doctype, user=user)
...
def has_permission(doctype=None, ptype="read", doc=None, user=None, verbose=False, throw=False):
	"""Raises `vmraid.PermissionError` if not permitted.
	...
	import vmraid.permissions
	out = vmraid.permissions.has_permission(doctype, ptype, doc=doc, verbose=verbose, user=user, raise_exception=throw)
	if throw and not out:
		if doc:
			vmraid.throw(_("No permission for {0}").format(doc.doctype + " " + doc.name))
		else:
			vmraid.throw(_("No permission for {0}").format(doctype))
...
def new_doc(doctype, parent_doc=None, parentfield=None, as_dict=False):
	...
	from vmraid.model.create_new import get_new_doc
	return get_new_doc(doctype, parent_doc, parentfield, as_dict=as_dict)

def set_value(doctype, docname, fieldname, value=None):
	"""Set document value. Calls `vmraid.client.set_value`"""
	import vmraid.client
	return vmraid.client.set_value(doctype, docname, fieldname, value)
...
def get_doc(*args, **kwargs):
	"""Return a `vmraid.model.document.Document` object of the given type and name.
	...
	Examples:
		...
		todo = vmraid.get_doc({"doctype":"ToDo", "description": "test"})
		todo.insert()

		# open an existing document
		todo = vmraid.get_doc("ToDo", "TD0001")

	"""
	import vmraid.model.document
	doc = vmraid.model.document.get_doc(*args, **kwargs)
...
def get_single(doctype):
	"""Return a `vmraid.model.document.Document` object of the given Single doctype."""
	return get_doc(doctype, doctype)

def get_meta(doctype, cached=True):
	"""Get `vmraid.model.meta.Meta` instance of given doctype name."""
	import vmraid.model.meta
	return vmraid.model.meta.get_meta(doctype, cached=cached)

def get_meta_module(doctype):
	import vmraid.modules
	return vmraid.modules.load_doctype_module(doctype)

def delete_doc(doctype=None, name=None, force=0, ignore_doctypes=None, for_reload=False,
	ignore_permissions=False, flags=None, ignore_on_trash=False, ignore_missing=True, delete_permanently=False):
	"""Delete a document. Calls `vmraid.model.delete_doc.delete_doc`.
	...
	import vmraid.model.delete_doc
	vmraid.model.delete_doc.delete_doc(doctype, name, force, ignore_doctypes, for_reload,
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	...
	import vmraid.modules
	return vmraid.modules.reload_doc(module, dt, dn, force=force, reset_permissions=reset_permissions)
...
def rename_doc(*args, **kwargs):
	"""
		...
		Calls `vmraid.model.rename_doc.rename_doc`
	"""
...
def get_installed_apps(sort=False, vmraid_last=False):
	...
	if vmraid_last:
		if 'vmraid' in installed:
			installed.remove('vmraid')
		installed.append('vmraid')
...
def get_hooks(hook=None, default=None, app_name=None):
	...
	def load_app_hooks(app_name=None):
		...
		for app in [app_name] if app_name else get_installed_apps(sort=True):
			app = "vmraid" if app=="webnotes" else app
			try:
...
def get_file_items(path, raise_not_found=False, ignore_empty_lines=True):
	...
	if content:
		content = vmraid.utils.strip(content)
...
def compare(val1, condition, val2):
	"""Compare two values using `vmraid.utils.compare`
	...
	"""
	import vmraid.utils
	return vmraid.utils.compare(val1, condition, val2)
...
def redirect_to_message(title, html, http_status_code=None, context=None, indicator_color=None):
	...
	Example Usage:
		vmraid.redirect_to_message(_('Thank you'), "<div><p>You will receive an email at test@example.com</p></div>")
...
def build_match_conditions(doctype, as_condition=True):
	...
	import vmraid.desk.reportview
	return vmraid.desk.reportview.build_match_conditions(doctype, as_condition=as_condition)

def get_list(doctype, *args, **kwargs):
	"""List database query via `vmraid.model.db_query`. Will also check for permissions.
	...
	Example usage:
		...
		# simple dict filter
		vmraid.get_list("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

		# filter as a list of lists
		vmraid.get_list("ToDo", fields="*", filters = [["modified", ">", "2014-01-01"]])

		# filter as a list of dicts
		vmraid.get_list("ToDo", fields="*", filters = {"description": ("like", "test%")})
	"""
	import vmraid.model.db_query
	return vmraid.model.db_query.DatabaseQuery(doctype).execute(*args, **kwargs)

def get_all(doctype, *args, **kwargs):
	"""List database query via `vmraid.model.db_query`. Will **not** check for permissions.
	Parameters are same as `vmraid.get_list`
	...
	Example usage:
		...
		# simple dict filter
		vmraid.get_all("ToDo", fields=["name", "description"], filters = {"owner":"test@example.com"})

		# filter as a list of lists
		vmraid.get_all("ToDo", fields=["*"], filters = [["modified", ">", "2014-01-01"]])

		# filter as a list of dicts
		vmraid.get_all("ToDo", fields=["*"], filters = {"description": ("like", "test%")})
	"""
...
def get_test_records(doctype):
	...
	from vmraid.modules import get_doctype_module, get_module_path
...
def format_value(*args, **kwargs):
	...
	import vmraid.utils.formatters
	return vmraid.utils.formatters.format_value(*args, **kwargs)
...
def attach_print(doctype, name, file_name=None, print_format=None,
	...
	from vmraid.utils import scrub_urls
...
def publish_progress(*args, **kwargs):
	...
	"""
	import vmraid.realtime
	return vmraid.realtime.publish_progress(*args, **kwargs)
...
def publish_realtime(*args, **kwargs):
	...
	return vmraid.realtime.publish_realtime(*args, **kwargs)

def local_cache(namespace, key, generator, regenerate_if_none=False):
	"""A key value store for caching within a request

	:param namespace: vmraid.local.cache[namespace]
	:param key: vmraid.local.cache[namespace][key] used to retrieve value
...
def enqueue(*args, **kwargs):
	...
	'''
	import vmraid.utils.background_jobs
	return vmraid.utils.background_jobs.enqueue(*args, **kwargs)
...
def enqueue_doc(*args, **kwargs):
	...
	'''
	import vmraid.utils.background_jobs
	return vmraid.utils.background_jobs.enqueue_doc(*args, **kwargs)
...
def logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	from vmraid.utils.logger import get_logger
...
def get_active_domains():
	from vmraid.core.doctype.domain_settings.domain_settings import get_active_domains
...
def get_version(doctype, name, limit=None, head=False, raise_err=True):
	...
	Example
	>>> vmraid.get_version('User', 'foobar@gmail.com')
	>>>
	[
		{
	...
	if meta.track_changes:
		...
		from vmraid.chat.util import squashify, dictify, safe_json_loads
...
def parse_json(val):
	from vmraid.utils import parse_json
...
def mock(type, size=1, locale='en'):
	...
	from vmraid.chat.util import squashify
	return squashify(results)

def validate_and_sanitize_search_inputs(fn):
	from vmraid.desk.search import validate_and_sanitize_search_inputs as func
	return func(fn)
