...
def get_message_mentions(content):
	...
	for token in tokens:
		if token.startswith('@'):
			what = token[1:]
			if vmraid.db.exists('User', what):
				mentions.append(what)
		else:
			if vmraid.db.exists('User', token):
...
def get_new_chat_message_doc(user, room, content, type = "Content", link = True):
	...
	room = vmraid.get_doc('Chat Room', room)

	meta = get_message_meta(content)
	mess = vmraid.new_doc('Chat Message')
	mess.room 	   = room.name
...
def send(user, room, content, type = "Content"):
	...
	vmraid.publish_realtime('vmraid.chat.message:create', mess, room = room,
		after_commit = True)
...
def seen(message, user = None):
	...
	has_message = vmraid.db.exists('Chat Message', message)

	if has_message:
		mess = vmraid.get_doc('Chat Message', message)
		mess.add_seen(user)
		mess.load_from_db()
		room = mess.room
		resp = dict(message = message, data = dict(seen = json.loads(mess._seen) if mess._seen else []))

		vmraid.publish_realtime('vmraid.chat.message:update', resp, room = room, after_commit = True)

def history(room, fields = None, limit = 10, start = None, end = None):
	room = vmraid.get_doc('Chat Room', room)
	mess = vmraid.get_all('Chat Message',
		filters  = [
	...
	vmraid.enqueue('vmraid.chat.doctype.chat_message.chat_message.mark_messages_as_seen',
		message_names=[m.name for m in mess], user=vmraid.session.user)
...
def mark_messages_as_seen(message_names, user):
	...
	for name in message_names:
		seen = vmraid.db.get_value('Chat Message', name, '_seen') or '[]'
		seen = json.loads(seen)
		seen.append(user)
		seen = json.dumps(seen)
		vmraid.db.set_value('Chat Message', name, '_seen', seen, update_modified=False)
...
def get(name, rooms = None, fields = None):
	...
	has_message = vmraid.db.exists('Chat Message', name)

	if has_message:
		dmess = vmraid.get_doc('Chat Message', name)
		data  = dict(
			name      = dmess.name,
			user      = dmess.user,
			room      = dmess.room,
