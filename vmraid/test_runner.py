...
from vmraid.modules import get_module_name, load_doctype_module
...
def xmlrunner_wrapper(output):
	...
	try:
		...
		print("$ chair setup requirements --dev")
		raise
...
def main(
	...
	if doctype_list_path:
		...
		with open(vmraid.get_app_path(app, doctype_list_path), "r") as f:
			doctype = f.read().strip().splitlines()

	if ui_tests:
		print(
			"Selenium testing has been deprecated\nUse chair --site {site_name} run-ui-tests for Cypress tests"
		)
	...
	try:
		vmraid.flags.print_messages = verbose
		...
		vmraid.clear_cache()
		scheduler_disabled_by_user = vmraid.utils.scheduler.is_scheduler_disabled()
		if not scheduler_disabled_by_user:
			vmraid.utils.scheduler.disable_scheduler()

		set_test_email_config()
		vmraid.conf.update({"chair_id": "test_chair", "use_rq_auth": False})

		if not vmraid.flags.skip_before_tests:
			if verbose:
				print('Running "before_tests" hooks')
			for fn in vmraid.get_hooks("before_tests", app_name=app):
				vmraid.get_attr(fn)()
...
def run_all_tests(
	...
	apps = [app] if app else vmraid.get_installed_apps()
...
def run_tests_for_doctype(
	...
	for doctype in doctypes:
		...
		if force:
			for name in vmraid.db.sql_list("select name from `tab%s`" % doctype):
				vmraid.delete_doc(doctype, name, force=True)
...
def _run_unittest(
	...
	vmraid.flags.tests_verbose = verbose
...
def _add_test(app, path, filename, verbose, test_suite=None, ui_tests=False):
	...
	app_path = vmraid.get_pymodule_path(app)
...
def make_test_records(doctype, verbose=0, force=False):
	...
	if vmraid.flags.skip_test_records:
		return
	...
	for options in get_dependencies(doctype):
		...
		if options not in vmraid.local.test_objects:
			vmraid.local.test_objects[options] = []
...
def get_dependencies(doctype):
	...
	for df in meta.get_table_fields():
		link_fields.extend(vmraid.get_meta(df.options).get_link_fields())
...
def make_test_records_for_doctype(doctype, verbose=0, force=False):
	...
	if hasattr(test_module, "_make_test_records"):
		vmraid.local.test_objects[doctype] += test_module._make_test_records(verbose)

	elif hasattr(test_module, "test_records"):
		if doctype in vmraid.local.test_objects:
			vmraid.local.test_objects[doctype] += make_test_objects(
		...
		if test_records:
			vmraid.local.test_objects[doctype] += make_test_objects(doctype, test_records, verbose, force)
...
def make_test_objects(doctype, test_records=None, verbose=None, reset=False):
	...
	for doc in test_records:
		...
		if vmraid.db.exists(d.doctype, d.name) and not reset:
			vmraid.db.rollback()
...
def add_to_test_record_log(doctype):
	...
	if doctype not in test_record_log:
		vmraid.flags.test_record_log.append(doctype)
		with open(vmraid.get_site_path(".test_log"), "w") as f:
			f.write("\n".join(filter(None, vmraid.flags.test_record_log)))


def get_test_record_log():
	"""Return the list of doctypes for which test records have been created"""
	if "test_record_log" not in vmraid.flags:
		if os.path.exists(vmraid.get_site_path(".test_log")):
			with open(vmraid.get_site_path(".test_log"), "r") as f:
				vmraid.flags.test_record_log = f.read().splitlines()
		else:
			vmraid.flags.test_record_log = []

	return vmraid.flags.test_record_log
