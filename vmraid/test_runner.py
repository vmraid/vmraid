...
import xmlrunner
import importlib
from vmraid.modules import load_doctype_module, get_module_name
...
def main(app=None, module=None, doctype=None, verbose=False, tests=(),
	...
	try:
		...
		# if not vmraid.conf.get("db_name").startswith("test_"):
...
def make_test_records(doctype, verbose=0, force=False):
	...
	for options in get_dependencies(doctype):
		...
		if not options in vmraid.local.test_objects:
...
def make_test_records_for_doctype(doctype, verbose=0, force=False):
	...
	if hasattr(test_module, "_make_test_records"):
		...
		vmraid.local.test_objects[doctype] += make_test_objects(doctype, test_module.test_records, verbose, force)
