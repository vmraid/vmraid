...
import vmraid, os
import vmraid.model
from vmraid.modules import scrub, get_module_path, scrub_dt_dn
...
def export_to_files(record_list=None, record_module=None, verbose=0, create_init=None):
	...
	if record_list:
		for record in record_list:
			...
			write_document_file(vmraid.get_doc(record[0], record[1]), record_module, create_init=create_init, folder_name=folder_name)
...
def write_document_file(doc, record_module=None, create_init=True, folder_name=None):
	...
	for df in doc.meta.get_table_fields():
		for d in newdoc.get(df.fieldname):
			for fieldname in vmraid.model.default_fields:
				if fieldname in d:
					del d[fieldname]
	...
	with open(os.path.join(folder, fname + ".json"), 'w+') as txtfile:
		txtfile.write(vmraid.as_json(newdoc))
