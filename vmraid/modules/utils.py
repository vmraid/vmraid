...
def export_module_json(doc, is_standard, module):
	...
	if not vmraid.flags.in_import and getattr(vmraid.get_conf(), "developer_mode", 0) and is_standard:
		...
		path = os.path.join(
			vmraid.get_module_path(module), scrub(doc.doctype), scrub(doc.name), scrub(doc.name)
		)
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	...
	def add(_doctype):
		custom["custom_fields"] += vmraid.get_all("Custom Field", fields="*", filters={"dt": _doctype})
		custom["property_setters"] += vmraid.get_all(
			"Property Setter", fields="*", filters={"doc_type": _doctype}
		)
		custom["links"] += vmraid.get_all("DocType Link", fields="*", filters={"parent": _doctype})

	add(doctype)

	if with_permissions:
		custom["custom_perms"] = vmraid.get_all(
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		def sync_single_doctype(doc_type):
			...
			if custom_doctype != "Custom Field":
				vmraid.db.delete(custom_doctype, {doctype_fieldname: doc_type})
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(
				os.path.join(folder, vmraid.scrub(doc_type) + ".json")
			):
