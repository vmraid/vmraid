...
def export_module_json(doc, is_standard, module):
	...
	if (not vmraid.flags.in_import and getattr(vmraid.get_conf(),'developer_mode', 0)
		and is_standard):
		...
		path = os.path.join(vmraid.get_module_path(module), scrub(doc.doctype),
...
def get_doc_module(module, doctype, name):
	...
	module_name = "{app}.{module}.{doctype}.{name}.{name}".format(
			app = vmraid.local.module_app[scrub(module)],
...
def export_customizations(module, doctype, sync_on_migrate=0, with_permissions=0):
	"""Export Custom Field and Property Setter for the current document to the app folder.
		This will be synced with chair migrate"""

	sync_on_migrate = cint(sync_on_migrate)
	with_permissions = cint(with_permissions)

	if not vmraid.get_conf().developer_mode:
	...
	def add(_doctype):
		custom['custom_fields'] += vmraid.get_all('Custom Field',
			fields='*', filters={'dt': _doctype})
		custom['property_setters'] += vmraid.get_all('Property Setter',
			fields='*', filters={'doc_type': _doctype})

	add(doctype)

	if with_permissions:
		custom['custom_perms'] = vmraid.get_all('Custom DocPerm',
			fields='*', filters={'parent': doctype})

	# also update the custom fields and property setters for all child tables
	for d in vmraid.get_meta(doctype).get_table_fields():
	...
	if custom["custom_fields"] or custom["property_setters"] or custom["custom_perms"]:
		...
		with open(path, 'w') as f:
			f.write(vmraid.as_json(custom))

		vmraid.msgprint(_('Customizations for <b>{0}</b> exported to:<br>{1}').format(doctype,path))
...
def sync_customizations(app=None):
	...
	for app_name in apps:
		for module_name in vmraid.local.app_modules.get(app_name) or []:
			folder = vmraid.get_app_path(app_name, module_name, 'custom')
...
def sync_customizations_for_doctype(data, folder):
	...
	def sync(key, custom_doctype, doctype_fieldname):
		...
		def sync_single_doctype(doc_type):
			def _insert(data):
				if data.get(doctype_fieldname) == doc_type:
					...
					doc = vmraid.get_doc(data)
					doc.db_insert()

			if custom_doctype != 'Custom Field':
				vmraid.db.sql('delete from `tab{0}` where `{1}` =%s'.format(
				...
				for d in data[key]:
					field = vmraid.db.get_value("Custom Field", {"dt": doc_type, "fieldname": d["fieldname"]})
					if not field:
		...
		for doc_type in doctypes:
			...
			if doc_type == doctype or not os.path.exists(os.path.join(folder, vmraid.scrub(doc_type)+".json")):
	...
	if update_schema and not vmraid.db.get_value('DocType', doctype, 'issingle'):
		vmraid.db.updatedb(doctype)

def scrub(txt):
	return vmraid.scrub(txt)
...
def get_module_path(module):
	...
	return vmraid.get_module_path(module)
...
def reload_doc(module, dt=None, dn=None, force=False, reset_permissions=False):
	from vmraid.modules.import_file import import_files
	return import_files(module, dt, dn, force=force, reset_permissions=reset_permissions)

def export_doc(doctype, name, module=None):
	"""Write a doc to standard path."""
	from vmraid.modules.export_file import write_document_file
	print(doctype, name)

	if not module: module = vmraid.db.get_value('DocType', name, 'module')
	write_document_file(vmraid.get_doc(doctype, name), module)

def get_doctype_module(doctype):
	"""Returns **Module Def** name of given doctype."""
	def make_modules_dict():
		return dict(vmraid.db.sql("select name, module from tabDocType"))
	return vmraid.cache().get_value("doctype_modules", make_modules_dict)[doctype]
...
def load_doctype_module(doctype, module=None, prefix="", suffix=""):
	...
	try:
		if key not in doctype_python_modules:
			doctype_python_modules[key] = vmraid.get_module(module_name)
...
def get_module_app(module):
	return vmraid.local.module_app[scrub(module)]

def get_app_publisher(module):
	app = vmraid.local.module_app[scrub(module)]
	if not app:
		vmraid.throw(_("App not found"))
	app_publisher = vmraid.get_hooks(hook="app_publisher", app_name=app)[0]
...
def make_boilerplate(template, doc, opts=None):
	...
	if not os.path.exists(target_file_path):
		...
		base_class = 'Document'
		base_class_import = 'from vmraid.model.document import Document'
		if doc.get('is_tree'):
			base_class = 'NestedSet'
			base_class_import = 'from vmraid.utils.nestedset import NestedSet'
	...
	def get_list(self, args):
		...
		with open(target_file_path, 'w') as target:
			with open(os.path.join(get_module_path("core"), "doctype", scrub(doc.doctype),
				...
				target.write(vmraid.as_unicode(
					vmraid.utils.cstr(source.read()).format(
						app_publisher=app_publisher,
						year=vmraid.utils.nowdate()[:4],
