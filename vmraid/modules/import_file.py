...
from vmraid.utils import get_datetime_str
	...
	if docs:
		...
		for doc in docs:
			if not force:
				...
				db_modified = vmraid.db.get_value(doc['doctype'], doc['name'], 'modified')
			...
			if original_modified:
				...
				if doc["doctype"] == doc["name"] and doc["name"]!="DocType":
					vmraid.db.sql("""update tabSingles set value=%s where field="modified" and doctype=%s""",
						(original_modified, doc["name"]))
				else:
					vmraid.db.sql("update `tab%s` set modified=%s where name=%s" % \
						(doc['doctype'], '%s', '%s'),
						(original_modified, doc['name']))
