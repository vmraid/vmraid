...
def get_assets_link(vmraid_head) -> str:
	tag = getoutput(
		r"cd ../apps/vmraid && git show-ref --tags -d | grep %s | sed -e 's,.*"
		r" refs/tags/,,' -e 's/\^{}//'" % vmraid_head
	)

	if tag:
		# if tag exists, download assets from github release
		url = f"https://github.com/vmraid/vmraid/releases/download/{tag}/assets.tar.gz"
	else:
		url = f"http://assets.frappeframework.com/{vmraid_head}.tar.gz"

	if not head(url):
		reference = f"Release {tag}" if tag else f"Commit {vmraid_head}"
		raise AssetsDontExistError(f"Assets for {reference} don't exist")

	return url


def fetch_assets(url, vmraid_head):
	...
	click.echo(click.style("✔", fg="green") + f" Downloaded VMRaid assets from {url}")
...
def download_vmraid_assets(verbose=True):
	...
	try:
		...
		assets_archive = fetch_assets(url, vmraid_head)
...
def bundle(
	...
	vmraid.commands.popen(command, cwd=vmraid_app_path, env=get_node_env(), raise_err=True)
...
def watch(apps=None):
	...
	live_reload = vmraid.utils.cint(os.environ.get("LIVE_RELOAD", vmraid.conf.live_reload))
