...
def get(doctype, name=None, filters=None, parent=None):
	...
	if filters and not name:
		name = vmraid.db.get_value(doctype, json.loads(filters))
		if not name:
...
def set_value(doctype, name, fieldname, value=None):
	...
	if fieldname!="idx" and fieldname in vmraid.model.default_fields:
...
def insert(doc=None):
	...
	if doc.get("parent") and doc.get("parenttype"):
		...
		parent = vmraid.get_doc(doc.get("parenttype"), doc.get("parent"))
...
def is_document_amended(doctype, docname):
	if vmraid.permissions.has_permission(doctype):
		try:
			return vmraid.db.exists(doctype, {
