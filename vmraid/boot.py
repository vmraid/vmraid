...
from vmraid.social.doctype.post.post import frequently_visited_links
from vmraid.core.doctype.navbar_settings.navbar_settings import get_navbar_settings, get_app_logo
...
def load_desktop_data(bootinfo):
	from vmraid.desk.desktop import get_desk_sidebar_items
...
def get_user_pages_or_reports(parent, cache=False):
	...
	pages_with_custom_roles = vmraid.db.sql("""
		select
	...
	pages_with_standard_roles = vmraid.db.sql("""
		select distinct
	...
	if parent =="Page":
		pages_with_no_roles = vmraid.db.sql("""
			select
...
def get_user_info():
	user_info = vmraid.db.get_all('User', fields=['`name`', 'full_name as fullname', 'user_image as image',
...
def load_print_css(bootinfo, print_settings):
	...
	bootinfo.print_css = vmraid.www.printview.get_print_style(print_settings.print_style or "Redesign", for_legacy=True)

def get_unseen_notes():
	return vmraid.db.sql('''select `name`, title, content, notify_on_every_login from `tabNote` where notify_on_login=1
		and expire_notification_on > %s and %s not in
			(select user from `tabNote Seen By` nsb
				where nsb.parent=`tabNote`.name)''', (vmraid.utils.now(), vmraid.session.user), as_dict=True)
...
def get_link_preview_doctypes():
	...
	customizations = vmraid.get_all("Property Setter",
...
def get_desk_settings():
	role_list = vmraid.get_all('Role', fields=['*'], filters=dict(
		name=['in', vmraid.get_roles()]
	))
	desk_settings = {}
