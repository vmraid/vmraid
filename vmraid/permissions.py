...
def check_admin_or_system_manager(user=None):
	...
	if ("System Manager" not in vmraid.get_roles(user)) and (user!="Administrator"):
		vmraid.throw(_("Not permitted"), vmraid.PermissionError)

def print_has_permission_check_logs(func):
	def inner(*args, **kwargs):
		vmraid.flags['has_permission_check_logs'] = []
		result = func(*args, **kwargs)
		self_perm_check = True if not kwargs.get('user') else kwargs.get('user') == vmraid.session.user
		...
		if not result and self_perm_check and raise_exception:
			msgprint(('<br>').join(vmraid.flags.get('has_permission_check_logs', [])))
		vmraid.flags.pop('has_permission_check_logs', None)
		return result
	return inner
...
def has_permission(doctype, ptype="read", doc=None, verbose=False, user=None, raise_exception=True):
	...
	if doc:
		if isinstance(doc, string_types):
			doc = vmraid.get_doc(meta.name, doc)
		perm = get_doc_permissions(doc, user=user, ptype=ptype).get(ptype)
		if not perm: push_perm_check_log(_('User {0} does not have access to this document').format(vmraid.bold(user)))
	else:
		...
		if not perm:
			push_perm_check_log(_('User {0} does not have doctype access via role permission for document {1}').format(vmraid.bold(user), vmraid.bold(doctype)))

	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			shared = vmraid.share.get_shared(doctype, user,
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	if vmraid.is_table(doc.doctype): return {"read": 1, "write": 1}
	...
	def is_user_owner():
		...
		session_user = vmraid.session.user.lower()
...
def get_role_permissions(doctype_meta, user=None, is_owner=None):
	...
	if isinstance(doctype_meta, string_types):
		doctype_meta = vmraid.get_meta(doctype_meta) # assuming doctype name was passed
	...
	if not vmraid.local.role_permissions.get(cache_key):
		perms = vmraid._dict(
			if_owner={}
		)
		...
		vmraid.local.role_permissions[cache_key] = perms

	return vmraid.local.role_permissions[cache_key]

def get_user_permissions(user):
	from vmraid.core.doctype.user_permission.user_permission import get_user_permissions
...
def has_user_permission(doc, user=None):
	...
	apply_strict_user_permissions = vmraid.get_system_settings('apply_strict_user_permissions')
	...
	def check_user_permission_on_link_fields(d):
		...
		meta = vmraid.get_meta(d.get("doctype"))
...
def has_controller_permissions(doc, ptype, user=None):
	...
	methods = vmraid.get_hooks("has_permission").get(doc.doctype, [])

	if not methods:
		return None

	for method in methods:
		controller_permission = vmraid.call(vmraid.get_attr(method), doc=doc, ptype=ptype, user=user)
...
def get_all_perms(role):
	...
	perms = vmraid.get_all('DocPerm', fields='*', filters=dict(role=role))
	custom_perms = vmraid.get_all('Custom DocPerm', fields='*', filters=dict(role=role))
	doctypes_with_custom_perms = vmraid.db.sql_list("""select distinct parent
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == 'Administrator':
			return [r[0] for r in vmraid.db.sql("select name from `tabRole`")] # return all available roles
		else:
			return [r[0] for r in vmraid.db.sql("""select role from `tabHas Role`
				where parent=%s and role not in ('All', 'Guest')""", (user,))] + ['All', 'Guest']

	roles = vmraid.cache().hget("roles", user, get)
...
def get_perms_for(roles, perm_doctype='DocPerm'):
	...
	return vmraid.db.get_all(perm_doctype, fields=['*'], filters=filters)

def get_doctypes_with_custom_docperms():
	'''Returns all the doctypes with Custom Docperms'''

	doctypes = vmraid.db.get_all('Custom DocPerm', fields=['parent'], distinct=1)
...
def set_user_permission_if_allowed(doctype, name, user, with_message=False):
	if get_role_permissions(vmraid.get_meta(doctype), user).set_user_permissions!=1:
...
def add_user_permission(doctype, name, user, ignore_permissions=False, applicable_for=None,
	...
	'''Add user permission'''
	from vmraid.core.doctype.user_permission.user_permission import user_permission_exists

	if not user_permission_exists(user, doctype, name, applicable_for):
		if not vmraid.db.exists(doctype, name):
			vmraid.throw(_("{0} {1} not found").format(_(doctype), name), vmraid.DoesNotExistError)
...
def remove_user_permission(doctype, name, user):
	user_permission_name = vmraid.db.get_value('User Permission',
		dict(user=user, allow=doctype, for_value=name))
	vmraid.delete_doc('User Permission', user_permission_name)

def clear_user_permissions_for_doctype(doctype, user=None):
	filters = {'allow': doctype}
	if user:
		filters['user'] = user
	user_permissions_for_doctype = vmraid.db.get_all('User Permission', filters=filters)
	for d in user_permissions_for_doctype:
		vmraid.delete_doc('User Permission', d.name)

def can_import(doctype, raise_exception=False):
	if not ("System Manager" in vmraid.get_roles() or has_permission(doctype, "import")):
		if raise_exception:
			raise vmraid.PermissionError("You are not allowed to import: {doctype}".format(doctype=doctype))
		else:
			return False
	return True
...
def can_export(doctype, raise_exception=False):
	if "System Manager" in vmraid.get_roles():
		...
		role_permissions = vmraid.permissions.get_role_permissions(doctype)
		has_access = role_permissions.get('export') or \
			role_permissions.get('if_owner').get('export')
		if not has_access and raise_exception:
			raise vmraid.PermissionError(_("You are not allowed to export {} doctype").format(doctype))
		return has_access

def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	'''Update a property in Custom Perm'''
	from vmraid.core.doctype.doctype.doctype import validate_permissions_for_doctype
	out = setup_custom_perms(doctype)

	name = vmraid.get_value('Custom DocPerm', dict(parent=doctype, role=role,
		permlevel=permlevel))
...
def setup_custom_perms(parent):
	...
	if not vmraid.db.exists('Custom DocPerm', dict(parent=parent)):
		copy_perms(parent)
		return True
...
def add_permission(doctype, role, permlevel=0, ptype=None):
	...
	if vmraid.db.get_value('Custom DocPerm', dict(parent=doctype, role=role,
...
def copy_perms(parent):
	...
	for d in vmraid.get_all('DocPerm', fields='*', filters=dict(parent=parent)):
		custom_perm = vmraid.new_doc('Custom DocPerm')
...
def reset_perms(doctype):
	...
	vmraid.db.sql("""delete from `tabCustom DocPerm` where parent=%s""", doctype)

def get_linked_doctypes(dt):
	return list(set([dt] + [d.options for d in
		vmraid.get_meta(dt).get("fields", {
			"fieldtype":"Link",
...
def push_perm_check_log(log):
	if vmraid.flags.get('has_permission_check_logs') == None: return
	vmraid.flags.get('has_permission_check_logs').append(_(log))
