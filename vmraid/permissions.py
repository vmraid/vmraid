...
def has_permission(
	...
	if doc:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have access to this document").format(vmraid.bold(user))
			)
	else:
		...
		if not perm:
			push_perm_check_log(
				_("User {0} does not have doctype access via role permission for document {1}").format(
					vmraid.bold(user), vmraid.bold(doctype)
				)
			)

	def false_if_not_shared():
		if ptype in ("read", "write", "share", "submit", "email", "print"):
			shared = vmraid.share.get_shared(
...
def get_doc_permissions(doc, user=None, ptype=None):
	...
	if vmraid.is_table(doc.doctype):
...
def get_all_perms(role):
	...
	doctypes_with_custom_perms = vmraid.get_all("Custom DocPerm", pluck="parent", distinct=True)
...
def get_roles(user=None, with_standard=True):
	...
	def get():
		if user == "Administrator":
			return vmraid.get_all("Role", pluck="name")  # return all available roles
		else:
...
def remove_user_permission(doctype, name, user):
	user_permission_name = vmraid.db.get_value(
...
def update_permission_property(doctype, role, permlevel, ptype, value=None, validate=True):
	...
	vmraid.qb.update(table).set(ptype, value).where(table.name == name).run()
...
def reset_perms(doctype):
	...
	vmraid.db.delete("Custom DocPerm", {"parent": doctype})
...
def get_linked_doctypes(dt):
	return list(
		set(
			...
			+ [
				d.options
				for d in vmraid.get_meta(dt).get(
					"fields",
...
def push_perm_check_log(log):
	if vmraid.flags.get("has_permission_check_logs") is None:
		return
...
def has_child_table_permission(
	...
	if child_doc:
		...
		parent_doc = vmraid.get_cached_doc(
		...
		if not is_parent_valid(child_doctype, parent_doctype):
			vmraid.throw(
				_("{0} is not a valid parent DocType for {1}").format(
					vmraid.bold(parent_doctype), vmraid.bold(child_doctype)
				),
				title=_("Invalid Parent DocType"),
			)
	else:
		vmraid.throw(
			_("Please specify a valid parent DocType for {0}").format(vmraid.bold(child_doctype)),
...
def is_parent_valid(child_doctype, parent_doctype):
	...
	parent_meta = vmraid.get_meta(parent_doctype)
