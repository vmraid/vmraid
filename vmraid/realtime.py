...
def publish_realtime(
	...
	if after_commit:
		...
		if params not in vmraid.local.realtime_log:
...
def get_redis_server():
	...
	if not redis_server:
		...
		redis_server = Redis.from_url(vmraid.conf.redis_socketio or "redis://localhost:12311")
	return redis_server
...
def get_doc_room(doctype, docname):
	return "".join([vmraid.local.site, ":doc:", doctype, "/", cstr(docname)])
