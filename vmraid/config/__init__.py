...
from vmraid.desk.moduleview import (get_data, get_onboard_items, config_exists, get_module_link_items_from_list)
...
def get_modules_from_all_apps_for_user(user=None):
	...
	global_blocked_modules = vmraid.get_doc('User', 'Administrator').get_blocked_modules()
	...
	for module in allowed_modules_list:
		...
		# Set defaults links
		module["links"] =  get_onboard_items(module["app"], vmraid.scrub(module_name))[:5]
...
def get_modules_from_app(app):
	return vmraid.get_all('Module Def',
		filters={'app_name': app},
		fields=['module_name', 'app_name as app']
	)

def get_all_empty_tables_by_module():
	empty_tables = set(r[0] for r in vmraid.db.multisql({
		"mariadb": """
			SELECT table_name
			FROM information_schema.tables
			WHERE table_rows = 0 and table_schema = "{}"
			""".format(vmraid.conf.db_name),
		"postgres": """
	...
	results = vmraid.get_all("DocType", fields=["name", "module"])
