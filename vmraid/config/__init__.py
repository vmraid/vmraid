...
from vmraid.desk.moduleview import (
	config_exists,
	get_data,
...
def get_all_empty_tables_by_module():
	table_rows = vmraid.qb.Field("table_rows")
	table_name = vmraid.qb.Field("table_name")
	information_schema = vmraid.qb.Schema("information_schema")

	empty_tables = (
		vmraid.qb.from_(information_schema.tables).select(table_name).where(table_rows == 0)
	).run()
