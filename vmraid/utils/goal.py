...
def get_monthly_results(goal_doctype, goal_field, date_col, filter_str, aggregation = 'sum'):
	...
	results = vmraid.db.sql('''SELECT {aggregation}(`{goal_field}`) AS {goal_field},
...
def get_monthly_goal_graph_data(title, doctype, docname, goal_value_field, goal_total_field, goal_history_field,
	...
	if not vmraid.has_permission(goal_doctype):
		return None
	...
	from vmraid.utils import today, getdate, formatdate, add_months
	...
	if month_to_value_dict is None:
		doc_filter = (goal_doctype_link + " = " + vmraid.db.escape(docname)) if doctype != goal_doctype else ''
		if filter_str:
