...
from vmraid.query_builder.functions import DateFormat, Function
from vmraid.query_builder.utils import DocType
from vmraid.utils.data import add_to_date, cstr, flt, now_datetime
...
def get_monthly_results(
	...
	date_format = "%m-%Y" if vmraid.db.db_type != "postgres" else "MM-YYYY"

	return dict(
		vmraid.db.query.build_conditions(table=goal_doctype, filters=filters)
		.select(
...
def get_monthly_goal_graph_data(
	...
	if history and "{" in cstr(history):
		with suppress(ValueError):
			month_to_value_dict = vmraid.parse_json(history)

	if month_to_value_dict is None:  # nosemgrep
		doc_filter = {}
		with suppress(ValueError):
			doc_filter = vmraid.parse_json(filters or "{}")
