...
import vmraid
from vmraid.utils import get_sites
...
def get_logger(module=None, with_more_info=False, allow_site=True, filter=None, max_size=100_000, file_count=20):
	...
	Returns:
		<class 'logging.Logger'>: Returns a Python logger object with Site and Chair level logging capabilities.
	"""

	if allow_site is True:
		site = getattr(vmraid.local, "site", None)
	...
	try:
		return vmraid.loggers[logger_name]
	except KeyError:
		pass
	...
	logger.setLevel(vmraid.log_level or default_log_level)
	...
	vmraid.loggers[logger_name] = logger
...
class SiteContextFilter(logging.Filter):
	...
	def filter(self, record):
		if "Form Dict" not in text_type(record.msg):
			site = getattr(vmraid.local, "site", None)
			form_dict = getattr(vmraid.local, "form_dict", None)
...
def set_log_level(level):
	...
	vmraid.log_level = getattr(logging, (level or "").upper(), None) or default_log_level
	vmraid.loggers = {}
