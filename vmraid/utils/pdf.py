...
from vmraid.utils.jinja_globals import bundled_asset, is_rtl
...
def get_pdf(html, options=None, output=None):
	...
	try:
		...
		if any([error in str(e) for error in PDF_CONTENT_ERRORS]):
			if not filedata:
				print(html, options)
				vmraid.throw(_("PDF generation failed because of broken image links"))
...
def prepare_options(html, options):
	...
	# page size
	pdf_page_size = (
		options.get("page-size") or vmraid.db.get_single_value("Print Settings", "pdf_page_size") or "A4"
	)

	if pdf_page_size == "Custom":
		options["page-height"] = options.get("page-height") or vmraid.db.get_single_value(
			"Print Settings", "pdf_page_height"
		)
		options["page-width"] = options.get("page-width") or vmraid.db.get_single_value(
...
def prepare_header_footer(soup):
	...
	css = vmraid.read_file(os.path.join(vmraid.local.sites_path, print_css))
	...
	for html_id in ("header-html", "footer-html"):
		...
		if content:
			...
			# create temp file
			fname = os.path.join("/tmp", "vmraid-pdf-{0}.html".format(vmraid.generate_hash()))
			with open(fname, "wb") as f:
				f.write(html.encode("utf-8"))

			# {"header-html": "/tmp/vmraid-pdf-random.html"}
...
def get_wkhtmltopdf_version():
	wkhtmltopdf_version = vmraid.cache().hget("wkhtmltopdf_version", None)

	if not wkhtmltopdf_version:
		try:
			res = subprocess.check_output(["wkhtmltopdf", "--version"])
			wkhtmltopdf_version = res.decode("utf-8").split(" ")[1]
			vmraid.cache().hset("wkhtmltopdf_version", None, wkhtmltopdf_version)
		except Exception:
			pass
