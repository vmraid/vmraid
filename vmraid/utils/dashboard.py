...
def generate_and_cache_results(args, function, cache_key, chart):
	...
	vmraid.db.set_value(
		"Dashboard Chart", args.chart_name, "last_synced_on", vmraid.utils.now(), update_modified=False
	)
	return results
...
def get_dashboards_with_link(docname, doctype):
	...
	if doctype == "Dashboard Chart":
		links = vmraid.get_all("Dashboard Chart Link", fields=["parent"], filters={"chart": docname})
	elif doctype == "Number Card":
		links = vmraid.get_all("Number Card Link", fields=["parent"], filters={"card": docname})
