...
import mimetypes
import io
from vmraid.utils import get_hook_method, get_files_path, random_string, encode, cstr, call_hook_method, cint
...
def get_file_url(file_data_name):
	data = vmraid.db.get_value("File", file_data_name, ["file_name", "file_url"], as_dict=True)
	return data.file_url or data.file_name


def upload():
	# get record details
	dt = vmraid.form_dict.doctype
	dn = vmraid.form_dict.docname
	file_url = vmraid.form_dict.file_url
	filename = vmraid.form_dict.filename
	vmraid.form_dict.is_private = cint(vmraid.form_dict.is_private)

	if not filename and not file_url:
		vmraid.msgprint(_("Please select a file or url"),
	...
	comment = {}
	if dt and dn:
		comment = vmraid.get_doc(dt, dn).add_comment("Attachment",
...
def save_url(file_url, filename, dt, dn, folder, is_private, df=None):
	...
	# 	vmraid.msgprint("URL must start with 'http://' or 'https://'")
	# 	return None, None

	file_url = unquote(file_url)
	file_size = vmraid.form_dict.file_size
	...
	try:
		...
		return vmraid.get_doc("File", f.duplicate_entry)
	return f
...
def get_uploaded_content():
	...
	if 'filedata' in vmraid.form_dict:
		...
		vmraid.uploaded_filename = vmraid.form_dict.filename
		return vmraid.uploaded_filename, vmraid.uploaded_content
	else:
...
def get_file_data_from_hash(content_hash, is_private=0):
	for name in vmraid.db.sql_list("select name from `tabFile` where content_hash=%s and is_private=%s", (content_hash, is_private)):
...
def remove_file(fid, attached_to_doctype=None, attached_to_name=None, from_delete=False):
	...
	vmraid.delete_doc("File", fid, ignore_permissions=ignore_permissions)
...
def get_file_path(file_name):
	...
	f = vmraid.db.sql("""select file_url from `tabFile`
...
def get_file_name(fname, optional_suffix):
	...
	n_records = vmraid.db.sql("select name from `tabFile` where file_name=%s", fname)
...
def download_file(file_url):
	...
	Method : GET
	Endpoint : vmraid.utils.file_manager.download_file
	...
	vmraid.local.response.filecontent = filedata
...
def add_attachments(doctype, name, attachments):
	...
	for a in attachments:
		if isinstance(a, string_types):
			attach = vmraid.db.get_value("File", {"name":a}, ["file_name", "file_url", "is_private"], as_dict=1)
