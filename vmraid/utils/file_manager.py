...
def upload():
	...
	if not filename and not file_url:
		vmraid.msgprint(_("Please select a file or url"), raise_exception=True)

	file_doc = get_file_doc()

	comment = {}
	if dt and dn:
		comment = vmraid.get_doc(dt, dn).add_comment(
			"Attachment",
			_("added {0}").format(
...
def get_uploaded_content():
	...
	if "filedata" in vmraid.form_dict:
		...
		vmraid.uploaded_content = safe_b64decode(vmraid.form_dict.filedata)
...
def get_file_name(fname, optional_suffix):
	...
	n_records = vmraid.get_all("File", {"file_name": fname}, pluck="name")
