...
import vmraid
import json, os
from vmraid.modules import scrub, get_module_path, utils
from vmraid.custom.doctype.customize_form.customize_form import doctype_properties, docfield_properties
...
def reset_all():
	for doctype in vmraid.db.get_all('DocType', dict(custom=0)):
		print(doctype.name)
...
def reset_doc(doctype):
	...
	try:
		git_link = '/'.join(['https://raw.githubusercontent.com/vmraid',\
...
def remove_duplicate_fields(doctype):
	for field in vmraid.db.sql('''select fieldname, count(1) as cnt from tabDocField where parent=%s group by fieldname having cnt > 1''', doctype):
		vmraid.db.sql('delete from tabDocField where fieldname=%s and parent=%s limit 1', (field[0], doctype))
...
def make_custom_fields(doctype, local_doc, original_doc):
	...
	doctype_doc = vmraid.get_doc('DocType', doctype)
...
def get_custom_docfield_properties():
	fields_meta = vmraid.get_meta('Custom Field').fields
	fields = {}
...
def setup_perms_for(doctype):
	perms = vmraid.get_all('DocPerm', fields='*', filters=dict(parent=doctype), order_by='idx asc')
	# get default perms
	try:
	...
	if len(standard_perms) != len(perms):
		...
		for i, p in enumerate(perms):
			...
			for fieldname in vmraid.get_meta('DocPerm').get_fieldnames_with_value():
