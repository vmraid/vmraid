...
import json
import os
from vmraid.utils import cint, strip_html_tags
...
def reset():
	...
	:return:
	"""
	vmraid.db.sql('DELETE FROM `__global_search`')
...
def get_doctypes_with_global_search(with_child_tables=True):
	...
	def _get():
		...
		for d in vmraid.get_all('DocType', fields=['name', 'module'], filters=filters):
			meta = vmraid.get_meta(d.name)
			if len(meta.get_global_search_fields()) > 0:
				global_search_doctypes.append(d)

		installed_apps = vmraid.get_installed_apps()
		module_app = vmraid.local.module_app

		doctypes = [
			d.name for d in global_search_doctypes
			if module_app.get(vmraid.scrub(d.module))
			and module_app[vmraid.scrub(d.module)] in installed_apps
		]

		return doctypes

	return vmraid.cache().get_value('doctypes_with_global_search', _get)
...
def rebuild_for_doctype(doctype):
	...
	"""
	if vmraid.local.conf.get('disable_global_search'):
		return
	...
	def _get_filters():
		filters = vmraid._dict({ "docstatus": ["!=", 2] })
	...
	if cint(meta.istable) == 1:
		parent_doctypes = vmraid.get_all("DocField", fields="parent", filters={
	...
	all_records = vmraid.get_all(doctype, fields=fieldnames, filters=_get_filters())
		...
		if content:
			...
			try:
				if hasattr(get_controller(doctype), "is_website_published") and meta.allow_guest_to_view:
					d = vmraid.get_doc(doctype, doc.name)
			...
			all_contents.append({
				"doctype": vmraid.db.escape(doctype),
				"name": vmraid.db.escape(doc.name),
				"content": vmraid.db.escape(' ||| '.join(content or '')),
				"published": published,
				"title": vmraid.db.escape((title or '')[:int(vmraid.db.VARCHAR_LEN)]),
				"route": vmraid.db.escape((route or '')[:int(vmraid.db.VARCHAR_LEN)])
			})
	if all_contents:
...
def get_children_data(doctype, meta):
	...
	"""
	all_children = vmraid._dict()
	child_search_fields = vmraid._dict()

	for child in meta.get_table_fields():
		child_meta = vmraid.get_meta(child.options)
		...
		if search_fields:
			...
			child_records = vmraid.get_all(child.options, fields=child_fieldnames, filters={
				"docstatus": ["!=", 1],
				"parenttype": doctype
			})

			for record in child_records:
				all_children.setdefault(record.parent, vmraid._dict())\
...
def update_global_search(doc):
	...
	for field in doc.meta.get_global_search_fields():
		if doc.get(field.fieldname) and field.fieldtype not in vmraid.model.table_fields:
	...
	if content:
		...
		title = (doc.get_title() or '')[:int(vmraid.db.VARCHAR_LEN)]
...
def update_global_search_for_all_web_pages():
	if vmraid.conf.get('disable_global_search'):
		return
...
def get_routes_to_index():
	...
	routes_to_index = []
	for app in apps:
		base = vmraid.get_app_path(app, 'www')
		path_to_index = vmraid.get_app_path(app, 'www')
...
def add_route_to_global_search(route):
	...
	try:
		...
		sync_value_in_queue(value)
	except (vmraid.PermissionError, vmraid.DoesNotExistError, vmraid.ValidationError, Exception):
		pass
...
def get_formatted_value(value, field):
	...
	if getattr(field, 'fieldtype', None) in ["Text", "Text Editor"]:
		value = unescape_html(vmraid.safe_decode(value))
...
def sync_global_search():
	...
	:return:
	"""
	while vmraid.cache().llen('global_search_queue') > 0:
		value = json.loads(vmraid.cache().lpop('global_search_queue').decode('utf-8'))
		sync_value(value)

def sync_value_in_queue(value):
	try:
		# append to search queue if connected
		vmraid.cache().lpush('global_search_queue', json.dumps(value))
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from vmraid.desk.doctype.global_search_settings.global_search_settings import get_doctypes_for_global_search
	...
	for text in set(text.split('&')):
		...
		mariadb_text = vmraid.db.escape('+' + text + '*')

		mariadb_fields = '`doctype`, `name`, `content`, MATCH (`content`) AGAINST ({} IN BOOLEAN MODE) AS rank'.format(mariadb_text)
		postgres_fields = '`doctype`, `name`, `content`, TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({}) AS rank'.format(vmraid.db.escape(text))
	...
	for doctype in allowed_doctypes:
		for index, r in enumerate(results):
			if r.doctype == doctype and r.rank > 0.0:
				try:
					meta = vmraid.get_meta(r.doctype)
					if meta.image_field:
						r.image = vmraid.db.get_value(r.doctype, r.name, meta.image_field)
				except Exception:
...
def web_search(text, scope=None, start=0, limit=20):
	...
	for text in texts:
		...
		mariadb_conditions += 'MATCH(`content`) AGAINST ({} IN BOOLEAN MODE)'.format(vmraid.db.escape('+' + text + '*'))
		postgres_conditions += 'TO_TSVECTOR("content") @@ PLAINTO_TSQUERY({})'.format(vmraid.db.escape(text))
