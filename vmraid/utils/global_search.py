...
def reset():
	...
	:return:
	"""
	vmraid.db.delete("__global_search")
...
def rebuild_for_doctype(doctype):
	...
	if cint(meta.istable) == 1:
		parent_doctypes = vmraid.get_all(
			"DocField",
			fields="parent",
			filters={"fieldtype": ["in", vmraid.model.table_fields], "options": doctype},
		)
...
def delete_global_search_records_for_doctype(doctype):
	vmraid.db.delete("__global_search", {"doctype": doctype})
...
def get_children_data(doctype, meta):
	...
	for child in meta.get_table_fields():
		...
		if search_fields:
			...
			child_records = vmraid.get_all(
				child.options, fields=child_fieldnames, filters={"docstatus": ["!=", 1], "parenttype": doctype}
			)

			for record in child_records:
				all_children.setdefault(record.parent, vmraid._dict()).setdefault(child.options, []).append(
					record
				)
...
def update_global_search(doc):
	...
	if content:
		...
		title = (cstr(doc.get_title()) or "")[: int(vmraid.db.VARCHAR_LEN)]
...
def sync_global_search():
	...
	while vmraid.cache().llen("global_search_queue") > 0:
		...
		value = json.loads(vmraid.cache().rpop("global_search_queue").decode("utf-8"))
		sync_value(value)
...
def delete_for_document(doc):
	...
	"""
	vmraid.db.delete("__global_search", {"doctype": doc.doctype, "name": doc.name})
...
def search(text, start=0, limit=20, doctype=""):
	...
	"""
	from vmraid.desk.doctype.global_search_settings.global_search_settings import (
		get_doctypes_for_global_search,
	)
	from vmraid.query_builder.functions import Match
