...
def get_decrypted_password(doctype, name, fieldname='password', raise_exception=True):
	auth = vmraid.db.sql('''select `password` from `__Auth`
...
def set_encrypted_password(doctype, name, pwd, fieldname='password'):
	try:
		vmraid.db.sql("""insert into `__Auth` (doctype, name, fieldname, `password`, encrypted)
			values (%(doctype)s, %(name)s, %(fieldname)s, %(pwd)s, 1)
			{on_duplicate_update} `password`=%(pwd)s, encrypted=1""".format(
				on_duplicate_update=vmraid.db.get_on_duplicate_update(['doctype', 'name', 'fieldname'])
			), { 'doctype': doctype, 'name': name, 'fieldname': fieldname, 'pwd': encrypt(pwd) })
	except vmraid.db.DataError as e:
		if ((vmraid.db.db_type == 'mariadb' and e.args[0] == DATA_TOO_LONG) or
			(vmraid.db.db_type == 'postgres' and e.pgcode == STRING_DATA_RIGHT_TRUNCATION)):
			vmraid.throw("Most probably your password is too long.", exc=e)
		raise e
...
def check_password(user, pwd, doctype='User', fieldname='password', delete_tracker_cache=True):
	...
	auth = vmraid.db.sql("""select `name`, `password` from `__Auth`
...
def delete_all_passwords_for(doctype, name):
	try:
		vmraid.db.sql("""delete from `__Auth` where `doctype`=%(doctype)s and `name`=%(name)s""",
...
def rename_password(doctype, old_name, new_name):
	...
	vmraid.db.sql("""update `__Auth` set name=%(new_name)s
...
def rename_password_field(doctype, old_fieldname, new_fieldname):
	vmraid.db.sql('''update `__Auth` set fieldname=%(new_fieldname)s
...
def decrypt(pwd):
	try:
		...
		vmraid.throw(_('Encryption key is invalid, Please check site_config.json'))
