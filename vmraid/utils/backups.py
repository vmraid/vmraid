...
from vmraid.utils import cint, get_file_size, get_url, now, now_datetime
from vmraid.utils.password import get_encryption_key
...
class BackupGenerator:
	...
	def __init__(
		...
		site = vmraid.local.site or vmraid.generate_hash(length=8)
	...
	def setup_backup_tables(self):
		...
		existing_tables = vmraid.db.get_tables()
		...
		specified_tables = get_tables(vmraid.conf.get("backup", {}).get("includes", []))
		include_tables = (specified_tables + base_tables) if specified_tables else []

		conf_tables = {
			"include": include_tables,
			"exclude": get_tables(vmraid.conf.get("backup", {}).get("excludes", [])),
		}
	...
	def get_backup(self, older_than=24, ignore_files=False, force=False):
		...
		if not (last_db and last_file and last_private_file and site_config_backup_path):
			...
			if vmraid.get_system_settings("encrypt_backup"):
	...
	def set_backup_file_name(self):
		...
		enc = "-enc" if vmraid.get_system_settings("encrypt_backup") else ""
		...
		for path in paths:
			if os.path.exists(path):
				...
				try:
					...
					vmraid.utils.execute_in_shell(command)
	...
	def get_recent_backup(self, older_than, partial=False):
		...
		if not vmraid.get_system_settings("encrypt_backup"):
			file_type_slugs = {
	...
	def print_summary(self):
		...
		print("Backup Summary for {0} at {1}".format(vmraid.local.site, now()))
	...
	def backup_files(self):
		for folder in ("public", "private"):
			files_path = vmraid.get_site_path(folder, "files")
	...
	def copy_site_config(self):
		...
		site_config_path = os.path.join(vmraid.get_site_path(), "site_config.json")
	...
	def take_dump(self):
		...
		if not (gzip_exc and db_exc[1]):
			...
			vmraid.throw(
				f"{_exc} not found in PATH! This is required to take a backup.", exc=vmraid.ExecutableNotFound
			)
		db_exc = db_exc[0]

		database_header_content = [
			f"Backup generated by VMRaid {vmraid.__version__} on branch {get_app_branch('vmraid') or 'N/A'}",
			"",
		]

		# escape reserved characters
		args = vmraid._dict(
			[item[0], vmraid.utils.esc(str(item[1]), "$ ")] for item in self.__dict__.copy().items()
		)
		...
		if self.partial:
			...
			database_header_content.extend(
				[
					f"Partial Backup of VMRaid Site {vmraid.local.site}",
		...
		if self.db_type == "postgres":
			...
			if self.backup_includes:
				...
				args["exclude"] = " ".join(
					[
						"--ignore-table='{0}.{1}'".format(vmraid.conf.db_name, table)
		...
		vmraid.utils.execute_in_shell(command, low_priority=True)

	def send_email(self):
		"""
		Sends the link to backup file located at erpadda/backups
		"""
		from vmraid.email import get_system_managers
...
download only after 24 hours.""" % {
		...
		vmraid.sendmail(recipients=recipient_list, message=msg, subject=subject)
...
def fetch_latest_backups(partial=False):
	...
	odb = BackupGenerator(
		vmraid.conf.db_name,
		vmraid.conf.db_name,
		vmraid.conf.db_password,
		db_host=vmraid.db.host,
		db_type=vmraid.conf.db_type,
		db_port=vmraid.conf.db_port,
	)
...
def delete_temp_backups(older_than=24):
	...
	"""
	older_than = cint(vmraid.conf.keep_backups_for_hours) or older_than
	backup_path = get_backup_path()
...
def get_backup_path():
	backup_path = vmraid.utils.get_site_path(conf.get("backup_path", "private/backups"))
	return backup_path
...
def get_backup_encryption_key():
	vmraid.only_for("System Manager")
	return vmraid.conf.encryption_key
