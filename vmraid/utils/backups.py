...
from vmraid.utils import get_file_size, get_url, now, now_datetime, cint
...
class BackupGenerator:
	...
	def setup_backup_tables(self):
		...
		existing_doctypes = set([x.name for x in vmraid.get_all("DocType")])
	...
	def take_dump(self):
		...
		args = vmraid._dict(
			[item[0], vmraid.utils.esc(str(item[1]), "$ ")]
...
download only after 24 hours.""" % {
		...
		vmraid.sendmail(recipients=recipient_list, msg=msg, subject=subject)
