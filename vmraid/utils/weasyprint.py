...
def download_pdf(doctype, name, print_format, letterhead=None):
	...
	vmraid.local.response.filename = "{name}.pdf".format(
		name=name.replace(" ", "-").replace("/", "-")
	)
	vmraid.local.response.filecontent = pdf
	vmraid.local.response.type = "pdf"
...
class PrintFormatGenerator:
	...
	def __init__(self, print_format, doc, letterhead=None):
		...
		"""
		self.base_url = vmraid.utils.get_url()
		self.print_format = vmraid.get_doc("Print Format", print_format)
		self.doc = doc
		self.letterhead = vmraid.get_doc("Letter Head", letterhead) if letterhead else None
		self.build_context()
		self.layout = self.get_layout(self.print_format)
		self.context.layout = self.layout

	def build_context(self):
		self.print_settings = vmraid.get_doc("Print Settings")
		...
		print_style = (
			vmraid.get_doc("Print Style", self.print_settings.print_style)
	...
	def get_main_html(self):
		self.context.css = vmraid.render_template(
			"templates/print_format/print_format.css", self.context
		)
		return vmraid.render_template("templates/print_format/print_format.html", self.context)

	def get_header_footer_html(self):
		header_html = footer_html = None
		if self.letterhead:
			header_html = vmraid.render_template("templates/print_format/print_header.html", self.context)
		if self.letterhead:
			footer_html = vmraid.render_template("templates/print_format/print_footer.html", self.context)
	...
	def get_layout(self, print_format):
		layout = vmraid.parse_json(print_format.format_data)
	...
	def process_margin_texts(self, layout):
		...
		for key in margin_texts:
			...
			if text and "{{" in text:
				layout["text_" + key] = vmraid.render_template(text, self.context)
