...
class RedisWrapper(redis.Redis):
	...
	def make_key(self, key, user=None, shared=False):
		...
		return "{0}|{1}".format(vmraid.conf.db_name, key).encode('utf-8')
	...
	def get_value(self, key, generator=None, user=None, expires=False, shared=False):
		...
		:param expires: If the key is supposed to be with an expiry, don't store it in vmraid.local
		"""
		original_key = key
		key = self.make_key(key, user, shared)

		if key in vmraid.local.cache:
			val = vmraid.local.cache[key]
	...
	def get_keys(self, key):
		...
		try:
			...
			return [k for k in list(vmraid.local.cache) if regex.match(cstr(k))]
	...
	def hset(self, name, key, value, shared=False):
		...
		# set in local
		if not _name in vmraid.local.cache:
			vmraid.local.cache[_name] = {}
		vmraid.local.cache[_name][key] = value
	...
	def hget(self, name, key, generator=None, shared=False):
		...
		if key in vmraid.local.cache[_name]:
			return vmraid.local.cache[_name][key]
	...
	def hdel(self, name, key, shared=False):
		...
		if _name in vmraid.local.cache:
			if key in vmraid.local.cache[_name]:
				del vmraid.local.cache[_name][key]
		try:
	...
	def hdel_keys(self, name_starts_with, key):
		...
		for name in vmraid.cache().get_keys(name_starts_with):
			name = name.split("|", 1)[1]
			self.hdel(name, key)
