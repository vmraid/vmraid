...
def get_first_day_of_the_week():
	return vmraid.get_system_settings("first_day_of_the_week") or "Sunday"
...
def getdate(string_date: Optional[str] = None):
	...
	try:
		...
		vmraid.throw(
			vmraid._("{} is not a valid date string.").format(vmraid.bold(string_date)),
			title=vmraid._("Invalid Date"),
		)
...
def add_to_date(
	...
	if isinstance(date, str):
		...
		try:
			...
			vmraid.throw(vmraid._("Please select a valid date filter"), title=vmraid._("Invalid Date"))
...
def _get_time_zone():
	return vmraid.db.get_system_setting("time_zone") or "Asia/Kolkata"  # Default to India ?!
...
def get_time_zone():
	...
	return vmraid.cache().get_value("time_zone", _get_time_zone)
...
def now():
	...
	if vmraid.flags.current_date:
		return (
			getdate(vmraid.flags.current_date).strftime(DATE_FORMAT)
			+ " "
...
def get_user_date_format():
	...
	if getattr(vmraid.local, "user_date_format", None) is None:
		vmraid.local.user_date_format = vmraid.db.get_default("date_format")

	return vmraid.local.user_date_format or "yyyy-mm-dd"
...
def get_user_time_format():
	...
	if getattr(vmraid.local, "user_time_format", None) is None:
		vmraid.local.user_time_format = vmraid.db.get_default("time_format")

	return vmraid.local.user_time_format or "HH:mm:ss"
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string, locale=(vmraid.local.lang or "").replace("-", "_")
		)
	...
	* HH:mm
	...
	try:
		formatted_time = babel.dates.format_time(
			time_, format_string, locale=(vmraid.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(
			datetime, format_string, locale=(vmraid.local.lang or "").replace("-", "_")
		)
	except UnknownLocaleError:
...
def validate_duration_format(duration):
	...
	if not is_valid_duration:
		vmraid.throw(
			vmraid._("Value {0} must be in the valid duration format: d h m s").format(
				vmraid.bold(duration)
			)
		)
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(
		date, locale=(vmraid.local.lang or "en").replace("-", "_"), format=format
	)
	return formatted_date
...
def cast_fieldtype(fieldtype, value, show_warning=True):
	if show_warning:
		message = (
			"Function `vmraid.utils.data.cast` has been deprecated in favour"
			" of `vmraid.utils.data.cast`. Use the newer util for safer type casting."
		)
...
def cast(fieldtype, value=None):
	"""Cast the value to the Python native object of the VMRaid fieldtype provided.
	...
	Mapping of Python types => VMRaid types:
...
def cstr(s, encoding="utf-8"):
	return vmraid.as_unicode(s, encoding)
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(
		vmraid.db.get_value("Currency", currency, "smallest_currency_fraction_value", cache=True)
	)
...
def fmt_money(amount, precision=None, currency=None, format=None):
	...
	"""
	number_format = format or vmraid.db.get_default("number_format") or "#,###.##"
	if precision is None:
		precision = cint(vmraid.db.get_default("currency_precision")) or None
	...
	if decimal_str:
		...
		if precision > 2:
			if len(decimals) < 3:
				if currency:
					fraction = vmraid.db.get_value("Currency", currency, "fraction_units", cache=True) or 100
	...
	if currency and vmraid.defaults.get_global_default("hide_currency_symbol") != "Yes":
		symbol = vmraid.db.get_value("Currency", currency, "symbol", cache=True) or currency
		amount = vmraid._(symbol) + " " + amount
...
def money_in_words(
	...
	"""
	from vmraid.utils import get_defaults
	...
	if not fraction_currency:
		fraction_currency = vmraid.db.get_value("Currency", main_currency, "fraction", cache=True) or _(
			"Cent"
		)

	number_format = (
		vmraid.db.get_value("Currency", main_currency, "number_format", cache=True)
...
def in_words(integer, in_million=True):
	...
	locale = "en_IN" if not in_million else vmraid.local.lang
	integer = int(integer)
	try:
...
def get_thumbnail_base64_for_image(src):
	...
	from vmraid import cache, safe_decode
	from vmraid.core.doctype.file.file import get_local_image

	if not src:
		vmraid.throw("Invalid source for image: {0}".format(src))
	...
	def _get_base64():
		file_path = vmraid.get_site_path("public", src.lstrip("/"))
...
def pdf_to_base64(filename):
	from vmraid.utils.file_manager import get_file_path
...
def comma_or(some_list, add_quotes=True):
	return comma_sep(some_list, vmraid._("{0} or {1}"), add_quotes)


def comma_and(some_list, add_quotes=True):
	return comma_sep(some_list, vmraid._("{0} and {1}"), add_quotes)
...
def comma_sep(some_list, pattern, add_quotes=True):
	if isinstance(some_list, (list, tuple)):
		...
		if not some_list:
			...
			return pattern.format(", ".join(vmraid._(s) for s in some_list[:-1]), some_list[-1])
	else:
		return some_list
...
def get_url(uri=None, full_address=False):
	...
	host_name = vmraid.local.conf.host_name or vmraid.local.conf.hostname
	...
	if not host_name:
		...
		elif vmraid.local.site:
			protocol = "http://"

			if vmraid.local.conf.ssl_certificate:
				protocol = "https://"

			elif vmraid.local.conf.wildcard:
				domain = vmraid.local.conf.wildcard.get("domain")
				if (
					domain
					and vmraid.local.site.endswith(domain)
					and vmraid.local.conf.wildcard.get("ssl_certificate")
				):
					protocol = "https://"

			host_name = protocol + vmraid.local.site

		else:
			host_name = vmraid.db.get_value("Website Settings", "Website Settings", "subdomain")
	...
	if not uri and full_address:
		uri = vmraid.get_request_header("REQUEST_URI", "")

	port = vmraid.conf.http_port or vmraid.conf.webserver_port

	if (
		not (vmraid.conf.restart_supervisor_on_update or vmraid.conf.restart_systemd_on_update)
		and host_name
...
def get_host_name_from_request():
	if hasattr(vmraid.local, "request") and vmraid.local.request and vmraid.local.request.host:
		protocol = (
			"https://" if "https" == vmraid.get_request_header("X-Forwarded-Proto", "") else "http://"
		)
		return protocol + vmraid.local.request.host
...
def get_filter(doctype: str, f: Union[Dict, List, Tuple], filters_config=None) -> "vmraid._dict":
	...
	if not isinstance(f, (list, tuple)):
		vmraid.throw(vmraid._("Filter must be a tuple or list (in a list)"))
	...
	if len(f) == 3:
		...
		vmraid.throw(
			vmraid._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f))
		)

	f = vmraid._dict(doctype=f[0], fieldname=f[1], operator=f[2], value=f[3])
	...
	if f.operator.lower() not in valid_operators:
		vmraid.throw(vmraid._("Operator must be one of {0}").format(", ".join(valid_operators)))
	...
	if f.doctype and (f.fieldname not in default_fields + optional_fields + child_table_fields):
		...
		meta = vmraid.get_meta(f.doctype)
		if not meta.has_field(f.fieldname):

			# try and match the doctype name from child tables
			for df in meta.get_table_fields():
				if vmraid.get_meta(df.options).has_field(f.fieldname):
					f.doctype = df.options
					break

	try:
		df = vmraid.get_meta(f.doctype).get_field(f.fieldname)
	except vmraid.exceptions.DoesNotExistError:
		df = None
...
def sanitize_column(column_name):
	...
	def _raise_exception():
		vmraid.throw(_("Invalid field name {0}").format(column_name), vmraid.DataError)
...
def generate_hash(*args, **kwargs) -> str:
	return vmraid.generate_hash(*args, **kwargs)
...
def get_user_info_for_avatar(user_id: str) -> Dict:
	try:
		user = vmraid.get_cached_doc("User", user_id)
...
def validate_python_code(string: str, fieldname=None, is_expression: bool = True) -> None:
	...
	try:
		...
		msg = vmraid._("{} Invalid python code on line {}").format(
			fieldname + ":" if fieldname else "", line_no + 1
		)
		msg += f"<br><pre>{error_line}</pre>"
		msg += f"<pre>{' ' * offset}^</pre>"

		vmraid.throw(msg, title=vmraid._("Syntax Error"))
	except Exception as e:
		vmraid.msgprint(
			vmraid._("{} Possibly invalid python code. <br>{}").format(fieldname + ": " or "", str(e)),
			indicator="orange",
		)
...
def format_timedelta(o: datetime.timedelta) -> str:
	...
	# but vmraid doesnt - i think via babel : only allows 0..23 range for hour
