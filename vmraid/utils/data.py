...
def getdate(string_date=None):
	...
	try:
		...
		vmraid.throw(vmraid._('{} is not a valid date string.').format(
			vmraid.bold(string_date)
...
def now():
	...
	if vmraid.flags.current_date:
		return getdate(vmraid.flags.current_date).strftime(DATE_FORMAT) + " " + \
	...
	* dd/mm/yyyy
	...
	try:
		formatted_date = babel.dates.format_date(
			date, format_string,
			locale=(vmraid.local.lang or "").replace("-", "_"))
...
def format_datetime(datetime_string, format_string=None):
	...
	* mm-dd-yyyy HH:mm
	...
	try:
		formatted_datetime = babel.dates.format_datetime(datetime, format_string, locale=(vmraid.local.lang or "").replace("-", "_"))
	except UnknownLocaleError:
...
def validate_duration_format(duration):
	...
	if not is_valid_duration:
		vmraid.throw(vmraid._("Value {0} must be in the valid duration format: d h m s").format(vmraid.bold(duration)))
...
def global_date_format(date, format="long"):
	...
	date = getdate(date)
	formatted_date = babel.dates.format_date(date, locale=(vmraid.local.lang or "en").replace("-", "_"), format=format)
	return formatted_date
...
def round_based_on_smallest_currency_fraction(value, currency, precision=2):
	smallest_currency_fraction_value = flt(vmraid.db.get_value("Currency",
...
def money_in_words(number, main_currency = None, fraction_currency=None):
	...
	if not fraction_currency:
		fraction_currency = vmraid.db.get_value("Currency", main_currency, "fraction", cache=True) or _("Cent")

	number_format = vmraid.db.get_value("Currency", main_currency, "number_format", cache=True) or \
		vmraid.db.get_default("number_format") or "#,###.##"
...
def get_thumbnail_base64_for_image(src):
	...
	from vmraid import safe_decode, cache
...
def get_url(uri=None, full_address=False):
	...
	if not host_name:
		...
		if request_host_name:
			...
			if vmraid.local.conf.ssl_certificate:
				...
				if domain and vmraid.local.site.endswith(domain) and vmraid.local.conf.wildcard.get('ssl_certificate'):
					protocol = 'https://'

			host_name = protocol + vmraid.local.site

		else:
			host_name = vmraid.db.get_value("Website Settings", "Website Settings",
				"subdomain")
	...
	if not (vmraid.conf.restart_supervisor_on_update or vmraid.conf.restart_systemd_on_update) and host_name and not url_contains_port(host_name) and port:
...
def get_host_name_from_request():
	if hasattr(vmraid.local, "request") and vmraid.local.request and vmraid.local.request.host:
		protocol = 'https://' if 'https' == vmraid.get_request_header('X-Forwarded-Proto', "") else 'http://'
...
def get_filter(doctype, f, filters_config=None):
	...
	if len(f) == 3:
		...
		vmraid.throw(vmraid._("Filter must have 4 values (doctype, fieldname, operator, value): {0}").format(str(f)))
...
def get_user_info_for_avatar(user_id):
	...
	try:
		user_info["email"] = vmraid.get_cached_value("User", user_id, "email")
		user_info["name"] = vmraid.get_cached_value("User", user_id, "fullname")
		user_info["image"] = vmraid.get_cached_value("User", user_id, "user_image")
	except Exception:
