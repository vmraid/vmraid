...
def before_install():
	vmraid.reload_doc("core", "doctype", "doctype_state")
	...
	vmraid.reload_doc("desk", "doctype", "form_tour_step")
	vmraid.reload_doc("desk", "doctype", "form_tour")
...
def after_install():
	...
	# all roles to admin
	vmraid.get_doc("User", "Administrator").add_roles(*vmraid.get_all("Role", pluck="name"))
...
def create_user_type():
	for user_type in ["System User", "Website User"]:
		if not vmraid.db.exists("User Type", user_type):
			vmraid.get_doc({"doctype": "User Type", "name": user_type, "is_standard": 1}).insert(
...
def install_basic_docs():
	...
	for d in install_docs:
		try:
			vmraid.get_doc(d).insert(ignore_if_duplicate=True)
...
def before_tests():
	...
	vmraid.db.truncate("Custom Field")
	vmraid.db.truncate("Event")
...
def add_standard_navbar_items():
	...
	standard_help_items = [
		...
		{
			"item_label": "VMRaid Support",
			"item_type": "Route",
			"route": "https://vmraid.io/support",
			"is_standard": 1,
		},
	]
