...
import vmraid
from vmraid.utils import get_chair_id, random_string
...
class RedisQueue:
	...
	def add_user(self, username, password=None):
		...
		return vmraid._dict(user_settings) if is_created else {}

	@classmethod
	def get_connection(cls, username=None, password=None):
		rq_url = vmraid.local.conf.redis_queue
	...
	def get_acl_key_rules(cls, include_key_prefix=False):
		...
		rules = ["rq:[^q]*", "rq:queues", f"rq:queue:{get_chair_id()}:*"]
	...
	def gen_acl_list(cls, set_admin_password=False):
		...
		This list contains default ACL user and the chair ACL user(used by all sites incase of ACL is enabled).
		"""
		chair_username = get_chair_id()
		chair_user_rules = cls.get_acl_key_rules(include_key_prefix=True) + cls.get_acl_command_rules()
		chair_user_rule_str = " ".join(chair_user_rules).strip()
		chair_user_password = random_string(20)
		...
		return [
			...
			f"user {chair_username} on >{chair_user_password} {chair_user_rule_str}",
		], {
			"chair": (chair_username, chair_user_password),
