...
class UserPermissions:
	...
	def setup_user(self):
		def get_user_doc():
			...
			try:
				...
				# install boo-boo
				if not vmraid.db.is_table_missing(e): raise
	...
	def build_doctype_map(self):
		...
		self.doctype_map = {}
		for r in vmraid.db.sql("""select name, in_create, issingle, istable,
	...
	def build_permissions(self):
		...
		self.shared = vmraid.db.sql_list("""select distinct share_doctype from `tabDocShare`
		...
		if "System Manager" in self.get_roles():
			docs = vmraid.get_all("DocType", {'allow_import': 1})
			self.can_import += [doc.name for doc in docs]

			customizations = vmraid.get_all("Property Setter", fields=['doc_type'], filters={'property': 'allow_import', 'value': "1"})
	...
	def load_user(self):
		d = vmraid.db.sql("""select email, first_name, last_name, creation,
...
def get_user_fullname(user):
	fullname = vmraid.db.sql("SELECT CONCAT_WS(' ', first_name, last_name) FROM `tabUser` WHERE name=%s", (user,))
	return fullname and fullname[0][0] or ''

def get_fullname_and_avatar(user):
	first_name, last_name, avatar, name = vmraid.db.get_value("User",
...
def get_system_managers(only_name=False):
	...
	system_managers = vmraid.db.sql("""SELECT DISTINCT `name`, `creation`,
		CONCAT_WS(' ',
...
def is_website_user():
	return vmraid.db.get_value('User', vmraid.session.user, 'user_type') == "Website User"

def is_system_user(username):
	return vmraid.db.get_value("User", {"email": username, "enabled": 1, "user_type": "System User"})

def get_users():
	from vmraid.core.doctype.user.user import get_system_users
	users = []
	system_managers = vmraid.utils.user.get_system_managers(only_name=True)
	for user in get_system_users():
		users.append({
			"full_name": vmraid.utils.user.get_user_fullname(user),
			"email": user,
...
def reset_simultaneous_sessions(user_limit):
	for user in vmraid.db.sql("""select name, simultaneous_sessions from tabUser
		...
		if user.simultaneous_sessions < user_limit:
			...
			vmraid.db.set_value("User", user.name, "simultaneous_sessions", 1)
...
def get_users_with_role(role):
	return [p[0] for p in vmraid.db.sql("""SELECT DISTINCT `tabUser`.`name`
