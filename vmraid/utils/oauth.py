...
class SignupDisabledError(vmraid.PermissionError):
	pass
...
def get_oauth2_authorize_url(provider, redirect_to):
	...
	state = {
		"site": vmraid.utils.get_url(),
		"token": vmraid.generate_hash(),
...
def login_oauth_user(
	...
	try:
		...
		return vmraid.respond_as_web_page(
			"Signup is Disabled",
	...
	if vmraid.utils.cint(generate_login_token):
		...
		vmraid.cache().set_value(
			"login_token:{0}".format(login_token), vmraid.local.session.sid, expires_in_sec=120
		)
