...
class SignupDisabledError(vmraid.PermissionError): pass

def get_oauth2_providers():
	out = {}
	providers = vmraid.get_all("Social Login Key", fields=["*"])
...
def get_oauth_keys(provider):
	...
	# try conf
	keys = vmraid.conf.get("{provider}_login".format(provider=provider))

	if not keys:
		# try database
		client_id, client_secret = vmraid.get_value("Social Login Key", provider, ["client_id", "client_secret"])
...
def get_oauth2_authorize_url(provider, redirect_to):
	...
	state = { "site": vmraid.utils.get_url(), "token": vmraid.generate_hash(), "redirect_to": redirect_to 	}
...
def get_redirect_uri(provider):
	...
	if keys and keys.get("redirect_uri"):
		...
		return vmraid.utils.get_url(redirect_uri)
...
def get_info_via_oauth(provider, code, decoder=None, id_token=False):
	...
	if not (info.get("email_verified") or info.get("email")):
		vmraid.throw(_("Email not verified with {0}").format(provider.title()))
...
def login_oauth_user(data=None, provider=None, state=None, email_id=None, key=None, generate_login_token=False):
	...
	# if email_id and key:
	# 	data = json.loads(vmraid.db.get_temp(key))
	#	# What if data is missing because of an invalid key
	# 	data["email"] = email_id
	#
	# elif not (data.get("email") and get_first_name(data)) and not vmraid.db.exists("User", data.get("email")):
	# 	# ask for user email
	# 	key = vmraid.db.set_temp(json.dumps(data))
	# 	vmraid.db.commit()
	# 	vmraid.local.response["type"] = "redirect"
	# 	vmraid.local.response["location"] = "/complete_signup?key=" + key
	# 	return
	...
	if not (state and state["token"]):
		vmraid.respond_as_web_page(_("Invalid Request"), _("Token is missing"), http_status_code=417)
		return

	user = get_email(data)

	if not user:
		vmraid.respond_as_web_page(_("Invalid Request"), _("Please ensure that your profile has an email address"))
		return
	...
	try:
		...
		return vmraid.respond_as_web_page("Signup is Disabled", "Sorry. Signup from Website is disabled.",
			success=False, http_status_code=403)

	vmraid.local.login_manager.user = user
	vmraid.local.login_manager.post_login()

	# because of a GET request!
	vmraid.db.commit()

	if vmraid.utils.cint(generate_login_token):
		login_token = vmraid.generate_hash(length=32)
		vmraid.cache().set_value("login_token:{0}".format(login_token), vmraid.local.session.sid, expires_in_sec=120)

		vmraid.response["login_token"] = login_token


	else:
		redirect_to = state.get("redirect_to")
		redirect_post_login(
			desk_user=vmraid.local.response.get('message') == 'Logged In',
...
def update_oauth_user(user, data, provider):
	...
	if not vmraid.db.exists("User", user):
		...
		# is signup disabled?
		if vmraid.utils.cint(vmraid.db.get_single_value("Website Settings", "disable_signup")):
		...
		if gender and not vmraid.db.exists("Gender", gender):
			doc = vmraid.new_doc("Gender", {"gender": gender})
		...
		user.update({
			...
			"gender": gender,
			"enabled": 1,
			"new_password": vmraid.generate_hash(get_email(data)),
		...
		if not user.enabled:
			vmraid.respond_as_web_page(_('Not Allowed'), _('User {0} is disabled').format(user.email))
			return False
	...
	if provider=="facebook" and not user.get_social_login_userid(provider):
	...
	elif provider=="vmraid" and not user.get_social_login_userid(provider):
		save = True
		...
		save = True
		user_id_property = vmraid.db.get_value("Social Login Key", provider, "user_id_property") or "sub"
	...
	if save:
		...
		default_role = vmraid.db.get_single_value("Portal Settings", "default_role")
		if default_role:
...
def redirect_post_login(desk_user, redirect_to=None, provider=None):
	...
	if not redirect_to:
		...
		redirect_to = vmraid.utils.get_url(redirect_to)

	vmraid.local.response["location"] = redirect_to
