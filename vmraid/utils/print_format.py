...
from vmraid.utils.pdf import get_pdf,cleanup
...
def download_multi_pdf(doctype, name, format=None, no_letterhead=0):
	...
	if not isinstance(doctype, dict):
		...
		for i, ss in enumerate(result):
			output = vmraid.get_print(doctype, ss, format, as_pdf = True, output = output, no_letterhead=no_letterhead)
		vmraid.local.response.filename = "{doctype}.pdf".format(doctype=doctype.replace(" ", "-").replace("/", "-"))
	else:
		for doctype_name in doctype:
			for doc_name in doctype[doctype_name]:
				try:
					output = vmraid.get_print(doctype_name, doc_name, format, as_pdf = True, output = output, no_letterhead=no_letterhead)
				except Exception:
					vmraid.log_error("Permission Error on doc {} of doctype {}".format(doc_name, doctype_name))
		vmraid.local.response.filename = "{}.pdf".format(name)
...
def read_multi_pdf(output):
	...
	fname = os.path.join("/tmp", "vmraid-pdf-{0}.pdf".format(vmraid.generate_hash()))
...
def download_pdf(doctype, name, format=None, doc=None, no_letterhead=0):
	html = vmraid.get_print(doctype, name, format, doc=doc, no_letterhead=no_letterhead)
	vmraid.local.response.filename = "{name}.pdf".format(name=name.replace(" ", "-").replace("/", "-"))
	vmraid.local.response.filecontent = get_pdf(html)
	vmraid.local.response.type = "pdf"

@vmraid.whitelist()
def report_to_pdf(html, orientation="Landscape"):
	make_access_log(file_type='PDF', method='PDF', page=html)
	vmraid.local.response.filename = "report.pdf"
	vmraid.local.response.filecontent = get_pdf(html, {"orientation": orientation})
	vmraid.local.response.type = "pdf"
...
def print_by_server(doctype, name, print_format=None, doc=None, no_letterhead=0):
	...
	try:
		...
		output = PdfFileWriter()
		output = vmraid.get_print(doctype, name, print_format, doc=doc, no_letterhead=no_letterhead, as_pdf = True, output = output)
		file = os.path.join("/", "tmp", "vmraid-pdf-{0}.pdf".format(vmraid.generate_hash()))
		...
		if ("ContentNotFoundError" in e.message
			...
			vmraid.throw(_("PDF generation failed"))
	except cups.IPPError:
		vmraid.throw(_("Printing failed"))
	finally:
		cleanup(file,{})
