...
from vmraid.utils.pdf import cleanup, get_pdf
...
def download_multi_pdf(doctype, name, format=None, no_letterhead=False, options=None):
	...
	if not isinstance(doctype, dict):
		...
		vmraid.local.response.filename = "{doctype}.pdf".format(
...
def print_by_server(
	...
	print_settings = vmraid.get_doc("Network Printer Settings", printer_setting)
	try:
		import cups
	except ImportError:
	...
	try:
		...
		if not file_path:
			file_path = os.path.join("/", "tmp", "vmraid-pdf-{0}.pdf".format(vmraid.generate_hash()))
