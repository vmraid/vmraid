...
import vmraid
from vmraid import _, safe_decode
...
def get_change_log(user=None):
	...
	last_known_versions = vmraid._dict(json.loads(vmraid.db.get_value("User",
	...
	if "vmraid" in current_versions:
		set_in_change_log("vmraid", current_versions["vmraid"], change_log)

	return change_log

def get_change_log_for_app(app, from_version, to_version):
	change_log_folder = os.path.join(vmraid.get_app_path(app), "change_log")
		...
		if folder in major_version_folders:
			for file in os.listdir(os.path.join(change_log_folder, folder)):
				...
				if from_version < version <= to_version:
					...
					content = vmraid.read_file(file_path)
...
def update_last_known_versions():
	vmraid.db.set_value("User", vmraid.session.user, "last_known_versions",
...
def get_versions():
	...
	Example:
		...
		{
			"vmraid": {
				"title": "VMRaid Framework",
				"version": "5.0.0"
			}
		}"""
	versions = {}
	for app in vmraid.get_installed_apps(sort=True):
		app_hooks = vmraid.get_hooks(app_name=app)
		versions[app] = {
		...
		try:
			versions[app]["version"] = vmraid.get_attr(app + ".__version__")
...
def check_for_update():
	updates = vmraid._dict(major=[], minor=[], patch=[])
	apps = get_versions()
	...
	for app in apps:
		...
		for update_type in updates:
			if github_version.__dict__[update_type] > instance_version.__dict__[update_type]:
				updates[update_type].append(vmraid._dict(
...
def check_release_on_github(app: str):
	"""
	Check the latest release for a given VMRaid application hosted on Github.

	Args:
		app (str): The name of the VMRaid application.
...
def add_message_to_redis(update_json):
	...
	user_list = [x.name for x in vmraid.get_all("User", filters={"enabled": True})]
	system_managers = [user for user in user_list if 'System Manager' in vmraid.get_roles(user)]
...
def show_update_popup():
	...
	if update_message:
		vmraid.msgprint(update_message, title=_("New updates are available"), indicator='green')
