...
def report_error(status_code):
	...
	allow_traceback = cint(vmraid.db.get_system_setting('allow_error_traceback')) if vmraid.db else True
	if (allow_traceback and (status_code!=404 or vmraid.conf.logging)
...
def as_csv():
	...
	response.headers["Content-Disposition"] = ("attachment; filename=\"%s.csv\"" % vmraid.response['doctype'].replace(' ', '_')).encode("utf-8")
...
def as_txt():
	...
	response.charset = 'utf-8'
	response.headers["Content-Disposition"] = ("attachment; filename=\"%s.txt\"" % vmraid.response['doctype'].replace(' ', '_')).encode("utf-8")
	response.data = vmraid.response['result']
	return response

def as_raw():
	response = Response()
	response.mimetype = vmraid.response.get("content_type") or mimetypes.guess_type(vmraid.response['filename'])[0] or "application/unknown"
	response.headers["Content-Disposition"] = (f'{vmraid.response.get("display_content_as","attachment")}; filename="{vmraid.response["filename"].replace(" ", "_")}"').encode("utf-8")
...
def as_pdf():
	...
	response.headers["Content-Disposition"] = ("filename=\"%s\"" % vmraid.response['filename'].replace(' ', '_') + ";filename*=utf-8''%s" % encoded_filename).encode("utf-8")
	response.data = vmraid.response['filecontent']
	return response
...
def as_binary():
	...
	response.headers["Content-Disposition"] = ("filename=\"%s\"" % vmraid.response['filename'].replace(' ', '_')).encode("utf-8")
	response.data = vmraid.response['filecontent']
	return response
...
def make_logs(response = None):
	...
	if vmraid.local.message_log:
		response['_server_messages'] = json.dumps([vmraid.utils.cstr(d) for
			d in vmraid.local.message_log])
...
def as_page():
	...
	return render(vmraid.response['route'], http_status_code=vmraid.response.get("http_status_code"))
...
def handle_session_stopped():
	vmraid.respond_as_web_page(_("Updating"),
	...
	return vmraid.website.render.render("message", http_status_code=503)
