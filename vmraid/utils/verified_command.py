...
def get_secret():
	return vmraid.local.conf.get("secret") or str(
		vmraid.db.get_value("User", "Administrator", "creation")
	)


def verify_request():
	"""Verify if the incoming signed request if it is correct."""
	query_string = vmraid.safe_decode(
		vmraid.local.flags.signed_query_string or getattr(vmraid.request, "query_string", None)
	)
