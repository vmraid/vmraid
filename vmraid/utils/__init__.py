...
from vmraid.utils.data import *
from vmraid.utils.html_utils import sanitize_html
...
def get_fullname(user=None):
	...
	if not hasattr(vmraid.local, "fullnames"):
		vmraid.local.fullnames = {}

	if not vmraid.local.fullnames.get(user):
		p = vmraid.db.get_value("User", user, ["first_name", "last_name"], as_dict=True)
		if p:
			vmraid.local.fullnames[user] = (
				" ".join(filter(None, [p.get("first_name"), p.get("last_name")])) or user
			)
		else:
			vmraid.local.fullnames[user] = user

	return vmraid.local.fullnames.get(user)
...
def get_email_address(user=None):
	...
	return vmraid.db.get_value("User", user, "email")
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		vmraid.throw(
			vmraid._("{0} is not a valid Phone Number").format(phone_number), vmraid.InvalidPhoneNumberError
		)
...
def validate_name(name, throw=False):
	...
	if not match and throw:
		vmraid.throw(vmraid._("{0} is not a valid Name").format(name), vmraid.InvalidNameError)
...
def validate_email_address(email_str, throw=False):
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				invalid_email = vmraid.utils.escape_html(e)
				vmraid.throw(
					vmraid._("{0} is not a valid Email Address").format(invalid_email),
					vmraid.InvalidEmailAddressError,
				)
			return None
		else:
			return matched
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		vmraid.throw(vmraid._("'{0}' is not a valid URL").format(vmraid.bold(txt)))
...
def has_gravatar(email):
	...
	if vmraid.flags.in_import or vmraid.flags.in_install or vmraid.flags.in_test:
		# no gravatar if via upload
		# since querying gravatar for every item will be slow
		return ""

	hexdigest = hashlib.md5(vmraid.as_unicode(email).encode("utf-8")).hexdigest()
...
def get_gravatar(email):
	from vmraid.utils.identicon import Identicon
...
def get_traceback() -> str:
	...
	return "".join(cstr(t) for t in trace_list).replace(chair_path, "")


def log(event, details):
	vmraid.logger().info(details)
...
def get_defaults(key=None):
	...
	"""
	return vmraid.db.get_defaults(key)
...
def set_default(key, val):
	...
	"""
	return vmraid.db.set_default(key, val)
...
def get_path(*path, **kwargs):
	...
	if not base:
		base = vmraid.local.site_path
	return os.path.join(base, *path)


def get_site_base_path():
	return vmraid.local.site_path
...
def get_chair_path():
	return os.path.realpath(os.path.join(os.path.dirname(vmraid.__file__), "..", "..", ".."))


def get_chair_id():
	return vmraid.get_conf().get("chair_id", get_chair_path().strip("/").replace("/", "-"))


def get_site_id(site=None):
	return f"{site or vmraid.local.site}@{get_chair_id()}"
...
def get_site_url(site):
	return "http://{site}:{port}".format(site=site, port=vmraid.get_conf(site).webserver_port)
...
def get_test_client() -> Client:
	"""Returns an test instance of the VMRaid WSGI"""
	from vmraid.app import application

	return Client(application)


def get_hook_method(hook_name, fallback=None):
	method = vmraid.get_hooks().get(hook_name)
	if method:
		method = vmraid.get_attr(method[0])
		return method
	if fallback:
		return fallback
...
def call_hook_method(hook, *args, **kwargs):
	out = None
	for method_name in vmraid.get_hooks(hook):
		out = out or vmraid.get_attr(method_name)(*args, **kwargs)
...
def update_progress_bar(txt, i, l):
	...
	if not getattr(vmraid.local, "request", None) or is_cli():
		lt = len(txt)
		try:
...
def get_html_format(print_path):
	...
	if os.path.exists(print_path):
		...
		for include_directive, path in re.findall("""({% include ['"]([^'"]*)['"] %})""", html_format):
			for app_name in vmraid.get_installed_apps():
				include_path = vmraid.get_app_path(app_name, *path.split(os.path.sep))
...
def get_sites(sites_path=None):
	if not sites_path:
		sites_path = getattr(vmraid.local, "sites_path", None) or "."
...
def markdown(text, sanitize=True, linkify=True):
	html = text if is_html(text) else vmraid.utils.md_to_html(text)
...
def get_site_info():
	from vmraid.email.queue import get_emails_sent_this_month
	from vmraid.utils.user import get_system_managers

	# only get system users
	users = vmraid.get_all(
		"User",
		filters={"user_type": "System User", "name": ("not in", vmraid.STANDARD_USERS)},
	...
	system_settings = vmraid.db.get_singles_dict("System Settings")
	space_usage = vmraid._dict((vmraid.local.conf.limits or {}).get("space_usage", {}))
	...
	site_info = {
		...
		"last_logins": vmraid.get_all("Activity Log", **kwargs),
	}

	# from other apps
	for method_name in vmraid.get_hooks("get_site_info"):
		site_info.update(vmraid.get_attr(method_name)(site_info) or {})

	# dumps -> loads to prevent datatype conflicts
	return json.loads(vmraid.as_json(site_info))
...
def get_db_count(*args):
	...
	Example:
	        via terminal:
	                chair --site erpadda.local execute vmraid.utils.get_db_count --args "['DocType', 'Communication']"
	"""
	db_count = {}
	for doctype in args:
		db_count[doctype] = vmraid.db.count(doctype)

	return json.loads(vmraid.as_json(db_count))
...
def call(fn, *args, **kwargs):
	...
	Pass a doctype or a series of doctypes to get the count of docs in them
	Parameters:
	        fn: vmraid function to be called
	...
	Example:
	        via terminal:
	                chair --site erpadda.local execute vmraid.utils.call --args '''["vmraid.get_all", "Activity Log"]''' --kwargs '''{"fields": ["user", "creation", "full_name"], "filters":{"Operation": "Login", "Status": "Success"}, "limit": "10"}'''
	"""
	return json.loads(vmraid.as_json(vmraid.call(fn, *args, **kwargs)))
...
def get_safe_filters(filters):
	try:
		...
		if isinstance(filters, (int, float)):
			filters = vmraid.as_unicode(filters)
...
def set_request(**kwargs):
	...
	vmraid.local.request = Request(builder.get_environ())
...
def get_build_version():
	try:
		return str(os.path.getmtime(os.path.join(vmraid.local.sites_path, ".build")))
	except OSError:
		# .build can sometimes not exist
		# this is not a major problem so send fallback
		return vmraid.utils.random_string(8)


def get_assets_json():
	def _get_assets():
		# get merged assets.json and assets-rtl.json
		assets = vmraid.parse_json(vmraid.read_file("assets/assets.json"))

		if assets_rtl := vmraid.read_file("assets/assets-rtl.json"):
			assets.update(vmraid.parse_json(assets_rtl))

		return assets

	if not hasattr(vmraid.local, "assets_json"):
		if not vmraid.conf.developer_mode:
			vmraid.local.assets_json = vmraid.cache().get_value(
				"assets_json",
				_get_assets,
				shared=True,
			)
			...
			vmraid.local.assets_json = _get_assets()

	return vmraid.local.assets_json


def get_chair_relative_path(file_path):
	"""Fixes paths relative to the chair root directory if exists and returns the absolute path
