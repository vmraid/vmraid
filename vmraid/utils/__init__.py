...
def get_fullname(user=None):
	...
	if not vmraid.local.fullnames.get(user):
		...
		if p:
			vmraid.local.fullnames[user] = " ".join(filter(None,
...
def validate_phone_number(phone_number, throw=False):
	...
	if not match and throw:
		vmraid.throw(vmraid._("{0} is not a valid Phone Number").format(phone_number), vmraid.InvalidPhoneNumberError)
	...
	def _check(e):
		...
		if not _valid:
			if throw:
				...
				vmraid.throw(vmraid._("{0} is not a valid Email Address").format(invalid_email),
...
def validate_url(txt, throw=False, valid_schemes=None):
	...
	if not is_valid and throw:
		vmraid.throw(
			vmraid._("'{0}' is not a valid URL").format(vmraid.bold(txt))
		)
...
def get_site_url(site):
	return 'http://{site}:{port}'.format(
		site=site,
		port=vmraid.get_conf(site).webserver_port
	)
...
def get_site_info():
	...
	users = vmraid.get_all('User', filters={'user_type': 'System User', 'name': ('not in', STANDARD_USERS)},
...
def get_assets_json():
	if not hasattr(vmraid.local, "assets_json"):
		...
		vmraid.local.assets_json = vmraid.safe_decode(assets_json)

	return vmraid.parse_json(vmraid.local.assets_json)
