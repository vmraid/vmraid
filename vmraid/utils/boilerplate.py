...
import vmraid, os, re, git
from vmraid.utils import touch_file, cstr
...
def make_boilerplate(dest, app_name):
	...
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, vmraid.scrub(hooks.app_title)),
		with_init=True)
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates"), with_init=True)
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "www"))
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates",
		"pages"), with_init=True)
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "templates",
		"includes"))
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "config"), with_init=True)
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public",
		"css"))
	vmraid.create_folder(os.path.join(dest, hooks.app_name, hooks.app_name, "public",
		"js"))
