...
from vmraid.model.document import Document, get_controller
from vmraid.utils import cint, quoted
from vmraid.website.path_resolver import resolve_path
...
def get(doctype, txt=None, limit_start=0, limit=20, pathname=None, **kwargs):
	...
	for doc in raw_result:
		doc.doctype = doctype
		new_context = vmraid._dict(doc=doc, meta=meta, list_view_fields=list_view_fields)
...
def get_list_data(
	...
	if vmraid.is_table(doctype):
		vmraid.throw(_("Child DocTypes are not allowed"), title=_("Invalid DocType"))
...
def get_list_context(context, doctype, web_form_name=None):
	...
	if meta.custom and web_form_name:
		webform_list_contexts = vmraid.get_hooks("webform_list_context")
		if webform_list_contexts:
			out = vmraid._dict(vmraid.get_attr(webform_list_contexts[0])(meta.module) or {})
			if out:
				list_context = out
