...
import vmraid
from vmraid import _, get_module_path
...
from vmraid.utils.jinja_globals import is_rtl
...
def get_context(context):
	...
	make_access_log(
		doctype=vmraid.form_dict.doctype, document=vmraid.form_dict.name, file_type="PDF", method="Print"
	)
	...
	return {
		"body": get_rendered_template(
			...
			meta=meta,
			trigger_print=vmraid.form_dict.trigger_print,
			no_letterhead=vmraid.form_dict.no_letterhead,
...
def get_print_format_doc(print_format_name, meta):
	...
	if not print_format_name:
		print_format_name = vmraid.form_dict.format or meta.default_print_format or "Standard"
...
def get_rendered_template(
	...
	if letter_head.content:
		letter_head.content = vmraid.utils.jinja.render_template(
			letter_head.content, {"doc": doc.as_dict()}
		)

	if letter_head.footer:
		letter_head.footer = vmraid.utils.jinja.render_template(
...
def set_title_values_for_link_and_dynamic_link_fields(meta, doc, parent_doc=None):
	...
	for field in meta.get_link_fields() + meta.get_dynamic_link_fields():
		...
		link_title = vmraid.get_cached_value(doctype, doc.get(field.fieldname), meta.title_field)
		if parent_doc:
...
def set_title_values_for_table_and_multiselect_fields(meta, doc):
	for field in meta.get_table_fields():
		...
		_meta = vmraid.get_meta(field.options)
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		vmraid.throw(
			_("{0} is not a raw printing format.").format(print_format), vmraid.TemplateNotFoundError
		)
...
def validate_print_permission(doc):
	...
	for ptype in ("read", "print"):
		if not vmraid.has_permission(doc.doctype, ptype, doc) and not vmraid.has_website_permission(doc):
...
def get_letter_head(doc, no_letterhead, letterhead=None):
	...
	if doc.get("letter_head"):
		...
		return (
			vmraid.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}
		)
...
def get_print_format(doctype, print_format):
	if print_format.disabled:
		vmraid.throw(
			_("Print Format {0} is disabled").format(print_format.name), vmraid.DoesNotExistError
		)

	# server, find template
	module = print_format.module or vmraid.db.get_value("DocType", doctype, "module")
	path = os.path.join(
		get_module_path(module, "Print Format", print_format.name),
		vmraid.scrub(print_format.name) + ".html",
	)
	...
	if os.path.exists(path):
		...
		vmraid.throw(_("No template found at path: {0}").format(path), vmraid.TemplateNotFoundError)
