...
import vmraid, os, copy, json, re
from vmraid import _

from vmraid.modules import get_doc_path
from vmraid.core.doctype.access_log.access_log import make_access_log
from vmraid.utils import cint, sanitize_html, strip_html
...
def get_context(context):
	...
	if not ((vmraid.form_dict.doctype and vmraid.form_dict.name) or vmraid.form_dict.doc):
		return {
			"body": sanitize_html("""<h1>Error</h1>
				<p>Parameters doctype and name required</p>
				<pre>%s</pre>""" % repr(vmraid.form_dict))
		}

	if vmraid.form_dict.doc:
		doc = vmraid.form_dict.doc
	else:
		doc = vmraid.get_doc(vmraid.form_dict.doctype, vmraid.form_dict.name)

	settings = vmraid.parse_json(vmraid.form_dict.settings)

	letterhead = vmraid.form_dict.letterhead or None

	meta = vmraid.get_meta(doc.doctype)

	print_format = get_print_format_doc(None, meta = meta)

	make_access_log(doctype=vmraid.form_dict.doctype, document=vmraid.form_dict.name, file_type='PDF', method='Print')

	return {
		"body": get_rendered_template(doc, print_format = print_format,
			meta=meta, trigger_print = vmraid.form_dict.trigger_print,
			no_letterhead=vmraid.form_dict.no_letterhead, letterhead=letterhead,
			settings=settings),
		"css": get_print_style(vmraid.form_dict.style, print_format),
		"comment": vmraid.session.user,
		"title": doc.get(meta.title_field) if meta.title_field else doc.name,
		"has_rtl": True if vmraid.local.lang in ["ar", "he", "fa", "ps"] else False
	}

def get_print_format_doc(print_format_name, meta):
	"""Returns print format document"""
	if not print_format_name:
		print_format_name = vmraid.form_dict.format \
	...
	if print_format_name == "Standard":
		...
		try:
			return vmraid.get_doc("Print Format", print_format_name)
...
def get_rendered_template(doc, name=None, print_format=None, meta=None,
	...
	print_settings = vmraid.get_single("Print Settings").as_dict()
	...
	if not vmraid.flags.ignore_print_permissions:
		validate_print_permission(doc)

	if doc.meta.is_submittable:
		if doc.docstatus==0 and not cint(print_settings.allow_print_for_draft):
			vmraid.throw(_("Not allowed to print draft documents"), vmraid.PermissionError)

		if doc.docstatus==2 and not cint(print_settings.allow_print_for_cancelled):
			vmraid.throw(_("Not allowed to print cancelled documents"), vmraid.PermissionError)
	...
	letter_head = vmraid._dict(get_letter_head(doc, no_letterhead, letterhead) or {})

	if letter_head.content:
		letter_head.content = vmraid.utils.jinja.render_template(letter_head.content, {"doc": doc.as_dict()})

	if letter_head.footer:
		letter_head.footer = vmraid.utils.jinja.render_template(letter_head.footer, {"doc": doc.as_dict()})
	...
	for field in meta.fields:
		if field.fieldtype=='Text Editor':
			...
			if value and '<!-- markdown -->' in value:
				doc.set(field.fieldname, vmraid.utils.md_to_html(value))
...
def get_html_and_style(doc, name=None, print_format=None, meta=None,
	...
	if isinstance(doc, string_types) and isinstance(name, string_types):
		doc = vmraid.get_doc(doc, name)

	if isinstance(doc, string_types):
		doc = vmraid.get_doc(json.loads(doc))

	print_format = get_print_format_doc(print_format, meta=meta or vmraid.get_meta(doc.doctype))
	...
	try:
		html = get_rendered_template(doc, name=name, print_format=print_format, meta=meta,
			...
			settings=vmraid.parse_json(settings))
	except vmraid.TemplateNotFoundError:
...
def get_rendered_raw_commands(doc, name=None, print_format=None, meta=None, lang=None):
	...
	if not print_format or (print_format and not print_format.raw_printing):
		vmraid.throw(_("{0} is not a raw printing format.").format(print_format),
				vmraid.TemplateNotFoundError)
...
def validate_print_permission(doc):
	if vmraid.form_dict.get("key"):
		if vmraid.form_dict.key == doc.get_signature():
			return

	for ptype in ("read", "print"):
		if (not vmraid.has_permission(doc.doctype, ptype, doc)
			and not vmraid.has_website_permission(doc)):
			raise vmraid.PermissionError(_("No {0} permission").format(ptype))

def get_letter_head(doc, no_letterhead, letterhead=None):
	if no_letterhead:
		return {}
	if letterhead:
		return vmraid.db.get_value("Letter Head", letterhead, ["content", "footer"], as_dict=True)
	if doc.get("letter_head"):
		return vmraid.db.get_value("Letter Head", doc.letter_head, ["content", "footer"], as_dict=True)
	else:
		return vmraid.db.get_value("Letter Head", {"is_default": 1}, ["content", "footer"], as_dict=True) or {}

def get_print_format(doctype, print_format):
	if print_format.disabled:
		vmraid.throw(_("Print Format {0} is disabled").format(print_format.name),
			vmraid.DoesNotExistError)

	# server, find template
	path = os.path.join(get_doc_path(vmraid.db.get_value("DocType", doctype, "module"),
		"Print Format", print_format.name), vmraid.scrub(print_format.name) + ".html")
	...
	if os.path.exists(path):
		...
		vmraid.throw(_("No template found at path: {0}").format(path),
			vmraid.TemplateNotFoundError)
...
def make_layout(doc, meta, format_data=None):
	...
	for df in format_data or meta.fields:
		...
		if df.fieldtype=='Signature' and not doc.get(df.fieldname):
			placeholder_image = '/assets/vmraid/images/signature-placeholder.png'
...
def get_print_style(style=None, print_format=None, for_legacy=False):
	...
	css = vmraid.get_template("templates/styles/standard.css").render(context)

	if style and vmraid.db.exists('Print Style', style):
		css = css + '\n' + vmraid.db.get_value('Print Style', style, 'css')
...
def get_visible_columns(data, table_meta, df):
	...
	columns = []
	doc = data[0] or vmraid.new_doc(df.options)
