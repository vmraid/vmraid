...
import vmraid
import vmraid.utils
from vmraid.utils.oauth import get_oauth2_authorize_url, get_oauth_keys, login_via_oauth2, login_via_oauth2_id_token, login_oauth_user as _login_oauth_user, redirect_post_login
import json
...
def get_context(context):
	...
	context["disable_signup"] = vmraid.utils.cint(vmraid.db.get_value("Website Settings", "Website Settings", "disable_signup"))
	context["logo"] = vmraid.get_hooks("app_logo_url")[-1]
	context["app_name"] = vmraid.get_system_settings("app_name") or _("VMRaid")
	providers = [i.name for i in vmraid.get_all("Social Login Key", filters={"enable_social_login":1}, order_by="name")]
...
def login_via_token(login_token):
	...
	redirect_post_login(desk_user = vmraid.db.get_value("User", vmraid.session.user, "user_type")=="System User")
