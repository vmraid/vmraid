...
def add(
	...
	if vmraid.get_cached_value("User", user, "follow_shared_documents"):
...
def remove(doctype, name, user, flags=None):
	share_name = vmraid.db.get_value(
...
def get_shared_doctypes(user=None):
	...
	if not user:
		user = vmraid.session.user
	table = vmraid.qb.DocType("DocShare")
	query = (
...
def check_share_permission(doctype, name):
	...
	if not vmraid.has_permission(doctype, ptype="share", doc=name):
		vmraid.throw(
			_("No permission to {0} {1} {2}").format("share", doctype, name), vmraid.PermissionError
		)
