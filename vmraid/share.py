...
def add(doctype, name, user=None, read=1, write=0, submit=0, share=0, everyone=0, flags=None, notify=0):
	...
	if share_name:
		doc = vmraid.get_doc("DocShare", share_name)
	else:
		doc = vmraid.new_doc("DocShare")
		doc.update({
			"user": user,
...
def remove(doctype, name, user, flags=None):
	share_name = vmraid.db.get_value("DocShare", {"user": user, "share_name": name,
		"share_doctype": doctype})

	if share_name:
		vmraid.delete_doc("DocShare", share_name, flags=flags)
...
def set_permission(doctype, name, user, permission_to, value=1, everyone=0):
	...
	if not share_name:
		...
		share = vmraid.get_doc("DocShare", share_name)
...
def get_users(doctype, name):
	...
	return vmraid.db.get_all("DocShare",
...
def get_shared(doctype, user=None, rights=None):
	...
	shared_docs = vmraid.db.get_all('DocShare',
		fields=['share_name'],
		filters=filters,
...
def get_shared_doctypes(user=None):
	...
	return vmraid.db.sql_list("select distinct share_doctype from tabDocShare where (user=%s or everyone=1)", user)

def get_share_name(doctype, name, user, everyone):
	if cint(everyone):
		share_name = vmraid.db.get_value("DocShare", {"everyone": 1, "share_name": name,
			"share_doctype": doctype})
	else:
		share_name = vmraid.db.get_value("DocShare", {"user": user, "share_name": name,
			"share_doctype": doctype})
...
def check_share_permission(doctype, name):
	...
	if not vmraid.has_permission(doctype, ptype="share", doc=name):
		vmraid.throw(_("No permission to {0} {1} {2}").format("share", doctype, name), vmraid.PermissionError)
...
def notify_assignment(shared_by, doctype, doc_name, everyone, notify=0):
	...
	reference_user = get_fullname(vmraid.session.user)
	notification_message = _('{0} shared a document {1} {2} with you').format(
		vmraid.bold(reference_user), vmraid.bold(doctype), get_title_html(title))
