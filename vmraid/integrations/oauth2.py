...
def approve(*args, **kwargs):
	...
	try:
		(scopes, vmraid.flags.oauth_credentials,) = get_oauth_server().validate_authorization_request(
...
def authorize(**kwargs):
	...
	if vmraid.session.user == "Guest":
		...
		try:
			...
			unrevoked_tokens = vmraid.get_all("OAuth Bearer Token", filters={"status": "Active"})
			...
			if skip_auth or (get_oauth_settings().skip_authorization == "Auto" and unrevoked_tokens):
				...
				response_html_params = vmraid._dict(
					{
						"client_id": vmraid.db.get_value("OAuth Client", kwargs["client_id"], "app_name"),
...
def introspect_token(token=None, token_type_hint=None):
	...
	try:
		...
		if token_type_hint == "access_token":
			...
			bearer_token = vmraid.get_doc("OAuth Bearer Token", {"refresh_token": token})
