...
from vmraid.utils import get_url, call_hook_method, cint, flt, cstr
...
class PaytmSettings(Document):
	...
	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			vmraid.throw(_("Please select another payment method. Paytm does not support transactions in currency '{0}'").format(currency))
...
def get_paytm_config():
	...
	paytm_config = vmraid.db.get_singles_dict('Paytm Settings')
...
def get_paytm_params(payment_details, order_id, paytm_config):
	...
	redirect_uri = get_request_site_address(True) + "/api/method/vmraid.integrations.doctype.paytm_settings.paytm_settings.verify_transaction"
...
def verify_transaction(**paytm_params):
	...
	if is_valid_checksum and paytm_params.get('RESPCODE') == '01':
		...
		vmraid.respond_as_web_page("Payment Failed",
		...
		vmraid.log_error("Order unsuccessful. Failed Response:"+cstr(paytm_params), 'Paytm Payment Failed')
...
def finalize_request(order_id, transaction_response):
	request = vmraid.get_doc('Integration Request', order_id)
	transaction_data = vmraid._dict(json.loads(request.data))
	...
	if transaction_response['STATUS'] == "TXN_SUCCESS":
		if transaction_data.reference_doctype and transaction_data.reference_docname:
			...
			try:
				custom_redirect_to = vmraid.get_doc(transaction_data.reference_doctype,
	...
	vmraid.local.response['location'] = redirect_url
