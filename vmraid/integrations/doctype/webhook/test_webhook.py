...
import vmraid
from vmraid.integrations.doctype.webhook.webhook import (
	enqueue_webhook,
	get_webhook_data,
	get_webhook_headers,
)
...
class TestWebhook(unittest.TestCase):
	...
	def setUpClass(cls):
		...
		vmraid.db.delete("Webhook")
		# Delete existing logs if any
		vmraid.db.delete("Webhook Request Log")
	...
	def test_webhook_trigger_with_enabled_webhooks(self):
		...
		self.assertEqual(len(vmraid.flags.webhooks.get("User")), 1)
		self.assertTrue(self.test_user.email in vmraid.flags.webhooks_executed)
		self.assertEqual(
			vmraid.flags.webhooks_executed.get(self.test_user.email)[0], self.sample_webhooks[0].name
		)
	...
	def test_webhook_req_log_creation(self):
		if not vmraid.db.get_value("User", "user2@integration.webhooks.test.com"):
			user = vmraid.get_doc(
				{"doctype": "User", "email": "user2@integration.webhooks.test.com", "first_name": "user2"}
			).insert()
		else:
			user = vmraid.get_doc("User", "user2@integration.webhooks.test.com")

		webhook = vmraid.get_doc("Webhook", {"webhook_doctype": "User"})
		enqueue_webhook(user, webhook)

		self.assertTrue(vmraid.db.get_all("Webhook Request Log", pluck="name"))
