...
vmraid.ui.form.on("Google Calendar", {
	refresh: function(frm) {
		...
		if (frm.doc.refresh_token) {
			frm.add_custom_button(__("Sync Calendar"), function () {
				...
				vmraid.call({
					method: "vmraid.integrations.doctype.google_calendar.google_calendar.sync",
					args: {
	...
	authorize_google_calendar_access: function(frm) {
		...
		vmraid.call({
			method: "vmraid.integrations.doctype.google_calendar.google_calendar.authorize_access",
			args: {
				"g_calendar": frm.doc.name,
