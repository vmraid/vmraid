...
from vmraid.utils import (add_days, add_to_date, get_datetime,
	get_request_site_address, get_time_zone, get_weekdays, now_datetime)
from vmraid.utils.password import set_encrypted_password
...
class GoogleCalendar(Document):
	...
	def validate(self):
		google_settings = vmraid.get_single("Google Settings")
		...
		if not google_settings.client_id or not google_settings.client_secret:
			vmraid.throw(_("Enter Client Id and Client Secret in Google Settings."))
	...
	def get_access_token(self):
		...
		if not self.refresh_token:
			button_label = vmraid.bold(_("Allow Google Calendar Access"))
...
def authorize_access(g_calendar, reauthorize=None):
	...
	google_calendar = vmraid.get_doc("Google Calendar", g_calendar)

	redirect_uri = get_request_site_address(True) + "?cmd=vmraid.integrations.doctype.google_calendar.google_calendar.google_callback"

	if not google_calendar.authorization_code or reauthorize:
		vmraid.cache().hset("google_calendar", "google_calendar", google_calendar.name)
		...
		try:
			...
			if "refresh_token" in r:
				vmraid.db.set_value("Google Calendar", google_calendar.name, "refresh_token", r.get("refresh_token"))
				vmraid.db.commit()

			vmraid.local.response["type"] = "redirect"
			vmraid.local.response["location"] = "/app/Form/{0}/{1}".format(quote("Google Calendar"), quote(google_calendar.name))

			vmraid.msgprint(_("Google Calendar has been configured."))
...
def google_callback(code=None):
	...
	"""
	google_calendar = vmraid.cache().hget("google_calendar", "google_calendar")
	vmraid.db.set_value("Google Calendar", google_calendar, "authorization_code", code)
	vmraid.db.commit()
...
def sync(g_calendar=None):
	...
	google_calendars = vmraid.get_list("Google Calendar", filters=filters)
...
def get_google_calendar_object(g_calendar):
	...
	"""
	google_settings = vmraid.get_doc("Google Settings")
	account = vmraid.get_doc("Google Calendar", g_calendar)
	...
	try:
		if account.google_calendar_id:
			...
			calendar = {
				...
				"timeZone": vmraid.db.get_single_value("System Settings", "time_zone")
			}
			created_calendar = google_calendar.calendars().insert(body=calendar).execute()
			vmraid.db.set_value("Google Calendar", account.name, "google_calendar_id", created_calendar.get("id"))
			vmraid.db.commit()
	except HttpError as err:
		vmraid.throw(_("Google Calendar - Could not create Calendar for {0}, error code {1}.").format(account.name, err.resp.status))
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	while True:
		try:
			...
			if err.resp.status == 410:
				...
				vmraid.msgprint(msg, title='Invalid Sync Token', indicator='blue')
			else:
				vmraid.throw(msg)
	...
	for idx, event in enumerate(results):
		vmraid.publish_realtime("import_google_calendar", dict(progress=idx+1, total=len(results)), user=vmraid.session.user)
		...
		if event.get("status") == "confirmed":
			...
			if not vmraid.db.exists("Event", {"google_calendar_event_id": event.get("id")}):
			...
			vmraid.db.set_value("Event", {"google_calendar_id": account.google_calendar_id, "google_calendar_event_id": event.get("id")}, "status", "Closed")
			vmraid.get_doc({
				"doctype": "Comment",
				"comment_type": "Info",
				"reference_doctype": "Event",
				"reference_name": vmraid.db.get_value("Event", {"google_calendar_id": account.google_calendar_id, "google_calendar_event_id": event.get("id")}, "name"),
...
def insert_event_to_calendar(account, event, recurrence=None):
	"""
		Inserts event in VMRaid Calendar during Sync
	"""
	calendar_event = {
		"doctype": "Event",
	...
	vmraid.get_doc(calendar_event).insert(ignore_permissions=True)

def update_event_in_calendar(account, event, recurrence=None):
	"""
		Updates Event in VMRaid Calendar if any existing Google Calendar Event is updated
	"""
	calendar_event = vmraid.get_doc("Event", {"google_calendar_event_id": event.get("id")})
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if not vmraid.db.exists("Google Calendar", {"name": doc.google_calendar}) or doc.pulled_from_google_calendar \
	...
	try:
		...
		vmraid.db.set_value("Event", doc.name, "google_calendar_event_id", event.get("id"), update_modified=False)
		vmraid.msgprint(_("Event Synced with Google Calendar."))
	except HttpError as err:
		vmraid.throw(_("Google Calendar - Could not insert event in Google Calendar {0}, error code {1}.").format(account.name, err.resp.status))

def update_event_in_google_calendar(doc, method=None):
	"""
		Updates Events in Google Calendar if any existing event is modified in VMRaid Calendar
	"""
	# Workaround to avoid triggering updation when Event is being inserted since
	# creation and modified are same when inserting doc
	if not vmraid.db.exists("Google Calendar", {"name": doc.google_calendar}) or doc.modified == doc.creation \
		or not doc.sync_with_google_calendar:
		return
	...
	try:
		...
		vmraid.throw(_("Google Calendar - Could not update Event {0} in Google Calendar, error code {1}.").format(doc.name, err.resp.status))

def delete_event_from_google_calendar(doc, method=None):
	"""
		Delete Events from Google Calendar if VMRaid Event is deleted.
	"""

	if not vmraid.db.exists("Google Calendar", {"name": doc.google_calendar}):
		return
	...
	try:
		...
		vmraid.msgprint(_("Google Calendar - Could not delete Event {0} from Google Calendar, error code {1}.").format(doc.name, err.resp.status))
