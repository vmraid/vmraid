...
def authorize_access(g_calendar, reauthorize=None):
	...
	redirect_uri = (
		...
		+ "?cmd=vmraid.integrations.doctype.google_calendar.google_calendar.google_callback"
	)
	...
	if not google_calendar.authorization_code or reauthorize:
		...
		try:
			...
			vmraid.local.response["location"] = "/app/Form/{0}/{1}".format(
...
def sync_events_from_google_calendar(g_calendar, method=None):
	...
	for idx, event in enumerate(results):
		vmraid.publish_realtime(
			"import_google_calendar", dict(progress=idx + 1, total=len(results)), user=vmraid.session.user
		)
			...
			vmraid.get_doc(
				{
					...
					"reference_name": vmraid.db.get_value(
						"Event",
						{
							"google_calendar_id": account.google_calendar_id,
							"google_calendar_event_id": event.get("id"),
...
def insert_event_in_google_calendar(doc, method=None):
	...
	"""
	if (
		not vmraid.db.exists("Google Calendar", {"name": doc.google_calendar})
