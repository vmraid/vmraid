...
from vmraid.integrations.offsite_backup_utils import get_latest_backup_file, send_email, validate_file_size, generate_files_backup
...
class S3BackupSettings(Document):
	...
	def validate(self):
		...
		try:
			...
			bucket_name = vmraid.bold(self.bucket)
			if error_code == '403':
...
def take_backup():
	...
	enqueue("vmraid.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3", queue='long', timeout=1500)
...
def take_backups_if(freq):
	if cint(vmraid.db.get_value("S3 Backup Settings", None, "enabled")):
		if vmraid.db.get_value("S3 Backup Settings", None, "frequency") == freq:
			take_backups_s3()
...
def take_backups_s3(retry_count=0):
	try:
		...
		if retry_count < 2:
			...
			enqueue("vmraid.integrations.doctype.s3_backup_settings.s3_backup_settings.take_backups_s3",
...
def notify():
	error_message = vmraid.get_traceback()
	send_email(False, 'Amazon S3', "S3 Backup Settings", "notify_email", error_message)


def backup_to_s3():
	from vmraid.utils.backups import new_backup
	from vmraid.utils import get_backups_path

	doc = vmraid.get_single("S3 Backup Settings")
	bucket = doc.bucket
