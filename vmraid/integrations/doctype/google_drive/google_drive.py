...
from vmraid.integrations.offsite_backup_utils import (get_latest_backup_file,
	send_email, validate_file_size)
from vmraid.model.document import Document
from vmraid.utils import (get_backups_path, get_chair_path,
...
class GoogleDrive(Document):
	...
	def get_access_token(self):
		...
		if not google_settings.enable:
			vmraid.throw(_("Google Integration is disabled."))

		if not self.refresh_token:
			button_label = vmraid.bold(_("Allow Google Drive Access"))
...
def authorize_access(reauthorize=None):
	...
	google_settings = vmraid.get_doc("Google Settings")
	google_drive = vmraid.get_doc("Google Drive")

	redirect_uri = get_request_site_address(True) + "?cmd=vmraid.integrations.doctype.google_drive.google_drive.google_callback"

	if not google_drive.authorization_code or reauthorize:
		if reauthorize:
			vmraid.db.set_value("Google Drive", None, "backup_folder_id", "")
		...
		try:
			...
			if "refresh_token" in r:
				vmraid.db.set_value("Google Drive", google_drive.name, "refresh_token", r.get("refresh_token"))
				vmraid.db.commit()

			vmraid.local.response["type"] = "redirect"
			vmraid.local.response["location"] = "/app/Form/{0}".format(quote("Google Drive"))

			vmraid.msgprint(_("Google Drive has been configured."))
...
def google_callback(code=None):
	...
	"""
	vmraid.db.set_value("Google Drive", None, "authorization_code", code)
	vmraid.db.commit()
...
def get_google_drive_object():
	...
	"""
	google_settings = vmraid.get_doc("Google Settings")
	account = vmraid.get_doc("Google Drive")
...
def check_for_folder_in_google_drive():
	...
	def _create_folder_in_google_drive(google_drive, account):
		...
		try:
			...
			vmraid.db.set_value("Google Drive", None, "backup_folder_id", folder.get("id"))
			vmraid.db.commit()
		except HttpError as e:
			vmraid.throw(_("Google Drive - Could not create folder in Google Drive - Error Code {0}").format(e))
	...
	try:
		...
		vmraid.throw(_("Google Drive - Could not find folder in Google Drive - Error Code {0}").format(e))

	for f in google_drive_folders.get("files"):
		if f.get("name") == account.backup_folder_name:
			vmraid.db.set_value("Google Drive", None, "backup_folder_id", f.get("id"))
			vmraid.db.commit()
...
def take_backup():
	...
	enqueue("vmraid.integrations.doctype.google_drive.google_drive.upload_system_backup_to_google_drive", queue='long', timeout=1500)
...
def upload_system_backup_to_google_drive():
	...
	for fileurl in file_urls:
		...
		try:
			...
			vmraid.throw(_("Google Drive - Could not locate - {0}").format(e))
	...
	vmraid.db.set_value("Google Drive", None, "last_backup_on", vmraid.utils.now_datetime())
	send_email(True, "Google Drive", "Google Drive", "email")
	return _("Google Drive Backup Successful.")

def daily_backup():
	drive_settings = vmraid.db.get_singles_dict('Google Drive')
...
def get_absolute_path(filename):
	...
	return "{0}/sites/{1}".format(get_chair_path(), file_path)

def set_progress(progress, message):
	vmraid.publish_realtime("upload_to_google_drive", dict(progress=progress, total=3, message=message), user=vmraid.session.user)
