...
from vmraid.integrations.utils import create_request_log, make_post_request, create_payment_gateway
from vmraid.utils import get_url, call_hook_method, cint, get_datetime


api_path = '/api/method/vmraid.integrations.doctype.paypal_settings.paypal_settings'
...
class PayPalSettings(Document):
	...
	def setup_sandbox_env(self, token):
		data = json.loads(vmraid.db.get_value("Integration Request", token, "data"))
		setattr(self, "use_sandbox", cint(vmraid._dict(data).use_sandbox) or 0)
	...
	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			vmraid.throw(_("Please select another payment method. PayPal does not support transactions in currency '{0}'").format(currency))
	...
	def get_paypal_params_and_url(self):
		...
		if hasattr(self, "use_sandbox") and self.use_sandbox:
			params.update({
				"USER": vmraid.conf.sandbox_api_username,
				"PWD": vmraid.conf.sandbox_api_password,
				"SIGNATURE": vmraid.conf.sandbox_signature
			})
	...
	def validate_paypal_credentails(self):
		...
		try:
			...
			vmraid.throw(_("Invalid payment gateway credentials"))
	...
	def execute_set_express_checkout(self, **kwargs):
		...
		if response.get("ACK")[0] != "Success":
			vmraid.throw(_("Looks like something is wrong with this site's Paypal configuration."))
...
def get_paypal_and_transaction_details(token):
	doc = vmraid.get_doc("PayPal Settings")
	doc.setup_sandbox_env(token)
	params, url = doc.get_paypal_params_and_url()

	integration_request = vmraid.get_doc("Integration Request", token)
...
def setup_redirect(data, redirect_url, custom_redirect_to=None, redirect=True):
	...
	if redirect:
		...
		vmraid.local.response["location"] = get_url(redirect_url)
...
def get_express_checkout_details(token):
	try:
		...
		doc = vmraid.get_doc("Integration Request", token)
		...
		vmraid.local.response["type"] = "redirect"
		vmraid.local.response["location"] = get_redirect_uri(doc, token, response.get("PAYERID")[0])
	...
	try:
		...
		if response.get("ACK")[0] == "Success":
			...
			if data.get("reference_doctype") and data.get("reference_docname"):
				custom_redirect_to = vmraid.get_doc(data.get("reference_doctype"),
...
def create_recurring_profile(token, payerid):
	try:
		...
		starts_at = get_datetime(subscription_details.get("start_date")) or vmraid.utils.now_datetime()
		starts_at = starts_at.replace(tzinfo=pytz.timezone(vmraid.utils.get_time_zone())).astimezone(pytz.utc)
...
def manage_recurring_payment_profile_status(profile_id, action, args, url):
	...
	if response.get("ACK")[0] != "Success" and response.get("L_ERRORCODE0", [])[0] != '11556':
		vmraid.throw(_("Failed while amending subscription"))
...
def ipn_handler():
	try:
		...
		vmraid.enqueue(method='vmraid.integrations.doctype.paypal_settings.paypal_settings.handle_subscription_notification',
...
def validate_ipn_request(data):
	def _throw():
		vmraid.throw(_("In Valid Request"), exc=vmraid.InvalidStatusError)
