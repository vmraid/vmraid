...
from vmraid.integrations.utils import create_payment_gateway, create_request_log, make_post_request
from vmraid.model.document import Document
from vmraid.utils import call_hook_method, cint, get_datetime, get_url
...
def create_recurring_profile(token, payerid):
	try:
		...
		starts_at = starts_at.replace(tzinfo=pytz.timezone(vmraid.utils.get_time_zone())).astimezone(
			pytz.utc
		)
...
def ipn_handler():
	try:
		...
		vmraid.enqueue(
			method="vmraid.integrations.doctype.paypal_settings.paypal_settings.handle_subscription_notification",
			queue="long",
			timeout=600,
			is_async=True,
