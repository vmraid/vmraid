...
from vmraid.integrations.utils import (
...
from vmraid.utils import call_hook_method, cint, get_timestamp, get_url
		...
		if self.flags.status_changed_to in ("Authorized", "Verified", "Completed"):
			if self.data.reference_doctype and self.data.reference_docname:
				...
				try:
					...
					custom_redirect_to = vmraid.get_doc(
...
def razorpay_subscription_callback():
	try:
		...
		vmraid.enqueue(
			method="vmraid.integrations.doctype.razorpay_settings.razorpay_settings.handle_subscription_notification",
			queue="long",
			timeout=600,
			is_async=True,
