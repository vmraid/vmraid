...
from vmraid.utils import get_url, call_hook_method, cint, get_timestamp
from vmraid.integrations.utils import (make_get_request, make_post_request, create_request_log,
...
class RazorpaySettings(Document):
	...
	def validate_razorpay_credentails(self):
		if self.api_key and self.api_secret:
			try:
				...
				vmraid.throw(_("Seems API Key or API Secret is wrong !!!"))

	def validate_transaction_currency(self, currency):
		if currency not in self.supported_currencies:
			vmraid.throw(_("Please select another payment method. Razorpay does not support transactions in currency '{0}'").format(currency))
	...
	def setup_addon(self, settings, **kwargs):
		...
		try:
			if not vmraid.conf.converted_rupee_to_paisa:
			...
			for addon in kwargs.get("addons"):
				...
				if not resp.get('id'):
					vmraid.log_error(str(resp), 'Razorpay Failed while creating subscription')
		except:
	...
	def prepare_subscription_details(self, settings, **kwargs):
		...
		if vmraid.flags.status !='created':
	...
	def create_order(self, **kwargs):
		...
		if self.api_key and self.api_secret:
			try:
				...
				vmraid.log(vmraid.get_traceback())
				vmraid.throw(_("Could not create razorpay order"))

	def create_request(self, data):
		self.data = vmraid._dict(data)

		try:
			self.integration_request = vmraid.get_doc("Integration Request", self.data.token)
			...
			vmraid.log_error(vmraid.get_traceback())
			return{
				"redirect_to": vmraid.redirect_to_message(_('Server Error'), _("Seems issue with server's razorpay config. Don't worry, in case of failure amount will get refunded to your account.")),
				"status": 401
			}
	...
	def authorize_payment(self):
		...
		try:
			...
			if resp.get("status") == "captured":
				...
				vmraid.log_error(str(resp), 'Razorpay Payment not authorized')
		...
		status = vmraid.flags.integration_request.status_code
	...
	def get_settings(self, data):
		...
		if cint(data.get('notes', {}).get('use_sandbox')) or data.get("use_sandbox"):
			settings.update({
				...
				"api_secret": vmraid.conf.sandbox_api_secret,
			})
	...
	def verify_signature(self, body, signature, key):
		...
		if not result:
			vmraid.throw(_('Razorpay Signature Verification Failed'), exc=vmraid.PermissionError)
...
def capture_payment(is_sandbox=False, sanbox_response=None):
	...
	for doc in vmraid.get_all("Integration Request", filters={"status": "Authorized",
		...
		try:
			...
			if resp.get("status") == "captured":
				vmraid.db.set_value("Integration Request", doc.name, "status", "Completed")

		except Exception:
			doc = vmraid.get_doc("Integration Request", doc.name)
			doc.status = "Failed"
			doc.error = vmraid.get_traceback()
			vmraid.log_error(doc.error, '{0} Failed'.format(doc.name))
...
def get_order(doctype, docname):
	...
	try:
		...
		vmraid.log_error(vmraid.get_traceback(), _("Controller method get_razorpay_order missing"))
		vmraid.throw(_("Could not create Razorpay order. Please contact Administrator"))
...
def order_payment_success(integration_request, params):
	...
	integration = vmraid.get_doc("Integration Request", integration_request)
	...
	controller.data = vmraid._dict(data)
...
def order_payment_failure(integration_request, params):
	...
	"""
	vmraid.log_error(params, 'Razorpay Payment Failure')
	params = json.loads(params)
	integration = vmraid.get_doc("Integration Request", integration_request)
	integration.update_status(params, integration.status)
...
def convert_rupee_to_paisa(**kwargs):
	...
	vmraid.conf.converted_rupee_to_paisa = True
...
def razorpay_subscription_callback():
	try:
		...
		doc = vmraid.get_doc({
			"data": json.dumps(vmraid.local.form_dict),
		...
		vmraid.enqueue(method='vmraid.integrations.doctype.razorpay_settings.razorpay_settings.handle_subscription_notification',
			queue='long', timeout=600, is_async=True, **{"doctype": "Integration Request", "docname":  doc.name})

	except vmraid.InvalidStatusError:
		pass
	except Exception as e:
		vmraid.log(vmraid.log_error(title=e))

def validate_payment_callback(data):
	def _throw():
		vmraid.throw(_("Invalid Subscription"), exc=vmraid.InvalidStatusError)
