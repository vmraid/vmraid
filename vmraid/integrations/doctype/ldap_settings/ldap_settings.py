...
import vmraid
from vmraid import _, safe_encode
from vmraid.model.document import Document
from vmraid.twofactor import (should_run_2fa, authenticate_for_2factor,confirm_otp_token)
...
class LDAPSettings(Document):
	def validate(self):
		...
		if not self.flags.ignore_mandatory:
			if self.ldap_search_string and self.ldap_search_string.endswith("={0}"):
				...
				vmraid.throw(_("LDAP Search String needs to end with a placeholder, eg sAMAccountName={0}"))
	...
	def connect_to_ldap(self, base_dn, password, read_only=True):
		try:
			...
			vmraid.throw(msg, title=_("LDAP Not Installed"))
		except ldap3.core.exceptions.LDAPInvalidCredentialsResult:
			vmraid.throw(_("Invalid username or password"))
		except Exception as ex:
			vmraid.throw(_(str(ex)))
	...
	def get_ldap_client_settings():
		...
		ldap = vmraid.get_doc("LDAP Settings")
		if ldap.enabled:
			result["enabled"] = True
			result["method"] = "vmraid.integrations.doctype.ldap_settings.ldap_settings.login"
		return result
	...
	def sync_roles(self, user, additional_groups=None):
		...
		all_mapped_roles = {r.erpadda_role for r in self.ldap_groups}
		matched_roles = {r.erpadda_role for r in self.ldap_groups if r.ldap_group.lower() in lower_groups}
	...
	def create_or_update_user(self, user_data, groups=None):
		user = None
		if vmraid.db.exists("User", user_data['email']):
			user = vmraid.get_doc("User", user_data['email'])
	...
	def authenticate(self, username, password):
		...
		if not self.enabled:
			vmraid.throw(_("LDAP is not enabled."))
		...
		if conn.search(
			...
			if conn.entries and conn.entries[0]:
				...
				if conn.modify(entry_dn, changes=changes):
					...
					vmraid.msgprint(_("Password changed successfully."))
				else:
					vmraid.throw(_("Failed to change password."))
			else:
				vmraid.throw(_("No Entry for the User {0} found within LDAP!").format(user))
		else:
			vmraid.throw(_("No LDAP User found for email: {0}").format(user))
...
def login():
	# LDAP LOGIN LOGIC
	args = vmraid.form_dict
	ldap = vmraid.get_doc("LDAP Settings")

	user = ldap.authenticate(vmraid.as_unicode(args.usr), vmraid.as_unicode(args.pwd))

	vmraid.local.login_manager.user = user.name
	if should_run_2fa(user.name):
		authenticate_for_2factor(user.name)
		if not confirm_otp_token(vmraid.local.login_manager):
			return False
