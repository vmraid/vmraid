...
class LDAP_TestCase:
	...
	def mock_ldap_connection(f):
		...
		def wrapped(self, *args, **kwargs):
			...
			with mock.patch(
				"vmraid.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.connect_to_ldap"
			) as mock_connection:
	...
	def clean_test_users():
		try:  # clean up test user 1
			vmraid.get_doc("User", "posix.user1@unit.testing").delete()
		except Exception:
			pass

		try:  # clean up test user 2
			vmraid.get_doc("User", "posix.user2@unit.testing").delete()
		except Exception:
			pass
	...
	def setUpClass(self, ldapServer="OpenLDAP"):
		...
		self.user_ldap_settings = vmraid.get_doc("LDAP Settings")
		...
		user = vmraid.get_doc(self.user1doc)
		...
		user = vmraid.get_doc(self.user2doc)
		user.insert(ignore_permissions=True)
	...
	def tearDownClass(self):
		try:
			vmraid.get_doc("LDAP Settings").delete()
	...
	def test_mandatory_fields(self):
		...
		for mandatory_field in mandatory_fields:
			...
			except vmraid.exceptions.MandatoryError:
				pass

			except vmraid.exceptions.ValidationError:
			...
			with mock.patch("ldap3.Connection") as ldap3_connection_method:
				...
				with mock.patch("ldap3.Tls") as ldap3_Tls_method:
					...
					prevent_connection_parameters = {
						...
						"mode": {
							"IP_V4_ONLY": "Locks the user to IPv4 without vmraid providing a way to configure",
							"IP_V6_ONLY": "Locks the user to IPv6 without vmraid providing a way to configure",
						},
						"auto_bind": {
	...
	def test_update_user_fields(self):
		...
		test_user = vmraid.get_doc("User", test_user_data["email"])

		self.test_class.update_user_fields(test_user, test_user_data)

		updated_user = vmraid.get_doc("User", test_user_data["email"])
	...
	def test_sync_roles(self):
		...
		role_to_group_map = {
			self.doc["ldap_groups"][0]["erpadda_role"]: self.doc["ldap_groups"][0]["ldap_group"],
			self.doc["ldap_groups"][1]["erpadda_role"]: self.doc["ldap_groups"][1]["ldap_group"],
			self.doc["ldap_groups"][2]["erpadda_role"]: self.doc["ldap_groups"][2]["ldap_group"],
			"Newsletter Manager": "default_role",
			"All": "vmraid_default_all",
			"Guest": "vmraid_default_guest",
		}
		...
		for test_user in test_user_data:
			...
			test_user_doc = vmraid.get_doc("User", test_user + "@unit.testing")
			test_user_roles = vmraid.get_roles(test_user + "@unit.testing")

			self.assertTrue(
				len(test_user_roles) == 2, "User should only be a part of the All and Guest roles"
			)  # check default vmraid roles

			self.test_class.sync_roles(test_user_doc, test_user_data[test_user])  # update user roles

			vmraid.get_doc("User", test_user + "@unit.testing")
			updated_user_roles = vmraid.get_roles(test_user + "@unit.testing")
	...
	def test_create_or_update_user(self):
		...
		vmraid.get_doc("User", test_user + "@unit.testing").delete()  # remove user 1

		with self.assertRaises(
			vmraid.exceptions.DoesNotExistError
		):  # ensure user deleted so function can be tested
			vmraid.get_doc("User", test_user + "@unit.testing")

		with mock.patch(
			"vmraid.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.update_user_fields"
		) as update_user_fields_method:

			update_user_fields_method.return_value = None

			with mock.patch(
				"vmraid.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.sync_roles"
	...
	def test_authenticate(self):
		...
		with mock.patch(
			"vmraid.integrations.doctype.ldap_settings.ldap_settings.LDAPSettings.fetch_ldap_groups"
		...
		for username, password in enumerate(invalid_users):
			...
			with self.assertRaises(vmraid.exceptions.ValidationError) as display_massage:
	...
	DOCUMENT_GROUP_MAPPINGS = [
		{
			...
			"erpadda_role": "System Manager",
		},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Users", "erpadda_role": "Blogger"},
		{"doctype": "LDAP Group Mapping", "ldap_group": "Group3", "erpadda_role": "Accounts User"},
	]
...
class Test_ActiveDirectory(LDAP_TestCase, unittest.TestCase):
	...
	DOCUMENT_GROUP_MAPPINGS = [
		...
		{"doctype": "LDAP Group Mapping", "ldap_group": "Domain Users", "erpadda_role": "Blogger"},
		{
			"doctype": "LDAP Group Mapping",
			"ldap_group": "Enterprise Administrators",
			"erpadda_role": "Accounts User",
		},
	]
