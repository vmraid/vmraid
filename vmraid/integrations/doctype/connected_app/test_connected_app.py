...
import vmraid
from vmraid.integrations.doctype.social_login_key.test_social_login_key import create_or_update_social_login_key
...
class TestConnectedApp(unittest.TestCase):
	...
	def tearDown(self):
		...
		if getattr(self, 'oauth_client', None):
			tokens = vmraid.get_all('OAuth Bearer Token', filters={
			...
			codes = vmraid.get_all('OAuth Authorization Code', filters={
				'client': self.oauth_client.name
			})
			for code in codes:
