...
from vmraid.integrations.offsite_backup_utils import (get_chunk_site,
	get_latest_backup_file, send_email, validate_file_size)
from vmraid.integrations.utils import make_post_request
from vmraid.model.document import Document
from vmraid.utils import (cint, encode, get_backups_path, get_files_path,
	get_request_site_address, get_url)
from vmraid.utils.background_jobs import enqueue
from vmraid.utils.backups import new_backup
...
class DropboxSettings(Document):
	def onload(self):
		if not self.app_access_key and vmraid.conf.dropbox_access_key:
			self.set_onload("dropbox_setup_via_site_config", 1)

	def validate(self):
		if self.enabled and self.limit_no_of_backups and self.no_of_backups < 1:
			vmraid.throw(_('Number of DB backups cannot be less than 1'))

@vmraid.whitelist()
def take_backup():
	"""Enqueue longjob for taking backup to dropbox"""
	enqueue("vmraid.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox", queue='long', timeout=1500)
	vmraid.msgprint(_("Queued for backup. It may take a few minutes to an hour."))
...
def take_backups_if(freq):
	if vmraid.db.get_value("Dropbox Settings", None, "backup_frequency") == freq:
		take_backup_to_dropbox()

def take_backup_to_dropbox(retry_count=0, upload_db_backup=True):
	did_not_upload, error_log = [], []
	try:
		if cint(vmraid.db.get_value("Dropbox Settings", None, "enabled")):
			validate_file_size()

			did_not_upload, error_log = backup_to_dropbox(upload_db_backup)
			if did_not_upload: raise Exception

			if cint(vmraid.db.get_value("Dropbox Settings", None, "send_email_for_successful_backup")):
		...
		if retry_count < 2:
			...
			enqueue("vmraid.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
...
def backup_to_dropbox(upload_db_backup=True):
	...
	if upload_db_backup:
		if vmraid.flags.create_new_backup:
...
def upload_from_folder(path, is_private, dropbox_folder, dropbox_client, did_not_upload, error_log):
	...
	if is_fresh_upload():
		...
		response = vmraid._dict({"entries": []})

	path = str(path)

	for f in vmraid.get_all("File", filters={"is_folder": 0, "is_private": is_private,
		...
		for file_metadata in response.entries:
			try:
				...
				error_log.append(vmraid.get_traceback())
...
def upload_file_to_dropbox(filename, folder, dropbox_client):
	...
	try:
		...
		if isinstance(e.error, dropbox.files.UploadError):
			...
			error += vmraid.get_traceback()
			vmraid.log_error(error)
		else:
			raise
...
def update_file_dropbox_status(file_name):
	vmraid.db.set_value("File", file_name, 'uploaded_to_dropbox', 1, update_modified=False)

def is_fresh_upload():
	file_name = vmraid.db.get_value("File", {'uploaded_to_dropbox': 1}, 'name')
...
def get_uploaded_files_meta(dropbox_folder, dropbox_client):
	try:
		...
		if isinstance(e.error, dropbox.files.ListFolderError):
			return vmraid._dict({"entries": []})
		else:
			raise

def get_dropbox_settings(redirect_uri=False):
	if not vmraid.conf.dropbox_broker_site:
		vmraid.conf.dropbox_broker_site = 'https://dropbox.erpadda.com'
	settings = vmraid.get_doc("Dropbox Settings")
	app_details = {
		"app_key": settings.app_access_key or vmraid.conf.dropbox_access_key,
		"app_secret": settings.get_password(fieldname="app_secret_key", raise_exception=False)
			if settings.app_secret_key else vmraid.conf.dropbox_secret_key,
	...
	if redirect_uri:
		app_details.update({
			'redirect_uri': get_request_site_address(True) \
				+ '/api/method/vmraid.integrations.doctype.dropbox_settings.dropbox_settings.dropbox_auth_finish' \
				if settings.app_secret_key else vmraid.conf.dropbox_broker_site\
				+ '/api/method/dropbox_erpadda_broker.www.setup_dropbox.generate_dropbox_access_token',
		})
...
def get_redirect_url():
	if not vmraid.conf.dropbox_broker_site:
		vmraid.conf.dropbox_broker_site = 'https://dropbox.erpadda.com'
	url = "{0}/api/method/dropbox_erpadda_broker.www.setup_dropbox.get_authotize_url".format(vmraid.conf.dropbox_broker_site)
...
def dropbox_auth_finish(return_access_token=False):
	...
	if callback.state or callback.code:
		...
		vmraid.respond_as_web_page(_("Dropbox Setup"),
			_("Illegal Access Token. Please try again") + close,
			indicator_color='red',
			http_status_code=vmraid.AuthenticationError.http_status_code)
...
def set_dropbox_access_token(access_token):
	vmraid.db.set_value("Dropbox Settings", None, 'dropbox_access_token', access_token)
	vmraid.db.commit()
