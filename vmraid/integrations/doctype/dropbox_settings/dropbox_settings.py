...
from vmraid.integrations.offsite_backup_utils import (
	get_chunk_site,
...
def take_backup():
	...
	enqueue(
		"vmraid.integrations.doctype.dropbox_settings.dropbox_settings.take_backup_to_dropbox",
		queue="long",
		timeout=1500,
	)
...
def get_dropbox_settings(redirect_uri=False):
	...
	app_details = {
		...
		else vmraid.conf.dropbox_secret_key,
		...
		app_details.update(
			{
				...
				if settings.app_secret_key
				else vmraid.conf.dropbox_broker_site
...
def get_redirect_url():
	if not vmraid.conf.dropbox_broker_site:
		vmraid.conf.dropbox_broker_site = "https://dropbox.erpadda.com"
	url = "{0}/api/method/dropbox_erpadda_broker.www.setup_dropbox.get_authotize_url".format(
		vmraid.conf.dropbox_broker_site
	)
