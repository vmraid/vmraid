...
from vmraid.utils import get_url, call_hook_method
from vmraid.integrations.utils import create_request_log, create_payment_gateway
...
class BraintreeSettings(Document):
	...
	def create_payment_request(self, data):
		...
		try:
			...
			return{
				"redirect_to": vmraid.redirect_to_message(_('Server Error'), _("There seems to be an issue with the server's braintree configuration. Don't worry, in case of failure, the amount will get refunded to your account.")),
				"status": 401
			}
	...
	def create_charge_on_braintree(self):
		...
		if result.is_success:
			...
			error_log = vmraid.log_error("code: " + str(result.transaction.processor_response_code) + " | text: " + str(result.transaction.processor_response_text), "Braintree Payment Error")
			self.integration_request.db_set('error', error_log.error, update_modified=False)
		else:
			self.integration_request.db_set('status', 'Failed', update_modified=False)
			for error in result.errors.deep_errors:
				error_log = vmraid.log_error("code: " + str(error.code) + " | message: " + str(error.message), "Braintree Payment Error")
				self.integration_request.db_set('error', error_log.error, update_modified=False)
		...
		if self.flags.status_changed_to == "Completed":
			...
			if self.data.reference_doctype and self.data.reference_docname:
				...
				try:
					...
					braintree_success_page = vmraid.get_hooks('braintree_success_page')
					if braintree_success_page:
						custom_redirect_to = vmraid.get_attr(braintree_success_page[-1])(self.data)
				except Exception:
					vmraid.log_error(vmraid.get_traceback())
...
def get_gateway_controller(doc):
	payment_request = vmraid.get_doc("Payment Request", doc)
	gateway_controller = vmraid.db.get_value("Payment Gateway", payment_request.payment_gateway, "gateway_controller")
	return gateway_controller

def get_client_token(doc):
	gateway_controller = get_gateway_controller(doc)
	settings = vmraid.get_doc("Braintree Settings", gateway_controller)
