...
import glob
import os
from vmraid.utils import split_emails, cint
...
def send_email(success, service_name, doctype, email_field, error_status=None):
	...
	if success:
		if not vmraid.db.get_single_value(doctype, "send_email_for_successful_backup"):
			return
...
def get_recipients(doctype, email_field):
	...
	return split_emails(vmraid.db.get_value(doctype, None, email_field))


def get_latest_backup_file(with_files=False):
	from vmraid.utils.backups import BackupGenerator
...
def validate_file_size():
	vmraid.flags.create_new_backup = True
	latest_file, site_config = get_latest_backup_file()
	file_size = get_file_size(latest_file, unit="GB")

	if file_size > 1:
		vmraid.flags.create_new_backup = False

def generate_files_backup():
	from vmraid.utils.backups import BackupGenerator

	backup = BackupGenerator(vmraid.conf.db_name, vmraid.conf.db_name,
		vmraid.conf.db_password, db_host = vmraid.db.host,
		db_type=vmraid.conf.db_type, db_port=vmraid.conf.db_port)
