...
class TestWebTemplate(unittest.TestCase):
	def test_render_web_template_with_values(self):
		doc = vmraid.get_doc("Web Template", "Hero with Right Image")
		values = {
			"title": "Test Hero",
	...
	def test_custom_stylesheet(self):
		...
		vmraid.get_doc("Website Theme", "Standard").set_as_default()

	def create_web_page(self):
		if not vmraid.db.exists("Web Page", "test-web-template"):
			vmraid.get_doc(
				{
					"doctype": "Web Page",
	...
	def create_website_theme(self):
		if not vmraid.db.exists("Website Theme", "Custom"):
			theme = vmraid.get_doc({"doctype": "Website Theme", "theme": "Custom"}).insert()
		else:
			theme = vmraid.get_doc("Website Theme", "Custom")
		return theme
