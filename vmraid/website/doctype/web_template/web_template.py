...
from vmraid.modules.export_file import get_module_path, scrub_dt_dn, write_document_file
...
class WebTemplate(Document):
	def validate(self):
		if self.standard and not (vmraid.conf.developer_mode or vmraid.flags.in_patch):
			vmraid.throw(_("Enable developer mode to create a standard Web Template"))

		for field in self.fields:
			if not field.fieldname:
				field.fieldname = vmraid.scrub(field.label)
	...
	def on_trash(self):
		if vmraid.conf.developer_mode and self.standard:
	...
	def get_template_path(self):
		...
		file_name = vmraid.scrub(self.name) + ".html"
	...
	def render(self, values=None):
		if not values:
			values = {}
		values = vmraid.parse_json(values)
		values.update({"values": values})
		template = self.get_template(self.standard)

		return vmraid.render_template(template, values)
