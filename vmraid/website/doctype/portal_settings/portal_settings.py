...
class PortalSettings(Document):
	...
	def sync_menu(self):
		...
		dirty = False
		for item in vmraid.get_hooks('standard_portal_menu_items'):
			if item.get('role') and not vmraid.db.exists("Role", item.get('role')):
				vmraid.get_doc({"doctype": "Role", "role_name": item.get('role'), "desk_access": 0}).insert()
