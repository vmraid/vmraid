...
from vmraid.utils import get_datetime, get_fullname, time_diff_in_hours
...
class PersonalDataDeletionRequest(Document):
	def __init__(self, *args, **kwargs):
		...
		self.user_data_fields = vmraid.get_hooks("user_data_fields")
	...
	def autoname(self):
		from vmraid.model.naming import set_name_from_naming_options
		...
		domain = vmraid.local.site.replace("_", "-")
		...
		vmraid.utils.validate_email_address(self.email, throw=True)

	def after_insert(self):
		self.send_verification_mail()

	def generate_url_for_confirmation(self):
		params = {"email": self.email, "name": self.name, "host_name": vmraid.local.site}
		api = vmraid.utils.get_url(
			"/api/method/vmraid.website.doctype.personal_data_deletion_request"
	...
	def disable_user(self):
		user = vmraid.get_doc("User", self.email)
		user.enabled = False
		user.save()
	...
	def send_verification_mail(self):
		...
		vmraid.sendmail(
			...
			args={
				"email": self.email,
				"name": self.name,
				"host_name": vmraid.utils.get_url(),
				"link": url,
			},
	...
	def notify_system_managers(self):
		...
		vmraid.sendmail(
			...
			args={"user": self.email, "url": vmraid.utils.get_url(self.get_url())},
	...
	def validate_data_anonymization(self):
		...
		if self.status != "Pending Approval":
			vmraid.throw(_("This request has not yet been approved by the user."))
	...
	def generate_anonymization_dict(self, ref):
		...
		meta = vmraid.get_meta(ref["doctype"])
	...
	def redact_doc(self, doc, ref):
		...
		if ref.get("rename") and doc["name"] != self.anon:
			vmraid.rename_doc(ref["doctype"], doc["name"], self.anon, force=True, show_alert=False)
	...
	def _anonymize_data(self, email=None, anon=None, set_data=True, commit=False):
		...
		vmraid.rename_doc("User", email, anon, force=True, show_alert=False)
	...
	def __redact_partial_match_data(self, doctype):
		...
		for df in vmraid.get_meta(doctype["doctype"]).fields:
...
def process_data_deletion_request():
	auto_account_deletion = vmraid.db.get_single_value("Website Settings", "auto_account_deletion")
	if auto_account_deletion < 1:
		return

	requests = vmraid.get_all(
		"Personal Data Deletion Request", filters={"status": "Pending Approval"}, pluck="name"
	)

	for request in requests:
		doc = vmraid.get_doc("Personal Data Deletion Request", request)
...
def confirm_deletion(email, name, host_name):
	...
	doc = vmraid.get_doc("Personal Data Deletion Request", name)
	host_name = vmraid.utils.get_url()
