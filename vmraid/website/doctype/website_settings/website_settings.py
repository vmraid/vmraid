...
from vmraid.utils import get_request_site_address, encode
from vmraid.model.document import Document
from six.moves.urllib.parse import quote
from vmraid.website.router import resolve_route
from vmraid.website.doctype.website_theme.website_theme import add_website_theme
...
class WebsiteSettings(Document):
	...
	def validate_home_page(self):
		...
		if self.home_page and not resolve_route(self.home_page):
			vmraid.msgprint(_("Invalid Home Page") + " (Standard pages - index, login, products, blog, about, contact)")
			self.home_page = ''
		...
		for top_bar_item in self.get("top_bar_items"):
			if top_bar_item.parent_label:
				...
				if not parent_label_item:
					# invalid item
					vmraid.throw(_("{0} does not exist in row {1}").format(top_bar_item.parent_label, top_bar_item.idx))

				elif not parent_label_item[0] or parent_label_item[0].url:
					# parent cannot have url
					vmraid.throw(_("{0} in row {1} cannot have both URL and child items").format(top_bar_item.parent_label,
						top_bar_item.idx))
	...
	def validate_footer_items(self):
		...
		for footer_item in self.get("footer_items"):
			if footer_item.parent_label:
				...
				if not parent_label_item:
					# invalid item
					vmraid.throw(_("{0} does not exist in row {1}").format(footer_item.parent_label, footer_item.idx))

				elif not parent_label_item[0] or parent_label_item[0].url:
					# parent cannot have url
					vmraid.throw(_("{0} in row {1} cannot have both URL and child items").format(footer_item.parent_label,
						footer_item.idx))

	def validate_google_settings(self):
		if self.enable_google_indexing and not vmraid.db.get_single_value("Google Settings", "enable"):
	...
	def get_access_token(self):
		...
		if not self.indexing_refresh_token:
			button_label = vmraid.bold(_("Allow API Indexing Access"))
		...
		try:
			...
			button_label = vmraid.bold(_("Allow Google Indexing Access"))
...
def get_website_settings(context=None):
	...
	context = context or vmraid._dict()
	...
	settings = vmraid.get_single("Website Settings")
	...
	via_hooks = vmraid.get_hooks("website_context")
	...
	if not context.get("favicon"):
		context["favicon"] = "/assets/vmraid/images/vmraid-favicon.svg"
