...
def authorize_access(reauthorize=None):
	...
	redirect_uri = get_request_site_address(True) + "?cmd=vmraid.website.doctype.website_settings.google_indexing.google_callback"
	...
	if not website_settings.indexing_authorization_code or reauthorize:
		...
		try:
			...
			if "refresh_token" in res:
				vmraid.db.set_value("Website Settings", website_settings.name, "indexing_refresh_token", res.get("refresh_token"))
				vmraid.db.commit()

			vmraid.local.response["type"] = "redirect"
			vmraid.local.response["location"] = "/app/Form/{0}".format(quote("Website Settings"))

			vmraid.msgprint(_("Google Indexing has been configured."))
...
def google_callback(code=None):
	...
	vmraid.db.set_value("Website Settings", None, "indexing_authorization_code", code)
	vmraid.db.commit()
...
def get_google_indexing_object():
	...
	google_settings = vmraid.get_doc("Google Settings")
	account = vmraid.get_doc("Website Settings")
...
def publish_site(url, operation_type="URL_UPDATED"):
	...
	try:
		...
		vmraid.log_error(message=e, title='API Indexing Issue')
