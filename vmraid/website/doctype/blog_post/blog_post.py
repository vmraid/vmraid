...
from vmraid.utils import today, cint, global_date_format, get_fullname, strip_html_tags, markdown, sanitize_html
from math import ceil
from vmraid.website.utils import (find_first_image, get_html_content_based_on_type,
	get_comment_list)
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return vmraid.db.get_value('Blog Category', self.blog_category,
	...
	def validate(self):
		...
		if self.featured:
			if not self.meta_image:
				vmraid.throw(_("A featured post must have a cover image"))
			self.reset_featured_for_other_blogs()

		self.set_read_time()

	def reset_featured_for_other_blogs(self):
		all_posts = vmraid.get_all("Blog Post", {"featured": 1})
		for post in all_posts:
			vmraid.db.set_value("Blog Post", post.name, "featured", 0)
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and vmraid.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True)

		if self.blogger:
			context.blogger_info = vmraid.get_doc("Blogger", self.blogger).as_dict()
		...
		context.category = vmraid.db.get_value("Blog Category",
	...
	def fetch_cta(self):
		if vmraid.db.get_single_value("Blog Settings", "show_cta_in_blog", cache=True):
			blog_settings = vmraid.get_cached_doc("Blog Settings")
	...
	def fetch_social_links_info(self):
		if not vmraid.db.get_single_value("Blog Settings", "enable_social_sharing", cache=True):
			return []

		url = vmraid.local.site + "/" +self.route
...
def get_list_context(context=None):
	...
	category = vmraid.utils.escape_html(vmraid.local.form_dict.blog_category or vmraid.local.form_dict.category)
	if category:
	...
	elif vmraid.local.form_dict.blogger:
		blogger = vmraid.db.get_value("Blogger", {"name": vmraid.local.form_dict.blogger}, "full_name")
		list_context.sub_title = _("Posts by {0}").format(blogger)
		list_context.title = blogger

	elif vmraid.local.form_dict.txt:
		list_context.sub_title = _('Filtered by "{0}"').format(sanitize_html(vmraid.local.form_dict.txt))
	...
	list_context.update(vmraid.get_doc("Blog Settings").as_dict(no_default_fields=True))

	return list_context

def get_children():
	return vmraid.db.sql("""select route as name,
...
def clear_blog_cache():
	for blog in vmraid.db.sql_list("""select route from
...
def get_blog_category(route):
	return vmraid.db.get_value("Blog Category", {"name": route}, "title") or route

def get_blog_list(doctype, txt=None, filters=None, limit_start=0, limit_page_length=20, order_by=None):
	conditions = []
	category = filters.blog_category or vmraid.utils.escape_html(vmraid.local.form_dict.blog_category or vmraid.local.form_dict.category)
	if filters:
		if filters.blogger:
			conditions.append('t1.blogger=%s' % vmraid.db.escape(filters.blogger))
	if category:
		conditions.append('t1.blog_category=%s' % vmraid.db.escape(category))

	if txt:
		conditions.append('(t1.content like {0} or t1.title like {0}")'.format(vmraid.db.escape('%' + txt + '%')))
	...
	posts = vmraid.db.sql(query, as_dict=1)
	...
	for post in posts:
		...
		post.category = vmraid.db.get_value('Blog Category', post.blog_category,
