...
from vmraid.website.utils import (
	clear_cache,
	find_first_image,
	get_comment_list,
...
class BlogPost(WebsiteGenerator):
	...
	def make_route(self):
		if not self.route:
			return (
				vmraid.db.get_value("Blog Category", self.blog_category, "route")
				+ "/"
	...
	def get_context(self, context):
		...
		context.enable_cta = not self.hide_cta and vmraid.db.get_single_value(
		...
		context.category = vmraid.db.get_value(
		...
		context.guest_allowed = vmraid.db.get_single_value("Blog Settings", "allow_guest_to_comment")
	...
	def load_feedback(self, context):
		...
		if vmraid.db.count("Feedback"):
			like_count = vmraid.db.count(
...
def get_list_context(context=None):
	...
	category = vmraid.utils.escape_html(
		vmraid.local.form_dict.blog_category or vmraid.local.form_dict.category
	)
	if category:
	...
	blog_settings = vmraid.get_doc("Blog Settings").as_dict(no_default_fields=True)
...
def get_blog_categories():
	...
	post, category = vmraid.qb.DocType("Blog Post"), vmraid.qb.DocType("Blog Category")
	return (
		vmraid.qb.from_(category)
		.select(category.name, category.route, category.title)
		.where(
			(category.published == 1)
			& ExistsCriterion(
				vmraid.qb.from_(post)
				.select("name")
...
def clear_blog_cache():
	for blog in vmraid.db.sql_list(
		"""select route from
...
def get_blog_list(
	...
	conditions = []
	category = filters.blog_category or vmraid.utils.escape_html(
		vmraid.local.form_dict.blog_category or vmraid.local.form_dict.category
	)
	if filters:
		if filters.blogger:
	...
	if txt:
		conditions.append(
			'(t1.content like {0} or t1.title like {0}")'.format(vmraid.db.escape("%" + txt + "%"))
		)
	...
	for post in posts:
		...
		post.category = vmraid.db.get_value(
