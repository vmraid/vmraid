...
vmraid.ui.form.on('Blog Post', {
	refresh: function(frm) {
		vmraid.db.get_single_value('Blog Settings', 'show_cta_in_blog').then(value => {
...
function generate_google_search_preview(frm) {
	...
	let date = frm.doc.published_on ? new vmraid.datetime.datetime(frm.doc.published_on).moment.format('ll') + ' - ' : '';
