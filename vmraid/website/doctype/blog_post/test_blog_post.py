...
from vmraid.utils import random_string, set_request
from vmraid.website.doctype.blog_post.blog_post import get_blog_list
...
class TestBlogPost(unittest.TestCase):
	...
	def test_generator_not_found(self):
		pages = vmraid.get_all("Blog Post", fields=["name", "route"], filters={"published": 0}, limit=1)

		route = f"test-route-{vmraid.generate_hash(length=5)}"

		vmraid.db.set_value("Blog Post", pages[0].name, "route", route)
	...
	def test_category_link(self):
		...
		blog_page_html = vmraid.safe_decode(blog_page_response.get_data())
		...
		cached_value = vmraid.db.value_cache[("DocType", "Blog Post", "name")]
		vmraid.db.value_cache[("DocType", "Blog Post", "name")] = (("Blog Post",),)
		...
		category_page_html = vmraid.safe_decode(category_page_response.get_data())
		# Category page should contain the blog post title
		self.assertIn(blog.title, category_page_html)

		# Cleanup
		vmraid.db.value_cache[("DocType", "Blog Post", "name")] = cached_value
		vmraid.delete_doc("Blog Post", blog.name)
		vmraid.delete_doc("Blog Category", blog.blog_category)
	...
	def test_blog_pagination(self):
		...
		filters = vmraid._dict({"blog_category": scrub(category_title)})
		...
		for blog in blogs:
			vmraid.delete_doc(blog.doctype, blog.name)
		vmraid.delete_doc("Blog Category", blogs[0].blog_category)

	def test_caching(self):
		# to enable caching
		vmraid.flags.force_website_cache = True
		print(vmraid.session.user)
...
def make_test_blog(category_title="Test Blog Category"):
	...
	if not vmraid.db.exists("Blog Category", category_name):
		vmraid.get_doc(dict(doctype="Blog Category", title=category_title)).insert()
	if not vmraid.db.exists("Blogger", "test-blogger"):
		vmraid.get_doc(
			dict(doctype="Blogger", short_name="test-blogger", full_name="Test Blogger")
		).insert()

	test_blog = vmraid.get_doc(
		dict(
			doctype="Blog Post",
