...
vmraid.ui.form.on('Web Page', {
	title: function(frm) {
		if (frm.doc.title && !frm.doc.route) {
			frm.set_value('route', vmraid.scrub(frm.doc.title, '-'));
		}
	},
	layout: function(frm) {
		if (frm.is_new()) {
			if (frm.doc.insert_code) {
				if (!frm.doc.javascript) {
					frm.set_value('javascript', `vmraid.ready(() => {\n\t\n});`);
				}
			}
		}
	},
	...
	published: function(frm) {
		...
		if (!frm.doc.published && frm.doc.end_date) {
			var end_date = vmraid.datetime.str_to_obj(vmraid.datetime.now_datetime());
...
vmraid.ui.form.on("Web Page Block", {
	edit_values(frm, cdt, cdn) {
		...
		open_web_template_values_editor(row.web_template, values)
			.then(new_values => {
				vmraid.model.set_value(cdt, cdn, "web_template_values", JSON.stringify(new_values));
			});
	},
});

vmraid.tour['Web Page'] = [
	{
		fieldname: "title",
	...
	},
	{
		...
		description: `${__('You can select one from the following,')} <br>
					<ul>
						...
						<li><b>${__('Page Builder')}</b>: ${__('VMRaid page builder using components')}</li>
					</ul>
					`
	},
	{
