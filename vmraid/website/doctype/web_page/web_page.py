...
from vmraid.utils import get_datetime, now, strip_html, quoted
...
from vmraid.website.utils import (extract_title, find_first_image, get_comment_list,
...
class WebPage(WebsiteGenerator):
	...
	def get_context(self, context):
		...
		if self.context_script:
			_locals = dict(context = vmraid._dict())
	...
	def validate_dates(self):
		if self.end_date:
			...
			if self.published and now() > self.end_date:
				...
				vmraid.msgprint(_("Clearing end date, as it cannot be in the past for published pages."))


def check_publish_status():
	# called via daily scheduler
	web_pages = vmraid.get_all("Web Page", fields=["name", "published", "start_date", "end_date"])
	...
	for page in web_pages:
		...
		if page.published:
			...
			if (start_date and now_date < start_date) or (end_date and now_date > end_date):
				vmraid.db.set_value("Web Page", page.name, "published", 0)
		else:
			# Publish pages that are inside the set date ranges
			if start_date:
				if not end_date or (end_date and now_date < end_date):
					vmraid.db.set_value("Web Page", page.name, "published", 1)



def check_broken_links():
	cnt = 0
	for p in vmraid.db.sql("select name, main_section from `tabWeb Page`", as_dict=True):
		for link in re.findall('href=["\']([^"\']*)["\']', p.main_section):
			if link.startswith("http"):
				try:
					res = requests.get(link)
				except requests.exceptions.SSLError:
					res = vmraid._dict({"status_code": "SSL Error"})
				except requests.exceptions.ConnectionError:
					res = vmraid._dict({"status_code": "Connection Error"})
...
def get_web_blocks_html(blocks):
	...
	out = vmraid._dict(html='', scripts=[], styles=[])
	extracted_scripts = []
	extracted_styles = []
	for block in blocks:
		web_template = vmraid.get_cached_doc('Web Template', block.web_template)
		rendered_html = vmraid.render_template('templates/includes/web_block.html', context={
			'web_block': block,
