...
class WebForm(WebsiteGenerator):
	...
	def validate(self):
		...
		if (not (vmraid.flags.in_install or vmraid.flags.in_patch or vmraid.flags.in_test or vmraid.flags.in_fixtures)
			and self.is_standard and not vmraid.conf.developer_mode):
...
""")
	...
	def get_context(self, context):
		...
		if vmraid.session.user == "Guest" and vmraid.form_dict.name:
			vmraid.throw(_("You need to be logged in to access this {0}.").format(self.doc_type), vmraid.PermissionError)

		if vmraid.form_dict.name and not self.has_web_form_permission(self.doc_type, vmraid.form_dict.name):
			vmraid.throw(_("You don't have the permissions to access this document"), vmraid.PermissionError)
			...
			if self.allow_edit:
				if self.allow_multiple:
					...
					if vmraid.session.user != 'Guest' and not vmraid.form_dict.name:
						vmraid.form_dict.name = vmraid.db.get_value(self.doc_type, {"owner": vmraid.session.user}, "name")
		...
		context.has_header = ((vmraid.form_dict.name or vmraid.form_dict.new)
			and (vmraid.session.user!="Guest" or not self.login_required))
	...
	def load_document(self, context):
		...
		if vmraid.form_dict.name:
			...
			if self.show_attachments:
			    context.attachments = vmraid.get_all('File', filters= {"attached_to_name": context.reference_name, "attached_to_doctype": context.reference_doctype, "is_private": 0},
	...
	def validate_mandatory(self, doc):
		...
		if missing:
			vmraid.throw(_('Mandatory Information missing:') + '<br><br>'
...
def delete_multiple(web_form_name, docnames):
	...
	if restricted_docnames:
		raise vmraid.PermissionError("You do not have permisssion to delete " + ", ".join(restricted_docnames))
...
def get_link_options(web_form_name, doctype, allow_read_on_all_link_options=False):
	...
	if doctype_validated:
		link_options = []
		if limited_to_user:
			link_options = "\n".join([doc.name for doc in vmraid.get_all(doctype, filters = {"owner":vmraid.session.user})])
		else:
			link_options = "\n".join([doc.name for doc in vmraid.get_all(doctype)])
