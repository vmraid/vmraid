...
from vmraid.website.utils import cleanup_page_name
from vmraid.website.render import clear_cache
from vmraid.modules import get_module_name
from vmraid.search.website_search import update_index_for_path, remove_document_from_index
...
class WebsiteGenerator(Document):
	...
	def send_indexing_request(self, operation_type='URL_UPDATED'):
		...
		if vmraid.db.get_single_value('Website Settings', 'enable_google_indexing') \
			and self.is_website_published() and self.meta.allow_guest_to_view:

			url = vmraid.utils.get_url(self.route)
			vmraid.enqueue('vmraid.website.doctype.website_settings.google_indexing.publish_site', \
	...
	def remove_old_route_from_index(self):
		...
		if self.allow_website_search_indexing() or vmraid.flags.in_test:
			return
	...
	def update_website_search_index(self):
		...
		"""
		if not self.allow_website_search_indexing() or vmraid.flags.in_test:
			return

		if self.is_website_published():
			vmraid.enqueue(update_index_for_path, path=self.route)
		elif self.route:
