...
import vmraid
from vmraid.website.page_renderers.base_template_page import BaseTemplatePage
from vmraid.website.router import get_base_template, get_page_info
...
class TemplatePage(BaseTemplatePage):
	...
	def get_index_path_options(search_path):
		return (
			vmraid.as_unicode(f"{search_path}{d}") for d in ("", ".html", ".md", "/index.html", "/index.md")
		)
	...
	def update_context(self):
		...
		self.context.build_version = vmraid.utils.get_build_version()

		if self.pymodule_name:
			self.pymodule = vmraid.get_module(self.pymodule_name)
		...
		self.context.developer_mode = vmraid.conf.developer_mode
		...
		if hasattr(self.pymodule, method_name):
			try:
				...
				if inspect.getfullargspec(method).args:
					...
					return method()
			except (vmraid.PermissionError, vmraid.DoesNotExistError, vmraid.Redirect):
				raise
			except Exception:
	...
	def render_template(self):
		if self.template_path.endswith("min.js"):
			...
			html = vmraid.render_template(self.source, self.context, safe_render=safe_render)
	...
	def get_raw_template(self):
		return vmraid.get_jloader().get_source(vmraid.get_jenv(), self.context.template)[0]
	...
	def convert_from_markdown(self):
		if self.template_path.endswith(".md"):
			self.source = vmraid.utils.md_to_html(self.source)
	...
	def set_standard_path(self, path):
		self.app = "vmraid"
		self.app_path = vmraid.get_app_path("vmraid")
		self.path = path
