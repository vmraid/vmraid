...
def get_comment_list(doctype, name):
	comments = vmraid.get_all('Comment',
	...
	communications = vmraid.get_all("Communication",
...
def get_home_page():
	...
	def _get_home_page():
		...
		if vmraid.session.user != 'Guest':
			...
			# portal default
			if not home_page:
				home_page = vmraid.db.get_value("Portal Settings", None, "default_portal_home")
		...
		# global
		if not home_page:
			home_page = vmraid.db.get_value("Website Settings", None, "home_page")
...
def is_signup_enabled():
	if getattr(vmraid.local, "is_signup_enabled", None) is None:
		vmraid.local.is_signup_enabled = True
		if vmraid.utils.cint(vmraid.db.get_value("Website Settings",
			"Website Settings", "disable_signup")):
				vmraid.local.is_signup_enabled = False

	return vmraid.local.is_signup_enabled
...
def get_next_link(route, url_prefix=None, app=None):
	...
	if next_item:
		if next_item.route and next_item.title:
			html = ('<p class="btn-next-wrapper">' + vmraid._("Next")\
...
def add_missing_headers():
	'''Walk and add missing headers in docs (to be called from chair execute)'''
	path = vmraid.get_app_path('erpadda', 'docs')
	for basepath, folders, files in os.walk(path):
		for fname in files:
			if fname.endswith('.md'):
				with open(os.path.join(basepath, fname), 'r') as f:
					content = vmraid.as_unicode(f.read())
