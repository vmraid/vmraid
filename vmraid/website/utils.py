...
def can_cache(no_cache=False):
	if vmraid.flags.force_website_cache:
		return True
	if vmraid.conf.disable_website_cache or vmraid.conf.developer_mode:
		return False
	if getattr(vmraid.local, "no_cache", False):
		return False
	return not no_cache
...
def get_comment_list(doctype, name):
	comments = vmraid.get_all(
		...
		or_filters=[["owner", "=", vmraid.session.user], ["published", "=", 1]],
	)
	...
	communications = vmraid.get_all(
		...
		or_filters=[
			["recipients", "like", "%{0}%".format(vmraid.session.user)],
			["cc", "like", "%{0}%".format(vmraid.session.user)],
			["bcc", "like", "%{0}%".format(vmraid.session.user)],
		],
	)
...
def get_home_page():
	if vmraid.local.flags.home_page and not vmraid.flags.in_test:
		return vmraid.local.flags.home_page
	...
	def _get_home_page():
		...
		if vmraid.session.user != "Guest":
			# by role
			for role in vmraid.get_roles():
				home_page = vmraid.db.get_value("Role", role, "home_page")
				if home_page:
					break

			# portal default
			if not home_page:
				home_page = vmraid.db.get_single_value("Portal Settings", "default_portal_home")
		...
		# global
		if not home_page:
			home_page = vmraid.db.get_single_value("Website Settings", "home_page")

		if not home_page:
			home_page = "login" if vmraid.session.user == "Guest" else "me"
	...
	return vmraid.cache().hget("home_page", vmraid.session.user, _get_home_page)


def get_home_page_via_hooks():
	home_page = None

	home_page_method = vmraid.get_hooks("get_website_user_home_page")
	if home_page_method:
		home_page = vmraid.get_attr(home_page_method[-1])(vmraid.session.user)
	elif vmraid.get_hooks("website_user_home_page"):
		home_page = vmraid.get_hooks("website_user_home_page")[-1]

	if not home_page:
		role_home_page = vmraid.get_hooks("role_home_page")
		if role_home_page:
			for role in vmraid.get_roles():
	...
	if not home_page:
		home_page = vmraid.get_hooks("home_page")
		if home_page:
...
def is_signup_disabled():
	return vmraid.db.get_single_value("Website Settings", "disable_signup", True)
...
def get_shade(color, percent=None):
	vmraid.msgprint(_("get_shade method has been deprecated."))
	return color
...
def get_toc(route, url_prefix=None, app=None):
	...
	return vmraid.get_template("templates/includes/full_index.html").render(
...
def get_full_index(route=None, app=None):
	...
	if not vmraid.local.flags.children_map:
		...
		children_map = vmraid.cache().get_value("website_full_index", _build)

		vmraid.local.flags.children_map = children_map

	return vmraid.local.flags.children_map
...
def get_sidebar_items(parent_sidebar, basepath):
	...
	hooks = vmraid.get_hooks("look_for_sidebar_json")
	look_for_sidebar_json = hooks[0] if hooks else vmraid.flags.look_for_sidebar
...
def get_portal_sidebar_items():
	sidebar_items = vmraid.cache().hget("portal_menu_items", vmraid.session.user)
	if sidebar_items is None:
		sidebar_items = []
		roles = vmraid.get_roles()
		portal_settings = vmraid.get_doc("Portal Settings", "Portal Settings")
		...
		items_via_hooks = vmraid.get_hooks("portal_menu_items")
		if items_via_hooks:
			for i in items_via_hooks:
				i["enabled"] = 1
			add_items(sidebar_items, items_via_hooks)

		vmraid.cache().hset("portal_menu_items", vmraid.session.user, sidebar_items)
...
def get_sidebar_items_from_sidebar_file(basepath, look_for_sidebar_json):
	...
	with open(sidebar_json_path, "r") as sidebarfile:
		try:
			...
			vmraid.throw("Invalid Sidebar JSON at " + sidebar_json_path)
...
def cache_html(func):
	...
	def cache_html_decorator(*args, **kwargs):
		if can_cache():
			html = None
			page_cache = vmraid.cache().hget("website_page", args[0].path)
			if page_cache and vmraid.local.lang in page_cache:
				html = page_cache[vmraid.local.lang]
			if html:
		...
		if can_cache(context.no_cache):
			page_cache = vmraid.cache().hget("website_page", args[0].path) or {}
			page_cache[vmraid.local.lang] = html
			vmraid.cache().hset("website_page", args[0].path, page_cache)
