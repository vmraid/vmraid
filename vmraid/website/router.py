...
from vmraid.website.utils import can_cache, delete_page_cache, extract_comment_tag, extract_title
...
def get_page_context(path):
	page_context = None
	if can_cache():
		page_context_cache = vmraid.cache().hget("page_context", path) or {}
		page_context = page_context_cache.get(vmraid.local.lang, None)

	if not page_context:
		page_context = make_page_context(path)
		if can_cache(page_context.no_cache):
			page_context_cache[vmraid.local.lang] = page_context
			vmraid.cache().hset("page_context", path, page_context_cache)
...
def make_page_context(path):
	...
	if not context:
		raise vmraid.PageDoesNotExistError
	...
	context.pathname = vmraid.local.path

	return context

def get_page_info_from_template(path):
	'''Return page_info from path'''
	for app in vmraid.get_installed_apps(vmraid_last=True):
...
def get_page_context_from_doctype(path):
	...
	if page_info:
		return vmraid.get_doc(page_info.get("doctype"),
...
def get_all_page_context_from_doctypes():
	...
	'''
	routes = vmraid.cache().get_value("website_generator_routes")
	if not routes:
		routes = get_page_info_from_doctypes()
		vmraid.cache().set_value("website_generator_routes", routes)
...
def get_page_info_from_doctypes(path=None):
	...
	for doctype in get_doctypes_with_web_view():
		...
		try:
			for r in vmraid.get_all(doctype, fields = ['name', 'route', 'modified'],
...
def get_page_info_from_web_page_with_dynamic_routes(path):
	...
	for d in vmraid.get_all('Web Page', fields = ['name', 'route', 'modified'],
...
def get_page_info(path, app, start, basepath=None, app_path=None, fname=None):
	...
	page_info.build_version = vmraid.utils.get_build_version()
...
def get_doctypes_with_web_view():
	...
	def _get():
		...
		doctypes += [d.name for d in vmraid.get_all('DocType', 'name, module',
			dict(has_web_view=1)) if vmraid.local.module_app[vmraid.scrub(d.module)] in installed_apps]
		return doctypes
