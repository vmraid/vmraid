...
from vmraid.website.page_renderers.list_page import ListPage
from vmraid.website.page_renderers.not_found_page import NotFoundPage
from vmraid.website.page_renderers.print_page import PrintPage
...
from vmraid.website.page_renderers.web_form import WebFormPage
from vmraid.website.router import evaluate_dynamic_routes
from vmraid.website.utils import can_cache, get_home_page
...
class PathResolver:
	...
	def resolve(self):
		...
		request = vmraid._dict()
		if hasattr(vmraid.local, "request"):
			request = vmraid.local.request or request

		# check if the request url is in 404 list
		if request.url and can_cache() and vmraid.cache().hget("website_404", request.url):
			return self.path, NotFoundPage(self.path)

		try:
			resolve_redirect(self.path, request.query_string)
		except vmraid.Redirect:
			return vmraid.flags.redirect_location, RedirectPage(self.path)
	...
	def get_custom_page_renderers():
		custom_renderers = []
		for renderer_path in vmraid.get_hooks("page_renderer") or []:
			try:
				renderer = vmraid.get_attr(renderer_path)
...
def resolve_redirect(path, query_string=None):
	...
	for rule in redirects:
		...
		if rule.get("match_with_query_string"):
			path_to_match = path + "?" + vmraid.safe_decode(query_string)

		if re.match(pattern, path_to_match):
			redirect_to = re.sub(pattern, rule["target"], path_to_match)
			vmraid.flags.redirect_location = redirect_to
			vmraid.cache().hset("website_redirects", path_to_match, redirect_to)
			raise vmraid.Redirect
