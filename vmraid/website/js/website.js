...
vmraid.provide("website");
vmraid.provide("vmraid.awesome_bar_path");
window.cur_frm = null;
...
$.extend(vmraid, {
	...
	add_asset_to_head(link) {
		return new Promise(resolve => {
			if (vmraid._assets_loaded.includes(link)) return resolve();
			let el;
			...
			el.onload = () => {
				vmraid._assets_loaded.push(link);
				resolve();
			};
		});
	},
	...
	call: function(opts) {
		...
		return $.ajax({
			...
			data: opts.args,
			dataType: "json",
			headers: { "X-VMRaid-CSRF-Token": vmraid.csrf_token, "X-VMRaid-CMD": (opts.args && opts.args.cmd  || '') || '' },
			statusCode: opts.statusCode || {
				404: function() {
					vmraid.msgprint(__("Not found"));
				},
				403: function() {
					vmraid.msgprint(__("Not permitted"));
				},
				200: function(data) {
					if(opts.callback)
						opts.callback(data);
			...
			vmraid.process_response(opts, data);
		});
	},
	...
	process_response: function(opts, data) {
		...
		if (data._server_messages) {
			...
			if(opts.error_msg) {
				...
				vmraid.msgprint(server_messages);
			}
		}
	...
	send_message: function(opts, btn) {
		return vmraid.call({
			type: "POST",
			method: "vmraid.www.contact.send_message",
			btn: btn,
			args: opts,
	...
	has_permission: function(doctype, docname, perm_type, callback) {
		return vmraid.call({
			type: "GET",
			method: "vmraid.client.has_permission",
			no_spinner: true,
	...
	render_user: function() {
		if (vmraid.is_user_logged_in()) {
			$(".btn-login-area").toggle(false);
			$(".logged-in").toggle(true);
			$(".user-image").attr("src", vmraid.get_cookie("user_image"));

			$('.user-image-wrapper').html(vmraid.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-sidebar').html(vmraid.avatar(null, 'avatar-medium', null, null, null, true));
			$('.user-image-myaccount').html(vmraid.avatar(null, 'avatar-large', null, null, null, true));
		}
	},
	freeze_count: 0,
	freeze: function(msg) {
	...
	unfreeze: function() {
		if(!vmraid.freeze_count) return; // anything open?
		vmraid.freeze_count--;
		if(!vmraid.freeze_count) {
	...
	trigger_ready: function() {
		vmraid.ready_events.forEach(function(fn) {
			fn();
		});
	},
	...
	bind_filters: function() {
		...
		$(".filter").each(function() {
			...
			var val = vmraid.utils.get_url_arg(key).replace(/\+/g, " ");
	...
	bind_navbar_search: function() {
		vmraid.get_navbar_search().on("keypress", function(e) {
			var val = $(this).val();
			if(e.which===13 && val) {
				$(this).val("").blur();
				vmraid.do_search(val);
				return false;
			}
		});
	},
	do_search: function(val) {
		var path = (vmraid.awesome_bar_path && vmraid.awesome_bar_path[location.pathname]
			|| window.search_path || location.pathname);

		window.location.href = path + "?txt=" + encodeURIComponent(val);
	},
	set_search_path: function(path) {
		vmraid.awesome_bar_path[location.pathname] = path;
	},
	...
	is_user_logged_in: function() {
		return vmraid.get_cookie("user_id") !== "Guest" && vmraid.session.user !== "Guest";
	},
	...
	show_language_picker() {
		if (vmraid.session.user === 'Guest' && window.show_language_picker) {
			vmraid.call("vmraid.translate.get_all_languages", {
				with_language_name: true
			}).then(res => {
				let language_list = res.message;
				let language = vmraid.get_cookie('preferred_language');
				let language_codes = [];
				...
				language_switcher.change(() => {
					...
					vmraid.call("vmraid.translate.set_preferred_language_cookie", {
...
vmraid.setup_search = function (target, search_scope) {
	...
	$input.on('input', vmraid.utils.debounce(() => {
		if (!$input.val()) {
			clear_dropdown();
			return;
		}
		...
		vmraid.call({
			method: 'vmraid.search.web_search',
			args: {
...
window.valid_email = function(id) {
	...
	// copied regex from vmraid/utils.js validate_type
...
$(document).ready(function() {
	window.full_name = vmraid.get_cookie("full_name");
	var logged_in = vmraid.is_user_logged_in();
	$("#website-login").toggleClass("hide", logged_in ? true : false);
	$("#website-post-login").toggleClass("hide", logged_in ? false : true);
	$(".logged-in").toggleClass("hide", logged_in ? false : true);

	vmraid.bind_navbar_search();

	// switch to app link
	if(vmraid.get_cookie("system_user")==="yes" && logged_in) {
		vmraid.add_switch_to_desk();
	}

	vmraid.render_user();
	vmraid.setup_lazy_images();
...
$(document).on("page-change", function() {
	...
	$.extend(vmraid, vmraid.get_cookies());
	vmraid.session = {'user': vmraid.user_id};

	vmraid.datetime.refresh_when();
	vmraid.trigger_ready();
	vmraid.bind_filters();
	vmraid.highlight_code_blocks();
	vmraid.add_link_to_headings();
	vmraid.make_navbar_active();
	// scroll to hash
...
vmraid.ready(function() {
	vmraid.show_language_picker();
	if (window.is_chat_enabled) {
		vmraid.require([
			"/assets/vmraid/node_modules/moment/min/moment-with-locales.min.js",
			"/assets/vmraid/node_modules/moment-timezone/builds/moment-timezone-with-data.min.js",
			"chat.bundle.css",
			"/assets/vmraid/js/lib/socket.io.min.js"
		], () => {
			vmraid.require('chat.bundle.js', () => {
				vmraid.chat.setup();
			});
		});
	}
	vmraid.socketio.init(window.socketio_port);
});
