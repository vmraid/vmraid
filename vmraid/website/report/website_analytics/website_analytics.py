# Copyright (c) 2013, VMRaid Technologies and contributors
# License: MIT. See LICENSE

from datetime import datetime

import vmraid
from vmraid.query_builder.functions import Coalesce, Count
from vmraid.utils import getdate
from vmraid.utils.dateutils import get_dates_from_timegrain
...
class WebsiteAnalytics(object):
	def __init__(self, filters=None):
		self.filters = vmraid._dict(filters or {})

		if not self.filters.to_date:
			self.filters.to_date = datetime.now()

		if not self.filters.from_date:
			self.filters.from_date = vmraid.utils.add_days(self.filters.to_date, -7)

		if not self.filters.range:
			self.filters.range = "Daily"

		self.filters.to_date = vmraid.utils.add_days(self.filters.to_date, 1)
	...
	def get_data(self):
		WebPageView = vmraid.qb.DocType("Web Page View")
		count_all = Count("*").as_("count")
		case = vmraid.qb.terms.Case().when(WebPageView.is_unique == "1", "1")
		count_is_unique = Count(case).as_("unique_count")

		return (
			vmraid.qb.from_(WebPageView)
			...
			.orderby("count", Order=vmraid.qb.desc)
		).run()
	...
	def get_chart_data(self):
		...
		self.chart_data = vmraid.db.sql(query, values=values, as_dict=1)
	...
	def prepare_chart_data(self, data):
		...
		if self.filters.range == "Monthly":
			date_range = [vmraid.utils.add_days(dd, 1) for dd in date_range]
