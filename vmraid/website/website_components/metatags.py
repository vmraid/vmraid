...
class MetaTags:
	def __init__(self, path, context):
		...
		self.tags = vmraid._dict(self.context.get("metatags") or {})
	...
	def init_metatags_from_context(self):
		...
		if self.tags.get("image"):
			self.tags["image"] = vmraid.utils.get_url(self.tags["image"])

		self.tags["language"] = vmraid.local.lang or "en"
	...
	def set_metatags_from_website_route_meta(self):
		...
		route_exists = (
			route and not route.endswith((".js", ".css")) and vmraid.db.exists("Website Route Meta", route)
		)
