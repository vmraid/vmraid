...
from vmraid.website.router import get_page_context
...
def get_context(path, args=None):
	...
	if hasattr(vmraid.local, 'request'):
		# for <body data-path=""> (remove leading slash)
		# path could be overriden in render.resolve_from_map
		context["path"] = vmraid.local.request.path.strip('/ ')
	else:
		context["path"] = path

	context.canonical = vmraid.utils.get_url(vmraid.utils.escape_html(context.path))
	...
	if module:
		...
		if hasattr(module, "get_context"):
			...
			try:
				...
				if ret:
					context.update(ret)
			except (vmraid.PermissionError, vmraid.PageDoesNotExistError, vmraid.Redirect):
				raise
			except:
				if not any([vmraid.flags.in_migrate, vmraid.flags.in_website_search_build]):
...
def build_context(context):
	...
	context.update(vmraid.local.conf.get("website_context") or {})
	...
	if context.doc:
		...
		context_controller_hooks = vmraid.get_hooks("extend_website_page_controller_context") or {}
	...
	for method in update_website_context:
		values = vmraid.get_attr(method)(context)
		if values:
...
def add_sidebar_and_breadcrumbs(context):
	...
	from vmraid.website.router import get_page_info_from_template
...
def add_sidebar_data(context):
	from vmraid.utils.user import get_fullname_and_avatar
	import vmraid.www.list

	if context.show_sidebar and context.website_sidebar:
		context.sidebar_items = vmraid.get_all('Website Sidebar Item',
	...
	info = get_fullname_and_avatar(vmraid.session.user)
...
def add_metatags(context):
	tags = vmraid._dict(context.get("metatags") or {})
	...
	if image:
		tags["og:image"] = tags["twitter:image"] = tags["image"] = vmraid.utils.get_url(image)
		tags['twitter:card'] = "summary_large_image"

	if "author" not in tags and context.author:
		tags["author"] = context.author

	tags["og:url"] = tags["url"] = vmraid.utils.get_url(context.path)
	...
	tags["language"] = vmraid.local.lang or "en"
	...
	route_exists = (route
		...
		and vmraid.db.exists('Website Route Meta', route))

	if route_exists:
		website_route_meta = vmraid.get_doc('Website Route Meta', route)
