...
3. 	Inititate the payment in client using checkout API
	function make_payment(ticket) {
		...
		razorpay = new vmraid.checkout.razorpay(options)
		razorpay.on_open = () => {
...
vmraid.provide("vmraid.checkout");

vmraid.require('https://checkout.razorpay.com/v1/checkout.js').then(() => {
	vmraid.checkout.razorpay = class RazorpayCheckout {
		constructor(opts) {
		...
		get_key() {
			return new Promise(resolve => {
				vmraid.call("vmraid.integrations.doctype.razorpay_settings.razorpay_settings.get_api_key").then(res => {
		...
		make_order() {
			return new Promise(resolve => {
				vmraid.call("vmraid.integrations.doctype.razorpay_settings.razorpay_settings.get_order", {
		...
		order_success(response) {
			vmraid.call("vmraid.integrations.doctype.razorpay_settings.razorpay_settings.order_payment_success", {
		...
		order_fail(response) {
			vmraid.call( "vmraid.integrations.doctype.razorpay_settings.razorpay_settings.order_payment_failure", {
				integration_request: this.order.integration_request,
				params: response
			})
		}
