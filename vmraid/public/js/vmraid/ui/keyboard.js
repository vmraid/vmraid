...
vmraid.provide('vmraid.ui.keys.handlers');

vmraid.ui.keys.setup = function() {
	$(window).on('keydown', function(e) {
		var key = vmraid.ui.keys.get_key(e);
		if(vmraid.ui.keys.handlers[key]) {
			var out = null;
			for(var i=0, l = vmraid.ui.keys.handlers[key].length; i<l; i++) {
				var handler = vmraid.ui.keys.handlers[key][i];
...
vmraid.ui.keys.standard_shortcuts = standard_shortcuts;
vmraid.ui.keys.add_shortcut = ({shortcut, action, description, page, target, condition, ignore_inputs = false} = {}) => {
	...
	vmraid.ui.keys.off(shortcut, page);
	// attach new handler
	vmraid.ui.keys.on(shortcut, handler);
...
vmraid.ui.keys.show_keyboard_shortcut_dialog = () => {
	if (vmraid.ui.keys.is_dialog_shown) return;
		...
		let html = shortcuts
			...
			.map(shortcut => {
				let shortcut_label = shortcut.shortcut
					.split('+')
					.map(vmraid.utils.to_title_case)
					.join('+');
				if (vmraid.utils.is_mac()) {
	...
	let dialog = new vmraid.ui.Dialog({
		...
		on_hide() {
			vmraid.ui.keys.is_dialog_shown = false;
		}
	});
	...
	dialog.show();
	vmraid.ui.keys.is_dialog_shown = true;
}

vmraid.ui.keys.get_key = function(e) {
	var keycode = e.keyCode || e.which;
	var key = vmraid.ui.keys.key_map[keycode] || String.fromCharCode(keycode);
...
vmraid.ui.keys.on = function(key, handler) {
	if(!vmraid.ui.keys.handlers[key]) {
		vmraid.ui.keys.handlers[key] = [];
	}
	vmraid.ui.keys.handlers[key].push(handler);
}

vmraid.ui.keys.off = function(key, page) {
	let handlers = vmraid.ui.keys.handlers[key];
	if (!handlers || handlers.length === 0) return;
	vmraid.ui.keys.handlers[key] = handlers.filter(h => {
...
vmraid.ui.keys.add_shortcut({
	shortcut: 'ctrl+s',
	action: function(e) {
		vmraid.app.trigger_primary_action();
		e.preventDefault();
		return false;
	},
...
vmraid.ui.keys.on('escape', function(e) {
	close_grid_and_dialog();
});

vmraid.ui.keys.on('esc', function(e) {
	close_grid_and_dialog();
});

vmraid.ui.keys.on('enter', function(e) {
	if(window.cur_dialog && cur_dialog.confirm_dialog) {
		cur_dialog.get_primary_btn().trigger('click');
	}
});

vmraid.ui.keys.on('ctrl+down', function(e) {
	var grid_row = vmraid.ui.form.get_open_grid_form();
	grid_row && grid_row.toggle_view(false, function() { grid_row.open_next() });
});

vmraid.ui.keys.on('ctrl+up', function(e) {
	var grid_row = vmraid.ui.form.get_open_grid_form();
...
'abcdefghijklmnopqrstuvwxyz'.split('').forEach((letter, i) => {
	vmraid.ui.keys.key_map[65 + i] = letter;
});
...
$(document).on('keydown', '.dropdown-toggle', (e) => {
	if (e.which === vmraid.ui.keyCode.ESCAPE) {
