vmraid.provide('vmraid.ui.keys');

let shortcut_groups = new WeakMap();
let shortcut_group_list = [];
vmraid.ui.keys.shortcut_groups = shortcut_groups;

vmraid.ui.keys.get_shortcut_group = (parent) => {
	// parent must be an object
	if (!shortcut_groups.has(parent)) {
		shortcut_groups.set(parent, new vmraid.ui.keys.AltShortcutGroup());
	}
...
vmraid.ui.keys.bind_shortcut_group_event = () => {
	...
	$(document).on('keydown', (e) => {
		let key = (vmraid.ui.keys.key_map[e.which] || '').toLowerCase();
...
vmraid.ui.keys.AltShortcutGroup = class AltShortcutGroup {
	constructor() {
		this.shortcuts_dict = {};
	...
	is_taken(letter) {
		let is_in_global_shortcut = vmraid.ui.keys.standard_shortcuts
			.filter(s => !s.page)
