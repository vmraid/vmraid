...
vmraid.ui.ThemeSwitcher = class ThemeSwitcher {
	constructor() {
		this.setup_dialog();
		this.refresh();
	}
	...
	toggle_theme(theme) {
		...
		vmraid.show_alert(__("Theme Changed"), 3);

		vmraid.xcall("vmraid.core.doctype.user.user.switch_theme", {
			theme: toTitle(theme)
		});
	}
...
vmraid.ui.add_system_theme_switch_listener = () => {
	vmraid.ui.dark_theme_media_query.addEventListener('change', () => {
		vmraid.ui.set_theme();
	});
};

vmraid.ui.dark_theme_media_query = window.matchMedia("(prefers-color-scheme: dark)");

vmraid.ui.set_theme = (theme) => {
	const root = document.documentElement;
	let theme_mode = root.getAttribute("data-theme-mode");
	if (!theme) {
		if (theme_mode === "automatic") {
			theme = vmraid.ui.dark_theme_media_query.matches ? 'dark' : 'light';
		}
	}
