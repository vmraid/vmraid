...
vmraid.provide('vmraid.tags');

vmraid.search.AwesomeBar = class AwesomeBar {
	setup(element) {
		var me = this;
		...
		$input.on("input", vmraid.utils.debounce(function(e) {
			...
			if (txt && txt.length > 1) {
				...
				me.options = me.options.concat(
					me.deduplicate(vmraid.search.utils.get_recent_pages(txt || "")));
				me.options = me.options.concat(vmraid.search.utils.get_frequent_links());
			}
			me.add_help();
		...
		$input.on("awesomplete-select", function(e) {
			...
			if(item.route_options) {
				vmraid.route_options = item.route_options;
			}

			if(item.onclick) {
				item.onclick(item.match);
			} else {
				vmraid.set_route(item.route);
			}
			$input.val("");
		});
		...
		vmraid.search.utils.setup_recent();
		vmraid.tags.utils.fetch_tags();
	}
	...
	add_help() {
		this.options.push({
			...
			onclick: function() {
				...
				vmraid.msgprint(txt, __("Search Help"));
			}
		});
	}
	...
	build_options(txt) {
		var options = vmraid.search.utils.get_creatables(txt).concat(
			vmraid.search.utils.get_search_in_list(txt),
			vmraid.search.utils.get_doctypes(txt),
			vmraid.search.utils.get_reports(txt),
			vmraid.search.utils.get_pages(txt),
			vmraid.search.utils.get_workspaces(txt),
			vmraid.search.utils.get_dashboards(txt),
			vmraid.search.utils.get_recent_pages(txt || ""),
			vmraid.search.utils.get_executables(txt)
		);
		if (txt.charAt(0) === "#") {
			options = vmraid.tags.utils.get_tags(txt);
		}
	...
	make_global_search(txt) {
		...
		// 		<span class="ellipsis">Search for ${vmraid.utils.xss_sanitise(txt).bold()}</span>
		// 		<kbd>↵</kbd>
		// 	</span>
		// `);

		// search_text.click(() => {
		// 	vmraid.searchdialog.search.init_search(txt, "global_search");
		// });
		...
		this.options.push({
			label: `
				<span class="flex justify-between text-medium">
					<span class="ellipsis">${ __('Search for {0}', [vmraid.utils.xss_sanitise(txt).bold()])}</span>
					<kbd>↵</kbd>
				</span>
			`,
	...
	make_search_in_current(txt) {
		...
		if(route[0]==="List" && txt.indexOf(" in") === -1) {
			...
			var meta = vmraid.get_meta(vmraid.container.page.list_view.doctype);
		...
		if(first==parseInt(first) || first==="(" || first==="=") {
			...
			try {
				...
				this.options.push({
					...
					default: "Calculator",
					onclick: function() {
						vmraid.msgprint(formatted_value, __("Result"));
					}
				});
			} catch(e) {
				// pass
	...
	make_random(txt) {
		if(txt.toLowerCase().includes('random')) {
			this.options.push({
				...
				value: vmraid.utils.get_random(16),
				onclick: function() {
					vmraid.msgprint(vmraid.utils.get_random(16), __("Result"));
				}
			})
		}
	}
