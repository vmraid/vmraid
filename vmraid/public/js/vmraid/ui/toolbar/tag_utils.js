...
vmraid.tags.utils = {
	get_tags: function(txt) {
		...
		for (let i in vmraid.tags.tags) {
			let tag = vmraid.tags.tags[i];
			let level = vmraid.search.utils.fuzzy_search(txt, tag);
			if (level) {
				out.push({
					type: "Tag",
					label: __("#{0}", [vmraid.search.utils.bolden_match_part(__(tag), txt)]),
					value: __("#{0}", [__(tag)]),
					index: 1 + level,
					match: tag,
					onclick() {
						// Use Global Search Dialog for tag search too.
						vmraid.searchdialog.search.init_search("#".concat(tag), "tags");
					}
				});
			}
		}
	...
	fetch_tags() {
		vmraid.call({
			method: "vmraid.desk.doctype.tag.tag.get_tags_list_for_awesomebar",
			callback: function(r) {
				if (r && r.message) {
					vmraid.tags.tags = $.extend([], r.message);
				}
			}
		});
	},
	...
	get_tag_results: function(tag) {
		...
		return new Promise(function(resolve) {
			vmraid.call({
				method: "vmraid.desk.doctype.tag.tag.get_documents_for_tag",
				args: {
					tag: tag
				},
				callback: function(r) {
