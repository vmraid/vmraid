...
vmraid.search.SearchDialog = class {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	make() {
		this.search_dialog = new vmraid.ui.Dialog({
			minimizable: true,
			size: "large"
		});
		this.set_header();
		...
		this.searches = {
			global_search: {
				...
				get_results: (keywords, callback) => {
					let start = 0,
						limit = 1000;
					let results = vmraid.search.utils.get_nav_results(keywords);
					vmraid.search.utils
			...
			tags: {
				...
				no_results_status: keyword =>
					...
					"</div>",
				get_results: (keywords, callback) => {
					var results = vmraid.search.utils.get_nav_results(keywords);
					vmraid.tags.utils.get_tag_results(keywords).then(
						global_results => {
							results = results.concat(global_results);
							callback(results, keywords);
						},
	...
	render_data(result_sets) {
		let $search_results = $(vmraid.render_template("search")).addClass(
			"hide"
		);
	...
	render_result(type, result) {
		...
		if (result.image !== undefined) {
			let avatar_html = vmraid.get_avatar(
				"avatar-medium",
				result.label,
				result.image
			);
	...
	handle_result_click(result, $result) {
		if (result.route_options) {
			vmraid.route_options = result.route_options;
		}
		$result.on("click", () => {
