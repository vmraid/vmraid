 ...
 *
 * @returns {vmraid.ui.Page}
 */

/**
 * @typedef {Object} vmraid.ui.Page
 */


vmraid.ui.make_app_page = function(opts) {
	opts.parent.page = new vmraid.ui.Page(opts);
	return opts.parent.page;
}

vmraid.ui.pages = {};

vmraid.ui.Page = class Page {
	constructor(opts) {
		$.extend(this, opts);
		...
		this.make();
		vmraid.ui.pages[vmraid.get_route_str()] = this;
	}
	...
	setup_scroll_handler() {
		let last_scroll = 0;
		window.addEventListener('scroll', vmraid.utils.throttle(() => {
	...
	load_lib(callback) {
		vmraid.require(this.required_libs, callback);
	}

	add_main_section() {
		$(vmraid.render_template("page", {})).appendTo(this.wrapper);
	...
	setup_page() {
		...
		this.card_layout && this.main.addClass('vmraid-card');
	...
	setup_sidebar_toggle() {
		...
		if (this.disable_sidebar_toggle || !sidebar_wrapper.length) {
			...
			sidebar_toggle.click(() => {
				if (vmraid.utils.is_xs() || vmraid.utils.is_sm()) {
	...
	update_sidebar_icon() {
		...
		sidebar_toggle_icon.html(vmraid.utils.icon(is_sidebar_visible ? 'sidebar-collapse' : 'sidebar-expand', 'md'));
	}
	...
	add_action_icon(icon, click, css_class='', tooltip_label) {
		const button = $(`
			<button class="text-muted btn btn-default ${css_class} icon-btn">
				${vmraid.utils.icon(icon)}
			</button>
		`);

		button.appendTo(this.icon_group.removeClass("hide"));
		button.click(click);
		button.attr("title", __(tooltip_label || vmraid.unscrub(icon)))
	...
	get_icon_label(icon, label) {
		...
		return `${icon ? vmraid.utils.icon(icon_name, size) : ''} <span class="hidden-xs"> ${__(label)} </span>`;
	}
	...
	*/
	add_dropdown_item({label, click, standard, parent, shortcut, show_parent=true, icon=null}) {
		...
		if (icon) {
			$icon = `<span class="menu-item-icon">${vmraid.utils.icon(icon)}</span>`;
		}
		...
		if (shortcut) {
			...
			`);
			vmraid.ui.keys.add_shortcut(shortcut_obj);
		} else {
			$li = $(`
				<li>
					<a class="grey-link dropdown-item" href="#" onClick="return false;">
	...
	*/
	...
	set_title(title, icon=null, strip=true, tab_title="") {
		...
		this.title = title;
		vmraid.utils.set_title(tab_title || title);
		if (icon) {
			title = `${vmraid.utils.icon(icon)} ${title}`;
		}
	...
	add_button(label, click, opts) {
		...
		let button = $(`<button
			class="btn ${opts.btn_class || 'btn-default'} ${opts.btn_size || 'btn-sm'} ellipsis">
				${opts.icon ? vmraid.utils.icon(opts.icon): ''}
				${label}
		</button>`);
	...
	add_dropdown_button(parent, label, click, icon) {
		vmraid.ui.toolbar.add_dropdown_button(parent, label, click, icon);
	}
	...
	add_field(df, parent) {
		...
		var f = vmraid.ui.form.make_control({
			df: df,
