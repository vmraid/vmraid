vmraid.ui.Filter = class {
	constructor(opts) {
		...
		this.utils = vmraid.ui.filter_utils;
		this.set_conditions();
	...
	set_conditions_from_config() {
		if (vmraid.boot.additional_filters_config) {
			this.filters_config = vmraid.boot.additional_filters_config;
	...
	make() {
		this.filter_edit_area = $(
			vmraid.render_template('edit_filter', {
	...
	make_select() {
		this.fieldselect = new vmraid.ui.FieldSelect({
	...
	make_field(df, old_fieldtype) {
		...
		let f = vmraid.ui.form.make_control({
			df: df,
			parent: field_area,
			only_input: true,
		});
	...
	toggle_nested_set_conditions(df) {
		let show_condition =
			df.fieldtype === 'Link' &&
			vmraid.boot.nested_set_doctypes.includes(df.options);
...
vmraid.ui.filter_utils = {
	get_formatted_value(field, value) {
		...
		return vmraid.format(value, field.df, { only_value: 1 });
	},
