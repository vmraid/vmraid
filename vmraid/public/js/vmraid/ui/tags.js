...
vmraid.ui.Tags = class {
	...
	bind() {
		...
		const select_tag = function() {
			const tagValue = vmraid.utils.xss_sanitise(me.$input.val());
			me.addTag(tagValue);
			me.$input.val('');
		};
	...
	removeTag(label) {
		label = vmraid.utils.xss_sanitise(label);
	...
	get_tag(label) {
		let $tag = vmraid.get_data_pill(label, label, (target, pill_wrapper) => {
