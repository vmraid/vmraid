...
vmraid.ui.Tree = class {
	constructor({
		...
		if (!icon_set) {
			this.icon_set = {
				open: vmraid.utils.icon('folder-open', 'md'),
				closed: vmraid.utils.icon('folder-normal', 'md'),
				leaf: vmraid.utils.icon('primitive-dot', 'xs')
			};
		}
	...
	get_all_nodes(value, is_root, label) {
		...
		return new Promise(resolve => {
			vmraid.call({
				method: 'vmraid.desk.treeview.get_all_nodes',
				args: args,
				callback: (r) => {
					resolve(r.message);
				}
	...
	on_node_click(node) {
		this.expand_node(node);
		vmraid.dom.activate(this.wrapper, node.$tree_link, 'tree-link');
