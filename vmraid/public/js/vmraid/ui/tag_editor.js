vmraid.ui.TagEditor = class TagEditor {
	constructor(opts) {
		/* docs:
		Arguments
	...
	setup_tags() {
		...
		this.tags = new vmraid.ui.Tags({
			parent: this.wrapper,
			placeholder: __("Add a tag ..."),
			onTagAdd: (tag) => {
				if(me.initialized && !me.refreshing) {
					return vmraid.call({
						method: "vmraid.desk.doctype.tag.tag.add_tag",
			...
			onTagRemove: (tag) => {
				if(!me.refreshing) {
					return vmraid.call({
						method: "vmraid.desk.doctype.tag.tag.remove_tag",
						args: me.get_args(tag),
						callback: function(r) {
	...
	setup_awesomplete() {
		...
		$input.on("input", function(e) {
			...
			vmraid.call({
				method: "vmraid.desk.doctype.tag.tag.get_tags",
				args:{
