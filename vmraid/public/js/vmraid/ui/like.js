...
vmraid.ui.is_liked = function(doc) {
	var liked = vmraid.ui.get_liked_by(doc);
	return liked.indexOf(vmraid.session.user)===-1 ? false : true;
}

vmraid.ui.get_liked_by = function(doc) {
...
vmraid.ui.toggle_like = function($btn, doctype, name, callback) {
	var add = $btn.hasClass("not-liked") ? "Yes" : "No";
	// disable click
	$btn.css('pointer-events', 'none');

	vmraid.call({
		method: "vmraid.desk.like.toggle_like",
		quiet: true,
		args: {
			doctype: doctype,
			name: name,
			...
			if(!r.exc) {
				...
				if(doc) {
					var liked_by = JSON.parse(doc._liked_by || "[]"),
						idx = liked_by.indexOf(vmraid.session.user);
					if(add==="Yes") {
						if(idx===-1)
							liked_by.push(vmraid.session.user);
					} else {
						if(idx!==-1) {
...
vmraid.ui.click_toggle_like = function() {
	var $btn = $(this);
	...
	vmraid.ui.toggle_like($btn, doctype, name, function() {
		if (not_liked) {
...
vmraid.ui.setup_like_popover = ($parent, selector, check_not_liked=true) => {
	if (vmraid.dom.is_touchscreen()) {
		return;
	}
			...
			content: () => {
				...
				liked_by.forEach(user => {
					...
					liked_by_list.append(`
						<li data-user=${user}>${vmraid.avatar(user, "avatar-xs")}
							<span>${vmraid.user.full_name(user)}</span>
						</li>
					`);
				});
				...
				liked_by_list.children('li').click(ev => {
					...
					vmraid.set_route(link_base + user);
				});
