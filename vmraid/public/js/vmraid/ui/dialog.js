...
vmraid.ui.open_dialogs = [];

vmraid.ui.Dialog = class Dialog extends vmraid.ui.FieldGroup {
	constructor(opts) {
		super();
		this.display = false;
		this.is_dialog = true;
	...
	make() {
		...
		this.$wrapper
			.on("hide.bs.modal", function() {
				...
				if(vmraid.ui.open_dialogs[vmraid.ui.open_dialogs.length-1]===me) {
					vmraid.ui.open_dialogs.pop();
					if(vmraid.ui.open_dialogs.length) {
						window.cur_dialog = vmraid.ui.open_dialogs[vmraid.ui.open_dialogs.length-1];
					} else {
						window.cur_dialog = null;
					}
				}
			...
			.on("shown.bs.modal", function() {
				...
				vmraid.ui.open_dialogs.push(me);
				me.focus_on_first_input();
				me.on_page_show && me.on_page_show();
				$(document).trigger('vmraid.ui.Dialog:shown');
			})
			.on('scroll', function() {
	...
	toggle_minimize() {
		...
		this.get_minimize_btn().html(vmraid.utils.icon(icon));
