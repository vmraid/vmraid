...
vmraid.ui.color = {
	get: function(color_name, shade) {
		...
		return vmraid.ui.color_map;
	},
	get_color: function(color_name) {
		const color_names = Object.keys(vmraid.ui.color_map);
		if(color_names.includes(color_name)) {
			return vmraid.ui.color_map[color_name];
		} else {
	...
	all: function() {
		return Object.values(vmraid.ui.color_map)
			.reduce((acc, curr) => acc.concat(curr) , []);
	},
	names: function() {
		return Object.keys(vmraid.ui.color_map);
	},
	...
	get_color_name: function(hex) {
		for (const key in vmraid.ui.color_map) {
			const colors = vmraid.ui.color_map[key];
...
vmraid.ui.color_map = vmraid.ui.color.get_color_map();
