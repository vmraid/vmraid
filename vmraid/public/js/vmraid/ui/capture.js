 ...
 *
 * @example
 * vmraid._.get_data_uri(video)
 * // returns "data:image/pngbase64,..."
 */
vmraid._.get_data_uri = element => {
...
/**
 * @description VMRaid's Capture object.
 *
 * @example
 * const capture = vmraid.ui.Capture()
 * capture.show()
 *
 ...
 */
vmraid.ui.Capture = class {
	constructor(options = {}) {
		this.options = vmraid.ui.Capture.OPTIONS;
	...
	set_options(options) {
		this.options = { ...vmraid.ui.Capture.OPTIONS, ...options };
	...
	build_dialog() {
		...
		me.$template = $(vmraid.ui.Capture.TEMPLATE);
	...
	show_for_desktop() {
		...
		this.render_stream()
			...
			.catch(err => {
				if (me.options.error) {
					vmraid.show_alert(vmraid.ui.Capture.ERR_MESSAGE, 3);
				}
	...
	setup_take_photo_action() {
		...
		this.dialog.set_primary_action(__('Take Photo'), () => {
			const data_url = vmraid._.get_data_uri(me.video);
...
vmraid.ui.Capture.OPTIONS = {
	title: __("Camera"),
	animate: false,
	error: false
};
vmraid.ui.Capture.ERR_MESSAGE = __('Unable to load camera.');
vmraid.ui.Capture.TEMPLATE = `
<div class="vmraid-capture">
	<div class="embed-responsive embed-responsive-16by9 fc-stream-container">
		<video class="fc-stream embed-responsive-item">${vmraid.ui.Capture.ERR_MESSAGE}</video>
	</div>
