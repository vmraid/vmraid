...
vmraid.ui.Notifications = class Notifications {
	constructor() {
		this.tabs = {};
		this.notification_settings = vmraid.boot.notification_settings;
		this.make();
	}
	...
	setup_headers() {
		...
		$(`<span class="notification-settings pull-right" data-action="go_to_settings">
			${vmraid.utils.icon('setting-gear')}
		</span>`)
			.on('click', (e) => {
				e.stopImmediatePropagation();
				this.dropdown.dropdown('hide');
				vmraid.set_route('Form', 'Notification Settings', vmraid.session.user);
			}).appendTo(this.header_actions)
			.attr('title', __("Notification Settings"))
			.tooltip({ delay: { "show": 600, "hide": 100}, trigger: "hover"  });

		$(`<span class="mark-all-read pull-right" data-action="mark_all_as_read">
			${vmraid.utils.icon('mark-as-read')}
		</span>`)
	...
	mark_all_as_read(e) {
		...
		vmraid.call(
			'vmraid.desk.doctype.notification_log.notification_log.mark_all_as_read',
		);
	}
...
vmraid.ui.notifications = {
	get_notification_config() {
		return vmraid.xcall('vmraid.desk.notifications.get_notification_info').then(r => {
			vmraid.ui.notifications.config = r;
			return r;
		});
	},

	show_open_count_list(doctype) {
		if (!vmraid.ui.notifications.config) {
	...
	route_to_list_with_filters(doctype) {
		let filters = vmraid.ui.notifications.config['conditions'][doctype];
		if (filters && $.isPlainObject(filters)) {
			if (!vmraid.route_options) {
				vmraid.route_options = {};
			}
			$.extend(vmraid.route_options, filters);
		}
		vmraid.set_route('List', doctype);
	}
};
...
class NotificationsView extends BaseNotificationsView {
	...
	mark_as_read(docname, $el) {
		vmraid.call(
			'vmraid.desk.doctype.notification_log.notification_log.mark_as_read',
			{ docname: docname }
		).then(() => {
	...
	get_dropdown_item_html(field) {
		...
		message = title ? message.replace(title[1], vmraid.ellipsis(strip_html(title[1]), 100)) : message;

		let timestamp = vmraid.datetime.comment_when(field.creation);
		...
		let user_avatar = vmraid.avatar(user, 'avatar-medium user-avatar');
			...
			if (this.dropdown_items.length) {
				...
				this.container.append($(`<div class="notification-null-state">
					<div class="text-center">
						<img src="/assets/vmraid/images/ui-states/notification-empty-state.svg" alt="Generic Empty State" class="null-state">
	...
	get_notifications_list(limit) {
		return vmraid.db.get_list('Notification Log', {
			fields: ['*'],
			limit: limit,
	...
	get_item_link(notification_doc) {
		...
		return vmraid.utils.get_form_link(
			link_doctype,
			link_docname
		);
	}
	...
	toggle_seen(flag) {
		vmraid.call(
			'vmraid.desk.doctype.notification_settings.notification_settings.set_seen_value',
			{
				value: cint(flag),
	...
	setup_notification_listeners() {
		vmraid.realtime.on('notification', () => {
			this.toggle_notification_icon(false);
			this.update_dropdown();
		});

		vmraid.realtime.on('indicator_hide', () => {
		...
		this.parent.on('show.bs.dropdown', () => {
			...
			if (this.notifications_icon.find('.notifications-unseen').is(':visible')) {
				this.toggle_notification_icon(true);
				vmraid.call(
					'vmraid.desk.doctype.notification_log.notification_log.trigger_indicator_hide'
				);
			}
		});
...
class EventsView extends BaseNotificationsView {
	make() {
		let today = vmraid.datetime.get_today();
		vmraid.xcall('vmraid.desk.doctype.event.event.get_events', {
			start: today,
			end: today
		}).then(event_list => {
	...
	render_events_html(event_list) {
		...
		if (event_list.length) {
			let get_event_html = (event) => {
				let time = __("All Day");
				if (!event.all_day) {
					let start_time = vmraid.datetime.get_time(event.starts_on);
					let days_diff = vmraid.datetime.get_day_diff(event.ends_on, event.starts_on);
					let end_time = vmraid.datetime.get_time(event.ends_on);
					if (days_diff > 1) {
				...
				if (event.particpants) {
					particpants = vmraid.avatar_group(event.particpants, 3);
				}
			...
			html = `
				<div class="notification-null-state">
					<div class="text-center">
					<img src="/assets/vmraid/images/ui-states/event-empty-state.svg" alt="Generic Empty State" class="null-state">
