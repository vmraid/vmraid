...
vmraid.provide("vmraid.web_form");
import EventEmitterMixin from '../../vmraid/event_emitter';

export default class WebForm extends vmraid.ui.FieldGroup {
	constructor(opts) {
		super();
		Object.assign(this, opts);
		vmraid.web_form = this;
		vmraid.web_form.events = {};
		Object.assign(vmraid.web_form.events, EventEmitterMixin);
	...
	make() {
		...
		vmraid.init_client_script && vmraid.init_client_script();
		vmraid.web_form.events.trigger('after_load');
	...
	set_default_values() {
		let values = vmraid.utils.get_query_params();
		delete values.new;
	...
	setup_primary_action() {
		...
		if (!this.is_multi_step_form && $('.vmraid-card').height() > 600) {
	...
	is_next_section_empty(section) {
		...
		let visible_controls = _section.find(".vmraid-control:not(.hide-control)");
	...
	save() {
		...
		if (this.validate && !this.validate()) {
			vmraid.throw(__("Couldn't save, please check the data you have entered"), __("Validation Error"));
		}
		...
		// Save
		window.saving = true;
		vmraid.form_dirty = false;

		vmraid.call({
			type: "POST",
			method: "vmraid.website.doctype.web_form.web_form.accept",
			args: {
				data: this.doc,
				web_form: this.name,
			...
			callback: response => {
				...
				if (!response.exc) {
					...
					vmraid.web_form.events.trigger('after_save');
					...
					if (is_new && (response.message.attachment || response.message.file)) {
						vmraid.call({
							type: 'POST',
							method: "vmraid.handler.upload_file",
							args: {
	...
	handle_success(data) {
		...
		vmraid.toast({message: success_message, indicator:'green'});
