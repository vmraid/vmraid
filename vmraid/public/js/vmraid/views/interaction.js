...
vmraid.provide("vmraid.interaction_settings");

vmraid.views.InteractionComposer = class InteractionComposer {
	constructor(opts) {
		$.extend(this, opts);
		this.make();
	}
	...
	get_event_categories() {
		let me = this;
		vmraid.model.with_doctype('Event', () => {
			let categories = vmraid.meta.get_docfield("Event", "event_category").options.split("\n");
	...
	create_interaction(btn, form_values, selected_attachments) {
		...
		if (!("owner" in interaction_values)){
			interaction_values["owner"] = vmraid.session.user;
		}
		if (!("assigned_by" in interaction_values) && interaction_values["doctype"] == "ToDo") {
			interaction_values["assigned_by"] = vmraid.session.user;
		}
		return vmraid.call({
			...
			callback: function(r) {
				if(!r.exc) {
					...
					vmraid.msgprint(__("There were errors while creating the document. Please try again."));
				}
			}
		});
	...
	add_attachments(doc, attachments) {
		vmraid.call({
			method:'vmraid.utils.file_manager.add_attachments',
			args: {
				doctype: doc.doctype,
				name: doc.name,
