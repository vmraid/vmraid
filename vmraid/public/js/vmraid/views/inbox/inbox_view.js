/**
 * vmraid.views.InboxView
 */

vmraid.provide("vmraid.views");

vmraid.views.InboxView = class InboxView extends vmraid.views.ListView {
	static load_last_view() {
		const route = vmraid.get_route();
		if (!route[3] && vmraid.boot.email_accounts.length) {
			let email_account;
			if (vmraid.boot.email_accounts[0].email_id == "All Accounts") {
				email_account = "All Accounts";
			} else {
				email_account = vmraid.boot.email_accounts[0].email_account;
			}
			vmraid.set_route("List", "Communication", "Inbox", email_account);
			return true;
		} else if (!route[3] || (route[3] !== "All Accounts" && !is_valid(route[3]))) {
			vmraid.throw(__('No email account associated with the User. Please add an account under User > Email Inbox.'));
		}
		return false;

		function is_valid(email_account) {
			return vmraid.boot.email_accounts.find(d => d.email_account === email_account);
		}
	}
	...
	setup_defaults() {
		...
		this.email_account = vmraid.get_route()[3];
	...
	get_seen_class(doc) {
		const seen =
			Boolean(doc.seen) || JSON.parse(doc._seen || '[]').includes(vmraid.session.user)
				? ''
				: 'bold';
		return seen;
	}
	...
	get_meta_html(email) {
		...
		const form_link = vmraid.utils.get_form_link(email.reference_doctype, email.reference_name);
	...
	get_inbox_filters() {
		...
		if (email_account === "Sent") {
			...
			filters = default_filters.concat([
				...
				["Communication", "email_account", "in", vmraid.boot.all_accounts, true]
			]);
		} else {
			var op = "=";
			if (email_account == "All Accounts") {
				op = "in";
				email_account = vmraid.boot.all_accounts;
			}
	...
	get_no_result_message() {
		...
		} else if (!email_account && !vmraid.boot.email_accounts.length) {
		...
		const html = vmraid.model.can_create(args.doctype) ?
			`<p>${args.msg}</p>
			<p>
	...
	make_new_doc() {
		if (!this.email_account && !vmraid.boot.email_accounts.length) {
			vmraid.route_options = {
				'email_id': vmraid.session.user_email
			};
			vmraid.new_doc('Email Account');
		} else {
