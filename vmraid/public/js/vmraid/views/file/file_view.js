...
vmraid.views.FileView = class FileView extends vmraid.views.ListView {
	static load_last_view() {
		const route = vmraid.get_route();
		if (route.length === 2) {
			const view_user_settings = vmraid.get_user_settings("File", "File");
			vmraid.set_route(
				"List",
				"File",
				view_user_settings.last_folder || vmraid.boot.home_folder
			);
			return true;
		}
	...
	setup_view() {
		...
		this.page.add_button(__("Toggle Grid View"), () => {
			vmraid.views.FileView.grid_view = !vmraid.views.FileView.grid_view;
			this.refresh();
		});
	}
	...
	get_args() {
		...
		if (vmraid.views.FileView.grid_view) {
			Object.assign(args, {
		...
		const items = [
			{
				label: __("Home"),
				action: () => {
					vmraid.set_route("List", "File", "Home");
				}
			},
			{
				label: __("New Folder"),
				action: () => {
					vmraid.prompt(
						...
						values => {
							...
							vmraid.call({
								method:
									"vmraid.core.doctype.file.file.create_new_folder",
								args: data
							});
						},
			...
			},
			{
				...
				action: () => {
					new vmraid.ui.FileUploader({
						...
						on_success: file => {
							vmraid.show_alert(__("Unzipping files..."));
							vmraid
								.call(
									"vmraid.core.doctype.file.file.unzip_file",
									{
										name: file.name
									}
								)
	...
	add_file_action_buttons() {
		...
		this.$paste_btn = this.page
			.add_button(__("Paste"), () =>
				vmraid.file_manager.paste(this.current_folder)
			)
			.hide();
		...
		this.page.add_actions_menu_item(__('Export as zip'), () => {
			...
			if (docnames.length) {
				open_url_post('/api/method/vmraid.core.doctype.file.file.zip_files', {
	...
	set_fields() {
		this.fields = this.meta.fields
			.filter(
				df => vmraid.model.is_value_type(df.fieldtype) && !df.hidden
			)
			.map(df => df.fieldname)
	...
	prepare_datum(d) {
		...
		if (d.is_folder) {
			...
			type = "folder";
		} else if (vmraid.utils.is_image_file(d.file_name)) {
			icon_class = "image";
			type = "image";
		} else {
			icon_class = "file";
	...
	before_render() {
		...
		vmraid.model.user_settings.save(
			"File",
			"grid_view",
			vmraid.views.FileView.grid_view
		);
	...
	get_breadcrumbs_html() {
		const route = vmraid.router.parse();
	...
	get_header_html() {
		...
		let header_selector_html = !vmraid.views.FileView.grid_view
			? `<input class="level-item list-check-all hidden-xs" type="checkbox" title="${__(
				"Select All"
			)}">`
			: "";

		let header_columns_html = !vmraid.views.FileView.grid_view
	...
	get_creation_date(file) {
		...
		let created_on;
		if (date === vmraid.datetime.now_date()) {
			created_on = comment_when(file.creation);
		} else {
			created_on = vmraid.datetime.str_to_user(date);
		}
		return created_on;
	}
	...
	get_left_html(file) {
		...
		const file_size = file.file_size
			? vmraid.form.formatters.FileSize(file.file_size)
			: "";
		...
		this.$result.on("drop", e => {
			...
			if (dataTransfer.files && dataTransfer.files.length > 0) {
				...
				if ($el.parents(".folders").length !== 0) {
					...
					vmraid.file_manager.paste(folder_name);
					vmraid.show_alert(
	...
	toggle_cut_paste_buttons() {
		const hide_paste_btn =
			!vmraid.file_manager.can_paste ||
			vmraid.file_manager.old_folder === this.current_folder;
...
vmraid.views.FileView.grid_view =
	vmraid.get_user_settings("File").grid_view || false;
