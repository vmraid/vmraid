export default class HeaderSize {
	...
	render() {
		...
		this.button.innerHTML = `${vmraid.utils.icon('header', 'sm')}${vmraid.utils.icon('small-down', 'xs')}`;
	...
	checkState(selection) {
		...
		for (const h of ['h1', 'h2', 'h3', 'h4', 'h5', 'h6']) {
			if (termWrapper && termWrapper.classList.contains(h)) {
				let num =  h.match(/\d+/)[0];
				$('.header-inline-tool svg:first-child').replaceWith(vmraid.utils.icon(`header-${num}`, 'md'));
			}
		}
