...
vmraid.views.Workspace = class Workspace {
	...
	get_page_to_show() {
		...
		} else if (vmraid.boot.allowed_workspaces) {
			default_page = vmraid.boot.allowed_workspaces[0].name;
		} else {
			default_page = "Build";
		}

		let page = vmraid.get_route()[1] || default_page;
		return page;
	}
	...
	build_sidebar_section(title, items) {
		...
		const get_sidebar_item = function (item) {
			return $(`
				<a
					href="/app/${vmraid.router.slug(item.name)}"
					class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}"
				>
					<span>${vmraid.utils.icon(item.icon || "folder-normal", "md")}</span>
...
class DesktopPage {
	...
	make() {
		...
		this.page.append(vmraid.render_template('workspace_loading_skeleton'));
		this.page.appendTo(this.container);

		this.get_data().then(() => {
			if (Object.keys(this.data).length == 0) {
				delete localStorage.current_workspace;
				vmraid.set_route("workspace");
				return;
			}
			this.refresh();
	...
	get_data() {
		return vmraid.xcall("vmraid.desk.desktop.get_desktop_page", {
			page: this.page_name
		}).then(data => {
			this.data = data;
	...
	save_customization() {
		...
		vmraid.call('vmraid.desk.desktop.save_customization', {
			page: this.page_name,
			config: config
		}).then(res => {
			vmraid.dom.unfreeze();
			if (res.message) {
				vmraid.show_alert({ message: __("Customizations Saved Successfully"), indicator: "green" });
				this.reload();
			} else {
				vmraid.throw({ message: __("Something went wrong while saving customizations"), indicator: "red" });
				this.reload();
			}
		});
	}
	...
	reset_customization() {
		vmraid.call('vmraid.desk.desktop.reset_customization', {
			page: this.page_name
		}).then(() => {
			vmraid.show_alert({ message: __("Removed page customizations"), indicator: "green" });
			this.reload();
		});
	}

	make_onboarding() {
		this.onboarding_widget = vmraid.widget.make_widget({
	...
	make_charts() {
		this.sections["charts"] = new vmraid.widget.WidgetGroup({
			container: this.page,
			type: "chart",
			columns: 1,
	...
	make_shortcuts() {
		this.sections["shortcuts"] = new vmraid.widget.WidgetGroup({
	...
	make_cards() {
		let cards = new vmraid.widget.WidgetGroup({
