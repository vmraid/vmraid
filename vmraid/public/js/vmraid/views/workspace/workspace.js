...
vmraid.standard_pages['Workspaces'] = function() {
	var wrapper = vmraid.container.add_page('Workspaces');
	...
	vmraid.workspace = new vmraid.views.Workspace(wrapper);
	$(wrapper).bind('show', function () {
		vmraid.workspace.show();
	});
};

vmraid.views.Workspace = class Workspace {
	constructor(wrapper) {
		this.wrapper = $(wrapper);
		this.page = wrapper.page;
		this.blocks = vmraid.workspace_block.blocks;
	...
	async setup_pages(reload) {
		...
		if (this.all_pages) {
			...
			for (let page of this.all_pages) {
				vmraid.workspaces[vmraid.router.slug(page.name)] = {title: page.title};
			}
			this.make_sidebar();
			reload && this.show();
		}
	...
	get_pages() {
		return vmraid.xcall("vmraid.desk.desktop.get_workspace_sidebar_items");
	}
		...
		return $(`
			<div class="sidebar-item-container ${item.is_editable ? "is-draggable" : ""}" item-parent="${item.parent_page}" item-name="${item.title}" item-public="${item.public || 0}">
				<div class="desk-sidebar-item standard-sidebar-item ${item.selected ? "selected" : ""}">
					<a
						href="/app/${item.public ? vmraid.router.slug(item.title) : 'private/'+vmraid.router.slug(item.title) }"
						class="item-anchor ${item.is_editable ? "" : "block-click" }" title="${__(item.title)}"
					>
						<span class="sidebar-item-icon" item-icon=${item.icon || "folder-normal"}>${vmraid.utils.icon(item.icon || "folder-normal", "md")}</span>
	...
	make_sidebar() {
		...
		this.sidebar.find('.selected').length && !vmraid.dom.is_element_in_viewport(this.sidebar.find('.selected'))
	...
	build_sidebar_section(title, root_pages) {
		...
		let $title = $(`<div class="standard-sidebar-label">
			<span>${vmraid.utils.icon("small-down", "xs")}</span>
	...
	append_item(item, container) {
		let is_current_page = vmraid.router.slug(item.title) == vmraid.router.slug(this.get_page_to_show().name)
	...
	add_drop_icon(item, sidebar_control, item_container) {
		...
		let $drop_icon = $(`<span class="drop-icon hidden">${vmraid.utils.icon(drop_icon, "sm")}</span>`)
	...
	get_data(page) {
		return vmraid.call("vmraid.desk.desktop.get_desktop_page", {
			page: page
		}).then(data => {
			...
			return vmraid.dashboard_utils.get_dashboard_settings().then(settings => {
				if (settings) {
	...
	get_page_to_show() {
		...
		let page = (vmraid.get_route()[1] == 'private' ? vmraid.get_route()[2] : vmraid.get_route()[1]) || default_page.name;
		let is_public = vmraid.get_route()[1] ? vmraid.get_route()[1] != 'private' : default_page.public;
	...
	async show_page(page) {
		...
		if (this.all_pages) {
			...
			if (this.pages && this.pages[current_page.name]) {
				...
				await vmraid.after_ajax(() => this.get_data(current_page));
			}
	...
	setup_customization_buttons(page) {
		...
		this.page.set_secondary_action(
			...
			async () => {
				...
				this.reload();
				vmraid.show_alert({ message: __("Customizations Discarded"), indicator: "info" });
			}
		);

		page.name && this.page.add_inner_button(__("Settings"), () => {
			vmraid.set_route(`workspace/${page.name}`);
		});
	}
	...
	add_sidebar_actions(item, sidebar_control, is_new) {
		if (!item.is_editable) {
			sidebar_control.parent().click(() => {
				!this.is_read_only && vmraid.show_alert({
					message: __("Only Workspace Manager can sort or edit this page"),
					indicator: 'info'
				}, 5);
			});

			vmraid.utils.add_custom_button(
				vmraid.utils.icon('duplicate', 'sm'),
	...
	edit_page(item) {
		...
		const d = new vmraid.ui.Dialog({
			...
			primary_action: (values) => {
				...
				vmraid.call({
					method: "vmraid.desk.doctype.workspace.workspace.update_page",
					args: {
						name: old_item.name,
						title: values.title,
						icon: values.icon || '',
					...
					callback: function(res) {
						if (res.message) {
							...
							vmraid.show_alert({ message: __(message), indicator: "green" });
						}
					}
				});
				...
				if (this.make_page_selected) {
					...
					let route = pre_url + vmraid.router.slug(values.title);
	...
	update_cached_values(old_item, new_item, duplicate, new_page) {
		...
		// update vmraid.workspaces
		if (vmraid.workspaces[vmraid.router.slug(old_item.name)] || new_page) {
			!duplicate && delete vmraid.workspaces[vmraid.router.slug(old_item.name)];
			if (new_item) {
				vmraid.workspaces[vmraid.router.slug(new_item.name)] = {'title': new_item.title};
			}
		}
	...
	add_settings_button(item, sidebar_control) {
		this.dropdown_list = [
			{
				...
				icon: vmraid.utils.icon('edit', 'sm'),
				action: () => this.edit_page(item)
			},
			{
				label: 'Duplicate',
				title: 'Duplicate Workspace',
				icon: vmraid.utils.icon('duplicate', 'sm'),
				action: () => this.duplicate_page(item)
			},
			{
				label: 'Delete',
				title: 'Delete Workspace',
				icon: vmraid.utils.icon('delete-active', 'sm'),
		...
		let $button = $(`
			<div class="btn btn-secondary btn-xs setting-btn dropdown-btn" title="${__('Setting')}">
				${vmraid.utils.icon('dot-horizontal', 'xs')}
			</div>
	...
	delete_page(page) {
		vmraid.confirm(__("Are you sure you want to delete page {0}?", [page.title]), () => {
			vmraid.call({
				method: "vmraid.desk.doctype.workspace.workspace.delete_page",
				args: { page: page },
				callback: function(res) {
					if (res.message) {
	...
	duplicate_page(page) {
		...
		const d = new vmraid.ui.Dialog({
			...
			primary_action: (values) => {
				...
				d.hide();
				vmraid.call({
					method: "vmraid.desk.doctype.workspace.workspace.duplicate_page",
					args: {
						page_name: page.name,
						new_page: values
					},
				...
				new_page.name = values.title + (new_page.public ? '' : '-' + vmraid.session.user);
				new_page.label = new_page.name;
				new_page.icon = values.icon;
				new_page.parent_page = values.parent || '';
				new_page.for_user = new_page.public ? '' : vmraid.session.user;
	...
	update_sorted_sidebar() {
		if (this.sorted_public_items || this.sorted_private_items) {
			vmraid.call({
				method: "vmraid.desk.doctype.workspace.workspace.sort_pages",
				args: {
	...
	initialize_new_page() {
		...
		const d = new vmraid.ui.Dialog({
			...
			primary_action: (values) => {
				...
				let name = values.title + (values.is_public ? '' : '-' + vmraid.session.user);
				let blocks = [{
					type: "header",
				...
				let new_page = {
					...
					title: values.title,
					public: values.is_public || 0,
					for_user: values.is_public ? '' : vmraid.session.user,
					icon: values.icon,
				...
				this.editor.render({
					...
					vmraid.call({
						method: "vmraid.desk.doctype.workspace.workspace.new_page",
						args: {
							new_page: new_page
						},
						callback: function(res) {
					...
					let route = pre_url + vmraid.router.slug(new_page.title);
					vmraid.set_route(route);
	...
	validate_page(new_page, old_page) {
		...
		if (vmraid.router.doctype_route_exist(vmraid.router.slug(new_page.title))) {
	...
	initialize_editorjs(blocks) {
		this.tools = {
			...
			HeaderSize: vmraid.workspace_block.tunes['header_size'],
		};
	...
	save_page(page) {
		...
		return this.editor.save().then((outputData) => {
			...
			if (page.content == JSON.stringify(blocks) && Object.keys(new_widgets).length === 0) {
				...
				vmraid.show_alert({ message: __("No changes made on the page"), indicator: "warning" });
				return false;
			}
			...
			vmraid.call({
				method: "vmraid.desk.doctype.workspace.workspace.save_page",
				args: {
					title: page.title,
				...
				callback: function(res) {
					if (res.message) {
						...
						me.reload();
						vmraid.show_alert({ message: __("Page Saved Successfully"), indicator: "green" });
					}
				}
			});
			return true;
	...
	create_page_skeleton() {
		...
		$('.layout-main-section').prepend(vmraid.render_template('workspace_loading_skeleton'));
	...
	create_sidebar_skeleton() {
		...
		$('.list-sidebar').prepend(vmraid.render_template('workspace_sidebar_loading_skeleton'));
