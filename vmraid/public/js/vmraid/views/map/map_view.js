/**
 * vmraid.views.MapView
 */
vmraid.provide('vmraid.utils.utils');
vmraid.provide("vmraid.views");

vmraid.views.MapView = class MapView extends vmraid.views.ListView {
	get view_name() {
		return 'Map';
	}
	...
	render_map_view() {
		...
		this.map = L.map(this.map_id).setView(vmraid.utils.map_defaults.center,
			vmraid.utils.map_defaults.zoom);
	...
	get_coords() {
		let get_coords_method = this.settings && this.settings.get_coords_method || 'vmraid.geo.utils.get_coords';
