...
vmraid.views.DashboardView = class DashboardView extends vmraid.views.ListView {
	get view_name() {
		return 'Dashboard';
	}
	...
	setup_defaults() {
		return super.setup_defaults()
			.then(() => {
				...
				this.dashboard_settings = vmraid.get_user_settings(this.doctype)['dashboard_settings'] || null;
			});
	}
	...
	render_empty_state() {
		...
		const empty_state_image = '/assets/vmraid/images/ui-states/list-empty-state.svg';
	...
	save_dashboard_customization() {
		...
		vmraid.model.user_settings.save(this.doctype, 'dashboard_settings', this.dashboard_settings);
		this.make_dashboard();
	}
	...
	reset_dashboard_customization() {
		vmraid.confirm(__("Are you sure you want to reset all customizations?"), () => {
		...
		const dialog = new vmraid.ui.Dialog({
			...
			fields: [
				...
				},
				{
					...
					get_query: () => {
						return {
							'query': 'vmraid.desk.doctype.dashboard_chart.dashboard_chart.get_charts_for_user',
							filters: {
								document_type: this.doctype,
							}
						};
			...
			primary_action: (values) => {
				...
				if (chart.new_or_existing == 'New Chart') {
					...
					chart.filters_json = '[]';
					vmraid.xcall('vmraid.desk.doctype.dashboard_chart.dashboard_chart.create_dashboard_chart', {'args': chart}).then((doc)=> {
	...
	get_field_options() {
		...
		vmraid.get_meta(this.doctype).fields.map(df => {
