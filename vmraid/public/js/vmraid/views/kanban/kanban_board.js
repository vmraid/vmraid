...
(function() {
	...
	var method_prefix = 'vmraid.desk.doctype.kanban_board.kanban_board.';
	...
	var store = fluxify.createStore({
		...
		actionCallbacks: {
			...
			add_column: function(updater, col) {
				if(vmraid.model.can_create('Custom Field')) {
			...
			add_card: function(updater, card_title, column_title) {
				...
				if (field && !quick_entry) {
					...
					vmraid.new_doc(this.doctype, doc);
				}
			},
	...
	vmraid.views.KanbanBoard = function(opts) {
		...
		function prepare() {
			...
			if(self.$kanban_board.length === 0) {
				self.$kanban_board = $(vmraid.render_template("kanban_board"));
		...
		function make_columns() {
			...
			columns.filter(is_active_column).map(function(col) {
				vmraid.views.KanbanBoardColumn(col, self.$kanban_board);
			});
		}
	...
	vmraid.views.KanbanBoardColumn = function(column, wrapper) {
		var self = {};
		var filtered_cards = [];
		...
		function make_dom() {
			self.$kanban_column = $(vmraid.render_template(
				'kanban_column', {
					title: column.title,
					doctype: store.getState().doctype,
					indicator: vmraid.scrub(column.indicator, '-')
				})).appendTo(wrapper);
			...
			if(order) {
				...
				filtered_cards.forEach(function(card) {
					if(order.indexOf(card.name) === -1) {
						vmraid.views.KanbanBoardCard(card, self.$kanban_cards);
					}
				});
				order.forEach(function(name) {
					if (!filtered_cards_names.includes(name)) return;
					vmraid.views.KanbanBoardCard(get_card(name), self.$kanban_cards);
				});
			} else {
			...
			get_column_indicators(function(indicators) {
				let html = `<li class="button-group">${
					indicators.map(indicator => {
						let classname = vmraid.scrub(indicator, '-');
	...
	vmraid.views.KanbanBoardCard = function(card, wrapper) {
		var self = {};
		...
		function make_dom() {
			var opts = {
				name: card.name,
				title: vmraid.utils.html2text(card.title),
				disable_click: card._disable_click ? 'disable-click' : '',
				creation: card.creation,
				doc_content: get_doc_content(card),
				image_url: cur_list.get_image_url(card),
				form_link: vmraid.utils.get_form_link(card.doctype, card.name)
			};

			self.$card = $(vmraid.render_template('kanban_card', opts))
				.appendTo(wrapper);
		}
		...
		function get_doc_content(card) {
			...
			for (let field_name of cur_list.board.fields) {
				let field = (
					vmraid.meta.get_docfield(card.doctype, field_name, card.name)
					|| vmraid.model.get_std_field(field_name)
				);
				let label = cur_list.board.show_labels ? `<span>${__(field.label)}: </span>` : '';
				let value = vmraid.format(card.doc[field_name], field);
				fields.push(`
		...
		function render_card_meta() {
			...
			if (card.color && vmraid.ui.color.validate_hex(card.color)) {
				const $div = $('<div>');
				$('<div></div>').css({
					width: '30px',
					height: '4px',
		...
		function get_assignees_group() {
			return vmraid.avatar_group(card.assigned_list, 3, {
		...
		function show_assign_to_dialog(e) {
			e.preventDefault();
			e.stopPropagation();
			self.assign_to = new vmraid.ui.form.AssignToDialog({
				obj: self,
				method: 'vmraid.desk.form.assign_to.add',
	...
	function fetch_customization(doctype) {
		return new Promise(function(resolve) {
			vmraid.model.with_doc("Customize Form", "Customize Form", function() {
				var doc = vmraid.get_doc("Customize Form");
	...
	function insert_doc(doc) {
		return vmraid.call({
			method: "vmraid.client.insert",
			args: {
				doc: doc
			},
			callback: function() {
				vmraid.model.clear_doc(doc.doctype, doc.name);
				vmraid.show_alert({ message: __("Saved"), indicator: 'green' }, 1);
			}
		});
	}
	...
	function get_column_indicators(callback) {
		vmraid.model.with_doctype('Kanban Board Column', function() {
			var meta = vmraid.get_meta('Kanban Board Column');
			var indicators;
