/**
 * vmraid.views.ImageView
 */
vmraid.provide("vmraid.views");

vmraid.views.ImageView = class ImageView extends vmraid.views.ListView {
	get view_name() {
		return "Image";
	}
	...
	item_html(item) {
		...
		const escaped_title = vmraid.utils.escape_html(title);
	...
	get_attached_images() {
		return vmraid
			.call({
				method: "vmraid.core.doctype.file.file.get_attached_images",
				args: {
	...
	setup_gallery() {
		var me = this;
		this.gallery = new vmraid.views.GalleryView({
			doctype: this.doctype,
			items: this.items,
			wrapper: this.$result,
...
vmraid.views.GalleryView = class GalleryView {
	constructor(opts) {
		$.extend(this, opts);
		var me = this;
	...
	prepare() {
		...
		if (this.pswp_root.length === 0) {
			var pswp = vmraid.render_template("photoswipe_dom");
	...
	load_lib() {
		return new Promise(resolve => {
			var asset_dir = "assets/vmraid/js/lib/photoswipe/";
			vmraid.require(
				[
