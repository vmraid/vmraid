...
vmraid.breadcrumbs = {
	...
	add(module, doctype, type) {
		...
		this.all[vmraid.breadcrumbs.current_page()] = obj;
		this.update();
	},

	current_page() {
		return vmraid.get_route_str();
	},

	update() {
		var breadcrumbs = this.all[vmraid.breadcrumbs.current_page()];
	...
	set_workspace_breadcrumb(breadcrumbs) {
		...
		if (breadcrumbs.workspace) {
			if (!breadcrumbs.module_info.blocked && vmraid.visible_modules.includes(breadcrumbs.module_info.module)) {
				$(`<li><a href="/app/${vmraid.router.slug(breadcrumbs.workspace)}">${__(breadcrumbs.workspace)}</a></li>`)
	...
	set_workspace(breadcrumbs) {
		...
		if (breadcrumbs.module) {
			...
			breadcrumbs.module_info = vmraid.get_module(breadcrumbs.module);

			// set workspace
			if (breadcrumbs.module_info && vmraid.boot.module_page_map[breadcrumbs.module]) {
				breadcrumbs.workspace = vmraid.boot.module_page_map[breadcrumbs.module];
			}
		}
	},
	...
	set_list_breadcrumb(breadcrumbs) {
		...
		const doctype_meta = vmraid.get_doc('DocType', doctype);
		if ((doctype==="User" && !vmraid.user.has_role('System Manager'))
			|| (doctype_meta && doctype_meta.issingle)) {
			// no user listview for non-system managers and single doctypes
		} else {
			let route;
			const doctype_route = vmraid.router.slug(vmraid.router.doctype_layout || doctype);
			if (vmraid.boot.treeviews.indexOf(doctype) !== -1) {
				let view = vmraid.model.user_settings[doctype].last_view || 'Tree';
	...
	set_form_breadcrumb(breadcrumbs, view) {
		const doctype = breadcrumbs.doctype;
		const docname = vmraid.get_route()[2];
		let form_route = `/app/${vmraid.router.slug(doctype)}/${docname}`;
		...
		if (view === "form") {
			...
			last_crumb.click((event) => {
				...
				vmraid.utils.copy_to_clipboard(last_crumb.text());
			});
		}
	...
	set_dashboard_breadcrumb(breadcrumbs) {
		const doctype = breadcrumbs.doctype;
		const docname = vmraid.get_route()[1];
		let dashboard_route = `/app/${vmraid.router.slug(doctype)}/${docname}`;
		$(`<li><a href="${dashboard_route}">${__(docname)}</a></li>`)
			.appendTo(this.$breadcrumbs);
	},

	setup_modules() {
		if (!vmraid.visible_modules) {
			vmraid.visible_modules = $.map(vmraid.boot.allowed_workspaces, (m) => {
				return m.module;
			});
		}
	},
	...
	rename(doctype, old_name, new_name) {
		...
		delete vmraid.breadcrumbs.all[old_route_str];
		this.update();
	},
