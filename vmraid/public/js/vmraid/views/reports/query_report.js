...
vmraid.provide('vmraid.query_reports');

vmraid.standard_pages['query-report'] = function() {
	var wrapper = vmraid.container.add_page('query-report');
	...
	vmraid.query_report = new vmraid.views.QueryReport({
		parent: wrapper,
	});

	$(wrapper).bind('show', function() {
		vmraid.query_report.show();
	});
};

vmraid.views.QueryReport = class QueryReport extends vmraid.views.BaseList {
	show() {
	...
	init() {
		...
		this.init_promise = vmraid.run_serially(tasks);
		return this.init_promise;
	}
	...
	setup_defaults() {
		...
		this.refresh = vmraid.utils.throttle(this.refresh, 300);
	...
	get_no_result_message() {
		return `<div class="msg-box no-border">
			<div>
				<img src="/assets/vmraid/images/ui-states/list-empty-state.svg" alt="Generic Empty State" class="null-state">
			</div>
	...
	setup_events() {
		vmraid.realtime.on("report_generated", (data) => {
			...
			if(data.report_name) {
				...
				if(data.name == this.prepared_report_doc_name) {
					...
					vmraid.show_alert({message: alert_message, indicator: 'orange'});
				}
			}
		});
	...
	load() {
		if (vmraid.get_route().length < 2) {
			this.toggle_nothing_to_show(true);
			return;
		}

		let route_options = {};
		route_options = Object.assign(route_options, vmraid.route_options);

		if (this.report_name !== vmraid.get_route()[1]) {
			// different report
			this.load_report(route_options);
		}
		else if (vmraid.has_route_options()) {
	...
	add_card_to_dashboard() {
		let field_options = vmraid.report_utils.get_field_options_from_report(this.columns, this.raw_data);
		const dashboard_field = vmraid.dashboard_utils.get_dashboard_link_field();
		const set_standard = vmraid.boot.developer_mode;
	...
	add_chart_to_dashboard() {
		if (this.chart_fields || this.chart_options) {
			...
			vmraid.msgprint(__('Please Set Chart'));
		}
	}
	...
	create_number_card(values, dashboard_name, card_name, set_standard) {
		...
		this.add_to_dashboard(
			'vmraid.desk.doctype.number_card.number_card.create_report_number_card',
			args,
			dashboard_name,
			card_name,
			'Number Card'
	...
	create_dashboard_chart(chart_args, dashboard_name, chart_name, set_standard) {
		let args = {
			...
			'chart_type': 'Report',
			'report_name': this.report_name,
			'type': chart_args.chart_type || vmraid.model.unscrub(chart_args.type),
		...
		this.add_to_dashboard(
			'vmraid.desk.doctype.dashboard_chart.dashboard_chart.create_report_chart',
			args,
			dashboard_name,
			chart_name,
			'Dashboard Chart'
	...
	add_to_dashboard(method, args, dashboard_name, name, doctype) {
		vmraid.xcall(
			...
			vmraid.msgprint(message, __("New {0} Created", [__(doctype)]));
		});
	}
	...
	get_report_doc() {
		return vmraid.model.with_doc('Report', this.report_name)
			.then(doc => {
				this.report_doc = doc;
			})
			.then(() => vmraid.model.with_doctype(this.report_doc.ref_doctype));
	}

	get_report_settings() {
		return new Promise((resolve, reject) => {
			if (vmraid.query_reports[this.report_name]) {
				this.report_settings = vmraid.query_reports[this.report_name];
				resolve();
			} else {
				vmraid.xcall('vmraid.desk.query_report.get_script', {
					report_name: this.report_name
				}).then(settings => {
					vmraid.dom.eval(settings.script || '');
					vmraid.after_ajax(() => {
						...
						vmraid.query_reports[this.report_name] = this.report_settings;
	...
	get_local_report_settings() {
		let report_script_name = this.report_doc.report_type === 'Custom Report'
			...
			: this.report_name;
		return vmraid.query_reports[report_script_name] || {};
	}
	...
	setup_progress_bar() {
		...
		this.interval = setInterval(function()  {
			seconds_elapsed += 1;
			vmraid.show_progress(__('Preparing Report'), seconds_elapsed, execution_time);
		}, 1000);
	}
	...
	evaluate_depends_on_value(expression, filter_label) {
		...
		if (doc) {
			if (typeof expression === 'boolean') {
				...
				try {
					out = vmraid.utils.eval(expression.substr(5), { doc });
				} catch (e) {
					vmraid.throw(__('Invalid "depends_on" expression set in filter {0}', [filter_label]));
				}
			} else {
	...
	set_route_filters(route_options) {
		if (!route_options) route_options = vmraid.route_options;
		...
		if (route_options) {
			...
			return vmraid.run_serially(promises);
		}
	}
	...
	refresh() {
		...
		return new Promise(resolve => {
			this.last_ajax = vmraid.call({
				method: 'vmraid.desk.query_report.run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters,
			...
			if (data.result && data.result.length) {
				...
				if (this.chart_options) {
					...
					if (this.chart_fields) {
						this.chart_options =
							vmraid.report_utils.make_chart_options(
								this.columns,
								this.raw_data,
								this.chart_fields
							);
	...
	render_summary(data) {
		data.forEach((summary) => {
			vmraid.utils.build_summary_item(summary).appendTo(this.$summary);
		})
	...
	get_query_params() {
		const query_string = vmraid.utils.get_query_string(vmraid.get_route_str());
		const query_params = vmraid.utils.get_query_params(query_string);
		return query_params;
	}
	...
	add_prepared_report_buttons(doc) {
			...
			this.page.add_inner_button(__("Download Report"), function (){
				window.open(
					vmraid.urllib.get_full_url(
						"/api/method/vmraid.core.doctype.prepared_report.prepared_report.download_attachment?"
						+"dn="+encodeURIComponent(doc.name)));
			});

			const part1 = __('This report was generated {0}.', [vmraid.datetime.comment_when(doc.report_end_time)]);
		...
		this.primary_action_map = {
			...
			"Edit": {
				label: __("Edit"),
				click: () => {
					vmraid.set_route(vmraid.get_route());
				}
			},
			"Rebuild": {
				label:	__("Rebuild"),
	...
	show_warning_or_generate_report() {
		vmraid.xcall(
			'vmraid.core.doctype.prepared_report.prepared_report.get_reports_in_queued_state',
			{
			...
			if (reports.length) {
				...
				this.prepared_report_dialog = vmraid.warn(
				...
				vmraid.utils.bind_actions_with_object(this.prepared_report_dialog.wrapper, this);
			} else {
				this.generate_background_report();
			}
		});
	...
	delete_old_queued_reports() {
		...
		vmraid.xcall(
			'vmraid.core.doctype.prepared_report.prepared_report.delete_prepared_reports',
			{
	...
	generate_background_report() {
		...
		if (!missing_mandatory.length) {
			let filters = this.get_filter_values(true);
			return new Promise(resolve => vmraid.call({
				method: 'vmraid.desk.query_report.background_enqueue_run',
				type: 'GET',
				args: {
					report_name: this.report_name,
					filters: filters
				...
				vmraid.show_alert({message: alert_message, indicator: 'orange'}, 10);
				this.toggle_nothing_to_show(true);
			});
		}
	}
	...
	render_datatable() {
		...
		if (this.datatable && this.datatable.options
			...
			let datatable_options = {
				...
				data: data,
				inlineFilters: true,
				language: vmraid.boot.lang,
				translations: vmraid.utils.datatable.get_translations(),
				treeView: this.tree_report,
				layout: 'fixed',
				cellHeight: 33,
				showTotalRow: this.raw_data.add_total_row && !this.report_settings.tree,
				direction: vmraid.utils.is_rtl() ? 'rtl' : 'ltr',
				hooks: {
					columnTotal: vmraid.utils.report_column_total
				}
			};
	...
	get_chart_options(data) {
		...
		if (options.fieldtype) {
			options.tooltipOptions = {
				formatTooltipY: d => vmraid.format(d, {
	...
	render_chart(options) {
		this.$chart.empty();
		this.$chart.show();
		this.chart = new vmraid.Chart(this.$chart[0], options);
	}
	...
	open_create_chart_dialog() {
		...
		function preview_chart() {
			...
			if (values.x_field && values.y_fields.length) {
				let options = vmraid.report_utils.make_chart_options(me.columns, me.raw_data, values);
				me.chart_fields = values;
				wrapper.empty();
				new vmraid.Chart(wrapper[0], options);
	...
	prepare_columns(columns) {
		return columns.map(column => {
			column = vmraid.report_utils.prepare_field_from_column(column);
			...
			const format_cell = (value, row, column, data) => {
				...
				return vmraid.format(value, column,
			...
			if (column.fieldtype === 'Date') {
				compareFn = (cell, keyword) => {
					...
					const keywordValue = vmraid.datetime.user_to_obj(keyword);
					const cellValue = vmraid.datetime.str_to_obj(cell.content);
	...
	set_breadcrumbs() {
		...
		const ref_doctype = vmraid.get_meta(this.report_doc.ref_doctype);
		vmraid.breadcrumbs.add(ref_doctype.module);
	}

	make_access_log (method, file_format) {
		vmraid.call("vmraid.core.doctype.access_log.access_log.make_access_log",
			{
	...
	pdf_report(print_settings) {
		const base_url = vmraid.urllib.get_base_url();
		const print_css = vmraid.boot.print_css;
		const landscape = print_settings.orientation == 'Landscape';
		...
		const content = vmraid.render_template(template, {
			title: __(this.report_name),
			subtitle: filters_html,
		...
		const html = vmraid.render_template('print_template', {
			title: __(this.report_name),
			content: content,
			base_url: base_url,
			print_css: print_css,
			...
			landscape: landscape,
			columns: columns,
			lang: vmraid.boot.lang,
			layout_direction: vmraid.utils.is_rtl() ? "rtl" : "ltr"
		});

		vmraid.render_pdf(html, print_settings);
	}
	...
	get_filters_html_for_print() {
		...
		return Object.keys(applied_filters)
			.map(fieldname => {
				const label = vmraid.query_report.get_filter(fieldname).df.label;
	...
	export_report() {
		...
		this.export_dialog = vmraid.prompt(export_dialog_fields, ({ file_format, include_indentation }) => {
			...
			if (file_format === 'CSV') {
				...
				vmraid.tools.downloadify(out, null, this.report_name);
			} else {
				let filters = this.get_filter_values(true);
				if (vmraid.urllib.get_dict("prepared_report_name")) {
					filters = Object.assign(vmraid.urllib.get_dict("prepared_report_name"), filters);
				}
				...
				const args = {
					cmd: 'vmraid.desk.query_report.export_query',
					report_name: this.report_name,
			...
			return row
				...
				.map((cell, i) => {
					if (cell.column.fieldtype === "Duration") {
						cell.content = vmraid.utils.get_formatted_duration(cell.content);
					}
	...
	get_menu_items() {
		let items = [
			...
			{
				label: __('Edit'),
				action: () => vmraid.set_route('Form', 'Report', this.report_name),
				condition: () => vmraid.user.is_report_manager(),
				standard: true
			},
			{
				label: __('Print'),
				action: () => {
					let dialog = vmraid.ui.get_print_settings(
						false,
				...
				condition: () => vmraid.model.can_print(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('PDF'),
			...
			},
			{
				...
				condition: () => vmraid.model.can_export(this.report_doc.ref_doctype),
				standard: true
			},
			{
				label: __('Setup Auto Email'),
				action: () => vmraid.set_route('List', 'Auto Email Report', {'report' : this.report_name}),
				standard: true
			},
			{
				label: __('Add Column'),
				action: () => {
					let d = new vmraid.ui.Dialog({
						...
						fields: [
							{
								...
								change: () => {
									...
									vmraid.model.with_doctype(doctype, () => {
										let options = vmraid.meta.get_docfields(doctype)
											.filter(vmraid.model.is_value_type)
						...
						primary_action: (values) => {
							...
							let df = vmraid.meta.get_docfield(values.doctype, values.field);
							...
							vmraid.call({
								method: 'vmraid.desk.query_report.get_data_for_custom_field',
								args: {
									field: values.field,
									doctype: values.doctype
								},
			...
			},
			{
				...
				action: () => vmraid.set_route('List', 'User Permission', {
					doctype: 'Report',
					name: this.report_name
				}),
				condition: () => vmraid.model.can_set_user_permissions('Report'),
				standard: true
			}
		];

		if (vmraid.user.is_report_manager()) {
			items.push({
				label: __('Save'),
				action: () => {
					let d = new vmraid.ui.Dialog({
						...
						primary_action: (values) => {
							vmraid.call({
								method: "vmraid.desk.query_report.save_report",
								args: {
								...
								callback: function(r) {
									this.show_save = false;
									d.hide();
									vmraid.set_route('query-report', r.message);
								}
							});
						}
					});
...
Object.defineProperty(vmraid, 'query_report_filters_by_name', {
	get() {
		console.warn('[Query Report] vmraid.query_report_filters_by_name is deprecated. Please use the new api: vmraid.query_report.get_filter_value(fieldname) and vmraid.query_report.set_filter_value(fieldname, value)');
		return null;
	}
});
