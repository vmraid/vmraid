...
vmraid.views.ReportView = class ReportView extends vmraid.views.ListView {
	...
	is_editable(df, data) {
		if (df
			...
			&& !vmraid.model.std_fields_list.includes(df.fieldname))
			return true;
		return false;
	}
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		if (df && df.fieldtype === 'Currency' && df.options &&
			...
			vmraid.show_alert(__('Also adding the dependent currency field {0}', [field[0].bold()]));
		}
	}
	...
	add_status_dependency_column(col, doctype) {
		...
		if (!this.fields.find(f => f[0] === col)) {
			...
			this.fields.push(field);
			this.refresh();
			vmraid.show_alert(__('Also adding the status dependency field {0}', [field[0].bold()]));
		}
	}
	...
	build_column(c) {
		...
		const editable = vmraid.model.is_non_std_field(fieldname) && !docfield.read_only;
