...
vmraid.views.ReportView = class ReportView extends vmraid.views.ListView {
	get view_name() {
		return 'Report';
	}
	...
	setup_sort_selector() {
		this.sort_selector = new vmraid.ui.SortSelector({
		...
		this.group_by_control = new vmraid.ui.GroupBy(this);
	...
	before_refresh() {
		if (this.report_doc) {
			// don't parse vmraid.route_options if this is a Custom Report
	...
	set_dirty_state_for_custom_report() {
		...
		if (!vmraid.utils.deep_equal(current_settings, report_settings)) {
	...
	prepare_data(r) {
		let data = r.message || {};
		data = vmraid.utils.dict(data.keys, data.values);
	...
	on_update(data) {
		if (this.doctype === data.doctype && data.name) {
			...
			const flash_row = data.user !== vmraid.session.user;
			if (this.data.find(d => d.name === data.name)) {
				// update existing
				vmraid.db.get_doc(data.doctype, data.name)
					...
					const d = new vmraid.ui.Dialog({
						...
						primary_action: ({ column, insert_before }) => {
							if (!columns_in_picker.map(col => col.value).includes(column)) {
								vmraid.show_alert({message: __('Invalid column'), indicator: 'orange'});
								d.hide();
								return;
							}
	...
	setup_charts() {
		...
		for (let col of this.columns) {
			...
			// numeric values in y
			if (col.docfield && vmraid.model.numeric_fieldtypes.includes(col.docfield.fieldtype)) {
				y_fields.push({
					label: col.content,
					fieldname: col.id,
					value: col.id
			...
			fields: [
				...
				},
				{
					...
					default: defaults.chart_type ?
						vmraid.utils.to_title_case(defaults.chart_type) :
						'Bar'
				}
			],
			primary_action: (data) => {
	...
	make_chart() {
		...
		this.chart = new vmraid.Chart(this.$charts_wrapper.find('.charts-inner-wrapper')[0], {
			...
			tooltipOptions: {
				formatTooltipY: value => vmraid.format(value, get_df(this.chart_args.y_axes[0]), { always_show_decimals: true, inline: true }, get_doc(value.doc))
			}
		});
	}
	...
	set_control_value(doctype, docname, fieldname, value) {
		...
		return new Promise((resolve, reject) => {
			vmraid.db.set_value(doctype, docname, {[fieldname]: value})
				.then(r => {
					if (r.message) {
						resolve(r.message);
					} else {
	...
	render_editing_input(colIndex, value, parent) {
		...
		if (col.docfield.fieldtype === 'Text Editor') {
			...
			// make control
			control = vmraid.ui.form.make_control({
				df: col.docfield,
				parent: parent,
				render_input: true
			});
	...
	is_editable(df, data) {
		if (df
			&& vmraid.model.can_write(this.doctype)
			...
			&& vmraid.model.is_non_std_field(df.fieldname))
			return true;
		return false;
	}
	...
	set_default_fields() {
		...
		const fields = this.meta.fields.filter(df => {
			return (df.in_list_view || df.in_standard_filter)
				&& vmraid.perm.has_perm(this.doctype, df.permlevel, 'read')
				&& vmraid.model.is_value_type(df.fieldtype)
				&& !df.report_hide;
		});
	...
	get_fields() {
		let fields = this.fields.map(f => {
			let column_name = vmraid.model.get_full_column_name(f[0], f[1]);
			if (f[1] !== this.doctype) {
				// child table field
		...
		const cdt_name_fields =
			this.get_unique_cdt_in_view()
				.map(cdt => vmraid.model.get_full_column_name('name', cdt) + ' as ' + `'${cdt}:name'`);
	...
	add_currency_column(fieldname, doctype, col_index) {
		...
		const df = vmraid.meta.get_docfield(doctype, fieldname);
		if (df && df.fieldtype === 'Currency' && df.options &&
			!df.options.includes(':') && vmraid.meta.has_field(doctype, df.options)
		) {
			...
			const field_label = vmraid.meta.get_label(doctype, field[0]);
			vmraid.show_alert(
	...
	remove_column_from_datatable(column) {
		...
		if (field[0] === 'name') {
			this.refresh();
			vmraid.throw(__('Cannot remove ID field'));
		}
	...
	get_columns_for_picker() {
		...
		const standard_fields_filter = df => !in_list(vmraid.model.no_value_type, df.fieldtype);

		let doctype_fields = vmraid.meta.get_docfields(this.doctype).filter(standard_fields_filter);

		// filter out docstatus field from picker
		let std_fields = vmraid.model.std_fields.filter( df => df.fieldname !== 'docstatus');

		// add status field derived from docstatus, if status is not a standard field
		let has_status_values = false;

		if (this.data) {
			has_status_values = vmraid.get_indicator(this.data[0], this.doctype);
		}

		if (!vmraid.meta.has_field(this.doctype, 'status') && has_status_values) {
			doctype_fields = [{
				label: __('Status'),
				fieldname: 'docstatus',
				fieldtype: 'Data'
		...
		const table_fields = vmraid.meta.get_table_fields(this.doctype);

		table_fields.forEach(df => {
			const cdt = df.options;
			const child_table_fields = vmraid.meta.get_docfields(cdt).filter(standard_fields_filter);
	...
	setup_columns() {
		...
		for (let f of this.fields) {
			...
			if (f[0]!=='docstatus') {
				...
				if (!this.fields.includes(['status', this.doctype]) && !vmraid.meta.has_field(this.doctype, 'status')) {
	...
	build_column(c) {
		...
		let docfield = vmraid.meta.docfield_map[doctype || this.doctype][fieldname];
		...
		if (!docfield) {
			docfield = vmraid.model.get_std_field(fieldname, true);
			...
			if (docfield) {
				...
				if (fieldname == 'docstatus' && !vmraid.meta.has_field(this.doctype, 'status')) {
		...
		const editable = vmraid.model.is_non_std_field(fieldname) && !docfield.read_only && !docfield.is_virtual;

		const align = (() => {
			const is_numeric = vmraid.model.is_numeric_field(docfield);
			if (is_numeric) {
				return 'right';
			}
		...
		return {
			...
			format: (value, row, column, data) => {
				...
				return vmraid.format(value, column.docfield, { always_show_decimals: true }, doc);
			}
		};
	}
		...
		if (this.add_totals_row) {
			...
			const totals_row = this.columns.map((col, i) => {
				return {
					...
					format: value => {
						let formatted_value = vmraid.format(value, col.docfield, {
	...
	format_total_cell(formatted_value, df) {
		...
		if (vmraid.model.is_numeric_field(df.docfield)) {
		...
		return this.columns.map(col => {
			if (col.docfield.parent !== this.doctype) {
				...
				return {
					...
					format: value => {
						return vmraid.format(value, col.docfield, { always_show_decimals: true }, d);
					}
				};
			}
			if (col.field === 'docstatus' && !vmraid.meta.has_field(this.doctype, 'status')) {
				// get status from docstatus
				let status = vmraid.get_indicator(d, this.doctype);
				if (status) {
					if (!status[0]) {
	...
	save_report(save_type) {
		const _save_report = (name) => {
			...
			return vmraid.call({
				method: 'vmraid.desk.reportview.save_report',
				args: {
					name: name,
					doctype: this.doctype,
				...
				callback:(r) => {
					if(r.exc) {
						vmraid.msgprint(__("Report was not saved (there were errors)"));
						return;
					}
					if(r.message != this.report_name) {
						// Rerender the reports dropdown,
						// so that this report is included in the dropdown as well.
						vmraid.boot.user.all_reports[r.message] = {
							ref_doctype: this.doctype,
							report_type: "Report Builder",
							title: r.message,
						};

						vmraid.set_route('List', this.doctype, 'Report', r.message);
						return;
					}
		...
		if(this.report_name && save_type == "save") {
			...
			vmraid.prompt({fieldname: 'name', label: __('New Report name'), reqd: 1, fieldtype: 'Data'}, (data) => {
	...
	delete_report() {
		return vmraid.call({
			method: 'vmraid.desk.reportview.delete_report',
	...
	get_report_doc() {
		return new Promise(resolve => {
			vmraid.model.with_doc('Report', this.report_name, () => {
				resolve(vmraid.get_doc('Report', this.report_name));
			});
		});
	}
	...
	get_filters_html_for_print() {
		...
		return filters.map(f => {
			...
			const label = vmraid.meta.get_label(doctype, fieldname);
	...
	get_columns_totals(data) {
		...
		this.columns.forEach((col, i) => {
			const totals = data.reduce((totals, d) => {
				if (col.id in d && vmraid.model.is_numeric_field(col.docfield)) {
					totals += flt(d[col.id]);
					return totals;
				}
			}, 0);
	...
	report_menu_items() {
		let items = [
			...
			},
			{
				...
				action: () => {
					...
					vmraid.utils.setup_search(d.$body, '.unit-checkbox', '.label-area');
					d.show();
				}
			}
		];

		if (vmraid.model.can_export(this.doctype)) {
			items.push({
				label: __('Export'),
				action: () => {
					...
					const d = new vmraid.ui.Dialog({
						...
						primary_action: (data) => {
							args.cmd = 'vmraid.desk.reportview.export_query';
		...
		items.push({
			...
			action: () => {
				if(this.report_name) {
					vmraid.set_route('List', 'Auto Email Report', {'report' : this.report_name});
				} else {
					vmraid.msgprint(__('Please save the report first'));
				}
			}
		});
		...
		const can_edit_or_delete = (action) => {
			...
			return (
				...
				&& (
					vmraid.model[method]("Report")
					|| this.report_doc.owner === vmraid.session.user
				)
			);
		};
		...
		// user permissions
		if(this.report_name && vmraid.model.can_set_user_permissions("Report")) {
			items.push({
				...
				action: () => {
					...
					vmraid.set_route('List', 'User Permission', args);
				}
			});
		}
