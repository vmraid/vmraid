vmraid.provide('vmraid.report_utils');
...
vmraid.report_utils = {
	make_chart_options: function(columns, raw_data, { y_fields, x_field, chart_type, colors, height }) {
		...
		let datasets = y_fields.map(y_field => ({
			name: vmraid.model.unscrub(y_field),
		...
		function get_options(fields) {
			return fields.map((field) => {
				if (field.fieldname) {
					...
					field = vmraid.report_utils.prepare_field_from_column(field);
					return {label: field.label, value: field.fieldname};
				}
			});
		}
	...
	get_report_filters: function(report_name) {
		...
		if (vmraid.query_reports[report_name]) {
			let filters = vmraid.query_reports[report_name].filters;
			return Promise.resolve(filters);
		}

		return vmraid.xcall(
			'vmraid.desk.query_report.get_script',
			{
				report_name: report_name
			}
		).then(r => {
			vmraid.dom.eval(r.script || '');
			return vmraid.after_ajax(() => {
				return vmraid.query_reports[report_name] && vmraid.query_reports[report_name].filters;
			})
		});
	},
