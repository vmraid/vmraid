	...
	<div class="file-preview">
		<div class="file-icon">
			<img
				...
				:alt="file.name"
			>
			<div class="fallback" v-else v-html="vmraid.utils.icon('file', 'md')">
			</div>
		</div>
		<div>
			<div>
		...
		<div class="file-actions">
			<ProgressRing
				...
				:stroke="3"
			/>
			<div v-if="uploaded" v-html="vmraid.utils.icon('solid-success', 'lg')"></div>
			<div v-if="file.failed" v-html="vmraid.utils.icon('solid-error', 'lg')"></div>
			<div class="file-action-buttons">
				<button v-if="is_cropable" class="btn btn-crop muted" @click="$emit('toggle_image_cropper')" v-html="vmraid.utils.icon('crop', 'md')"></button>
				<button v-if="!uploaded && !file.uploading && !file.failed" class="btn muted" @click="$emit('remove')" v-html="vmraid.utils.icon('delete', 'md')"></button>
			</div>
		</div>
	</div>
</template>
...
export default {
	...
	filters: {
		file_size(value) {
			return vmraid.form.formatters.FileSize(value);
		},
		file_name(value) {
			return value;
			// return vmraid.utils.file_name_ellipsis(value, 9);
		}
	},
	computed: {
		private_icon() {
			return vmraid.utils.icon(this.is_private ? 'lock' : 'unlock');
		},
		is_private() {
