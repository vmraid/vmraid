				...
				<div class="mt-2 text-center">
					...
					<button v-if="google_drive_settings.enabled" class="btn btn-file-upload" @click="show_google_drive_picker">
						<svg width="30" height="30">
							<image xlink:href="/assets/vmraid/icons/social/google_drive.svg" width="30" height="30"/>
						</svg>
...
export default {
	...
	created() {
		...
		if (vmraid.user_id !== "Guest") {
			vmraid.call({
				// method only available after login
				method: "vmraid.integrations.doctype.google_settings.google_settings.get_file_picker_settings",
				callback: (resp) => {
					if (!resp.exc) {
		...
		if (this.restrictions.max_file_size == null) {
			vmraid.call('vmraid.core.doctype.file.file.get_max_file_size')
				.then(res => {
	...
	methods: {
		...
		upload_via_file_browser() {
			...
			if (!selected_file.value) {
				vmraid.msgprint(__('Click on a file to select it.'));
		...
		upload_via_web_link() {
			...
			if (!file_url) {
				vmraid.msgprint(__('Invalid URL'));
				this.close_dialog = true;
				return Promise.reject();
			}
		...
		return_as_dataurl() {
			let promises = this.files.map(file =>
				vmraid.dom.file_to_base64(file.file_obj)
					.then(dataurl => {
		...
		capture_image() {
			...
			capture.submit(data_urls => {
				data_urls.forEach(data_url => {
					let filename = `capture_${vmraid.datetime.now_datetime().replaceAll(/[: -]/g, '_')}.png`;
