...
export default {
	...
	methods: {
		...
		get_files_in_folder(folder, start) {
			return vmraid
				.call("vmraid.core.doctype.file.file.get_files_in_folder", {
					folder,
					start,
		...
		search_by_name: vmraid.utils.debounce(function() {
			if (this.search_text === "") {
			...
			vmraid
				.call(
					"vmraid.core.doctype.file.file.get_files_by_search_text",
					{
		...
		make_file_node(file) {
			...
			let label = vmraid.utils.file_name_ellipsis(filename, 40);
			return {
				label: label,
				filename: filename,
