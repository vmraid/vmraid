...
vmraid.views.BaseList = class BaseList {
	constructor(opts) {
	...
	setup_defaults() {
		...
		this.page_title = this.page_title || vmraid.router.doctype_layout || __(this.doctype);
		this.meta = vmraid.get_meta(this.doctype);
		this.settings = vmraid.listview_settings[this.doctype] || {};
		this.user_settings = vmraid.get_user_settings(this.doctype);

		this.start = 0;
		this.page_length = 20;
		this.data = [];
		this.method = "vmraid.desk.reportview.get";

		this.can_create = vmraid.model.can_create(this.doctype);
	...
	_add_field(fieldname, doctype) {
		...
		const is_valid_field =
			vmraid.model.std_fields_list.includes(fieldname) ||
			vmraid.meta.has_field(doctype, fieldname) ||
			fieldname === "_seen";
	...
	set_stats() {
		...
		this.workflow_state_fieldname = vmraid.workflow.get_state_fieldname(
			this.doctype
		);
		if (this.workflow_state_fieldname) {
			if (!vmraid.workflow.workflows[this.doctype]["override_status"]) {
	...
	fetch_meta() {
		return vmraid.model.with_doctype(this.doctype);
	}
	...
	setup_page() {
		...
		!this.hide_card_layout && this.page.main.addClass('vmraid-card');
	...
	setup_view_menu() {
		...
		if (vmraid.boot.desk_settings.view_switcher) {
			/* @preserve
			...
			this.views_list = new vmraid.views.ListViewSelect({
	...
	set_breadcrumbs() {
		vmraid.breadcrumbs.add(this.meta.module, this.doctype);
	}

	setup_side_bar() {
		if (this.hide_sidebar || !vmraid.boot.desk_settings.list_sidebar) return;
		this.list_sidebar = new vmraid.views.ListSidebar({
			doctype: this.doctype,
			stats: this.stats,
	...
	setup_list_wrapper() {
		this.$vmraid_list = $('<div class="vmraid-list">').appendTo(
			this.page.main
		);
	}
	...
	setup_result_area() {
		...
		this.$vmraid_list.append(this.$result);
	}
	...
	setup_no_result_area() {
		...
		`).hide();
		this.$vmraid_list.append(this.$no_result);
	}

	setup_freeze_area() {
		this.$freeze = $('<div class="freeze"></div>').hide();
		this.$vmraid_list.append(this.$freeze);
	}
	...
	setup_paging_area() {
		...
		this.$vmraid_list.append(this.$paging_area);
	...
	get_fields() {
		...
		return this.fields.map((f) =>
			vmraid.model.get_full_column_name(f[0], f[1])
		);
	}
	...
	get_group_by() {
		...
		if (name_field) {
			return vmraid.model.get_full_column_name(name_field[0], name_field[1]);
		}
		return null;
	}
	...
	refresh() {
		...
		return vmraid.call(args).then((r) => {
			// render
			this.prepare_data(r);
	...
	prepare_data(r) {
		...
		Object.assign(vmraid.boot.user_info, data.user_info);
		delete data.user_info;

		data = !Array.isArray(data)
			? vmraid.utils.dict(data.keys, data.values)
			: data;
...
class FilterArea {
	...
	clear(refresh = true) {
		...
		return vmraid.run_serially(promises).then(() => {
			this.trigger_refresh = true;
		});
	}
			...
			doctype_fields
				...
				.map((df) => {
					...
					if (fieldtype === "Link" && !has_existing_filters) {
						default_value = vmraid.defaults.get_user_default(options);
					}
	...
	make_filter_list() {
		...
		this.filter_list = new vmraid.ui.FilterGroup({
...
vmraid.views.is_valid = (view_mode) =>
	vmraid.views.view_modes.includes(view_mode);
