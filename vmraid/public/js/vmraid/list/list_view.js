...
vmraid.views.ListView = class ListView extends vmraid.views.BaseList {
	static load_last_view() {
		const route = vmraid.get_route();
		const doctype = route[1];

		if (route.length === 2) {
			const user_settings = vmraid.get_user_settings(doctype);
			const last_view = user_settings.last_view;
			vmraid.set_route(
				"list",
				vmraid.router.doctype_layout || doctype,
				vmraid.views.is_valid(last_view) ? last_view.toLowerCase() : "list"
			);
			return true;
		}
		return false;
	...
	has_permissions() {
		const can_read = vmraid.perm.has_perm(this.doctype, 0, "read");
		return can_read;
	}
	...
	check_permissions() {
		if (!this.has_permissions()) {
			vmraid.set_route('');
			vmraid.throw(__("Not permitted to view {0}", [this.doctype]));
		}
	}
	...
	get_list_view_settings() {
		return vmraid
			.call("vmraid.desk.listview.get_list_settings", {
	...
	validate_filters(filters) {
		...
		valid_fields = valid_fields.concat(vmraid.model.std_fields_list);
		return filters
	...
	show_restricted_list_indicator_if_applicable() {
		const match_rules_list = vmraid.perm.get_match_rules(this.doctype);
		if (match_rules_list.length) {
			this.restricted_list = $(
				`<button class="btn btn-xs restricted-button flex align-center">
					${vmraid.utils.icon('restriction', 'xs')}
				</button>`
	...
	show_restrictions(match_rules_list = []) {
		vmraid.msgprint(
			vmraid.render_template("list_view_permission_restrictions", {
	...
	set_fields() {
		...
		this.fields.forEach((f) => {
			const df = vmraid.meta.get_docfield(f[1], f[0]);
			if (
				df &&
	...
	patch_refresh_and_load_lib() {
		...
		this.refresh = vmraid.utils.throttle(this.refresh, 1000);
		this.load_lib = new Promise((resolve) => {
			if (this.required_libs) {
				vmraid.require(this.required_libs, resolve);
			} else {
				resolve();
			}
		});
		...
		setInterval(() => {
			...
			if (vmraid.get_route_str() === this.page_name) {
				this.refresh();
			}
		}, interval);
	}
	...
	set_primary_action() {
		if (this.can_create) {
			const doctype_name = __(vmraid.router.doctype_layout) || __(this.doctype);
	...
	make_new_doc() {
		...
		this.filter_area.get().forEach((f) => {
			if (f[2] === "=" && vmraid.model.is_non_std_field(f[1])) {
				options[f[1]] = f[3];
			}
		});
		vmraid.new_doc(doctype, options);
	}
	...
	setup_columns() {
		...
		const get_df = vmraid.meta.get_docfield.bind(null, this.doctype);
		...
		if (vmraid.has_indicator(this.doctype)) {
			// indicator
			this.columns.push({
				type: "Status",
			});
		...
		this.columns = this.columns.concat(
			fields_in_list_view
				.filter((df) => {
					if (
						vmraid.has_indicator(this.doctype) &&
						df.fieldname === "status"
					) {
						return false;
					}
	...
	get_no_result_message() {
		...
		let empty_state_image =
			...
			"/assets/vmraid/images/ui-states/list-empty-state.svg";
	...
	before_refresh() {
		if (vmraid.route_options && this.filter_area) {
	...
	before_render() {
		...
		vmraid.model.user_settings.save(
			this.doctype,
			"last_view",
			this.view_name
		);
	...
	get_header_html() {
		...
		let subject_html = `
			...
			<span class="level-item list-liked-by-me hidden-xs">
				<span title="${__("Likes")}">${vmraid.utils.icon('heart', 'sm', 'like-icon')}</span>
			</span>
		...
		const $columns = this.columns
			.map(col => {
				let classes = [
					...
					vmraid.model.is_numeric_field(col.df) ? "text-right" : "",
				].join(" ");
	...
	get_column_html(col, doc) {
		...
		const format = () => {
			if (df.fieldtype === "Code") {
				...
				return vmraid.format(value, df, null, doc);
			}
		};
		...
		const field_html = () => {
			...
			if (strip_html_required) {
				...
				_value =
					typeof value === "string"
						? vmraid.utils.escape_html(value)
						: value;
			}
			...
			if (df.fieldtype === "Image") {
				html = df.options ? `<img src="${doc[df.options]}"
					...
					: `<div class="missing-image small">
						${vmraid.utils.icon('restriction')}
					</div>`;
			} else if (df.fieldtype === "Select") {
				html = `<span class="filterable indicator-pill ${vmraid.utils.guess_colour(
					_value
				)} ellipsis"
				...
				html = `<a class="filterable ellipsis"
					data-filter="${fieldname},=,${vmraid.utils.escape_html(value)}">
					${format()}
				</a>`;
			}

			return `<span class="ellipsis"
				title="${__(label)}: ${vmraid.utils.escape_html(_value)}">
				${html}
			</span>`;
		};
		...
		const css_class = [
			"list-row-col ellipsis",
			class_map[col.type],
			vmraid.model.is_numeric_field(df) ? "text-right" : "",
		].join(" ");
	...
	get_meta_html(doc) {
		...
		if (assigned_users.length) {
			assigned_to = `<div class="list-assignments">
					${vmraid.avatar_group(assigned_users, 3, { filterable: true })[0].outerHTML}
				</div>`;
		}
		...
		const comment_count = `<span class="${
			!doc._comment_count ? "text-extra-muted" : ""
				...
				${vmraid.utils.icon('small-message')}
	...
	get_count_str() {
		...
		return vmraid.db.count(this.doctype, {
	...
	get_form_link(doc) {
		...
		return `/app/${vmraid.router.slug(vmraid.router.doctype_layout || this.doctype)}/${docname}`;
	}

	get_seen_class(doc) {
		return JSON.parse(doc._seen || '[]').includes(vmraid.session.user)
			? ''
			: 'bold';
	}
	...
	get_like_html(doc) {
		...
		let heart_class = liked_by.includes(vmraid.session.user)
			? "liked-by liked"
			: "not-liked";
		...
		return `<span
			...
			title="${liked_by.map(u => vmraid.user_info(u).fullname).join(', ')}">
			${vmraid.utils.icon('heart', 'sm', 'like-icon')}
		</span>
	...
	get_subject_html(doc) {
		...
		let escaped_subject = vmraid.utils.escape_html(subject);
	...
	get_indicator_html(doc) {
		const indicator = vmraid.get_indicator(doc, this.doctype);
	...
	get_image_url(doc) {
		...
		if (window.cordova && !vmraid.utils.is_url(url)) {
			url = vmraid.base_url + url;
		}
		return url || null;
	}
	...
	setup_filterable() {
		...
		this.$result.on("click", ".filterable", (e) => {
			...
			const filters_to_apply = filters.map((f) => {
				f = f.split(",");
				if (f[2] === "Today") {
					f[2] = vmraid.datetime.get_today();
				} else if (f[2] == "User") {
					f[2] = vmraid.session.user;
				}
	...
	setup_list_click() {
		this.$result.on("click", ".list-row, .image-view-header, .file-header", (e) => {
			...
			if (link) {
				vmraid.set_route(link.pathname);
				return false;
			}
		});
	}
	...
	setup_like() {
		this.$result.on("click", ".like-action", vmraid.ui.click_toggle_like);
	...
	setup_realtime_updates() {
		...
		vmraid.realtime.on("list_update", (data) => {
			...
			vmraid.call(call_args).then(({ message }) => {
				if (!message) return;
				const data = vmraid.utils.dict(message.keys, message.values);
				if (!(data && data.length)) {
	...
	save_view_user_settings(obj) {
		return vmraid.model.user_settings.save(
			this.doctype,
			this.view_name,
			obj
		);
	...
	share_url() {
		const d = new vmraid.ui.Dialog({
			...
			primary_action: () => {
				vmraid.utils.copy_to_clipboard(this.get_share_url());
				d.hide();
			},
		});
		d.show();
	...
	get_menu_items() {
		...
		if (vmraid.model.can_import(doctype, null, this.meta)) {
			items.push({
				label: __("Import", null, "Button in list view menu"),
				action: () =>
					vmraid.set_route("list", "data-import", {
		...
		if (vmraid.model.can_set_user_permissions(doctype)) {
			items.push({
		...
		if (vmraid.user_roles.includes("System Manager")) {
			...
			items.push({
				...
				action: () => {
					if (!this.meta) return;
					if (this.meta.custom) {
						vmraid.set_route("form", "doctype", doctype);
		...
		if (
			...
			vmraid.boot.developer_mode === 1
		) {
			// edit doctype
			items.push({
				label: __("Edit DocType", null, "Button in list view menu"),
				action: () => vmraid.set_route("form", "doctype", doctype),
				standard: true,
			});
		}
	...
	get_workflow_action_menu_items() {
		...
		if (vmraid.model.has_workflow(this.doctype)) {
			const actions = vmraid.workflow.get_all_transition_actions(
				this.doctype
			);
			actions.forEach((action) => {
				workflow_actions.push({
					...
					action: () => {
						vmraid.xcall(
							"vmraid.model.workflow.bulk_workflow_approval",
							{
	...
	toggle_workflow_actions() {
		if (!vmraid.model.has_workflow(this.doctype)) return;
		const checked_items = this.get_checked_items();
		vmraid
			.xcall("vmraid.model.workflow.get_common_transition_actions", {
				docs: checked_items,
				doctype: this.doctype,
			})
			.then((actions) => {
	...
	get_actions_menu_items() {
		...
		const is_field_editable = (field_doc) => {
			return (
				field_doc.fieldname &&
				vmraid.model.is_value_type(field_doc) &&
		...
		const has_submit_permission = (doctype) => {
			return vmraid.perm.has_perm(doctype, 0, "submit");
		};
		...
		const bulk_edit = () => {
			return {
				...
				action: () => {
					...
					vmraid.meta.get_docfields(doctype).forEach((field_doc) => {
		...
		// bulk printing
		if (vmraid.model.can_print(doctype)) {
			actions_menu_items.push(bulk_printing());
		}

		// bulk submit
		if (
			vmraid.model.is_submittable(doctype) &&
			has_submit_permission(doctype) &&
			!vmraid.model.has_workflow(doctype)
		) {
		...
		// bulk cancel
		if (
			vmraid.model.can_cancel(doctype) &&
			!vmraid.model.has_workflow(doctype)
		) {
		...
		// bulk delete
		if (vmraid.model.can_delete(doctype)) {
	...
	parse_filters_from_route_options() {
		...
		for (let field in vmraid.route_options) {
			let doctype = null;
			let value = vmraid.route_options[field];
			...
			if (!doctype) {
				doctype = vmraid.meta.get_doctype_for_field(
					this.doctype,
					field
				);
			}
	...
	static trigger_list_update(data) {
		...
		const list_view = vmraid.views.list_view[page_name];
		list_view && list_view.on_update(data);
	}
};

$(document).on("save", (event, doc) => {
	vmraid.views.ListView.trigger_list_update(doc);
});

vmraid.get_list_view = (doctype) => {
	let route = `List/${doctype}/List`;
	return vmraid.views.list_view[route];
};
