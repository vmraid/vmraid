...
vmraid.views.ListView = class ListView extends vmraid.views.BaseList {
	...
	show() {
		...
		if (!this.has_permissions()) {
			vmraid.set_route('');
			vmraid.msgprint(__("Not permitted to view {0}", [this.doctype]));
			return;
		}
	...
	set_primary_action() {
		if (this.can_create) {
			this.page.set_primary_action(
				`${__("Add")} ${vmraid.router.doctype_layout || __(this.doctype)}`,
				() => {
	...
	get_menu_items() {
		...
		if (vmraid.model.can_import(doctype)) {
			items.push({
				label: __("Import"),
				action: () =>
	...
	static trigger_list_update(data) {
		...
		// refresh tree view
		if (vmraid.views.trees[doctype]) {
			vmraid.views.trees[doctype].tree.refresh();
			return;
		}
