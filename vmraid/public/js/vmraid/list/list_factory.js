...
vmraid.views.ListFactory = class ListFactory extends vmraid.views.Factory {
	make (route) {
		...
		vmraid.views.list_view[me.page_name] = new view_class({
			doctype: doctype,
	...
	re_route_to_view() {
		...
		const last_route = vmraid.route_history.slice(-2)[0];
		if (
			this.route[0] === 'List' &&
			this.route.length === 2	&&
			vmraid.views.list_view[doctype] &&
			last_route &&
			last_route[0] === 'List' &&
	...
	set_module_breadcrumb() {
		if (vmraid.route_history.length > 1) {
			const prev_route = vmraid.route_history[vmraid.route_history.length - 2];
	...
	set_cur_list() {
		cur_list = vmraid.views.list_view[this.page_name];
