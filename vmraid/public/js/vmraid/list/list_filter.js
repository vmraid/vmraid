...
export default class ListFilter {
	constructor({ wrapper, doctype }) {
		...
		this.can_add_global = vmraid.user.has_role([
			'System Manager',
			'Administrator',
		]);
		this.filters = [];
	...
	make() {
		...
		this.filter_input = vmraid.ui.form.make_control({
			df: {
				fieldtype: 'Data',
		...
		this.is_global_input = vmraid.ui.form.make_control({
			df: {
				fieldtype: 'Check',
				label: __('Is Global'),
			},
	...
	filter_template(filter) {
		return `<div class="list-link filter-pill list-sidebar-button btn btn-default" data-name="${
			...
			<a class="remove">${vmraid.utils.icon('close')}</a>
		</div>`;
	}
	...
	save_filter(filter_name) {
		return vmraid.db.insert({
			doctype: 'List Filter',
			reference_doctype: this.list_view.doctype,
			filter_name,
			for_user: this.is_global_input.get_value() ? '' : vmraid.session.user,
	...
	remove_filter(name) {
		if (!name) return;
		return vmraid.db.delete_doc('List Filter', name);
	}
