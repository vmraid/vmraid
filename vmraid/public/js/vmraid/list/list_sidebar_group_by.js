...
vmraid.views.ListGroupBy = class ListGroupBy {
	constructor(opts) {
		$.extend(this, opts);
		this.make_wrapper();
	...
	render_group_by_items() {
		let get_item_html = (fieldname) => {
			...
			if (fieldname === 'assigned_to') {
				...
				label = vmraid.meta.get_label(this.doctype, fieldname);
				let docfield = vmraid.meta.get_docfield(this.doctype, fieldname);
				if (!docfield) {
					return;
				}
			...
			return `<li class="group-by-field list-link">
					...
					href="#" onclick="return false;">
						...
						<span>${vmraid.utils.icon('select', 'xs')}</span>
					</a>
	...
	get_group_by_count(field) {
		...
		return vmraid
			.call('vmraid.desk.listview.get_group_by_count', args)
			.then((r) => {
				let field_counts = r.message || [];
				field_counts = field_counts.filter((f) => f.count !== 0);
				let current_user = field_counts.find(
					(f) => f.name === vmraid.session.user
				);
				field_counts = field_counts.filter(
					(f) =>
						!['Guest', 'Administrator', vmraid.session.user].includes(f.name)
				);
				// Set vmraid.session.user on top of the list
	...
	get_dropdown_html(field, fieldtype, applied=false) {
		...
		let applied_html = applied ? `<span class="applied"> ${vmraid.utils.icon('tick', 'xs')} </span>` : '';
