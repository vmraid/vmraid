export default class BulkOperations {
	constructor({ doctype }) {
		if (!doctype) vmraid.throw(__('Doctype required'));
		this.doctype = doctype;
	}

	print (docs) {
		const print_settings = vmraid.model.get_doc(':Print Settings', 'Print Settings');
		const allow_print_for_draft = cint(print_settings.allow_print_for_draft);
		const is_submittable = vmraid.model.is_submittable(this.doctype);
		...
		const valid_docs = docs.filter(doc => {
			return !is_submittable || doc.docstatus === 1 ||
				...
				vmraid.user.has_role('Administrator');
		}).map(doc => doc.name);

		const invalid_docs = docs.filter(doc => !valid_docs.includes(doc.name));

		if (invalid_docs.length > 0) {
			vmraid.msgprint(__('You selected Draft or Cancelled documents'));
			return;
		}

		if (valid_docs.length === 0) {
			vmraid.msgprint(__('Select atleast 1 record for printing'));
			return;
		}
		...
		const dialog = new vmraid.ui.Dialog({
			...
			},
			{
				...
				fieldname: 'print_sel',
				options: vmraid.meta.get_print_formats(this.doctype)
			},
			{
				fieldtype: 'Select',
				label: __('Page Size'),
				fieldname: 'page_size',
				options: vmraid.meta.get_print_sizes(),
		...
		dialog.set_primary_action(__('Print'), args => {
			if (!args) return;
			const default_print_format = vmraid.get_meta(this.doctype).default_print_format;
			...
			if (args.page_size === "Custom") {
				if (args.page_height === 0 || args.page_width === 0) {
					vmraid.throw(__('Page height and width cannot be zero'));
				}
			...
			const w = window.open(
				'/api/method/vmraid.utils.print_format.download_multi_pdf?' +
	...
	delete (docnames, done = null) {
		vmraid
			.call({
				method: 'vmraid.desk.reportview.delete_items',
				freeze: true,
				args: {
					items: docnames,
			...
			.then((r) => {
				...
				if (failed.length && !r._server_messages) {
					vmraid.throw(__('Cannot delete {0}', [failed.map(f => f.bold()).join(', ')]));
				}
	...
	assign (docnames, done) {
		if (docnames.length > 0) {
			const assign_to = new vmraid.ui.form.AssignToDialog({
				obj: this,
				method: 'vmraid.desk.form.assign_to.add_multiple',
				doctype: this.doctype,
				docname: docnames,
				bulk_assign: true,
				re_assign: true,
			...
			vmraid.msgprint(__('Select records for assignment'));
		}
	}

	apply_assignment_rule (docnames, done) {
		if (docnames.length > 0) {
			vmraid.call('vmraid.automation.doctype.assignment_rule.assignment_rule.bulk_apply', {
				doctype: this.doctype,
				docnames: docnames
			}).then(() => done());
		}
	...
	submit_or_cancel (docnames, action = 'submit', done = null) {
		...
		vmraid
			.call({
				method: 'vmraid.desk.doctype.bulk_update.bulk_update.submit_cancel_or_update_docs',
				args: {
					doctype: this.doctype,
					action: action,
					docnames: docnames
			...
			.then((r) => {
				...
				if (failed.length && !r._server_messages) {
					vmraid.throw(__('Cannot {0} {1}', [action, failed.map(f => f.bold()).join(', ')]));
				}
				if (failed.length < docnames.length) {
					vmraid.utils.play_sound(action);
					if (done) done();
				}
			});
	}
			...
			primary_action: ({ value }) => {
				...
				vmraid.call({
					...
					if (failed.length && !r._server_messages) {
						...
						vmraid.throw(__('Cannot update {0}', [failed.map(f => f.bold ? f.bold() : f).join(', ')]));
					}
					done();
					dialog.hide();
					vmraid.show_alert(__('Updated successfully'));
				});
			},
		...
		const dialog = new vmraid.ui.Dialog({
			...
			fields: [
				{
					...
					get_data: function (txt) {
						return vmraid.db.get_link_options("Tag", txt);
					}
				},
			],
			...
			primary_action: () => {
				...
				if (args && args.tags) {
					...
					vmraid.call({
						method: "vmraid.desk.doctype.tag.tag.add_tags",
						args: {
							'tags': args.tags,
							'dt': this.doctype,
							'docs': docnames,
	...
	export (doctype, docnames) {
		vmraid.require('data_import_tools.bundle.js', () => {
			const data_exporter = new vmraid.data_import.DataExporter(doctype, 'Insert New Records');
