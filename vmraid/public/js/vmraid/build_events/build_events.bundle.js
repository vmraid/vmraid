...
vmraid.realtime.on("build_event", data => {
	if (data.success) {
			...
			for (let file of changed_files) {
				if (file.includes(".bundle.")) {
					...
					if (parts.length === 2) {
						...
						vmraid.assets.executed_ = vmraid.assets.executed_.filter(
		...
		// update assets json
		vmraid.call("vmraid.sessions.get_boot_assets_json").then(r => {
			if (r.message) {
				vmraid.boot.assets_json = r.message;

				if (vmraid.hot_update) {
					vmraid.hot_update.forEach(callback => {
						callback();
					});
				}
			}
