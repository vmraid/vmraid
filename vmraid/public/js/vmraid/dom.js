...
vmraid.dom = {
	...
	get_unique_id: function() {
		const id = 'unique-' + vmraid.dom.id_count;
		vmraid.dom.id_count++;
		return id;
	},
	set_unique_id: function(ele) {
		...
		var id = 'unique-' + vmraid.dom.id_count;
		$ele.attr('id', id);
		vmraid.dom.id_count++;
		return id;
	},
	...
	add: function(parent, newtag, className, cs, innerHTML, onclick) {
		if(parent && parent.substr)parent = vmraid.dom.by_id(parent);
	...
	freeze: function(msg, css_class) {
		...
		vmraid.dom.freeze_count++;
	},
	unfreeze: function() {
		if(!vmraid.dom.freeze_count) return; // anything open?
		vmraid.dom.freeze_count--;
	...
	scroll_to_section(section_name) {
		setTimeout(() => {
			...
			if (section.length) {
				...
				vmraid.ui.scroll(section.parent().parent());
			}
		}, 200);
	},
...
vmraid.ellipsis = function(text, max) {
	if(!max) max = 20;
	text = cstr(text);
	if(text.length > max) {
...
vmraid.run_serially = function(tasks) {
...
vmraid.load_image = (src, onload, onerror, preprocess = () => {}) => {
...
vmraid.timeout = seconds => {
	return new Promise((resolve) => {
		setTimeout(() => resolve(), seconds * 1000);
	});
};

vmraid.scrub = function(text, spacer='_') {
	return text.replace(/ /g, spacer).toLowerCase();
};

vmraid.unscrub = function(txt) {
	return vmraid.model.unscrub(txt);
};

vmraid.get_data_pill = (label, target_id=null, remove_action=null, image=null) => {
...
vmraid.get_modal = function(title, content) {
				...
				<div class="modal-header">
					...
					<div class="modal-actions">
						<button class="btn btn-modal-minimize btn-link hide">
							${vmraid.utils.icon('collapse')}
						</button>
						<button class="btn btn-modal-close btn-link" data-dismiss="modal">
							${vmraid.utils.icon('close-alt', 'sm', 'close-alt')}
						</button>
					</div>
				</div>
...
vmraid.is_online = function() {
	if (vmraid.boot.developer_mode == 1) {
