...
$.extend(vmraid.model, {
	...
	is_non_std_field: function(fieldname) {
		return ![...vmraid.model.std_fields_list, ...vmraid.model.child_table_field_list].includes(fieldname);
	},
	...
	set_value: function(doctype, docname, fieldname, value, fieldtype, skip_dirty_trigger=false) {
		...
		$.each(to_update, (key, value) => {
			if (doc && doc[key] !== value) {
				...
				doc[key] = value;
				tasks.push(() => vmraid.model.trigger(key, value, doc, skip_dirty_trigger));
			} else {
	...
	trigger: function(fieldname, value, doc, skip_dirty_trigger=false) {
		...
		if(vmraid.model.events[doc.doctype]) {
			enqueue_events(vmraid.model.events[doc.doctype][fieldname]);
			enqueue_events(vmraid.model.events[doc.doctype]['*']);
		}
	...
	delete_doc: function(doctype, docname, callback) {
		let title = docname;
		const title_field = vmraid.get_meta(doctype).title_field;
		if (vmraid.get_meta(doctype).autoname == "hash" && title_field) {
			const value = vmraid.model.get_value(doctype, docname, title_field);
			if (value) {
				title = `${value} (${docname})`;
			}
		}
		vmraid.confirm(__("Permanently delete {0}?", [title.bold()]), function() {
			return vmraid.call({
