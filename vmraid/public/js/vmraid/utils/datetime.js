...
vmraid.defaultDateFormat = "YYYY-MM-DD";
vmraid.defaultTimeFormat = "HH:mm:ss";
vmraid.defaultDatetimeFormat = vmraid.defaultDateFormat + " " + vmraid.defaultTimeFormat;
moment.defaultFormat = vmraid.defaultDateFormat;

vmraid.provide("vmraid.datetime");

$.extend(vmraid.datetime, {
	convert_to_user_tz: function(date, format) {
		...
		let date_obj = null;
		if (vmraid.boot.time_zone && vmraid.boot.time_zone.system && vmraid.boot.time_zone.user) {
			date_obj = moment.tz(date, vmraid.boot.time_zone.system)
				.clone()
				.tz(vmraid.boot.time_zone.user);
		} else {
			date_obj = moment(date);
		}

		return format === false ? date_obj : date_obj.format(vmraid.defaultDatetimeFormat);
	},
	...
	convert_to_system_tz: function(date, format) {
		...
		let date_obj = null;
		if (vmraid.boot.time_zone && vmraid.boot.time_zone.system && vmraid.boot.time_zone.user) {
			date_obj = moment.tz(date, vmraid.boot.time_zone.user)
				.clone()
				.tz(vmraid.boot.time_zone.system);
		} else {
			date_obj = moment(date);
		}
	...
	is_system_time_zone: function() {
		if (vmraid.boot.time_zone && vmraid.boot.time_zone.system && vmraid.boot.time_zone.user) {
			return moment().tz(vmraid.boot.time_zone.system).utcOffset() === moment().tz(vmraid.boot.time_zone.user).utcOffset();
		}
	...
	is_timezone_same: function() {
		return vmraid.datetime.is_system_time_zone();
	},

	str_to_obj: function(d) {
		return moment(d, vmraid.defaultDatetimeFormat)._d;
	},
	...
	obj_to_user: function(d) {
		return moment(d).format(vmraid.datetime.get_user_date_fmt().toUpperCase());
	},
	...
	get_user_time_fmt: function() {
		return vmraid.sys_defaults && vmraid.sys_defaults.time_format || "HH:mm:ss";
	},

	get_user_date_fmt: function() {
		return vmraid.sys_defaults && vmraid.sys_defaults.date_format || "yyyy-mm-dd";
	},
	...
	str_to_user: function(val, only_time = false) {
		if (!val) return "";
		const user_date_fmt = vmraid.datetime.get_user_date_fmt().toUpperCase();
		const user_time_fmt = vmraid.datetime.get_user_time_fmt();
		let user_format = user_time_fmt;

		if (only_time) {
			let date_obj = moment(val, vmraid.defaultTimeFormat);
			return date_obj.format(user_format);
		} else {
			let date_obj = moment.tz(val, vmraid.boot.time_zone.system);
	...
	user_to_str: function(val, only_time = false) {
		...
		var user_time_fmt = vmraid.datetime.get_user_time_fmt();
		if(only_time) {
			return moment(val, user_time_fmt)
				.format(vmraid.defaultTimeFormat);
		}

		var user_fmt = vmraid.datetime.get_user_date_fmt().toUpperCase();
	...
	user_to_obj: function(d) {
		return vmraid.datetime.str_to_obj(vmraid.datetime.user_to_str(d));
	},
	...
	now_date: function(as_obj = false) {
		return vmraid.datetime._date(vmraid.defaultDateFormat, as_obj);
	},

	now_time: function(as_obj = false) {
		return vmraid.datetime._date(vmraid.defaultTimeFormat, as_obj);
	},

	now_datetime: function(as_obj = false) {
		return vmraid.datetime._date(vmraid.defaultDatetimeFormat, as_obj);
	},
	...
	_date: function(format, as_obj = false) {
		 ...
		 */
		let time_zone = vmraid.boot.time_zone ? vmraid.boot.time_zone.user || vmraid.boot.time_zone.system : vmraid.sys_defaults.time_zone;
		let date = moment.tz(time_zone);

		return as_obj ? vmraid.datetime.moment_to_date_obj(date) : date.format(format);
	},
	...
	nowdate: function() {
		return vmraid.datetime.now_date();
	},
	...
	validate: function(d) {
		return moment(d, [
			vmraid.defaultDateFormat,
			vmraid.defaultDatetimeFormat,
			vmraid.defaultTimeFormat
		], true).isValid();
	},

	get_first_day_of_the_week_index() {
		const first_day_of_the_week = vmraid.sys_defaults.first_day_of_the_week || "Sunday";
...
Object.defineProperties(window, {
	'dateutil': {
		get: function() {
			console.warn('Please use `vmraid.datetime` instead of `dateutil`. It will be deprecated soon.');
			return vmraid.datetime;
		},
		configurable: true
	},
	'date': {
		get: function() {
			console.warn('Please use `vmraid.datetime` instead of `date`. It will be deprecated soon.');
			return vmraid.datetime;
		},
		configurable: true
	},
	'get_today': {
		get: function() {
			console.warn('Please use `vmraid.datetime.get_today` instead of `get_today`. It will be deprecated soon.');
			return vmraid.datetime.get_today;
		},
		configurable: true
	}
});
