...
$.extend(vmraid.datetime, {
	convert_to_user_tz: function(date, format) {
		...
		if(vmraid.sys_defaults.time_zone) {
			var date_obj = moment.tz(date, vmraid.sys_defaults.time_zone).local();
		} else {
	...
	convert_to_system_tz: function(date, format) {
		...
		if(vmraid.sys_defaults.time_zone) {
			var date_obj = moment(date).tz(vmraid.sys_defaults.time_zone);
		} else {
			var date_obj = moment(date);
		}
	...
	is_timezone_same: function() {
		if(vmraid.sys_defaults.time_zone) {
			return moment().tz(vmraid.sys_defaults.time_zone).utcOffset() === moment().utcOffset();
		} else {
			return true;
		}
	},
	...
	str_to_user: function(val, only_time = false) {
		...
		if(only_time) {
			return moment(val, vmraid.defaultTimeFormat)
				.format(user_time_fmt);
		}

		var user_date_fmt = vmraid.datetime.get_user_date_fmt().toUpperCase();
	...
	_date: function(format, as_obj = false) {
		const time_zone = vmraid.sys_defaults && vmraid.sys_defaults.time_zone;
		let date;
		if (time_zone) {
		...
		if (as_obj) {
			return vmraid.datetime.moment_to_date_obj(date);
		} else {
