...
vmraid.provide("vmraid.tools");

vmraid.tools.downloadify = function(data, roles, title) {
	if(roles && roles.length && !has_common(roles, roles)) {
		vmraid.msgprint(__("Export not allowed. You need {0} role to export.", [vmraid.utils.comma_or(roles)]));
		return;
	}

	var filename = title + ".csv";
	var csv_data = vmraid.tools.to_csv(data);
...
vmraid.markdown = function(txt) {
	if(!vmraid.md2html) {
		vmraid.md2html = new showdown.Converter();
	}
	...
	return vmraid.md2html.makeHtml(txt);
};

vmraid.tools.to_csv = function(data) {
	var res = [];
