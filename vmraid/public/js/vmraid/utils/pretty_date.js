function prettyDate(date, mini) {
	...
	if (typeof (date) == "string") {
		date = vmraid.datetime.convert_to_user_tz(date);
		date = new Date((date || "").replace(/-/g, "/").replace(/[TZ]/g, " ").replace(/\.[0-9]*/, ""));
	}

	let diff = (((new Date(vmraid.datetime.now_datetime())).getTime() - date.getTime()) / 1000);
...
window.comment_when = function(datetime, mini) {
	var timestamp = vmraid.datetime.str_to_user ?
		vmraid.datetime.str_to_user(datetime) : datetime;
	return '<span class="vmraid-timestamp '
		+ (mini ? " mini" : "") + '" data-timestamp="' + datetime
		+ '" title="' + timestamp + '">'
		+ prettyDate(datetime, mini) + '</span>';
};
vmraid.datetime.comment_when = comment_when;
vmraid.datetime.prettyDate = prettyDate;

vmraid.datetime.refresh_when = function() {
	if (jQuery) {
		$(".vmraid-timestamp").each(function() {
