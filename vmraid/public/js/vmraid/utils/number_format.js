...
if (!window.vmraid) window.vmraid = {};
...
window.format_number = function (v, format, decimals) {
	if (!format) {
		...
		if (decimals == null) decimals = cint(vmraid.defaults.get_default("float_precision")) || 3;
	}
...
function format_currency(v, currency, decimals) {
	...
	if(decimals === undefined) {
		decimals = vmraid.boot.sysdefaults.currency_precision || null;
	}
...
function get_currency_symbol(currency) {
	if (vmraid.boot) {
		if (vmraid.boot.sysdefaults && vmraid.boot.sysdefaults.hide_currency_symbol == "Yes")
			return null;

		if (!currency)
			currency = vmraid.boot.sysdefaults.currency;

		return vmraid.model.get_value(":Currency", currency, "symbol") || currency;
	} else {
		// load in template
		return vmraid.currency_symbols[currency];
	}
}

function get_number_format(currency) {
	return (vmraid.boot && vmraid.boot.sysdefaults && vmraid.boot.sysdefaults.number_format) || "#,###.##";
}

function get_number_format_info(format) {
	var info = vmraid.number_format_info[format];
...
function precision(fieldname, doc) {
	if (cur_frm) {
		...
		var df = vmraid.meta.get_docfield(doc.doctype, fieldname, doc.parent || doc.name);
		if (!df) console.log(fieldname + ": could not find docfield in method precision()");
		return vmraid.meta.get_field_precision(df, doc);
	} else {
		return vmraid.boot.sysdefaults.float_precision
	}
}
...
function round_based_on_smallest_currency_fraction(value, currency, precision) {
	var smallest_currency_fraction_value = flt(vmraid.model.get_value(":Currency",
