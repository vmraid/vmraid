...
export default class OnboardingWidget extends Widget {
	...
	add_step(step, index) {
		...
		let $step = $(`<a class="onboarding-step ${status}">
				<div class="step-title">
					...
					<div class="step-index step-skipped">${vmraid.utils.icon('tick', 'xs')}</div>
					<div class="step-index step-complete">${vmraid.utils.icon('tick', 'xs')}</div>
	...
	show_step(step) {
		...
		let actions = {
			...
			"Create Entry": (step) => {
				if (step.is_complete) {
					vmraid.set_route(`/app/List/${step.reference_document}`);
				} else {
		...
		const toggle_content = () => {
			...
			this.step_body.html(
				step.description ?
					vmraid.markdown(step.description)
					: `<h1>${step.title}</h1>`
			);
	...
	go_to_page(step) {
		...
		vmraid.set_route(step.path).then(() => {
			...
			let msg_dialog = vmraid.msgprint({
				message: message,
				title: title,
				primary_action: {
					action: () => {
	...
	open_report(step) {
		let route = vmraid.utils.generate_route({
		...
		let current_route = vmraid.get_route();

		vmraid.set_route(route).then(() => {
			let msg_dialog = vmraid.msgprint({
				message: __(step.report_description),
				title: __(step.reference_report),
				primary_action: {
					action: () => {
						vmraid.set_route(current_route).then(() => {
							this.mark_complete(step);
						});
						msg_dialog.hide();
					},
			...
			vmraid.msg_dialog.custom_onhide = () => this.mark_complete(step);
		});
	}
	...
	show_form_tour(step) {
		let route;
		if (step.is_single) {
			route = vmraid.router.slug(step.reference_document);
		} else {
			route = `${vmraid.router.slug(step.reference_document)}/new`;
		}

		let current_route = vmraid.get_route();

		vmraid.route_hooks = {};
		vmraid.route_hooks.after_load = (frm) => {
			const on_finish = () => {
				let msg_dialog = vmraid.msgprint({
	...
	update_settings(step) {
		...
		vmraid.route_hooks.after_save = (frm) => {
			let success = false;
			let args = {};
			...
			if (success) {
				...
				args.secondary_action = {
					action: () => vmraid.set_route(current_route),
					label: __("Go Back"),
				};
			...
			vmraid.msgprint(args);
			vmraid.msg_dialog.custom_onhide = () => custom_onhide();
		};

		vmraid.set_route("Form", step.reference_document);
	}
	...
	async create_entry(step) {
		...
		if (step.is_submittable) {
			vmraid.route_hooks.after_save = () => {
				vmraid.msgprint({
					message: __("Submit this document to complete this step."),
					title: __("Great")
				});
			};
			vmraid.route_hooks.after_submit = callback;
		} else {
			vmraid.route_hooks.after_save = callback;
		}

		vmraid.set_route('Form', step.reference_document, docname);
	}

	show_quick_entry(step) {
		let current_route = vmraid.get_route_str();
		vmraid.ui.form.make_quick_entry(
			step.reference_document,
			() => {
				if (vmraid.get_route_str() != current_route) {
					let success_dialog = vmraid.msgprint({
						message: __("Let's take you back to onboarding"),
	...
	show_video(step) {
		vmraid.help.show_video(step.video_url, step.title);
		this.mark_complete(step);
	}
	...
	update_step_status(step, status, value, callback) {
		...
		vmraid
			.call("vmraid.desk.desktop.update_onboarding_step", {
				name: step.name,
				field: status,
				value: value,
			})
	...
	show_success() {
		...
		let success_state_image =
			...
			"/assets/vmraid/images/ui-states/success-color.png";
		let documentation = "";
		if (this.docs_url) {
			documentation = __(
	...
	is_dismissed() {
		...
		if (Object.keys(dismissed).includes(this.title)) {
			...
			let today = new Date();
			let diff = vmraid.datetime.get_hour_diff(today, last_hidden);
			return diff < 24;
		}
		return false;
	}
	...
	set_actions() {
		...
		dismiss.on("click", () => {
			...
			dismissed[this.title] = vmraid.datetime.now_datetime();
		...
		return vmraid.model
			...
			.then(onboarding_doc => {
				if (onboarding_doc) {
					...
					const method =
						"vmraid.desk.doctype.onboarding_step.onboarding_step.get_onboarding_steps";
					return vmraid
	...
	async get_first_document(doctype) {
		const { message } = await vmraid.db.get_value('Form Tour', { 'reference_doctype': doctype }, ["first_document"]);
		let docname;
