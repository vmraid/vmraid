class WidgetDialog {
		...
		this.dialog = new vmraid.ui.Dialog({
			...
			primary_action: (data) => {
				...
				if (!this.editing && !data.name) {
					data.name = `${this.type}-${this.label}-${vmraid.utils.get_random(20)}`;
				}
	...
	get_title() {
		...
		let label = action = action + " " + vmraid.model.unscrub(this.type);
		return __(label);
	}
...
class ShortcutDialog extends WidgetDialog {
		...
		return [
			...
			},
			{
				...
				onchange: () => {
					if (this.dialog.get_value("type") == "DocType") {
						...
						if (doctype && vmraid.boot.single_types.includes(doctype)) {
							this.hide_filters();
						} else if (doctype) {
						...
						if (vmraid.boot.treeviews.includes(doctype)) views.push("Tree");
						if (vmraid.boot.calendars.includes(doctype)) views.push("Calendar");
			...
			},
			{
				...
				depends_on: (state) => {
					if (this.dialog) {
						let doctype = this.dialog.get_value("link_to");
						let is_single = vmraid.boot.single_types.includes(doctype);
	...
	process_data(data) {
		...
		data.label = data.label
			? data.label
			: vmraid.model.unscrub(data.link_to);
...
class NumberCardDialog extends WidgetDialog {
	...
	get_fields() {
		...
		fields = [
			...
			},
			{
				...
				get_query: () => {
					return {
						'query': 'vmraid.desk.doctype.number_card.number_card.get_cards_for_user',
						filters: {
	...
	set_aggregate_function_fields() {
		...
		if (this.document_type && vmraid.get_meta(this.document_type)) {
			vmraid.get_meta(this.document_type).fields.map(df => {
