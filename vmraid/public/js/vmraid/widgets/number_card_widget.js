...
export default class NumberCardWidget extends Widget {
	...
	make_card() {
		vmraid.model.with_doc('Number Card', this.name).then(card => {
			if (!card) {
	...
	create_number_card() {
		this.set_doc_args();
		return vmraid.xcall(
			'vmraid.desk.doctype.number_card.number_card.create_number_card',
			{
				'args': this.card_doc
			}
		).then(doc => {
	...
	set_route() {
		...
		const route = vmraid.utils.generate_route({
			name: name,
		...
		if (is_document_type) {
			...
			vmraid.route_options = filters.reduce((acc, filter) => {
	...
	get_settings(type) {
		...
		const settings_map = {
			...
			'Document Type': {
				method: 'vmraid.desk.doctype.number_card.number_card.get_result',
				args: {
					doc: this.card_doc,
					filters: this.filters,
				},
	...
	get_filters() {
		const filters = vmraid.dashboard_utils.get_all_filters(this.card_doc);
		return filters;
	}
	...
	get_number() {
		return vmraid.xcall(this.settings.method, this.settings.args).then(res => {
	...
	get_number_for_doctype_card(res) {
		...
		if (this.card_doc.function !== 'Count') {
			return vmraid.model.with_doctype(this.card_doc.document_type, () => {
				const based_on_df =
					vmraid.meta.get_docfield(this.card_doc.document_type, this.card_doc.aggregate_function_based_on);
	...
	get_number_for_report_card(res) {
		...
		this.number = vmraid.report_utils.get_result_of_fn(this.card_doc.report_function, vals);
		this.set_formatted_number(col);
	}

	set_formatted_number(df) {
		const default_country = vmraid.sys_defaults.country;
		const shortened_number = vmraid.utils.shorten_number(this.number, default_country, 5);
		let number_parts = shortened_number.split(' ');

		const symbol = number_parts[1] || '';
		const formatted_number = $(vmraid.format(number_parts[0], df)).text();
			...
			if (this.percentage_stat == 0 || this.percentage_stat == undefined) {
				...
				caret_html =
					`<span class="indicator-pill-round green">
						${vmraid.utils.icon('arrow-up-right', 'xs')}
					</span>`;
				color_class = 'green-stat';
			} else {
				caret_html =
					`<span class="indicator-pill-round red">
						${vmraid.utils.icon('arrow-down-left', 'xs')}
					</span>`;
	...
	get_percentage_stats() {
		return vmraid.xcall('vmraid.desk.doctype.number_card.number_card.get_percentage_difference', {
			doc: this.card_doc,
			filters: this.filters,
			result: this.number
		}).then(res => {
			if (res !== undefined) {
				this.percentage_stat = vmraid.utils.shorten_number(res);
			}
		});
	}
