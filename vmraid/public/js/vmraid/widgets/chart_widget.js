...
export default class ChartWidget extends Widget {
	...
	render_time_series_filters() {
		...
		vmraid.dashboard_utils.render_chart_filters(
			filters,
			"chart-actions",
			this.action_area,
			0
	...
	render_date_range_field() {
		if (
			...
			this.date_range_field = vmraid.ui.form.make_control({
				df: {
					fieldtype: 'DateRange',
					fieldname: 'from_date',
	...
	get_report_chart_data(result) {
		if (result.chart && this.chart_doc.use_report_chart) {
			...
			let columns = result.columns.map(col => {
				return vmraid.report_utils.prepare_field_from_column(col);
			});

			let data = vmraid.report_utils.make_chart_options(
				columns,
				result,
				chart_fields
			).data;
	...
	prepare_chart_actions() {
		...
		if (this.chart_doc.document_type) {
			actions.push({
				...
				action: "action-list",
				handler: () => {
					vmraid.set_route("List", this.chart_doc.document_type);
				}
			});
	...
	setup_filter_dialog(fields) {
		...
		if (this.chart_doc.chart_type == 'Report') {
			...
			vmraid.query_reports[this.chart_doc.report_name].onload
					&& vmraid.query_reports[this.chart_doc.report_name].onload(vmraid.query_report);
		}
	...
	save_chart_config_for_user(config, reset=0) {
		...
		vmraid.xcall('vmraid.desk.doctype.dashboard_settings.dashboard_settings.save_chart_config', {
			'reset': reset,
	...
	create_filter_group_and_add_filters() {
		this.filter_group = new vmraid.ui.FilterGroup({
		...
		this.filters &&
			vmraid.model.with_doctype(this.chart_doc.document_type, () => {
	...
	fetch(filters, refresh = false, args) {
		...
		return vmraid.xcall(method, args);
	}
	...
	render() {
		if (!this.data || !this.data.labels || !Object.keys(this.data).length) {
			...
			if (!this.dashboard_chart) {
				this.dashboard_chart = vmraid.utils.make_chart(this.chart_wrapper[0], chart_args);
			} else {
	...
	update_chart_object() {
		vmraid.db.get_doc("Dashboard Chart", this.chart_doc.name).then(doc => {
			this.chart_doc = doc;
	...
	prepare_chart_object() {
		if (this.chart_doc.type == 'Heatmap' && !this.chart_doc.heatmap_year) {
			this.chart_doc.heatmap_year = vmraid.dashboard_utils.get_year(vmraid.datetime.now_date());
		}
	...
	set_chart_filters() {
		...
		let chart_saved_filters = vmraid.dashboard_utils.get_all_filters(this.chart_doc);

		if (this.chart_doc.chart_type == 'Report') {
			return vmraid.dashboard_utils
				.get_filters_for_chart_type(this.chart_doc).then(filters => {
					...
					this.filters =
					vmraid.utils.parse_array(user_saved_filters) || vmraid.utils.parse_array(this.filters)
						|| vmraid.utils.parse_array(chart_saved_filters);
				});
		} else {
			this.filters =
				vmraid.utils.parse_array(user_saved_filters) || vmraid.utils.parse_array(this.filters)
			...
			.then(chart_doc => {
				if (chart_doc) {
					...
					if (this.chart_doc.chart_type == "Custom") {
						// custom source
						if (
							vmraid.dashboards.chart_sources[this.chart_doc.source]
						) {
							this.settings =
								vmraid.dashboards.chart_sources[
									this.chart_doc.source
								];
							return Promise.resolve();
						} else {
							const method =
								"vmraid.desk.doctype.dashboard_chart_source.dashboard_chart_source.get_config";
							return vmraid
								.xcall(method, { name: this.chart_doc.source })
								.then(config => {
									vmraid.dom.eval(config);
									this.settings =
										vmraid.dashboards.chart_sources[
											this.chart_doc.source
										];
						...
						this.settings = {
							method: "vmraid.desk.doctype.dashboard_chart.dashboard_chart.get"
						};
						return Promise.resolve();
					}
				}
