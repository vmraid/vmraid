...
vmraid.Application = class Application {
	...
	startup() {
		...
		vmraid.ui.keys.add_shortcut({
			...
			action: () => {
				if (vmraid.theme_switcher && vmraid.theme_switcher.dialog.is_visible) {
					vmraid.theme_switcher.hide();
				} else {
	...
	email_password_prompt(email_account,user,i) {
		...
		d.get_input("submit").on("click", function() {
			...
			s.show();
			vmraid.call({
				method: 'vmraid.email.doctype.email_account.email_account.set_email_password',
				args: {
	...
	show_update_available() {
		if (vmraid.boot.sysdefaults.disable_system_update_notification) return;
	...
	setup_copy_doc_listener() {
		$('body').on('paste', (e) => {
			try {
				...
				if (doc.doctype) {
					e.preventDefault();
					const sleep = vmraid.utils.sleep;
	...
	setup_moment() {
		moment.updateLocale('en', {
			week: {
				dow: vmraid.datetime.get_first_day_of_the_week_index(),
			}
		});
		moment.locale("en");
