...
vmraid.ui.form.Toolbar = class Toolbar {
	constructor(opts) {
		$.extend(this, opts);
		this.refresh();
	...
	set_title() {
		if (this.frm.is_new()) {
			...
			if(this.frm.doc.__islocal || title === this.frm.docname || this.frm.meta.autoname==="hash") {
				...
				this.page.$sub_title_area.on('click', (event) => {
					...
					vmraid.utils.copy_to_clipboard(this.frm.docname);
				});
			}
		} else {
		...
		if(this.frm.meta.title_field) {
			vmraid.utils.set_title(title + " - " + this.frm.docname);
		}
	...
	rename_document_title(new_name, new_title, merge=false) {
		...
		let rename_document = () => {
			return vmraid.xcall("vmraid.model.rename_doc.update_document_title", {
				doctype,
				docname,
				new_name,
				title_field,
		...
		return new Promise((resolve, reject) => {
			if (new_title === this.frm.doc[title_field] && new_name === docname) {
				...
				vmraid.confirm(confirm_message, () => {
	...
	set_indicator() {
		var indicator = vmraid.get_indicator(this.frm.doc);
	...
	make_viewers() {
		...
		this.frm.viewers = new vmraid.ui.form.FormViewers({
			frm: this.frm,
	...
	make_menu_items() {
		...
		const is_submittable = vmraid.model.is_submittable(this.frm.doc.doctype);
		...
		if (!is_submittable || docstatus == 1  ||
			...
			if (vmraid.model.can_print(null, me.frm) && !this.frm.meta.issingle) {
		...
		// email
		if(vmraid.model.can_email(null, me.frm) && me.frm.doc.docstatus < 2) {
		...
		// duplicate
		if(in_list(vmraid.boot.user.can_create, me.frm.doctype) && !me.frm.meta.allow_copy) {
		...
		this.page.add_menu_item(__("Copy to Clipboard"), function() {
			vmraid.utils.copy_to_clipboard(JSON.stringify(me.frm.doc));
		}, true);
		...
		if((cint(me.frm.doc.docstatus) != 1) && !me.frm.doc.__islocal
			&& vmraid.model.can_delete(me.frm.doctype)) {
		...
		if(this.can_repeat()) {
			this.page.add_menu_item(__("Repeat"), function(){
				vmraid.utils.new_auto_repeat_prompt(me.frm);
			}, true);
		}
		...
		if(p[CREATE] && !this.frm.meta.issingle) {
			this.page.add_menu_item(__("New {0}", [__(me.frm.doctype)]), function() {
				vmraid.new_doc(me.frm.doctype, true);
			}, true, {
				shortcut: 'Ctrl+B',
				condition: () => !this.frm.is_new()
			});
	...
	make_customize_buttons() {
		if (vmraid.user_roles.includes("System Manager")) {
			...
			if (vmraid.boot.developer_mode===1 && !is_doctype_form) {
				// edit doctype
				this.page.add_menu_item(__("Edit DocType"), () => {
					vmraid.set_route('Form', 'DocType', this.frm.doctype);
				}, true);
			}
		}
	...
	has_workflow() {
		if(this._has_workflow === undefined)
			this._has_workflow = vmraid.get_list("Workflow", {document_type: this.frm.doctype}).length;
	...
	show_linked_with() {
		if(!this.frm.linked_with) {
			this.frm.linked_with = new vmraid.ui.form.LinkedWith({
				frm: this.frm
			});
		}
	...
	set_page_actions(status) {
		...
		if(status === "Edit") {
			...
			if (this.has_workflow()) {
				vmraid.xcall('vmraid.model.workflow.can_cancel_document', {
