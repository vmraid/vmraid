...
class FormTimeline extends BaseTimeline {
		...
		if (this.frm.meta.allow_events_in_timeline) {
			let create_event = () => {
				const args = {
					...
					txt: vmraid.markdown(this.frm.comment_box.get_value())
				};
				return new vmraid.views.InteractionComposer(args);
			};
		...
		if (doc_info.document_email) {
			...
			this.document_email_link_wrapper
				...
				.on("click", e => {
					...
					vmraid.utils.copy_to_clipboard(text);
				});
		}
	}
	...
	render_timeline_items() {
		...
		vmraid.utils.bind_actions_with_object(this.timeline_items_wrapper, this);
	}
	...
	set_document_info() {
		...
		if (this.frm.doc.route && cint(vmraid.boot.website_tracking_enabled)) {
			let route = this.frm.doc.route;
			vmraid.utils.get_page_view_count(route).then((res) => {
	...
	get_user_link(user) {
		const user_display_text = (
			(vmraid.session.user == user ? __("You") : vmraid.user_info(user).fullname) || ''
		).bold();
		return vmraid.utils.get_form_link('User', user, true, user_display_text);
	}
	...
	get_view_timeline_contents() {
		...
		(this.doc_info.views || []).forEach(view => {
			let view_content = `
				<a href="${vmraid.utils.get_form_link('View Log', view.name)}">
	...
	get_communication_timeline_content(doc, allow_reply=true) {
		doc._url = vmraid.utils.get_form_link("Communication", doc.name);
		...
		doc.content = vmraid.dom.remove_script_and_style(doc.content);
		let communication_content = $(vmraid.render_template('timeline_message_box', { doc }));
		if (allow_reply) {
	...
	get_comment_timeline_content(doc) {
		doc.content = vmraid.dom.remove_script_and_style(doc.content);
		const comment_content = $(vmraid.render_template('timeline_message_box', { doc }));
		...
		(this.doc_info.milestones || []).forEach(milestone_log => {
			milestone_timeline_contents.push({
				...
				content: __('{0} changed {1} to {2}', [
					...
					vmraid.meta.get_label(this.frm.doctype, milestone_log.track_field),
	...
	get_custom_timeline_contents() {
		...
		(this.doc_info.additional_timeline_content || []).forEach(custom_item => {
			custom_timeline_contents.push({
				...
				content: custom_item.content || vmraid.render_template(custom_item.template, custom_item.template_data),
			});
		});
	...
	get_energy_point_timeline_contents() {
		...
		(this.doc_info.energy_point_logs || []).forEach(log => {
			...
			energy_point_timeline_contents.push({
				...
				content: vmraid.energy_points.format_form_log(log)
			});
		});
	...
	setup_reply(communication_box, communication_doc) {
		...
		let reply = $(`<a class="action-btn reply">${vmraid.utils.icon('reply', 'md')}</a>`).click(() => {
			this.compose_mail(communication_doc);
		});
		let reply_all = $(`<a class="action-btn reply-all">${vmraid.utils.icon('reply-all', 'md')}</a>`).click(() => {
	...
	compose_mail(communication_doc=null, reply_all=false) {
		const args = {
			doc: this.frm.doc,
			frm: this.frm,
			recipients: communication_doc && communication_doc.sender != vmraid.session.user_email ? communication_doc.sender : this.get_recipient(),
		...
		if (this.frm.doctype === "Communication") {
			...
			const comment_value = vmraid.markdown(this.frm.comment_box.get_value());
			args.message = strip_html(comment_value) ? comment_value : '';
		}

		new vmraid.views.CommunicationComposer(args);
	}
	...
	setup_comment_actions(comment_wrapper, doc) {
		...
		if (vmraid.model.can_delete("Comment") && (
			vmraid.session.user == doc.owner ||
		...
		let edit_button = $();
		let current_user = vmraid.session.user;
	...
	make_editable(container) {
		return vmraid.ui.form.make_control({
			parent: container,
			df: {
				fieldtype: 'Comment',
				fieldname: 'comment',
	...
	update_comment(name, content) {
		return vmraid.xcall('vmraid.desk.form.utils.update_comment', { name, content })
			.then(() => {
	...
	delete_comment(comment_name) {
		vmraid.confirm(__('Delete comment?'), () => {
			return vmraid.xcall("vmraid.client.delete", {
				doctype: "Comment",
				name: comment_name
			}).then(() => {
	...
	copy_link(ev) {
		let doc_link = vmraid.urllib.get_full_url(
			vmraid.utils.get_form_link(this.frm.doctype, this.frm.docname)
		);
		let element_id = $(ev.currentTarget).closest(".timeline-content").attr("id");
		vmraid.utils.copy_to_clipboard(`${doc_link}#${element_id}`);
	}
}
