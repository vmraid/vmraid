...
vmraid.ui.form.Dashboard = class FormDashboard {
	constructor(opts) {
		$.extend(this, opts);
	...
	setup_dashboard_sections() {
		...
		this.heatmap_area = new Section(this.parent, {
			...
			collapsible: 1,
			body_html: `
				<div id="heatmap-${vmraid.model.scrub(this.frm.doctype)}" class="heatmap"></div>
	...
	refresh() {
		this.reset();
		if (this.frm.doc.__islocal || !vmraid.boot.desk_settings.dashboard) {
			return;
		}
	...
	render_links() {
		...
		$(vmraid.render_template('form_links', this.data))
			.appendTo(transactions_area_body);

		if (this.data.reports && this.data.reports.length) {
			$(vmraid.render_template('report_links', this.data))
				.appendTo(transactions_area_body);
		}
	...
	open_report($link) {
		...
		vmraid.route_options[fieldname] = this.frm.doc.name;
		vmraid.set_route("query-report", report);
	}
	...
	open_document_list($link, show_open) {
		...
		if (this.data.internal_links[doctype]) {
			if (names.length) {
				vmraid.route_options = {'name': ['in', names]};
			} else {
				return false;
			}
		} else if (this.data.fieldname) {
			vmraid.route_options = this.get_document_filter(doctype);
			if (show_open && vmraid.ui.notifications) {
				vmraid.ui.notifications.show_open_count_list(doctype);
			}
		}
	...
	set_open_count() {
		...
		var method = this.data.method || 'vmraid.desk.notifications.get_open_count';
		vmraid.call({
			type: "GET",
			method: method,
			args: {
	...
	render_heatmap() {
		if (!this.heatmap) {
			this.heatmap = new vmraid.Chart("#heatmap-" + vmraid.model.scrub(this.frm.doctype), {
				type: 'heatmap',
	...
	render_graph(args) {
		...
		this.chart = new vmraid.Chart('.form-graph', args);
		if (!this.chart) {
			this.hide();
		}
	}
