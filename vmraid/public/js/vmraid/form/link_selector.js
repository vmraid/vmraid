	...
	set_in_grid(value) {
		return new Promise((resolve) => {
			if (this.qty_fieldname) {
				vmraid.prompt({
					...
					let updated = (this.target.frm.doc[this.target.df.fieldname] || []).some(d => {
						if (d[this.fieldname] === value) {
							vmraid.model.set_value(d.doctype, d.name, this.qty_fieldname, data.qty).then(() => {
								vmraid.show_alert(__("Added {0} ({1})", [value, d[this.qty_fieldname]]));
								resolve();
							});
							return true;
						}
				...
				vmraid.model.set_value(d.doctype, d.name, this.dynamic_link_field, this.dynamic_link_reference);
				vmraid.model.set_value(d.doctype, d.name, this.fieldname, value).then(() => {
					vmraid.show_alert(__("{0} {1} added", [this.dynamic_link_reference, value]));
					resolve();
				});
			} else {
				let d = this.target.add_new_row();
