vmraid.ui.form.ControlDate = class ControlDate extends vmraid.ui.form.ControlData {
	...
	set_formatted_input(value) {
		...
		if(should_refresh) {
			this.datepicker.selectDate(vmraid.datetime.str_to_obj(value));
		}
	}
	set_date_options() {
		// webformTODO:
		...
		let lang = 'en';
		vmraid.boot.user && (lang = vmraid.boot.user.language);
		...
		this.date_format = vmraid.defaultDateFormat;
		this.datepicker_options = {
			...
			keyboardNav: false,
			firstDay: vmraid.datetime.get_first_day_of_the_week_index(),
			onSelect: () => {
	...
	get_now_date() {
		return vmraid.datetime.convert_to_system_tz(vmraid.datetime.now_date(true), false).toDate();
	}
	set_t_for_today() {
		var me = this;
		this.$input.on("keydown", function(e) {
			if(e.which===84) { // 84 === t
				if(me.df.fieldtype=='Date') {
					me.set_value(vmraid.datetime.nowdate());
				} if(me.df.fieldtype=='Datetime') {
					me.set_value(vmraid.datetime.now_datetime());
				} if(me.df.fieldtype=='Time') {
					me.set_value(vmraid.datetime.now_time());
				}
				return false;
			}
		});
	...
	parse(value) {
		if(value) {
			return vmraid.datetime.user_to_str(value);
		}
	}
	format_for_input(value) {
		if(value) {
			return vmraid.datetime.str_to_user(value);
		}
		return "";
	}
	validate(value) {
		if(value && !vmraid.datetime.validate(value)) {
			...
			let date_format = sysdefaults && sysdefaults.date_format
				? sysdefaults.date_format : 'yyyy-mm-dd';
			vmraid.msgprint(__("Date {0} must be in format: {1}", [value, date_format]));
			return '';
		}
		return value;
	}
