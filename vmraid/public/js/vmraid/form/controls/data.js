vmraid.provide('vmraid.phone_call');

vmraid.ui.form.ControlData = class ControlData extends vmraid.ui.form.ControlInput {
	...
	make_input() {
		...
		this.$input.on('paste', (e) => {
			...
			if (maxlength && pasted_data.length > maxlength) {
				...
				if (this.frm && vmraid.model.can_write(this.frm.doctype)) {
					let doctype_edit_link = null;
					if (this.frm.meta.custom) {
						doctype_edit_link = vmraid.utils.get_form_link(
							'DocType',
							this.frm.doctype, true,
							__('this form')
						);
					} else {
						doctype_edit_link = vmraid.utils.get_form_link('Customize Form', 'Customize Form', true, null, {
	...
	bind_change_event() {
		...
		if (this.constructor.trigger_change_on_input_event && !this.in_grid()) {
			...
			this.$input.on("input", vmraid.utils.debounce(change_handler, 500));
		}
	}
	setup_autoname_check() {
		if (!this.df.parent) return;
		this.meta = vmraid.get_meta(this.df.parent);
			...
			this.$input.on('keyup', () => {
				...
				if (this.doc && this.doc.__islocal) {
					...
					let timeout = setTimeout(() => {
						...
						vmraid.db.get_value(this.doctype, this.$input.val(),
							'name', (val) => {
								if (val && val.name) {
	...
	validate(v) {
		...
		if(this.df.options == 'Phone') {
			...
			var email_list = vmraid.utils.split_emails(v);
			if (!email_list) {
				return '';
			} else {
...
vmraid.ui.form.ControlReadOnly = vmraid.ui.form.ControlData;
