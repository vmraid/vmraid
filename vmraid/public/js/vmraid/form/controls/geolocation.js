vmraid.provide('vmraid.utils.utils');

vmraid.ui.form.ControlGeolocation = class ControlGeolocation extends vmraid.ui.form.ControlData {
	static horizontal = false

	async make() {
		await vmraid.require(this.required_libs);
		super.make();
	}
	...
	make_wrapper() {
		...
		this.map_id = vmraid.dom.get_unique_id();
		this.map_area = $(
		...
		if (this.frm) {
			...
			$(document).on('vmraid.ui.Dialog:shown', () => {
				this.make_map();
			});
		}
	}
	...
	format_for_input(value) {
		...
		if(value) {
			...
			this.map.setView(vmraid.utils.map_defaults.center, vmraid.utils.map_defaults.zoom);
		}
	...
	bind_leaflet_map() {
		...
		L.Icon.Default.imagePath = '/assets/vmraid/images/leaflet/';
		this.map = L.map(this.map_id);

		L.tileLayer(vmraid.utils.map_defaults.tiles,
			vmraid.utils.map_defaults.options).addTo(this.map);
	}
	...
	bind_leaflet_draw_control() {
		...
		var options = {
			...
			draw: {
				polyline: {
					shapeOptions: {
						color: vmraid.ui.color.get('blue'),
						weight: 10
					}
				},
				polygon: {
					...
					drawError: {
						color: vmraid.ui.color.get('orange'), // Color the shape will turn when intersects
	...
	get required_libs() {
		return [
			"assets/vmraid/js/lib/leaflet/easy-button.css",
			"assets/vmraid/js/lib/leaflet/L.Control.Locate.css",
			"assets/vmraid/js/lib/leaflet/leaflet.draw.css",
			"assets/vmraid/js/lib/leaflet/leaflet.css",
			"assets/vmraid/js/lib/leaflet/leaflet.js",
			"assets/vmraid/js/lib/leaflet/easy-button.js",
			"assets/vmraid/js/lib/leaflet/leaflet.draw.js",
			"assets/vmraid/js/lib/leaflet/L.Control.Locate.js",
		];
	}
};
