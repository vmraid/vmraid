vmraid.ui.form.ControlInput = class ControlInput extends vmraid.ui.form.Control {
	...
	make_wrapper() {
		if(this.only_input) {
			this.$wrapper = $('<div class="form-group vmraid-control">').appendTo(this.parent);
		} else {
			this.$wrapper = $('<div class="vmraid-control">\
	...
	refresh_input() {
		...
		if (me.disp_status != "None") {
			// refresh value
			if (me.frm) {
				me.value = vmraid.model.get_value(me.doctype, me.docname, me.df.fieldname);
			} else if (me.doc) {
	...
	set_disp_area(value) {
		...
		if (this.df.fieldtype === 'Data') {
			value = vmraid.utils.escape_html(value);
		}
		let doc = this.doc || (this.frm && this.frm.doc);
		let display_value = vmraid.format(value, this.df, { no_icon: true, inline: true }, doc);
