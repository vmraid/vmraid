vmraid.ui.form.ControlMultiSelectList = class ControlMultiSelectList extends vmraid.ui.form.ControlData {
	...
	make_input() {
		...
		this.$list_wrapper.on('input', 'input', vmraid.utils.debounce((e) => {
			this.set_options()
				.then(() => {
	...
	set_selectable_items(options) {
		let html = options.map(option => {
			...
			return `<li class="selectable-item ${selected}" data-value="${encoded_value}">
				...
				<div class="multiselect-check">${vmraid.utils.icon('tick', 'xs')}</div>
			</li>`;
		}).join('');
		if (!html) {
