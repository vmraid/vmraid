...
vmraid.ui.form.ControlTable = class ControlTable extends vmraid.ui.form.Control {
	make() {
		super.make();
		...
		this.$wrapper.on('paste', ':text', e => {
			...
			let data = vmraid.utils.csv_to_array(pasted_data, '\t');
					...
					if (!blank_row) {
						...
						row.forEach((value, data_index) => {
							if (fieldnames[data_index]) {
								vmraid.model.set_value(doctype, row_name, fieldnames[data_index], value);
							}
						});
						row_idx++;
						if (data_length >= 10) {
							let progress = i + 1;
							vmraid.show_progress(__('Processing'), progress, data_length, null, true);
						}
					}
				}, 0);
			});
	...
	get_field(field_name) {
		...
		this.grid.meta.fields.some(field => {
			if (vmraid.model.no_value_type.includes(field.fieldtype)) {
				return false;
			}
