...
vmraid.ui.form.ControlIcon = class ControlIcon extends vmraid.ui.form.ControlData {
	make_input() {
	...
	get_all_icons() {
		vmraid.symbols = [];
		$("#vmraid-symbols > symbol[id]").each(function() {
			vmraid.symbols.push(this.id.replace('icon-', ''));
		});
	}
	...
	make_icon_input() {
		...
		if (!this.selected_icon) {
			this.selected_icon = $(`<div class="selected-icon">${vmraid.utils.icon("folder-normal", "md")}</div>`);
