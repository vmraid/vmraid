vmraid.ui.form.ControlCode = class ControlCode extends vmraid.ui.form.ControlText {
	make_input() {
	...
	make_ace_editor() {
		...
		// events
		this.editor.session.on('change', vmraid.utils.debounce(() => {
	...
	load_lib() {
		...
		if (vmraid.boot.developer_mode) {
			this.root_lib_path = '/assets/vmraid/node_modules/ace-builds/src-noconflict/';
		} else {
			this.root_lib_path = '/assets/vmraid/node_modules/ace-builds/src-min-noconflict/';
		}

		this.library_loaded = new Promise(resolve => {
			vmraid.require(this.root_lib_path + 'ace.js', () => {
