...
vmraid.ui.form.ControlTextEditor = class ControlTextEditor extends vmraid.ui.form.ControlCode {
	make_wrapper() {
		super.make_wrapper();
	}
	...
	bind_events() {
		this.quill.on('text-change', vmraid.utils.debounce((delta, oldDelta, source) => {
		...
		$(this.quill.root).on('keydown', (e) => {
			const key = vmraid.ui.keys && vmraid.ui.keys.get_key(e);
	...
	parse(value) {
		...
		return vmraid.dom.remove_script_and_style(value);
	}
