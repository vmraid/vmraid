vmraid.ui.form.ControlSelect = class ControlSelect extends vmraid.ui.form.ControlData {
	...
	set_icon(is_xs_input) {
		const select_icon_html =
			`<div class="select-icon ${is_xs_input ? 'xs' : ''}">
				${vmraid.utils.icon('select', is_xs_input ? 'xs' : 'sm')}
			</div>`;
		if (this.only_input) {
	...
	get_file_attachment_list() {
		if(!this.frm) return;
		var fl = vmraid.model.docinfo[this.frm.doctype][this.frm.docname];
