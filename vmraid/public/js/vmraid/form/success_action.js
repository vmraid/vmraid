...
vmraid.provide('vmraid.success_action');

vmraid.ui.form.SuccessAction = class SuccessAction {
	constructor(form) {
		this.form = form;
		this.load_setting();
	}
	...
	load_setting() {
		this.setting = vmraid.boot.success_action
	...
	show_alert() {
		vmraid.db.get_list(this.form.doctype, {limit: 2})
			.then(result => {
	...
	get default_actions() {
		return {
			'new': {
				label: __('New'),
				action: (frm) => vmraid.new_doc(frm.doctype)
			},
			'print': {
				label: __('Print'),
			...
			'list': {
				label: __('View All'),
				action: (frm) => {
					vmraid.set_route('List', frm.doctype);
				}
			}
		};
	}
