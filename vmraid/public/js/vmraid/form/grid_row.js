...
export default class GridRow {
	...
	set_docfields(update=false) {
		if (this.doc && this.parent_df.options) {
			vmraid.meta.make_docfield_copy_for(this.parent_df.options, this.doc.name, this.docfields);
			const docfields = vmraid.meta.get_docfields(this.parent_df.options, this.doc.name);
			if (update) {
		...
		if(this.grid.is_editable()) {
			if(this.frm) {
				...
				vmraid.run_serially([
					...
					() => {
						vmraid.model.clear_doc(this.doc.doctype, this.doc.name);
	...
	move() {
		...
		vmraid.prompt({
			...
			if (me.doc._sortable === false) {
				vmraid.msgprint(__('Cannot move row'));
				return;
			}
	...
	render_template() {
		...
		this.row_display = $('<div class="row-data sortable-handle template-row"></div>').appendTo(this.row)
			.html(vmraid.render(this.grid.template, {
				doc: this.doc ? vmraid.get_format_helper(this.doc) : null,
				frm: this.frm,
				row: this
			}));
	}
	render_row(refresh) {
		...
		if (!this.row_index && !this.show_search) {
			...
			this.row_index.find('input').on('keyup', vmraid.utils.debounce((e) => {
				let df = {
					fieldtype: "Sr No"
				};
			...
			vmraid.utils.only_allow_num_decimal(this.row_index.find('input'));
		} else {
		...
		if (this.configure_columns) {
			this.configure_columns_button = $(`
				<div class="col grid-static-col col-xs-1 d-flex justify-content-center" style="cursor: pointer;">
					<a>${vmraid.utils.icon('setting-gear', 'sm', '', 'filter: opacity(0.5)')}</a>
				</div>
			`)
				.appendTo(this.row)
				.on('click', () => {
	...
	configure_dialog_for_columns_selector() {
		this.grid_settings_dialog = new vmraid.ui.Dialog({
		...
		d.set_primary_action(__('Add'), () => {
			...
			if (selected_fields) {
				selected_fields.forEach(selected_column => {
					let docfield = vmraid.meta.get_docfield(this.grid.doctype, selected_column);
	...
	prepare_columns_for_dialog(selected_fields) {
		...
		const blocked_fields = vmraid.model.no_value_type;
	...
	render_selected_columns() {
		...
		if (this.selected_columns_for_grid) {
			this.selected_columns_for_grid.forEach(d => {
				let docfield = vmraid.meta.get_docfield(this.grid.doctype, d.fieldname);
					...
					<div class='control-input flex align-center form-control fields_order sortable-handle sortable'
						...
						<div class='row'>
							<div class='col-md-1' style='padding-top: 2px'>
								<a style='cursor: grabbing;'>${vmraid.utils.icon('drag', 'xs')}</a>
							</div>
	...
	update_column_width() {
		$(this.fields_html_wrapper).find('.column-width').change((event) => {
			if (cint(event.target.value) === 0) {
				...
				vmraid.throw(__('Column width cannot be zero.'));
			}
	...
	validate_columns_width() {
		...
		if (total_column_width && total_column_width > 10) {
			vmraid.throw(__('The total column width cannot be more than 10.'));
		}
	}
	...
	remove_selected_column() {
		$(this.fields_html_wrapper).find('.remove-field').click((event) => {
			...
			if (selected_columns_for_grid && selected_columns_for_grid.length === 0) {
				vmraid.throw(__('At least one column is required to show in the grid.'));
			}
	...
	update_user_settings_for_grid() {
		...
		vmraid.model.user_settings.save(this.frm.doctype, 'GridView', value)
			.then((r) => {
				vmraid.model.user_settings[this.frm.doctype] = r.message || r;
				this.grid.reset_grid();
			});
	}

	reset_user_settings_for_grid() {
		vmraid.model.user_settings.save(this.frm.doctype, 'GridView', null)
			.then((r) => {
				vmraid.model.user_settings[this.frm.doctype] = r.message || r;
				this.grid.reset_grid();
			});
	...
	setup_columns() {
		...
		this.grid.visible_columns.forEach((col, ci) => {
			...
			let colsize = col[1];
			let txt = this.doc ?
				vmraid.format(this.doc[df.fieldname], df, null, this.doc) :
				__(df.label);
	...
	make_search_column(df, colsize) {
		...
		$search_input.on('keyup', vmraid.utils.debounce((e) => {
		...
		["Currency", "Float", "Int", "Percent", "Rating"].includes(df.fieldtype) &&
			vmraid.utils.only_allow_num_decimal($search_input);
	...
	make_column(df, colsize, txt, ci) {
		...
		var $col = $('<div class="col grid-static-col col-xs-'+colsize+' '+add_class+'"></div>')
			...
			.appendTo(this.row)
			.on('click', function() {
				if(vmraid.ui.form.editable_row===me) {
					return;
				}
	...
	toggle_editable_row(show) {
		...
		if(this.grid.allow_on_grid_editing() && this.grid.is_editable() && this.doc && show !== false) {
			...
			if(vmraid.ui.form.editable_row
				&& vmraid.ui.form.editable_row !== this) {
			...
			vmraid.ui.form.editable_row = null;
		}
	}
	...
	make_control(column) {
		...
		var field = vmraid.ui.form.make_control({
			df: df,
			parent: parent,
			only_input: true,
			with_link_btn: true,
	...
	set_arrow_keys(field) {
		...
		if (field.$input) {
			field.$input.on('keydown', function(e) {
				var { TAB, UP: UP_ARROW, DOWN: DOWN_ARROW } = vmraid.ui.keyCode;
	...
	show_form() {
		...
		vmraid.dom.freeze("", "dark");
		if (cur_frm) cur_frm.cur_grid = this;
		this.wrapper.addClass("grid-row-open");
		if (!vmraid.dom.is_element_in_viewport(this.wrapper)
			&& !vmraid.dom.is_element_in_modal(this.wrapper)) {
			// -15 offset to make form look visually centered
			vmraid.utils.scroll_to(this.wrapper, true, -15);
		}
	...
	hide_form() {
		...
		if (!vmraid.dom.is_element_in_modal(this.row)) {
			vmraid.utils.scroll_to(this.row, true, 15);
		}
		this.refresh();
	...
	refresh_field(fieldname, txt) {
		...
		if (df && this.doc) {
			txt = vmraid.format(this.doc[fieldname], df, null, this.doc);
		}

		if (!txt && this.frm) {
			txt = vmraid.format(this.doc[fieldname], df, null, this.frm.doc);
		}
	...
	get_visible_columns(blacklist=[]) {
		...
		var visible_columns = $.map(this.docfields, function(df) {
			var visible = !df.hidden && df.in_list_view && me.grid.frm.get_perm(df.permlevel, "read")
				&& !in_list(vmraid.model.layout_fields, df.fieldtype) && !in_list(blacklist, df.fieldname);
