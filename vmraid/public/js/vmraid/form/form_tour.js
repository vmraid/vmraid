vmraid.ui.form.FormTour = class FormTour {
	constructor({ frm }) {
		this.frm = frm;
		this.driver_steps = [];
	}
	...
	init_driver() {
		this.driver = new vmraid.Driver({
			...
			onHighlighted: (step) => {
				...
				if ($input)
					vmraid.utils.sleep(200).then(() => $input.focus());
			}
		});

		vmraid.router.on('change', () => this.driver.reset());
		this.frm.layout.sections.forEach(section => section.collapse(false));
	}

	async init({ tour_name, on_finish }) {
		if (tour_name) {
			this.tour = await vmraid.db.get_doc('Form Tour', tour_name);
		} else {
			const doctype_tour_exists = await vmraid.db.exists('Form Tour', this.frm.doctype);
			if (doctype_tour_exists) {
				this.tour = await vmraid.db.get_doc('Form Tour', this.frm.doctype);
			} else {
				this.tour = { steps: vmraid.tour[this.frm.doctype] };
			}
		}
	...
	get_step(step_info, on_next) {
		...
		let element = `.vmraid-control[data-fieldname='${fieldname}']`;
		...
		if (is_table_field) {
			...
			element = `.grid-row-open .vmraid-control[data-fieldname='${fieldname}']`;
		}

		return {
			element,
			name,
			popover: { title, description, position: vmraid.router.slug(position || 'Bottom') },
			onNext: on_next
		};
	}
	...
	add_new_row_step(step) {
		const $add_row = `.vmraid-control[data-fieldname='${step.fieldname}'] .grid-add-row`;
		const add_row_step = {
			element: $add_row,
	...
	expand_row_and_proceed(step, start_from) {
		...
		vmraid.utils.sleep(300).then(() => this.driver.start(start_from));
	}
	...
	open_first_row_of(fieldname) {
		...
		$($close_row).one('click', () => {
			...
			vmraid.utils.scroll_to(next_element, true, 150, null, () => {
				this.driver.moveNext();
				vmraid.flags.disable_auto_scroll = false;
			});
			vmraid.flags.disable_auto_scroll = true;
		});
	}
	...
	add_step_to_save() {
		...
		vmraid.ui.form.on(this.frm.doctype, 'after_save', () => this.on_finish && this.on_finish());
	}
