...
vmraid.ui.form.States = class FormStates {
	constructor(opts) {
		$.extend(this, opts);
		this.state_fieldname = vmraid.workflow.get_state_fieldname(this.frm.doctype);

		// no workflow?
		if(!this.state_fieldname)
			return;

		this.update_fields = vmraid.workflow.get_update_fields(this.frm.doctype);
	...
	setup_help() {
		...
		this.frm.page.add_action_item(__("Help"), function() {
			vmraid.workflow.setup(me.frm.doctype);
			var state = me.get_state();
			var d = new vmraid.ui.Dialog({
				title: "Workflow: "
					+ vmraid.workflow.workflows[me.frm.doctype].name
			});

			vmraid.workflow.get_transitions(me.frm.doc).then((transitions) => {
				const next_actions = $.map(transitions, d => `${d.action.bold()} ${__("by Role")} ${d.allowed}`)
					.join(", ") || __("None: End of Workflow").bold();

				const document_editable_by = vmraid.workflow.get_document_state(me.frm.doctype, state).allow_edit.bold();
	...
	show_actions() {
		...
		vmraid.workflow.get_transitions(this.frm.doc).then(transitions => {
			this.frm.page.clear_actions_menu();
			transitions.forEach(d => {
				if (vmraid.user_roles.includes(d.allowed) && has_approval_access(d)) {
					added = true;
					me.frm.page.add_action_item(__(d.action), function() {
						// set the workflow_action for use in form scripts
						me.frm.selected_workflow_action = d.action;
						me.frm.script_manager.trigger('before_workflow_action').then(() => {
							vmraid.xcall('vmraid.model.workflow.apply_workflow',
	...
	set_default_state() {
		var default_state = vmraid.workflow.get_default_state(this.frm.doctype, this.frm.doc.docstatus);
		if(default_state) {
