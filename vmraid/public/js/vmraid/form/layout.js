...
vmraid.ui.form.Layout = class Layout {
	constructor (opts) {
		this.views = {};
		this.pages = [];
		this.sections = [];
	...
	show_empty_form_message() {
		if (!(this.wrapper.find(".vmraid-control:visible").length || this.wrapper.find(".section-head.collapsed").length)) {
	...
	get_doctype_fields() {
		...
		if (this.doctype_layout) {
			...
			fields = fields.concat(vmraid.meta.sort_docfields(vmraid.meta.docfield_map[this.doctype]));
		}
	...
	get_fields_from_layout() {
		...
		for (let f of this.doctype_layout.fields) {
			const docfield = copy_dict(vmraid.meta.docfield_map[this.doctype][f.fieldname]);
	...
	init_field (df, render = false) {
		const fieldobj = vmraid.ui.form.make_control({
			df: df,
	...
	make_page (df) { // eslint-disable-line no-unused-vars
		...
		this.fold_btn = head.find(".btn-fold").on("click", function () {
			...
			if (page.hasClass("hide")) {
				...
				vmraid.utils.scroll_to($(this), true, 30);
				me.folded = false;
			} else {
	...
	make_section (df) {
		this.section = new vmraid.ui.form.Section(this, df);
	...
	make_column (df) {
		this.column = new vmraid.ui.form.Column(this.section, df);
		if (df && df.fieldname) {
	...
	refresh_sections() {
		...
		this.wrapper.find(".form-section:not(.hide-control)").each(function() {
			...
			if (section.find(".vmraid-control:not(.hide-control)").length) {
	...
	attach_doc_and_docfields (refresh) {
		...
		for (var i = 0, l = this.fields_list.length; i < l; i++) {
			...
			if (me.doc) {
				...
				fieldobj.df = vmraid.meta.get_docfield(me.doc.doctype,
	...
	focus_on_next_field (start_idx, fields) {
		...
		for (var i = start_idx + 1, len = fields.length; i < len; i++) {
			...
			if (this.is_visible(field)) {
				...
				} else if (!in_list(vmraid.model.no_value_type, field.df.fieldtype)) {
	...
	evaluate_depends_on_value (expression) {
		...
		if (typeof (expression) === 'boolean') {
			...
			try {
				...
				vmraid.throw(__('Invalid "depends_on" expression'));
			}
...
vmraid.ui.form.Section = class FormSection {
	...
	collapse (hide) {
		...
		this.indicator & this.indicator.html(vmraid.utils.icon(indicator_icon, 'sm', 'mb-1'));
...
vmraid.ui.form.Column = class FormColumn {
