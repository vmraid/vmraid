...
vmraid.ui.form.AssignTo = class AssignTo {
	constructor(opts) {
		$.extend(this, opts);
	...
	render(assignments) {
		...
		let avatar_group = vmraid.avatar_group(assigned_users, 5, {'align': 'left', 'overlap': true});

		assignments_wrapper.show();
		assignments_wrapper.append(avatar_group);
		avatar_group.click(() => {
			new vmraid.ui.form.AssignmentDialog({
	...
	add() {
		...
		if (this.frm.is_new()) {
			vmraid.throw(__("Please save the document before assignment"));
			return;
		}

		if (!me.assign_to) {
			me.assign_to = new vmraid.ui.form.AssignToDialog({
	...
	remove(owner) {
		if (this.frm.is_new()) {
			vmraid.throw(__("Please save the document before removing assignment"));
			return;
		}

		return vmraid.xcall('vmraid.desk.form.assign_to.remove', {
...
vmraid.ui.form.AssignToDialog = class AssignToDialog {
	constructor(opts) {
		$.extend(this, opts);
	...
	make() {
		...
		me.dialog = new vmraid.ui.Dialog({
	...
	assign_to_me() {
		...
		if (me.dialog.get_value("assign_to_me")) {
			assign_to.push(vmraid.session.user);
		}
		...
		return [
			...
			},
			{
				...
				get_data: function(txt) {
					return vmraid.db.get_link_options("User", txt, {user_type: "System User", enabled: 1});
				}
			},
			{
				label: __("Comment"),
...
vmraid.ui.form.AssignmentDialog = class {
	constructor(opts) {
		this.frm = opts.frm;
	...
	make() {
		...
		this.assignment_list.removeClass('vmraid-control');
	...
	add_assignment(assignment) {
		return vmraid.xcall('vmraid.desk.form.assign_to.add', {
			doctype: this.frm.doctype,
			name: this.frm.docname,
	...
	get_assignment_row(assignment) {
		let row = $(`
			<div class="dialog-assignment-row" data-user="${assignment}">
				<span>
					${vmraid.avatar(assignment)}
					${vmraid.user.full_name(assignment)}
				</span>
			</div>
		`);

		if (assignment === vmraid.session.user || this.frm.perm[0].write) {
			row.append(`
				<span class="remove-btn cursor-pointer">
					${vmraid.utils.icon('close')}
				</span>
			`);
