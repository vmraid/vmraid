...
vmraid.ui.form.Share = class Share {
	constructor(opts) {
		$.extend(this, opts);
	...
	render_sidebar() {
		...
		this.shares.append(vmraid.avatar_group(shared_users, 5, {'align': 'left', 'overlap': true}));
	}
	show() {
		var me = this;
	...
	render_shared(shared) {
		...
		$(vmraid.render_template("set_sharing", {frm: this.frm, shared: this.shared, everyone: everyone}))
			.appendTo(d.body);

		if(vmraid.model.can_share(null, this.frm)) {
	...
	make_user_input() {
		// make add-user input
		this.dialog.share_with = vmraid.ui.form.make_control({
	...
	set_edit_share_events() {
		...
		$(d.body).find(".edit-share").on("click", function() {
			...
			vmraid.call({
				method: "vmraid.share.set_permission",
				args: {
					doctype: me.frm.doctype,
					name: me.frm.doc.name,
					user: user,
