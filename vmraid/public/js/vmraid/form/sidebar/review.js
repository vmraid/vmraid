...
vmraid.ui.form.Review = class Review {
	constructor({ parent, frm }) {
		this.parent = parent;
		this.frm = frm;
		this.points = vmraid.boot.points;
		this.reviews = this.parent.find('.reviews');
		this.setup_add_review_button();
		this.update_reviewers();
	}
	update_points() {
		return vmraid.xcall('vmraid.social.doctype.energy_point_log.energy_point_log.get_energy_points', {
			user: vmraid.session.user
		}).then(data => {
			vmraid.boot.points = data;
			this.points = data;
		});
	}
	...
	get_involved_users() {
		...
		return involved_users
			.uniqBy(u => u)
			.filter(user => !['Administrator', vmraid.session.user].includes(user))
			.filter(Boolean);
	}
	show_review_dialog() {
		const user_options = this.get_involved_users();
		const review_dialog = new vmraid.ui.Dialog({
			'title': __('Add Review'),
			'fields': [{
				fieldname: 'to_user',
			...
			primary_action: (values) => {
				...
				if (values.points > this.points.review_points) {
					return vmraid.msgprint(__('You do not have enough points'));
				}
				vmraid.xcall('vmraid.social.doctype.energy_point_log.energy_point_log.review', {
					doc: {
	...
	setup_detail_popover(el, data) {
		let subject = '';
		let fullname = vmraid.user.full_name(data.user);
		let timestamp = `<span class="text-muted">${vmraid.datetime.comment_when(data.creation)}</span>`;
			...
			content: () => {
				return `
					<div class="text-medium">
						...
						<div class="subject">
							${vmraid.utils.icon('review')}
							${subject}

							<p class="mt-1">
								<!-- ${vmraid.avatar("shivam@example.com", "avatar-xs")} -->
								<span>${vmraid.user.full_name(data.owner)}</span> - ${timestamp}
							</p>
						</div>
					</div>
				`;
