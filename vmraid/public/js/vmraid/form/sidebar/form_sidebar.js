...
vmraid.ui.form.Sidebar = class {
	constructor(opts) {
		$.extend(this, opts);
	}

	make () {
		var sidebar_content = vmraid.render_template("form_sidebar", {doctype: this.frm.doctype, frm:this.frm});
		...
		vmraid.ui.form.setup_user_image_event(this.frm);
	...
	bind_events () {
		...
		this.comments.on("click", function() {
			vmraid.utils.scroll_to(me.frm.footer.wrapper.find(".comment-box"), true);
		});

		this.like_icon.on("click", function() {
			vmraid.ui.toggle_like(me.like_wrapper, me.frm.doctype, me.frm.doc.name, function() {
				me.refresh_like();
			});
		});
	}
	...
	refresh () {
		if (this.frm.doc.__islocal) {
			...
			this.sidebar
				...
				.html(
					__("{0} edited this {1}", [
						vmraid.user.full_name(this.frm.doc.modified_by).bold(),
			...
			this.sidebar
				...
				.html(
					__("{0} created this {1}", [
						vmraid.user.full_name(this.frm.doc.owner).bold(),
			...
			vmraid.ui.form.set_user_image(this.frm);
		}
	}
			...
			vmraid.call({
				...
				callback: function(res) {
					...
					me.sidebar.find(".auto-repeat-status").on("click", function(){
						vmraid.set_route("Form", "Auto Repeat", me.frm.doc.auto_repeat);
					});
				}
			});
		}
	...
	make_tags() {
		...
		this.frm.tags = new vmraid.ui.TagEditor({
			parent: tags_parent,
	...
	make_attachments() {
		var me = this;
		this.frm.attachments = new vmraid.ui.form.Attachments({
	...
	make_assignments() {
		this.frm.assign_to = new vmraid.ui.form.AssignTo({
	...
	make_shared() {
		this.frm.shared = new vmraid.ui.form.Share({
			frm: this.frm,
	...
	make_like() {
		...
		vmraid.ui.setup_like_popover(this.sidebar.find(".form-stats-likes"), ".like-icon");
	}
	...
	make_follow() {
		...
		this.follow_button.on('click', () => {
			...
			vmraid.call('vmraid.desk.form.document_follow.update_follow', {
				'doctype': this.frm.doctype,
				'doc_name': this.frm.doc.name,
				'following': !is_followed
			}).then(() => {
				vmraid.model.set_docinfo(this.frm.doctype, this.frm.doc.name, "is_document_followed", !is_followed);
	...
	refresh_like() {
		...
		const liked = vmraid.ui.is_liked(this.frm.doc);
		this.like_wrapper
	...
	make_review() {
		...
		if (vmraid.boot.energy_points_enabled && !this.frm.is_new()) {
			this.frm.reviews = new vmraid.ui.form.Review({
	...
	reload_docinfo(callback) {
		vmraid.call({
			method: "vmraid.desk.form.load.get_docinfo",
			args: {
				doctype: this.frm.doctype,
