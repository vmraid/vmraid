...
vmraid.ui.form.get_open_grid_form = function () {
	return $(".grid-row-open").data("grid_row");
}

vmraid.ui.form.close_grid_form = function () {
	var open_form = vmraid.ui.form.get_open_grid_form();
	open_form && open_form.hide_form();

	// hide editable row too
	if (vmraid.ui.form.editable_row) {
		vmraid.ui.form.editable_row.toggle_editable_row(false);
	}
}
...
export default class Grid {
	constructor(opts) {
		...
		this.debounced_refresh = vmraid.utils.debounce(this.debounced_refresh, 100);
	}

	allow_on_grid_editing() {
		if (vmraid.utils.is_xs()) {
			return false;
	...
	make() {
			...
			<div class="form-grid">
				...
				<div class="grid-body">
					...
					<div class="grid-empty text-center">
						<img
							src="/assets/vmraid/images/ui-states/grid-empty-state.svg"
		...
		vmraid.utils.bind_actions_with_object(this.wrapper, this);
	...
	delete_all_rows() {
		vmraid.confirm(__("Are you sure you want to delete all rows?"), () => {
	...
	scroll_to_top() {
		vmraid.utils.scroll_to(this.wrapper);
	}
	...
	refresh() {
		...
		if (this.frm) {
			this.display_status = vmraid.perm.get_field_display_status(this.df, this.frm.doc,
				this.perm);
	...
	setup_fields() {
		...
		if (this.frm && this.frm.docname) {
			...
			this.df = vmraid.meta.get_docfield(this.frm.doctype, this.df.fieldname,
				this.frm.docname);
		} else {
			// use non-doc specific docfield
			if (this.df.options) {
				this.df = vmraid.meta.get_docfield(this.df.options, this.df.fieldname) || this.df || null;
			}
		}

		if (this.doctype && this.frm) {
			this.docfields = vmraid.meta.get_docfields(this.doctype, this.frm.docname);
		} else {
	...
	get_data_based_on_fieldtype(df, data, value) {
		...
		if (fieldtype === "Check") {
			value = vmraid.utils.string_to_boolean(value);
			...
			let formatted_duration = vmraid.utils.get_formatted_duration(fieldvalue);
			...
			let user_formatted_date = vmraid.datetime.str_to_user(fieldvalue);
				...
				row.columns_list.forEach(column => {
					...
					if (column.df.fieldname == fieldname) {
						if (show) {
							...
							if (row != vmraid.ui.form.editable_row) {
	...
	get_docfield(fieldname) {
		return vmraid.meta.get_docfield(this.doctype, fieldname, this.frm ? this.frm.docname : null);
	}
	...
	add_new_row(idx, callback, show, copy_doc, go_to_last_page = false, go_to_first_page = false) {
		if (this.is_editable()) {
			...
			if (this.frm) {
				var d = vmraid.model.add_child(this.frm.doc, this.df.options, this.df.fieldname, idx);
				if (copy_doc) {
	...
	setup_visible_columns() {
		...
		for (var ci in fields) {
			...
			if (df && !df.hidden
				...
				&& !in_list(vmraid.model.layout_fields, df.fieldtype)) {
				...
				if (df.fieldtype == 'Link' && !df.formatter && df.parent && vmraid.meta.docfield_map[df.parent]) {
					const docfield = vmraid.meta.docfield_map[df.parent][df.fieldname];
	...
	setup_user_defined_columns() {
		...
		let user_settings = vmraid.get_user_settings(this.frm.doctype, 'GridView');
		if (user_settings && user_settings[this.doctype] && user_settings[this.doctype].length) {
			this.user_defined_columns = user_settings[this.doctype].map(row => {
				let column = vmraid.meta.get_docfield(this.doctype, row.fieldname);
	...
	set_multiple_add(link, qty) {
		...
		var link_field = vmraid.meta.get_docfield(this.df.options, link);
	...
	setup_allow_bulk_edit() {
		...
		if (this.frm && this.frm.get_docfield(this.df.fieldname).allow_bulk_edit) {
			...
			const value_formatter_map = {
				"Date": val => val ? vmraid.datetime.user_to_str(val) : val,
				"Int": val => cint(val),
				"Check": val => cint(val),
				"Float": val => flt(val),
			};
			...
			// upload
			vmraid.flags.no_socketio = true;
			$(this.wrapper).find(".grid-upload").removeClass('hidden').on("click", () => {
				new vmraid.ui.FileUploader({
					as_dataurl: true,
					allow_multiple: false,
					on_success(file) {
						var data = vmraid.utils.csv_to_array(vmraid.utils.get_decoded_string(file.dataurl));
							...
							if (i > 6) {
								...
								if (!blank_row) {
									...
									$.each(row, (ci, value) => {
										...
										var df = vmraid.meta.get_docfield(me.df.options, fieldname);
										if (df) {
						...
						vmraid.msgprint({ message: __('Table updated'), title: __('Success'), indicator: 'green' });
					}
				});
				return false;
			});
	...
	setup_download() {
		let title = this.df.label || vmraid.model.unscrub(this.df.fieldname);
		$(this.wrapper).find(".grid-download").removeClass('hidden').on("click", () => {
			...
			data.push(["------"]);
			$.each(vmraid.get_meta(this.df.options).fields, (i, df) => {
				// don't include the read-only field in the template
				if (vmraid.model.is_value_type(df.fieldtype)) {
					data[1].push(df.label);
					data[2].push(df.fieldname);
					let description = (df.description || "") + ' ';
					if (df.fieldtype === "Date") {
						description += vmraid.boot.sysdefaults.date_format;
					}
			...
			vmraid.tools.downloadify(data, null, title);
			return false;
		});
	}
