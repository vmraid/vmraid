...
vmraid.ui.form.LinkedWith = class LinkedWith {
	...
	load_doctypes() {
		...
		const promises = doctypes_to_load.map(dt => {
			return vmraid.model.with_doctype(dt, () => {
				if(vmraid.listview_settings[dt]) {
					// add additional fields to __linked_doctypes
					this.frm.__linked_doctypes[dt].add_fields =
						vmraid.listview_settings[dt].add_fields;
				}
			});
		});
	...
	links_not_permitted_or_missing() {
		...
		if (this.frm.__linked_doctypes) {
			links =
				Object.keys(this.frm.__linked_doctypes)
					.filter(vmraid.model.can_get_report);
		}
	...
	get_linked_docs() {
		return vmraid.call({
			method: "vmraid.desk.form.linked_with.get_linked_docs",
			args: {
				doctype: this.frm.doctype,
		...
		return `<div class="list-row-container">
			<div class="level list-row small">
				<div class="level-left bold">
					<a href="/app/${vmraid.router.slug(doctype)}/${doc.name}">${doc.name}</a>
				</div>
			</div>
		</div>`;
	}
