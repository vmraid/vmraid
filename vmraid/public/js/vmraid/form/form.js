...
vmraid.ui.form.Form = class VMRaidForm {
	...
	setup() {
		...
		if (!this.meta.hide_toolbar && vmraid.boot.desk_settings.timeline) {
			// this.footer_tab = new vmraid.ui.form.Tab(this.layout, {
			// 	label: __("Activity"),
			// 	fieldname: 'timeline'
			// });
	...
	setup_std_layout() {
		...
		this.dashboard = new vmraid.ui.form.Dashboard(dashboard_parent, this);

		this.tour = new vmraid.ui.form.FormTour({
			frm: this
		});
	...
	watch_model_updates() {
		...
		// on main doc
		vmraid.model.on(me.doctype, "*", function(fieldname, value, doc, skip_dirty_trigger=false) {
			// set input
		...
		this.fields.map(field => {
			if (field.df.fieldtype === 'Markdown Editor') {
				this.set_df_property(field.df.fieldname, 'autocompletions', () => {
					...
					return attachments
						.filter(file => vmraid.utils.is_image_file(file.file_url))
						.map(file => {
							return {
	...
	execute_action(action) {
		...
		if (action.action_type==='Server Action') {
			return vmraid.xcall(action.action, {'doc': this.doc}).then((doc) => {
				if (doc.doctype) {
			...
			return vmraid.set_route(action.action);
		}
	}
	...
	rename_notify(dt, old, name) {
		...
		!vmraid._from_link && vmraid.set_route('Form', this.doctype, name);
	}
	...
	reload_doc() {
		...
		if(!this.doc.__islocal) {
			...
			return vmraid.model.with_doc(this.doctype, this.docname, () => {
				this.refresh();
			});
		}
	}
	...
	scroll_to_element() {
		if (vmraid.route_options && vmraid.route_options.scroll_to) {
			...
			vmraid.utils.scroll_to(window.location.hash, true, 200, null, null, true);
		}
	}
			...
			if(fieldobj) {
				if(!if_missing || !vmraid.model.has_value(me.doctype, me.doc.name, f)) {
					if(vmraid.model.table_fields.includes(fieldobj.df.fieldtype) && $.isArray(v)) {
						...
						return vmraid.model.set_value(me.doctype, me.doc.name, f, v, me.fieldtype, skip_dirty_trigger);
					}
				}
			} else {
				...
				if($.isPlainObject(r.message)) {
					if(opts.child) {
						...
						var std_field_list = ["doctype"]
							.concat(vmraid.model.std_fields_list)
							.concat(vmraid.model.child_table_field_list);
	...
	make_new(doctype) {
		...
		if(this.make_methods && this.make_methods[doctype]) {
			...
			vmraid.model.with_doctype(doctype, function() {
				let new_doc = vmraid.model.get_new_doc(doctype, null, null, true);
	...
	update_in_all_rows(table_fieldname, fieldname, value) {
		...
		vmraid.model
			...
			.filter(child => !vmraid.model.has_value(child.doctype, child.name, fieldname))
			.forEach(child =>
				vmraid.model.set_value(child.doctype, child.name, fieldname, value)
			);
	}
