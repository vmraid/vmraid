...
vmraid.provide("vmraid.ui.form.handlers");
...
vmraid.ui.form.get_event_handler_list = function(doctype, fieldname) {
	if(!vmraid.ui.form.handlers[doctype]) {
		vmraid.ui.form.handlers[doctype] = {};
	}
	if(!vmraid.ui.form.handlers[doctype][fieldname]) {
		vmraid.ui.form.handlers[doctype][fieldname] = [];
	}
	return vmraid.ui.form.handlers[doctype][fieldname];
}

vmraid.ui.form.on = vmraid.ui.form.on_change = function(doctype, fieldname, handler) {
	var add_handler = function(fieldname, handler) {
		var handler_list = vmraid.ui.form.get_event_handler_list(doctype, fieldname);
...
vmraid.ui.form.off = function(doctype, fieldname, handler) {
	var handler_list = vmraid.ui.form.get_event_handler_list(doctype, fieldname);
...
vmraid.ui.form.trigger = function(doctype, fieldname) {
	cur_frm.script_manager.trigger(fieldname, doctype);
}

vmraid.ui.form.ScriptManager = class ScriptManager {
	constructor(opts) {
		$.extend(this, opts);
	}
	...
	trigger(event_name, doctype, name) {
		...
		this.frm.selected_doc = vmraid.get_doc(doctype, name);
	...
	get_handlers(event_name, doctype) {
		...
		if(vmraid.ui.form.handlers[doctype] && vmraid.ui.form.handlers[doctype][event_name]) {
			$.each(vmraid.ui.form.handlers[doctype][event_name], function(i, fn) {
	...
	setup() {
		...
		$.each(this.frm.fields, function(i, field) {
			...
			if(vmraid.model.table_fields.includes(field.df.fieldtype)) {
				$.each(vmraid.meta.get_docfields(field.df.options, me.frm.docname), function(i, df) {
					setup_add_fetch(df);
				});
			}
		});
		...
		// css
		doctype.__css && vmraid.dom.set_style(doctype.__css);

		this.trigger('setup');
	}
	log_error(caller, e) {
		vmraid.show_alert({message: __("Error in Client Script."), indicator: "error"});
	...
	copy_from_first_row(parentfield, current_row, fieldnames) {
		...
		$.each(fieldnames, function(i, fieldname) {
			vmraid.model.set_value(current_row.doctype, current_row.name, fieldname,
				data[0][fieldname]);
		});
	}
};
