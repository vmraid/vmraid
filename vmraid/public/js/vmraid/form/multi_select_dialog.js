vmraid.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (!this.for_select) {
			vmraid.model.with_doctype(this.doctype, () => this.init());
		} else {
			this.init();
		}
	}
	...
	make_new_document(e) {
		...
		if (e) {
			...
			vmraid.new_doc(this.doctype, true);
		}
	}
	...
	set_route_options() {
		...
		if (Array.isArray(this.setters)) {
			for (let df of this.setters) {
				vmraid.route_options[df.fieldname] = this.dialog.fields_dict[df.fieldname].get_value() || undefined;
			}
		} else {
			Object.keys(this.setters).forEach(setter => {
				vmraid.route_options[setter] = this.dialog.fields_dict[setter].get_value() || undefined;
			});
		}
	}
	...
	get_child_datatable_columns() {
		...
		return [parent, ...this.child_columns].map(d => ({ name: vmraid.unscrub(d), editable: false }));
	}
	...
	setup_child_datatable() {
		...
		this.child_datatable = new vmraid.DataTable(this.$child_wrapper.get(0), {
	...
	get_primary_filters() {
		...
		if ($.isArray(this.setters)) {
			...
			Object.keys(this.setters).forEach((setter, index) => {
				let df_prop = vmraid.meta.docfield_map[this.doctype][setter];
		...
		if (this.allow_child_item_selection) {
			this.child_doctype = vmraid.meta.get_docfield(this.doctype, this.child_fieldname).options;
			columns[0].push({
				fieldtype: "Check",
	...
	bind_events() {
		...
		this.$parent.find('.input-with-feedback').on('change', () => {
			vmraid.flags.auto_scroll = false;
			if (this.is_child_selection_enabled()) {
				this.show_child_results();
			} else {
				this.get_results();
	...
	make_list_row(result = {}) {
		...
		this.get_datatable_columns().forEach(function (column) {
			contents += `<div class="list-item__content ellipsis">
				${
	head ? `<span class="ellipsis text-muted" title="${__(vmraid.model.unscrub(column))}">${__(vmraid.model.unscrub(column))}</span>`
		: (column !== "name" ? `<span class="ellipsis result-row" title="${__(result[column] || '')}">${__(result[column] || '')}</span>`
			: `<a href="${"/app/" + vmraid.router.slug(me.doctype) + "/" + result[column] || ''}" class="list-id ellipsis" title="${__(result[column] || '')}">
							${__(result[column] || '')}</a>`)}
			</div>`;
		});
	...
	render_result_list(results, more = 0, empty = true) {
		...
		if (!vmraid.flags.auto_scroll && empty) {
			this.empty_list();
		}
		more_btn.hide();
