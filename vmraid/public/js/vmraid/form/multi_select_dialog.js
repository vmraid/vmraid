vmraid.ui.form.MultiSelectDialog = class MultiSelectDialog {
	constructor(opts) {
		...
		if (this.doctype != "[Select]") {
			vmraid.model.with_doctype(this.doctype, function () {
				me.make();
			});
		} else {
			this.make();
				...
				if (e) {
					...
					if (Array.isArray(me.setters)) {
						for (let df of me.setters) {
							vmraid.route_options[df.fieldname] = me.dialog.fields_dict[df.fieldname].get_value() || undefined;
						}
					} else {
						Object.keys(me.setters).forEach(function (setter) {
							vmraid.route_options[setter] = me.dialog.fields_dict[setter].get_value() || undefined;
						});
					}

					vmraid.new_doc(me.doctype, true);
				}
			}
		});
