vmraid.RoleEditor = class {
	constructor(wrapper, frm, disable) {
		...
		this.multicheck = vmraid.ui.form.make_control({
			...
			df: {
				...
				columns: 3,
				get_data: () => {
					return vmraid.xcall('vmraid.core.doctype.user.user.get_all_roles').then(roles => {
	...
	show_permissions(role) {
		...
		return vmraid.xcall('vmraid.core.doctype.user.user.get_perm_info', { role })
			.then(permissions => {
						...
						<table class="user-perm">
							<thead>
								<tr>
									...
									<th> ${__('Level')} </th>
									${vmraid.perm.rights.map(p => `<th> ${vmraid.unscrub(p)}</th>`).join("")}
								</tr>
							</thead>
							<tbody></tbody>
						</table>
					...
					permissions.forEach(perm => {
						$body.find('tbody').append(`
							<tr>
								...
								${vmraid.perm.rights.map(p => `<td class="text-muted bold">${perm[p] ? vmraid.utils.icon('check', 'xs') : '-'}</td>`).join("")}
							</tr>
						`);
					});
				}
	...
	make_perm_dialog() {
		this.perm_dialog = new vmraid.ui.Dialog({
	...
	set_roles_in_table() {
		...
		roles.map(role_doc => {
			if (!checked_options.includes(role_doc.role)) {
				vmraid.model.clear_doc(role_doc.doctype, role_doc.name);
			}
		});
		checked_options.map(role => {
			if (!roles.find(d => d.role === role)) {
				let role_doc = vmraid.model.add_child(this.frm.doc, "Has Role", "roles");
				role_doc.role = role;
			}
		});
	}
