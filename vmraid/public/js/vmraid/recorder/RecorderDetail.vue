...
export default {
	...
	methods: {
		...
		refresh: function() {
			vmraid.call("vmraid.recorder.get").then( r => this.requests = r.message);
		},
		update: function(message) {
			this.requests.push(JSON.parse(message));
		},
		clear: function() {
			vmraid.call("vmraid.recorder.delete").then(r => this.refresh());
		},
		start: function() {
			vmraid.call("vmraid.recorder.start").then(r => this.fetch_status());
		},
		stop: function() {
			vmraid.call("vmraid.recorder.stop").then(r => this.fetch_status());
		},
		fetch_status: function() {
			vmraid.call("vmraid.recorder.status").then(r => this.update_status(r.message));
		},
		update_status: function(result) {
			...
			if(this.status.status == "Active") {
				vmraid.realtime.on("recorder-dump-event", this.update);
			} else {
				vmraid.realtime.off("recorder-dump-event", this.update);
			}
		...
		export_data: function() {
			...
			vmraid.call("vmraid.recorder.export_data")
				.then((r) => {
