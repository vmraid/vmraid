...
export default {
	...
	mounted() {
		this.doc_select = vmraid.ui.form.make_control({
		...
		this.preview_type = vmraid.ui.form.make_control({
	...
	methods: {
		...
		get_default_docname() {
			return vmraid.db.get_list(this.doctype, { limit: 1 }).then(doc => {
	...
	computed: {
		...
		url() {
			...
			let url =
				this.type == "PDF"
					? `/api/method/vmraid.utils.weasyprint.download_pdf`
					: "/printpreview";
