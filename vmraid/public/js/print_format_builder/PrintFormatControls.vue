...
export default {
	...
	mounted() {
		let method =
			"vmraid.printing.page.print_format_builder_beta.print_format_builder_beta.get_google_fonts";
	...
	methods: {
		...
		clone_field(df) {
			...
			if (cloned.custom) {
				...
				cloned.fieldname += "_" + vmraid.utils.get_random(8);
			}
			return cloned;
		}
	},
		...
		print_templates() {
			...
			for (let template of templates) {
				...
				if (template.field) {
					...
					df = {
						label: template.name,
						fieldname: vmraid.scrub(template.name)
					};
				}
				out.push({
...
.control-font >>> .vmraid-control[data-fieldname="font"] label {
	display: none;
}
</style>
