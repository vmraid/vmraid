...
from vmraid.utils import cstr, getdate, split_emails, add_days, today, get_last_day, get_first_day, month_diff
from vmraid.model.document import Document
from vmraid.core.doctype.communication.email import make
from vmraid.utils.background_jobs import get_jobs
from vmraid.automation.doctype.assignment_rule.assignment_rule import get_repeated
from vmraid.contacts.doctype.contact.contact import get_contacts_linked_from
from vmraid.contacts.doctype.contact.contact import get_contacts_linking_to
...
class AutoRepeat(Document):
	...
	def after_save(self):
		vmraid.get_doc(self.reference_doctype, self.reference_document).notify_update()

	def on_trash(self):
		vmraid.db.set_value(self.reference_doctype, self.reference_document, 'auto_repeat', '')
		vmraid.get_doc(self.reference_doctype, self.reference_document).notify_update()
	...
	def validate_reference_doctype(self):
		if vmraid.flags.in_test or vmraid.flags.in_patch:
			return
		if not vmraid.get_meta(self.reference_doctype).allow_auto_repeat:
			vmraid.throw(_("Enable Allow Auto Repeat for the doctype {0} in Customize Form").format(self.reference_doctype))

	def validate_submit_on_creation(self):
		if self.submit_on_creation and not vmraid.get_meta(self.reference_doctype).is_submittable:
			vmraid.throw(_('Cannot enable {0} for a non-submittable doctype').format(
				vmraid.bold('Submit on Creation')))
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			vmraid.throw(_('{0} should not be same as {1}').format(vmraid.bold('End Date'), vmraid.bold('Start Date')))
			...
			if self.recipients:
				...
				for email in email_list:
					if not validate_email_address(email):
						vmraid.throw(_("{0} is an invalid email address in 'Recipients'").format(email))
			else:
				vmraid.throw(_("'Recipients' not specified"))
	...
	def validate_auto_repeat_days(self):
		...
		if not len(set(auto_repeat_days)) == len(auto_repeat_days):
			...
			vmraid.throw(_('Auto Repeat Day {0} has been repeated.').format(vmraid.bold(repeated_days)))

	def update_auto_repeat_id(self):
		#check if document is already on auto repeat
		auto_repeat = vmraid.db.get_value(self.reference_doctype, self.reference_document, "auto_repeat")
		if auto_repeat and auto_repeat != self.name and not vmraid.flags.in_patch:
			vmraid.throw(_("The {0} is already on auto repeat {1}").format(self.reference_document, auto_repeat))
		else:
			vmraid.db.set_value(self.reference_doctype, self.reference_document, "auto_repeat", self.name)
	...
	def create_documents(self):
		try:
			...
			error_log = vmraid.log_error(vmraid.get_traceback(), _("Auto Repeat Document Creation Failure"))

			self.disable_auto_repeat()

			if self.reference_document and not vmraid.flags.in_test:
				self.notify_error_to_user(error_log)

	def make_new_document(self):
		reference_doc = vmraid.get_doc(self.reference_doctype, self.reference_document)
		new_doc = vmraid.copy_doc(reference_doc, ignore_no_copy = False)
	...
	def update_doc(self, new_doc, reference_doc):
		...
		auto_repeat_doc = vmraid.get_doc('Auto Repeat', self.name)
	...
	def set_auto_repeat_period(self, new_doc):
		...
		if mcount and new_doc.meta.get_field('from_date') and new_doc.meta.get_field('to_date'):
			last_ref_doc = vmraid.db.get_all(doctype = self.reference_doctype,
	...
	def send_notification(self, new_doc):
		...
		if not self.subject:
			...
			subject = vmraid.render_template(self.subject, {'doc': new_doc})

		print_format = self.print_format or 'Standard'
		error_string = None

		try:
			attachments = [vmraid.attach_print(new_doc.doctype, new_doc.name,
			...
			error_string += _("{0}: Failed to attach new recurring document. To enable attaching document in the auto repeat notification email, enable {1} in Print Settings").format(
				vmraid.bold(_('Note')),
				vmraid.bold(_('Allow Print for Draft'))
			)
			attachments = '[]'
		...
		if error_string:
			...
			message = vmraid.render_template(self.message, {'doc': new_doc})
	...
	def fetch_linked_contacts(self):
		if self.reference_doctype and self.reference_document:
			...
			if not email_ids:
				vmraid.msgprint(_('No contacts linked to document'), alert=True)
			else:
				self.recipients = ', '.join(email_ids)

	def disable_auto_repeat(self):
		vmraid.db.set_value('Auto Repeat', self.name, 'disabled', 1)
	...
	def notify_error_to_user(self, error_log):
		...
		form_link = vmraid.utils.get_link_to_form(self.reference_doctype, self.reference_document)
		auto_repeat_failed_for = _('Auto Repeat failed for {0}').format(form_link)

		error_log_link = vmraid.utils.get_link_to_form('Error Log', error_log.name)
...
def make_auto_repeat_entry():
	enqueued_method = 'vmraid.automation.doctype.auto_repeat.auto_repeat.create_repeated_entries'
	jobs = get_jobs()

	if not jobs or enqueued_method not in jobs[vmraid.local.site]:
		date = getdate(today())
		data = get_auto_repeat_entries(date)
		vmraid.enqueue(enqueued_method, data=data)


def create_repeated_entries(data):
	for d in data:
		doc = vmraid.get_doc('Auto Repeat', d.name)
		...
		if schedule_date == current_date and not doc.disabled:
			...
			if schedule_date and not doc.disabled:
				vmraid.db.set_value('Auto Repeat', doc.name, 'next_schedule_date', schedule_date)


def get_auto_repeat_entries(date=None):
	if not date:
		date = getdate(today())
	return vmraid.db.get_all('Auto Repeat', filters=[
...
def set_auto_repeat_as_completed():
	auto_repeat = vmraid.get_all("Auto Repeat", filters = {'status': ['!=', 'Disabled']})
	for entry in auto_repeat:
		doc = vmraid.get_doc("Auto Repeat", entry.name)
		if doc.is_completed():
...
def make_auto_repeat(doctype, docname, frequency = 'Daily', start_date = None, end_date = None):
	...
	doc = vmraid.new_doc('Auto Repeat')
...
def get_auto_repeat_doctypes(doctype, txt, searchfield, start, page_len, filters):
	res = vmraid.db.get_all('Property Setter', {
		'property': 'allow_auto_repeat',
		'value': '1',
	}, ['doc_type'])
	docs = [r.doc_type for r in res]

	res = vmraid.db.get_all('DocType', {
		'allow_auto_repeat': 1,
	}, ['name'])
...
def update_reference(docname, reference):
	result = ""
	try:
		vmraid.db.set_value("Auto Repeat", docname, "reference_document", reference)
		result = "success"
...
def generate_message_preview(reference_dt, reference_doc, message=None, subject=None):
	vmraid.has_permission("Auto Repeat", "write", throw=True)
	doc = vmraid.get_doc(reference_dt, reference_doc)
	subject_preview = _("Please add a subject to your email")
	msg_preview = vmraid.render_template(message, {'doc': doc})
	if subject:
		subject_preview = vmraid.render_template(subject, {'doc': doc})
