...
from vmraid.contacts.doctype.contact.contact import (
...
class AutoRepeat(Document):
	...
	def validate_submit_on_creation(self):
		if self.submit_on_creation and not vmraid.get_meta(self.reference_doctype).is_submittable:
			vmraid.throw(
				_("Cannot enable {0} for a non-submittable doctype").format(vmraid.bold("Submit on Creation"))
			)
	...
	def validate_dates(self):
		...
		if self.end_date == self.start_date:
			vmraid.throw(
				_("{0} should not be same as {1}").format(vmraid.bold("End Date"), vmraid.bold("Start Date"))
			)
		...
		if not len(set(auto_repeat_days)) == len(auto_repeat_days):
			...
			vmraid.throw(
				_("Auto Repeat Day{0} {1} has been repeated.").format(
					plural, vmraid.bold(", ".join(repeated_days))
				)
			)
	...
	def set_auto_repeat_period(self, new_doc):
		...
		if mcount and new_doc.meta.get_field("from_date") and new_doc.meta.get_field("to_date"):
			last_ref_doc = vmraid.db.get_all(
	...
	def send_notification(self, new_doc):
		...
		try:
			...
			).format(vmraid.bold(_("Note")), vmraid.bold(_("Allow Print for Draft")))
			attachments = "[]"
