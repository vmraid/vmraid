...
from vmraid.automation.doctype.auto_repeat.auto_repeat import get_auto_repeat_entries, create_repeated_entries, week_map
from vmraid.utils import today, add_days, getdate, add_months
...
class TestAutoRepeat(unittest.TestCase):
	def setUp(self):
		if not vmraid.db.sql("SELECT `fieldname` FROM `tabCustom Field` WHERE `fieldname`='auto_repeat' and `dt`=%s", "Todo"):
			add_custom_fields()
	...
	def test_daily_auto_repeat(self):
		...
		todo = vmraid.get_doc(doc.reference_doctype, doc.reference_document)
		self.assertEqual(todo.auto_repeat, doc.name)

		new_todo = vmraid.db.get_value('ToDo',
			{'auto_repeat': doc.name, 'name': ('!=', todo.name)}, 'name')

		new_todo = vmraid.get_doc('ToDo', new_todo)
	...
	def test_monthly_auto_repeat(self):
		...
		todo = vmraid.get_doc(dict(doctype='ToDo', description='test recurring todo without end_date', assigned_by='Administrator')).insert()
	...
	def monthly_auto_repeat(self, doctype, docname, start_date, end_date = None):
		...
		data = get_auto_repeat_entries(getdate(today()))
		create_repeated_entries(data)
		docnames = vmraid.get_all(doc.reference_doctype, {'auto_repeat': doc.name})
		self.assertEqual(len(docnames), 1)

		doc = vmraid.get_doc('Auto Repeat', doc.name)
	...
	def test_notification_is_attached(self):
		...
		linked_comm = vmraid.db.exists("Communication", dict(reference_doctype="ToDo", reference_name=new_todo))
	...
	def test_submit_on_creation(self):
		...
		submittable_doc = vmraid.get_doc(dict(doctype=doctype, test='test submit on creation')).insert()
		...
		create_repeated_entries(data)
		docnames = vmraid.db.get_all(doc.reference_doctype,
...
def make_auto_repeat(**args):
	...
	doc = vmraid.get_doc({
		...
		'reference_document': args.reference_document or vmraid.db.get_value('ToDo', 'name'),
