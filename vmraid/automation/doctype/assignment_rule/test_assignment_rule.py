...
class TestAutoAssign(unittest.TestCase):
	...
	def test_round_robin(self):
		...
		self.assertEqual(vmraid.db.get_value('ToDo', dict(
			reference_type = 'Note',
	...
	def test_load_balancing(self):
		...
		for user in ('test@example.com', 'test1@example.com', 'test2@example.com'):
			self.assertEqual(len(vmraid.get_all('ToDo', dict(owner = user, reference_type = 'Note'))), 10)

		# clear 5 assignments for first user
		# can't do a limit in "delete" since postgres does not support it
		for d in vmraid.get_all('ToDo', dict(reference_type = 'Note', owner = 'test@example.com'), limit=5):
			vmraid.db.sql("delete from tabToDo where name = %s", d.name)
	...
	def test_clear_assignment(self):
		...
		# check if auto assigned to first user
		todo = vmraid.get_list('ToDo', dict(
			reference_type = 'Note',
			reference_name = note.name,
			status = 'Open'
		))[0]

		todo = vmraid.get_doc('ToDo', todo['name'])
	...
	def check_assignment_rule_scheduling(self):
		vmraid.db.sql("DELETE FROM `tabAssignment Rule`")
		...
		vmraid.flags.assignment_day = "Monday"
		note = make_note(dict(public=1))

		self.assertIn(vmraid.db.get_value('ToDo', dict(
			reference_type = 'Note',
			reference_name = note.name,
			status = 'Open'
		), 'owner'), ['test@example.com', 'test1@example.com', 'test2@example.com'])

		vmraid.flags.assignment_day = "Friday"
		note = make_note(dict(public=1))
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = vmraid.get_doc(dict(
		...
		expiry_date = vmraid.utils.add_days(vmraid.utils.nowdate(), 2)
		note1 = make_note({'expiry_date': expiry_date})
		note2 = make_note({'expiry_date': expiry_date})

		note1_todo = vmraid.get_all('ToDo', filters=dict(
			reference_type = 'Note',
			reference_name = note1.name,
			status = 'Open'
		))[0]

		note1_todo_doc = vmraid.get_doc('ToDo', note1_todo.name)
		self.assertEqual(vmraid.utils.get_date_str(note1_todo_doc.date), expiry_date)

		# due date should be updated if the reference doc's date is updated.
		note1.expiry_date = vmraid.utils.add_days(expiry_date, 2)
		note1.save()
		note1_todo_doc.reload()
		self.assertEqual(vmraid.utils.get_date_str(note1_todo_doc.date), note1.expiry_date)

		# saving one note's expiry should not update other note todo's due date
		note2_todo = vmraid.get_all('ToDo', filters=dict(
			reference_type = 'Note',
			reference_name = note2.name,
			status = 'Open'
		), fields=['name', 'date'])[0]
		self.assertNotEqual(vmraid.utils.get_date_str(note2_todo.date), note1.expiry_date)
		self.assertEqual(vmraid.utils.get_date_str(note2_todo.date), expiry_date)
		assignment_rule.delete()

def clear_assignments():
	vmraid.db.sql("delete from tabToDo where reference_type = 'Note'")

def get_assignment_rule(days, assign=None):
	vmraid.delete_doc_if_exists('Assignment Rule', 'For Note 1')
	...
	vmraid.delete_doc_if_exists('Assignment Rule', 'For Note 2')
