...
class TestAutoAssign(unittest.TestCase):
	@classmethod
	def setUpClass(cls):
		vmraid.db.delete("Assignment Rule")
	...
	def test_load_balancing(self):
		...
		for user in ("test@example.com", "test1@example.com", "test2@example.com"):
			self.assertEqual(
				len(vmraid.get_all("ToDo", dict(allocated_to=user, reference_type="Note"))), 10
			)
		...
		for d in vmraid.get_all(
			...
			vmraid.db.delete("ToDo", {"name": d.name})
	...
	def test_assignment_rule_condition(self):
		...
		assignment_rule = vmraid.get_doc(
			dict(
...
def clear_assignments():
	vmraid.db.delete("ToDo", {"reference_type": "Note"})
...
def make_note(values=None):
	note = vmraid.get_doc(dict(doctype="Note", title=random_string(10), content=random_string(20)))
