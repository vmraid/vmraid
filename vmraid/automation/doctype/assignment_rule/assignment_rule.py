...
from vmraid.cache_manager import clear_doctype_map, get_doctype_map
...
class AssignmentRule(Document):
	...
	def validate_document_types(self):
		if self.document_type == "ToDo":
			vmraid.throw(
				_("Assignment Rule is not allowed on {0} document type").format(vmraid.bold("ToDo"))
			)
...
def reopen_closed_assignment(doc):
	...
	for todo in todo_list:
		todo_doc = vmraid.get_doc("ToDo", todo)
...
def apply(doc=None, method=None, doctype=None, name=None):
	...
	assignment_rule_docs: List[AssignmentRule] = [
		vmraid.get_cached_doc("Assignment Rule", d.get("name")) for d in assignment_rules
	]
			...
			if not new_apply:
				...
				if to_close_todos:
					...
					for todo in todos_to_close:
						_todo = vmraid.get_doc("ToDo", todo)
						_todo.status = "Closed"
...
def update_due_date(doc, state=None):
	...
	for rule in assignment_rules:
		...
		if field_updated:
			assignment_todos = vmraid.get_all(
				"ToDo",
				filters={
...
def get_assignment_rules() -> List[str]:
	return vmraid.get_all("Assignment Rule", filters={"disabled": 0}, pluck="document_type")
