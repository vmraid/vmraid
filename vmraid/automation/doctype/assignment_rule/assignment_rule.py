...
class AssignmentRule(Document):
	...
	def validate(self):
		...
		if not len(set(assignment_days)) == len(assignment_days):
			...
			vmraid.throw(_("Assignment Day {0} has been repeated.").format(vmraid.bold(repeated_days)))
		if self.document_type == 'ToDo':
			vmraid.throw(_('Assignment Rule is not allowed on {0} document type').format(vmraid.bold("ToDo")))
	...
	def do_assignment(self, doc):
		...
		if user:
			assign_to.add(dict(
				...
				name = doc.get('name'),
				description = vmraid.render_template(self.description, doc),
	...
	def get_user_load_balancing(self):
		...
		for d in self.users:
			counts.append(dict(
				user = d.user,
				count = vmraid.db.count('ToDo', dict(
	...
	def get_user_based_on_field(self, doc):
		...
		if vmraid.db.exists('User', val):
			return val

	def safe_eval(self, fieldname, doc):
		try:
			if self.get(fieldname):
				return vmraid.safe_eval(self.get(fieldname), None, doc)
		except Exception as e:
			# when assignment fails, don't block the document as it may be
			# a part of the email pulling
			vmraid.msgprint(vmraid._('Auto assignment failed: {0}').format(str(e)), indicator = 'orange')
	...
	def is_rule_not_applicable_today(self):
		today = vmraid.flags.assignment_day or vmraid.utils.get_weekday()
		assignment_days = self.get_assignment_days()
...
def get_assignments(doc):
	return vmraid.get_all('ToDo', fields = ['name', 'assignment_rule'], filters = dict(
...
def bulk_apply(doctype, docnames):
	...
	for name in docnames:
		if background:
			vmraid.enqueue('vmraid.automation.doctype.assignment_rule.assignment_rule.apply', doc=None, doctype=doctype, name=name)
		else:
			apply(None, doctype=doctype, name=name)

def reopen_closed_assignment(doc):
	todo_list = vmraid.db.get_all('ToDo', filters = dict(
	...
	for todo in todo_list:
		todo_doc = vmraid.get_doc('ToDo', todo.name)
...
def apply(doc, method=None, doctype=None, name=None):
	...
	assignment_rules = vmraid.cache_manager.get_doctype_map('Assignment Rule', doc.doctype, dict(
	...
	for d in assignment_rules:
		assignment_rule_docs.append(vmraid.get_cached_doc('Assignment Rule', d.get('name')))
...
def update_due_date(doc, state=None):
	...
	if (vmraid.flags.in_patch
		...
		or vmraid.flags.in_setup_wizard):
		return
	assignment_rules = vmraid.cache_manager.get_doctype_map('Assignment Rule', 'due_date_rules_for_' + doc.doctype, dict(
		document_type = doc.doctype,
		disabled = 0,
		due_date_based_on = ['is', 'set']
	))
	for rule in assignment_rules:
		rule_doc = vmraid.get_cached_doc('Assignment Rule', rule.get('name'))
		...
		if doc.meta.has_field(due_date_field) and \
			...
			assignment_todos = vmraid.get_all('ToDo', {
...
def get_assignment_rules():
	return [d.document_type for d in vmraid.db.get_all('Assignment Rule', fields=['document_type'], filters=dict(disabled = 0))]
...
def clear_assignment_rule_cache(rule):
	vmraid.cache_manager.clear_doctype_map('Assignment Rule', rule.document_type)
	vmraid.cache_manager.clear_doctype_map('Assignment Rule', 'due_date_rules_for_' + rule.document_type)
