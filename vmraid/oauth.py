...
class OAuthWebRequestValidator(RequestValidator):
	...
	def validate_redirect_uri(self, client_id, redirect_uri, request, *args, **kwargs):
		...
		redirect_uris = vmraid.db.get_value(
	...
	def get_default_redirect_uri(self, client_id, request, *args, **kwargs):
		...
		redirect_uri = vmraid.db.get_value(
		...
		if request.client_id:
			...
			if "refresh_token" in vmraid.form_dict:
				...
				oc = vmraid.get_doc(
					"OAuth Client",
					vmraid.db.get_value(
						"OAuth Bearer Token", vmraid.form_dict["token"], "client"
					),
				)
			else:
				oc = vmraid.get_doc(
	...
	def validate_code(self, client_id, code, client, request, *args, **kwargs):
		...
		if code in checkcodes:
			request.scopes = vmraid.db.get_value(
			...
			code_challenge = vmraid.db.get_value(
	...
	def confirm_redirect_uri(
		...
		saved_redirect_uri = vmraid.db.get_value(
			"OAuth Client", client_id, "default_redirect_uri"
		)
	...
	def validate_bearer_token(self, token, scopes, request):
		...
		is_token_valid = (
			vmraid.utils.datetime.datetime.utcnow().replace(tzinfo=pytz.utc)
			< token_expiration_utc
		) and otoken.status != "Revoked"
		client_scopes = vmraid.db.get_value(
