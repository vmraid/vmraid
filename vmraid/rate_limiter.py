...
def rate_limit(key: str, limit: Union[int, Callable] = 5, seconds: int= 24*60*60, methods: Union[str, list]='ALL'):
	...
	def ratelimit_decorator(fun):
		...
		def wrapper(*args, **kwargs):
			...
			if methods != 'ALL' and vmraid.request.method.upper() not in methods:
				return vmraid.call(fun, **vmraid.form_dict)

			_limit = limit() if callable(limit) else limit

			identity = vmraid.form_dict[key]
			cache_key = f"rl:{vmraid.form_dict.cmd}:{identity}"

			value = vmraid.cache().get_value(cache_key, expires=True) or 0
			if not value:
				vmraid.cache().set_value(cache_key, 0, expires_in_sec=seconds)

			value = vmraid.cache().incrby(cache_key, 1)
			if value > _limit:
				vmraid.throw(_("You hit the rate limit because of too many requests. Please try after sometime."))
