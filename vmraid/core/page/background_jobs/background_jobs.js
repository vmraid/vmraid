vmraid.pages["background_jobs"].on_page_load = (wrapper) => {
...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.remove_failed_button = this.page.add_inner_button(__("Remove Failed Jobs"), () => {
			vmraid.call({
				method: 'vmraid.core.page.background_jobs.background_jobs.remove_failed_jobs',
				callback: () => {
					this.refresh_jobs();
				}
			});
		...
		$(vmraid.render_template('background_jobs_outer')).appendTo(this.page.body);
	...
	refresh_jobs() {
		...
		vmraid.call({
			method: 'vmraid.core.page.background_jobs.background_jobs.get_info',
			args: {
			...
			callback: (res) => {
				...
				$(vmraid.render_template('background_jobs', { jobs: res.message || [] })).appendTo(this.content);

				if (vmraid.get_route()[0] === 'background_jobs') {
