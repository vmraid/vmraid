...
class BackgroundJobs {
	constructor(wrapper) {
		...
		this.page.add_inner_button(__("Remove Failed Jobs"), () => {
			vmraid.confirm(
				...
				() => {
					vmraid
						.call(
							"vmraid.core.page.background_jobs.background_jobs.remove_failed_jobs"
						)
		...
		this.make_filters();
		this.refresh_jobs = vmraid.utils.throttle(
	...
	update_scheduler_status() {
		vmraid.call({
			method:
				"vmraid.core.page.background_jobs.background_jobs.get_scheduler_status",
			callback: r => {
	...
	refresh_jobs() {
		...
		vmraid.call({
			...
			callback: res => {
				...
				this.$content.html(
					vmraid.render_template(template, {
						jobs: res.message || []
					})
				);
				...
				if (
					vmraid.get_route()[0] === "background_jobs" &&
					auto_refresh
				) {
