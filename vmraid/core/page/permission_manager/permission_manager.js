vmraid.pages['permission-manager'].on_page_load = (wrapper) => {
	let page = vmraid.ui.make_app_page({
		parent: wrapper,
	...
	$(vmraid.render_template("permission_manager_help", {})).appendTo(page.main);
	wrapper.permission_engine = new vmraid.PermissionEngine(wrapper);

};

vmraid.pages['permission-manager'].refresh = function (wrapper) {
	wrapper.permission_engine.set_from_route();
};

vmraid.PermissionEngine = class PermissionEngine {
	constructor(wrapper) {
	...
	setup_page() {
		this.doctype_select
			= this.wrapper.page.add_select(__("Document Type"),
				...
				.change(function () {
					vmraid.set_route("permission-manager", $(this).val());
				});
		...
		this.page.add_inner_button(__('Set User Permissions'), () => {
			return vmraid.set_route('List', 'User Permission');
		});
		this.set_from_route();
	}
	...
	set_from_route() {
		...
		if (vmraid.get_route()[1]) {
			this.doctype_select.val(vmraid.get_route()[1]);
		} else if (vmraid.route_options) {
			if (vmraid.route_options.doctype) {
				this.doctype_select.val(vmraid.route_options.doctype);
			}
			if (vmraid.route_options.role) {
				this.role_select.val(vmraid.route_options.role);
			}
	...
	reset_std_permissions(data) {
		let doctype = this.get_doctype();
		let d = vmraid.confirm(__("Reset Permissions for {0}?", [doctype]), () => {
			return vmraid.call({
				module: "vmraid.core",
				page: "permission_manager",
				method: "reset",
		...
		let $d = $(d.wrapper).find(".vmraid-confirm-message").append("<hr><h5>Standard Permissions:</h5><br>");
		let $wrapper = $("<p></p>").appendTo($d);
		data.message.forEach((d) => {
			let rights = this.rights
				.filter((r) => d[r])
				.map((r) => {
					return __(toTitle(vmraid.unscrub(r)));
				});
			...
			.click(function () {
				...
				vmraid.call({
					...
					callback: function (r) {
						...
						vmraid.msgprint(__("Users with role {0}:", [__(role)])
	...
	add_delete_button(row, d) {
		$(`<button class='btn btn-danger btn-remove-perm btn-xs'>${vmraid.utils.icon('delete')}</button>`)
			...
			.click(function () {
				return vmraid.call({
					...
					callback: (r) => {
						if (r.exc) {
							vmraid.msgprint(__("Did not remove"));
						} else {
							this.refresh();
						}
					}
	...
	add_check_events() {
		this.body.on("click", ".show-user-permissions", () => {
			vmraid.route_options = { allow: this.get_doctype() || "" };
			...
			() => {
				...
				d.set_primary_action(__('Add'), () => {
					...
					vmraid.call({
						...
						callback: (r) => {
							if (r.exc) {
								vmraid.msgprint(__("Did not add"));
							} else {
								this.refresh();
							}
						}
	...
	get_link_fields(doctype) {
		return vmraid.get_children("DocType", doctype, "fields",
