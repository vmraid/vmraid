...
from vmraid.utils import format_datetime
...
def get_data(filters=None):
	...
	logs = vmraid.db.sql("SELECT * FROM `tabTransaction Log` order by creation desc ", as_dict=1)
	result = []
	for l in logs:
		row_index = int(l.row_index)
		if row_index > 1:
			previous_hash = vmraid.db.sql("SELECT chaining_hash FROM `tabTransaction Log` WHERE row_index = {0}".format(row_index - 1))
