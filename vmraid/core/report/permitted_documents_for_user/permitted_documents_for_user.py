...
from vmraid.permissions import check_admin_or_system_manager, rights
...
def execute(filters=None):
	...
	data = vmraid.get_list(doctype, fields=fields, as_list=True, user=user)

	if show_permissions:
		columns = columns + [vmraid.unscrub(right) + ':Check:80' for right in rights]
		data = list(data)
		for i, doc in enumerate(data):
			permission = vmraid.permissions.get_doc_permissions(vmraid.get_doc(doctype, doc[0]), user)
...
def get_columns_and_fields(doctype):
	...
	fields = ["`name`"]
	for df in vmraid.get_meta(doctype).fields:
...
def query_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	user_perms = vmraid.utils.user.UserPermissions(user)
	user_perms.build_permissions()
	can_read = user_perms.can_read # Does not include child tables

	single_doctypes = [d[0] for d in vmraid.db.get_values("DocType", {"issingle": 1})]
