...
from vmraid.utils import cstr, get_site_url
...
class TestAccessLog(unittest.TestCase):
	def setUp(self):
		...
		generate_keys(vmraid.session.user)
		vmraid.db.commit()
		generated_secret = vmraid.utils.password.get_decrypted_password("User",
			vmraid.session.user, fieldname='api_secret')
		...
		self.file_name = vmraid.utils.random_string(10) + '.txt'
		self.test_content = vmraid.utils.random_string(1024)
	...
	def test_make_full_access_log(self):
		...
		last_doc = vmraid.get_last_doc('Access Log')
	...
	def test_private_file_download(self):
		...
		new_private_file = vmraid.get_doc({
		...
		private_file_link = get_site_url(vmraid.local.site) + new_private_file.file_url
