...
vmraid.ui.form.on('Server Script', {
	setup: function(frm) {
	...
	refresh: function(frm) {
		...
		if (!frm.is_new()) {
			frm.add_custom_button(__('Compare Versions'), () => {
				new vmraid.ui.DiffView("Server Script", "script", frm.doc.name);
			});
		}
...
if vmraid.form_dict.message == "ping":
	vmraid.response['message'] = "pong"
else:
