...
from vmraid.utils import cint, cstr, flt, getdate, get_datetime, get_url, get_absolute_url, duration_to_seconds
...
def upload(rows = None, submit_after_import=None, ignore_encoding_errors=False, no_email=True, overwrite=None,
	...
	# for translations
	if user:
		vmraid.cache().hdel("lang", user)
		vmraid.set_user_lang(user)

	if data_import_doc and isinstance(data_import_doc, string_types):
		data_import_doc = vmraid.get_doc("Data Import Legacy", data_import_doc)
	if data_import_doc and from_data_import == "Yes":
		...
		# extra input params
		params = json.loads(vmraid.form_dict.get("params") or '{}')
	...
	vmraid.flags.mute_emails = no_email

	def get_data_keys_definition():
		return get_data_keys()

	def bad_template():
		vmraid.throw(_("Please do not change the rows above {0}").format(get_data_keys_definition().data_separator))

	def check_data_length():
		if not data:
			vmraid.throw(_("No data found in the file. Please reattach the new file with data."))
	...
	def filter_empty_columns(columns):
		...
		if empty_cols:
			if columns[-1*len(empty_cols):] == empty_cols:
				...
				vmraid.msgprint(_("Please make sure that there are no empty columns in the file."),
					raise_exception=1)
	...
	def get_doc(start_idx):
		if doctypes:
			...
			doc = vmraid._dict(zip(columns, rows[start_idx][1:]))
			doc['doctype'] = doctype
			return doc, [], None
	...
	def validate_naming(doc):
		autoname = vmraid.get_meta(doctype).autoname
		if autoname:
			...
			if (autoname not in doc) or (not doc[autoname]):
				...
				if not hasattr(get_controller(doctype), "autoname"):
					vmraid.throw(_("{0} is a mandatory field").format(autoname))
		return True

	users = vmraid.db.sql_list("select name from tabUser")
	def prepare_for_insert(doc):
		# don't block data import if user is not set
		# migrating from another system
		if not doc.owner in users:
			doc.owner = vmraid.session.user
		if not doc.modified_by in users:
			doc.modified_by = vmraid.session.user
	...
	def attach_file_to_doc(doctype, docname, file_url):
		...
		files = vmraid.db.sql("""Select name from `tabFile` where attached_to_doctype='{doctype}' and
	...
	if not rows:
		_file = vmraid.get_doc("File", {"file_url": data_import_doc.import_file})
		...
		if file_extension == '.xlsx' and from_data_import == 'Yes':
			...
			vmraid.throw(_("Unsupported File Format"))
	...
	try:
		...
		vmraid.throw(_("Cannot change header content"))
	doctypes = []
	...
	if submit_after_import and not cint(vmraid.db.get_value("DocType",
	...
	# check permissions
	if not vmraid.permissions.can_import(parenttype or doctype):
	...
	def publish_progress(achieved, reload=False):
		if data_import_doc:
			vmraid.publish_realtime("data_import_progress", {"progress": str(int(100.0*achieved/total)),
				"data_import": data_import_doc.name, "reload": reload}, user=vmraid.session.user)
	...
	for batch_start in range(0, total, batch_size):
		...
		for i, row in enumerate(batch):
			...
			try:
				...
				original = None
				if parentfield:
					parent = vmraid.get_doc(parenttype, doc["parent"])
					doc = parent.append(parentfield, doc)
					parent.save()
				else:
					if overwrite and doc.get("name") and vmraid.db.exists(doctype, doc["name"]):
						original = vmraid.get_doc(doctype, doc["name"])
				...
				if vmraid.local.message_log:
					err_msg = "\n".join(['<p class="border-bottom small">{}</p>'.format(json.loads(msg).get('message')) for msg in vmraid.local.message_log])
				else:
					err_msg = '<p class="border-bottom small">{}</p>'.format(cstr(e))

				error_trace = vmraid.get_traceback()
				if error_trace:
					error_log_doc = vmraid.log_error(error_trace)
				...
				log(**{
					"row": row_idx + 1,
					"title": 'Error for row %s' % (len(row)>1 and vmraid.safe_decode(row[1]) or ""),
					"message": err_msg,
					"indicator": "red",
					"link":error_link
				})
...
def get_parent_field(doctype, parenttype):
	...
	# get parentfield
	if parenttype:
		for d in vmraid.get_meta(parenttype).get_table_fields():
			if d.options==doctype:
				parentfield = d.fieldname
				break

		if not parentfield:
			vmraid.msgprint(_("Did not find {0} for {0} ({1})").format("parentfield", parenttype, doctype))
			raise Exception
...
def delete_child_rows(rows, doctype):
	...
	for p in list(set([r[1] for r in rows])):
		if p:
			vmraid.db.sql("""delete from `tab{0}` where parent=%s""".format(doctype), p)
