...
vmraid.ui.form.on('DocType', {
	refresh: function(frm) {
		frm.set_query('role', 'permissions', function(doc) {
			if (doc.custom && vmraid.session.user != 'Administrator') {
				return {
					query: "vmraid.core.doctype.role.role.role_query",
					filters: [['Role', 'name', '!=', 'All']]
				};
			}
		});

		if(vmraid.session.user !== "Administrator" || !vmraid.boot.developer_mode) {
			if(frm.is_new()) {
		...
		if (!frm.is_new() && !frm.doc.istable) {
			if (frm.doc.issingle) {
				frm.add_custom_button(__('Go to {0}', [__(frm.doc.name)]), () => {
					window.open(`/app/${vmraid.router.slug(frm.doc.name)}`);
				});
			} else {
...
vmraid.ui.form.on("DocField", {
	form_render(frm, doctype, docname) {
		...
		let row = vmraid.get_doc(doctype, docname);
		...
		function update_fieldname_options() {
			...
			vmraid.model.with_doctype(link_doctype, () => {
				let fields = vmraid.meta
					.get_docfields(link_doctype, null, {
						fieldtype: ["not in", vmraid.model.no_value_type]
					})
					.map(df => ({
