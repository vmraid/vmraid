...
# test_records = vmraid.get_test_records('DocType')
...
class TestDocType(unittest.TestCase):
	...
	def test_validate_name(self):
		self.assertRaises(vmraid.NameError, new_doctype("_Some DocType").insert)
		self.assertRaises(vmraid.NameError, new_doctype("8Some DocType").insert)
		self.assertRaises(vmraid.NameError, new_doctype("Some (DocType)").insert)
		self.assertRaises(
			vmraid.NameError,
		...
		for name in ("Some DocType", "Some_DocType", "Some-DocType"):
			if vmraid.db.exists("DocType", name):
				vmraid.delete_doc("DocType", name)

			doc = new_doctype(name).insert()
			doc.delete()

	def test_doctype_unique_constraint_dropped(self):
		if vmraid.db.exists("DocType", "With_Unique"):
			vmraid.delete_doc("DocType", "With_Unique")

		dt = new_doctype("With_Unique", unique=1)
		dt.insert()

		doc1 = vmraid.new_doc("With_Unique")
		doc2 = vmraid.new_doc("With_Unique")
		...
		doc1.insert()
		self.assertRaises(vmraid.UniqueValidationError, doc2.insert)
		vmraid.db.rollback()
	...
	def test_data_field_options(self):
		...
		valid_data_field_options = vmraid.model.data_field_options + ("",)
		invalid_data_field_options = ("Invalid Option 1", vmraid.utils.random_string(5))

		for field_option in valid_data_field_options + invalid_data_field_options:
			test_doctype = vmraid.get_doc(
				{
					"doctype": "DocType",
					"name": doctype_name,
					"module": "Core",
			...
			if field_option in invalid_data_field_options:
				# assert that only data options in vmraid.model.data_field_options are valid
				self.assertRaises(vmraid.ValidationError, test_doctype.insert)
			else:
				test_doctype.insert()
	...
	def test_sync_field_order(self):
		...
		vmraid.delete_doc_if_exists("DocType", "Test Field Order DocType")
		if os.path.isfile(path):
			os.remove(path)

		try:
			vmraid.flags.allow_doctype_export = 1
			test_doctype.save()

			# assert that field_order list is being created with the default order
			test_doctype_json = vmraid.get_file_json(path)
			...
			with open(path, "w+") as txtfile:
				txtfile.write(vmraid.as_json(test_doctype_json))
			...
			vmraid.reload_doctype(test_doctype.name, force=True)
			test_doctype.reload()
			...
			vmraid.flags.allow_doctype_export = 0
	...
	def test_cancel_link_doctype(self):
		...
		from vmraid.desk.form.linked_with import cancel_all_linked_docs, get_submitted_linked_docs
		...
		# create doctype data
		data_link_doc = vmraid.new_doc("Test Linked Doctype")
		data_link_doc.some_fieldname = "Data1"
		data_link_doc.insert()
		data_link_doc.save()
		data_link_doc.submit()

		data_doc = vmraid.new_doc("Test Doctype")
	...
	def test_ignore_cancelation_of_linked_doctype_during_cancel(self):
		...
		data_doc = vmraid.new_doc("Test Doctype 2")
		data_doc.some_fieldname = "Data1"
		...
		self.assertRaises(vmraid.LinkExistsError, data_link_doc_1.cancel)
	...
	def test_links_table_fieldname_validation(self):
		...
		self.assertRaises(vmraid.DoesNotExistError, validate_links_table_fieldnames, doc)
		doc.links = []  # reset links table
	...
	def test_create_virtual_doctype(self):
		...
		virtual_doc.insert()
		virtual_doc.save()
		doc = vmraid.get_doc("DocType", "Test Virtual Doctype")

		self.assertEqual(doc.is_virtual, 1)
		self.assertFalse(vmraid.db.table_exists("Test Virtual Doctype"))
	...
	def test_json_field(self):
		...
		json_doc.insert()
		json_doc.save()
		doc = vmraid.get_doc("DocType", "Test Json Doctype")
		...
		test_json = vmraid.get_doc("Test Json Doctype", doc.name)
