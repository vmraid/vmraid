...
import json
from vmraid.cache_manager import clear_user_cache, clear_controller_cache
...
from vmraid.utils import now, cint
from vmraid.model import no_value_fields, default_fields, data_fieldtypes, table_fields, data_field_options
...
from vmraid.desk.notifications import delete_notification_count_for
from vmraid.modules import make_boilerplate, get_doc_path
from vmraid.database.schema import validate_column_name, validate_column_length
from vmraid.model.docfield import supports_translation
from vmraid.modules.import_file import get_file_path
from vmraid.model.meta import Meta
from vmraid.desk.utils import validate_route_conflict

class InvalidFieldNameError(vmraid.ValidationError): pass
class UniqueFieldnameError(vmraid.ValidationError): pass
class IllegalMandatoryError(vmraid.ValidationError): pass
class DoctypeLinkError(vmraid.ValidationError): pass
class WrongOptionsDoctypeLinkError(vmraid.ValidationError): pass
class HiddenAndMandatoryWithoutDefaultError(vmraid.ValidationError): pass
class NonUniqueError(vmraid.ValidationError): pass
class CannotIndexedError(vmraid.ValidationError): pass
class CannotCreateStandardDoctypeError(vmraid.ValidationError): pass
...
class DocType(Document):
	...
	def validate(self):
		...
		if not self.is_new():
			self.before_update = vmraid.get_doc('DocType', self.name)
		...
		if self.default_print_format and not self.custom:
			vmraid.throw(_('Standard DocType cannot have default print format, use Customize Form'))
	...
	def check_developer_mode(self):
		...
		if vmraid.flags.in_patch or vmraid.flags.in_test:
			return

		if not vmraid.conf.get("developer_mode") and not self.custom:
			vmraid.throw(_("Not in Developer Mode! Set in site_config.json or make 'Custom' DocType."), CannotCreateStandardDoctypeError)

		if self.is_virtual and self.custom:
			vmraid.throw(_("Not allowed to create custom Virtual DocType."), CannotCreateStandardDoctypeError)
	...
	def setup_fields_to_fetch(self):
		...
		try:
			old_meta = vmraid.get_meta(vmraid.get_doc('DocType', self.name), cached=False)
			old_fields_to_fetch = [df.fieldname for df in old_meta.get_fields_to_fetch()]
		except vmraid.DoesNotExistError:
			old_fields_to_fetch = []

		new_meta = vmraid.get_meta(self, cached=False)
	...
	def validate_website(self):
		...
		if self.has_web_view:
			...
			if not 'route' in [d.fieldname for d in self.fields]:
				vmraid.throw(_('Field "route" is mandatory for Web Views'), title='Missing Field')
	...
	def change_modified_of_parent(self):
		...
		if vmraid.flags.in_import:
			return
		parent_list = vmraid.db.get_all('DocField', 'parent',
			dict(fieldtype=['in', vmraid.model.table_fields], options=self.name))
		for p in parent_list:
			vmraid.db.sql('UPDATE `tabDocType` SET modified=%s WHERE `name`=%s', (now(), p.parent))
		...
		for d in self.get("fields"):
			if d.fieldtype:
				if (not getattr(d, "fieldname", None)):
					...
					if d.fieldname in restricted:
						vmraid.throw(_("Fieldname {0} is restricted").format(d.fieldname), InvalidFieldNameError)
	...
	def on_update(self):
		...
		try:
			vmraid.db.updatedb(self.name, Meta(self))
		...
		allow_doctype_export = (
			not self.custom
			and not vmraid.flags.in_import
			and (
				vmraid.conf.developer_mode
				or vmraid.flags.allow_doctype_export
			)
		)
		...
		vmraid.clear_cache(doctype=self.name)

		if not vmraid.flags.in_install and hasattr(self, 'before_update'):
			self.sync_global_search()

		# clear from local cache
		if self.name in vmraid.local.meta_cache:
			del vmraid.local.meta_cache[self.name]
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			now = (not vmraid.request) or vmraid.flags.in_test or vmraid.flags.in_install
			vmraid.enqueue('vmraid.utils.global_search.rebuild_for_doctype',
	...
	def set_base_class_for_controller(self):
		...
		controller_path = vmraid.get_module_path(vmraid.scrub(self.module),
			'doctype', vmraid.scrub(self.name), vmraid.scrub(self.name) + '.py')

		with open(controller_path, 'r') as f:
			code = f.read()

		class_string = '\nclass {0}(Document)'.format(self.name.replace(' ', ''))
		if '\nfrom vmraid.model.document import Document' in code and class_string in code:
			code = code.replace('from vmraid.model.document import Document',
	...
	def before_rename(self, old, new, merge=False):
		...
		if not self.custom and vmraid.session.user != "Administrator":
			vmraid.throw(_("DocType can only be renamed by Administrator"))

		self.check_developer_mode()
		self.validate_name(new)

		if merge:
			vmraid.throw(_("DocType can not be merged"))
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			vmraid.db.sql("""update tabSingles set doctype=%s where doctype=%s""", (new, old))
			vmraid.db.sql("""update tabSingles set value=%s
	...
	def rename_files_and_folders(self, old, new):
		...
		for fname in os.listdir(new_path):
			if vmraid.scrub(old) in fname:
				old_file_name = os.path.join(new_path, fname)
				new_file_name = os.path.join(new_path, fname.replace(vmraid.scrub(old), vmraid.scrub(new)))
				shutil.move(old_file_name, new_file_name)

		self.rename_inside_controller(new, old, new_path)
		vmraid.msgprint(_('Renamed files and replaced code in controllers, please check!'))

	def rename_inside_controller(self, new, old, new_path):
		for fname in ('{}.js', '{}.py', '{}_list.js', '{}_calendar.js', 'test_{}.py', 'test_{}.js'):
			fname = os.path.join(new_path, fname.format(vmraid.scrub(new)))
			if os.path.exists(fname):
				...
				with open(fname, 'w') as f:
					if fname.endswith('.js'):
						...
						file_content = code.replace(vmraid.scrub(old), vmraid.scrub(new)) # replace str with _ (py imports)
						file_content = file_content.replace(old.replace(' ', ''), new.replace(' ', '')) # replace str (py controllers)

					f.write(file_content)

		# updating json file with new name
		doctype_json_path = os.path.join(new_path, '{}.json'.format(vmraid.scrub(new)))
		current_data = vmraid.get_file_json(doctype_json_path)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (vmraid.db.table_exists(self.name) and vmraid.db.sql("select name from `tab{}` limit 1".format(self.name))):
			return

		existing_property_setter = vmraid.db.get_value("Property Setter", {"doc_type": self.name,
	...
	def import_doc(self):
		...
		from vmraid.modules.import_module import import_from_files
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = vmraid.get_module_path(vmraid.scrub(self.module), 'doctype', vmraid.scrub(self.name), 'templates')
	...
	def make_amendable(self):
		...
		if self.is_submittable:
			if not vmraid.db.sql("""select name from tabDocField
	...
	def make_repeatable(self):
		...
		if self.allow_auto_repeat:
			if not vmraid.db.exists('Custom Field', {'fieldname': 'auto_repeat', 'dt': self.name}) and \
				not vmraid.db.exists('DocField', {'fieldname': 'auto_repeat', 'parent': self.name}):
	...
	def validate_nestedset(self):
		...
		if not self.nsm_parent_field:
			field_label = vmraid.bold(_("Parent Field (Tree)"))
			vmraid.throw(_("{0} is a mandatory field").format(field_label), vmraid.MandatoryError)

		# check if field is valid
		fieldnames = [df.fieldname for df in self.fields]
		if self.nsm_parent_field and self.nsm_parent_field not in fieldnames:
			vmraid.throw(_("Parent Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def add_nestedset_fields(self):
		...
		parent_field_name = vmraid.scrub(parent_field_label)
		self.append("fields", {
	...
	def get_max_idx(self):
		...
		max_idx = vmraid.db.sql("""select max(idx) from `tabDocField` where parent = %s""",
			self.name)
	...
	def validate_name(self, name=None):
		...
		if re.search(r"^[ \t\n\r]+|[ \t\n\r]+$", name, **flags):
			vmraid.throw(_("DocType's name should not start or end with whitespace"), vmraid.NameError)

		# a DocType's name should not start with a number or underscore
		# and should only contain letters, numbers and underscore
		if not re.match(r"^(?![\W])[^\d_\s][\w ]+$", name, **flags):
			vmraid.throw(_("DocType's name should start with a letter and it can only consist of letters, numbers, spaces and underscores"), vmraid.NameError)
...
def validate_series(dt, autoname=None, name=None):
	...
	if not autoname and dt.get("fields", {"fieldname":"naming_series"}):
		...
		vmraid.throw(_("Invalid fieldname '{0}' in autoname").format(dt.autoname))
	...
	if autoname and autoname.startswith('field:'):
		...
		if not field or field not in [df.fieldname for df in dt.fields]:
			vmraid.throw(_("Invalid fieldname '{0}' in autoname").format(field))
		else:
			for df in dt.fields:
	...
	if autoname and (not autoname.startswith('field:')) \
		...
		""", (prefix, name))
		if used_in:
			vmraid.throw(_("Series {0} already used in {1}").format(prefix, used_in[0][0]))

def validate_links_table_fieldnames(meta):
	"""Validate fieldnames in Links table"""
	if vmraid.flags.in_patch: return
	if vmraid.flags.in_fixtures: return
	if not meta.links: return

	for index, link in enumerate(meta.links):
		link_meta = vmraid.get_meta(link.link_doctype)
		if not link_meta.get_field(link.link_fieldname):
			message = _("Row #{0}: Could not find field {1} in {2} DocType").format(index+1, vmraid.bold(link.link_fieldname), vmraid.bold(link.link_doctype))
			vmraid.throw(message, InvalidFieldNameError, _("Invalid Fieldname"))
...
def validate_fields(meta):
	...
	:param meta: `vmraid.model.meta.Meta` object to check."""
	...
	def check_invalid_fieldnames(docname, fieldname):
		...
		if fieldname in invalid_fields:
			vmraid.throw(_("{0}: Fieldname cannot be one of {1}")
				.format(docname, ", ".join([vmraid.bold(d) for d in invalid_fields])))
	...
	def check_unique_fieldname(docname, fieldname):
		...
		if len(duplicates) > 1:
			vmraid.throw(_("{0}: Fieldname {1} appears multiple times in rows {2}").format(docname, fieldname, ", ".join(duplicates)), UniqueFieldnameError)
	...
	def check_illegal_mandatory(docname, d):
		if (d.fieldtype in no_value_fields) and d.fieldtype not in table_fields and d.reqd:
			vmraid.throw(_("{0}: Field {1} of type {2} cannot be mandatory").format(docname, d.label, d.fieldtype), IllegalMandatoryError)
	...
	def check_link_table_options(docname, d):
		...
		if d.fieldtype in ("Link",) + table_fields:
			if not d.options:
				vmraid.throw(_("{0}: Options required for Link or Table type field {1} in row {2}").format(docname, d.label, d.idx), DoctypeLinkError)
			if d.options=="[Select]" or d.options==d.parent:
				return
			if d.options != d.parent:
				options = vmraid.db.get_value("DocType", d.options, "name")
				if not options:
					vmraid.throw(_("{0}: Options must be a valid DocType for field {1} in row {2}").format(docname, d.label, d.idx), WrongOptionsDoctypeLinkError)
				elif not (options == d.options):
					vmraid.throw(_("{0}: Options {1} must be the same as doctype name {2} for the field {3}")
	...
	def check_hidden_and_mandatory(docname, d):
		if d.hidden and d.reqd and not d.default:
			vmraid.throw(_("{0}: Field {1} in row {2} cannot be hidden and mandatory without default").format(docname, d.label, d.idx), HiddenAndMandatoryWithoutDefaultError)

	def check_width(d):
		if d.fieldtype == "Currency" and cint(d.width) < 100:
			vmraid.throw(_("Max width for type Currency is 100px in row {0}").format(d.idx))
	...
	def check_in_list_view(is_table, d):
		if d.in_list_view and (d.fieldtype in not_allowed_in_list_view):
			...
			vmraid.throw(_("'{0}' not allowed for type {1} in row {2}").format(property_label, d.fieldtype, d.idx))

	def check_in_global_search(d):
		if d.in_global_search and d.fieldtype in no_value_fields:
			vmraid.throw(_("'In Global Search' not allowed for type {0} in row {1}")
	...
	def check_dynamic_link_options(d):
		if d.fieldtype=="Dynamic Link":
			...
			if not doctype_pointer or (doctype_pointer[0].fieldtype not in ("Link", "Select")) \
				...
				vmraid.throw(_("Options 'Dynamic Link' type of field must point to another Link Field with options as 'DocType'"))

	def check_illegal_default(d):
		if d.fieldtype == "Check" and not d.default:
			d.default = '0'
		if d.fieldtype == "Check" and cint(d.default) not in (0, 1):
			vmraid.throw(_("Default for 'Check' type of field {0} must be either '0' or '1'").format(vmraid.bold(d.fieldname)))
		if d.fieldtype == "Select" and d.default:
			if not d.options:
				vmraid.throw(_("Options for {0} must be set before setting the default value.").format(vmraid.bold(d.fieldname)))
			elif d.default not in d.options.split("\n"):
				vmraid.throw(_("Default value for {0} must be in the list of options.").format(vmraid.bold(d.fieldname)))

	def check_precision(d):
		if d.fieldtype in ("Currency", "Float", "Percent") and d.precision is not None and not (1 <= cint(d.precision) <= 6):
			vmraid.throw(_("Precision should be between 1 and 6"))
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			if d.fieldtype not in ("Data", "Link", "Read Only"):
				vmraid.throw(_("{0}: Fieldtype {1} for {2} cannot be unique").format(docname, d.fieldtype, d.label), NonUniqueError)

			if not d.get("__islocal") and vmraid.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = vmraid.db.sql("""select `{fieldname}`, count(*)
				...
				if has_non_unique_values and has_non_unique_values[0][0]:
					vmraid.throw(_("{0}: Field '{1}' cannot be set as Unique as it has non-unique values").format(docname, d.label), NonUniqueError)

		if d.search_index and d.fieldtype in ("Text", "Long Text", "Small Text", "Code", "Text Editor"):
			vmraid.throw(_("{0}:Fieldtype {1} for {2} cannot be indexed").format(docname, d.fieldtype, d.label), CannotIndexedError)
	...
	def check_fold(fields):
		...
		for i, f in enumerate(fields):
			if f.fieldtype=="Fold":
				if fold_exists:
					vmraid.throw(_("There can be only one Fold in a form"))
				fold_exists = True
				if i < len(fields)-1:
					nxt = fields[i+1]
					if nxt.fieldtype != "Section Break":
						vmraid.throw(_("Fold must come before a Section Break"))
				else:
					vmraid.throw(_("Fold can not be at the end of the form"))
	...
	def check_search_fields(meta, fields):
		...
		for fieldname in search_fields:
			...
			if (fieldtype_mapper.get(fieldname) in no_value_fields) or \
				...
				vmraid.throw(_("Search field {0} is not valid").format(fieldname))
	...
	def check_title_field(meta):
		...
		if meta.title_field not in fieldname_list:
			vmraid.throw(_("Title field must be a valid fieldname"), InvalidFieldNameError)
		...
		def _validate_title_field_pattern(pattern):
			...
			for fieldname in re.findall("{(.*?)}", pattern, re.UNICODE):
				...
				if fieldname not in fieldname_list:
					vmraid.throw(_("{{{0}}} is not a valid fieldname pattern. It should be {{field_name}}.").format(fieldname),
	...
	def check_image_field(meta):
		...
		if not df:
			vmraid.throw(_("Image field must be a valid fieldname"), InvalidFieldNameError)
		if df[0].fieldtype != 'Attach Image':
			vmraid.throw(_("Image field must be of type Attach Image"), InvalidFieldNameError)
	...
	def check_is_published_field(meta):
		...
		if meta.is_published_field not in fieldname_list:
			vmraid.throw(_("Is Published Field must be a valid fieldname"), InvalidFieldNameError)
	...
	def check_timeline_field(meta):
		...
		if meta.timeline_field not in fieldname_list:
			vmraid.throw(_("Timeline field must be a valid fieldname"), InvalidFieldNameError)

		df = meta.get("fields", {"fieldname": meta.timeline_field})[0]
		if df.fieldtype not in ("Link", "Dynamic Link"):
			vmraid.throw(_("Timeline field must be a Link or Dynamic Link"), InvalidFieldNameError)
		...
		if meta.sort_field:
			...
			for fieldname in sort_fields:
				if not fieldname in fieldname_list + list(default_fields):
					vmraid.throw(_("Sort field {0} must be a valid fieldname").format(fieldname),
						InvalidFieldNameError)
	...
	def check_illegal_depends_on_conditions(docfield):
		...
		for field in depends_on_fields:
			...
			if depends_on and ("=" in depends_on) and \
				...
				vmraid.throw(_("Invalid {0} condition").format(vmraid.unscrub(field)), vmraid.ValidationError)
	...
	def check_table_multiselect_option(docfield):
		...
		if not link_field:
			vmraid.throw(_('DocType <b>{0}</b> provided for the field <b>{1}</b> must have atleast one Link field')
				.format(doctype, docfield.fieldname), vmraid.ValidationError)
	...
	def validate_data_field_type(docfield):
		if docfield.fieldtype == "Data" and not (docfield.oldfieldtype and docfield.oldfieldtype != "Data"):
			if docfield.options and (docfield.options not in data_field_options):
				df_str = vmraid.bold(_(docfield.label))
				...
				vmraid.msgprint(text_str + df_options_str, title="Invalid Data Field", raise_exception=True)
	...
	def check_child_table_option(docfield):
		...
		if not meta.istable:
			vmraid.throw(_('Option {0} for field {1} is not a child table')
				.format(vmraid.bold(doctype), vmraid.bold(docfield.fieldname)), title=_("Invalid Option"))
...
def validate_permissions_for_doctype(doctype, for_remove=False, alert=False):
	...
	doctype = vmraid.get_doc("DocType", doctype)
...
def clear_permissions_cache(doctype):
	...
	for user in vmraid.db.sql_list("""
		SELECT
...
def validate_permissions(doctype, for_remove=False, alert=False):
	...
	if not permissions and alert:
		vmraid.msgprint(_('No Permissions Specified'), alert=True, indicator='orange')
	...
	def check_atleast_one_set(d):
		if not d.select and not d.read and not d.write and not d.submit and not d.cancel and not d.create:
			vmraid.throw(_("{0}: No basic permissions set").format(get_txt(d)))
	...
	def check_double(d):
		...
		if has_similar:
			vmraid.throw(_("{0}: Only one rule allowed with the same Role, Level and {1}")\
	...
	def check_level_zero_is_set(d):
		if cint(d.permlevel) > 0 and d.role != 'All':
			...
			if not has_zero_perm:
				vmraid.throw(_("{0}: Permission at level 0 must be set before higher levels are set").format(get_txt(d)))
	...
	def check_permission_dependency(d):
		if d.cancel and not d.submit:
			vmraid.throw(_("{0}: Cannot set Cancel without Submit").format(get_txt(d)))

		if (d.submit or d.cancel or d.amend) and not d.write:
			vmraid.throw(_("{0}: Cannot set Submit, Cancel, Amend without Write").format(get_txt(d)))
		if d.amend and not d.write:
			vmraid.throw(_("{0}: Cannot set Amend without Cancel").format(get_txt(d)))
		if d.get("import") and not d.create:
			vmraid.throw(_("{0}: Cannot set Import without Create").format(get_txt(d)))
	...
	def remove_rights_for_single(d):
		...
		if d.report:
			vmraid.msgprint(_("Report cannot be set for Single types"))
			d.report = 0
			d.set("import", 0)
			d.set("export", 0)
		...
		for ptype, label in [["set_user_permissions", _("Set User Permissions")]]:
			if d.get(ptype):
				d.set(ptype, 0)
				vmraid.msgprint(_("{0} cannot be set for Single types").format(label))

	def check_if_submittable(d):
		if d.submit and not issubmittable:
			vmraid.throw(_("{0}: Cannot set Assign Submit if not Submittable").format(get_txt(d)))
		elif d.amend and not issubmittable:
			vmraid.throw(_("{0}: Cannot set Assign Amend if not Submittable").format(get_txt(d)))

	def check_if_importable(d):
		if d.get("import") and not isimportable:
			vmraid.throw(_("{0}: Cannot set import as {1} is not importable").format(get_txt(d), doctype))
	...
	def validate_permission_for_all_role(d):
		...
		if doctype.custom:
			if d.role == 'All':
				vmraid.throw(_('Row # {0}: Non administrator user can not set the role {1} to the custom doctype')
					.format(d.idx, vmraid.bold(_('All'))), title=_('Permissions Error'))

			roles = [row.name for row in vmraid.get_all('Role', filters={'is_custom': 1})]

			if d.role in roles:
				vmraid.throw(_('Row # {0}: Non administrator user can not set the role {1} to the custom doctype')
					.format(d.idx, vmraid.bold(_(d.role))), title=_('Permissions Error'))
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if hasattr(doc,'restrict_to_domain') and doc.restrict_to_domain and \
			not vmraid.db.exists('Domain', doc.restrict_to_domain):
			vmraid.get_doc(dict(doctype='Domain', domain=doc.restrict_to_domain)).insert()

		if	("tabModule Def" in vmraid.db.get_tables()
			and not vmraid.db.exists("Module Def", doc.module)):
			m = vmraid.get_doc({"doctype": "Module Def", "module_name": doc.module})
			m.app_name = vmraid.local.module_app[vmraid.scrub(doc.module)]
		...
		for role in list(set(roles)):
			if not vmraid.db.exists("Role", role):
				r = vmraid.get_doc(dict(doctype= "Role", role_name=role, desk_access=1))
				r.flags.ignore_mandatory = r.flags.ignore_permissions = True
				r.insert()
	except vmraid.DoesNotExistError as e:
		pass
...
def check_fieldname_conflicts(doctype, fieldname):
	...
	if fieldname in method_list + property_list:
		vmraid.throw(_("Fieldname {0} conflicting with meta object").format(fieldname))

def clear_linked_doctype_cache():
	vmraid.cache().delete_value('linked_doctypes_without_ignore_user_permissions_enabled')
...
def check_email_append_to(doc):
	...
	if doc.subject_field and not subject_field:
		vmraid.throw(_("Select a valid Subject field for creating documents from Email"))

	if subject_field and subject_field.fieldtype not in ["Data", "Text", "Long Text", "Small Text", "Text Editor"]:
		vmraid.throw(_("Subject Field type should be Data, Text, Long Text, Small Text, Text Editor"))
	...
	if doc.sender_field and not sender_field:
		vmraid.throw(_("Select a valid Sender Field for creating documents from Email"))

	if not sender_field.options == "Email":
		vmraid.throw(_("Sender Field should have Email in options"))
