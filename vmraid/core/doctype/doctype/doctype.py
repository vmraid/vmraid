...
from vmraid.cache_manager import clear_controller_cache, clear_user_cache
...
from vmraid.modules import get_doc_path, make_boilerplate
from vmraid.modules.import_file import get_file_path
from vmraid.query_builder.functions import Concat
from vmraid.utils import cint, now
from vmraid.website.utils import clear_cache


class InvalidFieldNameError(vmraid.ValidationError):
	pass


class UniqueFieldnameError(vmraid.ValidationError):
	pass


class IllegalMandatoryError(vmraid.ValidationError):
	pass


class DoctypeLinkError(vmraid.ValidationError):
	pass


class WrongOptionsDoctypeLinkError(vmraid.ValidationError):
	pass


class HiddenAndMandatoryWithoutDefaultError(vmraid.ValidationError):
	pass


class NonUniqueError(vmraid.ValidationError):
	pass


class CannotIndexedError(vmraid.ValidationError):
	pass


class CannotCreateStandardDoctypeError(vmraid.ValidationError):
	pass
...
class DocType(Document):
	...
	def ensure_minimum_max_attachment_limit(self):
		...
		from vmraid.model import attachment_fieldtypes
		...
		if total_attach_fields > self.max_attachments:
			...
			field_label = vmraid.bold(self.meta.get_field("max_attachments").label)
			vmraid.msgprint(
	...
	def change_modified_of_parent(self):
		...
		if vmraid.flags.in_import:
			return
		parent_list = vmraid.db.get_all(
			"DocField", "parent", dict(fieldtype=["in", vmraid.model.table_fields], options=self.name)
		)
		for p in parent_list:
			vmraid.db.update("DocType", p.parent, {}, for_update=False)
	...
	def on_update(self):
		...
		allow_doctype_export = (
			...
			and (vmraid.conf.developer_mode or vmraid.flags.allow_doctype_export)
		)
	...
	def sync_global_search(self):
		...
		if set(global_search_fields_before_update) != set(global_search_fields_after_update):
			...
			vmraid.enqueue("vmraid.utils.global_search.rebuild_for_doctype", now=now, doctype=self.name)
	...
	def set_base_class_for_controller(self):
		...
		scrubbed_name = vmraid.scrub(self.name)
		scrubbed_module = vmraid.scrub(self.module)
		controller_path = vmraid.get_module_path(
			scrubbed_module, "doctype", scrubbed_name, f"{scrubbed_name}.py"
		)

		document_cls_tag = f"class {despaced_name}(Document)"
		document_import_tag = "from vmraid.model.document import Document"
		website_generator_cls_tag = f"class {despaced_name}(WebsiteGenerator)"
		website_generator_import_tag = "from vmraid.website.website_generator import WebsiteGenerator"
	...
	def after_rename(self, old, new, merge=False):
		...
		if self.issingle:
			...
			vmraid.db.rename_table(old, new)
			vmraid.db.commit()
				...
				with open(fname, "w") as f:
					if fname.endswith(".js"):
						...
						file_content = code.replace(
							vmraid.scrub(old), vmraid.scrub(new)
	...
	def preserve_naming_series_options_in_property_setter(self):
		...
		if not (
			vmraid.db.table_exists(self.name)
			and vmraid.get_all(self.name, fields=["name"], limit=1, as_list=True)
		):
			return

		existing_property_setter = vmraid.db.get_value(
	...
	def make_controller_template(self):
		...
		if self.has_web_view:
			templates_path = vmraid.get_module_path(
				vmraid.scrub(self.module), "doctype", vmraid.scrub(self.name), "templates"
			)
	...
	def make_amendable(self):
		...
		if self.is_submittable:
			docfield_exists = vmraid.get_all(
	...
	def get_max_idx(self):
		...
		max_idx = vmraid.db.sql("""select max(idx) from `tabDocField` where parent = %s""", self.name)
		...
		if not self.is_new():
			...
			if doc_before_save:
				if (self.autoname == "autoincrement" and doc_before_save.autoname != "autoincrement") or (
					...
					vmraid.throw(_("Cannot change to/from Autoincrement naming rule"))
	...
	def validate_name(self, name=None):
		...
		max_length = vmraid.db.MAX_COLUMN_LENGTH - 3
		if len(name) > max_length:
			# length(tab + <Doctype Name>) should be equal to 64 characters hence doctype should be 61 characters
			vmraid.throw(
				_("Doctype name is limited to {0} characters ({1})").format(max_length, name), vmraid.NameError
			)
...
def validate_links_table_fieldnames(meta):
	...
	if not meta.links or vmraid.flags.in_patch or vmraid.flags.in_fixtures:
		return

	fieldnames = tuple(field.fieldname for field in meta.fields)
	for index, link in enumerate(meta.links, 1):
		if not vmraid.get_meta(link.link_doctype).has_field(link.link_fieldname):
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, vmraid.bold(link.link_fieldname), vmraid.bold(link.link_doctype)
			)
		...
		if not link.parent_doctype:
			...
			vmraid.throw(message, vmraid.ValidationError, _("Parent Missing"))

		if not link.table_fieldname:
			message = _("Document Links Row #{0}: Table Fieldname is mandatory for internal links").format(
				index
			)
			vmraid.throw(message, vmraid.ValidationError, _("Table Fieldname Missing"))

		if meta.name == link.parent_doctype:
			field_exists = link.table_fieldname in fieldnames
		else:
			field_exists = vmraid.get_meta(link.parent_doctype).has_field(link.table_fieldname)

		if not field_exists:
			message = _("Document Links Row #{0}: Could not find field {1} in {2} DocType").format(
				index, vmraid.bold(link.table_fieldname), vmraid.bold(meta.name)
			)
			vmraid.throw(message, vmraid.ValidationError, _("Invalid Table Fieldname"))
...
def validate_fields(meta):
		...
		if fieldname in invalid_fields:
			vmraid.throw(
				_("{0}: Fieldname cannot be one of {1}").format(
					docname, ", ".join(vmraid.bold(d) for d in invalid_fields)
				)
			)
	...
	def check_illegal_default(d):
		...
		if d.fieldtype == "Select" and d.default:
			if not d.options:
				...
				vmraid.throw(
					_("Default value for {0} must be in the list of options.").format(vmraid.bold(d.fieldname))
				)
	...
	def check_unique_and_text(docname, d):
		...
		if getattr(d, "unique", False):
			...
			if not d.get("__islocal") and vmraid.db.has_column(d.parent, d.fieldname):
				has_non_unique_values = vmraid.db.sql(
	...
	def check_website_search_field(meta):
		...
		if meta.website_search_field not in fieldname_list:
			vmraid.throw(_("Website Search Field must be a valid fieldname"), InvalidFieldNameError)
		...
		if not meta.istable:
			vmraid.throw(
				_("Option {0} for field {1} is not a child table").format(
					vmraid.bold(doctype), vmraid.bold(docfield.fieldname)
				),
	...
	def check_max_height(docfield):
		if getattr(docfield, "max_height", None) and (docfield.max_height[-2:] not in ("px", "em")):
			vmraid.throw("Max for {} height must be in px, em, rem".format(vmraid.bold(docfield.fieldname)))
	...
	def check_no_of_ratings(docfield):
		if docfield.fieldtype == "Rating":
			if docfield.options and (int(docfield.options) > 10 or int(docfield.options) < 3):
				vmraid.throw(_("Options for Rating field can range from 3 to 10"))
...
def make_module_and_roles(doc, perm_fieldname="permissions"):
	...
	try:
		if (
			...
			and not vmraid.db.exists("Domain", doc.restrict_to_domain)
		):
			vmraid.get_doc(dict(doctype="Domain", domain=doc.restrict_to_domain)).insert()

		if "tabModule Def" in vmraid.db.get_tables() and not vmraid.db.exists("Module Def", doc.module):
			m = vmraid.get_doc({"doctype": "Module Def", "module_name": doc.module})
			if vmraid.scrub(doc.module) in vmraid.local.module_app:
				m.app_name = vmraid.local.module_app[vmraid.scrub(doc.module)]
			else:
				m.app_name = "vmraid"
			m.flags.ignore_mandatory = m.flags.ignore_permissions = True
			if vmraid.flags.package:
				m.package = vmraid.flags.package.name
				m.custom = 1
			m.insert()
		...
		for role in list(set(roles)):
			if vmraid.db.table_exists("Role", cached=False) and not vmraid.db.exists("Role", role):
...
def check_fieldname_conflicts(docfield):
	...
	doc = vmraid.get_doc({"doctype": docfield.dt})
	...
	if docfield.fieldname in method_list + property_list:
		vmraid.msgprint(msg, raise_exception=not docfield.is_virtual)
