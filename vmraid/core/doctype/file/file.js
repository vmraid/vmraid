vmraid.ui.form.on("File", "refresh", function(frm) {
	...
	var is_viewable = vmraid.utils.is_image_file(frm.doc.file_url);
	...
	if (is_optimizable) {
		frm.add_custom_button(__("Optimize"), function() {
			vmraid.show_alert(__("Optimizing image..."));
			frm.call("optimize_file").then(() => {
				vmraid.show_alert(__("Image optimized"));
			});
		});
	}
	...
	if(frm.doc.file_name && frm.doc.file_name.split('.').splice(-1)[0]==='zip') {
		frm.add_custom_button(__('Unzip'), function() {
			vmraid.call({
				method: "vmraid.core.doctype.file.file.unzip_file",
				args: {
					name: frm.doc.name,
				},
				callback: function() {
					vmraid.set_route('List', 'File');
				}
			});
		});
	}
