...
import vmraid
from vmraid import _, conf
from vmraid.model.document import Document
from vmraid.utils import call_hook_method, cint, cstr, encode, get_files_path, get_hook_method, random_string, strip
from vmraid.utils.image import strip_exif_data
...
class File(Document):
	...
	def get_successor(self):
		return vmraid.db.get_values(doctype='File',
	...
	def handle_is_private_changed(self):
		...
		vmraid.db.set_value(self.attached_to_doctype, self.attached_to_name,
	...
	def validate_attachment_limit(self):
		...
		if attachment_limit:
			current_attachment_count = len(vmraid.get_all('File', filters={
	...
	def generate_content_hash(self):
		...
		try:
			...
			vmraid.msgprint(_("File {0} does not exist").format(self.file_url))
			raise
	...
	def call_delete_file(self):
		...
		if self.file_name and self.content_hash and (not vmraid.db.count("File",
	...
	def unzip(self):
		...
		if not ".zip" in self.file_name:
			vmraid.msgprint(_("Not a zip file"))
			return

		zip_path = vmraid.get_site_path(self.file_url.strip('/'))
	...
	def get_file_url(self):
		data = vmraid.db.get_value("File", self.file_data_name, ["file_name", "file_url"], as_dict=True)
	...
	def get_uploaded_content(self):
		# should not be unicode when reading a file, hence using vmraid.form
		if 'filedata' in vmraid.form_dict:
			if "," in vmraid.form_dict.filedata:
				vmraid.form_dict.filedata = vmraid.form_dict.filedata.rsplit(",", 1)[1]
			vmraid.uploaded_content = base64.b64decode(vmraid.form_dict.filedata)
			return vmraid.uploaded_content
		elif self.content:
			return self.content
		vmraid.msgprint(_('No file attached'))
		return None
	...
	def save_file(self, content=None, decode=False, ignore_existing_file_check=False):
		...
		if not ignore_existing_file_check:
			duplicate_file = vmraid.get_value("File", {
					"content_hash": self.content_hash,
					"is_private": self.is_private
				},
	...
	def get_file_data_from_hash(self):
		for name in vmraid.db.sql_list("select name from `tabFile` where content_hash=%s and is_private=%s",
			(self.content_hash, self.is_private)):
			b = vmraid.get_doc('File', name)
			return {k: b.get(k) for k in vmraid.get_hooks()['write_file_keys']}
		return False
	...
	def check_max_file_size(self):
		...
		if file_size > max_file_size:
			vmraid.msgprint(_("File size exceeded the maximum allowed size of {0} MB").format(
...
def get_local_image(file_url):
	...
	try:
		...
		vmraid.msgprint(_("Unable to read file format for {0}").format(file_url), raise_exception=True)
...
def get_web_image(file_url):
	...
	image = Image.open(StringIO(vmraid.safe_decode(r.content)))
...
def delete_file(path):
	...
	if path:
		if ".." in path.split("/"):
			vmraid.msgprint(_("It is risky to delete this file: {0}. Please contact your System Manager.").format(path))
...
def remove_file(fid=None, attached_to_doctype=None, attached_to_name=None, from_delete=False, delete_permanently=False):
	...
	if not (attached_to_doctype and attached_to_name):
		attached = vmraid.db.get_value("File", fid,
	...
	if attached_to_doctype and attached_to_name and not from_delete:
		doc = vmraid.get_doc(attached_to_doctype, attached_to_name)
		ignore_permissions = doc.has_permission("write") or False
		if vmraid.flags.in_web_form:
			ignore_permissions = True
		if not file_name:
			file_name = vmraid.db.get_value("File", fid, "file_name")
		comment = doc.add_comment("Attachment Removed", _("Removed {0}").format(file_name))
		vmraid.delete_doc("File", fid, ignore_permissions=ignore_permissions, delete_permanently=delete_permanently)
...
def remove_all(dt, dn, from_delete=False, delete_permanently=False):
	...
	try:
		for fid in vmraid.db.sql_list("""select name from `tabFile` where
			attached_to_doctype=%s and attached_to_name=%s""", (dt, dn)):
			if from_delete:
				# If deleting a doc, directly delete files
				vmraid.delete_doc("File", fid, ignore_permissions=True, delete_permanently=delete_permanently)
			else:
		...
		try:
			...
			if ptype in ['write', 'create', 'delete']:
				...
				if ptype == 'delete' and not has_access:
					vmraid.throw(_("Cannot delete file as it belongs to {0} {1} for which you do not have permissions").format(
...
def get_attached_images(doctype, names):
	...
	img_urls = vmraid.db.get_list('File', filters={
		'attached_to_doctype': doctype,
...
def get_files_in_folder(folder, start=0, page_length=20):
	...
	attachment_folder = vmraid.db.get_value('File',
		'Home/Attachments',
		['name', 'file_name', 'file_url', 'is_folder', 'modified'],
		as_dict=1
	)

	files = vmraid.db.get_list('File',
		{ 'folder': folder },
		['name', 'file_name', 'file_url', 'is_folder', 'modified'],
		start=start,
