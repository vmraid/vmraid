...
class TestTranslation(unittest.TestCase):
	def setUp(self):
		vmraid.db.sql('delete from tabTranslation')

	def tearDown(self):
		vmraid.local.lang = 'en'
		vmraid.local.lang_full_dict=None

	def test_doctype(self):
		translation_data = get_translation_data()
		for key, val in translation_data.items():
			vmraid.local.lang = key
			vmraid.local.lang_full_dict=None
			translation = create_translation(key, val)
			self.assertEqual(_(val[0]), val[1])

			vmraid.delete_doc('Translation', translation.name)
			vmraid.local.lang_full_dict=None
...
def create_translation(key, val):
	translation = vmraid.new_doc('Translation')
