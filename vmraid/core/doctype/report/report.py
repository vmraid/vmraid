...
class Report(Document):
	def validate(self):
		...
		if not self.is_standard:
			self.is_standard = "No"
			if vmraid.session.user=="Administrator" and getattr(vmraid.local.conf, 'developer_mode',0)==1:
	...
	def execute_query_report(self, filters):
		...
		if not self.query.lower().startswith("select"):
			vmraid.throw(_("Query must be a SELECT"), title=_('Report Document Error'))

		result = [list(t) for t in vmraid.db.sql(self.query, filters, debug=True)]
	...
	def run_query_report(self, filters, user, ignore_prepared_report=False):
		columns, result = [], []
		data = vmraid.desk.query_report.run(self.name,
		...
		for d in data.get('columns'):
			if isinstance(d, dict):
				...
				columns.append(vmraid._dict(label=parts[0], fieldtype=fieldtype, fieldname=parts[0], options=options))
	...
	def run_standard_report(self, filters, limit, user):
		...
		_result = vmraid.get_list(self.ref_doctype,
			fields = [
...
def is_prepared_report_disabled(report):
	return vmraid.db.get_value('Report',
		report, 'disable_prepared_report') or 0

def get_report_module_dotted_path(module, report_name):
	return vmraid.local.module_app[scrub(module)] + "." + scrub(module) \
