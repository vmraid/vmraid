vmraid.ui.form.on('User', {
	...
	refresh: function(frm) {
		...
		if (frm.is_new()) {
			frm.set_value("time_zone", vmraid.sys_defaults.time_zone);
		}
		...
		if(!frm.is_new()) {
			...
			frm.add_custom_button(__("Reset OTP Secret"), function() {
				vmraid.call({
					method: "vmraid.twofactor.reset_otp_secret",
					args: {
						"user": frm.doc.name
					}
				});
	...
	on_update: function(frm) {
		if (vmraid.boot.time_zone && vmraid.boot.time_zone.user !== frm.doc.time_zone) {
