...
from vmraid.core.doctype.user.user import (
	extract_mentions,
	reset_password,
	sign_up,
...
user_module = vmraid.core.doctype.user.user
...
class TestUser(unittest.TestCase):
	...
	def test_delete(self):
		...
		vmraid.db.delete("Has Role", {"role": "_Test Role 2"})
		...
		self.assertTrue(
			not vmraid.db.sql("""select * from `tabToDo` where allocated_to=%s""", ("_test@example.com",))
		)
	...
	def test_get_value(self):
		...
		self.assertEqual(
			vmraid.db.get_value("User", {"email": "test@example.com"}, "email"), "test@example.com"
		)
		self.assertEqual(
			vmraid.db.get_value("User", {"email": "test@example.com"}, ["first_name", "email"]),
		...
		test_user = vmraid.db.sql("select * from tabUser where name='test@example.com'", as_dict=True)[0]
		self.assertEqual(
			vmraid.db.get_value("User", {"email": "test@example.com"}, "*", as_dict=True), test_user
		)
		...
		self.assertEqual(
			vmraid.db.get_value("Website Settings", "Website Settings", "_test"), "_test_val"
		)
	...
	def test_password_strength(self):
		...
		vmraid.flags.in_test = False
		user.new_password = "password"
		self.assertRaisesRegex(vmraid.exceptions.ValidationError, "Invalid Password", user.save)
		user.reload()
	...
	def test_comment_mentions(self):
		...
		vmraid.delete_doc("User Group", "Team")
		doc = vmraid.get_doc(
			{
				"doctype": "User Group",
				"name": "Team",
	...
	def test_user_rename(self):
		...
		vmraid.rename_doc("User", user.name, new_name)
		self.assertTrue(vmraid.db.exists("Notification Settings", new_name))

		vmraid.delete_doc("User", new_name)

	def test_signup(self):
		import vmraid.website.utils

		random_user = vmraid.mock("email")
		random_user_name = vmraid.mock("name")
		# disabled signup
		...
		self.assertEqual(vmraid.cache().hget("redirect_after_login", random_user), "/welcome")
		...
		# disabled user
		user = vmraid.get_doc("User", random_user)
		user.enabled = 0
		user.save()
		...
		with patch.object(user_module.vmraid.db, "get_creation_count", return_value=301):
			self.assertRaisesRegex(
				vmraid.exceptions.ValidationError,
				"Throttled",
				sign_up,
	...
	def test_reset_password(self):
		...
		vmraid.set_user("testpassword@example.com")
		test_user = vmraid.get_doc("User", "testpassword@example.com")
		...
		self.assertRaises(vmraid.exceptions.AuthenticationError, verify_password, old_password)
		...
		self.assertRaisesRegex(
			vmraid.exceptions.ValidationError, "Invalid key type", update_password, "test", 1, ["like", "%"]
		)
		...
		# test API endpoint
		with patch.object(user_module.vmraid, "sendmail") as sendmail:
			vmraid.clear_messages()
			test_user = vmraid.get_doc("User", "test2@example.com")
...
def delete_contact(user):
	vmraid.db.delete("Contact", {"email_id": user})
	vmraid.db.delete("Contact Email", {"email_id": user})
