...
from vmraid.utils import (cint, flt, has_gravatar, escape_html, format_datetime,
	now_datetime, get_formatted_email, today)
from vmraid import throw, msgprint, _
from vmraid.utils.password import update_password as _update_password, check_password, get_password_reset_limit
from vmraid.desk.notifications import clear_notifications
from vmraid.desk.doctype.notification_settings.notification_settings import create_notification_settings, toggle_notifications
from vmraid.utils.user import get_system_managers
from vmraid.website.utils import is_signup_enabled
...
class MaxUsersReachedError(vmraid.ValidationError):
	pass
...
class User(Document):
	...
	def validate(self):
		...
		if (self.name not in ["Administrator", "Guest"]) and (not self.get_social_login_userid("vmraid")):
	...
	def on_update(self):
		...
		vmraid.enqueue(
			'vmraid.core.doctype.user.user.create_contact',
			user=self,
	...
	def check_demo(self):
		if vmraid.session.user == 'demo@erpadda.com':
			vmraid.throw(_('Cannot change user details in demo. Please signup for a new account at https://erpadda.com'), title=_('Not Allowed'))
	...
	def set_roles_and_modules_based_on_user_type(self):
		...
		if user_type_doc.role:
			...
			if user_linked_with_permission_on_doctype(user_type_doc, self.name):
				...
				vmraid.msgprint(_('Role has been set as per the user type {0}')
	...
	def has_desk_access(self):
		...
		return len(vmraid.db.sql("""select name
	...
	def share_with_self(self):
		vmraid.share.add(self.doctype, self.name, self.name, write=1, share=1,
	...
	def reset_password(self, send_email=False, password_expired=False):
		from vmraid.utils import random_string, get_url
	...
	def get_other_system_managers(self):
		return vmraid.db.sql("""select distinct `user`.`name` from `tabHas Role` as `user_role`, `tabUser` as `user`
	...
	def send_login_mail(self, subject, template, add_args, now=None):
		...
		sender = vmraid.session.user not in STANDARD_USERS and get_formatted_email(vmraid.session.user) or None

		vmraid.sendmail(recipients=self.email, sender=sender, subject=subject,
	...
	def on_trash(self):
		...
		# delete todos
		vmraid.db.sql("""DELETE FROM `tabToDo` WHERE `owner`=%s""", (self.name,))
		vmraid.db.sql("""UPDATE `tabToDo` SET `assigned_by`=NULL WHERE `assigned_by`=%s""",
			(self.name,))

		# delete events
		vmraid.db.sql("""delete from `tabEvent` where owner=%s
			and event_type='Private'""", (self.name,))

		# delete shares
		vmraid.db.sql("""delete from `tabDocShare` where user=%s""", self.name)

		# delete messages
		vmraid.db.sql("""delete from `tabCommunication`
			where communication_type in ('Chat', 'Notification')
			and reference_doctype='User'
			and (reference_name=%s or owner=%s)""", (self.name, self.name))

		# unlink contact
		vmraid.db.sql("""update `tabContact`
			set `user`=null
	...
	def after_rename(self, old_name, new_name, merge=False):
		...
		for tab in tables:
			...
			for field in has_fields:
				vmraid.db.sql("""UPDATE `%s`
					SET `%s` = %s
					WHERE `%s` = %s""" %
					(tab, field, '%s', field, '%s'), (new_name, old_name))

		if vmraid.db.exists("Chat Profile", old_name):
			vmraid.rename_doc("Chat Profile", old_name, new_name, force=True, show_alert=False)
	...
	def suggest_username(self):
		...
		if not username:
			# @firstname_last_name
			username = _check_suggestion(vmraid.scrub("{0} {1}".format(self.first_name, self.last_name or "")))
	...
	def username_exists(self, username=None):
		return vmraid.db.get_value("User", {"username": username or self.username, "name": ("!=", self.name)})
	...
	def find_by_credentials(cls, user_name: str, password: str, validate_password: bool = True):
		...
		login_with_mobile = cint(vmraid.db.get_value("System Settings", "System Settings", "allow_login_using_mobile_number"))
		login_with_username = cint(vmraid.db.get_value("System Settings", "System Settings", "allow_login_using_user_name"))
...
def update_password(new_password, logout_all_sessions=0, key=None, old_password=None):
	...
	logout_all_sessions = cint(logout_all_sessions) or vmraid.db.get_single_value("System Settings", "logout_on_password_reset")
...
def test_password_strength(new_password, key=None, old_password=None, user_data=None):
	...
	password_policy = vmraid.db.get_value("System Settings", None,
	...
	if not user_data:
		user_data = vmraid.db.get_value('User', vmraid.session.user,
...
def get_email_awaiting(user):
	waiting = vmraid.db.sql("""select email_account,email_id
		from `tabUser Email`
	...
	if waiting:
		...
		vmraid.db.sql("""update `tabUser Email`
...
def set_email_password(email_account, user, password):
	account = vmraid.get_doc("Email Account", email_account)
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing):
	...
	user_names = vmraid.db.get_values("User", { "email": email_id }, as_dict=True)
	if not user_names:
		return
	...
	for user in user_names:
		...
		user_inbox = vmraid.db.get_value("User Email", {
	...
	if udpate_user_email_settings:
		vmraid.db.sql("""UPDATE `tabUser Email` SET awaiting_password = %(awaiting_password)s,
		...
		users = " and ".join([vmraid.bold(user.get("name")) for user in user_names])
		vmraid.msgprint(_("Enabled email inbox for user {0}").format(users))
...
def remove_user_email_inbox(email_account):
	...
	users = vmraid.get_all("User Email", filters={
		"email_account": email_account
	}, fields=["parent as name"])

	for user in users:
		doc = vmraid.get_doc("User", user.get("name"))
...
def ask_pass_update():
	...
	users = vmraid.db.sql("""SELECT DISTINCT(parent) as user FROM `tabUser Email`
...
def _get_user_for_update_password(key, old_password):
	# verify old password
	if key:
		user = vmraid.db.get_value("User", {"reset_password_key": key})
		if not user:
			return {
...
def sign_up(email, full_name, redirect_to):
	if not is_signup_enabled():
		vmraid.throw(_('Sign Up is disabled'), title='Not Allowed')
	...
	if user:
		...
		if vmraid.db.sql("""select count(*) from tabUser where
			HOUR(TIMEDIFF(CURRENT_TIMESTAMP, TIMESTAMP(modified)))=1""")[0][0] > 300:

			vmraid.respond_as_web_page(_('Temporarily Disabled'),
...
def reset_password(user):
	...
	try:
		...
		return vmraid.msgprint(_("Password reset instructions have been sent to your email"))
...
def user_query(doctype, txt, searchfield, start, page_len, filters):
	from vmraid.desk.reportview import get_match_cond, get_filters_cond
	conditions=[]
	...
	txt = "%{}%".format(txt)
	return vmraid.db.sql("""SELECT `name`, CONCAT_WS(' ', first_name, middle_name, last_name)
		FROM `tabUser`
		WHERE `enabled`=1
			{user_type_condition}
			AND `docstatus` < 2
...
def get_total_users():
	...
	return flt(vmraid.db.sql('''SELECT SUM(`simultaneous_sessions`)
		FROM `tabUser`
		WHERE `enabled` = 1
...
def get_system_users(exclude_users=None, limit=None):
	...
	system_users = vmraid.db.sql_list("""select name from `tabUser`
...
def get_active_users():
	...
	return vmraid.db.sql("""select count(*) from `tabUser`
		where enabled = 1 and user_type != 'Website User'
		and name not in ({})
...
def send_token_via_sms(tmp_id,phone_no=None,user=None):
	try:
		from vmraid.core.doctype.sms_settings.sms_settings import send_request
	except:
		return False

	if not vmraid.cache().ttl(tmp_id + '_token'):
		return False
	ss = vmraid.get_doc('SMS Settings', 'SMS Settings')
	if not ss.sms_gateway_url:
		return False

	token = vmraid.cache().get(tmp_id + '_token')
	...
	if user:
		user_phone = vmraid.db.get_value('User', user, ['phone','mobile_no'], as_dict=1)
	...
	if 200 <= status < 300:
		vmraid.cache().delete(tmp_id + '_token')
		return True
	else:
		return False
...
def send_token_via_email(tmp_id,token=None):
	...
	user = vmraid.cache().get(tmp_id + '_user')
	count = token or vmraid.cache().get(tmp_id + '_token')

	if ((not user) or (user == 'None') or (not count)):
		return False
	user_email = vmraid.db.get_value('User',user, 'email')
	if not user_email:
		return False

	otpsecret = vmraid.cache().get(tmp_id + '_otp_secret')
...
def reset_otp_secret(user):
	otp_issuer = vmraid.db.get_value('System Settings', 'System Settings', 'otp_issuer_name')
	user_email = vmraid.db.get_value('User',user, 'email')
	if vmraid.session.user in ["Administrator", user] :
		vmraid.defaults.clear_default(user + '_otplogin')
		vmraid.defaults.clear_default(user + '_otpsecret')
		email_args = {
			'recipients':user_email, 'sender':None, 'subject':'OTP Secret Reset - {}'.format(otp_issuer or "VMRaid Framework"),
			'message':'<p>Your OTP secret on {} has been reset. If you did not perform this reset and did not request it, please contact your System Administrator immediately.</p>'.format(otp_issuer or "VMRaid Framework"),
			'delayed':False,
			'retry':3
		}
		enqueue(method=vmraid.sendmail, queue='short', timeout=300, event=None, is_async=True, job_name=None, now=False, **email_args)
		return vmraid.msgprint(_("OTP Secret has been reset. Re-registration will be required on next login."))
	else:
		return vmraid.throw(_("OTP secret can only be reset by the Administrator."))
...
def update_roles(role_profile):
	users = vmraid.get_all('User', filters={'role_profile_name': role_profile})
	role_profile = vmraid.get_doc('Role Profile', role_profile)
	roles = [role.role for role in role_profile.roles]
	for d in users:
		user = vmraid.get_doc('User', d)
		user.set('roles', [])
...
def generate_keys(user):
	...
	:param user: str
	"""
	if "System Manager" in vmraid.get_roles():
	...
	vmraid.throw(vmraid._("Not Permitted"), vmraid.PermissionError)
