...
class TestModuleProfile(unittest.TestCase):
	def test_make_new_module_profile(self):
		if not vmraid.db.get_value('Module Profile', '_Test Module Profile'):
			vmraid.get_doc({
		...
		if not vmraid.db.get_value('User', 'test-for-module_profile@example.com'):
			...
			new_user = vmraid.get_doc('User', 'test-for-module_profile@example.com')
