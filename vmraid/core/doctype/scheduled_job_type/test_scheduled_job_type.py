...
class TestScheduledJobType(unittest.TestCase):
	def setUp(self):
		vmraid.db.rollback()
		vmraid.db.truncate("Scheduled Job Type")
		sync_jobs()
		vmraid.db.commit()

	def test_sync_jobs(self):
		all_job = vmraid.get_doc("Scheduled Job Type", dict(method="vmraid.email.queue.flush"))
		self.assertEqual(all_job.frequency, "All")

		daily_job = vmraid.get_doc(
			"Scheduled Job Type", dict(method="vmraid.email.queue.set_expiry_for_email_queue")
		)
		self.assertEqual(daily_job.frequency, "Daily")

		# check if cron jobs are synced
		cron_job = vmraid.get_doc("Scheduled Job Type", dict(method="vmraid.oauth.delete_oauth2_data"))
		...
		updated_scheduled_job = vmraid.get_doc(
			"Scheduled Job Type", {"method": "vmraid.email.queue.flush"}
		)
	...
	def test_weekly_job(self):
		job = vmraid.get_doc(
			"Scheduled Job Type",
			dict(method="vmraid.social.doctype.energy_point_log.energy_point_log.send_weekly_summary"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
	...
	def test_monthly_job(self):
		job = vmraid.get_doc(
			"Scheduled Job Type",
			dict(method="vmraid.email.doctype.auto_email_report.auto_email_report.send_monthly"),
		)
		job.db_set("last_execution", "2019-01-01 00:00:00")
