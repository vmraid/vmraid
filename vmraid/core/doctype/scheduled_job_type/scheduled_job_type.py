...
class ScheduledJobType(Document):
		...
		if self.is_event_due() or force:
			...
			if vmraid.flags.execute_job:
				...
				if not self.is_job_in_queue():
					enqueue(
						"vmraid.core.doctype.scheduled_job_type.scheduled_job_type.run_scheduled_job",
	...
	def update_scheduler_log(self, status):
		...
		if not self.scheduler_log:
			self.scheduler_log = vmraid.get_doc(
	...
	def on_trash(self):
		vmraid.db.delete("Scheduled Job Log", {"scheduled_job_type": self.name})
...
def clear_events(all_events: List):
	for event in vmraid.get_all("Scheduled Job Type", fields=["name", "method", "server_script"]):
