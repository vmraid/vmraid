...
from vmraid import _, throw, msgprint
...
def get_contact_number(contact_name, ref_doctype, ref_name):
	...
	number = vmraid.db.sql("""select mobile_no, phone from tabContact 
		where name=%s 
			and exists(
...
def send_sms(receiver_list, msg, sender_name = '', success_msg = True):
	...
	arg = {
		'receiver_list' : receiver_list,
		'message'		: vmraid.safe_decode(msg).encode('utf-8'),
		'success_msg'	: success_msg
	}

	if vmraid.db.get_value('SMS Settings', None, 'sms_gateway_url'):
...
def send_via_gateway(arg):
	...
	message = vmraid.safe_decode(arg.get('message'))
	...
	if len(success_list) > 0:
		...
		if arg.get('success_msg'):
			vmraid.msgprint(_("SMS sent to following numbers: {0}").format("\n" + "\n".join(success_list)))

def get_headers(sms_settings=None):
	if not sms_settings:
		sms_settings = vmraid.get_doc('SMS Settings', 'SMS Settings')
