...
from vmraid.utils import validate_email_address, strip_html, cstr, time_diff_in_seconds
from vmraid.core.doctype.communication.email import validate_email, notify, _notify
from vmraid.core.utils import get_parent_doc
from vmraid.utils.bot import BotReply
...
from vmraid.automation.doctype.assignment_rule.assignment_rule import apply as apply_assignment_rule
...
class Communication(Document):
	...
	def onload(self):
		...
		if self.communication_type == "Communication" and self.communication_medium == "Email" \
			...
			email_flag_queue = vmraid.db.get_value("Email Flag Queue", {
	...
	def validate_reference(self):
		if self.reference_doctype and self.reference_name:
			if not self.reference_owner:
				self.reference_owner = vmraid.db.get_value(self.reference_doctype, self.reference_name, "owner")

			# prevent communication against a child table
			if vmraid.get_meta(self.reference_doctype).istable:
				vmraid.throw(_("Cannot create a {0} against a child document: {1}")
			...
			if self.reference_doctype == "Communication":
				...
				if circular_linking:
					vmraid.throw(_("Please make sure the Reference Communication Docs are not circularly linked."), vmraid.CircularLinkingError)
	...
	def after_insert(self):
		...
		if self.communication_type == "Communication":
			...
			if self.reference_name == vmraid.session.user:
				...
				vmraid.publish_realtime('new_message', self.as_dict(),
	...
	def set_sender_full_name(self):
		if not self.sender_full_name and self.sender:
			if self.sender == "Administrator":
				...
				if not self.sender_full_name:
					first_name, last_name = vmraid.db.get_value('Contact',
	...
	def set_delivery_status(self, commit=False):
		...
		status_counts = Counter(vmraid.db.sql_list('''select status from `tabEmail Queue` where communication=%s''', self.name))
	...
	def set_timeline_links(self):
		contacts = []
		create_contact_enabled = self.email_account and vmraid.db.get_value("Email Account", self.email_account, "create_contact")
...
def get_permission_query_conditions_for_communication(user):
	...
	if "Super Email User" in roles or "System Manager" in roles:
		...
		accounts = vmraid.get_all("User Email", filters={ "parent": user },
...
def add_contact_links_to_communication(communication, contact_name):
	contact_links = vmraid.get_list("Dynamic Link", filters={
			"parenttype": "Contact",
			"parent": contact_name
...
def set_avg_response_time(parent, communication):
	if parent.meta.has_field("avg_response_time") and communication.sent_or_received == "Sent":
		...
		communications = vmraid.get_list("Communication", filters={
