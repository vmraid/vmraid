...
import vmraid
from vmraid.core.doctype.communication.communication import get_emails
from vmraid.email.doctype.email_queue.email_queue import EmailQueue

test_records = vmraid.get_test_records("Communication")
...
class TestCommunication(unittest.TestCase):
	def test_email(self):
		...
		for x in valid_email_list:
			self.assertTrue(vmraid.utils.parse_addr(x)[1])

		for x in invalid_email_list:
			self.assertFalse(vmraid.utils.parse_addr(x)[0])
	...
	def test_name(self):
		...
		for x in valid_email_list:
			self.assertTrue(vmraid.utils.parse_addr(x)[0])
	...
	def test_circular_linking(self):
		...
		a = vmraid.get_doc("Communication", a.name)
		a.reference_doctype = "Communication"
		a.reference_name = c.name

		self.assertRaises(vmraid.CircularLinkingError, a.save)

	def test_deduplication_timeline_links(self):
		vmraid.delete_doc_if_exists("Note", "deduplication timeline links")
	...
	def test_contacts_attached(self):
		...
		contact_recipient = vmraid.get_doc(
			{
				"doctype": "Contact",
	...
	def test_get_communication_data(self):
		...
		vmraid.delete_doc_if_exists("Note", "get communication data")

		note = vmraid.get_doc(
			{"doctype": "Note", "title": "get communication data", "content": "get communication data"}
		).insert(ignore_permissions=True)

		comm_note_1 = vmraid.get_doc(
			{
				"doctype": "Communication",
				"communication_type": "Communication",
	...
	def test_link_in_email(self):
		vmraid.delete_doc_if_exists("Note", "test document link in email")
...
def create_email_account():
	vmraid.delete_doc_if_exists("Email Account", "_Test Comm Account 1")

	vmraid.flags.mute_emails = False
	vmraid.flags.sent_mail = None

	email_account = vmraid.get_doc(
		{
			"is_default": 1,
			"is_global": 1,
