...
def make(
	...
	**kwargs,
	...
	if kwargs:
		...
		warn(
			f"Options {kwargs} used in vmraid.core.doctype.communication.email.make "
			"are deprecated or unsupported",
			category=DeprecationWarning,
		)

	if doctype and name and not vmraid.has_permission(doctype=doctype, ptype="email", doc=name):
		raise vmraid.PermissionError(f"You are not allowed to send emails related to: {doctype} {name}")
...
def _make(
	...
	if cint(send_email):
		if not comm.get_outgoing_email_account():
			vmraid.throw(msg=OUTGOING_EMAIL_ACCOUNT_MISSING, exc=vmraid.OutgoingEmailError)
...
@vmraid.whitelist(allow_guest=True, methods=("GET",))
def mark_email_as_seen(name: str = None):
	try:
		update_communication_as_read(name)
		vmraid.db.commit()  # nosemgrep: this will be called in a GET request

	except Exception:
		vmraid.log_error(vmraid.get_traceback())

	finally:
		vmraid.response.update(
			{
				"type": "binary",
...
def update_communication_as_read(name):
	...
	communication = vmraid.db.get_value("Communication", name, "read_by_recipient", as_dict=True)
