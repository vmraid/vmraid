...
from vmraid.utils import (get_url, get_formatted_email, cint, list_to_str,
	validate_email_address, split_emails, parse_addr, get_datetime)
from vmraid.email.email_body import get_message_id
import vmraid.email.smtp
import time
...
def make(doctype=None, name=None, content=None, subject=None, sent_or_received = "Sent",
	...
	is_error_report = (doctype=="User" and name==vmraid.session.user and subject=="Error Report")
	send_me_a_copy = cint(send_me_a_copy)

	if not ignore_permissions:
		if doctype and name and not is_error_report and not vmraid.has_permission(doctype, "email", name) and not (flags or {}).get('ignore_doctype_permissions'):
			raise vmraid.PermissionError("You are not allowed to send emails related to: {doctype} {name}".format(
				doctype=doctype, name=name))

	if not sender:
		sender = get_formatted_email(vmraid.session.user)
	...
	if cint(send_email):
		vmraid.flags.print_letterhead = cint(print_letterhead)
...
def notify(doc, print_html=None, print_format=None, attachments=None,
	...
	if vmraid.flags.in_test:
		...
		enqueue(sendmail, queue="default", timeout=300, event="sendmail",
			...
			recipients=recipients, cc=cc, bcc=bcc, lang=vmraid.local.lang,
			session=vmraid.local.session, print_letterhead=vmraid.flags.print_letterhead)
...
def _notify(doc, print_html=None, print_format=None, attachments=None,
	...
	vmraid.sendmail(
		...
		print_letterhead=vmraid.flags.print_letterhead
	)
...
def prepare_to_notify(doc, print_html=None, print_format=None, attachments=None):
	...
	view_link = vmraid.utils.cint(vmraid.db.get_value("System Settings", "System Settings", "attach_view_link"))
		...
		for a in attachments:
			if isinstance(a, string_types):
				...
				try:
					...
					file_id = vmraid.db.get_list('File', or_filters={'file_name': a, 'name': a}, limit=1)
					if not file_id:
						vmraid.throw(_("Unable to find attachment {0}").format(a))
					file_id = file_id[0]['name']
					_file = vmraid.get_doc("File", file_id)
					_file.get_content()
...
def get_recipients(doc, fetched_from_email_account=False):
	...
	recipients = split_emails(doc.recipients)
		...
		#doc.previous_email_sender = vmraid.db.get_value("Communication", doc.in_reply_to, "sender")
		#recipients.append(doc.previous_email_sender)
...
def get_cc(doc, recipients=None, fetched_from_email_account=False):
	...
	if cc:
		...
		exclude += [d[0] for d in vmraid.db.get_all("User", ["email"], {"thread_notify": 0}, as_list=True)]
		...
		if doc.reference_doctype and doc.reference_name:
			exclude += [d[0] for d in vmraid.db.get_all("Email Unsubscribe", ["email"],
...
def add_attachments(name, attachments):
	...
	for a in attachments:
		if isinstance(a, string_types):
			attach = vmraid.db.get_value("File", {"name":a},
...
def filter_email_list(doc, email_list, exclude, is_cc=False, is_bcc=False):
	...
	for email in list(set(email_list)):
		...
		if is_cc:
			is_user_enabled = vmraid.db.get_value("User", email_address, "enabled")
...
def get_assignees(doc):
	return [( get_formatted_email(d.owner) or d.owner ) for d in
		vmraid.db.get_all("ToDo", filters={
...
def get_attach_link(doc, print_format):
	...
	return vmraid.get_template("templates/emails/print_link.html").render({
		"url": get_url(),
...
def sendmail(communication_name, print_html=None, print_format=None, attachments=None,
	...
	try:
		...
		if session:
			...
			session['data'] = vmraid._dict(session['data'])
			vmraid.local.session.update(session)

		if print_letterhead:
			vmraid.flags.print_letterhead = print_letterhead

		# upto 3 retries
		for i in range(3):
			try:
				communication = vmraid.get_doc("Communication", communication_name)
				communication._notify(print_html=print_html, print_format=print_format, attachments=attachments,
					recipients=recipients, cc=cc, bcc=bcc)

			except vmraid.db.InternalError as e:
				# deadlock, try again
				if vmraid.db.is_deadlocked(e):
					vmraid.db.rollback()
					time.sleep(1)
					continue
				else:
		...
		traceback = vmraid.log_error("vmraid.core.doctype.communication.email.sendmail")
		raise

@vmraid.whitelist(allow_guest=True)
def mark_email_as_seen(name=None):
	try:
		if name and vmraid.db.exists("Communication", name) and not vmraid.db.get_value("Communication", name, "read_by_recipient"):
			vmraid.db.set_value("Communication", name, "read_by_recipient", 1)
			vmraid.db.set_value("Communication", name, "delivery_status", "Read")
			vmraid.db.set_value("Communication", name, "read_by_recipient_on", get_datetime())
			vmraid.db.commit()
	except Exception:
		...
		vmraid.response["filename"] = "imaginary_pixel.png"
		vmraid.response["filecontent"] = buffered_obj.getvalue()
