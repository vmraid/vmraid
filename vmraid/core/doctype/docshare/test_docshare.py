...
class TestDocShare(unittest.TestCase):
	...
	def test_share_with_submit_perm(self):
		...
		submittable_doc = vmraid.get_doc(
		...
		vmraid.set_user(self.user)
		self.assertTrue(
			vmraid.has_permission(doctype, "submit", doc=submittable_doc.name, user=self.user)
		)

		# test cascade
		self.assertTrue(vmraid.has_permission(doctype, "read", doc=submittable_doc.name, user=self.user))
		self.assertTrue(
			vmraid.has_permission(doctype, "write", doc=submittable_doc.name, user=self.user)
		)
