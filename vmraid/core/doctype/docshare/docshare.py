...
from vmraid.utils import cint, get_fullname
...
class DocShare(Document):
	...
	def check_share_permission(self):
		if not self.flags.ignore_share_permission and not vmraid.has_permission(
	...
	def check_is_submittable(self):
		if self.submit and not cint(
			vmraid.db.get_value("DocType", self.share_doctype, "is_submittable")
		):
			vmraid.throw(
