...
from vmraid.permissions import (get_valid_perms, update_permission_property)
from vmraid import _
from vmraid.utils import cstr
from vmraid.core.utils import find
from vmraid.desk.form.linked_with import get_linked_doctypes
...
class UserPermission(Document):
	...
	def on_update(self):
		...
		vmraid.publish_realtime('update_user_permissions')
	...
	def validate_user_permission(self):
		...
		duplicate_exists = vmraid.db.get_all(self.doctype, filters={
			'allow': self.allow,
		...
		if duplicate_exists:
			vmraid.throw(_("User permission already exists"), vmraid.DuplicateEntryError)

	def validate_default_permission(self):
		''' validate user permission overlap for default value of a particular doctype '''
		overlap_exists = []
		if self.is_default:
			overlap_exists = vmraid.get_all(self.doctype, filters={
				'allow': self.allow,
				'user': self.user,
				'is_default': 1,
				'name': ['!=', self.name]
		...
		if overlap_exists:
			ref_link = vmraid.get_desk_link(self.doctype, overlap_exists[0].name)
			vmraid.throw(_("{0} has already assigned default value for {1}.").format(ref_link, self.allow))
...
def get_user_permissions(user=None):
	...
	if vmraid.request and vmraid.local.form_dict.cmd == 'get_user_permissions':
	...
	cached_user_permissions = vmraid.cache().hget("user_permissions", user)
	...
	def add_doc_to_perm(perm, doc_name, is_default):
		...
		out[perm.allow].append(vmraid._dict({
			'doc': doc_name,
...
def check_applicable_doc_perm(user, doctype, docname):
	...
	applicable = []
	doc_exists = vmraid.get_all('User Permission',
		fields=['name'],
		filters={"user": user,
			"allow": doctype,
			"for_value": docname,
	...
	if doc_exists:
		...
		data = vmraid.get_all('User Permission',
...
def clear_user_permissions(user, for_doctype):
	vmraid.only_for('System Manager')
	total = vmraid.db.count('User Permission', filters = dict(user=user, allow=for_doctype))
	if total:
		vmraid.db.sql('DELETE FROM `tabUser Permission` WHERE `user`=%s AND `allow`=%s', (user, for_doctype))
		vmraid.clear_cache()
	return total
...
def add_user_permissions(data):
	...
	exists = vmraid.db.exists("User Permission", {
		"user": data.user,
		"allow": data.doctype,
...
def insert_user_perm(user, doctype, docname, is_default=0, hide_descendants=0, apply_to_all=None, applicable=None):
	user_perm = vmraid.new_doc("User Permission")
	user_perm.user = user
