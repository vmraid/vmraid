...
class Role(Document):
	def before_rename(self, old, new, merge=False):
		if old in ("Guest", "Administrator", "System Manager", "All"):
			vmraid.throw(vmraid._("Standard roles cannot be renamed"))

	def after_insert(self):
		vmraid.cache().hdel('roles', 'Administrator')
	...
	def disable_role(self):
		if self.name in ("Guest", "Administrator", "System Manager", "All"):
			vmraid.throw(vmraid._("Standard roles cannot be disabled"))
		else:
			self.remove_roles()
	...
	def remove_roles(self):
		vmraid.db.sql("delete from `tabHas Role` where role = %s", self.name)
		vmraid.clear_cache()

	def on_update(self):
		'''update system user desk access if this has changed in this update'''
		if vmraid.flags.in_install: return
...
def get_info_based_on_role(role, field='email'):
	...
	users = vmraid.get_list("Has Role", filters={"role": role, "parenttype": "User"},
...
def get_user_info(users, field='email'):
	...
	info_list = []
	for user in users:
		user_info, enabled = vmraid.db.get_value("User", user.get("user_name"), [field, "enabled"])
		if enabled and user_info not in ["admin@example.com", "guest@example.com"]:
			info_list.append(user_info)
	return info_list

def get_users(role):
	return [d.parent for d in vmraid.get_all("Has Role", filters={"role": role, "parenttype": "User"},
		fields=["parent"])]
...
def role_query(doctype, txt, searchfield, start, page_len, filters):
	...
	return vmraid.get_all('Role', limit_start=start, limit_page_length=page_len,
