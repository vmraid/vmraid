...
vmraid.ui.form.on('Data Import', {
	setup(frm) {
		vmraid.realtime.on('data_import_refresh', ({ data_import }) => {
			frm.import_in_progress = false;
			if (data_import !== frm.doc.name) return;
			vmraid.model.clear_doc('Data Import', frm.doc.name);
			vmraid.model.with_doc('Data Import', frm.doc.name).then(() => {
				frm.refresh();
			});
		});
		vmraid.realtime.on('data_import_progress', data => {
		...
		frm.set_query('reference_doctype', () => {
			return {
				filters: {
					name: ['in', vmraid.boot.user.can_import]
				}
			};
		});
	...
	refresh(frm) {
		...
		if (frm.doc.status.includes('Success')) {
			frm.add_custom_button(
				...
				() => vmraid.set_route('List', frm.doc.reference_doctype)
			);
		}
	},
	...
	update_indicators(frm) {
		const indicator = vmraid.get_indicator(frm.doc);
		if (indicator) {
	...
	show_import_status(frm) {
		vmraid.call({
			'method': 'vmraid.core.doctype.data_import.data_import.get_import_status',
			'args': {
				...
				.then(result => {
					if (result.length > 0) {
						frm.add_custom_button('Report Error', () => {
							...
							vmraid.request.report_error(fake_xhr, {});
						});
					}
				});
		}
	...
	download_template(frm) {
		vmraid.require('data_import_tools.bundle.js', () => {
			frm.data_exporter = new vmraid.data_import.DataExporter(
	...
	toggle_submit_after_import(frm) {
		...
		if (doctype) {
			vmraid.model.with_doctype(doctype, () => {
				let meta = vmraid.get_meta(doctype);
	...
	show_import_preview(frm, preview_data) {
		...
		vmraid.require('data_import_tools.bundle.js', () => {
			frm.import_preview = new vmraid.data_import.ImportPreview({
	...
	export_errored_rows(frm) {
		open_url_post(
			'/api/method/vmraid.core.doctype.data_import.data_import.download_errored_template',
			{
				data_import_name: frm.doc.name
			}
		);
	...
	export_import_log(frm) {
		open_url_post(
			'/api/method/vmraid.core.doctype.data_import.data_import.download_import_log',
			{
				data_import_name: frm.doc.name
			}
		);
						...
						if (log.success) {
							if (frm.doc.import_type === 'Insert New Records') {
								html = __('Successfully imported {0}', [
									`<span class="underline">${vmraid.utils.get_form_link(
										frm.doc.reference_doctype,
										log.docname,
										true
									)}<span>`
							...
							let messages = (JSON.parse(log.messages || '[]'))
								...
								.join('');
							let id = vmraid.dom.get_unique_id();
							html = `${messages}
	...
	show_import_log(frm) {
		...
		vmraid.call({
			'method': 'vmraid.client.get_count',
			'args': {
				'doctype': 'Data Import Log',
				'filters': {
					'data_import': frm.doc.name
