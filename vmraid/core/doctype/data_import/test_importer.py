...
from vmraid.utils import getdate, format_duration
...
class TestImporter(unittest.TestCase):
	...
	def test_data_import_without_mandatory_values(self):
		...
		data_import.start_import()
		data_import.reload()
		import_log = vmraid.parse_json(data_import.import_log)
		...
		self.assertEqual(vmraid.parse_json(import_log[0]['messages'][0])['message'], expected_error)
		expected_error = "Error: <b>Child 1 of DocType for Import</b> Row #2: Value missing for: Child Title"
		self.assertEqual(vmraid.parse_json(import_log[0]['messages'][1])['message'], expected_error)

		self.assertEqual(import_log[1]['row_indexes'], [4])
		self.assertEqual(vmraid.parse_json(import_log[1]['messages'][0])['message'], "Title is required")
...
def get_import_file(csv_file_name, force=False):
	...
	if vmraid.db.exists('File', {'file_name': file_name}):
		...
		f = vmraid.get_doc(
			doctype='File',
			content=vmraid.read_file(full_path),
			file_name=file_name,
			is_private=1
		)
