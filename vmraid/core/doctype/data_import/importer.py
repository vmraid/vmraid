...
from vmraid.utils import cint, flt, update_progress_bar, cstr, duration_to_seconds
from vmraid.utils.csvutils import read_csv_content, get_csv_content_from_google_sheets
from vmraid.utils.xlsxutils import (
	read_xlsx_file_from_attached_file,
	read_xls_file_from_attached_file,
)
from vmraid.model import no_value_fields, table_fields as table_fieldtypes
...
class Importer:
	...
	def import_data(self):
		...
		if self.data_import.import_log:
			import_log = vmraid.parse_json(self.data_import.import_log)
		else:
			import_log = []
		...
		for batch_index, batched_payloads in enumerate(
			vmraid.utils.create_batch(payloads, batch_size)
		):
			for i, payload in enumerate(batched_payloads):
				...
				try:
					...
					import_log.append(
						vmraid._dict(success=True, docname=doc.name, row_indexes=row_indexes)
					)
	...
	def export_errored_rows(self):
		...
		import_log = vmraid.parse_json(self.data_import.import_log or "[]")
		failures = [log for log in import_log if not log.get("success")]
		row_indexes = []
		for f in failures:
...
class ImportFile:
	def __init__(self, doctype, file, template_options=None, import_type=None):
		self.doctype = doctype
		self.template_options = template_options or vmraid._dict(
			column_to_field_map=vmraid._dict()
		)
...
class Row:
	...
	def _parse_doc(self, doctype, columns, values, parent_doc=None, table_df=None):
		...
		for key in vmraid.model.default_fields + ("__islocal",):
			doc.pop(key, None)
		...
		if df.fieldtype == "Select":
			...
			if not exists:
				msg = _("Value {0} missing for {1}").format(
					vmraid.bold(value), vmraid.bold(df.options)
				)
				self.warnings.append(
					{"row": self.row_number, "field": df_as_json(df), "message": msg,}
				)
...
class Column:
	...
	def validate_values(self):
		...
		if self.df.fieldtype == "Link":
			...
			exists = [
				d.name for d in vmraid.db.get_all(self.df.options, filters={"name": ("in", values)})
			]
