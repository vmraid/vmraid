...
import re, csv, os
from vmraid.utils.csvutils import UnicodeWriter
from vmraid.utils import cstr, formatdate, format_datetime, parse_json, cint, format_duration
from vmraid.core.doctype.data_import_legacy.importer import get_data_keys
...
class DataExporter:
	...
	def prepare_args(self):
		...
		if self.all_doctypes:
			self.child_doctypes = []
			for df in vmraid.get_meta(self.doctype).get_table_fields():
	...
	def build_response(self):
		...
		if self.with_data and not self.data:
			vmraid.respond_as_web_page(_('No Data'), _('There is no data to be exported'), indicator_color='orange')

		if self.file_type == 'Excel':
			self.build_response_as_excel()
		else:
			# write out response as a type csv
			vmraid.response['result'] = cstr(self.writer.getvalue())
			vmraid.response['type'] = 'csv'
			vmraid.response['doctype'] = self.doctype
	...
	def build_field_columns(self, dt, parentfield=None):
		meta = vmraid.get_meta(dt)

		# build list of valid docfields
		tablecolumns = []
		table_name = 'tab' + dt
		for f in vmraid.db.get_table_columns_description(table_name):
		...
		_column_start_end = vmraid._dict(start=0)
		...
		if dt==self.doctype:
			...
			if meta.get('istable') and not self.with_data:
				self.append_field_column(vmraid._dict({
					"fieldname": "parent",
					"parent": "",
					"label": "Parent",
					"fieldtype": "Data",
			...
			_column_start_end = vmraid._dict(start=len(self.columns))
	...
	def getinforow(docfield):
		...
		if docfield.fieldtype == 'Select':
			...
			return cstr(vmraid.defaults.get_defaults().date_format)
	...
	def add_data(self):
		...
		vmraid.permissions.can_export(self.parent_doctype, raise_exception=True)

		# sort nested set doctypes by `lft asc`
		order_by = None
		table_columns = vmraid.db.get_table_columns(self.parent_doctype)
		if 'lft' in table_columns and 'rgt' in table_columns:
			order_by = '`tab{doctype}`.`lft` asc'.format(doctype=self.parent_doctype)
		# get permitted data only
		self.data = vmraid.get_list(self.doctype, fields=["*"], filters=self.filters, limit_page_length=None, order_by=order_by)
		...
		for doc in self.data:
			...
			if self.all_doctypes:
				...
				for c in self.child_doctypes:
					for ci, child in enumerate(vmraid.db.sql("""select * from `tab{0}`
	...
	def build_response_as_excel(self):
		filename = vmraid.generate_hash("", 10)
		...
		vmraid.response['filename'] = self.doctype + '.xlsx'
