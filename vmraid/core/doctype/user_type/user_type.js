...
vmraid.ui.form.on('User Type', {
	refresh: function(frm) {
		frm.toggle_display('is_standard', vmraid.boot.developer_mode);
		frm.set_df_property('is_standard', 'read_only', !vmraid.boot.developer_mode);
		...
		frm.set_query('apply_user_permission_on', function() {
			return {
				query: "vmraid.core.doctype.user_type.user_type.get_user_linked_doctypes"
			};
		});
	},
	...
	get_user_id_fields: function(frm) {
		if (frm.doc.apply_user_permission_on) {
			vmraid.call({
				method: 'vmraid.core.doctype.user_type.user_type.get_user_id',
				args: {
