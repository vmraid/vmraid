...
class UserType(Document):
	...
	def on_trash(self):
		if self.is_standard:
			vmraid.throw(_("Standard user type {0} can not be deleted.").format(vmraid.bold(self.name)))

	def set_modules(self):
		if not self.user_doctypes:
			return

		modules = vmraid.get_all(
			"DocType",
	...
	def validate_document_type_limit(self):
		...
		if not limit and vmraid.session.user != "Administrator":
			vmraid.throw(
				_("User does not have permission to create the new {0}").format(vmraid.bold(_("User Type"))),
	...
	def validate_role(self):
		if not self.role:
			vmraid.throw(_("The field {0} is mandatory").format(vmraid.bold(_("Role"))))

		if not vmraid.db.get_value("Role", self.role, "is_custom"):
			vmraid.throw(
				_("The role {0} should be a custom role.").format(
					vmraid.bold(get_link_to_form("Role", self.role))
				)
			)
	...
	def add_role_permissions_for_file(self):
		...
		vmraid.db.set_value("Custom DocPerm", docperm, {"read": 1, "create": 1, "write": 1})
...
def add_role_permissions(doctype, role):
	name = vmraid.get_value("Custom DocPerm", dict(parent=doctype, role=role, permlevel=0))
...
def get_user_linked_doctypes(doctype, txt, searchfield, start, page_len, filters):
	...
	custom_doctypes = vmraid.get_all(
...
def user_linked_with_permission_on_doctype(doc, user):
	...
	if not doc.user_id_field:
		vmraid.throw(_("User Id Field is mandatory in the user type {0}").format(vmraid.bold(doc.name)))

	if vmraid.db.get_value(doc.apply_user_permission_on, {doc.user_id_field: user}, "name"):
		return True
	else:
		...
		vmraid.msgprint(
			_(
				...
				vmraid.bold(doc.role),
				vmraid.bold(user),
				vmraid.bold(label),
				vmraid.bold(user),
				vmraid.bold(doc.apply_user_permission_on),
			)
		)
