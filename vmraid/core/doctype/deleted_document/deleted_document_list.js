vmraid.listview_settings["Deleted Document"] = {
	onload: function (doclist) {
		const action = () => {
			...
			if (selected_docs.length > 0) {
				...
				vmraid.call({
					method: "vmraid.core.doctype.deleted_document.deleted_document.bulk_restore",
					args: { docnames },
					callback: function (r) {
						if (r.message) {
							let body = (docnames) => {
							...
							vmraid.msgprint(summary, __("Document Restoration Summary"), true);
