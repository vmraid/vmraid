vmraid.ui.form.on("System Settings", {
	refresh: function(frm) {
		vmraid.call({
			method: "vmraid.core.doctype.system_settings.system_settings.load",
			callback: function(data) {
				vmraid.all_timezones = data.message.timezones;
				frm.set_df_property("time_zone", "options", vmraid.all_timezones);

				$.each(data.message.defaults, function(key, val) {
					frm.set_value(key, val);
					vmraid.sys_defaults[key] = val;
				});
				if (frm.re_setup_moment) {
					vmraid.app.setup_moment();
	...
	on_update: function(frm) {
		if (vmraid.boot.time_zone && vmraid.boot.time_zone.system !== frm.doc.time_zone) {
