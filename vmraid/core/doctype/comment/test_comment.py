...
class TestComment(unittest.TestCase):
	def test_comment_creation(self):
		...
		comment_1 = vmraid.get_all('Comment', fields = ['*'], filters = dict(
	...
	def test_public_comment(self):
		...
		vmraid.db.sql("delete from `tabComment` where reference_doctype = 'Blog Post'")
		...
		self.assertEqual(vmraid.get_all('Comment', fields = ['*'], filters = dict(
		...
		self.assertEqual(len(vmraid.get_all('Comment', fields = ['*'], filters = dict(
			reference_doctype = test_blog.doctype,
			reference_name = test_blog.name
		))), 0)
