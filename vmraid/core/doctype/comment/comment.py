	...
	def notify_mentions(self):
		if self.reference_doctype and self.reference_name and self.content:
			...
			notification_message = _("""{0} mentioned you in a comment in {1} {2}""").format(
				vmraid.bold(sender_fullname), vmraid.bold(self.reference_doctype), get_title_html(title)
			)
...
def update_comments_in_parent(reference_doctype, reference_name, _comments):
	...
	if (
		...
		or vmraid.db.get_value("DocType", reference_doctype, "issingle")
		or vmraid.db.get_value("DocType", reference_doctype, "is_virtual")
	):
		return
