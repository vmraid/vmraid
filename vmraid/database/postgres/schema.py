...
class PostgresTable(DBTable):
	def create(self):
		...
		vmraid.db.sql("""create table `%s` (
			...
			%s)""".format(varchar_len=vmraid.db.VARCHAR_LEN) % (self.table_name, add_text))
	...
	def alter(self):
		...
		for col in self.drop_index:
			...
			if col.fieldname != 'name':
				# if index key exists
				if not vmraid.db.has_index(self.table_name, col.fieldname):
		...
		if query:
			try:
				...
				if final_alter_query: vmraid.db.sql(final_alter_query)
				if create_index_query: vmraid.db.sql(create_index_query)
				if drop_index_query: vmraid.db.sql(drop_index_query)
				...
				if vmraid.db.is_duplicate_fieldname(e):
					...
					vmraid.throw(_("""{0} field cannot be set as unique in {1},
