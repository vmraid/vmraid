...
from vmraid.query_builder import Criterion, Field, Order


def like(key: str, value: str) -> vmraid.qb:
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `LIKE`
	"""
	return Field(key).like(value)


def func_in(key: str, value: Union[List, Tuple]) -> vmraid.qb:
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `IN`
	"""
	return Field(key).isin(value)


def not_like(key: str, value: str) -> vmraid.qb:
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `NOT LIKE`
	"""
...
def func_not_in(key: str, value: Union[List, Tuple]):
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `NOT IN`
	"""
	return Field(key).notin(value)


def func_regex(key: str, value: str) -> vmraid.qb:
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `REGEX`
	"""
	return Field(key).regex(value)


def func_between(key: str, value: Union[List, Tuple]) -> vmraid.qb:
	...
	Returns:
	        vmraid.qb: `vmraid.qb object with `BETWEEN`
	"""
...
class Query:
	def get_condition(self, table: str, **kwargs) -> vmraid.qb:
		...
		Returns:
		        vmraid.qb: DocType with initial condition
		"""
		if kwargs.get("update"):
			return vmraid.qb.update(table)
		if kwargs.get("into"):
			return vmraid.qb.into(table)
		return vmraid.qb.from_(table)

	def criterion_query(self, table: str, criterion: Criterion, **kwargs) -> vmraid.qb:
		...
		Returns:
		        vmraid.qb: condition object
		"""
		condition = self.add_conditions(self.get_condition(table, **kwargs), **kwargs)
		return condition.where(criterion)

	def add_conditions(self, conditions: vmraid.qb, **kwargs):
		"""Adding additional conditions

		Args:
		        conditions (vmraid.qb): built conditions

		Returns:
		        conditions (vmraid.qb): vmraid.qb object
		"""
	...
	def build_conditions(
		...
		Returns:
		        vmraid.qb: vmraid.qb conditions object
		"""
...
class Permission:
	...
	def check_permissions(cls, query, **kwargs):
		...
		for dt in doctype:
			dt = re.sub("^tab", "", dt)
			if not vmraid.has_permission(
				dt,
				"select",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			) and not vmraid.has_permission(
				dt,
				"read",
				user=kwargs.get("user"),
				parent_doctype=kwargs.get("parent_doctype"),
			):
				vmraid.throw(_("Insufficient Permission for {0}").format(vmraid.bold(dt)))
