...
from vmraid.utils import now, getdate, cast_fieldtype, get_datetime
...
class Database(object):
		...
		if query[:6].lower() in ('update', 'insert', 'delete'):
			...
			if self.transaction_writes > 200000:
				if self.auto_commit_on_many_writes:
					...
					vmraid.throw(_("Too many writes in one request. Please send smaller requests"), vmraid.ValidationError)
	...
	def get_single_value(self, doctype, fieldname, cache=False):
		...
		if not df:
			vmraid.throw(_('Invalid field name: {0}').format(vmraid.bold(fieldname)), self.InvalidColumnName)

		if df.fieldtype in vmraid.model.numeric_fieldtypes:
			val = cint(val)
	...
	def set_value(self, dt, dn, field, val=None, modified=None, modified_by=None,
		...
		**Warning:** this function will not call Document events and should be avoided in normal cases.
		...
		if not modified:
			modified = now()
		if not modified_by:
			modified_by = vmraid.session.user
		...
		vmraid.clear_document_cache(dt, dn)
	...
	def delete(self, doctype, conditions, debug=False):
		if conditions:
			...
			vmraid.throw(_('No conditions provided'))
...
def _cast_result(doctype, result):
	...
	try:
		for field, value in result:
			df = vmraid.get_meta(doctype).get_field(field)
			if df:
