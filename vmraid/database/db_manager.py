...
class DbManager:
	...
	def restore_database(target, source, user, password):
		from vmraid.utils import make_esc
		esc = make_esc('$ ')
		...
		command = '{pipe} mysql -u {user} -p{password} -h{host} ' + ('-P{port}' if vmraid.db.port else '') + ' {target} {source}'
		command = command.format(
			pipe=pipe,
			user=esc(user),
			password=esc(password),
			host=esc(vmraid.db.host),
			target=esc(target),
			source=source,
			port=vmraid.db.port
		)
