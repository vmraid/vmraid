...
from vmraid.utils import cstr, cint, flt


class InvalidColumnName(vmraid.ValidationError): pass

class DBTable:
	def __init__(self, doctype, meta=None):
		self.doctype = doctype
		self.table_name = 'tab{}'.format(doctype)
		self.meta = meta or vmraid.get_meta(doctype, False)
		self.columns = {}
	...
	def sync(self):
		...
		if self.is_new():
			...
			vmraid.cache().hdel('table_columns', self.table_name)
			self.alter()
	...
	def get_column_definitions(self):
		column_list = [] + vmraid.db.DEFAULT_COLUMNS
		ret = []
	...
	def get_index_definitions(self):
		...
		for key, col in self.columns.items():
			if (col.set_index
				...
				and vmraid.db.type_map.get(col.fieldtype)[0]
	...
	def get_columns_from_docfields(self):
		...
		if not self.meta.get('istable'):
			for fieldname in vmraid.db.OPTIONAL_COLUMNS:
				fields.append({
					"fieldname": fieldname,
					"fieldtype": "Text"
				})
	...
	def validate(self):
		...
		columns = [vmraid._dict({"fieldname": f, "fieldtype": "Data"}) for f in
			vmraid.db.STANDARD_VARCHAR_COLUMNS]
		columns += self.columns.values()

		for col in columns:
			if len(col.fieldname) >= 64:
				vmraid.throw(_("Fieldname is limited to 64 characters ({0})")
					.format(vmraid.bold(col.fieldname)))

			if 'varchar' in vmraid.db.type_map.get(col.fieldtype, ()):

				# validate length range
				new_length = cint(col.length) or cint(vmraid.db.VARCHAR_LEN)
				if not (1 <= new_length <= 1000):
					vmraid.throw(_("Length of {0} should be between 1 and 1000").format(col.fieldname))
				...
				if cint(current_length) != cint(new_length):
					try:
						...
						if vmraid.db.is_missing_column(e):
					...
					if max_length and max_length[0][0] and max_length[0][0] > new_length:
						...
						vmraid.msgprint(info_message)

	def is_new(self):
		return self.table_name not in vmraid.db.get_tables()

	def setup_table_columns(self):
		# TODO: figure out a way to get key data
		for c in vmraid.db.get_table_columns_description(self.table_name):
...
class DbColumn:
	...
	def get_definition(self, with_default=1):
		...
		elif self.default and (self.default not in vmraid.db.DEFAULT_SHORTCUTS) \
			and not cstr(self.default).startswith(":") and column_def not in ('text', 'longtext'):
			column_def += " default {}".format(vmraid.db.escape(self.default))
	...
	def build_for_alter_table(self, current_def):
		...
		if (self.default_changed(current_def)
			and (self.default not in vmraid.db.DEFAULT_SHORTCUTS)
...
def validate_column_name(n):
	...
	if special_characters:
		...
		vmraid.throw(_("Fieldname {0} cannot have special characters like {1}").format(
			vmraid.bold(cstr(n)), special_characters), vmraid.db.InvalidColumnName)
	return n

def validate_column_length(fieldname):
	if len(fieldname) > vmraid.db.MAX_COLUMN_LENGTH:
		vmraid.throw(_("Fieldname is limited to 64 characters ({0})").format(fieldname))

def get_definition(fieldtype, precision=None, length=None):
	d = vmraid.db.type_map.get(fieldtype)

	# convert int to long int if the length of the int is greater than 11
	if fieldtype == "Int" and length and length > 11:
		d = vmraid.db.type_map.get("Long Int")
...
def add_column(doctype, column_name, fieldtype, precision=None):
	if column_name in vmraid.db.get_table_columns(doctype):
		# already exists
		return

	vmraid.db.commit()
	vmraid.db.sql("alter table `tab%s` add column %s %s" % (doctype,
