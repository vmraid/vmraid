...
class DBTable:
	...
	def validate(self):
		...
		columns = [
			vmraid._dict({"fieldname": f, "fieldtype": "Data"}) for f in vmraid.db.STANDARD_VARCHAR_COLUMNS
		]
		if self.meta.get("istable"):
			columns += [
				vmraid._dict({"fieldname": f, "fieldtype": "Data"}) for f in vmraid.db.CHILD_TABLE_COLUMNS
			]
		...
		for col in columns:
			if len(col.fieldname) >= 64:
				vmraid.throw(
					_("Fieldname is limited to 64 characters ({0})").format(vmraid.bold(col.fieldname))
				)
	...
	if special_characters:
		...
		vmraid.throw(
			_("Fieldname {0} cannot have special characters like {1}").format(
				vmraid.bold(cstr(n)), special_characters
			),
			vmraid.db.InvalidColumnName,
		)
	return n
	...
	if size:
		...
		if length:
			if coltype == "varchar":
				...
				# NOTE: this will only be applicable for mariadb as vmraid implements int
