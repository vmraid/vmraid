...
from vmraid.database.mariadb.schema import MariaDBTable
from vmraid.utils import UnicodeWithAttrs, cstr, get_datetime, get_table_name
...
class MariaDBDatabase(Database):
	...
	def get_connection(self):
		usessl = 0
		if vmraid.conf.db_ssl_ca and vmraid.conf.db_ssl_cert and vmraid.conf.db_ssl_key:
			usessl = 1
			ssl_params = {
				"ca": vmraid.conf.db_ssl_ca,
				"cert": vmraid.conf.db_ssl_cert,
				"key": vmraid.conf.db_ssl_key,
			}
		...
		conn = pymysql.connect(
			...
			conv=conversions,
			local_infile=vmraid.conf.local_infile,
		)
	...
	def escape(s, percent=True):
		...
		s = vmraid.as_unicode(escape_string(vmraid.as_unicode(s)), "utf-8").replace("`", "\\`")
