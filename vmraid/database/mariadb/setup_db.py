...
import vmraid
import os
from vmraid.database.db_manager import DbManager
...
def get_mariadb_versions():
	...
	mariadb_variables = vmraid._dict(vmraid.db.sql("""show variables"""))
...
def setup_database(force, source_sql, verbose, no_mariadb_socket=False):
	vmraid.local.session = vmraid._dict({'user':'Administrator'})

	db_name = vmraid.local.conf.db_name
	...
	dbman.create_user(db_name, vmraid.conf.db_password)
	if no_mariadb_socket:
		dbman.create_user(db_name, vmraid.conf.db_password, host="%")
...
def setup_help_database(help_db_name):
	dbman = DbManager(get_root_connection(vmraid.flags.root_login, vmraid.flags.root_password))
...
def drop_user_and_database(db_name, root_login, root_password):
	vmraid.local.db = get_root_connection(root_login, root_password)
	dbman = DbManager(vmraid.local.db)
	dbman.delete_user(db_name, host="%")
	dbman.delete_user(db_name)
	dbman.drop_database(db_name)
...
def import_db_from_sql(source_sql=None, verbose=False):
	...
	db_name = vmraid.conf.db_name
	if not source_sql:
		source_sql = os.path.join(os.path.dirname(__file__), 'framework_mariadb.sql')
	DbManager(vmraid.local.db).restore_database(db_name, source_sql, db_name, vmraid.conf.db_password)
...
def check_database_settings():
	...
	if not result:
		...
		msg = ("Creation of your site - {x} failed because MariaDB is not properly {sep}"
			   ...
			   "then run `chair new-site {x}` again.{sep2}"
			   "").format(x=site, sep2="\n"*2, sep="\n")
		print_db_config(msg)
	return result
...
def get_root_connection(root_login, root_password):
	...
	if not vmraid.local.flags.root_connection:
		...
		vmraid.local.flags.root_connection = vmraid.database.get_db(user=root_login, password=root_password)
