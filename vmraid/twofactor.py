...
from vmraid.utils import cint, get_datetime, get_url, time_diff_in_seconds
...
def two_factor_is_enabled(user=None):
	...
	if enabled:
		bypass_two_factor_auth = int(
			vmraid.db.get_value("System Settings", None, "bypass_2fa_for_retricted_ip_users") or 0
		)
...
def two_factor_is_enabled_for_(user):
	...
	role_doctype = vmraid.qb.DocType("Role")
	no_of_users = vmraid.db.count(
		role_doctype,
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _("Login Verification Code from {}").format(
		vmraid.db.get_value("System Settings", "System Settings", "otp_issuer_name")
	)
...
def get_link_for_qrcode(user, totp_uri):
	...
	lifespan = (
		int(vmraid.db.get_value("System Settings", "System Settings", "lifespan_qrcode_image")) or 240
	)
...
def should_remove_barcode_image(barcode):
	...
	lifespan = (
		vmraid.db.get_value("System Settings", "System Settings", "lifespan_qrcode_image") or 240
	)
...
def reset_otp_secret(user):
	...
	if vmraid.session.user in ["Administrator", user]:
		...
		email_args = {
			"recipients": user_email,
			"sender": None,
			"subject": _("OTP Secret Reset - {0}").format(otp_issuer or "VMRaid Framework"),
			"message": _(
			...
			).format(otp_issuer or "VMRaid Framework"),
			"delayed": False,
			"retry": 3,
		}
		enqueue(
