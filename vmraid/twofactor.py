...
from vmraid.utils import get_url, get_datetime, time_diff_in_seconds, cint
...
def toggle_two_factor_auth(state, roles=[]):
	...
	for role in roles:
		role = vmraid.get_doc('Role', {'role_name': role})
		role.two_factor_auth = cint(state)
		role.save(ignore_permissions=True)

def two_factor_is_enabled(user=None):
	'''Returns True if 2FA is enabled.'''
	enabled = int(vmraid.db.get_value('System Settings', None, 'enable_two_factor_auth') or 0)
	if enabled:
		bypass_two_factor_auth = int(vmraid.db.get_value('System Settings', None, 'bypass_2fa_for_retricted_ip_users') or 0)
		if bypass_two_factor_auth and user:
			...
			if restrict_ip_list and vmraid.local.request_ip:
				for ip in restrict_ip_list:
					if vmraid.local.request_ip.startswith(ip):
						enabled = False
						break
...
def get_cached_user_pass():
	...
	user = pwd = None
	tmp_id = vmraid.form_dict.get('tmp_id')
	if tmp_id:
		user = vmraid.safe_decode(vmraid.cache().get(tmp_id+'_usr'))
		pwd = vmraid.safe_decode(vmraid.cache().get(tmp_id+'_pwd'))
	return (user, pwd)

def authenticate_for_2factor(user):
	'''Authenticate two factor for enabled user before login.'''
	if vmraid.form_dict.get('otp'):
		return
	otp_secret = get_otpsecret_for_(user)
	token = int(pyotp.TOTP(otp_secret).now())
	tmp_id = vmraid.generate_hash(length=8)
	cache_2fa_data(user, token, otp_secret, tmp_id)
	verification_obj = get_verification_obj(user, token, otp_secret)
	# Save data in local
	vmraid.local.response['verification'] = verification_obj
	vmraid.local.response['tmp_id'] = tmp_id

def cache_2fa_data(user, token, otp_secret, tmp_id):
	'''Cache and set expiry for data.'''
	pwd = vmraid.form_dict.get('pwd')
	...
	if verification_method in ['SMS', 'Email']:
		expiry_time = vmraid.flags.token_expiry or 300
		vmraid.cache().set(tmp_id + '_token', token)
		vmraid.cache().expire(tmp_id + '_token', expiry_time)
	else:
		expiry_time = vmraid.flags.otp_expiry or 180
	for k, v in iteritems({'_usr': user, '_pwd': pwd, '_otp_secret': otp_secret}):
		vmraid.cache().set("{0}{1}".format(tmp_id, k), v)
		vmraid.cache().expire("{0}{1}".format(tmp_id, k), expiry_time)
...
def two_factor_is_enabled_for_(user):
	...
	roles = [vmraid.db.escape(d.role) for d in user.roles or []]
	roles.append("'All'")
...
def get_otpsecret_for_(user):
	...
	otp_secret = vmraid.db.get_default(user + '_otpsecret')
	if not otp_secret:
		otp_secret = b32encode(os.urandom(10)).decode('utf-8')
		vmraid.db.set_default(user + '_otpsecret', otp_secret)
		vmraid.db.commit()
	return otp_secret

def get_verification_method():
	return vmraid.db.get_value('System Settings', None, 'two_factor_method')

def confirm_otp_token(login_manager, otp=None, tmp_id=None):
	'''Confirm otp matches.'''
	from vmraid.auth import get_login_attempt_tracker
	if not otp:
		otp = vmraid.form_dict.get('otp')
	if not otp:
	...
	if not tmp_id:
		tmp_id = vmraid.form_dict.get('tmp_id')
	hotp_token = vmraid.cache().get(tmp_id + '_token')
	otp_secret = vmraid.cache().get(tmp_id + '_otp_secret')
	if not otp_secret:
	...
	if totp.verify(otp):
		...
		if not vmraid.db.get_default(login_manager.user + '_otplogin'):
			vmraid.db.set_default(login_manager.user + '_otplogin', 1)
...
def get_verification_obj(user, token, otp_secret):
	...
	if verification_method == 'SMS':
		...
		if not vmraid.db.get_default(user + '_otplogin'):
...
def process_2fa_for_sms(user, token, otp_secret):
	...
	phone = vmraid.db.get_value('User', user, ['phone', 'mobile_no'], as_dict=1)
...
def process_2fa_for_otp_app(user, otp_secret, otp_issuer):
	...
	if vmraid.db.get_default(user + '_otplogin'):
...
def process_2fa_for_email(user, token, otp_secret, otp_issuer, method='Email'):
	...
	status = True
	prompt = ''
	if method == 'OTP App' and not vmraid.db.get_default(user + '_otplogin'):
...
def get_email_subject_for_2fa(kwargs_dict):
	...
	subject_template = _('Login Verification Code from {}').format(vmraid.db.get_value('System Settings', 'System Settings', 'otp_issuer_name'))
	subject = vmraid.render_template(subject_template, kwargs_dict)
	return subject
...
def get_email_body_for_2fa(kwargs_dict):
	...
	"""
	body = vmraid.render_template(body_template, kwargs_dict)
	return body

def get_email_subject_for_qr_code(kwargs_dict):
	'''Get QRCode email subject.'''
	subject_template = _('One Time Password (OTP) Registration Code from {}').format(vmraid.db.get_value('System Settings', 'System Settings', 'otp_issuer_name'))
	subject = vmraid.render_template(subject_template, kwargs_dict)
	return subject
...
def get_link_for_qrcode(user, totp_uri):
	...
	lifespan = int(vmraid.db.get_value('System Settings', 'System Settings', 'lifespan_qrcode_image')) or 240
	vmraid.cache().set_value(key_uri, totp_uri, expires_in_sec=lifespan)
	vmraid.cache().set_value(key_user, user, expires_in_sec=lifespan)
...
def send_token_via_email(user, token, otp_secret, otp_issuer, subject=None, message=None):
	...
	enqueue(method=vmraid.sendmail, queue='short', timeout=300, event=None,
...
def qrcode_as_png(user, totp_uri):
	...
	png_file_name = '{}.png'.format(vmraid.generate_hash(length=20))
	...
	file_path = os.path.join(vmraid.get_site_path('public', 'files'), _file.file_name)
	url = qrcreate(totp_uri)
...
def create_barcode_folder():
	...
	folder = vmraid.db.exists('File', {'file_name': folder_name})
	if folder:
		return folder
	folder = vmraid.get_doc({
			'doctype': 'File',
...
def delete_qrimage(user, check_expiry=False):
	...
	user_barcodes = vmraid.get_all('File', {'attached_to_doctype': 'User',
	...
	for barcode in user_barcodes:
		if check_expiry and not should_remove_barcode_image(barcode):
			continue
		barcode = vmraid.get_doc('File', barcode.name)
		vmraid.delete_doc('File', barcode.name, ignore_permissions=True)

def delete_all_barcodes_for_users():
	'''Task to delete all barcodes for user.'''

	users = vmraid.get_all('User', {'enabled':1})
	for user in users:
...
def should_remove_barcode_image(barcode):
	...
	if isinstance(barcode, string_types):
		barcode = vmraid.get_doc('File', barcode)
	lifespan = vmraid.db.get_value('System Settings', 'System Settings', 'lifespan_qrcode_image') or 240
	if time_diff_in_seconds(get_datetime(), barcode.creation) > int(lifespan):
		return True
	return False

def disable():
	vmraid.db.set_value('System Settings', None, 'enable_two_factor_auth', 0)
