...
import vmraid, os, re, io, codecs, json
from vmraid.model.utils import render_include, InvalidIncludePath
from vmraid.utils import strip, strip_html_tags, is_html
import itertools, operator

def guess_language(lang_list=None):
	"""Set `vmraid.local.lang` from HTTP headers at beginning of request"""
	user_preferred_language = vmraid.request.cookies.get('preferred_language')
	is_guest_user = not vmraid.session.user or vmraid.session.user == 'Guest'
	if is_guest_user and user_preferred_language:
		return user_preferred_language

	lang_codes = vmraid.request.accept_languages.values()
	if not lang_codes:
	...
	return guess or vmraid.local.lang
...
def get_user_lang(user=None):
	...
	if not lang:
		...
		lang = vmraid.db.get_value("User", user, "language")
		if not lang:
			lang = vmraid.db.get_default("lang")

		if not lang:
			lang = vmraid.local.lang or 'en'
...
def get_lang_code(lang):
	return vmraid.db.get_value('Language', {'language_name': lang}) or lang
...
def get_lang_dict():
	...
	return dict(vmraid.db.sql('select language_name, name from tabLanguage'))
...
def get_dict(fortype, name=None):
	...
	if not asset_key in translation_assets:
		...
		if fortype=="doctype":
			...
			messages += get_messages_from_include_files()
			messages += vmraid.db.sql("select 'Print Format:', name from `tabPrint Format`")
			messages += vmraid.db.sql("select 'DocType:', name from tabDocType")
			messages += vmraid.db.sql("select 'Role:', name from tabRole")
			messages += vmraid.db.sql("select 'Module:', name from `tabModule Def`")
			messages += vmraid.db.sql("select '', format from `tabWorkspace Shortcut` where format is not null")
			messages += vmraid.db.sql("select '', title from `tabOnboarding Step`")
...
def get_user_translations(lang):
	...
	if out is None:
		out = {}
		user_translations = vmraid.get_all('Translation',
...
def get_messages_for_app(app, deduplicate=True):
	...
	modules = ", ".join(['"{}"'.format(m.title().replace("_", " ")) \
		for m in vmraid.local.app_modules[app]])

	# doctypes
	if modules:
		for name in vmraid.db.sql_list("""select name from tabDocType
			where module in ({})""".format(modules)):
			messages.extend(get_messages_from_doctype(name))

		# pages
		for name, title in vmraid.db.sql("""select name, title from tabPage
			where module in ({})""".format(modules)):
		...
		# reports
		for name in vmraid.db.sql_list("""select tabReport.name from tabDocType, tabReport
...
def get_messages_from_custom_fields(app_name):
	...
	for fixture in fixtures:
		if isinstance(fixture, string_types) and fixture == 'Custom Field':
			custom_fields = vmraid.get_all('Custom Field', fields=['name','label', 'description', 'fieldtype', 'options'])
			break
		elif isinstance(fixture, dict) and fixture.get('dt', fixture.get('doctype')) == 'Custom Field':
			custom_fields.extend(vmraid.get_all('Custom Field', filters=fixture.get('filters'),
...
def get_messages_from_report(name):
	...
	messages = _get_messages_from_page_or_report("Report", name,
		vmraid.db.get_value("DocType", report.ref_doctype, "module"))
...
def get_messages_from_include_files(app_name=None):
	...
	for js_path in include_js:
		relative_path = os.path.join(vmraid.local.sites_path, js_path.lstrip('/'))
...
def update_translations(lang, untranslated_file, translated_file):
	...
	translation_dict = {}
	for key, value in zip(vmraid.get_file_items(untranslated_file, ignore_empty_lines=False),
...
def get_chair_dir():
	return os.path.join(vmraid.__file__, '..', '..', '..', '..')
...
def rename_language(old_name, new_name):
	...
	vmraid.db.sql("""update `tabUser` set language=%(new_name)s where language=%(old_name)s""",
...
def update_translations_for_source(source=None, translation_dict=None):
	...
	translation_records = vmraid.db.get_values('Translation', {
		'source_text': source
...
def get_translations(source_text):
	...
	return vmraid.db.get_list('Translation',
...
def get_contributions(language):
	return vmraid.get_all('Translation', fields=['*'], filters={
		'contributed': 1,
	})
...
def get_all_languages(with_language_name=False):
	...
	def get_language_codes():
		return vmraid.db.sql_list('select name from tabLanguage')
