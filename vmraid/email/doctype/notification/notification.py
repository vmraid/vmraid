...
from vmraid.utils import add_to_date, is_html, nowdate, parse_val, validate_email_address
...
"""
	...
	def validate_standard(self):
		if self.is_standard and self.enabled and not vmraid.conf.developer_mode:
			vmraid.throw(
	...
	def validate_forbidden_types(self):
		...
		if self.document_type in forbidden_document_types or vmraid.get_meta(self.document_type).istable:
	...
	def send_an_email(self, doc, context):
		...
		from vmraid.core.doctype.communication.email import _make as make_communication
	...
	def get_template(self):
		...
		def load_template(extn):
			template = ""
			template_path = os.path.join(os.path.dirname(module.__file__), vmraid.scrub(self.name) + extn)
...
def get_context(doc):
	return {
		"doc": doc,
		"nowdate": nowdate,
		"vmraid": vmraid._dict(utils=get_safe_globals().get("vmraid").get("utils")),
	}


def get_assignees(doc):
	assignees = []
	assignees = vmraid.get_all(
		"ToDo",
