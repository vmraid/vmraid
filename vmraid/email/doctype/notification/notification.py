...
from vmraid.core.doctype.role.role import get_info_based_on_role, get_user_info
from vmraid.utils import validate_email_address, nowdate, parse_val, is_html, add_to_date
...
from vmraid.integrations.doctype.slack_webhook_url.slack_webhook_url import send_slack_message
...
class Notification(Document):
	...
	def validate(self):
		...
		if self.event in ("Days Before", "Days After") and not self.date_changed:
			vmraid.throw(_("Please specify which date field must be checked"))

		if self.event=="Value Change" and not self.value_changed:
			vmraid.throw(_("Please specify which value field must be checked"))

		self.validate_forbidden_types()
		self.validate_condition()
		self.validate_standard()
		vmraid.cache().hdel('notifications', self.document_type)
...
""")
	...
	def validate_standard(self):
		if self.is_standard and not vmraid.conf.developer_mode:
			vmraid.throw(_('Cannot edit Standard Notification. To edit, please disable this and duplicate it'))

	def validate_condition(self):
		temp_doc = vmraid.new_doc(self.document_type)
		if self.condition:
			try:
				vmraid.safe_eval(self.condition, None, get_context(temp_doc.as_dict()))
			except Exception:
				vmraid.throw(_("The Condition '{0}' is invalid").format(self.condition))

	def validate_forbidden_types(self):
		forbidden_document_types = ("Email Queue",)
		if (self.document_type in forbidden_document_types
			or vmraid.get_meta(self.document_type).istable):
			# currently notifications don't work on child tables as events are not fired for each record of child table

			vmraid.throw(_("Cannot set Notification on Document Type {0}").format(self.document_type))
	...
	def get_documents_for_today(self):
		...
		doc_list = vmraid.get_all(self.document_type,
			fields='name',
			filters=[
		...
		for d in doc_list:
			doc = vmraid.get_doc(self.document_type, d.name)

			if self.condition and not vmraid.safe_eval(self.condition, None, get_context(doc)):
				continue
	...
	def send(self, doc):
		...
		try:
			...
			vmraid.log_error(title='Failed to send notification', message=vmraid.get_traceback())
		...
		if self.set_property_after_alert:
			...
			try:
				if allow_update and not doc.flags.in_notification_update:
					...
					if doc.meta.get_field(fieldname).fieldtype in vmraid.model.numeric_fieldtypes:
						value = vmraid.utils.cint(value)
				...
				vmraid.log_error(title='Document update failed', message=vmraid.get_traceback())

	def create_system_notification(self, doc, context):
		subject = self.subject
		if "{" in subject:
			subject = vmraid.render_template(self.subject, context)
		...
		notification_doc = {
			...
			'email_content': vmraid.render_template(self.message, context),
	...
	def send_an_email(self, doc, context):
		...
		from vmraid.core.doctype.communication.email import make as make_communication
		subject = self.subject
		if "{" in subject:
			subject = vmraid.render_template(self.subject, context)
		...
		sender = None
		message = vmraid.render_template(self.message, context)
		if self.sender and self.sender_email:
			sender = formataddr((self.sender, self.sender_email))
		vmraid.sendmail(recipients = recipients,
			subject = subject,
			sender = sender,
			cc = cc,
			bcc = bcc,
	...
	def send_sms(self, doc, context):
		send_sms(
			...
			msg=vmraid.render_template(self.message, context)
		)
	...
	def get_list_of_recipients(self, doc, context):
		...
		for recipient in self.recipients:
			if recipient.condition:
				if not vmraid.safe_eval(recipient.condition, None, context):
					continue
			...
			if recipient.cc and "{" in recipient.cc:
				recipient.cc = vmraid.render_template(recipient.cc, context)
			...
			if recipient.bcc and "{" in recipient.bcc:
				recipient.bcc = vmraid.render_template(recipient.bcc, context)
	...
	def get_attachment(self, doc):
		...
		print_settings = vmraid.get_doc("Print Settings", "Print Settings")
		if (doc.docstatus == 0 and not print_settings.allow_print_for_draft) or \
			...
			vmraid.throw(_("""Not allowed to attach {0} document, please enable Allow Print For {0} in Print Settings""").format(status),
			...
			return [{
				...
				"lang": vmraid.db.get_value('Print Format', self.print_format, 'default_print_language')
	...
	def load_standard_properties(self, context):
		...
		if not is_html(self.message):
			self.message = vmraid.utils.md_to_html(self.message)

@vmraid.whitelist()
def get_documents_for_today(notification):
	notification = vmraid.get_doc('Notification', notification)
...
def trigger_notifications(doc, method=None):
	if vmraid.flags.in_import or vmraid.flags.in_patch:
		# don't send notifications while syncing or patching
		return

	if method == "daily":
		doc_list = vmraid.get_all('Notification',
			filters={
				'event': ('in', ('Days Before', 'Days After')),
				'enabled': 1
			})
		for d in doc_list:
			alert = vmraid.get_doc("Notification", d.name)
...
def evaluate_alert(doc, alert, event):
	...
	try:
		if isinstance(alert, string_types):
			alert = vmraid.get_doc("Notification", alert)

		context = get_context(doc)

		if alert.condition:
			if not vmraid.safe_eval(alert.condition, None, context):
				return

		if event=="Value Change" and not doc.is_new():
			if not vmraid.db.has_column(doc.doctype, alert.value_changed):
				alert.db_set('enabled', 0)
				vmraid.log_error('Notification {0} has been disabled due to missing field'.format(alert.name))
				return
		...
		vmraid.throw(_("Error while evaluating Notification {0}. Please fix your template.").format(alert))
	except Exception as e:
		error_log = vmraid.log_error(message=vmraid.get_traceback(), title=str(e))
		vmraid.throw(_("Error in Notification: {}").format(
			vmraid.utils.get_link_to_form('Error Log', error_log.name)))

def get_context(doc):
	return {"doc": doc, "nowdate": nowdate, "vmraid": vmraid._dict(utils=get_safe_globals().get("vmraid").get("utils"))}

def get_assignees(doc):
	assignees = []
	assignees = vmraid.get_all('ToDo', filters={'status': 'Open', 'reference_name': doc.name,
