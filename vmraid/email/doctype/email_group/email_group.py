...
from vmraid.utils import parse_addr, validate_email_address
...
def add_subscribers(name, email_list):
	...
	for email in email_list:
		...
		if parsed_email:
			if not vmraid.db.get_value("Email Group Member", {"email_group": name, "email": parsed_email}):
				vmraid.get_doc(
...
def send_welcome_email(welcome_email, email, email_group):
	...
	message = vmraid.render_template(email_message, args)
