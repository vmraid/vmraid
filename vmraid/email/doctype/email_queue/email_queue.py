...
import vmraid
from vmraid import _, safe_encode, task
from vmraid.email.doctype.email_account.email_account import EmailAccount
from vmraid.email.email_body import add_attachment, get_email, get_formatted_html
from vmraid.email.queue import get_unsubcribed_url, get_unsubscribe_message
from vmraid.model.document import Document
from vmraid.query_builder.utils import DocType
from vmraid.utils import add_days, cint, cstr, get_hook_method, nowdate, split_emails
...
class EmailQueue(Document):
	...
	def prevent_email_queue_delete(self):
		if vmraid.session.user != "Administrator":
			vmraid.throw(_("Only Administrator can delete Email Queue"))

	def get_duplicate(self, recipients):
		values = self.as_dict()
		del values["name"]
		duplicate = vmraid.get_doc(values)
	...
	def new(cls, doc_data, ignore_permissions=False):
		...
		doc = vmraid.new_doc(cls.DOCTYPE)
		doc.update(data)
	...
	def update_status(self, status, commit=False, **kwargs):
		...
		if self.communication:
			communication_doc = vmraid.get_doc("Communication", self.communication)
	...
	def get_email_account(self):
		if self.email_account:
			return vmraid.get_doc("Email Account", self.email_account)
	...
	def can_send_now(self):
		hold_queue = cint(vmraid.defaults.get_defaults().get("hold_queue")) == 1
		if vmraid.are_emails_muted() or not self.is_to_be_sent() or hold_queue:
			return False
	...
	def send(self, is_background_task=False):
		...
		with SendMailContext(self, is_background_task) as ctx:
			...
			if vmraid.flags.in_test:
				vmraid.flags.sent_mail = message
				return
...
class SendMailContext:
	...
	def log_exception(self, exc_type, exc_val, exc_tb):
		if exc_type:
			...
			if self.is_background_task:
				vmraid.log_error(title="vmraid.email.queue.flush", message=traceback_string)
			else:
				vmraid.log_error(message=traceback_string)
	...
	def get_tracker_str(self):
		tracker_url_html = '<img src="https://{}/api/method/vmraid.core.doctype.communication.email.mark_email_as_seen?name={}"/>'

		message = ""
		if vmraid.conf.use_ssl and self.email_account_doc.track_email_status:
			message = quopri.encodestring(
				tracker_url_html.format(vmraid.local.site, self.queue_doc.communication).encode()
			).decode()
		return message
	...
	def include_attachments(self, message):
		...
		for attachment in attachments:
			...
			if file_filters:
				_file = vmraid.get_doc("File", file_filters)
				...
				print_format_file = vmraid.attach_print(**attachment)
...
def retry_sending(name):
	doc = vmraid.get_doc("Email Queue", name)
...
class QueueBuilder:
	...
	def unsubscribe_method(self):
		return self._unsubscribe_method or "/api/method/vmraid.email.queue.unsubscribe"
	...
	def get_unsubscribed_user_emails(self):
		...
		if len(all_ids) > 0:
			unsubscribed = (
				vmraid.qb.from_(EmailUnsubscribe)
