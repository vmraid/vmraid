...
import vmraid
from vmraid.desk.form.load import run_onload
from vmraid.email.doctype.newsletter.exceptions import (
...
class TestNewsletterMixin:
	...
	def tearDown(self):
		...
		for newsletter in newsletters:
			...
			vmraid.delete_doc("Newsletter", newsletter)
			vmraid.db.delete("Newsletter Email Group", {"parent": newsletter})
	...
	def setup_email_group(self):
		...
		for email in emails:
			...
			try:
				...
				vmraid.db.update(doctype, email_filters, "unsubscribed", 0)
	...
	def get_newsletter(**kwargs) -> "Newsletter":
		...
		similar_newsletters = vmraid.db.get_all(doctype, newsletter_content, pluck="name")

		for similar_newsletter in similar_newsletters:
			vmraid.delete_doc(doctype, similar_newsletter)

		newsletter = vmraid.get_doc({"doctype": doctype, **newsletter_content, **kwargs})
...
class TestNewsletter(TestNewsletterMixin, unittest.TestCase):
	...
	def test_send_scheduled_email_error_handling(self):
		...
		job_path = "vmraid.email.doctype.newsletter.newsletter.Newsletter.queue_all"
		m = MagicMock(side_effect=vmraid.OutgoingEmailError)

		with self.assertRaises(vmraid.OutgoingEmailError):
