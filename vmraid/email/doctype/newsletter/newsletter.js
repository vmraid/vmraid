...
vmraid.ui.form.on('Newsletter', {
	refresh(frm) {
		let doc = frm.doc;
		let can_write = in_list(vmraid.boot.user.can_write, doc.doctype);
		if (!frm.is_new() && !frm.is_dirty() && !doc.email_sent && can_write) {
			...
			frm.add_custom_button(__('Send now'), () => {
				if (frm.doc.schedule_send) {
					vmraid.confirm(__("This newsletter was scheduled to send on a later date. Are you sure you want to send it now?"), function () {
						frm.call('send_emails').then(() => frm.refresh());
					});
					return;
				}
				vmraid.confirm(__("Are you sure you want to send this newsletter now?"), function () {
					frm.call('send_emails').then(() => frm.refresh());
				});
			}, __('Send'));
		...
		if (frm.is_new() && !doc.sender_email) {
			let { fullname, email } = vmraid.user_info(doc.owner);
	...
	schedule_send_dialog(frm) {
		let hours = vmraid.utils.range(24);
	...
	update_schedule_message(frm) {
		if (!frm.doc.email_sent && frm.doc.schedule_send) {
			let datetime = vmraid.datetime.global_date_format(frm.doc.schedule_send);
