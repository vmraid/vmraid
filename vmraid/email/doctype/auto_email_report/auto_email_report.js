...
vmraid.ui.form.on('Auto Email Report', {
	refresh: function(frm) {
		...
		if(!frm.is_new()) {
			frm.add_custom_button(__('Download'), function() {
				var w = window.open(
					vmraid.urllib.get_full_url(
						"/api/method/vmraid.email.doctype.auto_email_report.auto_email_report.download?"
			...
			frm.add_custom_button(__('Send Now'), function() {
				vmraid.call({
					method: 'vmraid.email.doctype.auto_email_report.auto_email_report.send_now',
					args: {name: frm.doc.name},
					callback: function() {
						vmraid.msgprint(__('Scheduled to send'));
					}
				});
			});
		} else {
			...
			if(!frm.doc.email_to) {
				frm.set_value('email_to', vmraid.session.user);
			}
		}
	},
	report: function(frm) {
	...
	fetch_report_filters(frm) {
		if (frm.doc.report
			...
			vmraid.call({
				method: "vmraid.desk.query_report.get_script",
				args: {
					report_name: frm.doc.report
				},
				callback: function(r) {
					vmraid.dom.eval(r.message.script || "");
					frm.script_setup_for = frm.doc.report;
	...
	show_filters: function(frm) {
		...
		if(frm.doc.report_type === 'Custom Report' || (frm.doc.report_type !== 'Report Builder'
			&& vmraid.query_reports[frm.doc.report]
			&& vmraid.query_reports[frm.doc.report].filters)) {
			...
			if (frm.doc.report_type === 'Custom Report'
				&& vmraid.query_reports[frm.doc.reference_report]
				&& vmraid.query_reports[frm.doc.reference_report].filters) {
				report_filters = vmraid.query_reports[frm.doc.reference_report].filters;
			} else {
				report_filters = vmraid.query_reports[frm.doc.report].filters;
			}
			...
			report_filters.forEach(f => {
				...
				$("<td>" + vmraid.format(filters[f.fieldname], f) +"</td>")
					.css(css)
					.appendTo(row);
			});
