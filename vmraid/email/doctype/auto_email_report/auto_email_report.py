...
from vmraid.desk.query_report import build_xlsx_data
...
class AutoEmailReport(Document):
		...
		if self.format not in valid_report_formats:
			vmraid.throw(
				_("{0} is not a valid report format. Report format should one of the following {1}").format(
					vmraid.bold(self.format), vmraid.bold(", ".join(valid_report_formats))
				)
			)
	...
	def get_html_table(self, columns=None, data=None):
		...
		return vmraid.render_template(
			"vmraid/templates/emails/auto_email_report.html",
			{
				"title": self.name,
...
def send_daily():
	...
	enabled_reports = vmraid.get_all(
	...
	for row in data:
		...
		for col in columns:
			...
			if col.fieldtype == "Link":
				...
				doc = vmraid.get_doc(col.parent, doc_name) if doc_name and col.get("parent") else None
				# Pass the Document to get the currency based on docfield option
				row[col.fieldname] = vmraid.format_value(row[col.fieldname], col, doc=doc)
	return columns, data
