...
import vmraid
from vmraid.utils import add_to_date, get_link_to_form, today
from vmraid.utils.data import is_html

# test_records = vmraid.get_test_records('Auto Email Report')

class TestAutoEmailReport(unittest.TestCase):
	def test_auto_email(self):
		vmraid.delete_doc('Auto Email Report', 'Permitted Documents For User')
...
def get_auto_email_report():
	if not vmraid.db.exists('Auto Email Report', 'Permitted Documents For User'):
		auto_email_report = vmraid.get_doc(dict(
		...
		auto_email_report = vmraid.get_doc('Auto Email Report', 'Permitted Documents For User')
