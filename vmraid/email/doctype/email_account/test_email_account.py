...
from vmraid.email.receive import Email, InboundMail, SentEmailInInboxError
...
class TestEmailAccount(unittest.TestCase):
	...
	def setUp(self):
		...
		vmraid.db.delete("Unhandled Email")
	...
	def test_threading(self):
		...
		sent_name = make(
			...
			doctype="ToDo",
			name=vmraid.get_last_doc("ToDo").name,
			send_email=True,
		)["name"]
	...
	def test_handle_bad_emails(self):
		...
		self.assertTrue(vmraid.db.get_value("Unhandled Email", {"message_id": message_id}))
	...
	def test_append_to(self):
		...
		self.assertTrue(vmraid.db.exists(communication.reference_doctype, communication.reference_name))
	...
	def test_append_to_with_imap_folders(self):
		...
		for mail in mails:
			...
			if mail.append_to == "ToDo":
				...
				self.assertTrue(communication.reference_name)
				self.assertTrue(
					vmraid.db.exists(communication.reference_doctype, communication.reference_name)
				)
			else:
				test_folder_mails += 1
	...
	@patch("vmraid.email.receive.EmailServer.select_imap_folder", return_value=True)
	@patch("vmraid.email.receive.EmailServer.logout", side_effect=lambda: None)
	def mocked_get_inbound_mails(
		email_account, messages={}, mocked_logout=None, mocked_select_imap_folder=None
	):
		from vmraid.email.receive import EmailServer
...
class TestInboundMail(unittest.TestCase):
	...
	def test_find_parent_communication_from_header(self):
		...
		communication = self.new_communication()
		mail_content = self.get_test_mail(fname="reply-4.raw").replace(
			"{{ message_id }}", f"<{communication.name}@{vmraid.local.site}>"
		)
