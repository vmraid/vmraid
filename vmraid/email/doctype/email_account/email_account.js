...
vmraid.ui.form.on("Email Account", {
	service: function(frm) {
		$.each(vmraid.email_defaults[frm.doc.service], function(key, value) {
			frm.set_value(key, value);
		});
		if (!frm.doc.use_imap) {
			$.each(vmraid.email_defaults_pop[frm.doc.service], function(key, value) {
				frm.set_value(key, value);
			});
		}
	...
	onload: function(frm) {
		...
		frm.set_query("append_to", "vmraid.email.doctype.email_account.email_account.get_append_to");
	},
	...
	refresh: function(frm) {
		...
		if(vmraid.route_flags.delete_user_from_locals && vmraid.route_flags.linked_user) {
			delete vmraid.route_flags.delete_user_from_locals;
			delete locals['User'][vmraid.route_flags.linked_user];
		}
	},
		...
		vmraid.call({
			...
			callback: function (r) {
				if (r.message) {
					...
					vmraid.confirm(__('Email Domain not configured for this account, Create one?'),
						function () {
							vmraid.model.with_doctype("Email Domain", function() {
								vmraid.route_options = { email_id: frm.doc.email_id };
								vmraid.route_flags.return_to_email_account = 1;
								var doc = vmraid.model.get_new_doc("Email Domain");
								vmraid.set_route("Form", "Email Domain", doc.name);
							});
						}
					);
				}
	...
	set_domain_fields: function(frm, args) {
		if(!args){
			args = vmraid.route_flags.set_domain_values? vmraid.route_options: {};
		}
		...
		delete vmraid.route_flags.set_domain_values;
	...
	email_sync_option: function(frm) {
		...
		if(frm.doc.email_sync_option == "ALL"){
			...
			vmraid.confirm(msg, null, function() {
	...
	warn_autoreply_on_incoming: function(frm) {
		if (frm.doc.enable_incoming && frm.doc.enable_auto_reply && frm.doc.__islocal) {
			...
			vmraid.confirm(msg, null, function() {
				...
				vmraid.show_alert({message: __("Disabled Auto Reply"), indicator: "blue"});
			});
		}
	}
});
