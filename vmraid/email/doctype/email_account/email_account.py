...
import vmraid
from vmraid import _, are_emails_muted, safe_encode
from vmraid.desk.form import assign_to
from vmraid.email.receive import EmailServer, InboundMail, SentEmailInInboxError
from vmraid.email.smtp import SMTPServer
from vmraid.email.utils import get_port
from vmraid.model.document import Document
from vmraid.utils import cint, comma_or, cstr, parse_addr, validate_email_address
...
class EmailAccount(Document):
	...
	def validate(self):
		...
		if self.enable_incoming and self.use_imap and len(self.imap_folder) <= 0:
			vmraid.throw(_("You need to set one IMAP folder for {0}").format(vmraid.bold(self.email_id)))

		duplicate_email_account = vmraid.get_all(
		...
		if (
			...
			and not vmraid.local.flags.in_install
	...
	def before_save(self):
		...
		if not self.enable_incoming and self.default_incoming:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					vmraid.bold(_("Default Incoming")), vmraid.bold(_("Enable Incoming"))
				)
			)
		if not self.enable_outgoing and self.default_outgoing:
			...
			messages.append(
				_("{} has been disabled. It can only be enabled if {} is checked.").format(
					vmraid.bold(_("Default Outgoing")), vmraid.bold(_("Enable Outgoing"))
				)
			)
		if messages:
			if len(messages) == 1:
	...
	@raise_error_on_no_output(
		...
		error_type=vmraid.OutgoingEmailError,
	)  # noqa
	...
	def receive(self):
		...
		for mail in inbound_mails:
			try:
				...
				vmraid.db.rollback()
				vmraid.log_error(title="EmailAccount.receive")
				if self.use_imap:
					self.handle_bad_emails(mail.uid, mail.raw_message, vmraid.get_traceback())
				exceptions.append(vmraid.get_traceback())
			else:
				vmraid.db.commit()

		# notify if user is linked to account
		if len(inbound_mails) > 0 and not vmraid.local.flags.in_test:
	...
	def send_auto_reply(self, communication, email):
		...
		if self.enable_auto_reply:
			...
			vmraid.sendmail(
				...
				or vmraid.get_template("templates/emails/auto_reply.html").render(communication.as_dict()),
	...
	def on_trash(self):
		...
		vmraid.qb.update(Communication).set(Communication.email_account, "").where(
	...
	def mark_emails_as_read_unread(self, email_server=None, folder_name="INBOX"):
		...
		EmailFlagQ = vmraid.qb.DocType("Email Flag Queue")
		flags = (
			vmraid.qb.from_(EmailFlagQ)
			.select(EmailFlagQ.name, EmailFlagQ.communication, EmailFlagQ.uid, EmailFlagQ.action)
			.where(EmailFlagQ.is_completed == 0)
			.where(EmailFlagQ.email_account == vmraid.db.escape(self.name))
		).run(as_dict=True)
		...
		if flags and uid_list:
			...
			EmailFlagQueue = vmraid.qb.DocType("Email Flag Queue")
			vmraid.qb.update(EmailFlagQueue).set(EmailFlagQueue.is_completed, 1).where(
	...
	def set_communication_seen_status(self, docnames, seen=0):
		...
		if not docnames:
			return
		Communication = vmraid.qb.from_("Communication")
		vmraid.qb.update(Communication).set(Communication.seen == seen).where(
	...
	def append_email_to_sent_folder(self, message):
		...
		if email_server.imap:
			try:
				...
				vmraid.log_error(title="EmailAccount.append_email_to_sent_folder")
...
def notify_unreplied():
	...
	for email_account in vmraid.get_all(
...
def pull(now=False):
	...
	for email_account in vmraid.get_list(
...
def setup_user_email_inbox(email_account, awaiting_password, email_id, enable_outgoing):
	...
	from vmraid.core.doctype.user.user import ask_pass_update
	...
	if update_user_email_settings:
		UserEmail = vmraid.qb.DocType("User Email")
		vmraid.qb.update(UserEmail).set(UserEmail.awaiting_password, (awaiting_password or 0)).set(
