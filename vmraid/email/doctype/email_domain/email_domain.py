...
from vmraid.utils import validate_email_address ,cint, cstr
...
class EmailDomain(Document):
	...
	def validate(self):
		...
		if not vmraid.local.flags.in_install and not vmraid.local.flags.in_patch:
			try:
				if self.use_imap:
				...
				vmraid.throw(title=_("Incoming email account not correct"),
			...
			try:
				...
				vmraid.throw(title=_("Outgoing email account not correct"),
					msg='Error connecting SMTP "{host}": {e}'.format(host=self.smtp_server, e=e))

	def on_update(self):
		"""update all email accounts using this domain"""
		for email_account in vmraid.get_all("Email Account", filters={"domain": self.name}):
			try:
				...
				vmraid.msgprint(_("Error has occurred in {0}").format(email_account.name), raise_exception=e.__class__)
