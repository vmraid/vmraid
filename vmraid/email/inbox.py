...
def create_email_flag_queue(names, action):
	...
	for name in json.loads(names or []):
		uid, seen_status, email_account = vmraid.db.get_value("Communication", name,
		...
		if (action =='Read' and seen_status == 0) or (action =='Unread' and seen_status == 1):
			create_new = True
			email_flag_queue = vmraid.db.sql("""select name, action from `tabEmail Flag Queue`
			...
			if create_new:
				...
				vmraid.db.set_value("Communication", name, "seen", seen,
