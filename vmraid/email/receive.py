...
from vmraid.core.doctype.file.file import (MaxFileSizeReachedError,
	get_random_filename)
from vmraid.utils import (cint, convert_utc_to_user_timezone, cstr,
	extract_email_id, markdown, now, parse_addr, strip)


class EmailSizeExceededError(vmraid.ValidationError): pass
class EmailTimeoutError(vmraid.ValidationError): pass
class TotalSizeExceededError(vmraid.ValidationError): pass
class LoginLimitExceeded(vmraid.ValidationError): pass
...
class EmailServer:
	...
	def connect_imap(self):
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(self.settings.host, self.settings.incoming_port, timeout=vmraid.conf.get("pop_timeout"))
			else:
				self.imap = Timed_IMAP4(self.settings.host, self.settings.incoming_port, timeout=vmraid.conf.get("pop_timeout"))
	...
	def connect_pop(self):
		...
		try:
			if cint(self.settings.use_ssl):
				self.pop = Timed_POP3_SSL(self.settings.host, self.settings.incoming_port, timeout=vmraid.conf.get("pop_timeout"))
			else:
				self.pop = Timed_POP3(self.settings.host, self.settings.incoming_port, timeout=vmraid.conf.get("pop_timeout"))
...
class Email:
	...
	def decode_email(self, email):
		if not email: return
		decoded = ""
		for part, encoding in decode_header(vmraid.as_unicode(email).replace("\""," ").replace("\'"," ")):
			if encoding:
