...
import vmraid
from vmraid import _, safe_decode, safe_encode
from vmraid.core.doctype.file.file import MaxFileSizeReachedError, get_random_filename
...
class EmailSizeExceededError(vmraid.ValidationError):
	pass


class EmailTimeoutError(vmraid.ValidationError):
	pass


class TotalSizeExceededError(vmraid.ValidationError):
	pass


class LoginLimitExceeded(vmraid.ValidationError):
	pass
...
class EmailServer:
	...
	def setup(self, args=None):
		# overrride
		self.settings = args or vmraid._dict()
		...
		try:
			if cint(self.settings.use_ssl):
				self.imap = Timed_IMAP4_SSL(
					self.settings.host, self.settings.incoming_port, timeout=vmraid.conf.get("pop_timeout")
				)
			else:
			...
			vmraid.msgprint(_("Invalid Mail Server. Please rectify and try again."))
			raise
	...
	def connect_pop(self):
		...
		try:
			...
			vmraid.log_error("receive.connect_pop")
			...
			if self.is_temporary_system_problem(e):
				...
				vmraid.msgprint(_("Invalid User Name or Support Password. Please rectify and try again."))
				raise
	...
	def get_messages(self, folder="INBOX"):
		...
		try:
			...
			# size limits
			self.total_size = 0
			self.max_email_size = cint(vmraid.local.conf.get("max_email_size"))
	...
	def check_imap_uidvalidity(self, folder):
		...
		vmraid.db.set_value("Email Account", self.settings.email_account, "uidnext", uidnext)

		if not uid_validity or uid_validity != current_uid_validity:
			# uidvalidity changed & all email uids are reindexed by server
			Communication = vmraid.qb.DocType("Communication")
			vmraid.qb.update(Communication).set(Communication.uid, -1).where(
			...
			if self.settings.use_imap:
				...
				IMAPFolder = vmraid.qb.DocType("IMAP Folder")
				vmraid.qb.update(IMAPFolder).set(IMAPFolder.uidvalidity, current_uid_validity).set(
					IMAPFolder.uidnext, uidnext
				).where(IMAPFolder.parent == self.settings.email_account_name).where(
					IMAPFolder.folder_name == folder
				).run()
			else:
				EmailAccount = vmraid.qb.DocType("Email Account")
				vmraid.qb.update(EmailAccount).set(EmailAccount.uidvalidity, current_uid_validity).set(
	...
	def retrieve_message(self, message_meta, msg_num=None):
		...
		try:
			...
			if self.has_login_limit_exceeded(e):
				...
				# log performs rollback and logs error in Error Log
				vmraid.log_error("receive.get_messages", self.make_error_msg(msg_num, incoming_mail))
				self.errors = True
				vmraid.db.rollback()
	...
	def get_email_seen_status(self, uid, flag_string):
		...
		for flag in imaplib.ParseFlags(flag_string) or []:
			...
			match = re.search(pattern, vmraid.as_unicode(flag))
...
class Email:
	...
	def set_from(self):
		...
		if _reply_to and not vmraid.db.get_value("Email Account", {"email_id": _reply_to}, "email_id"):
	...
	def decode_email(self, email):
		...
		for part, encoding in decode_header(
			vmraid.as_unicode(email).replace('"', " ").replace("'", " ")
		):
			if encoding:
	...
	def save_attachments_in_doc(self, doc):
		...
		for attachment in self.attachments:
			try:
				...
				pass
			except vmraid.FileAlreadyAttachedException:
				pass
...
class InboundMail(Email):
	...
	def _build_communication_doc(self):
		...
		communication = vmraid.get_doc(data)
	...
	def is_exist_in_system(self):
		...
		from vmraid.core.doctype.communication.communication import Communication
	...
	def is_reply_to_system_sent_mail(self):
		...
		return self.is_reply() and vmraid.local.site in self.in_reply_to
	...
	def parent_email_queue(self):
		...
		"""
		from vmraid.email.doctype.email_queue.email_queue import EmailQueue
	...
	def match_record_by_subject_and_sender(self, doctype):
		...
		3. Find by matching subject alone (Special case)
		        ...
		        the system user is replying to via the common email account in VMRaid. This fix bypasses
		...
		if not record:
			...
			name = vmraid.db.get_value(self.email_account.append_to, filters=filters)
			record = self.get_doc(doctype, name, ignore_error=True) if name else None
		return record

	def _create_reference_document(self, doctype):
		"""Create reference document if it does not exist in the system."""
		parent = vmraid.new_doc(doctype)
		email_fileds = self.get_email_fields(doctype)

		if email_fileds.subject_field:
			parent.set(email_fileds.subject_field, vmraid.as_unicode(self.subject)[:140])

		if email_fileds.sender_field:
			parent.set(email_fileds.sender_field, vmraid.as_unicode(self.from_email))
		...
		try:
			...
			parent_name = vmraid.db.get_value(
	...
	def get_doc(doctype, docname, ignore_error=False):
		try:
			return vmraid.get_doc(doctype, docname)
	...
	def clean_subject(subject):
		...
		return vmraid.as_unicode(strip(re.sub(regex, "", subject, 0, flags=re.IGNORECASE)))
	...
	def get_document(self, doctype, name):
		"""Is same as vmraid.get_doc but suppresses the DoesNotExist error."""
		try:
			return vmraid.get_doc(doctype, name)
		except vmraid.DoesNotExistError:
			return None
