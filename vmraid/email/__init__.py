...
import vmraid
from vmraid.desk.reportview import build_match_conditions

def sendmail_to_system_managers(subject, content):
	vmraid.sendmail(recipients=get_system_managers(), subject=subject, content=content)
...
def get_contact_list(txt, page_length=20):
	...
	try:
		...
		out = vmraid.db.sql("""select email_id as value,
...
def get_system_managers():
	return vmraid.db.sql_list("""select parent FROM `tabHas Role`
...
def get_communication_doctype(doctype, txt, searchfield, start, page_len, filters):
	user_perms = vmraid.utils.user.UserPermissions(vmraid.session.user)
	...
	if len(txt)<2:
		...
		for name in vmraid.get_hooks("communication_doctypes"):
			try:
		...
		com_doctypes = [d[0] for d in vmraid.db.get_values("DocType", {"issingle": 0, "istable": 0, "hide_toolbar": 0})]
...
def get_cached_contacts(txt):
	contacts = vmraid.cache().hget("contacts", vmraid.session.user) or []
...
def update_contact_cache(contacts):
	cached_contacts = vmraid.cache().hget("contacts", vmraid.session.user) or []

	uncached_contacts = [d for d in contacts if d not in cached_contacts]
	cached_contacts.extend(uncached_contacts)

	vmraid.cache().hset("contacts", vmraid.session.user, cached_contacts)
