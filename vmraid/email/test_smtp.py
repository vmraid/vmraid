...
class TestSMTP(unittest.TestCase):
	...
	def test_get_email_account(self):
		existing_email_accounts = vmraid.get_all("Email Account", fields = ["name", "enable_outgoing", "default_outgoing", "append_to"])
		unset_details = {
			"enable_outgoing": 0,
			"default_outgoing": 0,
			"append_to": None
		...
		for email_account in existing_email_accounts:
			vmraid.db.set_value('Email Account', email_account['name'], unset_details)

		# remove mail_server config so that test@example.com is not created
		mail_server = vmraid.conf.get('mail_server')
		del vmraid.conf['mail_server']

		vmraid.local.outgoing_email_account = {}
		...
		vmraid.conf['mail_server'] = mail_server
		for email_account in existing_email_accounts:
			set_details = {
			...
			vmraid.db.set_value('Email Account', email_account['name'], set_details)
...
def create_email_account(email_id, password, enable_outgoing, default_outgoing=0, append_to=None):
	...
	email_account = vmraid.new_doc('Email Account')
