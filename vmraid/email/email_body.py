...
from vmraid.utils import (get_url, scrub_urls, strip, expand_relative_urls, cint,
...
class EMail:
	...
	def attach_file(self, n):
		...
		_file = vmraid.get_doc("File", {"file_name": n})
	...
	def make(self):
		...
		for hook in vmraid.get_hooks("make_email_body_message"):
			vmraid.get_attr(hook)(self)
...
def get_formatted_html(subject, message, footer=None, print_html=None,
	...
	rendered_email = vmraid.get_template("templates/emails/standard.html").render({
...
def get_email_html(template, args, subject, header=None, with_container=False):
	...
	email = vmraid.utils.jinja.get_email_from_template(template, args)
...
def inline_style_in_html(html):
	...
	css_files = vmraid.get_hooks('email_css')
...
def get_footer(email_account, footer=None):
	...
	sender_address = vmraid.db.get_default("email_footer_address")

	if sender_address:
		args.update({'sender_address': sender_address})

	if not cint(vmraid.db.get_default("disable_standard_email_footer")):
		args.update({'default_mail_footer': vmraid.get_hooks('default_mail_footer')})

	footer += vmraid.utils.jinja.get_email_from_template('email_footer', args)[0]

	return footer

def replace_filename_with_cid(message):
	""" Replaces <img embed="assets/vmraid/images/filename.jpg" ...> with
...
def get_filecontent_from_path(path):
	...
	if path.startswith('assets/'):
		...
		# public file
		full_path = vmraid.get_site_path('public', path)
	elif path.startswith('private/files/'):
		# private file
		full_path = vmraid.get_site_path(path)
	else:
		full_path = path
...
def get_header(header=None):
	...
	from vmraid.utils.jinja import get_email_from_template
	...
	if not title:
		title = vmraid.get_hooks('app_title')[-1]
