...
import vmraid
from vmraid.query_builder.terms import NamedParameterWrapper
...
class ImportMapper:
	...
	def __call__(self, *args: Any, **kwds: Any) -> Callable:
		db = db_type_is(vmraid.conf.db_type or "mariadb")
...
def DocType(*args, **kwargs):
	return vmraid.qb.DocType(*args, **kwargs)


def patch_query_execute():
	"""Patch the Query Builder with helper execute method
	This excludes the use of `vmraid.db.sql` method while
	executing the query object
	"""
	from vmraid.utils.safe_exec import check_safe_sql_query

	def execute_query(query, *args, **kwargs):
		query, params = prepare_query(query)
		return vmraid.db.sql(query, params, *args, **kwargs)  # nosemgrep
	...
	def prepare_query(query):
		...
		if vmraid.flags.in_safe_exec and not check_safe_sql_query(query, throw=False):
	...
	query_class = get_attr(str(vmraid.qb).split("'")[1])
...
def patch_query_aggregation():
	"""Patch aggregation functions to vmraid.qb"""
	from vmraid.query_builder.functions import _avg, _max, _min, _sum
