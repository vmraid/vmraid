...
from vmraid.event_streaming.doctype.event_producer.event_producer import pull_from_node
...
class TestEventProducer(unittest.TestCase):
	# @classmethod
	# def setUpClass(cls):
	# 	vmraid.print_sql(True)

	# @classmethod
	# def tearDownClass(cls):
	# 	vmraid.print_sql(False)
	...
	def test_insert(self):
		...
		self.assertTrue(vmraid.db.exists('ToDo', producer_doc.name))
	...
	def test_update(self):
		...
		self.pull_producer_data()
		local_doc = vmraid.get_doc(producer_doc.doctype, producer_doc.name)
	...
	def test_delete(self):
		...
		self.pull_producer_data()
		self.assertFalse(vmraid.db.exists('ToDo', producer_doc.name))
	...
	def test_multiple_doctypes_sync(self):
		...
		producer_note1 = vmraid._dict(doctype='Note', title='test multiple doc sync 1')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note1['title']})
		vmraid.db.delete('Note', {'title': producer_note1['title']})
		producer_note1 = producer.insert(producer_note1)
		producer_note2 = vmraid._dict(doctype='Note', title='test multiple doc sync 2')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note2['title']})
		vmraid.db.delete('Note', {'title': producer_note2['title']})
		...
		#check inserted
		self.assertTrue(vmraid.db.exists('ToDo', producer_todo.name))

		#check update
		local_todo = vmraid.get_doc('ToDo', producer_todo.name)
		self.assertEqual(local_todo.description, producer_todo.description)
		local_note1 = vmraid.get_doc('Note', producer_note1.name)
		self.assertEqual(local_note1.content, producer_note1.content)

		#check delete
		self.assertFalse(vmraid.db.exists('Note', producer_note2.name))

	def test_child_table_sync_with_dependencies(self):
		producer = get_remote_site()
		producer_user = vmraid._dict(doctype='User', email='test_user@sync.com', send_welcome_email=0,
		...
		vmraid.db.delete('User', {'email':producer_user.email})
		producer_user = producer.insert(producer_user)

		producer_note = vmraid._dict(doctype='Note', title='test child table dependency sync',
			seen_by=[{'user': producer_user.name}])
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note.title})
		vmraid.db.delete('Note', {'title': producer_note.title})
		producer_note = producer.insert(producer_note)

		self.pull_producer_data()
		self.assertTrue(vmraid.db.exists('User', producer_user.name))
		if self.assertTrue(vmraid.db.exists('Note', producer_note.name)):
			local_note = vmraid.get_doc('Note', producer_note.name)
	...
	def test_dynamic_link_dependencies_synced(self):
		...
		event_producer = vmraid.get_doc('Event Producer', producer_url, for_update=True)
		...
		producer_link_doc = vmraid._dict(doctype='Note', title='Test Dynamic Link 1')

		delete_on_remote_if_exists(producer, 'Note', {'title': producer_link_doc.title})
		vmraid.db.delete('Note', {'title': producer_link_doc.title})
		producer_link_doc = producer.insert(producer_link_doc)
		producer_doc = vmraid._dict(doctype='ToDo', description='Test Dynamic Link 2', assigned_by='Administrator',
		...
		self.assertTrue(vmraid.db.exists('Note', producer_link_doc.name))
		self.assertEqual(producer_link_doc.name, vmraid.db.get_value('ToDo', producer_doc.name, 'reference_name'))
	...
	def test_naming_configuration(self):
		...
		self.pull_producer_data()
		self.assertTrue(vmraid.db.exists('ToDo', {'remote_docname': producer_doc.name, 'remote_site_name': producer_url}))
	...
	def test_conditional_events(self):
		...
		# Add Condition
		event_producer = vmraid.get_doc('Event Producer', producer_url)
		note_producer_entry = [
		...
		# Make test doc
		producer_note1 = vmraid._dict(doctype='Note', public=0, title='test conditional sync')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note1['title']})
		producer_note1 = producer.insert(producer_note1)
		...
		self.assertFalse(vmraid.db.exists('Note', producer_note1.name))
		...
		# it should sync now
		self.assertTrue(vmraid.db.exists('Note', producer_note1.name))
		local_note = vmraid.get_doc('Note', producer_note1.name)
	...
	def test_conditional_events_with_cmd(self):
		...
		note_producer_entry.condition = 'cmd: vmraid.event_streaming.doctype.event_producer.test_event_producer.can_sync_note'
		event_producer.save()

		# Make test doc
		producer_note1 = vmraid._dict(doctype='Note', public=0, title='test conditional sync cmd')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note1['title']})
		producer_note1 = producer.insert(producer_note1)
	...
	def test_event_sync_log(self):
		...
		self.pull_producer_data()
		self.assertTrue(vmraid.db.exists('Event Sync Log', {'docname': producer_doc.name}))
	...
	def test_mapping(self):
		...
		producer_note = vmraid._dict(doctype='Note', title='Test Mapping', content='Test Mapping')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note.title})
		producer_note = producer.insert(producer_note)
		self.pull_producer_data()
		#check inserted
		self.assertTrue(vmraid.db.exists('ToDo', {'description': producer_note.content}))
		...
		self.pull_producer_data()
		#check delete
		self.assertFalse(vmraid.db.exists('ToDo', {'description': producer_note.content}))
	...
	def test_inner_mapping(self):
		...
		producer_note = vmraid._dict(doctype='Note', title='Inner Mapping Tester', content='Test Inner Mapping')
		delete_on_remote_if_exists(producer, 'Note', {'title': producer_note.title})
		producer_note = producer.insert(producer_note)
		self.pull_producer_data()

		#check dependency inserted
		self.assertTrue(vmraid.db.exists('Role', {'role_name': producer_note.title}))
		#check doc inserted
		self.assertTrue(vmraid.db.exists('ToDo', {'description': producer_note.content}))
...
def get_mapping(mapping_name, local, remote, field_map):
	name = vmraid.db.exists('Document Type Mapping', mapping_name)
	if name:
		doc = vmraid.get_doc('Document Type Mapping', name)
	else:
		doc = vmraid.new_doc('Document Type Mapping')
...
def create_event_producer(producer_url):
	if vmraid.db.exists('Event Producer', producer_url):
		event_producer = vmraid.get_doc('Event Producer', producer_url)
	...
	response = producer_site.post_api(
		'vmraid.core.doctype.user.user.generate_keys',
	...
	event_producer = vmraid.new_doc('Event Producer')
	event_producer.producer_doctypes = []
