...
class TestEventProducer(unittest.TestCase):
	...
	def test_dynamic_link_dependencies_synced(self):
		...
		self.assertEqual(
			producer_link_doc.name, vmraid.db.get_value("ToDo", producer_doc.name, "reference_name")
		)
	...
	def test_conditional_events_with_cmd(self):
		...
		note_producer_entry.condition = (
			"cmd: vmraid.event_streaming.doctype.event_producer.test_event_producer.can_sync_note"
		)
		event_producer.save()
...
def create_event_producer(producer_url):
	...
	response = producer_site.post_api(
		"vmraid.core.doctype.user.user.generate_keys", params={"user": "Administrator"}
	)
...
def connect():
	def _connect():
		return VMRaidClient(url=producer_url, username="Administrator", password="admin", verify=False)
