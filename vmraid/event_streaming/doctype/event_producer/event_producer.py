...
class EventProducer(Document):
	...
	def on_trash(self):
		last_update = vmraid.db.get_value("Event Producer Last Update", dict(event_producer=self.name))
		if last_update:
			vmraid.delete_doc("Event Producer Last Update", last_update)
	...
	def set_last_update(self, last_update):
		last_update_doc_name = vmraid.db.get_value(
...
def get_config(event_config):
	...
	for entry in event_config:
		if entry.status == "Approved":
			if entry.has_mapping:
				(mapped_doctype, mapping) = vmraid.db.get_value(
