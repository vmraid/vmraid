...
class EventConsumer(Document):
	def validate(self):
		...
		# vmraid.flags.in_test won't work here as tests are running on the consumer site
	...
	def on_update(self):
		if not self.incoming_change:
			...
			vmraid.db.set_value(self.doctype, self.name, 'incoming_change', 0)
		
		vmraid.cache().delete_value('event_consumer_document_type_map')

	def on_trash(self):
		for i in vmraid.get_all('Event Update Log Consumer', {'consumer': self.name}):
			vmraid.delete_doc('Event Update Log Consumer', i.name)
		vmraid.cache().delete_value('event_consumer_document_type_map')
	...
	def update_consumer_status(self):
		...
		event_producer = vmraid._dict(event_producer)
		...
		for entry in config:
			...
			entry['status'] = vmraid.db.get_value('Event Consumer Document Type', {'parent': self.name, 'ref_doctype': ref_doctype}, 'status')
...
def register_consumer(data):
	...
	if vmraid.db.exists('Event Consumer', data['event_consumer']):
		return None

	user = data['user']
	if not vmraid.db.exists('User', user):
		vmraid.throw(_('User {0} not found on the producer site').format(user))

	if "System Manager" not in vmraid.get_roles(user):
		vmraid.throw(_("Event Subscriber has to be a System Manager."))

	consumer = vmraid.new_doc('Event Consumer')
...
def get_consumer_site(consumer_url):
	"""create a VMRaidClient object for event consumer site"""
	consumer_doc = vmraid.get_doc('Event Consumer', consumer_url)
	consumer_site = VMRaidClient(
		url=consumer_url,
...
def get_last_update():
	...
	updates = vmraid.get_list('Event Update Log', 'creation', ignore_permissions=True, limit=1, order_by='creation desc')
	if updates:
		return updates[0].creation
	return vmraid.utils.now_datetime()


@vmraid.whitelist()
def notify_event_consumers(doctype):
	"""get all event consumers and set flag for notification status"""
	event_consumers = vmraid.get_all('Event Consumer Document Type', ['parent'], {'ref_doctype': doctype, 'status': 'Approved'})
	for entry in event_consumers:
		consumer = vmraid.get_doc('Event Consumer', entry.parent)
...
def notify(consumer):
	...
	if consumer_status == 'online':
		try:
			...
			client.post_request({
				'cmd': 'vmraid.event_streaming.doctype.event_producer.event_producer.new_event_notification',
	...
	if not consumer.flags.notified:
		enqueued_method = 'vmraid.event_streaming.doctype.event_consumer.event_consumer.notify'
		jobs = get_jobs()
		if not jobs or enqueued_method not in jobs[vmraid.local.site] and not consumer.flags.notifed:
			vmraid.enqueue(enqueued_method, queue='long', enqueue_after_commit=True, **{'consumer': consumer})
...
def has_consumer_access(consumer, update_log):
	...
	if isinstance(consumer, str):
		consumer = vmraid.get_doc('Event Consumer', consumer)

	if not vmraid.db.exists(update_log.ref_doctype, update_log.docname):
		# Delete Log
		# Check if the last Update Log of this document was read by this consumer
		last_update_log = vmraid.get_all(
			'Event Update Log',
			filters={
		...
		last_update_log = vmraid.get_doc('Event Update Log', last_update_log[0].name)
		return len([x for x in last_update_log.consumers if x.consumer == consumer.name])

	doc = vmraid.get_doc(update_log.ref_doctype, update_log.docname)
	try:
		for dt_entry in consumer.consumer_doctypes:
			...
			if condition.startswith('cmd:'):
				...
				return vmraid.call(cmd, **args)
			else:
				return vmraid.safe_eval(condition, vmraid._dict(doc=doc))
	except Exception as e:
		vmraid.log_error(title='has_consumer_access error', message=e)
	return False