...
class DocumentTypeMapping(Document):
	...
	def validate_inner_mapping(self):
		meta = vmraid.get_meta(self.local_doctype)
		for field_map in self.field_mapping:
			if field_map.local_fieldname not in default_fields:
				...
				if not field:
					vmraid.throw(_('Row #{0}: Invalid Local Fieldname').format(field_map.idx))
			...
			if fieldtype in ['Link', 'Dynamic Link', 'Table']:
				if not field_map.mapping and not field_map.default_value:
					msg = _('Row #{0}: Please set Mapping or Default Value for the field {1} since its a dependency field').format(
						field_map.idx, vmraid.bold(field_map.local_fieldname))
					vmraid.throw(msg, title='Inner Mapping Missing')
			...
			if field_map.mapping_type == 'Document' and not field_map.remote_value_filters:
				msg = _('Row #{0}: Please set remote value filters for the field {1} to fetch the unique remote dependency document').format(
					field_map.idx, vmraid.bold(field_map.remote_fieldname))
				vmraid.throw(msg, title='Remote Value Filters Missing')
	...
	def get_mapping(self, doc, producer_site, update_type):
		...
		mapping = {'doc': vmraid.as_json(doc)}
		if len(dependencies):
	...
	def get_mapped_update(self, update, producer_site):
		update_diff = vmraid._dict(json.loads(update.data))
		mapping = update_diff
		dependencies = []
		...
		update = {'doc': vmraid.as_json(mapping)}
		if len(dependencies):
			update['dependencies'] = dependencies
		return update

	def get_mapped_dependency(self, mapping, producer_site, doc):
		inner_mapping = vmraid.get_doc('Document Type Mapping', mapping.mapping)
		filters = json.loads(mapping.remote_value_filters)
		for key, value in iteritems(filters):
			if value.startswith('eval:'):
				val = vmraid.safe_eval(value[5:], None, dict(doc=doc))
				filters[key] = val
			if doc.get(value):
	...
	def map_rows_removed(self, update_diff, mapping):
		...
		for key, value in iteritems(update_diff.removed.copy()):
			local_table_name = vmraid.db.get_value('Document Type Field Mapping', {
	...
	def map_rows(self, update_diff, mapping, producer_site, operation):
		...
		for tablename, entries in iteritems(update_diff.get(operation).copy()):
			local_table_name = vmraid.db.get_value('Document Type Field Mapping', {'remote_fieldname': tablename}, 'local_fieldname')
			table_map = vmraid.db.get_value('Document Type Field Mapping', {'local_fieldname': local_table_name, 'parent': self.name}, 'mapping')
			table_map = vmraid.get_doc('Document Type Mapping', table_map)
			docs = []
...
def get_mapped_child_table_docs(child_map, table_entries, producer_site):
	...
	child_map = vmraid.get_doc('Document Type Mapping', child_map)
	mapped_entries = []
	remote_fields = []
