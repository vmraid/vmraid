...
from vmraid.model import no_value_fields, table_fields

class EventUpdateLog(Document):
	def after_insert(self):
		"""Send update notification updates to event consumers
		whenever update log is generated"""
		enqueued_method = 'vmraid.event_streaming.doctype.event_consumer.event_consumer.notify_event_consumers'
		jobs = get_jobs()
		if not jobs or enqueued_method not in jobs[vmraid.local.site]:
			vmraid.enqueue(enqueued_method, doctype=self.ref_doctype, queue='long',
				enqueue_after_commit=True)

def notify_consumers(doc, event):
	'''called via hooks'''
	# make event update log for doctypes having event consumers
	if vmraid.flags.in_install or vmraid.flags.in_migrate:
		return
...
def check_doctype_has_consumers(doctype):
	...
	return vmraid.cache_manager.get_doctype_map('Event Consumer Document Type', doctype,
...
def get_update(old, new, for_child=False):
	...
	out = vmraid._dict(changed={}, added={}, removed={}, row_changed={})
...
def make_event_update_log(doc, update_type):
	...
	if update_type != 'Delete':
		...
		data = vmraid.as_json(doc) if not doc.get('diff') else vmraid.as_json(doc.diff)
	else:
		data = None
...
def is_consumer_uptodate(update_log, consumer):
	...
	prev_log_consumers = vmraid.get_all(
...
def mark_consumer_read(update_log_name, consumer_name):
	...
	"""
	update_log = vmraid.get_doc('Event Update Log', update_log_name)
	if len([x for x in update_log.consumers if x.consumer == consumer_name]):
		return

	vmraid.get_doc(vmraid._dict(
...
def get_unread_update_logs(consumer_name, dt, dn):
	...
	"""
	already_consumed = [x[0] for x in vmraid.db.sql("""
		SELECT
			update_log.name
...
def get_update_logs_for_consumer(event_consumer, doctypes, last_update):
	...
	if isinstance(doctypes, str):
		doctypes = vmraid.parse_json(doctypes)
	
	from vmraid.event_streaming.doctype.event_consumer.event_consumer import has_consumer_access

	consumer = vmraid.get_doc('Event Consumer', event_consumer)
	docs = vmraid.get_list(
			doctype='Event Update Log',
