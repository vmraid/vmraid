		...
		if "run_method" in vmraid.local.form_dict:
			...
			if name:
				...
				if vmraid.local.request.method == "PUT":
					...
					doc = vmraid.get_doc(doctype, name, for_update=True)
					...
					vmraid.local.response.update({"data": doc.save().as_dict()})
...
def validate_oauth(authorization_header):
	...
	try:
		required_scopes = vmraid.db.get_value("OAuth Bearer Token", token, "scopes").split(
			get_url_delimiter()
		)
...
def validate_auth_via_api_keys(authorization_header):
	...
	try:
		...
		vmraid.throw(
			...
			vmraid.InvalidAuthorizationToken,
		)
...
def validate_api_key_secret(api_key, api_secret, vmraid_authorization_source=None):
	...
	doc = vmraid.db.get_value(doctype=doctype, filters={"api_key": api_key}, fieldname=["name"])
	form_dict = vmraid.local.form_dict
	doc_secret = vmraid.utils.password.get_decrypted_password(doctype, doc, fieldname="api_secret")
	if api_secret == doc_secret:
		if doctype == "User":
			user = vmraid.db.get_value(doctype="User", filters={"api_key": api_key}, fieldname=["name"])
		else:
