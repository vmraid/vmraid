function setup_list_filters() {
	...
	vmraid.call('vmraid.website.doctype.web_form.web_form.get_form_data', {
	...
	.then((r) => {
		...
		web_form_filters = web_form.web_form_fields.filter(df => {
			...
			const value = vmraid.utils.get_query_params()[df.fieldname];
			const f = vmraid.ui.form.make_control({
				df: {
					...
					is_filter: true,
					change: (e) => {
						const query_params = Object.assign(vmraid.utils.get_query_params(), {
							[df.fieldname]: f.get_value()
						})
						const query_string = vmraid.utils.make_query_string(query_params);
...
vmraid.ready(setup_list_filters);
