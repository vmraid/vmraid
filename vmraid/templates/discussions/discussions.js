...
const setup_socket_io = () => {
	const assets = [
		...
		"/assets/vmraid/js/vmraid/socketio_client.js"
	];

	vmraid.require(assets, () => {
		if (window.dev_server) {
			vmraid.boot.socketio_port = "9000";
		}

		vmraid.socketio.init(9000);
		vmraid.socketio.socket.on("publish_message", (data) => {
			publish_message(data);
		});
		vmraid.socketio.socket.on("update_message", (data) => {
			update_message(data);
		});
		vmraid.socketio.socket.on("delete_message", (data) => {
			delete_message(data);
		});
	});
};
...
const publish_message = (data) => {
	...
	if ($(`.discussion-on-page[data-topic=${topic.name}]`).length) {
		...
		if (topic.owner == vmraid.session.user) {
	...
	} else if (topic.owner == vmraid.session.user && document_match_found) {
...
const submit_discussion = (e) => {
	...
	if (reply) {
		...
		vmraid.call({
			method: "vmraid.website.doctype.discussion_topic.discussion_topic.submit_discussion",
			args: {
...
const get_color_from_palette = (element) => {
	const palette = vmraid.get_palette(element.attr("title"));
...
const perform_action = (e) => {
	...
	if (action === "edit") {
		...
		vmraid.call({
			method: "vmraid.website.doctype.discussion_reply.discussion_reply.delete_message",
			args: {
...
const hide_actions_on_conditions = (template, owner) => {
	...
	vmraid.session.user != owner && $template.find(".dropdown").addClass("hide");
	return $template.prop("outerHTML");
};
