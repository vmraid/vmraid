...
from vmraid.utils import cint, fmt_money
import json
from vmraid.integrations.doctype.stripe_settings.stripe_settings import get_gateway_controller
...
def get_context(context):
	...
	if not (set(expected_keys) - set(list(vmraid.form_dict))):
		...
		if is_a_subscription(context.reference_doctype, context.reference_docname):
			payment_plan = vmraid.db.get_value(context.reference_doctype, context.reference_docname, "payment_plan")
			recurrence = vmraid.db.get_value("Payment Plan", payment_plan, "recurrence")
...
def get_api_key(doc, gateway_controller):
	publishable_key = vmraid.db.get_value("Stripe Settings", gateway_controller, "publishable_key")
	if cint(vmraid.form_dict.get("use_sandbox")):
		publishable_key = vmraid.conf.sandbox_publishable_key

	return publishable_key

def get_header_image(doc, gateway_controller):
	header_image = vmraid.db.get_value("Stripe Settings", gateway_controller, "header_img")
...
def make_payment(stripe_token_id, data, reference_doctype=None, reference_docname=None):
	...
	if is_a_subscription(reference_doctype, reference_docname):
		reference = vmraid.get_doc(reference_doctype, reference_docname)
		data =  reference.create_subscription("stripe", gateway_controller, data)
	else:
		data =  vmraid.get_doc("Stripe Settings", gateway_controller).create_request(data)

	vmraid.db.commit()
	return data

def is_a_subscription(reference_doctype, reference_docname):
	if not vmraid.get_meta(reference_doctype).has_field('is_a_subscription'):
		return False
	return vmraid.db.get_value(reference_doctype, reference_docname, "is_a_subscription")