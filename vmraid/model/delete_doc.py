...
from vmraid import get_module_path
from vmraid.model.dynamic_links import get_dynamic_link_map
from vmraid.core.doctype.file.file import remove_all
from vmraid.utils.password import delete_all_passwords_for
from vmraid.model.naming import revert_series_if_last
from vmraid.utils.global_search import delete_for_document
from vmraid.desk.doctype.tag.tag import delete_tags_for_document
from vmraid.exceptions import FileNotFoundError
...
def delete_doc(doctype=None, name=None, force=0, ignore_doctypes=None, for_reload=False, ignore_permissions=False,
	...
	# get from form
	if not doctype:
		doctype = vmraid.form_dict.get('dt')
		name = vmraid.form_dict.get('dn')
	...
	for name in names or []:
		...
		if doctype=="DocType":
			if for_reload:
				...
				vmraid.db.sql("delete from `tabCustom Field` where dt = %s", name)
				vmraid.db.sql("delete from `tabClient Script` where dt = %s", name)
				vmraid.db.sql("delete from `tabProperty Setter` where doc_type = %s", name)
				vmraid.db.sql("delete from `tabReport` where ref_doctype=%s", name)
				vmraid.db.sql("delete from `tabCustom DocPerm` where parent=%s", name)
				vmraid.db.sql("delete from `__global_search` where doctype=%s", name)

			delete_from_table(doctype, name, ignore_doctypes, None)

			if vmraid.conf.developer_mode and not doc.custom and not (
				for_reload
				or vmraid.flags.in_migrate
				or vmraid.flags.in_install
				or vmraid.flags.in_uninstall
			):
				try:
			...
			if not for_reload:
				...
				vmraid.enqueue('vmraid.model.delete_doc.delete_dynamic_links',
					doctype=doc.doctype, name=doc.name,
					is_async=False if vmraid.flags.in_test else True)
		...
		if doc and not for_reload:
			...
			vmraid.defaults.clear_default(parenttype="User Permission", key=doctype, value=name)

def add_to_deleted_document(doc):
	'''Add this document to Deleted Document table. Called after delete'''
	if doc.doctype != 'Deleted Document' and vmraid.flags.in_install != 'vmraid':
		vmraid.get_doc(dict(
...
def delete_from_table(doctype, name, ignore_doctypes, doc):
	if doctype!="DocType" and doctype==name:
		vmraid.db.sql("delete from `tabSingles` where `doctype`=%s", name)
	else:
		vmraid.db.sql("delete from `tab{0}` where `name`=%s".format(doctype), name)
	...
	if doc:
		...
		def get_table_fields(field_doctype):
			...
			return [r[0] for r in vmraid.get_all(field_doctype,
				fields='options',
				filters={
		...
		if not vmraid.flags.in_install=="vmraid":
			tables += get_table_fields("Custom Field")

	# delete from child tables
	for t in list(set(tables)):
		if t not in ignore_doctypes:
			vmraid.db.sql("delete from `tab%s` where parenttype=%s and parent = %s" % (t, '%s', '%s'), (doctype, name))
...
def check_permission_and_not_submitted(doc):
	...
	if (not doc.flags.ignore_permissions
		and vmraid.session.user!="Administrator"
		and (
			not doc.has_permission("delete")
			or (doc.doctype=="DocType" and not doc.custom))):
		vmraid.msgprint(_("User not allowed to delete {0}: {1}")
			.format(doc.doctype, doc.name), raise_exception=vmraid.PermissionError)

	# check if submitted
	if doc.docstatus == 1:
		vmraid.msgprint(_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(_(doc.doctype), doc.name, "<a href='https://docs.erpadda.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>", "</a>"),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	for link_dt, link_field, issingle in link_fields:
		if not issingle:
			for item in vmraid.db.get_values(link_dt, {link_field:doc.name},
			...
			if vmraid.db.get_value(link_dt, None, link_field) == doc.name:
				raise_link_exists_exception(doc, link_dt, link_dt)

def check_if_doc_is_dynamically_linked(doc, method="Delete"):
	'''Raise `vmraid.LinkExistsError` if the document is dynamically linked'''
	for df in get_dynamic_link_map().get(doc.doctype, []):
		...
		meta = vmraid.get_meta(df.parent)
		if meta.issingle:
			...
			for refdoc in vmraid.db.sql("""select `name`, `docstatus` {table} from `tab{parent}` where
...
def raise_link_exists_exception(doc, reference_doctype, reference_docname, row=''):
	...
	vmraid.throw(_('Cannot delete or cancel because {0} {1} is linked with {2} {3} {4}')
		.format(doc.doctype, doc_link, reference_doctype, reference_link, row), vmraid.LinkExistsError)
...
def delete_references(doctype, reference_doctype, reference_name,
	...
	vmraid.db.sql('''delete from `tab{0}`
...
def clear_timeline_references(link_doctype, link_name):
	vmraid.db.sql("""DELETE FROM `tabCommunication Link`
...
def delete_controllers(doctype, module):
	...
	dir_path = os.path.join(module_path, 'doctype', vmraid.scrub(doctype))
