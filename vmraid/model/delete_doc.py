...
from vmraid.utils.file_manager import remove_all
	...
	for name in names or []:
		...
		if doctype == "DocType":
			if for_reload:
				...
				vmraid.db.delete("Custom Field", {"dt": name})
				vmraid.db.delete("Client Script", {"dt": name})
				vmraid.db.delete("Property Setter", {"doc_type": name})
				vmraid.db.delete("Report", {"ref_doctype": name})
				vmraid.db.delete("Custom DocPerm", {"parent": name})
				vmraid.db.delete("__global_search", {"doctype": name})
			...
			if (
				...
				and not doc.custom
				and not (
					for_reload or vmraid.flags.in_migrate or vmraid.flags.in_install or vmraid.flags.in_uninstall
				)
			):
				try:
			...
			if not for_reload:
				...
				vmraid.enqueue(
					"vmraid.model.delete_doc.delete_dynamic_links",
					doctype=doc.doctype,
					name=doc.name,
...
def delete_from_table(doctype, name, ignore_doctypes, doc):
	if doctype != "DocType" and doctype == name:
		vmraid.db.delete("Singles", {"doctype": name})
	else:
		vmraid.db.delete(doctype, {"name": name})
	# get child tables
	if doc:
		...
		def get_table_fields(field_doctype):
			...
			return [
				r[0]
				for r in vmraid.get_all(
					field_doctype,
					fields="options",
					filters={"fieldtype": ["in", vmraid.model.table_fields], "parent": doctype},
					as_list=1,
				)
			]
	...
	for t in list(set(tables)):
		if t not in ignore_doctypes:
			vmraid.db.delete(t, {"parenttype": doctype, "parent": name})
...
def check_permission_and_not_submitted(doc):
	...
	if (
		...
		vmraid.msgprint(
			...
			raise_exception=vmraid.PermissionError,
		)
	...
	if doc.docstatus.is_submitted():
		vmraid.msgprint(
			_("{0} {1}: Submitted Record cannot be deleted. You must {2} Cancel {3} it first.").format(
				_(doc.doctype),
				doc.name,
				"<a href='https://docs.erpadda.com//docs/user/manual/en/setting-up/articles/delete-submitted-document' target='_blank'>",
				"</a>",
			),
...
def check_if_doc_is_linked(doc, method="Delete"):
	...
	for lf in link_fields:
		...
		if not issingle:
			...
			if vmraid.get_meta(link_dt).istable:
				fields.extend(["parent", "parenttype"])

			for item in vmraid.db.get_values(link_dt, {link_field: doc.name}, fields, as_dict=True):
...
def clear_timeline_references(link_doctype, link_name):
	vmraid.db.delete("Communication Link", {"link_doctype": link_doctype, "link_name": link_name})
