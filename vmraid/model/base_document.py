...
from vmraid.model import default_fields, table_fields
...
from vmraid.model import display_fieldtypes
from vmraid.utils import (cint, flt, now, cstr, strip_html,
...
def get_controller(doctype):
	...
	def _get_controller():
		...
		if custom:
			if vmraid.db.field_exists("DocType", "is_tree"):
				is_tree = vmraid.db.get_value("DocType", doctype, "is_tree", cache=True)
			else:
				is_tree = False
...
class BaseDocument(object):
	...
	def db_insert(self):
		...
		columns = list(d)
		try:
			vmraid.db.sql("""INSERT INTO `tab{doctype}` ({columns})
			...
			if vmraid.db.is_primary_key_violation(e):
				...
				vmraid.msgprint(_("{0} {1} already exists").format(self.doctype, vmraid.bold(self.name)), title=_("Duplicate Name"), indicator="red")
	...
	def _get_missing_mandatory_fields(self):
		...
		def get_msg(df):
			if df.fieldtype in table_fields:
				...
				return "{}: {} {} #{}: {}: {}".format(_("Error"), vmraid.bold(_(self.doctype)),
			...
			if docname:
				...
				if not vmraid.get_meta(doctype).get('is_virtual'):
					if not fields_to_fetch:
						...
						values = vmraid._dict(name=vmraid.db.get_value(doctype, docname,
							'name', cache=True))
					else:
						...
						values = vmraid.db.get_value(doctype, docname,
				...
				if values:
					...
					if not values.name:
						...
						and (is_submittable or self.meta.is_submittable) and vmraid.get_meta(doctype).is_submittable
						and cint(vmraid.db.get_value(doctype, docname, "docstatus"))==2):
	...
	def set_fetch_from_value(self, doctype, df, values):
		...
		if df.fieldtype in ['Small Text', 'Text', 'Data']:
			if fetch_from_fieldname in default_fields:
				...
				fetch_from_df = vmraid.get_meta(doctype).get_field(fetch_from_fieldname)
	...
	def _validate_selects(self):
		...
		for df in self.meta.get_select_fields():
			...
			if value not in options and not (vmraid.flags.in_test and value.startswith("_T-")):
				...
				vmraid.throw(_('{0} {1} cannot be "{2}". It should be one of "{3}"').format(prefix, label,
					value, comma_options))
	...
	def _validate_constants(self):
		...
		for fieldname in constants:
			...
			if self.get(fieldname) != value:
				vmraid.throw(_("Value cannot be changed for {0}").format(self.meta.get_label(fieldname)),
	...
	def throw_length_exceeded_error(self, df, max_length, value):
		...
		vmraid.throw(_("{0}: '{1}' ({3}) will get truncated, as max characters allowed is {2}")\
			.format(reference, _(df.label), max_length, value), vmraid.CharacterLengthExceededError, title=_('Value too big'))
		...
		for key in self.as_dict():
			...
			if df and not df.allow_on_submit and (self.get(key) or db_value):
				...
				if self_value != db_value:
					vmraid.throw(_("Not allowed to change {0} after submission").format(df.label),
	...
	def _save_passwords(self):
		...
		from vmraid.utils.password import set_encrypted_password, remove_encrypted_password
	...
	def get_formatted(self, fieldname, doc=None, currency=None, absolute_value=False, translated=False):
		...
		if not currency and df:
			...
			if not vmraid.db.exists('Currency', currency, cache=True):
				currency = None
