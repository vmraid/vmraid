...
from vmraid.query_builder.utils import Column
...
class DatabaseQuery(object):
	...
	def execute(
		...
		if (
			...
			and not vmraid.has_permission(self.doctype, "select", user=user, parent_doctype=parent_doctype)
			and not vmraid.has_permission(self.doctype, "read", user=user, parent_doctype=parent_doctype)
		):
			vmraid.flags.error_message = _("Insufficient Permission for {0}").format(
				vmraid.bold(self.doctype)
			)
	...
	def build_and_run(self):
		...
		if vmraid.db.db_type == "postgres" and args.order_by and args.group_by:
	...
	def append_table(self, table_name):
		...
		if not self.flags.ignore_permissions and not vmraid.has_permission(
	...
	def prepare_filter_condition(self, f):
		...
		if f.operator.lower() in (
			...
			fallback = "''"
			value = [vmraid.db.escape((cstr(v.name) or "").strip(), percent=False) for v in result]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			fallback = "''"
			value = [vmraid.db.escape((cstr(v) or "").strip(), percent=False) for v in values]
			if len(value):
				value = f"({', '.join(value)})"
			else:
				value = "('')"
			...
			if isinstance(f.value, Column):
				...
				quote = '"' if vmraid.conf.db_type == "postgres" else "`"
				value = f"{tname}.{quote}{f.value.name}{quote}"

			# escape value
			elif isinstance(value, str) and f.operator.lower() != "between":
				value = f"{vmraid.db.escape(value, percent=False)}"
		...
		if (
			...
			if requires_owner_constraint(role_permissions):
				self.match_conditions.append(
					f"`tab{self.doctype}`.`owner` = {vmraid.db.escape(self.user, percent=False)}"
				)
	...
	def get_share_condition(self):
		return f"`tab{self.doctype}`.name in ({', '.join(vmraid.db.escape(s, percent=False) for s in self.shared)})"
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				...
				if docs:
					values = ", ".join(vmraid.db.escape(doc, percent=False) for doc in docs)
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not (
			vmraid.db.exists("DocField", {"parent": parent, "options": child_doctype})
			or vmraid.db.exists("Custom Field", {"dt": parent, "options": child_doctype})
		):
...
def get_between_date_filter(value, df=None):
	...
	if df and df.fieldtype == "Datetime":
		...
		data = "'%s' AND '%s'" % (vmraid.db.format_date(from_date), vmraid.db.format_date(to_date))
