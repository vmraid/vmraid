...
import vmraid, json, copy, re
from vmraid.model import optional_fields
from vmraid.model.utils.user_settings import get_user_settings, update_user_settings
from vmraid.utils import flt, cint, get_time, make_filter_tuple, get_filter, add_to_date, cstr, get_timespan_date_range
...
class DatabaseQuery(object):
	def __init__(self, doctype, user=None):
		...
		self.or_conditions = []
		self.fields = None
		self.user = user or vmraid.session.user
	...
	def execute(self, fields=None, filters=None, or_filters=None,
		...
		if not ignore_permissions and \
			not vmraid.has_permission(self.doctype, "select", user=user) and \
			not vmraid.has_permission(self.doctype, "read", user=user):

			vmraid.flags.error_message = _('Insufficient Permission for {0}').format(vmraid.bold(self.doctype))
			raise vmraid.PermissionError(self.doctype)
	...
	def build_and_run(self):
		...
		if self.return_query:
			...
			return vmraid.db.sql(query, as_dict=not self.as_list, debug=self.debug,
	...
	def sanitize_fields(self):
		...
		def _raise_exception():
			vmraid.throw(_('Use of sub-query or function is restricted'), vmraid.DataError)
		...
		for field in self.fields:
			...
			if self.strict:
				if re.compile(r".*/\*.*").match(field):
					vmraid.throw(_('Illegal SQL Query'))
	...
	def append_table(self, table_name):
		...
		ptype = 'select' if vmraid.only_has_select_perm(doctype) else 'read'

		if (not self.flags.ignore_permissions) and\
			 (not vmraid.has_permission(doctype, ptype=ptype)):
			vmraid.flags.error_message = _('Insufficient Permission for {0}').format(vmraid.bold(doctype))
			raise vmraid.PermissionError(doctype)
	...
	def prepare_filter_condition(self, f):
		...
		from vmraid.boot import get_additional_filters_from_hooks
		...
		if f.operator.lower() in ('ancestors of', 'descendants of', 'not ancestors of', 'not descendants of'):
			...
			if vmraid.get_meta(f.doctype).get_field(f.fieldname) is not None :
				ref_doctype = vmraid.get_meta(f.doctype).get_field(f.fieldname).options

			result=[]

			lft, rgt = '', ''
			if f.value:
				lft, rgt = vmraid.db.get_value(ref_doctype, f.value, ["lft", "rgt"])

			# Get descendants elements of a DocType with a tree structure
			if f.operator.lower() in ('descendants of', 'not descendants of') :
				result = vmraid.get_all(ref_doctype, filters={
					'lft': ['>', lft],
					'rgt': ['<', rgt]
				}, order_by='`lft` ASC')
			else :
			...
			fallback = "''"
			value = [vmraid.db.escape((v.name or '').strip(), percent=False) for v in result]
			if len(value):
			...
			fallback = "''"
			value = [vmraid.db.escape((v or '').strip(), percent=False) for v in values]
			if len(value):
				value = "({0})".format(", ".join(value))
			else:
				value = "('')"
		else:
			df = vmraid.get_meta(f.doctype).get("fields", {"fieldname": f.fieldname})
			...
			if f.operator in ('>', '<') and (f.fieldname in ('creation', 'modified')):
				...
				value = vmraid.db.format_date(f.value)
				fallback = "'0001-01-01'"

			elif (df and df.fieldtype=="Datetime") or isinstance(f.value, datetime):
				value = vmraid.db.format_datetime(f.value)
				fallback = "'0001-01-01 00:00:00'"
			...
			if isinstance(value, string_types) and not f.operator.lower() == 'between':
				value = "{0}".format(vmraid.db.escape(value, percent=False))

		if (self.ignore_ifnull
			or not can_be_null
			or (f.value and f.operator.lower() in ('=', 'like'))
			or 'ifnull(' in column_name.lower()):
			if f.operator.lower() == 'like' and vmraid.conf.get('db_type') == 'postgres':
				f.operator = 'ilike'
	...
	def build_match_conditions(self, as_condition=True):
		...
		role_permissions = vmraid.permissions.get_role_permissions(meta, user=self.user)
		self.shared = vmraid.share.get_shared(self.doctype, self.user)
		...
		if (not meta.istable and
			...
			only_if_shared = True
			if not self.shared:
				vmraid.throw(_("No permission to read {0}").format(self.doctype), vmraid.PermissionError)
			else:
	...
	def get_share_condition(self):
		return """`tab{0}`.name in ({1})""".format(self.doctype, ", ".join(["%s"] * len(self.shared))) % \
			tuple([vmraid.db.escape(s, percent=False) for s in self.shared])
	...
	def add_user_permissions(self, user_permissions):
		...
		for df in doctype_link_fields:
			...
			if user_permission_values:
				docs = []
				if vmraid.get_system_settings("apply_strict_user_permissions"):
					condition = ""
				else:
				...
				if docs:
					condition += "`tab{doctype}`.`{fieldname}` in ({values})".format(
						...
						values=", ".join(
							[(vmraid.db.escape(doc, percent=False)) for doc in docs])
						)
	...
	def get_permission_query_conditions(self):
		conditions = []
		condition_methods = vmraid.get_hooks("permission_query_conditions", {}).get(self.doctype, [])
		if condition_methods:
			for method in condition_methods:
				c = vmraid.call(vmraid.get_attr(method), self.user)
				if c:
					conditions.append(c)

		permision_script_name = get_server_script_map().get("permission_query", {}).get(self.doctype)
		if permision_script_name:
			script = vmraid.get_doc("Server Script", permision_script_name)
	...
	def validate_order_by_and_group_by(self, parameters):
		...
		if 'select' in _lower and 'from' in _lower:
			vmraid.throw(_('Cannot use sub-query in order by'))
		...
		for field in parameters.split(","):
			if "." in field and field.strip().startswith("`tab"):
				...
				if tbl not in self.tables:
					if tbl.startswith('`'):
						tbl = tbl[4:-1]
					vmraid.throw(_("Please select atleast 1 column from {0} to sort/group").format(tbl))
...
def check_parent_permission(parent, child_doctype):
	if parent:
		...
		if child_doctype and not vmraid.db.exists('DocField',
			{'parent': parent, 'options': child_doctype}):
			raise vmraid.PermissionError

		if vmraid.permissions.has_permission(parent):
			return
...
def has_any_user_permission_for_doctype(doctype, user, applicable_for):
	user_permissions = vmraid.permissions.get_user_permissions(user=user)
...
def get_between_date_filter(value, df=None):
	...
	'''
	from_date = vmraid.utils.nowdate()
	to_date = vmraid.utils.nowdate()
	...
	if df and df.fieldtype == 'Datetime':
		data = "'%s' AND '%s'" % (
			vmraid.db.format_datetime(from_date),
			vmraid.db.format_datetime(to_date))
	else:
		data = "'%s' AND '%s'" % (
			vmraid.db.format_date(from_date),
			vmraid.db.format_date(to_date))
...
def get_additional_filter_field(additional_filters_config, f, value):
	...
	f = vmraid._dict(vmraid.get_attr(additional_filter['get_field'])())
	if f.query_value:
		for option in f.options:
			option = vmraid._dict(option)
