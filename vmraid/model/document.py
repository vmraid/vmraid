...
import vmraid
import time
from vmraid import _, msgprint, is_whitelisted
from vmraid.utils import flt, cstr, now, get_datetime_str, file_lock, date_diff
from vmraid.model.base_document import BaseDocument, get_controller
from vmraid.model.naming import set_new_name
from six import iteritems, string_types
from werkzeug.exceptions import NotFound, Forbidden
import hashlib, json
from vmraid.model import optional_fields, table_fields
from vmraid.model.workflow import validate_workflow
from vmraid.model.workflow import set_workflow_state_on_action
from vmraid.utils.global_search import update_global_search
from vmraid.integrations.doctype.webhook import run_webhooks
from vmraid.desk.form.document_follow import follow_document
from vmraid.desk.utils import slug
from vmraid.core.doctype.server_script.server_script_utils import run_server_script_for_doc_event

# once_only validation
# methods

def get_doc(*args, **kwargs):
	"""returns a vmraid.model.Document object.
...
class Document(BaseDocument):
		...
		if args and args[0] and isinstance(args[0], string_types):
			...
			if len(args)==1:
				...
				if isinstance(args[1], dict):
					# filter
					self.name = vmraid.db.get_value(args[0], args[1], "name")
					if self.name is None:
						vmraid.throw(_("{0} {1} not found").format(_(args[0]), args[1]),
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = vmraid.db.get_singles_dict(self.doctype)
			if not single_doc:
				single_doc = vmraid.new_doc(self.doctype).as_dict()
			...
			d = vmraid.db.get_value(self.doctype, self.name, "*", as_dict=1, for_update=self.flags.for_update)
			if not d:
				vmraid.throw(_("{0} {1} not found").format(_(self.doctype), self.name), vmraid.DoesNotExistError)

			super(Document, self).__init__(d)

		if self.name=="DocType" and self.doctype=="DocType":
			from vmraid.model.meta import DOCTYPE_TABLE_FIELDS
			table_fields = DOCTYPE_TABLE_FIELDS
		else:
			table_fields = self.meta.get_table_fields()

		for df in table_fields:
			children = vmraid.db.get_values(df.options,
	...
	def get_latest(self):
		if not getattr(self, "latest", None):
			self.latest = vmraid.get_doc(self.doctype, self.name)
		return self.latest

	def check_permission(self, permtype='read', permlevel=None):
		"""Raise `vmraid.PermissionError` if not permitted"""
		if not self.has_permission(permtype):
			self.raise_no_permission_to(permlevel or permtype)

	def has_permission(self, permtype="read", verbose=False):
		"""Call `vmraid.has_permission` if `self.flags.ignore_permissions`
		is not set.

		:param permtype: one of `read`, `write`, `submit`, `cancel`, `delete`"""
		if self.flags.ignore_permissions:
			return True
		return vmraid.has_permission(self.doctype, permtype, self, verbose=verbose)

	def raise_no_permission_to(self, perm_type):
		"""Raise `vmraid.PermissionError`."""
		vmraid.flags.error_message = _('Insufficient Permission for {0}').format(self.doctype)
		raise vmraid.PermissionError
	...
	def insert(self, ignore_permissions=None, ignore_links=None, ignore_if_duplicate=False,
		...
		if not (vmraid.flags.in_migrate or vmraid.local.flags.in_install or vmraid.flags.in_setup_wizard):
			follow_document(self.doctype, self.name, vmraid.session.user)
		return self
	...
	def _save(self, ignore_permissions=None, ignore_version=None):
		...
		self.flags.ignore_version = vmraid.flags.in_test if ignore_version is None else ignore_version
	...
	def update_child_table(self, fieldname, df=None):
		...
		if rows:
			...
			deleted_rows = vmraid.db.sql("""select name from `tab{0}` where parent=%s
				and parenttype=%s and parentfield=%s
				and name not in ({1})""".format(df.options, ','.join(['%s'] * len(rows))),
					[self.name, self.doctype, fieldname] + rows)
			if len(deleted_rows) > 0:
				# delete rows that do not match the ones in the document
				vmraid.db.sql("""delete from `tab{0}` where name in ({1})""".format(df.options,
					','.join(['%s'] * len(deleted_rows))), tuple(row[0] for row in deleted_rows))

		else:
			# no rows found, delete all rows
			vmraid.db.sql("""delete from `tab{0}` where parent=%s
	...
	def set_new_name(self, force=False, set_name=None, set_child_names=True):
		"""Calls `vmraid.naming.set_new_name` for parent and child docs."""
	...
	def update_single(self, d):
		...
		vmraid.db.sql("""delete from `tabSingles` where doctype=%s""", self.doctype)
		for field, value in iteritems(d):
			if field != "doctype":
				vmraid.db.sql("""insert into `tabSingles` (doctype, field, value)
					values (%s, %s, %s)""", (self.doctype, field, value))

		if self.doctype in vmraid.db.value_cache:
			del vmraid.db.value_cache[self.doctype]

	def set_user_and_timestamp(self):
		self._original_modified = self.modified
		self.modified = now()
		self.modified_by = vmraid.session.user
		...
		vmraid.flags.currently_saving.append((self.doctype, self.name))
	...
	def _validate_non_negative(self):
		def get_msg(df):
			if self.parentfield:
				return "{} {} #{}: {} {}".format(vmraid.bold(_(self.doctype)),
					_("Row"), self.idx, _("Value cannot be negative for"), vmraid.bold(_(df.label)))
			else:
				return _("Value cannot be negative for {0}: {1}").format(_(df.parent), vmraid.bold(_(df.label)))
		...
		for df in self.meta.get('fields', {'non_negative': ('=', 1),
			...
			if flt(self.get(df.fieldname)) < 0:
				msg = get_msg(df)
				vmraid.throw(msg, vmraid.NonNegativeError, title=_("Negative Value"))
	...
	def validate_set_only_once(self):
		...
		if set_only_once_fields and self._doc_before_save:
			...
			for field in set_only_once_fields:
				...
				if fail:
					vmraid.throw(_("Value cannot be changed for {0}").format(self.meta.get_label(field.fieldname)),
	...
	def apply_fieldlevel_read_permissions(self):
		...
		for table_field in self.meta.get_table_fields():
			all_fields += vmraid.get_meta(table_field.options).fields or []
		...
		for table_field in self.meta.get_table_fields():
			for df in vmraid.get_meta(table_field.options).fields or []:
				if df.permlevel and not df.permlevel in has_access_to:
	...
	def validate_higher_perm_levels(self):
		...
		if self.flags.ignore_permissions or vmraid.flags.in_install:
			return
		...
		for df in self.meta.get_table_fields():
			high_permlevel_fields = vmraid.get_meta(df.options).get_high_permlevel_fields()
			if high_permlevel_fields:
				for d in self.get(df.fieldname):
	...
	def get_permissions(self):
		if self.meta.istable:
			...
			permissions = vmraid.get_meta(self.parenttype).permissions
		else:
	...
	def _set_defaults(self):
		...
		new_doc = vmraid.new_doc(self.doctype, as_dict=True)
		self.update_if_missing(new_doc)

		# children
		for df in self.meta.get_table_fields():
			new_doc = vmraid.new_doc(df.options, as_dict=True)
	...
	def check_if_latest(self):
		...
		if not self.get('__islocal') and not self.meta.get('is_virtual'):
			...
			if conflict:
				vmraid.msgprint(_("Error: Document has been modified after you have opened it") \
				+ (" (%s, %s). " % (modified, self.modified)) \
				+ _("Please refresh to get the latest document."),
					raise_exception=vmraid.TimestampMismatchError)
		else:
	...
	def check_docstatus_transition(self, docstatus):
		...
		if docstatus==0:
			if self.docstatus==0:
				...
				raise vmraid.DocstatusTransitionError(_("Cannot change docstatus from 0 to 2"))
			...
			if self.docstatus==1:
				...
				raise vmraid.DocstatusTransitionError(_("Cannot change docstatus from 1 to 0"))

		elif docstatus==2:
			raise vmraid.ValidationError(_("Cannot edit cancelled document"))
	...
	def _validate_mandatory(self):
		...
		if vmraid.flags.print_messages:
			print(self.as_json().encode("utf-8"))

		raise vmraid.MandatoryError('[{doctype}, {name}]: {fields}'.format(
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			vmraid.throw(_("Could not find {0}").format(msg),
				vmraid.LinkValidationError)

		if cancelled_links:
			msg = ", ".join((each[2] for each in cancelled_links))
			vmraid.throw(_("Cannot link cancelled document: {0}").format(msg),
				vmraid.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (vmraid.flags.in_import and vmraid.flags.mute_emails) or vmraid.flags.in_patch or vmraid.flags.in_install:
			return

		if self.flags.notifications_executed==None:
			self.flags.notifications_executed = []

		from vmraid.email.doctype.notification.notification import evaluate_alert

		if self.flags.notifications == None:
			alerts = vmraid.cache().hget('notifications', self.doctype)
			if alerts==None:
				alerts = vmraid.get_all('Notification', fields=['name', 'event', 'method'],
					filters={'enabled': 1, 'document_type': self.doctype})
				vmraid.cache().hset('notifications', self.doctype, alerts)
	...
	def delete(self, ignore_permissions=False):
		"""Delete document."""
		vmraid.delete_doc(self.doctype, self.name, ignore_permissions = ignore_permissions, flags=self.flags)
	...
	def load_doc_before_save(self):
		...
		if not self.is_new():
			try:
				self._doc_before_save = vmraid.get_doc(self.doctype, self.name)
	...
	def run_post_save_methods(self):
		...
		if (self.doctype, self.name) in vmraid.flags.currently_saving:
			vmraid.flags.currently_saving.remove((self.doctype, self.name))
	...
	def reset_seen(self):
		...
		if getattr(self.meta, 'track_seen', False):
			vmraid.db.set_value(self.doctype, self.name, "_seen", json.dumps([vmraid.session.user]), update_modified=False)

	def notify_update(self):
		"""Publish realtime that the current document is modified"""
		if vmraid.flags.in_patch: return

		vmraid.publish_realtime("doc_update", {"modified": self.modified, "doctype": self.doctype, "name": self.name},
		...
		if not self.meta.get("read_only") and not self.meta.get("issingle") and \
			...
			vmraid.publish_realtime("list_update", data, after_commit=True)
	...
	def db_set(self, fieldname, value=None, update_modified=True, notify=False, commit=False):
		...
		:param commit: default False. run vmraid.db.commit()
		"""
		...
		if update_modified and (self.doctype, self.name) not in vmraid.flags.currently_saving:
			# don't update modified timestamp if called from post save methods
			# like on_update or on_submit
			self.set("modified", now())
			self.set("modified_by", vmraid.session.user)

		self.load_doc_before_save()
		# to trigger notification on value change
		self.run_method('before_change')

		vmraid.db.set_value(self.doctype, self.name, fieldname, value,
	...
	def db_get(self, fieldname):
		...
		return vmraid.db.get_value(self.doctype, self.name, fieldname)

	def check_no_back_links_exist(self):
		"""Check if document links to any active document before Cancel."""
		from vmraid.model.delete_doc import check_if_doc_is_linked, check_if_doc_is_dynamically_linked
	...
	def save_version(self):
		...
		if (not getattr(self.meta, 'track_changes', False)
			...
			or (not self._doc_before_save and vmraid.flags.in_patch)):
			return

		version = vmraid.new_doc('Version')
		if not self._doc_before_save:
			version.for_insert(self)
			version.insert(ignore_permissions=True)
		elif version.set_diff(self._doc_before_save, self):
			version.insert(ignore_permissions=True)
			if not vmraid.flags.in_migrate:
	...
	def validate_value(self, fieldname, condition, val2, doc=None, raise_exception=None):
		...
		if not vmraid.compare(val1, condition, val2):
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			vmraid.throw(_("Table {0} cannot be empty").format(label), raise_exception or vmraid.EmptyTableError)
	...
	def add_comment(self, comment_type='Comment', text=None, comment_email=None, link_doctype=None, link_name=None, comment_by=None):
		...
		out = vmraid.get_doc({
			...
			"comment_email": comment_email or vmraid.session.user,
	...
	def add_seen(self, user=None):
		...
		if self.meta.track_seen:
			...
			_seen = vmraid.parse_json(_seen)

			if user not in _seen:
				_seen.append(user)
				vmraid.db.set_value(self.doctype, self.name, '_seen', json.dumps(_seen), update_modified=False)
	...
	def set_onload(self, key, value):
		if not self.get("__onload"):
			self.set("__onload", vmraid._dict())
		self.get("__onload")[key] = value

	def get_onload(self, key=None):
		if not key:
			return self.get("__onload", vmraid._dict())
	...
	def queue_action(self, action, **kwargs):
		...
		if file_lock.lock_exists(self.get_signature()):
			vmraid.throw(_('This document is currently queued for execution. Please try again'),
				title=_('Document Queued'))

		self.lock()
		enqueue('vmraid.model.document.execute_action', doctype=self.doctype, name=self.name,
			...
			if timeout:
				for i in range(timeout):
					...
					if not file_lock.lock_exists(signature):
						...
						break
			if lock_exists:
				raise vmraid.DocumentLockedError
	...
	def validate_from_to_dates(self, from_date_field, to_date_field):
		...
		if date_diff(self.get(to_date_field), self.get(from_date_field)) < 0:
			vmraid.throw(_('{0} must be after {1}').format(
				vmraid.bold(self.meta.get_label(to_date_field)),
				vmraid.bold(self.meta.get_label(from_date_field)),
			), vmraid.exceptions.InvalidDates)

	def get_assigned_users(self):
		assignments = vmraid.get_all('ToDo',
			fields=['owner'],
			filters={
	...
	def add_tag(self, tag):
		...
		from vmraid.desk.doctype.tag.tag import DocTags
...
def execute_action(doctype, name, action, **kwargs):
	...
	try:
		...
		if vmraid.local.message_log:
			msg = json.loads(vmraid.local.message_log[-1]).get('message')
		else:
			msg = '<pre><code>' + vmraid.get_traceback() + '</pre></code>'
