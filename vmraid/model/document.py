...
import vmraid
from vmraid import _, is_whitelisted, msgprint
...
from vmraid.model.naming import set_new_name, validate_name
from vmraid.model.workflow import set_workflow_state_on_action, validate_workflow
from vmraid.utils import cstr, date_diff, file_lock, flt, get_datetime_str, now
from vmraid.utils.data import get_absolute_url
...
class Document(BaseDocument):
	...
	def load_from_db(self):
		...
		if not getattr(self, "_metaclass", False) and self.meta.issingle:
			single_doc = vmraid.db.get_singles_dict(self.doctype, for_update=self.flags.for_update)
			if not single_doc:
				single_doc = vmraid.new_doc(self.doctype, as_dict=True)
			...
			if not d:
				vmraid.throw(
					_("{0} {1} not found").format(_(self.doctype), self.name), vmraid.DoesNotExistError
				)
	...
	def has_permission(self, permtype="read", verbose=False) -> bool:
		...
		if self.flags.ignore_permissions:
			return True
		return vmraid.permissions.has_permission(self.doctype, permtype, self, verbose=verbose)
	...
	def insert(
		...
		if not (
			vmraid.flags.in_migrate or vmraid.local.flags.in_install or vmraid.flags.in_setup_wizard
		):
			if vmraid.get_cached_value("User", vmraid.session.user, "follow_created_documents"):
	...
	def update_child_table(self, fieldname, df=None):
		...
		if rows:
			...
			if len(deleted_rows) > 0:
				...
				vmraid.db.delete(df.options, {"name": ("in", tuple(row[0] for row in deleted_rows))})
	...
	def update_single(self, d):
		...
		vmraid.db.delete("Singles", {"doctype": self.doctype})
	...
	def set_user_and_timestamp(self):
		...
		if self.is_new() and not (
			vmraid.flags.in_install or vmraid.flags.in_patch or vmraid.flags.in_migrate
		):
		...
		def get_msg(df):
			if self.get("parentfield"):
				return "{} {} #{}: {} {}".format(
					...
					vmraid.bold(_(df.label)),
				)
			else:
				return _("Value cannot be negative for {0}: {1}").format(
					_(df.parent), vmraid.bold(_(df.label))
				)
	...
	def validate_workflow(self):
		...
		if vmraid.flags.in_install == "vmraid":
			return
				...
				if fail:
					vmraid.throw(
						_("Value cannot be changed for {0}").format(
							vmraid.bold(self.meta.get_label(field.fieldname))
						),
						exc=vmraid.CannotChangeConstantError,
					)
	...
	def check_docstatus_transition(self, to_docstatus):
		...
		if to_docstatus == DocStatus.draft():
			if self.docstatus.is_draft():
				...
				raise vmraid.DocstatusTransitionError(
					_("Cannot change docstatus from 0 (Draft) to 2 (Cancelled)")
				)
			else:
				raise vmraid.ValidationError(_("Invalid docstatus"), self.docstatus)
	...
	def _validate_links(self):
		...
		if invalid_links:
			...
			vmraid.throw(_("Could not find {0}").format(msg), vmraid.LinkValidationError)

		if cancelled_links:
			msg = ", ".join((each[2] for each in cancelled_links))
			vmraid.throw(_("Cannot link cancelled document: {0}").format(msg), vmraid.CancelledLinkError)
	...
	def run_notifications(self, method):
		...
		if (
			(vmraid.flags.in_import and vmraid.flags.mute_emails)
	...
	def reset_seen(self):
		...
		if getattr(self.meta, "track_seen", False):
			vmraid.db.set_value(
				self.doctype, self.name, "_seen", json.dumps([vmraid.session.user]), update_modified=False
			)
	...
	def notify_update(self):
		...
		if (
			...
			data = {"doctype": self.doctype, "name": self.name, "user": vmraid.session.user}
	...
	def check_no_back_links_exist(self):
		...
		from vmraid.model.delete_doc import check_if_doc_is_dynamically_linked, check_if_doc_is_linked
	...
	def validate_table_has_rows(self, parentfield, raise_exception=None):
		...
		if not (isinstance(self.get(parentfield), list) and len(self.get(parentfield)) > 0):
			...
			vmraid.throw(
				_("Table {0} cannot be empty").format(label), raise_exception or vmraid.EmptyTableError
			)
	...
	def queue_action(self, action, **kwargs):
		...
		self.lock()
		enqueue(
			"vmraid.model.document.execute_action",
			doctype=self.doctype,
			name=self.name,
			action=action,
			**kwargs,
