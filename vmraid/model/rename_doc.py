...
def update_document_title(
	...
	**kwargs,
	...
	# TODO: omit this after runtime type checking (ref: https://github.com/vmraid/vmraid/pull/14927)
	for obj in [docname, updated_title, updated_name]:
		if not isinstance(obj, (str, type(None))):
			vmraid.throw(f"{obj=} must be of type str or None")
	...
	if title_updated:
		try:
			vmraid.db.set_value(doctype, docname, title_field, updated_title)
...
def rename_doc(
	...
	if not vmraid.db.exists(doctype, old):
		vmraid.errprint(_("Failed: {0} to {1} because {0} doesn't exist.").format(old, new))
		return

	if ignore_if_exists and vmraid.db.exists(doctype, new):
		vmraid.errprint(_("Failed: {0} to {1} because {1} already exists.").format(old, new))
		return
	...
	if merge:
		...
		new_doc.add_comment(
			"Edit", _("renamed from {0} to {1}").format(vmraid.bold(old), vmraid.bold(new))
		)
...
def update_user_settings(old: str, new: str, link_fields: List[Dict]) -> None:
	...
	user_settings_details = vmraid.db.sql(
...
def update_customizations(old: str, new: str) -> None:
	vmraid.db.set_value("Custom DocPerm", {"parent": old}, "parent", new, update_modified=False)
...
def rename_eps_records(doctype: str, old: str, new: str) -> None:
	epl = vmraid.qb.DocType("Energy Point Log")
	(
		vmraid.qb.update(epl)
...
def validate_rename(
	...
	exists = (
		vmraid.qb.from_(doctype).where(Field("name") == new).for_update().select("name").run(pluck=True)
	)
	exists = exists[0] if exists else None

	if merge and not exists:
		vmraid.throw(_("{0} {1} does not exist, select a new target to merge").format(doctype, new))
	...
	if (not merge) and exists:
		vmraid.throw(_("Another {0} with name {1} exists, select another name").format(doctype, new))

	if not (
		ignore_permissions or vmraid.permissions.has_permission(doctype, "write", raise_exception=False)
	):
		vmraid.throw(_("You need write permission to rename"))

	if not (force or ignore_permissions) and not meta.allow_rename:
		vmraid.throw(_("{0} not allowed to be renamed").format(_(doctype)))
...
def update_link_field_values(link_fields: List[Dict], old: str, new: str, doctype: str) -> None:
	for field in link_fields:
		if field["issingle"]:
			...
			vmraid.db.set_value(parent, {docfield: old}, docfield, new, update_modified=False)
...
def get_link_fields(doctype: str) -> List[Dict]:
	...
	if doctype not in vmraid.flags.link_fields:
...
def update_parenttype_values(old: str, new: str):
	child_doctypes = vmraid.db.get_all(
		"DocField",
		fields=["options", "fieldname"],
		filters={"parent": new, "fieldtype": ["in", vmraid.model.table_fields]},
	)

	custom_child_doctypes = vmraid.db.get_all(
		"Custom Field",
		fields=["options", "fieldname"],
		filters={"dt": new, "fieldtype": ["in", vmraid.model.table_fields]},
	)
