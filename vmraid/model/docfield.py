...
def rename(doctype, fieldname, newname):
	"""rename docfield"""
	df = vmraid.db.sql("""select * from tabDocField where parent=%s and fieldname=%s""",
	...
	if vmraid.db.get_value('DocType', doctype, 'issingle'):
...
def update_single(f, new):
	...
	vmraid.db.begin()
	vmraid.db.sql("""update tabSingles set field=%s where doctype=%s and field=%s""",
...
def update_parent_field(f, new):
	...
	if f['fieldtype'] in vmraid.model.table_fields:
		vmraid.db.begin()
		vmraid.db.sql("""update `tab%s` set parentfield=%s where parentfield=%s""" \
			% (f['options'], '%s', '%s'), (new, f['fieldname']))
		vmraid.db.commit()

def get_change_column_query(f, new):
	"""generate change fieldname query"""
	desc = vmraid.db.sql("desc `tab%s`" % f['parent'])
	for d in desc:
		if d[0]== f['fieldname']:
