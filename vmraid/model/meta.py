...
from vmraid.utils import cstr, cint, cast_fieldtype
from vmraid.model import default_fields, no_value_fields, optional_fields, data_fieldtypes, table_fields
...
class Meta(Document):
	...
	def apply_property_setters(self):
		...
		property_setters = vmraid.db.sql("""select * from `tabProperty Setter` where
	...
	def add_custom_links_and_actions(self):
		for doctype, fieldname in (('DocType Link', 'links'), ('DocType Action', 'actions')):
			...
			for d in vmraid.get_all(doctype, fields='*', filters=dict(parent=self.name, custom=1), ignore_ddl=True):
	...
	def set_custom_permissions(self):
		...
		if not self.istable and self.name not in ('DocType', 'DocField', 'DocPerm',
			'Custom DocPerm'):
			custom_perms = vmraid.get_all('Custom DocPerm', fields='*',
	...
	def get_web_template(self, suffix=''):
		...
		template_path = vmraid.get_module_path(module_name, 'doctype',
...
def get_parent_dt(dt):
	parent_dt = vmraid.db.get_all('DocField', 'parent', dict(fieldtype=['in', vmraid.model.table_fields], options=dt), limit=1)
...
def get_field_currency(df, doc=None):
	...
	if not (vmraid.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or
		(doc.parent and vmraid.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname))):
		...
		if currency:
			vmraid.local.field_currency.setdefault((doc.doctype, ref_docname), vmraid._dict())\
...
def trim_tables(doctype=None):
	...
	for doctype in vmraid.db.get_all("DocType", filters=filters):
		doctype = doctype.name
		columns = vmraid.db.get_table_columns(doctype)
		fields = vmraid.get_meta(doctype).get_fieldnames_with_value()
