...
from vmraid.utils import cast, cint, cstr


def get_meta(doctype, cached=True):
	if cached:
		if not vmraid.local.meta_cache.get(doctype):
			meta = vmraid.cache().hget("meta", doctype)
			if meta:
				meta = Meta(meta)
			else:
				meta = Meta(doctype)
				vmraid.cache().hset("meta", doctype, meta.as_dict())
			vmraid.local.meta_cache[doctype] = meta

		return vmraid.local.meta_cache[doctype]
	else:
...
def get_table_columns(doctype):
	return vmraid.db.get_table_columns(doctype)


def load_doctype_from_file(doctype):
	fname = vmraid.scrub(doctype)
	with open(vmraid.get_app_path("vmraid", "core", "doctype", fname, fname + ".json"), "r") as f:
...
class Meta(Document):
	...
	standard_set_once_fields = [
		vmraid._dict(fieldname="creation", fieldtype="Datetime"),
		vmraid._dict(fieldname="owner", fieldtype="Data"),
	]
	...
	def get_global_search_fields(self):
		...
		if getattr(self, "show_name_in_global_search", None):
			fields.append(vmraid._dict(fieldtype="Data", fieldname="name", label="Name"))

		return fields

	def get_valid_columns(self):
		if not hasattr(self, "_valid_columns"):
			table_exists = vmraid.db.table_exists(self.name)
	...
	def add_custom_fields(self):
		if not vmraid.db.table_exists("Custom Field"):
			return

		custom_fields = vmraid.db.sql(
			"""
	...
	def apply_property_setters(self):
		...
		"""
		if not vmraid.db.table_exists("Property Setter"):
			return

		property_setters = vmraid.db.sql(
	...
	def get_fields_to_check_permissions(self, user_permission_doctypes):
		...
		if self.name in user_permission_doctypes:
			fields.append(vmraid._dict({"label": "Name", "fieldname": "name", "options": self.name}))
	...
	def get_permissions(self, parenttype=None):
		if self.istable and parenttype:
			...
			permissions = vmraid.get_meta(parenttype).permissions
		else:
	...
	def get_dashboard_data(self):
		...
		implemented in other VMRaid applications via hooks.
		"""
		data = vmraid._dict()
		if not self.custom:
			try:
				...
				if hasattr(module, "get_data"):
					data = vmraid._dict(module.get_data())
			except ImportError:
				pass
		...
		if not self.custom:
			for hook in vmraid.get_hooks("override_doctype_dashboards", {}).get(self.name, []):
				data = vmraid._dict(vmraid.get_attr(hook)(data=data))
	...
	def get_web_template(self, suffix=""):
		...
		module_name = vmraid.scrub(self.module)
		doctype = vmraid.scrub(self.name)
		template_path = vmraid.get_module_path(
...
DOCTYPE_TABLE_FIELDS = [
	vmraid._dict({"fieldname": "fields", "options": "DocField"}),
	vmraid._dict({"fieldname": "permissions", "options": "DocPerm"}),
	vmraid._dict({"fieldname": "actions", "options": "DocType Action"}),
	vmraid._dict({"fieldname": "links", "options": "DocType Link"}),
	vmraid._dict({"fieldname": "states", "options": "DocType State"}),
]
...
def is_single(doctype):
	try:
		return vmraid.db.get_value("DocType", doctype, "issingle")
	except IndexError:
		raise Exception("Cannot determine whether %s is single" % doctype)


def get_parent_dt(dt):
	parent_dt = vmraid.db.get_all(
		"DocField", "parent", dict(fieldtype=["in", vmraid.model.table_fields], options=dt), limit=1
	)
	return parent_dt and parent_dt[0].parent or ""


def set_fieldname(field_id, fieldname):
	vmraid.db.set_value("DocField", field_id, "fieldname", fieldname)
...
def get_field_currency(df, doc=None):
	...
	if not getattr(vmraid.local, "field_currency", None):
		vmraid.local.field_currency = vmraid._dict()

	if not (
		vmraid.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname)
		or (
			doc.get("parent")
			and vmraid.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
		)
	):
		...
		if ":" in cstr(df.get("options")):
			...
			if len(split_opts) == 3 and doc.get(split_opts[1]):
				currency = vmraid.get_cached_value(split_opts[0], doc.get(split_opts[1]), split_opts[2])
		else:
			...
			if doc.get("parenttype"):
				if currency:
					...
					if vmraid.get_meta(doc.parenttype).has_field(df.get("options")):
						# only get_value if parent has currency field
						currency = vmraid.db.get_value(doc.parenttype, doc.parent, df.get("options"))

		if currency:
			vmraid.local.field_currency.setdefault((doc.doctype, ref_docname), vmraid._dict()).setdefault(
				df.fieldname, currency
			)

	return vmraid.local.field_currency.get((doc.doctype, doc.name), {}).get(df.fieldname) or (
		doc.get("parent")
		and vmraid.local.field_currency.get((doc.doctype, doc.parent), {}).get(df.fieldname)
	)
...
def get_field_precision(df, doc=None, currency=None):
	...
	if df.precision:
		...
		precision = cint(vmraid.db.get_default("currency_precision"))
		if not precision:
			number_format = vmraid.db.get_default("number_format") or "#,###.##"
			decimal_str, comma_str, precision = get_number_format_info(number_format)
	else:
		precision = cint(vmraid.db.get_default("float_precision")) or 3
...
def get_default_df(fieldname):
	if fieldname in (default_fields + child_table_fields):
		if fieldname in ("creation", "modified"):
			return vmraid._dict(fieldname=fieldname, fieldtype="Datetime")

		elif fieldname in ("idx", "docstatus"):
			return vmraid._dict(fieldname=fieldname, fieldtype="Int")

		return vmraid._dict(fieldname=fieldname, fieldtype="Data")
...
def trim_tables(doctype=None, dry_run=False, quiet=False):
	...
	for doctype in vmraid.db.get_all("DocType", filters=filters, pluck="name"):
		try:
...
def trim_table(doctype, dry_run=True):
	vmraid.cache().hdel("table_columns", f"tab{doctype}")
	ignore_fields = default_fields + optional_fields + child_table_fields
	columns = vmraid.db.get_table_columns(doctype)
	fields = vmraid.get_meta(doctype, cached=False).get_fieldnames_with_value()
	...
	if columns_to_remove and not dry_run:
		...
		vmraid.db.sql_ddl(f"ALTER TABLE `tab{doctype}` {columns_to_remove}")
