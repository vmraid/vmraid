...
def get_user_settings(doctype, for_update=False):
	user_settings = vmraid.cache().hget(
		"_user_settings", "{0}::{1}".format(doctype, vmraid.session.user)
	)
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	vmraid.cache().hset(
		"_user_settings", "{0}::{1}".format(doctype, vmraid.session.user), json.dumps(current)
	)


def sync_user_settings():
	"""Sync from cache to database (called asynchronously via the browser)"""
	for key, data in vmraid.cache().hgetall("_user_settings").items():
		key = safe_decode(key)
