...
def get_user_settings(doctype, for_update=False):
	user_settings = vmraid.cache().hget('_user_settings',
		'{0}::{1}'.format(doctype, vmraid.session.user))

	if user_settings is None:
		user_settings = vmraid.db.sql('''select data from `__UserSettings`
			where `user`=%s and `doctype`=%s''', (vmraid.session.user, doctype))
...
def update_user_settings(doctype, user_settings, for_update=False):
	...
	vmraid.cache().hset('_user_settings', '{0}::{1}'.format(doctype, vmraid.session.user),
		json.dumps(current))

def sync_user_settings():
	'''Sync from cache to database (called asynchronously via the browser)'''
	for key, data in iteritems(vmraid.cache().hgetall('_user_settings')):
		key = safe_decode(key)
			...
			if view_settings and view_settings.get("filters"):
				...
				for view_filter in view_filters:
					...
					if view_filter[filter_dict[fieldname]] == old:
						...
						update = True
		if update:
			vmraid.db.sql("update __UserSettings set data=%s where doctype=%s and user=%s",
				(json.dumps(data), user_setting.doctype, user_setting.user))

			# clear that user settings from the redis cache
			vmraid.cache().hset('_user_settings', '{0}::{1}'.format(user_setting.doctype,
