...
from vmraid.utils.password import rename_password_field
from vmraid.model.utils.user_settings import update_user_settings_data, sync_user_settings
...
def rename_field(doctype, old_fieldname, new_fieldname):
	...
	if not meta.issingle and not vmraid.db.has_column(doctype, old_fieldname):
...
def update_reports(doctype, old_fieldname, new_fieldname):
	...
	reports = vmraid.db.sql("""select name, ref_doctype, json from tabReport
	...
	for r in reports:
		...
		if report_dict.get("updated"):
			...
			vmraid.db.sql("""update `tabReport` set `json`=%s where name=%s""", (new_val, r.name))

def update_users_report_view_settings(doctype, ref_fieldname, new_fieldname):
	user_report_cols = vmraid.db.sql("""select defkey, defvalue from `tabDefaultValue` where
...
def update_user_settings(doctype, old_fieldname, new_fieldname):
	...
	user_settings = vmraid.db.sql(''' select user, doctype, data from `__UserSettings`
