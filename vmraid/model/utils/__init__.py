...
def set_default(doc, key):
	...
	if not doc.is_default:
		vmraid.db.set(doc, "is_default", 1)

	vmraid.db.sql("""update `tab%s` set `is_default`=0
...
def set_field_property(filters, key, value):
	...
	docs = [vmraid.get_doc('DocType', d.parent) for d in \
		vmraid.get_all("DocField", fields=['parent'], filters=filters)]
...
class InvalidIncludePath(vmraid.ValidationError): pass
...
def render_include(content):
	...
	for i in range(5):
		if "{% include" in content:
			...
			if not paths:
				vmraid.throw(_('Invalid include path'), InvalidIncludePath)

			for path in paths:
				app, app_path = path.split('/', 1)
				with io.open(vmraid.get_app_path(app, app_path), 'r', encoding = 'utf-8') as f:
					include = f.read()
...
def get_fetch_values(doctype, fieldname, value):
	...
	for df in meta.get_fields_to_fetch(fieldname):
		...
		out[df.fieldname] = vmraid.db.get_value(link_df.options, value, source_fieldname)
