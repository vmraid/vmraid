...
from vmraid.utils import now_datetime, cint, cstr
import re
...
def set_new_name(doc):
	...
	autoname = vmraid.get_meta(doc.doctype).autoname or ""
	...
	doc.name = validate_name(
		doc.doctype,
		doc.name,
		vmraid.get_meta(doc.doctype).get_field("name_case")
	)
...
def set_naming_from_document_naming_rule(doc):
	...
	for d in vmraid.get_all('Document Naming Rule',
...
def getseries(key, digits):
	# series created ?
	current = vmraid.db.sql("SELECT `current` FROM `tabSeries` WHERE `name`=%s FOR UPDATE", (key,))
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	current = vmraid.db.sql("SELECT `current` FROM `tabSeries` WHERE `name`=%s FOR UPDATE", (prefix,))
...
def validate_name(doctype, name, case=None, merge=False):
	...
	if name.startswith("New "+doctype):
		vmraid.throw(_("There were some errors setting the name, please contact the administrator"), vmraid.NameError)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
		name = name.upper()
	...
	if re.findall("[{0}]+".format(special_characters), name):
		...
		vmraid.throw(_("Name cannot contain special characters like {0}").format(message), vmraid.NameError)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	if exists:
		last = vmraid.db.sql("""SELECT `{fieldname}` FROM `tab{doctype}`
			WHERE `{fieldname}` {regex_character} %s
			ORDER BY length({fieldname}) DESC,
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype,
				fieldname=fieldname,
				regex_character=vmraid.db.REGEX_CHARACTER),
			regex)
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		vmraid.throw(_("Name not set via prompt"))
