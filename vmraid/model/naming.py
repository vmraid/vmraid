...
from vmraid.database.sequence import get_next_val, set_next_val
from vmraid.model import log_types
from vmraid.query_builder import DocType
from vmraid.utils import cint, cstr, now_datetime
...
def set_new_name(doc):
	...
	if autoname.lower() != "prompt" and not vmraid.flags.in_import:
		doc.name = None
	...
	if not doc.name and autoname.startswith("field:"):
		...
		vmraid.throw(_("{0} is required").format(doc.meta.get_label(fieldname)))
...
def is_autoincremented(doctype: str, meta: "Meta" = None):
	if doctype in log_types:
		if (
			vmraid.local.autoincremented_status_map.get(vmraid.local.site) is None
			or vmraid.local.autoincremented_status_map[vmraid.local.site] == -1
		):
			if (
				vmraid.db.sql(
				...
				vmraid.local.autoincremented_status_map[vmraid.local.site] = 1
				return True
			else:
				vmraid.local.autoincremented_status_map[vmraid.local.site] = 0

		elif vmraid.local.autoincremented_status_map[vmraid.local.site]:
			return True
...
def set_naming_from_document_naming_rule(doc):
	...
	for d in vmraid.get_all(
		...
		vmraid.get_cached_doc("Document Naming Rule", d.name).apply(doc)
		if doc.name:
			break
...
def set_name_by_naming_series(doc):
	...
	if not doc.naming_series:
		vmraid.throw(vmraid._("Naming Series mandatory"))
...
def make_autoname(key="", doctype="", doc=""):
	...
	*Example:*
	...
	"""
	if key == "hash":
		return vmraid.generate_hash(doctype, 10)
...
def parse_naming_series(parts, doctype="", doc=""):
	...
	for e in parts:
		...
		if e.startswith("#"):
			...
			part = vmraid.defaults.get_user_default("fiscal_year")
...
def getseries(key, digits):
	# series created ?
	# Using vmraid.qb as vmraid.get_values does not allow order_by=None
	series = DocType("Series")
	current = (vmraid.qb.from_(series).where(series.name == key).for_update().select("current")).run()

	if current and current[0][0] is not None:
		current = current[0][0]
		# yes, update it
		vmraid.db.sql("UPDATE `tabSeries` SET `current` = `current` + 1 WHERE `name`=%s", (key,))
		current = cint(current) + 1
	else:
		# no, create it
		vmraid.db.sql("INSERT INTO `tabSeries` (`name`, `current`) VALUES (%s, 1)", (key,))
		current = 1
...
def revert_series_if_last(key, name, doc=None):
	...
	*Example:*
	...
	series = DocType("Series")
	current = (
		vmraid.qb.from_(series).where(series.name == prefix).for_update().select("current")
	).run()

	if current and current[0][0] == count:
		vmraid.db.sql("UPDATE `tabSeries` SET `current` = `current` - 1 WHERE `name`=%s", prefix)


def get_default_naming_series(doctype):
	"""get default value for `naming_series` property"""
	naming_series = vmraid.get_meta(doctype).get_field("naming_series").options or ""
	if naming_series:
...
def validate_name(doctype: str, name: Union[int, str], case: Optional[str] = None):
	if not name:
		vmraid.throw(_("No Name Specified for {0}").format(doctype))
	...
	if isinstance(name, int):
		...
		vmraid.throw(_("Invalid name type (integer) for varchar name column"), vmraid.NameError)

	if name.startswith("New " + doctype):
		vmraid.throw(
			_("There were some errors setting the name, please contact the administrator"), vmraid.NameError
		)
	if case == "Title Case":
		name = name.title()
	if case == "UPPER CASE":
	...
	if not vmraid.get_meta(doctype).get("issingle") and (doctype == name) and (name != "DocType"):
		vmraid.throw(_("Name of {0} cannot be {1}").format(doctype, name), vmraid.NameError)

	special_characters = "<>"
	if re.findall("[{0}]+".format(special_characters), name):
		message = ", ".join("'{0}'".format(c) for c in special_characters)
		vmraid.throw(
			_("Name cannot contain special characters like {0}").format(message), vmraid.NameError
		)
...
def append_number_if_name_exists(doctype, value, fieldname="name", separator="-", filters=None):
	...
	exists = vmraid.db.exists(doctype, filters)
	...
	if exists:
		last = vmraid.db.sql(
			...
			`{fieldname}` DESC LIMIT 1""".format(
				doctype=doctype, fieldname=fieldname, regex_character=vmraid.db.REGEX_CHARACTER
			),
			regex,
		)
...
def _set_amended_name(doc):
	...
	if vmraid.db.get_value(doc.doctype, doc.amended_from, "amended_from"):
...
def _prompt_autoname(autoname, doc):
	...
	if not doc.name:
		vmraid.throw(_("Please set the document name"))
