...
def make_mapped_doc(method, source_name, selected_children=None, args=None):
	...
	for hook in vmraid.get_hooks("override_whitelisted_methods", {}).get(method, []):
	...
	if method not in vmraid.whitelisted:
	...
	if args:
		vmraid.flags.args = vmraid._dict(json.loads(args))

	vmraid.flags.selected_children = selected_children or None
...
def get_mapped_doc(from_doctype, from_docname, table_maps, target_doc=None,
	...
	# main
	if not target_doc:
		target_doc = vmraid.new_doc(table_maps[from_doctype]["doctype"])
	elif isinstance(target_doc, string_types):
		target_doc = vmraid.get_doc(json.loads(target_doc))

	if (not apply_strict_user_permissions
		and not ignore_permissions and not target_doc.has_permission("create")):
		target_doc.raise_no_permission_to("create")

	source_doc = vmraid.get_doc(from_doctype, from_docname)
		...
		for df in source_doc.meta.get_table_fields():
			...
			if table_map:
				for source_d in source_doc.get(df.fieldname):
					...
					if (vmraid.flags.selected_children
						and (df.fieldname in vmraid.flags.selected_children)
						and source_d.name not in vmraid.flags.selected_children[df.fieldname]):
						continue
		...
		for key, condition in table_map["validation"].items():
			if condition[0]=="=":
				if source_doc.get(key) != condition[1]:
					vmraid.throw(_("Cannot map because following condition fails: ")
...
def map_fetch_fields(target_doc, df, no_copy_fields):
	...
	for fetch_df in target_doc.meta.get("fields", {"fetch_from": "^{0}.".format(df.fieldname)}):
		...
		if ((not target_doc.get(fetch_df.fieldname) or fetch_df.fieldtype == "Read Only")
			...
			if not linked_doc:
				try:
					linked_doc = vmraid.get_doc(df.options, target_doc.get(df.fieldname))
				except:
					return
...
def map_child_doc(source_d, target_parent, table_map, source_parent=None):
	...
	target_d = vmraid.new_doc(target_child_doctype, target_parent, target_parentfield)
