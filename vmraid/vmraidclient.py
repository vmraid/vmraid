...
class VMRaidClient(object):
	def __init__(self, url, username=None, password=None, verify=True, api_key=None, api_secret=None, vmraid_authorization_source=None):
		self.headers = {
	...
	def insert(self, doc):
		...
		res = self.session.post(self.url + "/api/resource/" + doc.get("doctype"),
			data={"data":vmraid.as_json(doc)}, verify=self.verify, headers=self.headers)
	...
	def insert_many(self, docs):
		...
		return self.post_request({
			"cmd": "vmraid.client.insert_many",
			"docs": vmraid.as_json(docs)
		})
	...
	def update(self, doc):
		...
		res = self.session.put(url, data={"data":vmraid.as_json(doc)}, verify=self.verify, headers=self.headers)
		return vmraid._dict(self.post_process(res))
	...
	def submit(self, doc):
		...
		return self.post_request({
			"cmd": "vmraid.client.submit",
			"doc": vmraid.as_json(doc)
		})
	...
	def migrate_doctype(self, doctype, filters=None, update=None, verbose=1, exclude=None, preprocess=None):
		...
		for doc in docs:
			...
			if doctype != "User" and not vmraid.db.exists("User", doc.get("owner")):
				vmraid.get_doc({"doctype": "User", "email": doc.get("owner"),
