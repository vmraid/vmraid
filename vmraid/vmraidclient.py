"""
VMRaidClient is a library that helps you connect with other vmraid systems
"""
import base64
import json
...
class VMRaidException(Exception):
	pass


class VMRaidClient(object):
	def __init__(
		self,
		url,
		username=None,
		...
		api_key=None,
		api_secret=None,
		vmraid_authorization_source=None,
	):
		self.headers = {
		...
		self.vmraid_authorization_source = vmraid_authorization_source
	...
	def setup_key_authentication_headers(self):
		if self.api_key and self.api_secret:
			...
			if self.vmraid_authorization_source:
				auth_source = {"VMRaid-Authorization-Source": self.vmraid_authorization_source}
	...
	def insert(self, doc):
		...
		res = self.session.post(
			...
			data={"data": vmraid.as_json(doc)},
			verify=self.verify,
			headers=self.headers,
		)
		return vmraid._dict(self.post_process(res))
	...
	def insert_many(self, docs):
		...
		return self.post_request({"cmd": "vmraid.client.insert_many", "docs": vmraid.as_json(docs)})
	...
	def update(self, doc):
		...
		res = self.session.put(
			url, data={"data": vmraid.as_json(doc)}, verify=self.verify, headers=self.headers
		)
		return vmraid._dict(self.post_process(res))
	...
	def bulk_update(self, docs):
		...
		return self.post_request({"cmd": "vmraid.client.bulk_update", "docs": vmraid.as_json(docs)})
	...
	def delete(self, doctype, name):
		...
		return self.post_request({"cmd": "vmraid.client.delete", "doctype": doctype, "name": name})

	def submit(self, doc):
		"""Submit remote document

		:param doc: dict or Document object to be submitted remotely"""
		return self.post_request({"cmd": "vmraid.client.submit", "doc": vmraid.as_json(doc)})
	...
	def get_value(self, doctype, fieldname=None, filters=None):
		...
		return self.get_request(
			{
				"cmd": "vmraid.client.get_value",
				"doctype": doctype,
				"fieldname": fieldname or "name",
				"filters": vmraid.as_json(filters),
			}
		)
	...
	def cancel(self, doctype, name):
		...
		return self.post_request({"cmd": "vmraid.client.cancel", "doctype": doctype, "name": name})
	...
	def migrate_doctype(
		...
		for doc in docs:
			...
			if doctype != "User" and not vmraid.db.exists("User", doc.get("owner")):
				vmraid.get_doc(
	...
	def migrate_single(self, doctype):
		...
		doc.modified = vmraid.db.get_single_value(doctype, "modified")
		vmraid.get_doc(doc).insert()
	...
	def post_process(self, response):
		...
		if rjson and ("exc" in rjson) and rjson["exc"]:
			try:
				...
				exc = "VMRaidClient Request Failed\n\n" + exc
			except Exception:
				exc = rjson["exc"]

			raise VMRaidException(exc)
		if "message" in rjson:
...
class VMRaidOAuth2Client(VMRaidClient):
