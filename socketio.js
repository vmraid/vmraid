...
io.on('connection', function (socket) {
	...
	// vmraid.chat
	socket.on("vmraid.chat.room:subscribe", function (rooms) {
		if (!Array.isArray(rooms)) {
			rooms = [rooms];
		}

		for (var room of rooms) {
			log('vmraid.chat: Subscribing ' + socket.user + ' to room ' + room);
			room = get_chat_room(socket, room);

			log('vmraid.chat: Subscribing ' + socket.user + ' to event ' + room);
			socket.join(room);
		}
	});

	socket.on("vmraid.chat.message:typing", function (data) {
		const user = data.user;
		const room = get_chat_room(socket, data.room);

		log('vmraid.chat: Dispatching ' + user + ' typing to room ' + room);

		io.to(room).emit('vmraid.chat.room:typing', {
			room: data.room,
			user: user
		});
	});
	...
	let retries = 0;
	let join_chat_room = () => {
		request.get(get_url(socket, '/api/method/vmraid.realtime.get_user_info'))
			.type('form')
			.query({
				sid: sid
			})
...
function get_site_name(socket) {
	if (socket.request.headers['x-vmraid-site-name']) {
		return get_hostname(socket.request.headers['x-vmraid-site-name']);
...
function can_subscribe_doc(args) {
	...
	request.get(get_url(args.socket, '/api/method/vmraid.realtime.can_subscribe_doc'))
		.type('form')
		.query({
			sid: args.sid,
