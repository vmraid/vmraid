...
const html_plugin = require("./vmraid-html");
const rtlcss = require('rtlcss');
const postCssPlugin = require("@vmraid/esbuild-plugin-postcss2").default;
...
const argv = yargs
	...
	.option("skip_vmraid", {
		type: "boolean",
		description: "Skip building vmraid assets"
	})
	.option("files", {
		type: "string",
	...
	.example(
		"node esbuild --apps vmraid,erpadda",
		"Run build only for vmraid and erpadda"
	)
	.example(
		"node esbuild --files vmraid/website.bundle.js,vmraid/desk.bundle.js",
		"Run build only for specified bundles"
	)
	.version(false).argv;

const APPS = (!argv.apps ? app_list : argv.apps.split(",")).filter(
	app => !(argv.skip_vmraid && app == "vmraid")
);
...
function get_files_to_build(files) {
	// files: ['vmraid/website.bundle.js', 'erpadda/main.bundle.js']
	let include_patterns = [];
	let ignore_patterns = [];
...
async function notify_redis({ error, success, changed_files }) {
	...
	if (error) {
		...
		let stack = error.stack.replace(new RegExp(chair_path, "g"), "");
		payload = {
			error,
			formatted,
			stack
